...
multiple fragment outputs), but the features themselves (a stock normal-mapping/PBR shader variant,
real multi-attachment MRT support) remain unimplemented -- no new task is added for them here since
that is separate work, not something `SOKOL-28` itself needed to also deliver.
...
| MRT (`SetRenderTargets` with more than one binding) | ✅ real multiple colour attachments | ⬜ throws `NotYetImplemented` | `SOKOL-26` -- `SOKOL-28`'s custom-effect path is a genuine consumer now (a custom fragment shader can declare more than one `out` colour), but MRT support itself is still not implemented. |
