...
| 1 | `CBIND-041` C consumer documentation and examples | ⬜ | C-only examples that build in CI; the 31 documents under `docs/c-api/` are the material |
| 2 | `CBIND-042` experimental release gate | ⬜ | needs 041 first: it is the decision that everything above is enough |
| 3 | `CBIND-044` close the public API coverage matrix | ⬜ | the final close; every row implemented, or carrying an owner-approved limitation a caller can query |
...
- **Next task:** `CBIND-041`, the C consumer documentation and examples. This is the first task in
  the campaign whose deliverable is aimed at somebody who has never read this repository, and the
  temptation to write a fourth summary of what already exists should be resisted: the 31 documents
  under `docs/c-api/` are the reference, and what is missing is the thing a newcomer actually needs
  — **a C program that compiles, links and runs against an installed CNA**, proving the CMake
  package, the include spelling, the initialization order, the error-retrieval idiom and a clean
  shutdown, all in one file one can copy. The acceptance criterion says every example is C-only and
  builds in CI, so an example that only builds inside this source tree does not satisfy it: the
  `install(EXPORT CNACTargets)` rules in `modules/c-api/CMakeLists.txt` are what a consumer sees,
  and they have never been exercised by a consumer. `build-consumer/` is the shared build directory
  reserved for exactly this, and `docs/c-api/README.md` is where the tour belongs.
