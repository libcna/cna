...
| 1 | `CBIND-042` experimental release gate | ⬜ | the decision that everything above is enough; `docs/c-api/CONSUMING.md` records the two open packaging questions it must rule on |
| 2 | `CBIND-044` close the public API coverage matrix | ⬜ | the final close; every row implemented, or carrying an owner-approved limitation a caller can query |
...
- **Next task:** `CBIND-042`, the experimental release gate. This one is a **decision**, not an
  implementation: it asks whether the B7 matrix, a real C application, the documentation, the
  installability, the absence of an unreviewed ABI break and a known-limitations matrix are all in
  place. Most of that now exists and can be pointed at rather than rebuilt — `COMPATIBILITY.md`,
  `abi_baseline.json`, `hello_cna` and `CApi_InstalledConsumer`, `FUZZING.md`, `COVERAGE.md`. What
  is genuinely missing is the **known-limitations matrix** as a document a reader can act on, and
  two packaging questions that are the owner's to rule on, both recorded at the end of
  `docs/c-api/CONSUMING.md`: whether the package ships SDL3 and FFmpeg beside the library, and
  whether a static configuration is ever offered given that it would export every C++ symbol it
  archived. Do not decide those two alone. ABI 1.0 is explicitly a later, separate decision.
