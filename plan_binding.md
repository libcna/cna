...
| CBIND-040B | Add fuzz targets for the parser-like surfaces | ⬜ | Focused fuzzing for the routes that read caller-supplied bytes: the UTF-8 validator behind every `CNA_StringView`, the buffer/element-count arithmetic, and the versioned-struct prefix rules. Prefer a deterministic differential oracle over a random walk where the input space is small enough to cover exhaustively — an answer that is *proved* for every byte sequence up to a length beats one that is sampled — with a libFuzzer entry point for the spaces that are not. |
...
| 1 | `CBIND-040B` fuzz targets for the parser-like surfaces | ⬜ | the UTF-8 validator behind every `CNA_StringView`, the buffer arithmetic, the versioned-struct prefix rules |
...
- **Next task:** `CBIND-040B`, the fuzz targets for the parser-like surfaces. The subject that
  matters most is `ValidateStringView` in `CnaCApiDetail.cpp` — the UTF-8 validator standing behind
  every `CNA_StringView` the ABI accepts, already rejecting overlongs, surrogates and out-of-range
  lead bytes. It is reachable from C through any route taking a string view and directly from
  `tests/cpp/BoundaryDetailTest.cpp`. Prefer an **exhaustive differential oracle** to a random walk
  wherever the space allows it: every byte sequence up to three bytes is 16.8M cases, which *proves*
  the answer instead of sampling it, and a seeded random pass plus a libFuzzer entry point can cover
  what is left. Remember that a fuzzer which never produces a valid input finds nothing.
