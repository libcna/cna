...
| CBIND-037G | 665 | Complete the gamer-services module | ⬜ | Map the remaining gamer, profile, presence, privilege, achievement, leaderboard, avatar and guide surfaces on top of the minimum signed-in-gamer surface CBIND-036E2 and E3 already borrowed. **Split into seven slices below**; this row closes when all seven do. |
...
| CBIND-037G6a | 35 | Complete achievements | ⬜ | `Achievement` (14), `AchievementCollection` (18) and the three `SignedInGamer` achievement reads. An achievement carries strings and a picture stream and the collection's mutators take one by reference, so expect an **owned achievement handle** rather than an index-addressed value — the friend gamer is the precedent. `AchievementCollection::begin`/`end` have no C form, exactly as `GamerCollection`'s do not. `AwardAchievement` already persists locally (`CBIND-037G4`), so a collection read has something real to find. |
| CBIND-037G6b | 50 | Complete property storage and game defaults | ⬜ | `PropertyDictionary` (35), `GameDefaults` (14) and `SignedInGamer::getGameDefaultsProperty`. The dictionary is a **string-keyed variant map**: eight typed getters, eight typed setters, containment, removal, enumeration of keys and values, and an indexer answering a raw `std::any`. Its C shape is the typed family plus a key enumeration and a **value-kind query**, because a C caller reading a value it did not write has to be able to ask what is in it before choosing a getter; the raw indexer and the untyped `Add`/`Values` have no C form for the same reason a boxed value never crosses this ABI. `GameDefaults` is a fixed value with two **optional** colors, which is the availability-separate-from-the-answer rule again. |
...
32 partial, 212 planned and 305 not applicable.
...
- Branch: `feature/binding`. `CBIND-037G5` is the last task completed; the working tree is clean
  and every slice below is committed one-task-one-commit. `gamer-services` is the only module with
  planned rows left — 212 of them, and 212 in the whole campaign, in two slices.
- **Next task:** `CBIND-037G6a`, achievements — **35 rows**, the first of the three slices
  `CBIND-037G6` was split into. `Achievement`, `AchievementCollection` and the three `SignedInGamer`
  achievement reads. `CBIND-037G4`'s `cna_signed_in_gamer_award_achievement` already persists
  locally, so a collection read has something real to find rather than only an empty list.
  ...
  value, a documented refusal where it throws — rather than pretending a gamer exists.
