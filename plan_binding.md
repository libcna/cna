...
| CBIND-037E5 | 11 | Complete the CNA runtime facade | ⬜ | Map `CNA::Runtime` and `CNA::RuntimeOptions` — the extension facade that turns whole subsystems on and off before a game exists. Expect the hardest question of the slice to be **when** it may be called: `Initialize` takes an options value and `Shutdown` tears the runtime down, both outside any game, while every other route in this ABI is anchored to a game handle. Decide what a route with no handle validates against, and whether initializing twice or shutting down without initializing is refused or ignored, by reading `modules/core/src/Runtime.cpp` rather than assuming. The three `Is*Enabled` queries are the only read-back, so they are the evidence that an option was honored. |
...
partial, 875 planned and 260 not applicable, leaving the runtime module 11 rows from closed.
...
- Branch: `feature/binding`. `CBIND-037E2b` is the last task completed; the working tree is clean
  ...
  `net` and `core`.
- **Next task:** `CBIND-037E5`, the CNA runtime facade — **11 rows**: `CNA::Runtime` and
  `CNA::RuntimeOptions`. It is all that stands between this campaign and a closed runtime module,
  and it is the first surface in the ABI that is **not anchored to a game handle**: `Initialize`
  takes an options value and `Shutdown` tears the runtime down, both before or after any game
  exists. Read `modules/core/src/Runtime.cpp` first and decide from it what a handle-free route
  validates against, and whether a second initialization or a shutdown without one is refused or
  ignored. After it: `CBIND-037F` (audio, 205) and `CBIND-037G` (gamer services, 665).
