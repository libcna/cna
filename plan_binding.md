...
| CBIND-051 | Bind the glTF import report and the KHR PBR material surface | 🔵 | The 176 rows `CBIND-050` exposed, which the owner decided on 2026-08-17 to bind in full rather than disposition. Split into `051A` (done), `051B` and `051C`. `CNA::Internal::Graphics::ConfigureModelMaterialVariantsEXT` is the one row to record not-applicable rather than bind. Each slice finishes by re-approving **only its own** rule with `--approve-rule-symbols --rule <id>`; the parent closes when `coverage-closed` reads met again. |
...
| CBIND-051B | Bind the glTF import report and its diagnostics | ⬜ | The 37 rows in `GltfImportReportEXT.hpp`: twelve scene counts, three derived property getters, `AnythingLost()`, the two identity enums, and a diagnostic list whose entries carry four strings and a magnitude. Reachable already through `Model::getGltfImportReportEXTProperty`, which the C API binds. The design question is the diagnostic list: entries are variable-length and string-bearing, so the report itself is a fixed versioned POD of counts while the diagnostics are read by index with the established count/copy string pairs. Keep `Code` and `Message` visibly distinct — `Code` is the stable machine-readable identity a consumer branches on, `Message` is explicitly for people and may be reworded without an ABI break, and a C surface that blurs the two invites callers to match on the wrong one. |

| CBIND-051C | Bind the remaining model, animation and morph additions | ⬜ | The 55 rows left: `Model`'s cameras, skins, import report, bounding sphere and material variants (24, of which `ConfigureModelMaterialVariantsEXT` is recorded not-applicable as a `CNA::Internal` entry point); `ModelMeshPart`'s primitive type and the two sampler-state families (6); `SkinningData`'s skeleton-root pair, `ModelAnimationsEXT` and the two `Apply*EXT` bone helpers (7); `ClipTargetSpaceEXT` with `AnimationClipEXT::TargetSpace` (4); `MorphTargetDataEXT::TangentDeltas` (1); `CreateInfinitePerspectiveFieldOfViewEXT` (1); and the `ModelCameraEXT`/`ModelSkinEXT` type rows (12). Four of the six rules `CBIND-050` pinned are re-approved here, which closes `coverage-closed`. |
