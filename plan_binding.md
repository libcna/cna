...
planned and 307 not applicable.
...
- Branch: `feature/binding`. `CBIND-037G6a` is the last task completed; the working tree is clean
  and every slice below is committed one-task-one-commit. `gamer-services` is the only module with
  planned rows left — 177 of them, and 177 in the whole campaign, in three slices.
- **Next task:** `CBIND-037G6b`, property storage and game defaults — **50 rows**.
  `PropertyDictionary` is a **string-keyed variant map** and the design question is how a C caller
  reads a value it did not write: the typed getters and setters map one-for-one, but the raw indexer,
  the untyped `Add` and `Values` do not, because a boxed value never crosses this ABI. Plan for a
  **value-kind query** beside the typed family so a caller can ask what is in a slot before choosing a
  getter, and a key enumeration for walking it. `GameDefaults` is a fixed value whose two colors are
  **optional**, which is the availability-separate-from-the-answer rule again.
