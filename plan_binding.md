...
| 1 | `CBIND-044` close the public API coverage matrix | ⬜ | the final close; every row implemented, or carrying an owner-approved limitation a caller can query |
| — | **Two owner decisions** | ⬜ | shipping SDL3/FFmpeg with the package, and whether a static configuration is ever offered. Recorded in `docs/c-api/LIMITATIONS.md`, blocking `CBIND-042`'s verdict, and not an implementer's to make |
