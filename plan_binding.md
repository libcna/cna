...
| CBIND-051 | Bind the glTF import report and the KHR PBR material surface | ⬜ | The 176 rows `CBIND-050` exposed, which the owner decided on 2026-08-17 to bind in full rather than disposition. Four families. **`PbrEffect` and `SkinnedPbrEffect` (84)**: the KHR material extensions — index of refraction, specular factor and colour with their maps, normal scale, occlusion strength, per-slot texture coordinate sets and `TextureTransformEXT`, the base-colour/emissive/output sRGB flags, and the alpha mode/cutoff/double-sided triple. Mostly scalar accessors, so the work is in the two value types they carry, not the routes. **`GltfImportReportEXT` and its diagnostics (28+)**: twelve counts, three derived property getters, `AnythingLost()`, and a diagnostic list whose entries carry four strings — `Code` is the stable machine-readable identity a consumer branches on, `Message` is explicitly for people and may be reworded without an ABI break, so the C form must keep that distinction visible. Reachable already through `Model::getGltfImportReportEXTProperty`, which the C API binds. **`Model` (24), `ModelMeshPart`, `SkinningData`, `AnimationPlayer`, morph and skinned-model additions (12)**, plus `ClipTargetSpaceEXT`, `AlphaModeEXT`, `CreateInfinitePerspectiveFieldOfViewEXT` and the two `Apply*EXT` bone helpers. `CNA::Internal::Graphics::ConfigureModelMaterialVariantsEXT` is the one row to record not-applicable rather than bind. Finish by re-approving the six pinned rules with `--approve-rule-symbols`, flipping `coverage-closed` back to met, and re-running 81/81 and 88/88 in all four trees. |
