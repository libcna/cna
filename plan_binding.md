...
| CBIND-044 | Close the public API coverage matrix | ⬜ | Every row is implemented and tested, or carries an owner-approved native limitation with a callable C API that reports it. No unspecified omission remains. **Implementation is finished**: `CBIND-044A`–`C` turned the last three implementable rows into routes, and the snapshot is 6,083 implemented, 12 partial, 0 planned, 320 not-applicable. `CBIND-044D` gave every one of the remaining 12 a recorded disposition and a route that reports it. **The only thing left is the owner's approval of those 12 as permanent**, listed in `docs/c-api/LIMITATIONS.md`: four `GameServiceContainer` lookups and `ContentManager::Load` (C cannot name a C++ type), four content-manager service-provider members (the value is a Sharp Runtime object), two untyped content reads (the value is type-erased) and the network-session properties indexer (the canonical value is a proxy). Every one is a structural property of C, not unfinished work, and every one has a callable route that reports the reduced answer. |
...
| 1 | `CBIND-044` close the public API coverage matrix | ⬜ | the final close, and the only task left; every row implemented, or carrying an owner-approved limitation a caller can query |
