...
| CBIND-051D | Bind the remaining Model additions | ⬜ | The 35 rows left, all in `Model.hpp` and `AnimationPlayer.hpp`. `ModelCameraEXT` (12) and `ModelSkinEXT` (4) are read and written per index off the owning `CNA_ModelHandle`, the camera as a versioned POD with a name read by count/copy; `Model`'s cameras, skins, import report accessors, bounding sphere and material variants (10); `CreateInfinitePerspectiveFieldOfViewEXT` (1); `SkinningData`'s skeleton-root index and name, `ModelAnimationsEXT` and the two `Apply*EXT` bone helpers (7). `CNA::Internal::Graphics::ConfigureModelMaterialVariantsEXT` is recorded not-applicable: it is a `CNA::Internal` entry point, which the inventory excludes by path everywhere else and reaches here only because it is declared in a public header. Re-approving `Model.hpp`'s and `AnimationPlayer.hpp`'s rules closes `coverage-closed` and returns the release gate to ready. |
