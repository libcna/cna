...
- Branch: `feature/binding`. `CBIND-037G4` is the last task completed; the working tree is clean
  and every slice below is committed one-task-one-commit. `gamer-services` is the only module with
  planned rows left — 270 of them, and 270 in the whole campaign.
- **Next task:** `CBIND-037G5`, the guide, its dispatcher and its component — **58 rows**. `Guide`
  alone is 45 and is almost entirely modal UI: message boxes, keyboard input, marketplace and
  sign-in screens. **The asynchronous shape is already settled** — `CBIND-037G4` established it and
  should not be re-litigated: each canonical begin/end pair becomes **one route that produces the
  answer and then invokes a completion callback**, because the canonical operations complete before
  `Begin` returns and no operation object crosses this ABI. Check whether the guide's own operations
  complete synchronously too before reusing it; if one genuinely defers, that is a new decision to
  record rather than to assume.
