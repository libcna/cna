...
| CBIND-051E | Bind the animation-player additions | ⬜ | The last 7 rows, all in `AnimationPlayer.hpp`: `SkinningData`'s declared skeleton-root index and name (2); `ModelAnimationsEXT`, its type name and its clip map (3); and the two free helpers `ApplyClipToBonesEXT` and `ApplyBindPoseBoneTransformsEXT` (2). The first helper poses `Model::Bones` from a scene-node clip and **throws** for a joint-palette clip, since applying palette indices to bones would pose the wrong ones silently — so the C route must let that refusal through as `CNA_RESULT_INVALID_ARGUMENT` rather than swallowing it. The second returns how many skinned effects it posed, which is the natural out-parameter. Re-approving `AnimationPlayer.hpp`'s rule closes `coverage-closed` and returns the release gate to **ready**. |
