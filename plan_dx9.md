...
| D9-82c | `AlphaTestEffect` dispatch (8 `ShaderIndex` values, all unblocked — `isEqNe` sourced losslessly from `alphaTest[1] (tolerance) > 0` per `D9-81`'s own finding). | ⬜ | `D9-82b`'s shared dispatcher/entry-point plumbing (`DrawPrimitivesExImpl`) and register-upload helpers (`TryUpload*ShaderConstantEXT`) are ready to consume -- unblocked. |
| D9-82d | `DualTextureEffect` dispatch (4 `ShaderIndex` values, all unblocked). | ⬜ | `D9-82b` landed -- unblocked. Two-sampler draw (`texture0`+`texture1`), first real second-texture-unit exercise on this backend. |
| D9-82e | `EnvironmentMapEffect` dispatch (16 `ShaderIndex` values; 8 non-specular ones unblocked, 8 specular ones blocked on `D9-81`'s still-open `specularEnabled` gap). | ⬜ | `D9-82b` landed -- unblocked. First real cube-map sampling draw on this backend (`envMap`). Throw a named not-yet-implemented for the 8 specular `ShaderIndex` values specifically. |
| D9-82f | `SkinnedEffect` dispatch (18 `ShaderIndex` values; 12 non-`PreferPerPixelLighting` ones unblocked — vertex-lit/one-light × 1/2/4 bones — 6 `PreferPerPixelLighting` ones blocked same as `D9-82b`'s `BasicEffect` case). | ⬜ | `D9-82b` landed -- unblocked. First real per-vertex bone-matrix-array upload (`Bones[72]`, 216 registers) on this backend. |
