...
| D9-82b | **Full effect-aware dispatch**: `DrawPrimitivesEx`/`DrawIndexedPrimitivesEx` across all 5 Stock Effects (`BasicEffect`/`AlphaTestEffect`/`DualTextureEffect`/`EnvironmentMapEffect`/`SkinnedEffect`), consuming `D9-80`'s `D3D9ShaderDispatch` chained into `D9-74`'s `D3D9ShaderCache` and `D9-72`'s `D3D9ShaderRegisters.hpp` (this row's new `D3D9ConstantUpload` helper, from `D9-82` above) for every one of the 78 shader-variant register mappings, not just the one fixed `BasicEffect` variant `D9-82` hardcoded. Source `oneLight`/`isEqNe` from CNA's own real internal state per `D9-81`'s findings (`SkinnedEffect.cpp`'s real `DirectionalLight1/2.Enabled`, `alphaTest[1] (tolerance) > 0`), not by inferring from other `GpuDrawParams` fields. | ⬜ | Split out of the original `D9-82` row 2026-07-15 (see that row's own note) — real, separate-scale work, not a same-sitting extension. `PreferPerPixelLighting`/`EnvironmentMapEffect.specularEnabled` remain the two genuine, unresolved `GpuDrawParams` gaps `D9-81` reported — this row cannot close BasicEffect's per-pixel-lighting variants or EnvironmentMapEffect's specular variants until a project-owner-level `GpuDrawParams` decision lands; every OTHER variant (fog/vertex-color/texture/one-light/skinning-weight combinations) has no such blocker and can proceed now. |
| D9-83 | `DrawInstancedPrimitivesEx` via `SetStreamSourceFreq` (`D3DSTREAMSOURCE_INDEXEDDATA`/`INSTANCEDATA`), indexed-only. | ⬜ | D3D9 hardware instancing only works with indexed draws — and `IGraphicsBackend::DrawInstancedPrimitivesEx()` is **already** indexed-only. No interface change needed. Note: XNA 4.0's own instancing is `HiDef`-only — enforce that (Phase D9-10). Needs `D9-82b` first (a real effect-aware draw path to instance). |
