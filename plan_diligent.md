...
3. **Real device pixels** — a real Diligent device renders and a test asserts on read-back pixels.
   ✅ **reached, on a software device**: 17 of the 18 `Diligent_*` CTest binaries are fully green
   (75 checks total — `Diligent_2D` 6, `Diligent_3D` 6, `Diligent_RenderTarget` 5,
   ...
   run against a genuine Vulkan device provided by Mesa's `lavapipe` ICD under Xvfb. The 18th,
   `Diligent_DepthBias`
