...
| Task | Description | Status | Notes |
| --- | --- | --- | --- |
| `DILIGENT-48` | Real per-slot `SamplerState` (currently aliased to one shared state) | ⬜ | **Confirmed real bug, found by code reading.** `DiligentGraphicsBackend::ApplySamplerState(int slot, int filter, int addressU, int addressV, int maxAnisotropy)` takes a `slot` parameter but never uses it — it stores into flat scalar members (`samplerFilter_`/`samplerAddressU_`/`samplerAddressV_`/`samplerMaxAnisotropy_`), and every texture-binding site (`g_Texture`, `g_Texture2`, `g_EnvMap`, and all 4 PBR maps) reads that same single set of scalars regardless of which slot the caller actually configured. `GraphicsDevice.SamplerStates[1]` (or any slot > 0) is silently aliased to whatever slot 0's own state happens to be, instead of a real per-slot cache like D3D11's own `D3D11SamplerCache`/D3D12's `D3D12SamplerCache` (`DX-142`/`DX-154`, which specifically proved all 16 slots independent) or EasyGL/Vulkan. `DualTextureEffect`'s second layer and `EnvironmentMapEffect`'s cube map currently always sample with slot 0's filter/address mode, which is silently wrong the moment a caller sets a *different* `SamplerState` on slot 1/2 than slot 0 — undetected so far only because no existing Diligent test does that |
| `DILIGENT-49` | `RasterizerState.DepthBias`/`SlopeScaledDepthBias` pixel verification | ⬜ | `rasterizer.DepthBias`/`SlopeScaledDepthBias` are real fields on every pipeline, decoded from `PipelineKey::raster`'s upper bytes (`SetRasterizerState()` already packs them in), but no test proves a non-zero bias visibly changes a depth-test outcome. Same open-gap shape as D3D9's own still-unresolved `D9-62` (investigated, no observable effect found, suspected environment/driver limitation) and the real bugs Bgfx's/Vulkan's own depth-bias work (Task 767, `Vulkan_DepthBias`) found when someone actually tested this — worth attempting on `lavapipe` even if D3D9's own attempt suggests software rasterizers may not honor it |
| `DILIGENT-50` | `GraphicsDevice.ReferenceStencil` pixel verification | ⬜ | `SetReferenceStencil()` calls `context_->SetStencilRef()` on the immediate context, and every draw path re-issues `SetStencilRef(referenceStencil_)` before drawing — plausible, but nothing proves the value genuinely participates in a stencil-compare draw (e.g. `CompareFunction::Equal` against a non-default reference actually gating which pixels render). EasyGL/Bgfx don't even have this backend connection at all (`docs/graphics-backend-feature-matrix.md`'s own tracked gap, Task 872) — Diligent's own wiring looks more complete already, this task is about proving it, not building it |
| `DILIGENT-51` | `RasterizerState.FillMode::WireFrame` pixel verification | ⬜ | `FILL_MODE_WIREFRAME` is wired into every pipeline's rasterizer state, `SupportsCapability(WireFrame)` reports `true` unconditionally, but no test asserts a wireframe-filled triangle actually reads back as a hollow outline (interior pixels at the clear colour, edge pixels not) rather than either a silent solid-fill fallback or nothing rendered at all |
| `DILIGENT-52` | Anisotropic texture filtering pixel verification | ⬜ | `SupportsCapability(AnisotropicFiltering)` is device-conditional (`false` on the OpenGL device type), and `GetOrCreateSampler()` forwards `samplerMaxAnisotropy_` into the real `Dg::SamplerDesc`, but no test proves a higher anisotropy value changes sampled output at all (e.g. a grazing-angle textured quad reading back measurably sharper than at `MaxAnisotropy=1`) |
| `DILIGENT-53` | `SpriteFont` glyph placement/spacing/newline/flip pixel test | ⬜ | Shared, backend-agnostic C++ (`SpriteFont`/`SpriteBatch`), pixel-verified on EasyGL (Tasks 424-429), D3D11 (`DX-127`: glyph placement, per-glyph advance, newline line-drop+x-reset, `FlipVertically`) and D3D12 (`DX-132`, same proof off-screen) per `docs/graphics-backend-feature-matrix.md` — no Diligent equivalent exists. Same "not separately re-audited" category that document's own Vulkan audit (Task 861) flagged before closing it there |
| `DILIGENT-54` | `Model` multi-mesh/bone-hierarchy orchestration test | ⬜ | Same shared-code category as `DILIGENT-53`: D3D11 (`DX-128`) and D3D12 (`DX-148`, which also caught a real crash — unimplemented `SetDepthTestEnabled`/`SetDepthWriteEnabled`/`SetBlendEnabled` stubs) both pixel-verify a real 2-bone hierarchy driving `Model::Draw()`'s full orchestration (bone transform → `SetVertexBuffer`/`SetIndexBuffer`/`DrawIndexedPrimitives`/`EffectPass.Apply`). No Diligent equivalent exists; worth checking for the same class of "unimplemented-stub-behind-a-code-path-nothing-else-exercises" bug D3D12's own version of this task found |
| `DILIGENT-55` | `Texture2D` mip-level `SetData`/`GetData` (level > 0) dedicated round-trip test | ⬜ | `DiligentSampledTexture::SetData()`/`GetData()` both accept a mip level and `"What the baseline actually does"` above already claims per-level upload as done, but no dedicated test independently proves a specific non-zero level round-trips byte-exact *and* leaves level 0's own content unaffected — matching D3D11's own `DX-126` precedent, which exists specifically because other backends (Vulkan, Bgfx) were found to silently no-op this exact case (`Task 867`) |
| `DILIGENT-56` | NPOT (non-power-of-two) `Texture2D` real GPU round-trip test | ⬜ | No Diligent test uses a genuinely non-power-of-two texture size. D3D11's own `DX-140` needed this specifically to catch a real `D3D12_TEXTURE_DATA_PITCH_ALIGNMENT`-shaped row-pitch edge case on D3D12 — worth checking Diligent's own upload/readback staging-texture path (`ReadTextureRegion()`, `SetData()`) for the same class of stride-assumption bug, since Vulkan/D3D12 both have real row-pitch alignment requirements Diligent's own staging copy has to respect |
