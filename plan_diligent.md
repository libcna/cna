...
3. **Real device pixels** — a real Diligent device renders and a test asserts on read-back pixels.
   ✅ **reached, on a software device**: 18 of the 19 `Diligent_*` CTest binaries are fully green
   (76 checks total — `Diligent_2D` 6, `Diligent_3D` 6, `Diligent_RenderTarget` 5,
   ...
   `Diligent_Anisotropic` 1) run against a genuine Vulkan device provided by Mesa's `lavapipe` ICD
   under Xvfb. The 19th, `Diligent_DepthBias`
