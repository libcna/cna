...
3. **Real device pixels** — a real Diligent device renders and a test asserts on read-back pixels.
   ✅ **reached, on a software device**: 16 of the 17 `Diligent_*` CTest binaries are fully green
   (72 checks total — `Diligent_2D` 6, `Diligent_3D` 6, `Diligent_RenderTarget` 5,
   ...
   `Diligent_VertexLit` 4, `Diligent_Pbr` 5, `Diligent_ReferenceStencil` 1) run against a genuine
   Vulkan device provided by Mesa's `lavapipe` ICD under Xvfb. The 17th, `Diligent_DepthBias`
