...
- `BlendState` (factors, functions, slot-0 colour write mask, blend factor constant),
  ...
  mostly folded into the pipeline cache key. Scissor enable is now in that key (`DILIGENT-58`) and
  depth bias is stored losslessly (`DILIGENT-64`); sample mask is still discarded (`DILIGENT-60`).
