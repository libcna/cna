...
3. **Real device pixels** — a real Diligent device renders and a test asserts on read-back pixels.
   ✅ **reached, on a software device**: 21 of the 22 `Diligent_*` CTest binaries are fully green
   (102 of their own checks — `Diligent_2D` 6, `Diligent_3D` 6, `Diligent_RenderTarget` 5,
   ...
   `Diligent_Anisotropic` 1, `Diligent_SpriteFont` 4, `Diligent_Model` 1, `Diligent_Mip` 22 — plus
   `Diligent_DepthBias`'s own 4 checks, 3 of which pass, makes 106 checks total, 105 passing) run
   against a genuine Vulkan device provided by Mesa's `lavapipe` ICD under Xvfb. The 22nd,
   `Diligent_DepthBias`
