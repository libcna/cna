...
>
> Of 460 rows: **382 are closed (`✔` 256, `✅` 126)** and **59 remain `⬜`**. The other 19 carry a
> deliberate partial marker — 8 `🔬` (investigation, no implementation owed), 9 `✅/⬜`, no
...
| GLTF-175 | `tangent.w` handedness end-to-end | ✅/⬜ | GLTF-169 | Importer → stride 48/68 → `vBitangentSign` → `B = cross(N,T)*sign`. **Accept:** `tangent-handedness` renders the two halves of a normal-mapped quad with opposite, correct lighting at L7. **L1–L5 half landed; L7 remains.** `tangent-handedness` is one mesh with two adjacent primitives carrying the same `N=+Z`, `T=+X` and UV frame but opposite authored `w`; its deliberately non-flat tangent-space `+Y` normal map makes the sign affect pixels rather than merely an unused field. L3 asserts both signs, L4 reconstructs opposite bitangents, L5 locks the two independent vertex streams byte-for-byte, and a direct production-path test proves the importer did not default either sign. OPENGLES3 now rasterises in this environment; the remaining work is the dedicated fixture-driven pixel test, not renderer availability. |
