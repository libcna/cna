...
| GLTF-223 | Emissive applied after the texture, per spec order | ✅ | GLTF-222 | **Accept:** locked. **Lock blocked in this environment.** The rule is *shader-side* — the emissive term is added after the base-colour sample in the fragment program — and no shader is compiled under the `STUB` renderer. Verifying it needs either a renderer build (EasyGL needs the sibling `../easy-gl` and `../meta-gl` checkouts, absent here) or `L7`, which `GLTF-010` records as blocked. Same blocker as `GLTF-157`. |
