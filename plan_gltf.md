...
| ID | Title | St | Deps | Scope, evidence → acceptance |
|---|---|---|---|---|
| GLTF-383 | Run the whole corpus on `STUB` and `HEADLESS` | ✔ | GLTF-382 | **Accept:** L1–L6 green, no GPU required. **Both green, and the second renderer earned its build immediately.** Current `*Gltf*` selection after the `GLTF-373`/`GLTF-379` renderer-source audits: `HEADLESS` **518 passed, 0 skipped**; `STUB` **491 passed and 27 explicitly skipped**, all at the `GraphicsCapability::ThreeD` gate. `ctest -L gltf-conformance` is 10/10 on both. **Two capability-gated cases failed on their first execution in months** — `ExtractsEmbeddedBaseColorTextureAndLoadsIt` still expected `SkinnedEffect` and `SerializesAndReloadsKhrLightsPunctualThroughTheOfflineCnjPath` still expected `BasicEffect`, for materials that `GLTF-215` moved to the PBR path. Neither is a renderer bug: they are **stale expectations a capability skip had been hiding**, which is exactly the failure mode this row exists to surface — a test that skips everywhere is indistinguishable from a test that passes. Both now assert the effect the material model selects, and say in a comment why. |
| GLTF-384 | Run the corpus on `OPENGLES3` (EasyGL) | ✅/⬜ | GLTF-383 | The viewer's renderer. **Accept:** L1–L7 green. **L1–L6 and every currently registered glTF suite are green:** the OPENGLES3 `ctest -L gltf-conformance` run is 10/10; `*Gltf*` is 507/507 with no skips. `scripts/gltf-renderer-parity.sh` reports 42 L1–L5 tests identical across STUB, HEADLESS and Vulkan too, 507/507 identical to HEADLESS/Vulkan, and only STUB's 27 named ThreeD skips against OPENGLES3. Focused EasyGL L7 tests also rasterise PBR, Fresnel and sRGB cases. The row remains partial because its acceptance says the **corpus** L7 rung, which is still `GLTF-009`, not because EasyGL or the GPU is unavailable. |
| GLTF-385 | Run the corpus on `VULKAN` | ✅/⬜ | GLTF-383 | Independent shader source. **Accept:** L1–L7 green. **The newly available Vulkan/lavapipe build is green for every currently registered glTF test:** 507/507, no skips, and `ctest -L gltf-conformance` is 10/10, with a real Vulkan instance/device/swapchain and submit/present rather than a compile-only claim. Its 42 L1–L5 results are byte-identical to STUB, HEADLESS and OPENGLES3, and the full 507 outcomes are identical to HEADLESS and OPENGLES3. The row remains partial for the same reason as `GLTF-384`: corpus-wide L7 reference images/tolerances remain `GLTF-009`/`GLTF-390`; a green non-image corpus run does not invent that evidence. |
