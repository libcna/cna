...
| GLTF-388 | Assert L5 equality across renderers | ✅/⬜ | GLTF-168 | L5 is renderer-independent by construction. **Accept:** byte-identical; any divergence is a bug. **Asserted, on the two renderers this environment can build.** `scripts/gltf-renderer-parity.sh` compares two build directories and fails on any L1–L5 difference: `STUB` and `HEADLESS` agree on all 39 L1–L5 tests. The byte-identity is stronger than a run-to-run diff, because each run compares against the **same committed L5 goldens** — two runs agreeing with each other could be agreeing on a shared mistake; each agreeing with a third, committed artifact cannot. **The residue is renderers, not evidence:** `GLTF-168` names four, and `OPENGLES3`/`VULKAN` need sibling checkouts and a GPU this environment does not have. Adding them is a one-command check when they exist, which is why this row is `✅/⬜` rather than blocked. |
