...
| `KHR_materials_ior` | **PARSED_BUT_IGNORED** | The factor reaches PBR effect state and shader-ready dielectric F0/F90; EasyGL consumes both endpoints, but other PBR renderers do not yet. | `GLTF-343` |
| `KHR_materials_specular` | **PARSED_BUT_IGNORED** | Factor and colour reach PBR effect state and EasyGL's rigid/skinned shaders. The two optional textures are not imported and other PBR renderers do not consume the endpoints yet. | `GLTF-344` |
...
| GLTF-343 | `KHR_materials_ior` | ✅/⬜ | GLTF-235 | Affects `F0`. **Accept:** shader change + analytic check. **Transport, analytic derivation and EasyGL consumption are complete; other PBR renderers remain.** `ior` survives cgltf → `MaterialOut` → direct/offline loaders → both clone-safe PBR effects → `GpuDrawParams::pbrDielectricF0/F90`. `EasyGLRenderer` uploads those endpoints to both rigid and skinned programs, mixes dielectric F0 with albedo by metallic, and uses the transported F90 in Schlick. `EasyGL_Pbr_FresnelFactors` is a real OPENGLES3 pixel proof: its normal-incidence black/rough dielectric reduces to `F0/(4π)`, producing core `(11,11,11)` and authored IOR/specular `(2,9,43)` on both programs. A grazing pair holds F0 fixed at `.04` while changing only F90 from 1 to .3 and measures `(33,33,33)` versus `(15,15,15)`. The row stays partial until every PBR-capable renderer consumes the factors; the registry remains honestly unclaimed meanwhile. |
| GLTF-344 | `KHR_materials_specular` | ✅/⬜ | GLTF-343 | **Accept:** fixture correct. **Factor-only transport and EasyGL rendering are correct; texture and cross-renderer halves remain open.** Both PBR effects carry CNAEXT `SpecularFactor`/`SpecularColorFactor` with glTF defaults 1/white and derive RGB F0 plus scalar F90. The discriminating witness uses IOR 2, strength 0.3 and colour `(0.25,1,12)`: F0 `(1/120,1/30,0.3)` produces the pixel `(2,9,43)`, so blue proves the mandated `min(iorF0 * colour, 1) * strength` order reaches the real shader. The F90-only grazing comparison additionally proves strength changes grazing reflectance rather than merely the normal-incidence value. Direct/offline corpus parity, effect tests and old-`.cnj` defaults remain green. Still missing: `specularTexture`, `specularColorTexture`, their independent UV/colour-space handling, and the same endpoints on non-EasyGL PBR renderers. Validation names both residues and a required use remains refused. |
