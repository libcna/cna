...
| 10 | every fixture with `l5.supported` (131 of 139) | `GltfConformanceL5`, `GltfBufferOracle`, `GltfStrideAndBuffer`, `GltfVertexLayoutTable`, `GltfVertexBufferInvariants` | green |
...
| GLTF-416 | Corpus documentation | ✔ | GLTF-403 | **Accept:** `docs/gltf-conformance.md` lists every fixture and what it proves. **§7 lists all 139,** with owning group, declared layers and generated `features`; `python3 -m gltf_fixtures --fixture-table` is the source and `TheConformanceDocListsEveryFixtureTheManifestDeclares` checks both directions. A fixture missing a row or a stale row whose fixture disappeared therefore fails by name. §4.3 records the matching golden coverage: **131 of 139** carry an L5 golden; the eight unsupported cases each record `l5.supported = false` and their reason, keeping "no golden" distinguishable from "golden forgotten". |
