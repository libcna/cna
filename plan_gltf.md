...
| GLTF-372 | Alpha-test parameters actually configured | ✔ | GLTF-229 | `uAlphaTest` exists but nothing sets it. **Accept:** L6 shows the cutoff. **Fixed, and it was a live silent loss:** every PBR fragment program already evaluates `uAlphaTest` and discards on it, and neither `PbrEffect` nor `SkinnedPbrEffect` ever filled the vector in — so a `MASK` material rendered exactly like an `OPAQUE` one, at the only place the distinction exists. Both effects now derive it through one mapping (`AlphaTestVectorForAlphaModeEXT`, `CNA/Internal/Graphics/AlphaCoverageEXT.hpp`) rather than two hand-written copies of a four-element sign convention in which swapping `z`/`w` inverts the cutout. `MASK` is `{cutoff, 0, -1, +1}`; `OPAQUE` and `BLEND` keep the never-discard `{0,0,1,1}` default, because what they need is `BlendState` and a draw order the application owns — `GLTF-230`, and `docs/gltf-api-change-review.md` §1.3 now records where that line falls. The new fixture `mat-alpha-mask-cutoff` authors a cutoff of **0.75** — neither glTF's 0.5 default nor the material's own 0.875 alpha — so a cutoff that never arrived and one wired to the wrong alpha quantity are both visible as values; `AMaskMaterialsCutoffReachesTheDrawsAlphaTestVector` compares against the manifest's stated `gpuAlphaTest` and then evaluates the shader's own discard expression just below, exactly at and well above the cutoff, so the sign convention is asserted as behaviour. `OnlyAMaskMaterialWritesAnAlphaTestReference` is the corpus-wide control: an implementation that wrote a reference for every material would cut holes in every opaque surface, and it would have passed the positive test. |
