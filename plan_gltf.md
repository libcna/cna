...
>
> Of 460 rows: **389 are closed (`✔` 256, `✅` 133)** and **54 remain `⬜`**. The other 17 carry a
...
| GLTF-084 | `NORMAL` decode and default | ✅ | GLTF-083 | Absent normals currently emit `(0,0,1)` — a **fabricated** default, not glTF's "compute flat normals". **Accept:** decision recorded (§8 of Phase 8) and tested. **The default half is `GLTF-173`'s**, decided and landed there: CNA computes the area-weighted vertex normal, `normal-absent` is tilted out of the XY plane so it can tell that from the old fabricated `(0,0,1)`, and the renormalisation policy table in `docs/gltf-conventions.md` records where each generated normal is made unit. The **decode** half was the untested one. §3.7.2.1 allows `NORMAL` as `float`, `byte normalized` or `short normalized`, and every normal in the corpus was a plain float — two of the three legal storage forms had no asset behind them at all. Not hypothetical: quantized normals are what a size-conscious exporter emits, and reading one as raw integers gives a normal of **length 32767**, which no later layer rejects and which lights as pure white. `normal-quantized` authors only 32767 and 0, which §3.6.2.2 maps to exactly 1.0 and 0.0, so the expectation states no rounding and the assertion is `EXPECT_FLOAT_EQ` rather than a tolerance that would swallow a 32768 divisor. Its three normals point along three different axes, so a decoder reading the wrong component is wrong *differently* per vertex rather than uniformly wrong in a way that reads as a coordinate-system difference. A corpus-wide sweep then asserts every packed normal is unit length whatever its storage form, which doubles as a check on the pass-through policy: an authored normal is never renormalised, so a mis-decode has nowhere to hide. |
