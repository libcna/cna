...
| GLTF-379 | Cross-renderer shader semantic audit | 🔬 | GLTF-373 | **Accept:** a per-renderer matrix of the §21.1 rows. **Alpha coverage slice complete:** the source inventory found eight PBR implementations that did not consume `GpuDrawParams::alphaTest`, plus five renderers that gave their generic alpha-test effect priority over PBR and therefore lost the PBR material/layout. DirectX 11/12, LLGL, OpenGL 4, SDL GPU, Vulkan, WebGPU and Wicked now consume the common expression; PBR wins dispatch wherever the paths competed. The inventory gate covers all 15 implementations and both separately stored rigid/skinned variants, Vulkan renders four discriminating MASK pixels in addition to its ten texture-slot cases, the affected native/cross targets compile, and Wicked's extracted `PbrPS` compiles with glslang. `docs/gltf-renderer-pbr-fallbacks.md` carries the exact matrix. The remaining §21.1 rows keep this task open. |
