...
>
> Of 460 rows: **378 are closed (`✔` 252, `✅` 126)** and **65 remain `⬜`**. The other 17 carry a
...
| GLTF-017 | Prove the harness is renderer-independent at L1–L5 | ✔ | GLTF-010 | Run L1–L5 on `STUB` and `HEADLESS`; results must be identical. **Accept:** byte-identical L2–L5 output. **Done, and it needed a second build to be possible at all:** CNA selects its renderer at *compile* time, so one test binary can never notice a renderer leaking into a layer that should not have one. `scripts/gltf-renderer-parity.sh` takes two build directories and compares the two runs — **39 L1–L5 tests, identical on `STUB` and `HEADLESS`**, timings elided so a millisecond cannot fail it. The byte-identity the row asks for is stronger than the diff suggests: each run compares its own output against the **same committed L5 goldens**, so two agreeing runs are not agreeing with each other (which could be a shared mistake) but each with a third, committed artifact. The script also compares the full `Gltf*` selection and tolerates exactly one class of difference — `SKIPPED` versus `OK`, which is a renderer *capability* gate (`GraphicsCapability::ThreeD`) rather than a divergence — and fails on anything else, naming §6's rule that a shared-importer defect must not be fixed inside a renderer directory. |
...
| GLTF-388 | Assert L5 equality across renderers | ✅/⬜ | GLTF-168 | L5 is renderer-independent by construction. **Accept:** byte-identical; any divergence is a bug. **Asserted on STUB and HEADLESS.** `scripts/gltf-renderer-parity.sh` compares two build directories and fails on any L1–L5 difference: those two agree on all 39 L1–L5 tests. The byte-identity is stronger than a run-to-run diff, because each run compares against the **same committed L5 goldens** — two runs agreeing with each other could be agreeing on a shared mistake; each agreeing with a third, committed artifact cannot. OPENGLES3 is now available, so adding it to this comparison is actionable; Vulkan is still absent. This row remains `✅/⬜` until the four-renderer acceptance in `GLTF-168` is met. |
