...
| GLTF-379 | Cross-renderer shader semantic audit | 🔬 | GLTF-373 | **Accept:** a per-renderer matrix of the §21.1 rows. **Four slices complete.** Alpha coverage: the source inventory found eight PBR implementations that did not consume `GpuDrawParams::alphaTest`, plus five renderers that gave their generic alpha-test effect priority over PBR and therefore lost the PBR material/layout; all are corrected and PBR wins dispatch wherever the paths competed. Map scalars: EasyGL alone consumed `normalTexture.scale` and `occlusionTexture.strength`; the other 14 renderers now upload both values and implement the exact XY-only normal scaling and `1 + strength * (sampled - 1)` equations. Skinned normals: EasyGL alone inverse-transposed a non-uniform joint/palette matrix; all other skinned PBR paths now do the same, Metal gains its missing world normal matrix, OpenGL2 uses `uNormalMatrix`, and WebGPU's rigid/skinned 32-byte `PbrFactors` bindings no longer advertise an invalid 16-byte minimum. Tangent orientation: EasyGL alone folded direction-transform determinant signs into `tangent.w`; all 15 rigid/skinned PBR variants now cover World and blended skin, the two instance-aware PBR implementations cover their optional instance matrix, and Metal's skinned tangent gains its missing World direction transform. One source inventory suite covers all 15 implementations and every separately stored variant, with WebGPU's actual SkinnedPbr WGSL scoped explicitly after the first broad test exposed a false positive. Vulkan retains the 18 texture/scalar/MASK pixels; the focused executable is now 8/8 for joint inverse-transpose plus mirrored World/joint TBN. WebGPU rigid and skinned PBR each pass 5/5 and D3D9 passes 6/6. The affected shader generators and native/cross targets compile, and Wicked's extracted PBR entries compile with glslang. `docs/gltf-renderer-pbr-fallbacks.md` carries all four exact matrices. The remaining §21.1 rows keep this task open. |
