...
>
> Of 460 rows: **380 are closed (`✔` 254, `✅` 126)** and **61 remain `⬜`**. The other 19 carry a
...
numerical oracle hierarchy, a 144-asset conformance corpus, a 24-phase dependency-ordered backlog of
...
### 24.2 Target corpus — 144 distinct synthetic assets
...
| **Total** | **144** | **140** | **4 named assets** |

> **Materials group, three times adjusted (`GLTF-241`, `GLTF-224`/`GLTF-225`, `GLTF-341`).** `mat-vertex-color-pbr`
...
| 10 | every fixture with `l5.supported` (132 of 140) | `GltfConformanceL5`, `GltfBufferOracle`, `GltfStrideAndBuffer`, `GltfVertexLayoutTable`, `GltfVertexBufferInvariants` | green |
...
| 12 | `mat-factor-only-gold`, `mat-emissive-strength`, `mat-normal-occlusion-scale`, `mat-material-variants` | `GltfConformanceL6`, `GltfMaterialState`, `GltfMaterialVariants`, `GltfDrawParamsOracleL6`, `GltfToCnjToolTest` | green |
...
| GLTF-218 | `baseColorFactor` × `baseColorTexture` | ⬜ | GLTF-216 | The two multiply. **Accept:** `mat-basecolor-factor-times-texture` matches at L7. **EasyGL shader half pinned:** its real rigid and skinned programs turn a mid-grey sRGB sample with linear factor 0.5 into byte 92, exactly `OETF(decode(128/255)*0.5)`; ignoring the factor, multiplying after output encode or decoding the factor cannot match. The row remains open for the named corpus fixture and cross-renderer L7 route. |
...
| GLTF-416 | Corpus documentation | ✔ | GLTF-403 | **Accept:** `docs/gltf-conformance.md` lists every fixture and what it proves. **§7 lists all 140,** with owning group, declared layers and generated `features`; `python3 -m gltf_fixtures --fixture-table` is the source and `TheConformanceDocListsEveryFixtureTheManifestDeclares` checks both directions. A fixture missing a row or a stale row whose fixture disappeared therefore fails by name. §4.3 records the matching golden coverage: **132 of 140** carry an L5 golden; the eight unsupported cases each record `l5.supported = false` and their reason, keeping "no golden" distinguishable from "golden forgotten". |
