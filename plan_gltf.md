...
| GLTF-233 | Metallic-roughness channel semantics | ✅ | GLTF-220 | Shader reads `.g` roughness, `.b` metallic — correct per spec. **Accept:** locked with a three-distinct-value fixture. **Lock blocked in this environment** — same shader-side blocker as `GLTF-223`. `L6` shows the metallic-roughness map is bound and with which factors, not which channel the program reads out of it. |
