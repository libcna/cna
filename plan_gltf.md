...
| ID | Title | St | Deps | Scope, evidence → acceptance |
|---|---|---|---|---|
| GLTF-399 | Complete the 135-asset synthetic corpus | ✅/⬜ | GLTF-003 | §24.2's owning-group inventory (**136** distinct assets after `GLTF-011`'s and `GLTF-137`'s recorded count corrections), each with a `.glb` twin. **Accept:** every owning group's assets exist, are generated and are validated; **CI asserts the generator's manifest reports exactly that many distinct assets**. **From 69 to 111 in this session, with five groups now complete:** transforms **17/17** (§11.4's full ladder), component types **8/8**, accessors **12/13**, scenes **3/3**, morph **12** (§16.3's family, `GLTF-292`). The scene group's last fixture found a live defect: `scene-no-scenes` proved that a file with **no `scenes` array** — which §3.5 permits — imported every mesh **at the origin**, because `BuildSceneGraph` returned an empty graph and the caller's "every mesh" fallback carried no placement. That is D1's failure mode surviving in the one corner the scene traversal never covered, and the corpus had no asset that could reach it (`known_bugs.md`). Every one was authored to discriminate rather than to exist — the transform rungs each defeat one plausible wrong composition, `normalized-i8-normal` is axis-aligned *because* 127 is prime and no off-axis 8-bit triple is unit length, and `stride-padded` fills its padding with a sentinel so a tightly-walking reader produces a visibly broken triangle rather than a subtly wrong one. **What remains, and why the row stays open:** the texture group (7) needs real images, the container group (7) needs external `.bin`/image sidecars the generator does not emit yet, skinning (6), materials (2), scenes (2) and robustness (1) are ordinary work, and Draco (4) is `libdraco`-blocked. **One residue is an oracle limitation rather than a missing fixture, and it is stated where it bit:** `two-primitives-one-buffer` was authored as one mesh with two primitives, which made the L4 oracle (one instance per node-with-a-mesh) and CNA (one per primitive) disagree about the instance *count* before any geometry was compared. It is reshaped to two meshes sharing one bufferView — the accessor-sharing subject is unaffected — and closing the gap properly means teaching both the oracle and the generator to enumerate per primitive. |
