...
| `KHR_materials_ior` | **PARSED_BUT_IGNORED** | The factor reaches PBR effect state and shader-ready dielectric F0 at L6, but no renderer consumes it yet. | `GLTF-343` |
| `KHR_materials_specular` | **PARSED_BUT_IGNORED** | Factor and colour reach PBR effect state and dielectric F0/F90 at L6. The two optional textures are not imported and no renderer consumes the factors yet. | `GLTF-344` |
...
| GLTF-343 | `KHR_materials_ior` | ✅/⬜ | GLTF-235 | Affects `F0`. **Accept:** shader change + analytic check. **Factor transport and the analytic half landed; renderer consumption remains open.** `ior` now survives cgltf → `MaterialOut` → direct and offline loaders → both PBR effects, whose CNAEXT property defaults to 1.5 and is clone-safe. `FillGpuDrawParams` derives a shader-ready RGB `pbrDielectricF0`; the core default is exactly `(0.04,0.04,0.04)`. `GltfPbrBrdf.IorAndSpecularInteractionClampsColourBeforeStrength` independently pins the Khronos equation, while `mat-factor-only-gold` authors IOR 2 and proves the value at L3 and L6. The public additions passed `docs/gltf-api-change-review.md` §1.4a and old `.cnj` files retain the 1.5 default. The row stays partial because every renderer still hardcodes 0.04 in its BRDF; changing that without an L7 rasterising oracle would repeat `GLTF-157`'s unverified-shader failure mode. The registry therefore remains `PARSED_BUT_IGNORED`, unclaimed, and reports exactly that residue. |
| GLTF-344 | `KHR_materials_specular` | ✅/⬜ | GLTF-343 | **Accept:** fixture correct. **Factor-only transport is correct through L6 and `.cnj`; texture and renderer halves remain open.** Both PBR effects carry CNAEXT `SpecularFactor`/`SpecularColorFactor` with glTF defaults 1/white and derive RGB F0 plus scalar F90. The discriminating witness uses IOR 2, strength 0.3 and colour `(0.25,1,12)`: expected F0 is `(1/120,1/30,0.3)` and F90 0.3, so blue proves the mandated order is `min(iorF0 * colour, 1) * strength` rather than clamp after strength. Direct/offline corpus parity, effect default/setter/clone tests, and an old-`.cnj` default test are green on STUB and HEADLESS. Still missing: `specularTexture`, `specularColorTexture`, their independent UV/colour-space handling, and replacement of the hardcoded Fresnel endpoints in every renderer. Validation warns by extension name and names both residues; a required use remains refused. |
