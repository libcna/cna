...
| Non-uniform joint scale | positions are locked at L4; EasyGL normals now use and test the joint inverse-transpose at focused L7 | `GLTF-264` |
...
| `skin-nonuniform-joint-scale` | joint `S=[1,2,1]` | positions asserted at L4; the manifest's `(0,.351123,.936329)` normal is asserted by the matching focused EasyGL L7 rig (`GLTF-264`) |
...
| GLTF-264 | Skinned normals and tangents | ✔ | GLTF-248 | The skinned shader transforms Position, Normal and Tangent by the palette. **Accept:** `skin-nonuniform-joint-scale` normals correct at L4/L7 (inverse-transpose where required). **Landed on the EasyGL reference renderer.** The defect was measured before the fix: its direct `mat3(skinMat) * normal` sent `(0,.6,.8)` through joint `S=[1,2,1]` to `(0,.832050,.554700)` and rendered byte **212**, while the specification-preserving inverse transpose yields `(0,.351123,.936329)` and byte **90**. One shared GLSL ES 1.00-compatible cofactor helper now serves the pixel-lit, vertex-lit and skinned-PBR programs; it preserves determinant sign for mirrored joints and retains the existing near-singular fallback. `EasyGL_SkinnedEffect_WorldNormal` locks uniform/non-uniform joint scale on both stock programs (12 analytic cases), and `EasyGL_Pbr_MaterialMaps` independently locks the PBR path at byte **93** rather than the former **139**. Both pass on OPENGLES2 and OPENGLES3; the tangent remains a direction transformed by the palette 3×3 and is re-orthogonalised against the corrected normal when constructing PBR's TBN. The corpus position/palette halves remain L4/L5; corpus-wide image capture remains `GLTF-009`. |
...
| GLTF-379 | Cross-renderer shader semantic audit | 🔬 | GLTF-373 | **Accept:** a per-renderer matrix of the §21.1 rows. **Two slices complete.** Alpha coverage: the source inventory found eight PBR implementations that did not consume `GpuDrawParams::alphaTest`, plus five renderers that gave their generic alpha-test effect priority over PBR and therefore lost the PBR material/layout; all are corrected and PBR wins dispatch wherever the paths competed. Map scalars: EasyGL alone consumed `normalTexture.scale` and `occlusionTexture.strength`; the other 14 renderers now upload both values and implement the exact XY-only normal scaling and `1 + strength * (sampled - 1)` equations. One inventory gate covers all 15 implementations and both separately stored rigid/skinned variants. Vulkan renders 18 discriminating pixels: ten texture slots, four scalar cases and four MASK cases. The affected shader generators and native/cross targets compile, and Wicked's extracted `PbrPS` compiles with glslang. `docs/gltf-renderer-pbr-fallbacks.md` carries both exact matrices. The remaining §21.1 rows keep this task open. |
