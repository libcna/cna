...
| GLTF-175 | `tangent.w` handedness end-to-end | ✅ | GLTF-169 | Importer → stride 48/68 → `vBitangentSign` → `B = cross(N,T)*sign`. **Accept:** `tangent-handedness` renders the two halves of a normal-mapped quad with opposite, correct lighting at L7. **Complete on the EasyGL reference renderer.** `tangent-handedness` is one mesh with two primitives carrying the same `N=+Z`, `T=+X`, UV frame, material and constant tangent-space approximately-`+Y` normal map, but opposite authored `w`. L3 asserts both signs, L4 reconstructs `B=+Y/-Y`, L5 locks the two independent stride-48 streams byte-for-byte, and the direct production-path test proves the importer did not default either sign. Registered `EasyGL_Gltf_TangentHandedness` now closes the framebuffer half through `ContentManager` and `Model::Draw`: it moves a strictly interior point of each primitive to the same centre pixel, parks ambient/emissive, uses `L=+Y` and `V=+Z`, and obtains analytic byte **151** for `w=+1` versus exact **0** for `w=-1` on OPENGLES2 and OPENGLES3. Dropping the sign makes both bright, ignoring the normal map makes both black, and inverting the sign swaps them, so none can pass. |
| GLTF-176 | Normal/tangent transformation under node transforms | ✅ | GLTF-118, GLTF-119 | Only relevant if `GLTF-103` chooses baking; the shader's `uNormalMatrix` needs it either way. **Accept:** `xf-scale-nonuniform` and `tangent-mirrored` correct at L4/L7. **Complete on the EasyGL reference renderer, fixing the reproduced mirrored-TBN defect.** The normal half remains `normal-nonuniform-scale`: exact `Rz90*S(2,3,4)` plus a slanted normal separates inverse transpose from both raw world 3×3 and an accidental transpose at L4/L6. `tangent-mirrored` places one shared, normal-mapped `(+X,+1)` tangent buffer normally and through `T(3,0,0)*S(-1,1,1)`; L4 states that both placements reconstruct world `B=+Y`, while L5 proves the local buffer remains unchanged. Before the fix, registered `EasyGL_Gltf_MirroredTangent` rendered analytic byte **151** for the ordinary placement and exact **0** for the mirrored one. Both PBR vertex programs now compute the determinant sign through a GLSL-ES-compatible scalar triple product and multiply it into `tangent.w` per draw; the skinned variant includes the blended skin determinant and both include an optional instance transform. Singular transforms keep the authored sign rather than `sign(0)` erasing it. The same test now gives byte **151** for both placements on OPENGLES2 and OPENGLES3; the five existing rigid/skinned PBR regression witnesses remain green on both profiles. |
