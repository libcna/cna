...
| GLTF-028 | External `.bin` buffer resolution | ✅ | GLTF-026 | **Accept:** relative URI resolves against the `.gltf` directory; a missing file errors clearly. **Tested end to end from a scratch directory rather than as a corpus fixture**, deliberately: every corpus asset carries its buffer as a base64 `data:` URI so the two containers can be generated from one description and byte-compared, which is the right trade for the corpus and leaves exactly this case with no asset behind it — and what is under test here is *a file on disk*. The buffer lives in a **subdirectory**, because the document's directory, the process's working directory and the `ContentManager` root coincide in almost every other test, so a loader resolving against the wrong one would pass them all. The missing-file case asserts more than "it throws": the failure mode worth guarding is a load that *succeeds* with an empty buffer, giving a model whose geometry is silently all zeros with the vertex count still right. A third test proves the ordinary load path actually reaches `ResolveExternalUriEXT` — `GltfUriContainment` shows the resolver refuses a traversal, but a loader that resolved buffer paths itself and consulted containment only for images would satisfy every one of those tests. |
| GLTF-029 | Base64 `data:` URI buffers | ✔ | GLTF-026 | All 14 audit fixtures use them. **Accept:** locked by test, including odd padding lengths. **Locked at all three base64 padding residues** (12, 16 and 20-byte payloads — no pad, two pads, one pad). The one-pad and two-pad cases are where a decoder loses or invents a trailing byte, and since every asset in this corpus is self-contained a padding bug would break all of them at once — surfacing as a validation error about an *accessor*, which points at the wrong thing. |
| GLTF-030 | Percent-encoded URI handling | ✅ | GLTF-028 | `cgltf_decode_uri` is used for images; verify for buffers too. **Accept:** a percent-encoded buffer URI loads. **Verified for buffers, and in both directions** — which is the half that makes it an assertion. A file whose name contains a space is ordinary on every desktop platform, and cgltf decodes *image* URIs itself, exactly the kind of asymmetry that would leave buffers looking for a file literally called `my%20geometry.bin` and reporting it missing rather than wrongly spelled. So one test names the real file `my geometry.bin` and requires the load to succeed; the control names it `my%20geometry.bin` and requires the load to **fail**, because a loader that ignores percent-encoding entirely passes the first test and only the control can tell the two apart. A fourth test asserts the same geometry arrives identically from an external buffer and from a `data:` URI: where a buffer lives is a container decision, the two take genuinely different code paths, and a divergence would surface as geometry that depends on how the exporter happened to be configured. |
...
| GLTF-170 | Tangent generation when absent | ✔ | GLTF-169 | `ComputeTangentsEXT`: angle-weighted, Gram-Schmidt, documented non-MikkTSpace divergence. **Accept:** `tangent-absent-generated` matches the manifest; the divergence stays documented. **Locked on direction *and* length.** The basis is defined by the UVs — a tangent points along increasing U — so a UV layout with U along +X must produce a tangent along +X. Unit length is asserted separately because a Gram-Schmidt step that forgets to renormalise leaves a shorter vector, which does not look wrong anywhere: it silently dims the normal-map contribution. The MikkTSpace divergence remains `GLTF-179`'s to quantify. |
...
| GLTF-194 | External URI images | ✔ | GLTF-028 | **Accept:** locked; a missing file errors clearly. **Locked, including the missing-file message.** "Cannot open" has to name the path: the commonest cause is an asset shipped without its textures, and the whole fix is about which file is missing from where. |
| GLTF-195 | `data:` URI images | ✔ | GLTF-029 | **Accept:** locked. **Locked against the `bufferView` form rather than against a literal**, because the point is that they are the *same asset*: a `.glb` and a self-contained `.gltf` are two exports of one model, so a reader handling them differently makes the same model look different depending on how it was saved. A `data:` URI with no comma is additionally asserted to be **refused** — decoding the media type as base64 yields plausible bytes that are not an image, surfacing much later with nothing pointing back at the URI. |
...
| ID | Title | St | Deps | Scope, evidence → acceptance |
|---|---|---|---|---|
| GLTF-399 | Complete the 135-asset synthetic corpus | ✅/⬜ | GLTF-003 | §24.2's owning-group inventory (**136** distinct assets after `GLTF-011`'s and `GLTF-137`'s recorded count corrections), each with a `.glb` twin. **Accept:** every owning group's assets exist, are generated and are validated; **CI asserts the generator's manifest reports exactly that many distinct assets**. **From 69 to 120 in this session, with seven groups now complete:** transforms **17/17** (§11.4's full ladder), component types **8/8**, accessors **12/13**, scenes **3/3**, materials **12/12**, robustness **8/8**, skinning **16/16**, morph **12** (§16.3's family, `GLTF-292`). **Completing skinning meant evaluating §15.4's ladder rather than restating it.** Six of its rungs had a hand-written test because they owned a defect; the rest stated an exactly computable world position in this plan and in the manifest, and nothing read it. The six new fixtures — `skin-two-weighted`, `skin-four-weighted`, `skin-no-ibm`, `skin-nonuniform-joint-scale`, `skin-parented-joints`, `skin-ushort-joint-indices` — each carry the specification's answer, and `GltfSkinLadder` now runs §3.7.3.3's equation over **every** fixture declaring an `l4.skin` block using CNA's real bone palette, so the whole ladder is checked at once. Two things keep that from being circular: the expected positions are computed by the generator in Python from the authored transforms, and the file's joint index space is mapped into CNA's bone space through the importer's own `oldToNew` rather than assumed equal — §15.1.2 makes them deliberately different, so a reordering shows up as a wrong position instead of being absorbed. A second test asserts the ladder's *shape*: each of the six properties must be declared by some fixture, since a count alone would pass with six copies of one rung. **CNA answered all of them correctly**, which is itself the finding — the skin equation was right and unevidenced. **One §15.4 rung is unreachable and is replaced rather than faked:** `skin-256-joints` exists to prove a 256-joint rig does not wrap the index byte, but `GLTF-261` refuses any skin past the 72-joint palette long before its indices are decoded, and `skin-73-joints` already owns that refusal one joint past the boundary. The reachable half of the same hazard is the *component type* — `JOINTS_0` stored as `UNSIGNED_SHORT`, which §3.7.3.3 permits and XNA's four-byte `BlendIndices` must convert rather than copy — so `skin-ushort-joint-indices` takes the rung, with its two influencing joints chosen so a byte-wise misreading blends a different pair and lands the vertex somewhere no tolerance absorbs. The scene group's last fixture found a live defect: `scene-no-scenes` proved that a file with **no `scenes` array** — which §3.5 permits — imported every mesh **at the origin**, because `BuildSceneGraph` returned an empty graph and the caller's "every mesh" fallback carried no placement. That is D1's failure mode surviving in the one corner the scene traversal never covered, and the corpus had no asset that could reach it (`known_bugs.md`). Every one was authored to discriminate rather than to exist — the transform rungs each defeat one plausible wrong composition, `normalized-i8-normal` is axis-aligned *because* 127 is prime and no off-axis 8-bit triple is unit length, and `stride-padded` fills its padding with a sentinel so a tightly-walking reader produces a visibly broken triangle rather than a subtly wrong one. **What remains, and why the row stays open:** the texture group (7) needs real images, the container group (7) needs external `.bin`/image sidecars the generator does not emit yet, and Draco (4) is `libdraco`-blocked. **One residue is an oracle limitation rather than a missing fixture, and it is stated where it bit:** `two-primitives-one-buffer` was authored as one mesh with two primitives, which made the L4 oracle (one instance per node-with-a-mesh) and CNA (one per primitive) disagree about the instance *count* before any geometry was compared. It is reshaped to two meshes sharing one bufferView — the accessor-sharing subject is unaffected — and closing the gap properly means teaching both the oracle and the generator to enumerate per primitive. **A second residue is a case this corpus structurally cannot hold, and it is now asserted elsewhere instead:** a document declaring `asset.version` `"1.0"`. The vendored cgltf refuses such a file at **parse** (`cgltf_result_legacy_gltf`), so the version gate is enforced before CNA's own validation ever runs — while a corpus rejection fixture must *parse*, since one a parse-only reader already rejects proves nothing about CNA's checks and would break every sweep that loads the whole corpus. The behaviour is pinned in `GltfContainerRobustness.AGltf10DocumentIsRefusedAtParseByTheVendoredReader`, with a byte-identical 2.0 control so what is refused is unambiguously the version. **One malformed case turned out to be resolved rather than refused, and the corpus now says which:** `bad-matrix-and-trs` authors both `matrix` (+4 X) and `translation` (+9 Y) on one node, which §3.5.3 forbids. The campaign's recorded decision is deterministic resolution — `cgltf_node_transform_local` honours `has_matrix`, so the matrix wins whichever order the two appear in — and the fixture's two transforms differ precisely so that choice is visible in L4 geometry rather than asserted in prose. It puts a decision that lived in one hand-authored test under the corpus-wide sweep. |
| GLTF-400 | `.glb` twin for every synthetic asset | ✔ | GLTF-399 | **Accept:** twins agree at L3/L4. **True by construction and asserted per asset.** The generator emits `<id>.gltf` and `<id>.glb` from **one** `GltfBuilder`, so a twin cannot describe a different asset — the failure mode a hand-maintained pair has. What remains checkable is that the container round-trip itself is lossless, and `GltfFixtureCorpus.EveryGlbTwinIsAValidContainerCarryingTheSameAsset` checks it for all **102** assets: the GLB parses, its §4.4 chunk layout holds (JSON first, 4-byte-aligned lengths, JSON padded with **spaces** and BIN with zeros, since the JSON padding is inside the text a parser reads), and the document it carries has the same accessor, mesh and node counts as the `.gltf`. `EveryCommittedGlbFollowsTheChunkLayoutSection44Requires` sweeps the layout separately, so a twin that parses but is malformed still fails. |
