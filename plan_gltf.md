...
| 19 | the whole corpus | `GltfConformanceL5`, `GltfConformanceL6` on two renderers | **partial**: identical on `STUB` and `HEADLESS` (`GLTF-017`/`GLTF-383`); OPENGLES3 is now available for the next parity run, while Vulkan is not |
...
| GLTF-388 | Assert L5 equality across renderers | ✅/⬜ | GLTF-168 | L5 is renderer-independent by construction. **Accept:** byte-identical; any divergence is a bug. **Asserted on STUB, HEADLESS and OPENGLES3.** `scripts/gltf-renderer-parity.sh` compares two build directories and fails on any L1–L5 difference: OPENGLES3 agrees with each of the other two on all 41 current L1–L5 tests. The byte-identity is stronger than a run-to-run diff, because each run compares against the **same committed L5 goldens** — two runs agreeing with each other could be agreeing on a shared mistake; each agreeing with a third, committed artifact cannot. Vulkan remains absent, so this row stays `✅/⬜` until the four-renderer acceptance in `GLTF-168` is met. |
| GLTF-389 | Assert L6 equality across renderers | ✅/⬜ | GLTF-382 | **Accept:** identical parameter values. **Asserted on STUB, HEADLESS and OPENGLES3:** each L6 test compares the effect parameter block against the same manifest-derived oracle, and the rung is green on all three builds. The corrected whole-selection parity additionally reports all 505 test results identical between OPENGLES3 and HEADLESS; STUB differs only by its 26 explicit ThreeD skips, never by a pass/fail result. Vulkan remains the open renderer residue. |
