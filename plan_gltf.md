...
>
> Of 460 rows: **383 are closed (`✔` 256, `✅` 127)** and **59 remain `⬜`**. The other 18 carry a
...
| GLTF-369 | `PbrEffect` does not premultiply alpha into RGB | ✅ | GLTF-368 | Deliberate and correct for glTF. **Accept:** locked with a comment-backed test. **Lock blocked in this environment** — same shader-side blocker as `GLTF-223`. Whether RGB is premultiplied by alpha is a property of the fragment program's output, which nothing on the CPU side observes. |
