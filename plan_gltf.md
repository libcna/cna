...
| GLTF-226 | Occlusion channel is `.r` | ✅ | GLTF-225 | Shader reads `.r`. **Accept:** locked with a three-distinct-channel fixture. **Lock blocked in this environment** — same shader-side blocker as `GLTF-223`. `GpuDrawParams` carries the occlusion map but not which channel a program samples, so `L6` cannot see this rule. |
| GLTF-227 | Normal-map decode `rgb*2−1` in TBN | ✅ | GLTF-175 | **Accept:** locked at L7. **Lock blocked in this environment** — same shader-side blocker as `GLTF-223`; the row itself already says L7. |
