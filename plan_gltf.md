...
>
> Of 460 rows: **384 are closed (`✔` 256, `✅` 128)** and **58 remain `⬜`**. The other 18 carry a
> deliberate partial marker — 8 `🔬` (investigation, no implementation owed), 8 `✅/⬜`, no
...
| GLTF-176 | Normal/tangent transformation under node transforms | ✅/⬜ | GLTF-118, GLTF-119 | Only relevant if `GLTF-103` chooses baking; the shader's `uNormalMatrix` needs it either way. **Accept:** `xf-scale-nonuniform` and `tangent-mirrored` correct at L4/L7. **L4/L6 normal half landed; mirrored-tangent L7 remains and is a real renderer residue.** `normal-nonuniform-scale` uses exact `Rz90*S(2,3,4)` and a slanted normal, so its manifest's inverse-transpose answer differs from both the world 3x3 and an accidental transpose; L6 captures the bound normal matrix and reproduces that answer. `tangent-mirrored` places one shared, normal-mapped tangent buffer normally and through a negative determinant: L4 states the required per-draw tangent direction and handedness, while L5 proves the local buffer remains unchanged. Inspection of the current renderer shaders shows they carry local `TANGENT.w` without multiplying by `sign(det(world))`, so the mirrored draw is not claimed correct until a rasterising L7 fix and test exist. |
