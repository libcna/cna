...
| ASCII-3 | `CNA_ASCII_MODE` env-var parsing (`BLACKWHITE`/`COLOR`, default `COLOR`) + programmatic `SetMode()` override, same pattern as `HEADLESS-5` | ⬜ | Deliberately deferred to Phase G4 — mode selection is meaningless until quantization itself exists. `Present()` currently just forwards unchanged (see ASCII-4/ASCII-40). |
...
| # | Task | Status | Notes |
|---|---|---|---|
| ASCII-20 | The game's actual `SpriteBatch`/`Texture2D`/`RenderTarget2D` draws are forwarded to the wrapped `SdlGraphicsBackend`, targeting an offscreen render target sized to the game's own logical resolution | ⬜ | |
| ASCII-21 | Confirm this offscreen target is genuinely readable back (`GetBackBufferData`-style) before Phase G4 needs it | ⬜ | |

## Phase G4 — Quantization (framebuffer → glyph/color grid)

| # | Task | Status | Notes |
|---|---|---|---|
| ASCII-30 | Grid sizing from logical resolution + configurable cell pixel size, reusing shared letterbox/virtual-resolution math | ⬜ | Design decision 6. |
| ASCII-31 | `BlackWhite` mode: per-cell luminance → glyph-density ramp (e.g. `" .:-=+*#%@"`-style) | ⬜ | |
| ASCII-32 | `Color` mode: per-cell representative foreground color (+ optional background fill) via block sampling/averaging | ⬜ | |
| ASCII-33 | `Ascii_Quantize` test: known synthetic framebuffer → asserted exact glyph/color grid output — fully automatable (Design decision 9) | ⬜ | |

## Phase G5 — Drawing the grid into the real window

| # | Task | Status | Notes |
|---|---|---|---|
| ASCII-40 | `Present()` override: draw the quantized grid as textured glyph quads (reusing `SpriteFont`/`SDL_RenderTexture` draw calls, Design decision 3/4) into the real visible window | ⬜ | |
| ASCII-41 | `Ascii_Present` pixel test: render known content, read back the *real window's* presented pixels, assert the correct glyph cells/colors appear — genuinely possible here, unlike the terminal version | ⬜ | Design decision 9. |
| ASCII-42 | (Optional, perf-only) dirty-cell diff to skip redrawing unchanged cells | ⬜ | Design decision 7 — optimization, not correctness. |

## Phase G6 — Input (verification only)

| # | Task | Status | Notes |
|---|---|---|---|
| ASCII-50 | Confirm `Mouse`/`Keyboard`/`GamePad` work completely unmodified against this backend's real window | ⬜ | §0.2 — expect zero new code. |
| ASCII-51 | Mouse coordinate mapping: physical window pixel → logical game coordinate, reusing the same `TransformWindowToLogical` mechanism every windowed backend already has (optionally also expose the glyph-cell the mouse is over, as a convenience, not a replacement for pixel coordinates) | ⬜ | |

## Phase G7 — `ThrowNo3D` reuse

| # | Task | Status | Notes |
|---|---|---|---|
| ASCII-60 | Confirm every 3D-pipeline `IGraphicsBackend` method can forward directly to the wrapped `SdlGraphicsBackend`'s own existing `ThrowNo3D` calls rather than re-declaring them | ⬜ | Design decision 10. |
