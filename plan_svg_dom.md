...
| Task | Status | Description |
|---|---|---|
| SVGDOM-S2a | ✅ | `CNA_SvgDom_EnsureRoot()` — creates `<svg id="cna-svg-dom-root">` over SDL's canvas, hides the canvas (design decision 1). |
...
| Task | Status | Description |
|---|---|---|
| — | ✅ | **Root cause found and fixed**: `emscripten_set_main_loop(fn, fps, simulateInfiniteLoop=1)` unwinds via a raw JavaScript `throw 'unwind'`; under `-fwasm-exceptions`, the compiler-generated `catch_all`/cleanup landing pad for a local object with a non-trivial destructor genuinely catches that foreign throw. Every CNA Emscripten example stack-allocated its `Game` subclass in `main()`, so that object's destructor ran for real at the `emscripten_set_main_loop` call site — deleting the real `GraphicsDeviceManager` through its owning `unique_ptr` and leaving `Game::graphicsDeviceManager_` dangling, surfacing as a WebAssembly indirect-call fault frames later. Proven with a minimal reproducing spike (`emscripten-mainloop-stack-spike/`) and confirmed end-to-end on the real code. Fixed by heap-allocating every CNA Emscripten example's `Game` subclass instead of stack-allocating it; documented in `docs/emscripten-mainloop-game-lifetime.md` and `Game::Run()`'s own doc comment. Not a `SVG_DOM`, `GraphicsDeviceManager`, or `GameServiceContainer` defect — see the spike's own `README.md` for the ruled-out `GameServiceContainer` hypothesis. |
