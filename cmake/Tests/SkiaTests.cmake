...
if(CNA_BUILD_TESTS AND NOT EMSCRIPTEN AND NOT WIN32
   AND CNA_GRAPHICS_BACKEND STREQUAL "SKIA")
    ...
    # SKIA-156: hardens the SKIA-154 mesh-effect compilation cache (bounded LRU-evicted growth)
    # and the SKIA-155 GLSL-to-SkSL translator (oversized/malformed input stays bounded, never
    # crashes or hangs) -- a failed compile never poisons the cache or a later compile, and a
    # many-iteration stress loop exercises both under this build's sanitizer configuration.
    cna_skia_test(cna_test_skia_mesh_effect_hardening
                  examples/skia_mesh_effect_hardening_test.cpp)
    target_include_directories(cna_test_skia_mesh_effect_hardening PRIVATE "${CNA_SKIA_ROOT}")
    ...
    # SKIA-145: proves (or corrects) the dominant-axis cube face/UV table
    # docs/skia-cube-volume-sampling-contract.md hypothesized, compiled and run as real SkSL
    # against real pixels. Stays below the public ShaderEffect/SetTexture(TextureCube) API,
    # matching the SKIA-93 spike precedent above.
    cna_skia_test(cna_test_skia_cube_sampling_spike
                  examples/skia_cube_sampling_spike_test.cpp)
    target_include_directories(cna_test_skia_cube_sampling_spike PRIVATE "${CNA_SKIA_ROOT}")
