...
public class Probe : Game
{
    ...
    /// of the twenty formats each kind allows is exactly what this spike exists to avoid.
    void LegOtherKinds()
    {
        var dev = GraphicsDevice;
        ...
        Say("LEG-G TextureCube formats accepted, GraphicsProfile=" + dev.GraphicsProfile);
        foreach (var f in formats)
        {
            string verdict;
            try { using (var t = new TextureCube(dev, 4, false, f)) verdict = "accepted"; }
            catch (Exception e) { verdict = "THREW " + e.GetType().Name; }
            Say("        " + f.ToString().PadRight(16) + " " + verdict);
        }

        Say("LEG-H RenderTarget2D formats accepted, GraphicsProfile=" + dev.GraphicsProfile);
        foreach (var f in formats)
        {
            string verdict;
            try
            {
                // "Accepted" is not the same as "honoured": XNA may silently substitute a format it
                // cannot give you rather than refusing. Report what the target actually IS.
                using (var rt = new RenderTarget2D(dev, 4, 4, false, f, DepthFormat.None))
                    verdict = rt.Format == f
                        ? "accepted"
                        : "SUBSTITUTED -> " + rt.Format.ToString();
            }
            catch (Exception e) { verdict = "THREW " + e.GetType().Name; }
            Say("        " + f.ToString().PadRight(16) + " " + verdict);
        }
