...
> `.cnb`-specific coverage: **61 test cases across 14 gtest suites** (`JsonParseTest`,
> `ParseCnbEnvelopeTest`, `ValidateCnbEnvelopeTest`, `CnbResolverOrderTest`, `CnbSourceFileTest`,
> `CnbSourceFileSafetyTest`, `CnbCacheIsolationTest`, `CnbCapabilityMatrixTest`,
> `CnbAssetCacheTypeSafetyTest`, `CnbSpriteFontTest`, `CnbEffectTest`, `CnbModelTest`,
> `CnbCustomLoaderTest`, plus the `CnbCustomLoaderGraphicsTest` subclass). A 47-test
> graphics-independent subset is verified headless via
> `SDL_VIDEODRIVER=dummy ./cmake-build-debug/CnaTests --gtest_filter="JsonParseTest.*:ParseCnbEnvelopeTest.*:ValidateCnbEnvelopeTest.*:CnbCustomLoaderTest.*:CnbAssetCacheTypeSafetyTest.*"`
...
| CNB-33 | Isolate metadata-transformed textures from their native source cache entry | ✅ | **Investigated, found not reproducible with the current code — verified, not assumed.** Two targeted probe tests (sidecar-then-native; native-then-sidecar with a deliberately-kept live handle, to force the weak-cache *reuse* path rather than a fresh disk reload) both show the native/live copy keeps its original, un-color-keyed pixels. Root cause of why the described mechanism doesn't hold: `Texture2D::SetData(const Color*, int)` (what `ApplyColorKey` calls) *reassigns* `backend_`/`cpuPixels_` to freshly-created `shared_ptr` targets rather than mutating the existing shared objects in place — so by the time `ApplyColorKey` runs, the `.cnb` result's copy is already effectively detached from whatever the weak cache still (weakly) references. No source fix was needed to satisfy the acceptance criteria. The 2 tests (`tests/Microsoft/Xna/Framework/Content/CnbCacheIsolationTests.cpp`) are kept as a **permanent regression guard**: if `SetData` is ever changed to mutate in place instead of reassigning (e.g. as a future GPU-upload optimization), these tests fail immediately. Full-suite regression: 4416 tests, 4414 passed, same 2 pre-existing hardware skips, 0 failures |
| CNB-34 | Define and enforce the `.cnb`/`sourceFile` capability matrix for every built-in reader | ✅ | `SoundEffectTypeReader`/`TextureCubeTypeReader` gained `.cnb`+`sourceFile` delegation (same shape as `Texture2DTypeReader`'s `ReadCnb`, no metadata fields yet). `SpriteFontTypeReader`/`EffectTypeReader`/`ModelTypeReader` gained an explicit `RejectSourceFileForSelfContainedCnb()` call right after `ValidateCnbEnvelope` — throws `ContentLoadException` naming the reader and file instead of silently ignoring `sourceFile` (the old behavior) or letting a native decoder choke on raw `.cnb` JSON. `cnb.md` gained a real capability-matrix table (`### The sourceFile capability matrix`). 5 new tests in `CnbCapabilityMatrixTests.cpp` — `SoundEffect`/`TextureCube` delegation proven with real hand-built minimal WAV/DDS fixtures (not just "doesn't throw"), 3 rejection cases for `SpriteFont`/`Effect`/`Model`. Full-suite regression: 4421 tests, 4419 passed, same 2 pre-existing hardware skips, 0 failures |
| CNB-35 | Make envelope parsing and version policy strict | ✅ | **Design decision: wrote a small, complete, correct recursive-descent JSON parser (`include/CNA/Internal/Json.hpp`, ~350 lines) rather than vendor a third-party library.** Matches this project's established pattern — the only vendored deps (SDL, easy-gl, tinyxml2, miniz, enet) are all platform-binding C libraries, never general-purpose data-format parsers; `.cnb` documents are small and fully controlled by CNA itself, so a from-scratch parser is a bounded, well-understood problem, not an open-ended one. Full grammar: objects/arrays/strings (with `\"`, `\\`, `\/`, `\b`, `\f`, `\n`, `\r`, `\t`, `\uXXXX` incl. surrogate pairs)/numbers/`true`/`false`/`null`; a strict number-token grammar makes `"1abc"` a parse error rather than a silently-truncated `1`. `CnbEnvelope.hpp`'s `ParseCnbEnvelope`/`ValidateCnbEnvelope` rewired on top of it with their struct/function signatures kept stable — confirmed by re-running all 58 pre-existing `.cnb` tests unchanged against the new parser, 0 regressions. New: strict `cnbVersion == 1` check via a new `cnbVersionRaw` double field (the existing `int cnbVersion` field truncates fractional values, which would silently defeat a `1.5` rejection); a shared `ValidateCnbEnvelopeBaseline()` used by both `ValidateCnbEnvelope()` (built-in readers) and `RegisterCnbLoader<T>`'s `GenericCnbTypeReader` — the latter previously had its own weaker duplicate check that only verified `cnbVersion` was *present*, never that it equaled 1, so `cnbVersion: 0/-1/2/1.5` were all previously silently accepted through the custom-loader path. Non-object JSON roots and malformed documents now produce a specific `ContentLoadException` instead of being indistinguishable from "all fields missing". 24 new tests: `tests/CNA/Internal/JsonTests.cpp` (14, parser-level), `CnbEnvelopeTests.cpp` (+8, envelope/version-policy-level), plus 2 end-to-end tests (one through `SpriteFontTypeReader`, one through `RegisterCnbLoader<T>`) proving the policy is actually wired through real readers, not just unit-tested in isolation. Full-suite regression: 4447 tests, 4445 passed, same 2 pre-existing hardware skips, 0 failures |
| CNB-36 | Make the general asset cache type-safe | ✅ | **Verified before fixing** (CNB-33 rigor): a throwaway probe test registering two `RegisterCnbLoader<T>` factories for different `T`s and requesting both under the same logical name confirmed `std::any_cast` throws raw `std::bad_any_cast` — before any fix existed. Fixed: `loadedAssets_` re-keyed from `std::unordered_map<std::string, std::any>` to a new `AssetCacheKey{std::type_index, std::string}` + `AssetCacheKeyHash`, so a different `T` for the same logical name is simply a cache *miss* and falls through to normal loading + `.cnb` envelope validation, producing a clean `ContentLoadException` instead. `Unload()` (a bare `.clear()`) is unaffected by the key-type change. 2 new tests in `CnbAssetCacheTypeSafetyTests.cpp`: different-`T`-same-name now throws `ContentLoadException` (not `std::bad_any_cast`); same-`T`-same-name still correctly hits the cache (factory invoked exactly once). Full-suite regression: 4449 tests, 4447 passed, same 2 pre-existing hardware skips, 0 failures |
| CNB-37 | Make custom-loader registration deterministic and fail-fast | ✅ | `RegisterCnbLoader<T>` now rejects an empty `typeName` and an empty `factory` (`std::function`'s bool-conversion check) with `std::invalid_argument`, and a duplicate `(T, typeName)` registration with `std::logic_error` — matching the existing "already owned by another reader" error's type, since both are program-logic/state errors rather than bad-argument-value errors (empty `typeName`/`factory` got `std::invalid_argument` instead, a subtype of `std::logic_error`, for a more precise catch if a caller wants one). Confirmed the duplicate-rejection doesn't silently replace the original factory — a dedicated test registers twice (second throws), then loads through the *first* factory and asserts it (not the second) actually ran. Two distinct `typeName`s for the same `T` still coexist correctly (pre-existing test unaffected). `cnb.md`'s "Custom loaders" section documents the contract. 4 new tests in `CnbCustomLoaderTests.cpp`. Full-suite regression: 4452 tests, 4450 passed, same 2 pre-existing hardware skips, 0 failures |
| CNB-38 | Make `.cnb` verification runnable without a display and suitable for CI | ✅ | Audited every `.cnb` test file: `JsonTests.cpp`/`CnbEnvelopeTests.cpp`/`CnbAssetCacheTypeSafetyTests.cpp` were already graphics-free. `CnbCustomLoaderTests.cpp` had one shared fixture forcing `GraphicsDevice` construction (confirmed via reading `GraphicsDevice.cpp`: its constructor unconditionally calls `SDL_InitSubSystem(SDL_INIT_VIDEO)` under the EasyGL backend) on all 9 tests, even though only 1 (`FactoryCanRecursivelyLoadReferencedTexture`) loads a `Texture2D` — split into `CnbCustomLoaderTest` (no `GraphicsDevice`, 8 tests) and `CnbCustomLoaderGraphicsTest : public CnbCustomLoaderTest` (adds it, 1 test). **Decision: documented headless lane uses `SDL_VIDEODRIVER=dummy` against the existing EasyGL `CnaTests` binary**, not a separate `CNA_GRAPHICS_BACKEND=HEADLESS` rebuild — a standard, real CI technique that directly demonstrates "no display server needed" using the exact same binary the rest of Phase 9 was already validated against (avoiding any risk of behavioral drift from testing a different build), at a fraction of the cost of a second ~15–20 minute full rebuild. `CNA_GRAPHICS_BACKEND=HEADLESS` (real, proven per `plan_headless.md`) remains a valid, low-cost follow-up if the project owner later wants that stronger guarantee. Verified: `SDL_VIDEODRIVER=dummy ./cmake-build-debug/CnaTests --gtest_filter="JsonParseTest.*:ParseCnbEnvelopeTest.*:ValidateCnbEnvelopeTest.*:CnbCustomLoaderTest.*:CnbAssetCacheTypeSafetyTest.*"` → 47/47 pass, zero window/SDL-video calls in the output. Real-backend integration tests (`CnbSourceFileTest`'s `colorKey` pixel test, `CnbCapabilityMatrixTest`'s `SoundEffect`/`TextureCube` decode tests, etc.) were left untouched, as intended. Full-suite regression (normal, non-headless run): 4452 tests, 4450 passed, same 2 pre-existing hardware skips, 0 failures — unchanged from CNB-37, as expected for a pure test-fixture refactor |
| CNB-39 | Update documentation and perform final Phase 9 compliance sweep | ✅ | `cnb.md` updated: added the "Enforced safety guarantees" paragraph documenting `CNB-32`'s `ResolveCnbSourceFileSafely()` contract (rejects absolute paths, root escapes, `.cnb` chains, and extensionless self-cycles), and its own top status banner corrected to reflect all 9 phases / `CNB-1`–`CNB-39`. Doxygen spot-check: `include/CNA/Internal/CnbSourceFile.hpp` (4 `/**` blocks, matches its 4 public members) and `include/CNA/Internal/Json.hpp` (15 `/**` blocks on the public `JsonType`/`JsonValue`/`JsonParseException`/`ParseJson` surface; the internal `Detail::JsonParser` intentionally undocumented, matching CLAUDE.md's public-API-only rule) both confirmed adequate. Final full-suite regression: **4452 tests, 4450 passed, same 2 pre-existing hardware skips, 0 failures** (`grep -c FAILED` on the full log confirms 0). Final headless re-check: `SDL_VIDEODRIVER=dummy` targeted run → **47/47 pass**. Final dedicated `.cnb` count via `--gtest_filter="*Cnb*" --gtest_list_tests`: **61 test cases across 14 suites**. Milestone M5 reached. |
