...
| WAYLAND-0117 | Stress and soak | 🟨 | ≥ 10,000 window/state operations, lock/unlock, fullscreen, resize, clipboard, GL/Vulkan/shm lifetimes. 3000 random window operations clean on Weston and on headless GNOME, and 2000 under ASan; `soak` runs a rendered session with state changes. Open: the ≥ 10,000-operation run and a long soak on the real desktop. |
