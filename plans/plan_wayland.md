...
| D-16 | **`text-input-v3` provides composition when the compositor offers it; `ime` is true only when it does** -- preedit as `TextEditingEvent`, commit as `TextInputEvent`, `delete_surrounding_text` applied to the preedit. Candidate lists are drawn by the input method and never delivered (as under XIM). | The contract's IME events are what the protocol carries, apart from candidates, which X11's D16 already records as undeliverable. |
...
| D-24 | **Vulkan through `VK_KHR_surface` + `VK_KHR_wayland_surface`**, `vkCreateWaylandSurfaceKHR` resolved through the caller's `vkGetInstanceProcAddr` (the lookup X11 already had, moved to `src/Common/`). | No Vulkan loader in the link closure (D-1). |
...
| D-27 | **Shared Linux services** -- evdev gamepads/joysticks/haptics, power and host information, the desktop portal (file dialogs, OpenUrl, parent window via `xdg-foreign`), D-Bus -- are the X11 backend's, moved to `src/Linux/` and compiled for both. | Controllers, power and the portal are not window-system features. |
...
| messageBox | ⬜ | decided in WAYLAND-0094 |
...
| WAYLAND-0004 | Baseline test results before any change | ⬜ | X11 mapping, integration and window-manager suites on `cmake-build-x11`; platform conformance. |

### Phase B — shared, window-system independent code

| ID | Task | Status | Acceptance criteria / Notes |
|---|---|---|---|
| WAYLAND-0010 | XKB mapping to `src/Xkb/` | ⬜ | `ScancodeFromXkbKeyName`, `KeyCodeFromKeysym`, `UsLayoutKeyCode`, `BuildKeyCodeTable` in a directory compiled for X11 and Wayland, with no X or xkbcommon header in it. X11 keeps its names as aliases; X11 mapping suites unchanged and green; new `XkbKeyMapping` tests. |
| WAYLAND-0011 | D-Bus and desktop portal to `src/Linux/` | ⬜ | `X11DBus` → `Linux/DBusLibrary`, `X11DesktopPortal` → `Linux/DesktopPortal`, the parent window passed as the portal's own string (`x11:<xid>` / `wayland:<handle>`). X11 portal suites unchanged and green. |
| WAYLAND-0012 | Session-bus screen-saver inhibition to `src/Linux/` | ⬜ | The `org.freedesktop.ScreenSaver` half of `X11ScreenSaverInhibitor`; X11 keeps Xss and the timeout fallback. |
| WAYLAND-0013 | CPU frame fitting to `src/Common/` | ⬜ | `ComputePresentRect` and the RGBA→XRGB8888 scale/convert loop, shared by the X11 and Wayland presenters; X11 presenter tests green. |
| WAYLAND-0014 | Monotonic clock and Vulkan loader lookup to `src/Common/` | ⬜ | X11 conformance timing tests green. |
| WAYLAND-0015 | Build `src/Linux/` for WAYLAND | ⬜ | `CNA_PLATFORM_HAVE_EVDEV` for both selections. |
| WAYLAND-0016 | X11 regression after the extraction | ⬜ | Same results as WAYLAND-0004. |
...
| ID | Task | Status | Acceptance criteria / Notes |
|---|---|---|---|
| WAYLAND-0110 | `CnaWaylandMappingTests` | ⬜ | No compositor. |
| WAYLAND-0111 | In-process test compositor and `CnaWaylandProtocolTests` | ⬜ | libwayland-server in the test binary; scripted configures, input, seat/output hot-plug, clipboard owner loss, protocol-error detection. |
| WAYLAND-0112 | Weston launcher and `CnaWaylandWestonTests` | ⬜ | `tools/platform/wayland_test_server.sh`: private runtime dir, readiness, teardown, no leftovers. |
| WAYLAND-0113 | Headless gnome-shell launcher and `CnaWaylandMutterTests` | ⬜ | Private bus, RemoteDesktop input. |
| WAYLAND-0114 | Real-desktop validation harness | ⬜ | `cna_wayland_desktop_validation`. |
| WAYLAND-0115 | SDL-free and X11-free source scan, ratchet | ⬜ | `WaylandIsSdlFree`; `src/Wayland/` on the ratchet's denylist. |
| WAYLAND-0116 | Sanitizers | ⬜ | ASan + LSan (+ UBSan) over every Wayland suite. |
| WAYLAND-0117 | Stress and soak | ⬜ | ≥ 10,000 window/state operations, lock/unlock, fullscreen, resize, clipboard, GL/Vulkan/shm lifetimes. |

### Phase L — validation and regression

| ID | Task | Status | Acceptance criteria / Notes |
|---|---|---|---|
| WAYLAND-0120 | Real GNOME desktop validation | ⬜ | |
| WAYLAND-0121 | Hardware EGL/OpenGL | ⬜ | radeonsi, not llvmpipe. |
| WAYLAND-0122 | Hardware Vulkan + validation layers | ⬜ | RADV, zero CNA-caused validation errors. |
| WAYLAND-0123 | SDL-free proof | ⬜ | `CNA_ENABLE_SDL=OFF`; `ldd`, `readelf -d`. |
| WAYLAND-0124 | A real CNA application | ⬜ | |
| WAYLAND-0125 | SDL3 regression | ⬜ | |
| WAYLAND-0126 | X11 regression | ⬜ | |
| WAYLAND-0127 | Win32 cross-build regression | ⬜ | |
| WAYLAND-0128 | Broad platform matrix and gates | ⬜ | |
| WAYLAND-0129 | Error audit | ⬜ | |
| WAYLAND-0130 | `docs/platform-wayland.md` and final audit | ⬜ | |
