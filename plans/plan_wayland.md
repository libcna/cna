...
| `wp_presentation` | v2 | v2 | v1 | presentation timing (offered by all three, **not** bound: D-30) |
| `zxdg_exporter_v2` | v1 | v1 | — | portal parent window |
| `zwp_tablet_manager_v2` | v1 | v1 | — | pens, as touches (WAYLAND-0059) |
...
| D-31 | **A tablet tool is a touch; nothing else about it is delivered** (WAYLAND-0059). | The contract has no tablet service, and the X11 backend already answered the question (X11-0155): a pen whose tip is down is a touch with its pressure, a hovering pen is nothing. Doing the same over `zwp_tablet_v2` means a game that reads `TouchPanel` behaves identically on both backends and needs no tablet code of its own. Tilt, rotation, the slider, the wheel, the barrel buttons and the pad carry nothing the contract has, so they are dropped rather than invented as a private API -- the `zwp_tablet_pad_v2` object is destroyed as soon as it is announced, which also stops its traffic. Revisit if the contract grows a pen service. |
...
| WAYLAND-0059 | Tablets | ✅ | `zwp_tablet_manager_v2` per seat; a tip that is down is a touch with its pressure and a hovering tool is nothing (the X11 rule, X11-0155), one contact id per tool above the seat's global name, frames applied together, and proximity out / tool removed / tablet removed / window destroyed each `Cancelled` the stroke. Tools list as `InputDeviceKind::Touch` by their protocol type name. Tilt, rotation, slider, wheel, barrel buttons and the pad are not delivered and the pad object is destroyed at once (D-31). 7 protocol tests: `AHoveringPenIsNothingAndAStrokeIsATouchWithItsPressure`, `APenTakenOffTheTabletMidStrokeCancelsItsContact`, `ATabletUnpluggedMidStrokeCancelsItAndItsToolGoesAway`, `WhatTheContractDoesNotCarryIsNotInvented`, `APenIsATouchDeviceAndOneAttachedLaterIsFound`, `APenWithoutPressureReportsAFullContact`, `AStrokeOnAWindowThatGoesAwayIsForgotten`. |
...
| WAYLAND-0124 | A real CNA application | ✅ | `cna_demo_2d` -- a real XNA-shaped CNA game (a `Game` subclass with `Update`/`Draw`, `SpriteBatch`, a `Texture2D` and a `SoundEffect` through `ContentManager`) -- configured for this backend with nothing of SDL in it: `build-probe/wayland0124-software`, `CNA_PLATFORM=WAYLAND -DCNA_GRAPHICS_RENDERER=SOFTWARE -DCNA_AUDIO_PLATFORM=ALSA -DCNA_ENABLE_SDL=OFF`, so the window, the events, the pixels (`wl_shm` presenter) and the audio are all CNA's own. 600 frames on private Weston: `wall=10.63 s` (about 60 fps), assets loaded, exit 0, RSS 97 MB; the same 600 frames on the **real GNOME 48 session** (`WAYLAND_DISPLAY=wayland-0`): `wall=10.19 s`, exit 0, RSS 105 MB. `cna_demo_input` (platform input) and `cna_demo_sound` (three `SoundEffect`s to the null device, no audible sound) also run on this backend. Recorded, not a defect: the demo's link closure names `libX11`/`libxcb` **transitively through FFmpeg's `libavutil`** (the optional video backend), never through CNA -- no `NEEDED` entry of the executable or of any CNA library is an X11, xcb or SDL library, which is what `CnaWaylandLinkClosure` checks. Earlier renderer evidence stays in WAYLAND-0080/0081/0082 (the same game on `OPENGL33` and `VULKAN`). |
...
| WAYLAND-0128 | Broad platform matrix and gates | 🟨 | Gates: `sdl_inventory` (the §2 table regenerated: one more test file mentions SDL, the Wayland containment assertion), `sdl_classify`, `renderer_sdl_audit`, `sdl_ratchet` (0 files, 0 references), `hot_path_lint`, `nonproduction_sdl_audit`, `check_contract` -- all exit 0. Selections built and tested: WAYLAND, X11, SDL3, WIN32 (cross + Wine). SDL2, HEADLESS and TERMINAL also configure and build clean in `build-probe/wayland0128-<selection>` (`cmake` rc=0, `cmake --build` rc=0, zero `error:`): HEADLESS and TERMINAL run their platform suites green, and SDL2 (with `CNA_AUDIO_PLATFORM=SDL2`) passes 338/339 with one skip that needs a controlling terminal. **A defect found here and fixed:** `CNA_PLATFORM=SDL2` with the *default* `CNA_AUDIO_PLATFORM=SDL3` configured and built a binary linking **both** SDL2 and SDL3 -- `ldd` showed `libSDL2-2.0.so.0` and `libSDL3.so.0` -- which then refused `AcquireSubsystem(Audio)` and died on a jump through a null vtable at the first window; the reverse pair failed with CMake's own "SDL2::SDL2 target was not found" rather than a diagnostic. `cmake/Sdl2OnlyConfiguration.cmake` already explains why one process cannot hold two SDL majors (identical entry points; each backend binds to whichever library the loader reached first), so both pairs are now refused there at configure time, each naming an audio platform that works instead, and `cmake/Tests/Sdl2OnlyRendererGate.cmake` (ctest `CnaSdl2OnlyRendererGate`) checks both refusals and that seven single-SDL combinations still configure. Left for the SDL2 owner, recorded not fixed: `CNA_PLATFORM=SDL2` with `NULL` or `ALSA` audio is not treated as an SDL2-only configuration, so the *test* layer still links SDL3 into that binary. |
| WAYLAND-0129 | Error audit | ✅ | Every throw and every silent failure in `src/Wayland/` read against the contract: 53 throws (`PlatformException` for a failure, `PlatformNotSupportedException` naming the capability for a refusal), each traced for reachability from a `wl_*` listener, and every non-exception failure path (connection death, clipboard transfers, shm and cursor-theme allocation, the EGL/Vulkan `dlopen`s) checked against "a query answers, an action refuses loudly". **No throw is reachable from a protocol callback** -- verified listener by listener (registry, seat, output, surface, xdg_surface/toplevel, pointer, keyboard, touch, tablet, text-input, data device/offer/source, buffer, activation, exported, frame callbacks) down to their leaves; a listener that cannot do its work drops it and posts an event instead. No throwing `std` call (`at`, `stoi`, `stod`, `value()`, `std::get`) appears in the backend or in the shared helpers listeners call; allocation is left to terminate as anywhere else. Nine defects found and fixed in this pass: (1) `openGlContext` was reported from "the libraries loaded" without ever initialising an EGL display, so a broken GL install advertised the capability and failed later at `CreateContext` -- the display is now initialised when the platform is created, as X11 asks GLX (about 5 ms per platform; the first context needs it anyway); (2) `SetCursor(CursorImage)` worked while `cursorShapes` was false, contradicting `IPlatformMouse` -- the capability is now either half (system shapes *or* `wl_shm` images) and the half a compositor lacks refuses by name; (3) `ChooseConfig`/`EnsureSurface` failures reached from `MakeCurrent` reported `CreateContext` as the operation; (4) an unchecked `fcntl(O_NONBLOCK)` could leave a blocking `write` inside the `wl_data_source.send` callback, and the same in the paste read's bounded wait -- both now drop the transfer; (5) the message-box refusal quoted this plan's task id at the user instead of a cause; (6) a compositor with neither decorations nor `wl_subcompositor`/`wl_shm` left a window with no title bar and no diagnostic -- said once on stderr; (7) `IsScreenSaverEnabled` reported the request rather than what happened when no inhibition was available (X11 answers from the live inhibition); (8) a theme missing the requested shape left the previous cursor on screen -- falls back to the arrow every theme has; (9) three messages named no cause (`create_surface` without the connection error, a malformed cursor image without saying which part, EGL unavailability guessed at instead of reporting what the probe found). Documented in `docs/platform-wayland.md`: the no-throw-in-a-callback rule, and that a global the compositor withdraws is reported as a refusal because the capability set is fixed for the platform's lifetime. |
