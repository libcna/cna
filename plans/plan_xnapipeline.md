...
> **Status (2026-09-09).** **84 tasks: 81 done, 0 partial, 2 blocked, 1 open.** The two BLOCKED
> rows are blocked on software that does not exist in this environment and cannot be vendored
> (`XNAP-34`, a genuine Microsoft XNA 4.0 runtime; `XNAP-A4`, a genuine `fxc`), and each names
> that blocker in its own row. The one OPEN row, `XNAP-A6b`, **is** locally actionable, which is
> why this sentence no longer says nothing is: it was added on the `webgpu` branch without
> recounting the table, and `tools/xnb/check_plan_status.py` (ctest
> `CnaXnbPlanStatusConsistency`) failed on the merged tree when that branch came into `next`
> on 2026-09-09 -- which is the gate doing exactly what it exists for. There are no partial
...
| `XNAP-A6b` | `HostProcess` breaks the Emscripten link of `cna-content`. | [ ] **Found 2026-09-06 while verifying the WebGPU Emscripten target** (`plans/plan_webgpu.md`), and recorded here rather than worked around there, because it is not a renderer bug at all. `modules/content-pipeline/src/HostProcess.cpp` guards only `_WIN32`: every other platform takes the `<spawn.h>` branch, and `wasm-ld` then fails with `undefined symbol: posix_spawnp` linking `cna-content`. Emscripten has no process-spawning implementation, so this is not a missing symbol to find but a capability wasm does not have. **Renderer-independent by construction:** the failing link is `cna_content_pipeline` into the content tool, and no renderer target participates -- any Emscripten build of `cna-content` fails identically whichever renderer is selected. Introduced by `9a5e801b0` (`XNAP-A1`..`A4`), so it predates the WebGPU compiled-effect work that surfaced it. **What it costs today:** the WebGPU wasm build stops at `cna-content`; every other Emscripten target, including `cna_renderer_webgpu` and `cna_demo_2d`, builds and the browser tests run. **What it needs:** an `__EMSCRIPTEN__` arm that refuses `HostProcess` by name the way the rest of the pipeline refuses an absent capability, so the external-compiler route reports "not available on this platform" instead of failing to link. Not fixed here: this document owns that route. |
