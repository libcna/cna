...
| **D3D11** | yes | any valid `VertexDeclaration`, including declarations split across several per-vertex/per-instance streams | **none after `DX-221`/`DX-222` (2026-09-09)**; the complete declaration and stream shape enter the input-layout cache key | invalid native formats/usages are refused before submission |
| **D3D12** | yes | same, translated into the PSO input layout | **none after `DX-221`/`DX-222` (2026-09-09)**; the complete declaration and stream shape enter the PSO cache key | same |
...
translator was completed by `plans/plan_dx.md` `DX-221` on 2026-09-07. `DX-222` completed the
multi-stream and instanced D3D input work tracked by `REMED-GFX-207` on 2026-09-09.
...
| `REMED-GFX-207` | **D3D11 + D3D12 — DONE 2026-09-09 by `DX-222`** | Every active declaration element is mapped from the combined layout to its stream-local offset and actual native slot. Both renderers bind and explicitly clear all 16 slots with independent stream strides/offsets; instance step rates remain per stream. The D3D11 input-layout and D3D12 PSO keys contain the full slotted layout. The 48-test shared ordinary/instanced multi-stream matrix passes on each backend, including stale-slot, non-contiguous-slot and `REMED-GFX-123` offset/rate regressions. | COMPLETE |
...
**D3D12 proof unblocked.** `DX-235`'s public `HeadlessEXT` harness executes the shared fixture through
`GraphicsDevice`, so `REMED-BUILD-012` is no longer a dependency of this vertex-input task. Native
swap-chain/window behavior remains a separate Windows-only residue, but it does not participate in
vertex declaration translation or input-assembler binding. `DX-222` therefore closes the D3D12 leg
from real vkd3d-proton pixels rather than source inspection.
...
4. ~~**D3D11 / D3D12** (`-207`) — two backends, two cache models, and the stale-slot rule.~~
   **DONE 2026-09-09 by `DX-222`; D3D12 proof uses the public HeadlessEXT path.**
...
- **`REMED-BUILD-012`** — HIGH, P1, `NOT STARTED`, a Wine/vkd3d-proton **dev-environment limitation,
  ...
  verification. `DX-235`'s public HeadlessEXT harness removed it from `REMED-GFX-207`'s D3D12
  runtime proof; it stays where it is for the genuinely windowed tickets and is recorded here only
  because those entries still depend on it.
...
| `REMED-GFX-207` | **D3D11 + D3D12 — vertex input — DONE 2026-09-09** | Backend capability completion | **Resolved by `DX-222`.** Both report multi-stream support and render the shared 48-test matrix through the public path | MEDIUM / P2 | **DONE** | NO | NO | GFX-201, GFX-202, GFX-123 preserved | `DX-222` | **COMPLETE** | Both cache models include the full slotted layout; stale slots are explicitly cleared; D3D12 uses HeadlessEXT rather than BUILD-012 |
...
| `REMED-GFX-212` | **Vulkan, WebGPU, D3D11 and D3D12 — ALL DONE** *(D3D completed 2026-09-09 by `DX-222`)* — stock instanced shader | **Supported-path silent wrong result, resolved.** Reference settles it: case A | **Resolved.** Every named backend now multiplies the instanced route by the bound COLOR0 stream when `VertexColorEnabled` is true | LOW / P3 | **DONE** | **NO — RESOLVED** | NO | FNA reference answered; D3D runtime proof from `DX-222` | Complete | COMPLETE | FNA's shader index has no instancing term, so ordinary and instanced routes share vertex-colour semantics. D3D11/D3D12 use the shared `InstancedColored3d` HLSL variant and pass the ordinary-versus-instanced pixel oracle plus the full instanced colour suites under DXVK/vkd3d-proton. |
...
6. **Implement the remaining backend capability tickets while touching their backend modules** —
   ...
   `REMED-GFX-204` (abstraction path) → ~~`REMED-GFX-207` (D3D11/D3D12)~~ **DONE** → `REMED-GFX-208` (D3D9
