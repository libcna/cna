...
| FX-126 | A downward-facing surface takes a directional light in EasyGL that Direct3D 9 leaves black | FX-125 | **Open (2026-08-28, found by SAMPLE-047 PickingSample).** At camera angles that dip below the table -- 240 deg and 300 deg in that sample's own comparison -- XNA shades the tabletop's underside essentially black, (4,2,1), and EasyGL shades it dark brown, (68,21,4). Frame agreement drops from 97.35 %% at the angle the sample opens at to 92.4 %%, and unlike the residue at 185 deg this one is NOT boundary noise: only 93.1 %% of pixels are within 32 levels. **Bounded by measurement, and everything cheap is already excluded.** Coverage and centroid are identical to the digit (47.820 %% vs 47.821 %%), so it is the same surface rather than different geometry. With **ambient light only** the two frames agree on **99.99 %%** of pixels, so geometry, texture, UVs and the ambient term are all exact -- the divergence is entirely in the directional diffuse term. With a **single** directional light enabled CNA still lights the surface and XNA still does not, so it is not one particular light. The original's near plane (`.01f` against a far plane of 1000) is not the cause: widening it to 1.0 in both engines changes the number by 0.00. Forcing `CullNone` or `CullCounterClockwise` in both engines changes it by 0.00 either. The world-matrix determinants match in both engines and are **positive** for every mesh, so no mirroring flips a normal, and `BindDrawParams` computes `uNormalMatrix` as the proper inverse-transpose by cofactors rather than the raw upper-left 3x3. What remains is the SIGN of the transformed normal on those faces, which is where the next session should start: dump the interpolated normal for one pixel of that region in both engines rather than reasoning about the matrix. Not guessed at and not worked around; SAMPLE-047 ships at the angle it opens at |
