...
| FX-119 | Build with `CNA_EASYGL_COMPILED_EFFECTS` OFF again | FX-118 | **Done (2026-08-27, reported against `next`).** `FX-117`'s depth-range work (commit b718f950a) left the default configuration unbuildable in two places, both of the same shape: `viewportMinDepth_`/`viewportMaxDepth_` were declared inside the `#if defined(CNA_EASYGL_COMPILED_EFFECTS)` block while `EasyGLRenderer::SetViewport` writes them unconditionally, and `IsDepthTestedOnTheSameScaleAsStockGeometry` was appended AFTER the test file's own `#endif`, where the helpers it calls do not exist. The macro is OFF by default, so neither showed up in any build that runs the compiled-effect suite. The two members now live outside the guard (the viewport's depth range is not a compiled-effect concept -- narrowing it is) and the test lives inside it. Reproduced and verified in `cmake-build-video-off` (OPENGLES3, macro OFF): the reported `'viewportMinDepth_' was not declared in this scope` comes back when the declarations are moved back inside the guard, and the whole tree compiles with them outside it. **Nothing in this repository builds with the macro OFF as part of its normal checks**, which is why a two-line oversight survived; a gate for that is not written here |
