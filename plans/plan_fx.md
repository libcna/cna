...
- Status: **Six backends supported and gated: FNA3D, SDL_GPU, the EasyGL/OpenGL family, Vulkan,
  DirectX 11 and DirectX 12. The FNA3D vertical slice was declared usable (`FX-057`) on 2026-08-15; a repair pass on
  ...
  2026-09-10 (`FX-134`). `CompiledEffects` is true on all six and is justified by
  ...
  DirectX 9 and Metal remain. DirectX 9 already has a verified MinGW/Wine+DXVK renderer loop on
  this Linux machine, while Metal still requires macOS. The current next ready task is `FX-070`**
...
- additional renderer implementations (`FX-061`–`FX-071`). SDL_GPU (`FX-061`, `FX-071`),
  ...
  **done** -- real draw routes through the public `Effect`/`GraphicsDevice` API, golden-pixel tests,
  ...
  DirectX 12 (`FX-134`), Metal (`FX-066`) and DirectX 9 (`FX-070`,
  structurally the smallest of them because it consumes the effect's shader bytecode untranslated).
  DirectX 12 can be verified through CNA's fixed MinGW/Wine+vkd3d-proton loop; DirectX 9 still
  lacks an established executable loop and Metal requires macOS. Every
...
| FX-069 | Publish the final cross-renderer support matrix and project-wide completion definition | FX-061, FX-062, FX-063, FX-065, FX-066, FX-067, FX-068, FX-070, FX-134 | **Blocked on two incomplete backend tasks, with only Metal necessarily platform-gated.** Six production backends are done and verified on executing devices (`FX-061` SDL_GPU, `FX-062` EasyGL, `FX-063` DirectX 11, `FX-065` Vulkan, `FX-134` DirectX 12, plus FNA3D from `FX-057`). It cannot close until `FX-070` (DX9) and `FX-066` (Metal) have passed or reached an explicit unsupported conclusion. The interim matrix in section 10.2 identifies which rows are measured and which are unwritten. |
...
**Not yet closed**, and the plan says so rather than declaring victory on six backends. The
2026-08-18 closure pass added Vulkan, `FX-063` added DirectX 11 on 2026-09-09, and `FX-134` added
DirectX 12 on 2026-09-10. Two remain: DirectX 9 and Metal. CNA's existing DirectX 9 renderer has an
executable MinGW/Wine+DXVK loop; Metal requires macOS. Writing any backend blind behind a capability
gate remains forbidden -- a backend is promoted on executed evidence or not at all.
...
| SDL_GPU / EasyGL / Vulkan / DirectX 11 / DirectX 12 with the option off | false | The runtime is not compiled in; the capability reports false and construction refuses by name |
...
- **DirectX 9 (`FX-070`) and Metal (`FX-066`) are unwritten.** DirectX 9 can use the renderer's
  established MinGW/Wine+DXVK loop; Metal requires macOS. Their rows carry the concrete
  implementation and evidence requirements.
- `FX-069`'s final cross-renderer matrix depends on those two and on nothing else.
