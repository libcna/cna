...
- Status: **Five backends supported and gated: FNA3D, SDL_GPU, the EasyGL/OpenGL family, Vulkan,
  and DirectX 11. The FNA3D vertical slice was declared usable (`FX-057`) on 2026-08-15; a repair pass on
  ...
  adapter on 2026-09-09 (`FX-063`). `CompiledEffects` is true on all five and is justified by
  ...
  DirectX 12, DirectX 9 and Metal remain. DirectX 12 has a verified MinGW/Wine execution loop on
  this Linux machine, while DirectX 9 has not yet established one and Metal still requires macOS.
  The current next ready task is `FX-134`**
...
| FX-069 | Publish the final cross-renderer support matrix and project-wide completion definition | FX-061, FX-062, FX-063, FX-065, FX-066, FX-067, FX-068, FX-070, FX-134 | **Blocked on three incomplete backend tasks, with only Metal necessarily platform-gated.** Five production backends are done and verified on executing devices (`FX-061` SDL_GPU, `FX-062` EasyGL, `FX-063` DirectX 11, `FX-065` Vulkan, plus FNA3D from `FX-057`). It cannot close until `FX-134` (DX12), `FX-070` (DX9) and `FX-066` (Metal) have passed or reached an explicit unsupported conclusion. The interim matrix in section 10.2 identifies which rows are measured and which are unwritten. |
...
| FX-134 | Implement and gate DirectX 12 compiled XNA effects | FX-030, FX-060 | **Open (created by `plans/plan_dx.md` `DX-248`, 2026-09-09).** D3D12 has no MojoShader adapter, so this task starts with an executable existence gate on CNA's already-verified MinGW/Wine+vkd3d-proton loop, using the fixed `sharp-runtimenext` checkout: enable MojoShader's HLSL profile, parse the committed conformance fixture, and prove whether its Shader Model 3-era HLSL can be compiled by `D3DCompile` into bytecode accepted by CNA's D3D12 device. The gate must render and read back a minimal offscreen draw; parser or compiler success alone is insufficient. If feasible, implement `D3D12CompiledEffect` as an `ICompiledEffectRuntime` with CNA's own nine-function `MOJOSHADER_effectShaderContext`, route reflection, constants, textures, samplers, pass state and PSO/descriptor ownership through the frame/fence lifetime model established by `DX-237`/`DX-238`, and run every public-path contract listed by `FX-060`/`FX-063`. `SupportsCompiledEffects()` remains false until the complete shared suite passes; an infeasible compiler/profile result must remain a named refusal with measured evidence, never a stock-effect fallback. Native Windows verification belongs to `plans/plan_dx.md` `DX-114`, not this local implementation gate. This task adds no modern `CNAEXT` graphics API. |
...
**Not yet closed**, and the plan says so rather than declaring victory on five backends. The
2026-08-18 closure pass added Vulkan, and `FX-063` added DirectX 11 on 2026-09-09. Three remain:
DirectX 12, DirectX 9 and Metal. CNA has an executable MinGW/Wine+vkd3d-proton loop for DirectX 12;
DirectX 9 still lacks an established local loop and Metal requires macOS. Writing any backend blind
behind a capability gate remains forbidden -- a backend is promoted on executed evidence or not at
all.
...
| DirectX 12 (`FX-134`) | false | Not implemented. The current MinGW/Wine+vkd3d-proton loop can execute an existence gate and public offscreen contract; feasibility of the MojoShader HLSL output remains deliberately unclaimed until that gate runs |
...
| SDL_GPU / EasyGL / Vulkan / DirectX 11 with the option off | false | The runtime is not compiled in; the capability reports false and construction refuses by name |
| every other identity | false | Section 10.3: planned, assessed-feasible, or unsupported by design |

What is still open before 10.2 can close (updated 2026-08-18, amended 2026-09-09):

- **DirectX 12 (`FX-134`), DirectX 9 (`FX-070`) and Metal (`FX-066`) are unwritten.** DirectX 12
  has a current local execution loop; DirectX 9 must first establish its proposed Windows or
  DXVK-native loop, and Metal requires macOS. Their rows carry the concrete implementation and
...
| DirectX 12 | HLSL profile plus `D3DCompile`, with no MojoShader adapter; first prove the accepted shader model by a real offscreen draw, then supply CNA's binding/PSO/descriptor layer | `FX-134` |
