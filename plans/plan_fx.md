...
- Status: **Seven backends supported and gated: FNA3D, SDL_GPU, the EasyGL/OpenGL family, Vulkan,
  ...
  suite's own gating. DirectX 11 then passed the same public-path gate through MojoShader's D3D11
  adapter on 2026-09-09 (`FX-063`), followed by CNA's own HLSL/D3DCompile D3D12 backend on
  2026-09-10 (`FX-134`), then by the native-token DirectX 9 backend on the same day (`FX-070`).
  `CompiledEffects` is true on all seven and is justified by
  drawing rather than by state inspection. A later sample-mask edit broke the option-on Vulkan
  translation unit; `FX-129` repaired that compile regression on 2026-09-08 without relabelling the
  older runtime matrix. Every unsupported renderer identity is
  `CompiledEffects == false` and refuses by
  ...
  only Metal remains, and its executable gate requires macOS. There is no remaining Linux-executable
  backend rollout task; `FX-066` is the next backend task when a real Metal device is available.**
...
- additional renderer implementations (`FX-061`–`FX-071`). SDL_GPU (`FX-061`, `FX-071`),
  EasyGL/OpenGL/OpenGL ES (`FX-062`), Vulkan (`FX-064` prototype, `FX-065` renderer), and DirectX 11
  (`FX-063`) are all
  **done** -- real draw routes through the public `Effect`/`GraphicsDevice` API, golden-pixel tests,
  ...
  and hit five of the same defect shapes in its own code on the way (see `FX-065`). DirectX 12
  (`FX-134`) and DirectX 9 (`FX-070`) are now also done through CNA's fixed MinGW/Wine loops.
  Only Metal (`FX-066`) remains, and it requires macOS. Every
  renderer identity is now classified
...
| 2 | DirectX 11 | MojoShader D3D11 adapter | Done (`FX-063`) |
...
| 4 | D3D12 and middleware renderers | HLSL/SPIR-V/GLSL ingestion depending on renderer | D3D12 done (`FX-134`); middleware remains per-renderer |
...
| FX-005 | Extend the FNA reference tool to emit normalized JSON reflection and deterministic pixels/state observations for all fixtures | FX-003, FX-004 | **Done (2026-09-10).** `--effects` runs FNA's own `Effect.INTERNAL_parseEffectStruct` over all seven committed binaries and `StockFixtureReflectionMatchesTheFnaOracle` compares CNA's reflection subtree by subtree. `--effect-states` builds a real managed FNA `GraphicsDevice`, applies every technique/pass, records the public blend/depth/rasterizer/sampler properties installed by FNA's `PipelineCache`, and the CNA replay matches every observation. The new `--effect-pixels` completes the independent execution half: it renders all seven binaries and all nine passes into a clean 8x8 target with deterministic flat resources, records the changed-pixel count plus three interior RGBA samples, and fails generation if a pass draws nothing. All nine FNA passes changed 64/64 pixels; two runs were byte-identical (`SHA-256 b3045bf35de9e5744102d0e4225be9148a559bdec4a7370fa0439a8f075bdab8`). `Fna3dEffectPixelOracleTest.EveryCompilerProducedPassMatchesFnaPixels` replays the exact public `Effect`/`GraphicsDevice` path and matches the checked-in FNA output with an explicit three-byte per-channel tolerance. Measurement used managed FNA 26.05 with its matching FNA3D 26.05/MojoShader revision, llvmpipe OpenGL 4.5 and `glsl120`, private headless display `:4`, and SDL's offscreen video driver; no physical display was used. This corrected a false documentation premise: managed FNA 26.05 is not executable-effect compatible with CNA's FNA3D 26.08 pin. Both executable oracle modes now reject that mismatch through `FNA3D_LinkedVersion` instead of crashing. |
...
| FX-063 | Implement and gate DirectX 11 through the MojoShader D3D11 adapter | FX-030, FX-060 | **Done (2026-09-09) -- `SupportsCompiledEffects()` is true in the opt-in build.** `CNA_DIRECTX11_COMPILED_EFFECTS` is off by default and configures the FNA3D-pinned MojoShader HLSL profile only when requested. `D3D11CompiledEffect` owns the parsed Effect Framework runtime against MojoShader's native D3D11 adapter, reuses the renderer-neutral reflection/state translation, retains parameter/texture/technique state across clone and device recreation, and caches input layouts by the applied vertex shader plus the caller's reflected declaration. Ordinary buffered/user, indexed/non-indexed, multi-stream, and instanced draws all leave the stock path for the applied compiled shader pair. SpriteBatch executes every public technique pass, overrides slot zero with the sprite source, inherits the embedded stock SpriteEffect vertex stage for pixel-only passes, and avoids applying its CPU transform twice. The binding route resolves 2D, render-target, cube, and volume SRVs; applies sampler filtering/addressing/anisotropy/mip clamp/LOD bias to pixel and vertex stages; and explicitly rebinds MojoShader's current pair before `ProgramReady`, because intervening stock/`ShaderEffect` draws otherwise replace the native D3D11 shaders behind the adapter's state cache. **Executed gate:** all **18/18** focused tests pass with **0 skips**, covering the backend contract, every compiler-produced stock effect, the full draw matrix, multi-stream, instancing, SpriteBatch and multi-pass ordering, texture slots, sampler pixels, pass selection, render-target sources, stock isolation, orientation, effect switching, cube/volume sampling, 600 draws, and every truncation boundary. The complete DirectX 11 label passes **267/267 in 1339.00 seconds**, including smoke **21/21**. Both opt-in ON and OFF capability tests pass; OFF builds the renderer without MojoShader and reports false. **Environment:** MinGW cross-build with fixed `sharp-runtimenext` `0c82d9b888bdf5f7d5663c77942f339bcb2a7445`, FNA3D `3240147`, MojoShader `6333f74`, Wine+DXVK 2.6.0 on AMD Radeon 780M/RADV 25.0.7, private invisible headless Mutter/Xwayland `:4`; no physical display. Native Windows remains the separate `plans/plan_dx.md` `DX-90`/`DX-114` proof boundary. No modern CNAEXT graphics API was added. |
...
| FX-129 | Repair the Vulkan compiled-effect build regression introduced after its closure | FX-065, VULKAN-216 | **Done 2026-09-08.** `VULKAN-216` propagated sample-mask handling to every pipeline family, but the code inside `CNA_VULKAN_COMPILED_EFFECTS` called the free `NarrowSampleMaskEXT` helper as a nonexistent `Pending3DDraw` member and read a nonexistent local `blendParams` instead of the captured `draw.blendParams`, at both the cache-key and `VkPipelineMultisampleStateCreateInfo` sites. An option-on parse therefore failed before any compiled-effect test could link. Both expressions now use the captured draw state through the same helper as every stock family. The normal Vulkan renderer target builds through the system ccache; the production translation unit also compiles with `CNA_VULKAN_COMPILED_EFFECTS`, `MOJOSHADER_EFFECT_SUPPORT` and the renderer's MojoShader header contract enabled. This row repairs compilation only: `REMED-GFX-203` added the compiled multi-stream layout path, but the dated 24-pass/1-skip runtime matrix is not relabelled without an option-on conformance run. |
...
| FX-069 | Publish the final cross-renderer support matrix and project-wide completion definition | FX-061, FX-062, FX-063, FX-065, FX-066, FX-067, FX-068, FX-070, FX-134 | **Blocked only on Metal (`FX-066`).** Seven production backends are done and verified on executing devices (`FX-061` SDL_GPU, `FX-062` EasyGL, `FX-063` DirectX 11, `FX-065` Vulkan, `FX-070` DirectX 9, `FX-134` DirectX 12, plus FNA3D from `FX-057`). `FX-070` removed the last non-Apple blocker; the interim matrix in section 10.2 now has one unwritten row. |
| FX-070 | Implement and gate DirectX 9, which needs the effect container parsed but no shader translated | FX-060, FX-067 | **Done (2026-09-10) -- `SupportsCompiledEffects()` is true in the opt-in build.** `CNA_DIRECTX9_COMPILED_EFFECTS` is off by default and brings in the pinned MojoShader only when enabled. `D3D9CompiledEffect` supplies CNA's own nine-function Effect Framework backend: it parses through the HLSL profile for reflection but discards that translated output, copies the original aligned Shader Model 2/3 token stream into `CreateVertexShader` / `CreatePixelShader`, maintains the D3D9 float/int/bool register files, and reuses the renderer-neutral reflection and pass-state translation. Ordinary buffered/user, indexed/non-indexed and instanced draws select the effect's own shader pair and declaration; SpriteBatch runs every public pass, overrides slot zero with the sprite source, and supplies the embedded SpriteEffect vertex stage when an effect contains only a pixel shader. Pixel samplers resolve 2D, render-target, cube and volume textures and consume filtering, U/V/W addressing, anisotropy, mip clamp and LOD bias. **Executed gate:** 21 focused tests run, 20 pass and one skips solely because D3D9's renderer-wide `MultiStreamVertexInput` capability is false. The passing cases include all seven compiler-produced stock effects, public construction/reflection/state, ordinary/indexed/user/instanced draws, SpriteBatch and pixel-only vertex-stage inheritance, pass selection, effect switching, sampler pixels, 2D/render-target/cube/volume sources, clone/lifecycle including clone rejection after device teardown, 600 draws, and every truncation boundary. The shared SpriteBatch texture-slot oracle itself had one D3D9-era pixel-centre bug: sampling exactly at `u = 0.5` with point filtering is a tie under D3D9 integer pixel centres but not under modern half-integer centres. Moving the probe away from the tie preserves the slot/source/Y-flip proof and passes unchanged on D3D9, D3D11 and D3D12. Both opt-in ON and OFF capability tests pass; OFF builds no MojoShader runtime and reports false. D3D9 smoke is **62/62**. The task's initial complete D3D9 gate ran **54 tests: 45 pass and 9 pre-existing `D9-124` failures**, with the new compiled-effect aggregate accounting for the one additional pass over the 44/53 baseline. Subsequent fixes `D9-125` through `D9-128` raise the current label to **54/54**. **Environment:** MinGW cross-build with fixed `sharp-runtimenext` `0c82d9b888bdf5f7d5663c77942f339bcb2a7445`, pinned FNA3D `3240147` / MojoShader `6333f74`, Wine+DXVK 2.6.0 on AMD Radeon 780M/RADV 25.0.7, private headless Mutter/Xwayland `:4`; no physical display. Native Windows remains a separate evidence boundary. No modern public `CNAEXT` graphics API was added. |
...
| FX-107 | Rebuild the compiled route's GL objects across a context recreation | FX-082 | **Done fully (2026-09-04).** Every live EasyGL compiled effect retains shared source bytecode plus its changed parameter values, texture bindings and technique. The renderer's first recovery entry releases the old effects/MojoShader context and its renderer-owned VAO/flipped-source objects, then rebuilds the MojoShader context and all effects before child GL resources recover. `EasyGLCompiledEffectDrawTest.CompiledDrawObjectsSurviveAContextRecreation` now requires the same live `Effect` to draw the same pixels after a real desktop context recreation and also verifies cube-face content. Racing Game's production WebGL2 build then passed actual `WEBGL_lose_context` loss/restoration during loading, the main menu and a 3D race, with resource-backed captures and continued drawing after all three transitions. The asynchronous browser interval is handled at the framework boundary: `GraphicsDeviceManager::BeginDraw` skips Draw/Present while EasyGL reports the context lost, but Update continues and rendering resumes after restoration |
...
| FX-118 | Correct a render target's row order ONCE on the SpriteBatch compiled route | FX-080, FX-099 | **Done (2026-08-26, found by SAMPLE-031 BloomPostprocess).** FX-099 gave the compiled route its own row-order correction -- a per-sampler-slot copy of the source blitted with Y reversed -- because MojoShader's generated GLSL carries none of this renderer's own sampling-time correction. What it did not do is stop the OTHER correction from also running: `EasyGLSpriteBatchRenderer::Draw` mirrors V in the sprite's own vertex data whenever the source is a render target (REMED-GFX-147), and the compiled route reads exactly those vertices. Every `SpriteBatch` draw of a rendered source through a compiled `.fx` was therefore corrected twice and came out **upside down**, while every hop of the `DrawUserPrimitives` route FX-099 pinned was right way up -- which is why the existing contract passed throughout. The sprite's V mirror is now suppressed for a batch that flushes through the compiled route (`BatchFlushesThroughCompiledEffect()`), leaving the per-slot copy as the single correction: it is the one that can also correct a slot the sprite quad does not own, such as the base image `BloomCombine.fx` reads from `GraphicsDevice.Textures[1]`. `ResolveCurrentTextureRowOrder()` runs on both edges that can change the answer -- the source changing and the effect changing -- because `SetCustomEffect()`'s flush is a no-op for an empty batch and would otherwise leave the previous batch's answer standing across a `Begin()`. Pinned by `RunCompiledEffectSpriteBatchRenderTargetSourceContract` (one hop, two hops, and a plain `Texture2D` that must stay untouched), verified to fail on EasyGL when the suppression is removed, and wired into all four backends that run the compiled-effect suite |
| FX-119 | Build with `CNA_EASYGL_COMPILED_EFFECTS` OFF again | FX-118 | **Done (2026-08-27, reported against `next`; depth workaround superseded by SOFTWARE-336 on 2026-09-11).** `FX-117`'s depth-range work (commit b718f950a) left the default configuration unbuildable in two places, both of the same shape: `viewportMinDepth_`/`viewportMaxDepth_` were declared inside the `#if defined(CNA_EASYGL_COMPILED_EFFECTS)` block while `EasyGLRenderer::SetViewport` writes them unconditionally, and `IsDepthTestedOnTheSameScaleAsStockGeometry` was appended AFTER the test file's own `#endif`, where the helpers it calls do not exist. The macro is OFF by default, so neither showed up in any build that runs the compiled-effect suite. The two members were moved outside the guard and the test inside it, restoring both build modes. SOFTWARE-336 later proved the underlying upper-half stock depth convention was itself nonconforming: EasyGL stock shaders now apply MojoShader's XNA/D3D-to-OpenGL clip-Z conversion, the compiled per-draw range narrowing and these two cache members were removed, and both macro configurations build again. The original mixed stock/compiled test remains as an ordering regression. |
| FX-120 | Retain the compiled sprite route's own geometry instead of rebuilding it per flush | FX-080, FX-118 | **Done (2026-08-26, found by SAMPLE-033 NonPhotoRealistic).** `FlushBatchWithCompiledEffect` created a vertex buffer and an index buffer, drew with them and destroyed them, every flush. The compiled route records its geometry in ONE long-lived vertex array object (`EnsureCompiledEffectVaoEXT`), and an element buffer binding is part of a VAO's own state -- so each flush left that array object holding a deleted name for the next draw to read. Desktop GL tolerates it and draws; **WebGL 2 validates the binding and refuses the whole draw**, `GL_INVALID_OPERATION: glDrawElements: Insufficient buffer size`, so the first flush of a batch drew and every later one produced nothing. On `WEBGL2` that made five of NonPhotoRealistic's six presets a black frame while the sixth -- the only one that runs no postprocess -- rendered correctly. The buffers are now members, created once and re-uploaded, which also removes two creations and two deletions from every compiled-effect sprite flush. Diagnosis took Chrome's own `Log` domain: WebGL driver errors arrive as `Log.entryAdded`, not `Runtime.consoleAPICalled`, so the browser gate saw nothing until it subscribed to both -- and the draw route itself never asks for a GL error, so on that platform the sample rendered black in silence. Pinned by a two-flush case in `RunCompiledEffectSpriteBatchRenderTargetSourceContract`; it cannot fail on desktop GL, which is the whole reason the defect survived, and is recorded as such |
| FX-121 | Translate compiled-effect fragment shaders at `highp`, not GLSL ES's `mediump` default | FX-001 | **Done (2026-08-26, found by SAMPLE-034 NormalMapping).** MojoShader's GLSL ES profiles emit `precision highp float;` for a vertex shader but `precision mediump float;` for a fragment shader. A Direct3D 9 shader's registers and arithmetic are full 32-bit unless an instruction opts into partial precision, so that is a silent downgrade -- and `mediump` only guarantees fp16 RANGE. NormalMapping.fx interpolates an unnormalized world-space light vector and normalizes it in the pixel shader; on this sample's own scale (light radius 800, camera 1500) the very first `dot(v, v)` reaches ~10^6 and overflows, `inversesqrt` returns 0, the normalized direction collapses to zero, and BOTH the diffuse and the specular term become exactly zero. The frame still showed the model, still responded to the camera and still honoured `AmbientLightColor`, so it looked like a lit render -- it was `AmbientLightColor * diffuseTexture` and nothing else, and rotating the light changed not one pixel. Every layer above was measured correct first: the parameter reached the runtime boundary, the register file, and `vs_uniforms_vec4[12]` read back from GL with the right value; writing the register directly still changed nothing, which is what moved the search into the fragment shader. Fixed at the source, in a new `cmake/patches/mojoshader-6333f74-fragment-precision.patch`: GLSL ES 3.00 requires fragment `highp`, so it is asked for unconditionally; GLSL ES 1.00 makes it optional, so there it is guarded by `GL_FRAGMENT_PRECISION_HIGH` and keeps `mediump` otherwise -- on such an implementation the limit is real hardware rather than a translation choice. The apply script now takes a LIST of patches so an unrelated fix does not have to be merged into the parser-robustness patch. Native OPENGLES3 matches the original to a mean absolute difference of 0.76-1.34 levels of 255 over four frozen light angles, with frame mean brightness agreeing to 0.02 of a level; the browser gate's `modelIsLit` (peak model luminance 31 when broken, 147 lit) is the regression pin |
| FX-122 | Clamp a compiled effect's vertex COLOR output on the GLSL ES profiles, as Direct3D 9 does | FX-121 | **Done (2026-08-26, found by SAMPLE-035 PerPixelLighting).** Direct3D 9 clamps a vertex shader's colour output registers (`oD0`/`oD1`) to [0,1] BEFORE the rasterizer interpolates them. MojoShader's DESKTOP GLSL path inherits that for free by writing `gl_FrontColor`, which GL clamps by default -- but the GLSL ES profiles have no `gl_FrontColor`, so they fall through to a plain varying, and a plain varying is clamped nowhere. A shader whose per-vertex colour exceeds 1 therefore interpolates the UNclamped value and saturates per fragment instead, which is a different gradient, not a rounding difference: flat where D3D9 still has slope. `VertexLighting.fx` sums ambient (DarkSlateGray) + diffuse (CornflowerBlue) + specular, so its blue reaches ~1.24 at a lit vertex; on the low-poly cone that produced a solid saturated wedge covering 4.8 % of the frame. The signature that identified it is worth keeping: **red and green matched the original EXACTLY, pixel for pixel, and only blue differed -- only where blue would exceed 1**. Nothing else produces that. Fixed in a third patch, `cmake/patches/mojoshader-6333f74-vertex-color-clamp.patch`, emitting the clamp at the end of `main()`. The register list to consult there is `used_registers`, NOT `attributes`: an output register only reaches `attributes` in `process_definitions()`, which runs after `emit_GLSL_end()` has already closed `main()` -- a first attempt keyed on `attributes` compiled, ran, and emitted nothing at all. Frame agreement went from 95.2-98.1 % to a uniform **99.37-99.40 % of pixels within 8 levels across all ten frames**, the residue being the 1-px silhouette and far-field grid-line rasterization. SAMPLE-033's `CartoonEffect.Fx` declares a COLOR vertex output too and is byte-identical across the change, its normal/depth values already being inside [0,1] |
| FX-123 | Saturate the built-in per-vertex-lit programs' colour varyings, as Direct3D 9 saturates `oD0`/`oD1` | FX-122 | **Done (2026-08-28, found by SAMPLE-046 Graphics3DSample).** FX-122 fixed this D3D9 semantic in MojoShader's compiled-effect path; the BUILT-IN effect path had the same hole. EasyGL's `EnsureLit3DVertexLitProgram` and `EnsureSkinnedVertexLitProgram` carry the lit and specular RGB in plain `out vec3` varyings, and a plain varying is clamped nowhere, so a per-vertex sum above 1 interpolated unclamped and the triangle came out BRIGHTER than D3D9's -- a different gradient, not a rounding difference. **The isolation is the evidence:** the sample has three directional lights and a hook that forces each checkbox, and with any ONE of them on the frame agrees with real XNA on 99.99 % of pixels within 8 levels with a signed error of +-0.01 levels; with all three it drops to 90.31 % and the model is uniformly +11.2/+14.3/+8.9 brighter, the gap growing with the lit value and collapsing to zero in whichever channel XNA has already saturated. Only the accumulated sum crosses 1, which is exactly when the missing clamp can matter. After the fix the same frame is **99.99 % within 8 levels and 100.00 % after a 4 px blur**, mean absolute difference 0.030 of 255. Pinned by `easygl_basiceffect_vertex_color_clamp_test`, whose scene is built so the two answers are 102 and 153 at the sampled pixel: a grey (not white) texture is what keeps them apart, since with a white texture both saturate to 255 and the test could never fail. Removing the clamp makes it read 153 and fail both checks, confirmed |
| FX-124 | Translate the built-in per-pixel-lit fragment stages at `highp`, not GLSL ES's `mediump` default | FX-121, FX-123 | **Done (2026-08-28, found by SAMPLE-046 Graphics3DSample).** The same relationship to FX-121 that FX-123 has to FX-122: the compiled-effect path was fixed, the built-in path was not. Four EasyGL fragment shaders normalize a WORLD-SPACE view vector, `normalize(uEyePosition - vWorldPos)` -- `EnsureLit3DProgram`, `EnsureSkinnedProgram`, `EnsurePbrProgram`, `EnsurePbrSkinnedProgram` -- while declaring `precision mediump float;`, which guarantees only fp16 RANGE. `normalize()` computes `dot(v, v)` first, so an eye a few thousand units out overflows, `inversesqrt` returns 0 and the view direction collapses to zero. **Mesa does honour the qualifier**, which this measured rather than assumed: SAMPLE-046's camera sits 3500 units from the model, and with one directional light and `PreferPerPixelLighting` the frame agreed with real XNA on 90.62 % of pixels within 8 levels at `mediump` and **99.99 % at `highp`**, the signed error over the model going from -20.1 levels (uniform across R, G and B -- the signature of a lost grey specular term) to -0.01. With all three lights the diffuse saturates and hides most of it, 92.40 %, which is why the single-light leg is the one that names the defect. GLSL ES 3.00 requires fragment `highp` so it is asked for unconditionally; `TransformGlslEs300BodyToEs100` now rewrites that request into the `GL_FRAGMENT_PRECISION_HIGH` guard for the WEBGL1/OPENGLES2 profiles, where 1.00 makes it optional -- same shape FX-121 used. `EnsureEnvMapped3DProgram` normalizes in its VERTEX stage and passes a unit vector, so it is correct at `mediump` and is left alone. Pinned by `easygl_basiceffect_world_scale_precision_test`, whose oracle is scale invariance -- Blinn-Phong depends only on directions, so multiplying every length in the scene by 1000 must change nothing -- and which asserts BOTH legs actually have a highlight first, so it cannot pass by comparing two black frames. Reverting the qualifier moves the 1000x leg from 228 to 161 and fails it, confirmed |
| FX-125 | Light a vertex-coloured mesh instead of dropping it onto the unlit program | FX-123 | **Done (2026-08-28, found by SAMPLE-047 PickingSample).** XNA gives every lit `BasicEffect` family a `Vc` variant -- `VSBasicVertexLightingVc` and friends -- which multiplies the lit diffuse by the per-vertex colour. EasyGL's lit programs had no colour input at all, and `SelectStockProgramShape` dispatched on stride alone with cases for 20, 24 and 32. **Position+Normal+Color+TextureCoordinate is 36 bytes**, which is exactly what the stock `ModelProcessor` emits for a mesh carrying a colour channel -- setting `VertexColorEnabled` at the same time -- so it matched no case, fell through to the **unlit** `prog_colored_`, and the model rendered flat: no shading, no specular, no light at all. The isolation named it in one table: with the vertex colour switched off in both engines the sphere agreed on **99.76 %** of pixels within 8 levels, with it on **46.94 %**, and CNA was up to 84 levels too dark -- and the materials themselves were byte-identical on both sides, so the XNB reader was never in question. **Two things had to be right, and only measuring separated them.** The stride case is one; the other is that the attribute's location is the element's INDEX in that program's own input table -- `ConfigureDeclarationForStockProgramEXT` binds `inputs[i]` to location `i` -- so declaring `aColor` at 5, to match the skinned program's slot, left it at its generic default and the model went black under lighting while every summary number stayed put. Moving it to 3 took the frame from 95.21 % to **97.35 %** and the sphere to **99.76 %**, signed error over the model going from -41.5/-74.9/-21.8 to +0.07/-0.04/-0.00. The multiply also has to happen in the VERTEX stage of the per-vertex family, ahead of FX-123's saturate, because `vout.Diffuse *= vin.Color` runs before the value reaches `oD0` and it is `oD0` that D3D9 clamps; that ordering is worth 0.07 pp on this scene (97.35 vs 97.28) -- small, but the correct semantic, and stated here at its measured size rather than a claimed one. The uniform is declared in the vertex stage ONLY: that stage defaults to `highp` and the fragment stage to `mediump`, and GLSL ES 3.00 refuses to link a uniform shared across stages with different precision -- the same trap `uDiffuseColor` in that shader already documents, and it cost a build that failed to link and silently kept the previous binary. Pinned by `easygl_basiceffect_lit_vertex_color_test`, whose three checks separate "lit and coloured" (51,102,26) from "unlit vertex colour" (102,204,51) and from "lit but colourless" (128,128,128); removing the stride case makes it read the second of those and fail, confirmed |
| FX-126 | A downward-facing surface takes a directional light in EasyGL that Direct3D 9 leaves black | FX-125 | **Done (2026-09-10; the original normal-sign premise was disproved by measurement).** An isolated Microsoft XNA 4.0 probe rendered an upward-only light white `(255,255,255)` and a downward-only light black `(0,0,0)`, exactly like EasyGL, while an independent XNA `Viewport.Project` probe reproduced CNA's depth ordering for the overlapping top (`+Z`) and bottom (`-Z`) cap triangles at the suspect table pixel. Culling both ways still left the XNA table brown, so neither normals, projection nor winding caused the SAMPLE-047 difference. The black region appeared exactly when the isolated XNA table used `LinearClamp`: full XNA `PickingSample` leaves SpriteBatch's default `LinearClamp` in `GraphicsDevice.SamplerStates[0]`, and the table has out-of-range UVs. CNA's `SpriteBatch::Begin` applied that sampler only through the renderer-local sprite interface but left the public collection stale; the following 3D draw therefore restored the old `LinearWrap` and sampled brown texture instead of clamp-border black. FNA's `PrepRenderState` assigns the complete effective sampler through `GraphicsDevice.SamplerStates[0]`, so CNA now does the same before applying the renderer-local sprite state. The fixed CNA frame matches the XNA capture on **99.98% of pixels within 8 levels**, **99.98% within 32**, mean absolute difference **0.073/255**, with coverage `47.823%` vs `47.821%`, identical centroids, and 100% within 8 after a 4 px blur; representative table pixels agree as `(5,3,2)` vs `(4,2,1)`, black, black, and `(48,17,3)` vs `(47,17,3)`. Two unit tests pin both the default and every field of an explicit sampler publication; EasyGL passes them `2/2`, the public SpriteBatch/3D order fixture `86/86`, and the normal fixture `2/2`; D3D11 and D3D12 each pass both focused public fixtures `2/2` and the new unit tests `2/2`. Full parity labels remain **D3D11 266/266** and **D3D12 264/264**, with no failures or skips and smoke checks **21/21** and **25/25** respectively. Measurements used Microsoft XNA 4.0 under Wine+DXVK and private virtual Xvfb/Mutter displays only; no physical display was used. |
| FX-127 | Accept a `BLENDINDICES` element declared `Vector4`, not only `Byte4` | FX-125 | **Done (2026-08-28, found by SAMPLE-051 CustomModelAnimation).** XNA's `VertexElementFormat` describes the BYTES in the buffer; the shader register a `BLENDINDICES` semantic arrives in is a float4 either way, so a content processor may legally write either -- and this sample's own `SkinnedModelProcessor` does exactly that, `ConvertChannelContent<Vector4>("BlendIndices0")`. Real XNA renders it; EasyGL refused the draw outright with *"element 1 declares BlendIndices0@12 Vector4 but shader input 'aBoneIndices' expects Byte4"*. One shader attribute cannot be both an integer and a float, so the fix is one read mode on every profile: the three stock skinned vertex shaders (and the CNAEXT layer's `ShadowMap`/`DepthNormalPrepass` copies) now declare `in vec4 aBoneIndices` and cast to `int()` when indexing `uBones[]`, `Byte4` binds as non-normalized unsigned bytes read as floats -- 0-255, exactly representable and far above the <= 72 bone count -- and `Vector4` binds as plain floats. That is the shape the GLSL ES 1.00 profiles already ran in production (GLB-36), so the ES 1.00 rewrite that produced it is gone with the sources it rewrote. `StockProgramInput` grew an `alternateFormat`, defaulting to `format`, for a semantic with more than one legal spelling. Pinned by `easygl_skinnedeffect_vector4_bone_indices_test`: (a)+(b) a Vector4-declared index of 1 draws and selects bone 1, (c) index 0 selects the bone that moves the quad off screen, (d) the Byte4 spelling still draws. Both halves sabotage-verified -- removing the acceptance throws again, restoring the integer pointer fails (d) alone. The CNAEXT shadow copies were caught the same way: `ShadowVisibilityTest.ASkinnedMeshShadowsItself` passes on the pristine tree, failed with the renderer change alone, and passes again with them updated |
| FX-128 | Keep EasyGL compiled effects on the GLSL dialect of their actual GL context | FX-062 | **Done (2026-09-02, found by SAMPLE-152 Racing Game Kit).** `EasyGLRenderer::GetMojoShaderContextEXT` asked `MOJOSHADER_glBestProfile`, which prefers the pinned adapter's experimental `glspirv` route on a modern desktop core driver. Applying a valid pixel-only Effect pass then crashed in `impl_SPIRV_LinkProgram`: it dereferenced the absent vertex shader before either stage's null check, and later SPIR-V finalisation has the same both-stages assumption. EasyGL's promised route is the GL-family source dialects, not `glspirv`, so the renderer now maps its runtime `GlProfile` explicitly: `OPENGL33 -> glsl120`, `OPENGLES2/WEBGL1 -> glsles`, `OPENGLES3/WEBGL2 -> glsles3`. The mapping reads the renderer instance, not the build default, preserving multi-renderer builds. `PixelShaderOnlyPassAppliesOnDesktopCoreContext` pins the original shape. On a real OpenGL 4.5 core llvmpipe context the previously crashing shared contract and the new focused test pass under ASan/UBSan, and all 34 EasyGL compiled-effect tests pass in the normal build. No sample-side shader rewrite and no third-party source change were made |
| FX-129 | Refuse a compiled pass whose shader pair never became the bound program | FX-062 | **Done (2026-09-06, found by SAMPLE-066 ShipGame).** `MOJOSHADER_glBindShaders` returns SILENTLY when the pair fails to link -- `program = MOJOSHADER_glLinkProgram(v, p); if (program == NULL) return;` -- leaving the PREVIOUS program bound. Nothing downstream could tell: `BindCompiledEffectForDrawEXT` reads the pair back with `MOJOSHADER_glGetBoundShaders` and validated the draw's vertex declaration against a shader from an unrelated effect, so the game was told *"this compiled effect's vertex shader requires attribute 'vs_v3' (usage 7, index 0)"* about a shader its own effect never had -- and when two layouts happen to agree there is no symptom at all, just the previous program's pixels. `ApplyPass` now compares what the pass selected (`effectData_->current_vert`/`current_pixl`, which `MOJOSHADER_effectBeginPass` sets) against what is actually bound, and throws naming the GL link log. A pass with **no vertex shader of its own** is excluded: GL has no pixel-only program, so the link is *expected* to fail there and inheriting the bound program is how such a pass works (FX-128). Measured, not inferred -- a gdb trace of every `ApplyPass` and every `MOJOSHADER_glBindShaders` call showed Particle's pass binding its own shader and the FX-098 bounce then rebinding NormalMapping's. All 38 EasyGL compiled-effect tests pass; the pixel-only exclusion is the one the shared backend conformance contract exercises |
| FX-130 | Link a Direct3D 9 pixel shader input that no vertex shader writes | FX-062 | **Done (2026-09-06, found by SAMPLE-066 ShipGame).** Direct3D 9 links the two stages by REGISTER, so a pixel shader may read an input its vertex shader never writes; the value is undefined and the pair still links. GLSL links by NAME and refuses it: *"fragment shader input `io_5_0' has no matching output in the previous stage"*. Point-sprite shaders reach this as a matter of course -- ShipGame's `Particle.fx` pixel shader reads `TEXCOORD0` because Direct3D generated it in the rasterizer, and its vertex shader outputs only `POSITION`, `PSIZE`, `COLOR0` and `COLOR1` -- and XNA 4.0, which dropped point sprites, still ships and runs exactly that effect. A sixth pinned MojoShader patch, `mojoshader-6333f74-unmatched-fragment-input.patch`, gives those inputs a producer-free definition: `impl_GLSL_LinkProgram` compares the pixel shader's `attributes` against the vertex shader's `outputs` and, for this pairing only, compiles a fragment shader whose unmatched `in`/`varying` declarations become zero-initialised globals, deleting the temporary shader object once the program has linked. Which inputs are unmatched is decided by the PAIRING, not by either shader, which is why it belongs at link time and not in the emitter; on failure it falls back to the source as emitted so GL still reports the real error. No sample-side shader rewrite |
| FX-131 | A short `SetData` must not shrink the buffer a later draw may read | FX-129 | **Done (2026-09-06, found by SAMPLE-066 ShipGame).** XNA fixes `VertexBuffer.VertexCount` at construction and `SetData(data, startIndex, elementCount)` writes a PREFIX, leaving the rest of the buffer as it was. CNA tracked the last upload instead: `GraphicsDevice` validated draws against `GetRenderer().GetVertexCount()` -- the element count of the most recent `SetData` -- and EasyGL sized the VBO with `glBufferData(byte_count)`, so the storage itself shrank. A game that fills a large buffer once and rewrites a short prefix each frame is exactly the shape that hits it: ShipGame's `ParticleManager` allocates 8192 vertices, re-uploads only the live particles, and then draws `LineList` with the upstream's own vertex-count-as-primitive-count quirk -- 200 vertices out of 8192 -- which CNA refused with *"The requested primitive range exceeds the bound vertex buffer"*. The five `GraphicsDevice` sites now read the XNA-level `getVertexCountProperty()`, and EasyGL's `None` upload allocates the buffer's own capacity and `glBufferSubData`s the written prefix, the same allocation the `Discard` branch already made. 215 vertex-buffer, draw-range and declaration tests pass in isolation; the full graphics suite is unchanged at 2305 passing |
| FX-132 | A shader constant of 1e10 breaks the GLSL float printer on a 32-bit target | FX-130 | **Done (2026-09-06, found by SAMPLE-066 ShipGame).** `MOJOSHADER_printFloat` prints the integer part through `unsigned long`, which is 64 bits on LP64 and **32 on wasm32** and every other ILP32 target. ShipGame's `Particle.fx` says `Pos.xyz = 1e10`, so the conversion went out of range -- undefined behaviour that in practice saturated, after which the nine-digit fractional loop printed nine more full-width numbers and overflowed the caller's 32-byte buffer. The effect therefore compiled on the native OPENGLES3 build and, on WEBGL2, failed the whole game at load with *"BUG: internal buffer is too small"* -- the shader is identical, only the ABI differs. A seventh pinned patch, `mojoshader-6333f74-ilp32-float-literal.patch`, prints through `uint64` so the two agree, and sends a magnitude no integer type can hold down a plain `%.1f` route that is defined and still carries the `.` `floatstr()` requires. Found by making CNA report the message at all -- see the same row's `Game::Run` change |
| FX-133 | Apply the MojoShader patch series as a series, and stop reverting it every configure | FX-130 | **Done (2026-09-06, found by SAMPLE-066 ShipGame).** Two patches in the series now edit the same lines -- FX-132 rewrites statements the effect-parser hardening introduced -- and the per-patch `git apply --reverse --check` cannot express that: the later patch's post-image makes the earlier one's reverse check fail, the script then tried to apply an already-applied patch, and the configure died claiming *"the pinned FNA3D/MojoShader revisions may no longer match the patch"*. It is not a hypothetical: it stopped this session's very first build. The script now judges the whole series -- it must not apply forward (so the files really are patched) and a stamp must name this exact series (so a half-applied older list is not mistaken for it) -- and otherwise restores the tracked files and applies the series in order, which is self-healing rather than fatal. Separately, `FetchContent_Declare(fna3d ...)` gained `UPDATE_DISCONNECTED TRUE`: the ref is pinned, but the update step re-checked it out on **every** configure, reverting the series and recompiling MojoShader every time. The explicit re-apply after `MakeAvailable` still guarantees the sources are correct |
| FX-134 | Repair the shared `ps_1_x` SPIR-V linker defects: 21/27 committed passes -> 27/27 | FX-061, FX-065 | **Done (2026-09-06).** **Allocated as `FX-129` on the `webgpu` branch and renumbered to `FX-134` when that branch merged into `next` (2026-09-09): `next` had independently allocated `FX-129`/`FX-130` the same day for unrelated GLSL-side work, and both sets of rows are real. Commits reading `fix(FX-129): repair the shared ps_1_x SPIR-V defects, 21/27 -> 27/27` and `docs(FX-129)` are THIS row.** Six of the 27 committed compiled-effect passes -- every Shader Model 1.x pixel shader in the corpus that uses `t#` registers -- produced a fragment module WebGPU refuses with `Multiple bindings at location N are present`. **The recorded diagnosis was half wrong, and the wrong half is the more useful one.** It read the Location decorations, saw `ps_t0` at the location naga named, and concluded that `ps_t0` collided with a second TEXCOORD0 input created for the `gl_PointCoord`-or-`TEXCOORD0` dual-purpose variable. The decorations CANNOT show that: naga indexes its complaint into the **OpEntryPoint interface list**, and that variable is never created for a `ps_1_x` shader at all (`emit_SPIRV_attribute` skips the whole branch below `ps_1_4`). Dumping the interface list -- which `spikes/webgpu-spirv-spike` now does on every failure -- names TWO independent defects, both in shared MojoShader code that Vulkan (`VulkanCompiledEffect.cpp:678`) and SDL_GPU (`mojoshader_sdlgpu.c:670`) reach through the identical `MOJOSHADER_linkSPIRVShaders` on identical bytes. **(1) OpEntryPoint listed each `ps_1_x` texture register twice.** `emit_SPIRV_finalize` counts the interface from `ctx->attributes` and emits it from that list AND from `ctx->spirv.id_implicit_input[]`. Upstream those are disjoint for `ps_1_x`; CNA's own `legacy-texcoord-input.patch` registers exactly those pairs, so the two overlap. A repeated `<id>` is two arguments at one Location to a consumer. Repaired by gathering the ids first, dropping repeats, and emitting from the list that was counted, so count and contents cannot disagree again (`mojoshader-6333f74-spirv-entry-interface-unique.patch`). **(2) The SPIR-V header's VERSION word was overwritten with 0** on all six. `MOJOSHADER_spirv_link_attributes()` finishes the dual-purpose variable through `pointcoord_var_offset`/`pointcoord_load_offset` but GUARDS on `attrib_offsets[TEXCOORD][0]`. Those are different questions: a `ps_1_x` `t0` register fills that slot (upstream code, not CNA's patch) without creating the variable, so both offsets stay 0 and both writes land on `binary[1]`. naga loads such a module anyway, so nothing had ever reported it. Repaired by asking for the variable rather than for the slot (`mojoshader-6333f74-spirv-pointcoord-existence.patch`). Neither repair special-cases a fixture, a shader hash, a filename or a register number: one makes the entry point interface a set, as SPIR-V requires, and the other separates two meanings that had shared one bookkeeping slot. Point-sprite semantics, COLOR and TEXCOORD varyings, SM1.x and SM2/3 behaviour and every already-correct module are unchanged -- the 21 that worked are byte-identical. **Evidence.** 27/27 passes across all nine committed fixtures translate and create a shader module (was 21/27); zero zeroed headers and zero duplicated interface ids anywhere in the corpus. The regression guard is `MojoShaderSpirvEntryPointInterfaceTest` in `modules/renderers/common/mojoshader/tests/` -- the SHARED half, device-free, reading the two damaged words back out of every finished module -- and it was verified to FAIL with the patches reversed, on exactly those six passes. `WebGPUCompiledEffectTest.EveryPassOfTheRealXna4GameEffectsCreatesBothShaderModules` raises the WebGPU suite's own claim from parse+reflect (which those six passed while broken) to all 18 racing passes linking and creating both modules. **All four compiled-effect backends re-measured with the repaired dependency, and the two that consume this exact SPIR-V were A/B'd against the unpatched linker rather than assumed:** WebGPU 55/55; EasyGL 48/48 (GLSL profile, untouched by both patches); **Vulkan** 35 passed / 1 skipped / 1 failed and **SDL_GPU** 44 passed / 3 skipped / 1 failed -- the one failure on each is `SharedSpriteBatchRenderTargetSourceContract`, which fails IDENTICALLY with both patches reversed on both backends, so it is pre-existing and not a regression. The new shared guard runs and passes in the Vulkan and SDL_GPU builds too, which is the point of putting it in the shared module. A clean dependency reconstruction -- a fresh worktree at the pin with all nine declared patches applied in order -- reproduced the tree byte-identically **as this row stood on the `webgpu` branch. The merge into `next` (2026-09-09) makes the series ELEVEN**: `next` had independently added `unmatched-fragment-input` (`FX-130`) and `ilp32-float-literal` (`FX-132`), and the merged list is the union, the four SPIR-V patches last. The union was re-verified rather than assumed -- applied to a pristine MojoShader at the pin, it applies with no fuzz, and the resulting tree differs from each branch's own series in exactly the other branch's files and hunks and nowhere else (`mojoshader_common.c` is the only file both sides touch, in disjoint regions ~1006-1064 and ~1176). `cmake/UnitTests.cmake` gained a glob entry for `modules/renderers/common/*/tests`, which no shared renderer half had needed before |
| FX-135 | Browser compiled Effects: translate this route's SPIR-V to WGSL | FX-134 | **Allocated as `FX-130` on the `webgpu` branch and renumbered to `FX-135` at the same merge; see `FX-134`. The commit `docs(FX-129/FX-130/WEBGPU-203)` cites `FX-134`/`FX-135` under their old numbers.** **Done (2026-09-06); the implementation and its browser evidence live in `plans/plan_webgpu.md` `WEBGPU-203`, and this row exists so `plan_fx.md` does not keep saying the opposite.** §10.3's WebGPU paragraph used to end "browser WebGPU remains unsupported ... an implementation gap with a named route"; the named route was taken. `modules/renderers/common/mojoshader/src/SpirvToWgsl.cpp` is a SPIR-V module parser and typed IR over the subset THIS pipeline emits -- measured across all 27 passes before a line was written: 55 opcodes, 9 GLSL.std.450 instructions, five storage classes, seven decorations, one builtin, no matrices, no loops, no phi, one function per module -- and it refuses anything outside that subset by name. No new dependency: naga and Tint are general translators and neither is vendored. The two targets diverge at exactly one seam, `GetOrCreateCompiledEffectShaderModuleEXT`; everything above it is shared. It is verifiable natively because wgpu-native accepts WGSL, so 16 of the shared contracts in §9 re-run through the browser's representation with the SPIR-V route as the oracle, pixel checks included |
| FX-136 | Repair stale FNA3D synthetic render-state expectations | FX-005, FX-139 | **Done (2026-09-10, found by the fresh FX-005 FNA3D suite).** Two old backend-local tests contradicted the newer shared contract. The "every render-state token" list treated Effect Framework token 178 as an ordinary scalar state, but it is `SetSampler` metadata carrying a sampler-parameter reference and the generated fixture intentionally had no sampler; the dedicated sampler fixtures already exercise that encoding. The unknown-token test expected token 177 to survive construction and fail during `Apply`, but the hardened public `Effect` constructor now rejects it structurally before a backend runtime can inspect an invalid enum, exactly as `RunCompiledEffectStatePolicyContract` requires. The stale expectations now test the intended boundaries. |
| FX-137 | Keep the sampler-component isolation oracle valid after independent DualTexture UVs | FX-062 | **Done (2026-09-10, found while closing FX-126).** E4 claimed to isolate sampler slots by changing only slot 1's filter while binding a one-texel white texture there, but its stride-20 vertex supplied only `TextureCoordinate0`. FNA's authoritative `DualTextureEffect.fx` consumes `VSInputTx2` and therefore requires independent `TextureCoordinate0` and `TextureCoordinate1`; after SAMPLE-073 correctly taught EasyGL that contract, E4's missing input became undefined driver state. The same executable consequently passed **52/52** on virtual Xwayland/RADV but failed **51/52** on isolated Xvfb/llvmpipe, even though tracing showed slot 0 remained Point and slot 1 changed independently. The fixture now supplies an explicit stride-28 declaration with both UV channels, assigning them identical coordinates so its sampler-only premise is unchanged. The Xvfb red-first leg moves **51/52 -> 52/52**; EasyGL on virtual Xwayland, D3D11 and D3D12 each pass their registered fixture **1/1**. No renderer production changed, and removing FX-126's SpriteBatch state publication did not alter the pre-fix E4 failure. All runs used isolated Xvfb or private virtual Xwayland `:4`; no physical display was used. |
| FX-138 | Implement and gate DirectX 12 compiled XNA effects | FX-030, FX-060 | **Done (2026-09-10).** **Allocated as `FX-134` on the `dx` branch and renumbered to `FX-138` when that branch merged into `next` (2026-09-10): `next` had independently allocated `FX-134`/`FX-135` the same week for the WebGPU-side SPIR-V work, and both sets of rows are real. Commits reading `fix(FX-134)` on the `dx` branch are THIS row.** The existence probe parses `CnaConformanceEffect.fxb` through MojoShader's HLSL profile, compiles the Shader Model 3-era source to `vs_4_0`/`ps_4_0` DXBC with `D3DCompile`, executes it on the D3D12 device and verifies the off-screen read-back pixel; this is an execution proof, not parser/compiler success. `D3D12CompiledEffect` supplies CNA's own nine-function `MOJOSHADER_effectShaderContext`, D3D9 float/int/bool register files, reflection and pass-state translation, pair-specific stage-visible root signatures, frame-ring uniform snapshots, texture/sampler descriptors for 2D/render-target/cube/volume sources, and PSOs through the existing complete cache. MojoShader's independently named HLSL stage structs are linked by semantic before compilation. Ordinary, indexed, multi-stream and instanced draw routes dispatch the runtime; SpriteBatch combines its embedded XNA `SpriteEffect` vertex stage with a caller's compiled pixel stage and preserves pass/texture behavior. The opt-in `CNA_DIRECTX12_COMPILED_EFFECTS` remains OFF by default, so an ordinary D3D12 build pulls no FNA3D/MojoShader dependency and reports the capability false; the ON build advertises true only after this gate. **19/19 focused tests pass, 0 skipped**, including every shared public-path section used by FX-063 and a regression where the runtime outlives its `GraphicsDevice`; the renderer and live runtimes share ownership of their CPU-only MojoShader context so teardown never dereferences renderer storage after device destruction. Both registered gates pass (2/2), D3D12 smoke remains **25/25**, and the complete D3D12 label passes **266/266**, 0 failed, 0 skipped. The measured environment was the fixed `/rv/data/development/github.com/openeggbert/sharp-runtimenext` checkout, pinned FNA3D `3240147` / MojoShader `6333f74`, Wine+vkd3d-proton 3.1, and private headless Mutter/Xwayland `:4`; no physical display was used. Native Windows verification remains owned by `plans/plan_dx.md` `DX-114`. No modern public `CNAEXT` graphics API was added. |
| FX-139 | Restore FNA3D compilation after the texture-format query became non-throwing | FX-083 | **Done (2026-09-10, found while closing FX-005).** **Allocated as `FX-135` on the `dx` branch and renumbered to `FX-139` at the same merge; see `FX-138`. Commits reading `fix(FX-135)` on the `dx` branch are THIS row.** `ITextureRenderer::GetSurfaceFormatEXT()` became `noexcept` in the shared single-/dual-channel sampling repair, but FNA3D's only override retained the older throwing signature. A fresh FNA3D build therefore failed before compiling any test. The override now matches the base contract and explicitly says `override`; this is a declaration-only correction because it returns one stored integer and cannot throw. |
...
| Compatibility | FNA3D's own `StockFixtureReflectionMatchesTheFnaOracle`, `Fna3dEffectStateOracleTest`, and `Fna3dEffectPixelOracleTest` -- not portable, so backend-local |
...
source it controls, compiled by the same Effect compiler XNA's Content Pipeline used, plus FNA's
own reflection, pass-state observations, and deterministic rendered pixels for every committed
binary checked in as test data.
...
#### Assessment (2026-08-17, re-confirmed and extended 2026-09-10)

**Not yet closed**, and the plan says so rather than declaring victory on seven backends. The
2026-08-18 closure pass added the fourth backend (Vulkan); **2026-09-06 added the fifth, WebGPU**
(`plans/plan_webgpu.md` `WEBGPU-166`-`171`), which also retired this document's own §10.3 entry
excluding it. `FX-063` then added DirectX 11 on 2026-09-09, `FX-138` added DirectX 12 on
2026-09-10, and `FX-070` added DirectX 9 through the executable MinGW/Wine+DXVK loop on the same
day. Only Metal remains, and it requires macOS. Writing any backend blind behind a capability gate
remains forbidden -- a backend is promoted on executed evidence or not at all.
...
| WebGPU (`CNA_WEBGPU_COMPILED_EFFECTS=ON`) | **true** | **23 passed, 0 skipped** (2026-09-06). `plans/plan_webgpu.md` `WEBGPU-166`-`171`: MojoShader's SPIR-V profile with CNA's own nine-function backend, plus `SpirvCombinedSamplerSplit` -- WGSL has no combined image sampler, and that one construct is the whole gap between MojoShader's output and a WebGPU shader module. MojoShader's four fixed descriptor sets ARE WebGPU's four bind groups. Declaration-driven vertex input, compiled instancing and multi-stream, pixel-stage cube AND volume sampling, and a `Present()`-deferred replay whose uniforms and sampler bindings are snapshotted at apply time. No skips, and **since `plans/plan_webgpu.md` `WEBGPU-208` (2026-09-06) no options set to false either** -- `supportsLodBias` is now true. **This cell twice said otherwise and both reasons are superseded.** It first justified the false option as "renderer-wide -- `WGPUSamplerDescriptor` has no LOD-bias field at all", which stopped being true when `WEBGPU-205` implemented the state on every stock 3D route through WGSL `textureSampleBias`; the closeout pass then narrowed it to "a compiled pass's sampling call arrives from MojoShader as a plain `textureSample`", which `WEBGPU-208` fixed at the source. `MojoShaderEffect::InjectSamplerLodBias` rewrites the normalized SPIR-V so each implicit sample carries a `Bias` operand fed from its own D3D9 sampler register's slot in a renderer-owned uniform block -- before the two routes diverge, so naga lowers the operand natively and `SpirvToWgsl` emits `textureSampleBias` for the browser. The shared sampler pixel contract passes on both routes **Native AND browser since 2026-09-06** (`WEBGPU-203`): browser WebGPU still ingests WGSL exclusively, so CNA translates this route's SPIR-V into WGSL at the one seam where the targets differ, `GetOrCreateCompiledEffectShaderModuleEXT`. 16 of these shared contracts are additionally re-run natively through the WGSL representation (wgpu-native accepts WGSL, so the SPIR-V route is the oracle), and a browser page runs the acceptance ladder 11/11 in headless Chrome (13/13 since `WEBGPU-208` added the two LOD-bias rungs) |
| DirectX 11 (`CNA_DIRECTX11_COMPILED_EFFECTS=ON`) | **true** | 18 passed, 0 skipped. MojoShader's D3D11 adapter, every public ordinary and SpriteBatch route, multi-stream, instancing, sampler-state pixels, 2D/render-target/cube/volume sources, stock isolation, many-draw uniform retention and truncation rejection. Full D3D11 label 267/267 on Wine+DXVK 2.6.0 and a private headless display (`FX-063`) |
| DirectX 12 (`CNA_DIRECTX12_COMPILED_EFFECTS=ON`) | **true** | 19 passed, 0 skipped. CNA's own MojoShader HLSL backend compiles each pair to SM4 DXBC, uses stage-visible root signatures and frame-ring uniform snapshots, and passes ordinary/indexed/multi-stream/instanced, SpriteBatch, state, 2D/render-target/cube/volume, switching, retention, truncation and post-device teardown contracts. Full D3D12 label 266/266 on Wine+vkd3d-proton 3.1 and private virtual Xwayland (`FX-138`) |
| DirectX 9 (`CNA_DIRECTX9_COMPILED_EFFECTS=ON`) | **true** | 20 passed, 1 renderer-wide multi-stream skip. Raw D3D9 shader tokens, CNA's own MojoShader context, public ordinary/indexed/user/instanced and SpriteBatch routes, pass/state/sampler behavior, 2D/render-target/cube/volume sampling, switching, lifecycle, retention and truncation. Full D3D9 label 54/54 after `D9-128`; smoke 62/62 on Wine+DXVK 2.6.0 and private virtual Xwayland (`FX-070`) |
| Metal (`FX-066`) | false | Not implemented. Requires macOS; see the requirements note on `FX-066` |
| SDL_GPU / EasyGL / Vulkan / DirectX 9 / DirectX 11 / DirectX 12 with the option off | false | The runtime is not compiled in; the capability reports false and construction refuses by name |
| every other identity | false | Section 10.3: planned, assessed-feasible, or unsupported by design |

What is still open before 10.2 can close (updated 2026-08-18, amended 2026-09-10):

- **Metal (`FX-066`) is the sole unwritten backend.** Its executable gate requires macOS.
- `FX-069`'s final cross-renderer matrix depends on Metal and on nothing else.
- A compiled effect whose **vertex shader samples a texture** is refused on SDL_GPU, EasyGL and
  Vulkan. D3D11 now binds reflected vertex-stage samplers, textures, and constant storage through
  MojoShader's native adapter, but the shared suite has no differential vertex-stage texture oracle,
  so FNA3D remains the only backend with direct behavioral proof of that case. It consumes the
  effect's own assignments, bypassing the public collections (`FX-109`).
- EasyGL rebuilds live compiled effects and their retained parameter state after a GL context recreation (`FX-107`).
...
| EasyGL and the OpenGL/OpenGL ES family | Done. GLSL/GLSLES/GLSLES3 profiles plus `mojoshader_opengl.c`. One implementation serves `OPENGLES2`, `OPENGLES3`, `OPENGL33`, `WEBGL1` and `WEBGL2`, since EasyGL is their shared implementation. (`OPENGL4` is a separate renderer family and was never served by it; it gained its own port in `plans/plan_opengl4_modern_graphics.md` `GL4-0020`, behind `CNA_OPENGL4_COMPILED_EFFECTS`.) Ordinary, indexed, instanced, multi-stream and SpriteBatch draw routes, a golden-pixel test and the widened FX-060 suite passing in full, `SupportsCompiledEffects()` true | `FX-062`, `FX-080`, `FX-082`, `FX-083`, `FX-088` |
| DirectX 11 | Done. HLSL profile plus `mojoshader_d3d11.c`; every shared public-path section passes through CNA's MinGW/Wine+DXVK loop and `SupportsCompiledEffects()` is true behind the opt-in | `FX-063` |
| DirectX 12 | Done. HLSL profile plus `D3DCompile`, with CNA's own nine-function backend, stage-pair root signatures, frame-owned constants/descriptors and complete public draw/SpriteBatch routing | `FX-134` |
| DirectX 9 | Done. The HLSL profile supplies reflection while its translated output is discarded; the original D3D9 token streams feed the native device through CNA's own nine-function backend. The shared public-path suite runs through MinGW/Wine+DXVK and `SupportsCompiledEffects()` is true behind the opt-in | `FX-070` |
...
- **Reflection has to come from a profile.** The backend's `getParseData` must return a
  ...
  reflection *without* translating, `BYTECODE`, is one of the four the pin disables. `FX-070`
  therefore parses through the already available HLSL profile and discards the translated text;
  the native shader object is created from the original token buffer supplied to `compileShader`.

The structural prediction was correct, but the integration was not merely a parser shim: generic
vertex declarations, all draw routes, SpriteBatch's pixel-only effect inheritance, native constant
register files, and every texture kind still needed explicit D3D9 routing. `FX-070` now verifies that
route on a live Wine+DXVK D3D9 device: 20 focused tests pass, one renderer-wide multi-stream case
skips, smoke is 62/62, and its initial complete-label gate was 45/54. Subsequent fixes `D9-125`
through `D9-128` make the current label 54/54.
...
- **Programmable, but not through anything MojoShader emits**: `PORTABLEGL`, whose shader stage is
  ...
  **`WEBGPU` is no longer in this list. Superseded 2026-09-06 by `plans/plan_webgpu.md`
  ...
  suite 23/23 with no skips. **Browser WebGPU was supported on 2026-09-06 by `plans/plan_webgpu.md`
  `WEBGPU-203`.** This paragraph used to end "browser WebGPU remains unsupported ... an
  implementation gap with a named route, not a property of the target API", and the named route was
  taken: emdawnwebgpu's `createShaderModule` still accepts `ShaderSourceWGSL` and nothing else, so
  CNA now TRANSLATES this route's SPIR-V into WGSL
  (`modules/renderers/common/mojoshader/src/SpirvToWgsl.cpp`, a parser and typed IR over the 55
  opcodes and 9 GLSL.std.450 instructions the pipeline can actually emit -- measured, not assumed).
  The configure-time Emscripten refusal is gone, and a browser page runs the whole acceptance ladder
  11/11 -- **13/13 since `WEBGPU-208`**, which gave a compiled Effect's sampling its
  `SamplerState.MipMapLevelOfDetailBias` by injecting a `Bias` image operand into the shared SPIR-V
  before the two routes diverge, so `supportsLodBias` is now true on this backend as well.
...
| A compiled draw after a GL context recreation | **Case 1** -- retained bytecode, parameters and bindings rebuild the live effect | n/a (no context loss) | n/a (no context loss) | fully rebuilt and browser-qualified | n/a (no context loss) | `FX-107` |
