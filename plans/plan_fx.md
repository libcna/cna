...
- Status: **Four backends supported and gated: FNA3D, SDL_GPU, the EasyGL/OpenGL family and
  ...
  by state inspection. A later sample-mask edit broke the option-on Vulkan translation unit;
  `FX-129` repaired that compile regression on 2026-09-08 without relabelling the older runtime
  matrix. Every other renderer identity is `CompiledEffects == false` and refuses by
