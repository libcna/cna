...
> preserves all 20 classic Texture2D formats and all 18 XNA-permitted cube formats, including exact
> DXT block transfer on Software and EasyGL; `NormalizedByte2/4` cubes remain correctly excluded.
> The historical revision prose below remains as session history.
...
> shared bounded decoder now serves both runtime XNB loading and headless XNB-to-CNB transcoding.
> The historical status prose below is retained as session history; the Phase D table is current.
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| XNB-18A | Simple primitive readers: `Boolean`/`Byte`/`SByte`/`Int16/32/64`/`UInt16/32/64`/`Single`/`Double`/`Char`/`String` | ✅ | **Implemented 2026-07-16**: `CNA::Internal::Xnb::{Boolean,Byte,SByte,Int16,UInt16,Int32,UInt32,Int64,UInt64,Single,Double,Char,String}Reader` (`include`/`src`/`CNA/Internal/Xnb/PrimitiveContentTypeReaders.*`), each registered under its real FNA canonical name via `RegisterPrimitiveXnbReaders()`. Live in `CNA::Internal::Xnb`, not `Microsoft::Xna::Framework::Content` — FNA's own equivalents are all `internal`/default-visibility, never subclassed by game code. **SAMPLE-055 correction (2026-08-30):** the same primitive registration now also owns the standard closed `DictionaryReader<String, Int32>` shape used by processor-generated bone-name lookup tables; requiring every game to repeat that primitive-only registration was an AOT implementation leak, unlike closed collections of game-owned types. |
...
| XNB-23 | `Texture2DReader`, implemented strictly against CNA's **backend-neutral** `Texture2D`/`GraphicsDevice` API (`SurfaceFormat` + width/height/mip levels + per-level `SetData`) — the reader must not reference EasyGL (or any other backend) internals directly | ✅ | **Implemented 2026-07-16; full-format conformance corrected by SOFTWARE-275 on 2026-09-10.** `CNA::Internal::Xnb::Texture2DReader` remains backend-neutral. Its original closure covered Color, normalized formats and DXT while rejecting the other fourteen legal HiDef formats. It now constructs all 20 classic serialized formats and forwards exact uncompressed mip bytes through XNA's generic byte-transfer shape. DXT1/3/5 stay compressed on exact-transfer backends and otherwise use the existing lossless Color fallback. The original Color POD fix remains valid only for that fallback because `Color` has virtual packed-vector methods. |
| XNB-24 | `SurfaceFormat` capability inventory (not immediate blanket conversion): classify each `SurfaceFormat` as native-upload / CPU-conversion / GPU-transcode / unsupported / lossy-fallback per backend (`struct SurfaceFormatCapability{bool nativeUpload; optional<SurfaceFormat> fallback; ConversionFunction converter;}`); `Texture2DReader` only asks `GraphicsDevice`/a formats service for a compatible resource, it does not own per-backend conversion tables itself | ✅ | **Narrowed implementation 2026-07-16; completed for the classic runtime surface by SOFTWARE-275 on 2026-09-10.** The graphics device already exposes exact compressed-transfer capability, so the reader retains DXT blocks where supported and otherwise losslessly converts to Color. Every uncompressed classic format now uses its exact backend-neutral storage; no reader-local native-format table or lossy fallback is needed. |
...
> was subsequently completed by Content Pipeline CP-047 on 2026-08-30.
...
| XNB-30B | Re-run every Phase B/C fixture through its compressed form and confirm identical results | ✅ | **Implemented 2026-07-16, with a scope note**: verified against 2 *real, externally-produced* LZX-compressed fixtures (MonoGame's own `Explosion.xnb`/`FontCalibri14.xnb`, `tests/assets/xnb/monogame/windows/lzx/`) rather than re-compressing the existing synthetic Phase B/C test fixtures — CNA has no LZX *encoder* to compress them with (still true; `.xnb` **writing** landed later under `plans/plan_xnapipeline.md`, but uncompressed only -- LZX compression is gated there as `XNAP-023`), and real external fixtures are stronger evidence of true format compatibility than self-compressed ones would be anyway. `Explosion.xnb` verifies a single-block decode end-to-end through `content.Load<Texture2D>()` with real non-uniform pixel data; `FontCalibri14.xnb` verifies multi-block state persistence and recovers the exact reader set (including nested generic names) a real `SpriteFont` needs |
| XNB-30C | Phase D2 — investigate/implement MonoGame-supported alternative compression variants under the `XnbCompression` enum from XNB-27 | ✅ | **Implemented 2026-08-30 by Content Pipeline CP-047:** MonoGame's writer emits one raw LZ4 block after the 10-byte header and 4-byte decompressed size. `DecompressXnbLz4Payload` implements only that exact representation with checked extended lengths, input/output extents, nonzero in-history offsets, exact input consumption/output size, and `XnbReadLimits` before allocation. Runtime and canonical compiler paths share it. The oracle keeps the exact externally-produced MonoGame `white-1.xnb` body, compressed independently by upstream liblz4 1.10.0; runtime load, headless transcode, semantic pixels, malformed vectors, bomb limits, and 1,500 deterministic container mutations pass. No generic LZ4 frame parser or runtime library dependency was added. |
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| XNB-25 | `Texture3DReader`/`TextureCubeReader`/base `TextureReader` | ✅ | **Implemented 2026-07-16; base-reader omission corrected by SAMPLE-005 on 2026-08-23; volume-format conformance corrected by SOFTWARE-274 and cube-format conformance by SOFTWARE-275 on 2026-09-10.** `CNA::Internal::Xnb::Texture3DReader`/`TextureCubeReader` implement the concrete volume/cube formats. The inert FNA `TextureReader` base now consumes no bytes and returns the existing instance. `Texture3DReader` preserves all fifteen legal HiDef volume formats, rejects DXT1/3/5 and accepts depth-dominant mip chains. `TextureCubeReader` preserves all fifteen legal uncompressed formats plus DXT1/3/5, rejects the two XNA-invalid signed-normalized layouts, and retains exact face/mip bytes. The real `SampleCube64DXT1Mips.xnb` still verifies six faces and the complete 64→1 chain; no real volume fixture is available. `ContentManager::Load<TextureCube>()` continues to probe `.xnb` before loose DDS. |
...
| XNB-39 | `ModelReader`: real per-mesh `BoundingSphere`, model/mesh/mesh-part `Tag` | ✅ | **Implemented 2026-07-16; custom Tag completion added by SAMPLE-005 on 2026-08-23; stock dictionary coverage extended by SAMPLE-048 and SAMPLE-061.** `ModelMesh::setBoundingSphereProperty()` and the type-erased `ContentReader::ReadObject()` path remain the basis. The earlier implementation rejected every non-null Tag, but `ReachGraphicsDemo_4_0/dude.xnb` carries its processor-generated `SkinnedModel.SkinningData` in `Model.Tag`, making that rejection an actual XNA incompatibility rather than an exotic case. `ModelReader` accepts a custom reader erased explicitly as `std::shared_ptr<System::Object>`, assigns model/mesh/mesh-part Tags, and retains their owners with the Model's other reader-owned resources. SAMPLE-048 added the stock `Dictionary<string, object>` processor shape; SAMPLE-061 added the stock `Dictionary<string, List<Vector3>>` closed generic emitted by `MarbleMazeProcessor`. Both use `ObjectDictionaryEXT` as the C++ object carrier while preserving each entry's exact reader-produced type, and the typed form also preserves its true managed type name. Synthetic real-XNB tests prove all three Tag shapes survive `ContentManager::Load<Model>()`; SAMPLE-005 proves the complete official `dude.xnb` Tag graph, animation clip dictionary, skin transforms and `SkinnedEffect` render path. Unsupported erasure types are still refused by name instead of being dropped. |
