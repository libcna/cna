...
| `Compression::None` only in v1 | **adopted, then superseded** | one `compression` field per chunk. Adopted as written -- the reader rejected every non-zero codec -- and **overtaken by `CNBF-105`**, which implemented Zstandard as codec 2, opt-in per chunk and off by default. The field is no longer reserved; a codec this build does not implement is what is rejected. See §15.5 `CNBF-105`. |
...
* a `compression` codec this build does not implement ⇒ reject with the codec id named. *(Updated by `CNBF-105`. As written, and as the reader behaved until then, this read `compression != 0` ⇒ reject: no codec was implemented. Zstandard is codec 2 now, so the rule is about what the build implements rather than about zero. The refusal happens while the table of contents is read, before any chunk is decoded, which is why a compressed file cannot be inspected at all without its codec.)*
...
| chunk compression | ✅ **Zstandard as codec 2** (`CNBF-105`), opt-in per chunk, off by default | measured, not assumed |
...
| CNBF-113-scope | **Scope deliberately not widened by the move.** | At extraction time this remained DXT1/DXT3/DXT5, square faces, cube maps only. **2026-09-02 / SAMPLE-152:** the Racing Game Kit supplied the first authoritative need for legacy byte-aligned 24-bit DDS cube payloads (`NormalizeCubeMap.dds`: BGR888 masks, six 128×128 faces). The shared decoder now also accepts the two honest byte orders RGB888/BGR888 and expands them to RGBA8 with alpha 255; exact masks, every face/mip, real GPU upload/readback, and the original asset's CNB chunks are verified. DX10 extended headers, other packed formats and HDR DDS remain refused. | ✅ |
| CNBF-113-rgba | **Why the `.cnb` holds RGBA8 rather than the source payload.** | Not a shortcut. The runtime DDS path normalizes its supported source encodings to RGBA8 on the CPU, and texture schema 1's contract is the portable `Rgba8` baseline. At the time of CNBF-113 no renderer implemented compressed GPU texture formats end-to-end; selected renderers now support explicit native compressed transfers, but changing the portable stream/CNB contract is a separate versioned-format decision. | ✅ |
...
`CNBF-122` was added by a **residual audit of that pass**, also on 2026-08-27, and is the reason the
table has nine rows rather than eight. Every one of its six findings is a boundary `CNBF-114`-`121`
moved but did not close: an ownership rule expressed as an argument a caller could simply pass, a
symmetry proved over document *shapes* and not over document *sizes*, an equivalence established
for numbers and not for the two semantic rules the type depends on, a per-item ceiling standing in
for an aggregate one, two header numbers read but not checked, and a durability property nobody had
asked of the tools. Recorded here rather than in a new section because they restore the same
invariants.
...
| CNBF-115 | **Writer/reader symmetry.** | The writer could produce three files its own reader refuses. (1) `SetExternalReferences()` performed no name validation at all, while `CnbDocument` has always refused an `XREF` name that is absolute, drive-qualified, backslash-separated or carries a `..` segment. (2) `AddChunk()` accepted `CMET` and `XREF`, so a schema could append a second copy of a chunk the reader requires to be a singleton — `Build()` accepted it, `Parse()` rejected it. (3) `CnbMediaCodec` carried its own hand-written copy of the name rule, with a comment claiming the writer did not check, which would silently stop being true the moment the writer did. | One `CnbLogicalNameProblem()` in `CnbFormat.cpp` is now the single definition of the rule — non-empty, well-formed UTF-8, relative, `/`-only, no `..` — and `CnbDocument`, `CnbWriter` and `CnbMediaCodec` all call it; the two hand-written copies are gone. `AddChunk()` refuses a container-defined identifier by name and says which setter to use instead. Four tests, including one that asserts the invariant directly: over every combination of writer calls, whatever `Build()` accepts, `Parse()` loads. Three existing suites that hand-built hostile container chunks *through* `AddChunk` now build them by retyping a table-of-contents entry or by carrying references through `SetExternalReferences()`, so they still reach the reader's checks. | ✅ |
| CNBF-116 | **DDS cube-map header hardening.** | `DecodeDdsCube` cast every header field straight to `int` and did its block arithmetic there. `0x7FFFFFFF` therefore read as a positive two billion, passed the `width <= 0` guard, and reached `((width + 3) / 4) * ((height + 3) / 4) * blockSize` -- signed overflow, undefined behaviour, and in practice a *small* byte count that then satisfied the "still inside the file?" check before `DxtUtil` was handed a two-billion-texel extent. A mip count out of the same header was `reserve()`d for before a byte of payload had been consulted. And `caps2` was checked only for `DDSCAPS2_CUBEMAP`, while six faces are read unconditionally -- so a file declaring two faces had four faces' worth of bytes taken from whatever followed it, and `DDSCAPS2_VOLUME` beside the cube bit was accepted. | Header fields are read as the `std::uint32_t` the format defines and narrowed only after every bound; `CalculateDDSLevelSize` computes in `std::uint64_t`. A dimension ceiling of 16384 texels, derived from what `DecodedDdsCube::width` and `DxtUtil`'s `width * height * 4` can represent (`sqrt(INT32_MAX/4)` = 23170) rather than chosen; it also bounds the chain at 15 levels, inside CNB's own `CnbMaxTextureMipLevels` of 16. The mip count is validated against the face's physical chain **before** the reserve, all six `DDSCAPS2_CUBEMAP_*` face bits are required, and any other `caps2` bit is refused. Malformed headers are **refused, not repaired**. 13 tests in a new `DdsCubeDecoderTests.cpp`; four of them verified to fail against the previous decoder, and the DXT1/3/5-only scope and its existing exception types are pinned by a test of their own. | ✅ |
| CNBF-117 | **RIFF/WAVE parser hardening and portability.** | Five defects in the headless WAV importer. (1) Every integer was `std::memcpy`'d into a host integer, so on a big-endian machine each field -- channel count, sample rate, every chunk length -- decoded byte-swapped. (2) The `smpl` loop-entry read was bounded by the **file**, not by the `smpl` chunk: a 36-byte `smpl` declaring a loop, followed by any chunk at all, took its 24-byte "loop entry" out of the next chunk's header and contents, inventing a loop region from unrelated bytes. (3) The RIFF form's own declared length was never read, so the chunk walk was bounded only by the file. (4) `blockAlign` and `byteRate` were read past and never checked, although the importer splits the data chunk by a frame size it derives independently. (5) `WAVE_FORMAT_EXTENSIBLE` was unwrapped by reading the first two bytes of its SubFormat GUID, so any unrelated GUID beginning `01 00` was decoded as PCM. | Integers assembled from bytes, the way `CnbByteReader` already does. Every `smpl` read bounded by that chunk's payload; a declared loop with no room for its entry is refused. The RIFF length is validated and bounds the walk, an odd-length chunk must carry its pad byte when anything follows, and leftover bytes too short to be a chunk header are refused. **Amended by `plans/plan_xna_sample_xnb_sweep.md` `XNASWEEP-192`, 2026-09-10, on a measurement of the genuine `WavImporter`:** the RIFF length bounds *nothing* -- 0, 1, 2 and 3 are refused and every other value is accepted, including one that cuts the `data` chunk in half and `0xFFFFFFFF` -- the walk is bounded by the file, a chunk whose size runs past the file ends the walk rather than the import, and leftover bytes are ignored. Everything else this row hardened is unchanged. The reading here was chosen before the importer had been measured, and it cost six of SAMPLE-145's WAVs, whose RIFF length is 68 bytes short of a `smpl` chunk somebody inserted after it was computed. `blockAlign`/`byteRate` must agree with the fields they duplicate. `EXTENSIBLE` requires the full 22-byte extension, a `cbSize` of at least 22, valid bits equal to the container width, and all fourteen fixed bytes of a `KSDATAFORMAT_SUBTYPE_*` GUID. Duplicate `fmt `/`data` chunks refused. Ten tests; six verified to fail against the previous importer, and three of the ten are positive contracts (every accepted shape still round-trips, padded chunks still walk correctly, the little-endian field layout is asserted against the fixture's own bytes). | ✅ |
...
### 17.3 Verification at the end of `CNBF-122`

Every number below is from a run, not from memory. Repo-root CWD, `DISPLAY=:131` with
`SDL_VIDEODRIVER=x11` (the GPU-backed virtual display), `cmake-build-debug` unless stated. Each row
names the filter it was measured with, so it can be re-measured rather than believed.

| what | filter | result |
|---|---|---|
| CNB + DDS suites | `Cnb*:*Cnb*:DdsCubeDecoderTest.*` | **382 / 382 pass** — §17.2 recorded 366 for this filter before the pass, and 16 of this pass's 17 new tests fall inside it |
| The same plus the runtime `.cnj` SpriteFont suite | `…:CnjSpriteFontTest.*` | **387 / 387 pass** |
| Content, CNJ, SpriteFont, Texture3D, Model, XNB | `*Content*:*Cnj*:*SpriteFont*:*Texture3D*:*Xnb*:*Model*` | **743 pass, 3 skipped** — the two absent optional glTF fixtures and the renderer-gated `Texture3DUnsupportedRendererTest`, all three pre-existing. A second run of this filter *while the full `ctest` sweep was using the machine* lost one case to a timing flake; measured alone it is 743 |
| Real-executable subprocess suites | `CnbSourceToolTest.*:CnbCompilerToolTest.*:CnbGltfDirectToolTest.*:CnbInfoToolTest.*` | **30 / 30 pass** (25 before this pass) |
| Golden vectors | `CnbGoldenVectorTest.*` | **11 / 11 pass**, and `CnbGoldenVectorTests.cpp` is not in this pass's diff |
| **No Zstandard** (`build-probe/cnbf-nozstd`, `-DCNA_CNB_ZSTD=OFF`) | as row 2 | **377 pass, 10 skipped, 0 failures.** The ten skips are the tests that ask `IsCnbCompressionSupported()` first; all 17 of this pass's new tests **ran** there |
| **ASan + UBSan** (`build-asan`, `-DCNA_SANITIZE=address,undefined`) | as row 2 | **387 / 387 pass, zero UBSan runtime errors and zero AddressSanitizer memory errors.** LeakSanitizer reports allocations at exit whose **every frame is inside `libGLX_mesa.so.0`** — no CNA frame appears in any of them, and the same report is produced by a filter that constructs no `GraphicsDevice`. Recorded as what it is rather than as "no sanitizer output" |
| Full `ctest -j8` | (all 71 entries) | **8567 tests, 43 failed.** Re-running exactly that set **serially** leaves **25**; the 18 that recovered are 9 audio-timing cases, the 8 ENet/two-process networking cases and `LeaderboardWriterTest`, all of which need the machine to itself. See below |

**The 25 durable full-suite failures, and why none of them is this pass's.** By category:
`VertexDeclarationLayoutTest` (6) and `DeclarationGuardTest` (4), `EasyGL_*` (9),
`GltfLimitationsDoc` + `GltfRendererPbrFallbackPolicy` + `CnaGltfConformanceL0`/`L1` (4),
`CApiCoverageMatrix` (1) and one audio-timing case that stayed red serially,
`SoundBankTest.IsInUseFalseSoonAfterFireAndForgetCueNaturallyFinishes`. That is the same
composition §17.2 recorded for this branch — OpenGL under Xvfb, glTF conformance, the C-ABI
inventory, audio timing — and **not one of them is in CNB, in DDS, in the content module's `.cnb`
or `.cnj` paths, or in any file this pass touched.**

`CApiCoverageMatrix` is again the only one a change to a **public header** could plausibly cause,
and again it does not. Regenerating the inventory and diffing it against the committed
`docs/c-api/COVERAGE.md` adds **789 lines** — the whole `CNA/Content/Cnb/` header set that
`CNBF-001`–`CNBF-113` added and the snapshot never had. This pass adds no module public header at
all: `tools/common/CnaToolAtomicWrite.hpp` lives under `tools/`, which the generator does not scan
(verified: the regenerated inventory contains no mention of it). The regenerated file was **not**
committed — it belongs to `plans/plan_cabi.md`.

**Seventeen new tests, thirteen of them verified to fail with the fix backed out.** The back-out was
done in three passes — one patch disabling all six code changes at once, one restoring
`source_to_cnb`'s in-place write, and one disabling the container string/array checks — so the
mapping from test to fix is the one the runs reported rather than an assumed one:

| verified to fail when backed out | what it proves |
|---|---|
| `CnbContainerTest.TheWriterRefusesADocumentWhoseChunksUnpackAboveTheAggregateBudget` | writer aggregate budget |
| `CnbContainerTest.TheWriterRefusesAFileLargerThanTheReaderWouldOpen` | writer file-size ceiling |
| `CnbContainerTest.TheWriterRefusesTooManyChunksAndTooLargeAChunk` | writer chunk-count and per-chunk ceilings |
| `CnbContainerTest.EveryDefaultReaderLimitIsAlsoAWriterLimit` | the universal Build → Parse claim, over **all seven** `CnbReadLimits` entries — including `maxStringBytes` and `maxArrayElementCount`, which bound `CMET`'s names and `XREF`'s rows and which the first draft of this fix wrongly treated as schema-only |
| `CnbCompilerStrictnessTest.ASpriteFontsCharacterMapIsValidatedIdenticallyByBothRoutes` | compiler **and** runtime agree on ordering, duplicates and `defaultCharacter` |
| `CnjSpriteFontTest.UnsortedOrDuplicatedGlyphCharactersThrow` | the runtime half specifically |
| `CnjSpriteFontTest.DefaultCharacterAbsentFromGlyphsThrows` (pre-existing, retargeted) | the refusal now comes from the shared reader, not `SpriteFont`'s constructor |
| `DdsCubeDecoderTest.TheAggregateDecodedSizeIsBudgetedBeforeAnythingIsAllocated` | the DDS aggregate budget |
| `DdsCubeDecoderTest.TheLargestRepresentableDimensionIsStillAccepted` (pre-existing, extended) | the budget and the dimension ceiling are separate layers, in that order |
| `CnbProducerTest.AnExtensibleFmtMustDeclareAnExtensionThatFitsInsideItsChunk` | `18 + cbSize` |
| `CnbProducerTest.ADeclaredSmplLoopTableMustFitTheSmplChunk` | `36 + loops * 24`, including the 32-bit wrap |
| `CnbSourceToolTest.AFailingRunLeavesAnExistingOutputByteIdenticalAndNoTemporary` | the atomic write, through the real executable |

**The four that do not discriminate, said plainly rather than counted as if they did.**
`CnbContainerTest.TheWriterDefaultsToTheReadersOwnLimits` asserts a default. The two
`WriteFileAtomically…` tests exercise the shared helper directly, which the back-out did not
replace, so they are contract tests for the helper rather than regressions against the old writer.
`CnbSourceToolTest.ASuccessfulRunReplacesAnExistingOutputWholly` and
`CnbGltfDirectToolTest.AFailingRunPreservesAnExistingOutputAndLeavesNoPartialFile` hold under the
old in-place writer too, because every failure a subprocess test can induce happens *before* the
write; they pin the contract, and the read-only-directory arm of the source-tool test is what
actually distinguishes the two implementations.
`CnbContentManagerTest.AGameCannotForgeABuiltInRegistrationBeforeCnaInstallsIt` is likewise not a
runtime regression — `CNBF-119` already refused the three-argument call — and the thing it adds is
the `static_assert` that the built-in route is unreachable at all, which cannot fail at run time
because it fails to compile. Its twin `static_assert` on a deliberately-public shape is there so
the detector cannot pass vacuously.

**Statistics reconciliation.** The follow-up brief cited an arithmetic error, "4+4+6+6 is 20, not
19". No such sum appears in any committed document, and it could not be located in `plans/`,
`docs/`, `misc/`, `NEXT*.md` or the nine commit messages of `CNBF-114`–`121`; it is presumed to come
from that pass's chat report, which is not in the tree. What *is* in the tree was re-measured and is
correct: §17.2's "25 real-executable subprocess tests" is 8 + 4 + 7 + 6 over the four **suites** it
names. The likeliest source of a mis-sum is that `CnbCompilerToolTests.cpp` holds **eleven** `TEST`
macros across **two** suites — seven `CnbCompilerTest` and four `CnbCompilerToolTest` — so counting
files and counting suites give different answers. Every figure in §17.3 above names its filter for
exactly that reason.
...
* **The checked aggregate arithmetic is not reachable from any input.** `CnbWriter`'s
  ...
  term. Both are structural guarantees against a future change to those ceilings, and neither has —
  or can have — a test that exercises the overflow branch. Said here so the absence is not read as a
  gap in coverage.
* **The atomic write is atomic against process failure, not against power loss.** Neither the
  temporary nor its directory is `fsync`ed, so a machine that loses power mid-rename may show either
  file or a truncated one. That matches what the tools promised before and what a build artifact
  needs; a content pipeline that requires crash durability would need the `fsync` pair added.
* **A destination whose file name is within about twenty characters of `NAME_MAX`** cannot be given
  a sibling temporary, and the write is refused rather than performed in place. No real content
  name is near that.
