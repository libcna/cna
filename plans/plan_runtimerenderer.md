...
| RTR-P2-8 | ✅ | **Phase gate, met in the reduced form the project owner approved on 2026-08-15:** checking all 46 identities on every change is not affordable — each is its own configure and build, and this project treats repeated clean rebuilds as real SSD wear — so the full sweep is an occasional run, not a per-change one. `scripts/run-all-renderer-smoke-tests.sh` gains `--tier routine` (renderers needing no third-party checkout: `OPENGLES3`, `SDL_RENDERER`, `OPENGLES1`) and `--tier full` (all **9** families that own a `GraphicsSmoke` target: those three plus `VULKAN`, `LLGL`, `MAGNUM`, `DILIGENT`, `BGFX`, `WEBGPU`). No flag keeps the historical four-renderer set, so existing callers are unaffected. **Measured, not asserted:** `EasyGL_House3D_SmokeTest` and `EasyGL_Demo2D_SmokeTest` both PASS on the OPENGLES3 tree. **A trap found doing it:** the smoke tests run demo executables (`cna_house3d_demo`, `cna_demo_2d`) that building `CnaTests` does not produce, and CTest reports the absence as **"Not Run"**, not as a failure — so a gate that only built the test binary would report two green-looking non-results. The demo targets must be built explicitly. **Not run here:** `BGFX` (needs a ~332 MB checkout) and native `WEBGPU` (needs wgpu-native); both are reported by the script as unavailable rather than counted as passes, which is the distinction the gate exists for. |
...
| RTR-P3-18 | 🟨 | **Skia half done.** `scripts/run-skia-2d-oracle-diff.sh` passes 9/9 against the real-XNA oracle policy — seven scenes pixel-exact, two filtered ones within measured bounds — so P3's move of Skia's format tables behind `IGraphicsRenderer` virtuals changed no rendered pixel. The three SKIA unit failures were each shown pre-existing: two by a clean detached build at `a749fdce3`, and the third by quoting the implementation and test's disagreeing compressed-format lists from that commit. The test was corrected because Skia contains real DXT/BC7 decoders. The D3D9 divergence suite remains undone: it needs a Windows cross-build under Wine, which is out of scope. |
...
| RTR-P10-9 | ✅ | `+ BGFX` — its runtime `ResolveRendererType()` must not fight CNA's own selection. **It was fighting it, and this fixes that.** Built as `HEADLESS;BGFX;SOFTWARE;STUB` from a reusable `~/deps/bgfx-cmake` checkout; BGFX registers and is selected at runtime (`graphics renderer: BGFX (selected at runtime from 4 compiled in)`), and bgfx's own second level reports `requested renderer: OpenGL, active renderer: OpenGL 2.1` by default and switches to `Vulkan` under `CNA_BGFX_RENDERER=Vulkan`. **The defect:** the descriptor hardcoded `windowKind = Vulkan`, claiming that is what bgfx's default preference order resolves to — on Linux `Detail::GetDefaultRendererType()` returns `OpenGL` unconditionally, and `prepareWindowFlags()` accordingly asked SDL for `SDL_WINDOW_OPENGL`. CNA therefore recorded a Vulkan-kind window while creating a GL one. Harmless while the cross-kind branch was dead; RTR-P5-13 made it live, so a GL candidate would now be refused against a window that IS GL. Both statements now come from one `ResolvedWindowKind()`. **Three build-environment traps recorded on the way, none of which meant bgfx was unbuildable:** the shared clone's submodules sat on revisions the pinned tag does not record (`git submodule status` showed `+`), so the required MSAA patch applied to foreign code; `FETCHCONTENT_SOURCE_DIR_` skips the `PATCH_COMMAND` entirely, so it must be applied by hand; and `-lwayland-egl` failed only for a missing dev symlink — switched off with `BGFX_WITH_WAYLAND=OFF` rather than installing anything. **Direct proof added:** built `SDL_RENDERER;BGFX;HEADLESS` — SDL_RENDERER needs a `Plain` window and bgfx resolves to OpenGL on Linux, so the window-kind boundary MUST be crossed. All 12 `MultiRendererFallbackTest` cases pass, including the two that matter here and that skip in sets which cannot reach the branch: `FallingBackAcrossWindowKindsRecreatesTheWindow` (247 ms) and `ACallerSuppliedWindowRefusesACrossKindCandidateInsteadOfReusingIt` (87 ms). With the old hardcoded `windowKind = Vulkan` this is exactly the set where the branch would have decided on false information. |
...
| RTR-P10-12 | ✅ | `+ FNA3D` — likewise, plus MojoShader's symbol surface. Built as `HEADLESS;FNA3D;SOFTWARE;STUB` from a reusable `~/deps/FNA3D` checkout pinned at `3240147`; the registry holds four descriptors and `FNA3D` is reached at runtime (`graphics renderer: FNA3D (selected at runtime from 4 compiled in)`). **MojoShader was exactly the sharp edge the row names, and it exposed a third instance of one mistake of mine.** `mojoshader.h` includes the GENERATED `mojoshader_version.h` unless `MOJOSHADER_NO_VERSION_INCLUDE` is defined; `cna_fna3d` carries that switch, but RTR-P9-9 had given `CnaTests` the present renderers' include PATHS only. Same shape as the two before it: P9-9 first gave the `PRESENT_` macros without include roots (failed as "no tests"), then include roots without third-party paths (LLGL failed on `LLGL/Surface.h`), and now paths without switches. **The lesson, worth more than the fix:** the surface a family's suites compile against is its headers PLUS its dependencies' paths PLUS the definitions that change what those headers mean. `CnaTests` now links `cna_fna3d` when FNA3D is present, which carries all three. The bgfx traps did NOT recur — the clone went to the pinned tag with `--recursive --force` in one step, so `git submodule status` showed no `+`. |
| RTR-P10-13 | ✅ | `+ MAGNUM`. |
| RTR-P10-14 | ✅ | `+ WICKED`. |
| RTR-P10-15 | ✅ | `+ SOKOL` (GL). |
...
**Former OPENGL33 finding, resolved.** The reported
`GltfSceneGraphBones.SharedMeshGetsOneBonePerInstancingNode` segfault no longer reproduces: all six
`GltfSceneGraphBones.*` tests pass on both the real Mesa/OpenGL 4.6 display and Xvfb's OpenGL 4.5
context. Rechecking the four pixel failures exposed by the first completed OPENGL33 corpus found one
already passing and three point-primitive failures still specific to the real display. Their cause
was renderer state rather than the compositor: desktop GL had `GL_PROGRAM_POINT_SIZE` enabled for
compiled particle effects, while stock shaders left `gl_PointSize` undefined. The hardware driver
rasterized no points; llvmpipe happened to retain 1 pixel. Stock shaders now define a 1-pixel point
size centrally, and all three tests pass on the real display as well.
...
| RTR-P11-8 | ✅ | Multi set `WEBGL1 + WEBGL2` proven under Emscripten. Built 2026-08-15 with emsdk 6.0.6 and **no workaround flags** — possible only because the `sharp-runtime` blocker was fixed upstream in `bc8dbf41` first. Configure reports `renderer registry -- 2 family/families: EasyGL`, the full bundle builds (635 targets, 0 errors), and the evidence is in the artifacts rather than in the exit code: the generated registry holds **exactly** `EasyGL::GetDescriptorWebGL2()` and `EasyGL::GetDescriptorWebGL1()`, while `libcna_renderer_easygl.a` carries all five family accessors — so two identities of ONE family coexist without the archive being duplicated, which is the property this set exists to test. The runtime profile switch `ActiveGlProfile` is present in that archive, so the two profiles are distinguished at run time rather than at compile time. **Not verified:** that a browser actually takes the WebGL 1 path when asked. The ES2 API-generation routing itself is fixed and unit-tested to include both OpenGLES2 and WebGL1. |
...
| RTR-P14-7 | ✅ | `DIRECTX10`: replace the colored fallback for ordinary textured `BasicEffect` draws with real non-indexed and indexed `VertexPositionTexture` pipelines. The animated cna-template cube shows its texture under Wine/DXVK, and the permanent pixel oracle passes both routes with exact `texture * diffuse = (100,120,20,255)`. |
| RTR-P14-8 | ✅ | `DIRECTX12`: make the stock SpriteBatch PSO consume the active XNA blend factors/functions, write mask, sample mask and render-target format. The vkd3d-proton smoke suite passes with R9's exact premultiplied AlphaBlend result `(128,127,0,255)` and `0 failure(s)`. |
| RTR-P14-9 | ✅ | `GLIDE`: dgVoodoo 2.87.3 crashes in its own D3D11 `ResolveSubresource` path under both WineD3D and DXVK 2.6, while caller-supplied OpenGlide commit `b8ac1a32c98f9f8e8616aeffcf4b0af163b59b8f` runs the real cna-template GLIDE executable for three smoke frames and exits 0 under Wine 10. `scripts/run-wine-glide.sh` now enforces the selected runtime path and renderer identity. |
| RTR-P14-10 | ✅ | `cna-template` depth-only renderers: stop using the combined `DepthStencilBuffer` capability to disable the real Z buffer. Every `ThreeD` path now clears depth and applies XNA's standard `DepthStencilState::Default`; `GraphicsDevice` retains its existing per-plane clear masking. An OpenGlide/Xvfb image capture removes the overlapping rear faces seen in the reported broken cube, GLIDE draws three smoke frames and exits 0, and the second depth-only renderer TINYGL also draws three frames and exits 0. Template commit: `c3b804b`. |
