...
| `XNAPP-255` | A cube or volume DDS carried from source to a `TextureCube` / `Texture3D` output. | [ ] XNA's `TextureImporter` reads all three shapes out of a DDS and routes a cube to `TextureCubeContent` and a volume to `Texture3DContent`; the genuine build produces a `TextureCubeReader` of six 64-byte faces and a `Texture3DReader` of one 256-byte volume for the corpus's two fixtures, on Windows and on the Xbox alike (`texture/dds_cube`, `texture/dds_volume`, `xbox/*`). CNA *reads* both shapes already -- `ReadDdsSurfaces` answers exactly those types (`XNAPP-165`) -- and has no route that carries either to an output: the canonical image importer refuses with `DDS source is a cube map; this route builds a Texture2D`. The writers exist on both sides of the gap, so what is missing is the importer/processor pair between them. Closing this also closes the last two types of `XNAPP-252`, whose Xbox guard cannot come off until a fixture proves them end to end. |
