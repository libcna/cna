...
by hand, and the report regenerates with `parity_report.py`. Phases 4 and 5 done (`XNAPP-075`
hardening/fuzz and the `XmlImporter` leg of `074` remain). The intermediate serializer is
...
checkout concurrently, so stage only your own hunks there. Next: `XNAPP-075`, then Phase 6/7 in
ID order (the graphics intermediate types, then processors), then `XNAPP-230` (`XmlImporter`)
