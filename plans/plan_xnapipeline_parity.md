...
**Current (2026-09-05, after Phase 5, Phase 6 and six Phase 7 rows): types 91/128,
members 521/705, enum values 17/27, importers 0/10, extensions 0/18, processors 0/12,
...
`XNAPP-130`–`133`, `135` and `137` are done and `138` is partial; the remaining processors are
`XNAPP-134` (the model graph) and `XNAPP-136` (audio and video, which wait on the `…Audio` types). The next phases need their measurements before their code -- extend
