...
| `XNAPP-254` | XNA's mip-reduction filter identified and implemented. | [ ] The two builds agree texel for texel on level 0 and on the whole of a 2x2 source's chain, so the source, the colour key and the premultiplication all agree; every level below the first of a 4x4 differs. XNA reduces that 4x4 to 2x2 as (80, 71, 17, 211) where an area average of the four texels underneath is (72, 72, 8, 207), which is what CNA writes -- so XNA's kernel reaches beyond the block it is producing. One 4x4 image is far too little to say what it is: this needs a fixture built for the purpose, an image whose content isolates the kernel's support and weights (`phone/png_texture_mipmaps` in the differential corpus, `XNAPP-251`). |
