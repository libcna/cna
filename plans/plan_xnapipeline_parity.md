...
**Current (2026-09-05, after Phase 5 and the texture, font, material, animation and
vertex-collection sides of Phase 6): types 68/128, members 403/705, enum values 6/27, importers 0/10, extensions 0/18, processors 0/12,
...
| `XNAPP-096` | `VertexContent` (Channels, PositionIndices, Positions, VertexCount, Add/Insert/Remove/ConvertToVertexBufferContent), `VertexChannel`/`<T>`, `VertexChannelCollection` (20), `VertexChannelNames` (13), `PositionCollection`, `IndirectPositionCollection`, `IndexCollection`, `BoneWeight`, `BoneWeightCollection` (`NormalizeWeights` ×2), `VectorConverter` (5). | [~] `VertexChannelNames`, `BoneWeight`, `BoneWeightCollection`, `IndexCollection` and `PositionCollection` done, measured first (corpus cases `vertexnames/*`, `boneweight/*`, `indexcollection/*`, `positioncollection/*`): the blend weights channel is spelled `Weights` rather than by its `VertexElementUsage` name, a name with no trailing digits decodes to usage index 0, a bone weight is a value type whose weight must lie in [0, 1] (NaN passes, as neither comparison is true of it), and `NormalizeWeights` sorts largest first, keeps the largest ones, then scales them to sum to one -- the parameterless overload refusing an empty collection with the normalization message rather than the `maxWeights` range check. A reusable `CollectionSerializer<TCollection, TElement>` gives these XNA-named collections the same intermediate form a `List<T>` has (`0 1 2` packed, `<Item />` apiece). `VectorConverter` landed with `XNAPP-092`. Remaining: `VertexContent`, `VertexChannel`/`<T>`, `VertexChannelCollection` and `IndirectPositionCollection`, whose measurements are already in the corpus (`vertexcontent/*`) and which need `XNAPP-095`'s `GeometryContent` to resolve positions through. |
...
| `XNAPP-098` | Compile-parity and behaviour tests for every Phase 6 type. | [~] `XnaGraphicsBitmapTests.cpp`: 11 tests over the texture side (`XNAPP-090`–`092`), `XnaFontDescriptionTests.cpp`: 10 over `XNAPP-093` `XnaMaterialContentTests.cpp`: 15 over `XNAPP-094` `XnaAnimationContentTests.cpp`: 13 over `XNAPP-097` and `XnaVertexCollectionTests.cpp`: 11 over the first half of `XNAPP-096`, each comparing against `tests/reference/xna40/graphics/graphics-content-oracle.json` -- layouts, pixel data, conversions and the 22-type converter table exactly; resampled pixels and DXT blocks within the tolerances the row above names. The oracle driver is `tools/xna-pipeline-oracle/graphics/`. The node and vertex types (`XNAPP-095`, `XNAPP-096`) still need both their implementation and their measurements. |
...
`XmlImporter` leg of `074` remains, as `XNAPP-230`). Phase 6's texture, font, material and animation sides are done
(`XNAPP-090`–`094`, `097`, `099`, `098` partial); its remaining types are `XNAPP-095` (nodes
and meshes) and the second half of `XNAPP-096` (the vertex channels, whose measurements are
already in the corpus), which need their measurements before their code -- extend
