...
Two corpora are driven by that one oracle, and they answer different questions.
`corpus.json` (39 cases) is the one described above: what a build *produces*, compared
semantically by `compare.py` per §24. `errors.json` (25 cases) is the corpus of builds that are
meant to go wrong, one per failure class with a control beside each rule, compared by
`XnaErrorParityTests.cpp`: the outcome (refused, built, or built with a warning), and the subject
each side's diagnostic has to name. The sentences are each implementation's own; the parameter's
name, the missing file and the undeclared identifier belong to the content, and those are what is
compared. A case with no such shared token -- a corrupt file is simply corrupt -- is compared on
its outcome alone, which the manifest records.
...
| `XNAPP-008` | Provenance gate: a test that fails if any Microsoft binary (`Microsoft.Xna.*.dll`, `XnaNative.dll`, `*.exe` from the SDK) or a Microsoft font appears under `tests/`, `tools/`, `modules/`, `docs/`. | [x] `tools/provenance/provenance_gate.py`, and it is a ctest (`CnaProvenanceGate`). It walks every file Git tracks and fails on five things. **A proprietary binary image**, by content and not only by extension: a PE/COFF file, a .NET assembly or a compound-document installer. This is the check that would fail if the oracle's `Microsoft.Xna.Framework.Content.Pipeline.dll` were ever committed instead of copied into an ignored build directory, and it verifies with `git check-ignore` that the directory really is ignored -- so the arrangement the oracle depends on cannot quietly stop being true. **A font that is not licensed to be here**: the TrueType `name` table is parsed and its *identity* records are matched (copyright, family, subfamily, full and PostScript names, trademark, manufacturer), deliberately not the description -- Liberation Mono's description names Courier New in the sentence that says it is metrically compatible with it, and a cruder check flags the one font in this tree that is licensed for it. Every tracked font must also have a `PROVENANCE.json` row naming a licence, with a digest that matches the bytes. **A production source attributed to someone else**: a copyright notice naming Microsoft, MonoGame, FNA or SharpDX, or a path naming one of them. This is a check on attribution headers and paths rather than a grep for words, because `MonoGame` and `FNA` are named legitimately hundreds of times here -- in comments recording what was measured, in test names, in documentation -- and a gate that failed on those would be turned off within a day. What it proves is that no file arrived carrying someone else's header; what it cannot prove is that one was not laundered, and the tool says so. **An undeclared vendored third party**: every directory under `vendor/` needs a row in `tools/provenance/third-party.json` with an upstream, a licence and a licence file that is in the tree, and the manifest may not name a directory that is absent. **A licence header that is not this project's**, `MS-PL` or the `MIT` the ENet-facing files carry with CNA's own copyright line. A production source with *no* SPDX identifier is licence hygiene rather than provenance -- it says nothing about where the file came from -- and there were 361 of them, almost all in `modules/renderers`, from long before this plan; those are ratcheted against `tools/provenance/license-baseline.json` so the count is visible and may only fall. The gate finds **0** on this tree. And because a gate that has never failed is a gate nobody has tested, `provenance_gate_selftest.py` (ctest `CnaProvenanceGateSelfTest`) builds a throwaway repository, plants one of each -- a planted `MZ` image named `Microsoft.Xna.Framework.dll`, a font whose `name` table says `Segoe UI`, a source carrying `Copyright (c) The MonoGame Team`, a source path with `monogame` in it, a source declaring `GPL-3.0-only`, an undeclared `vendor/` directory and an un-ignored oracle path -- and asserts that each is found. |
...
| `XNAPP-043` | `VideoContent` (7 properties, `Dispose`). | [x] Done, and this row was stale: `VideoContent` landed with `XNAPP-210`, where the build-time probe it needed was decided. All seven properties and `Dispose` are implemented and tested -- `XnaVideoContent.AReadableVideoAnswersItsOwnShape` reads `Width`, `Height`, `FramesPerSecond`, `BitsPerSecond`, `Duration`, `VideoSoundtrackType` and `Filename` off two real files and checks that every property keeps answering after `Dispose` and that a second `Dispose` is accepted, which is XNA's own behaviour; `XnaVideoContent.EveryRefusalIsTheOneSentenceXnaGives` covers the eager constructor's refusals. |
...
| `XNAPP-045` | Compile-parity tests for every Phase 3 type (construct/derive/static_assert), plus behaviour tests. | [x] `XnaPipelineCoreTests.cpp`: 19 tests over every Phase 3 type, including an XNA-shaped importer and processor registered into a canonical registry and driven through canonical contexts, `DefaultProcessor` through `Build()`, `Convert`, the scanner, and parameter conversion edges. The two things this row was waiting for have landed: `VideoContent` with `XNAPP-043`, and the nested-build members with `XnaPipelineBridge.BuildAssetAndBuildAndLoadAssetRunNestedBuildsOnTheCanonicalGraph`, which drives `ContentProcessorContext::BuildAsset` and `BuildAndLoadAsset` through the real coordinator rather than a stub. Nothing in Phase 3 is now untested. |
...
| `XNAPP-098` | Compile-parity and behaviour tests for every Phase 6 type. | [x] `XnaGraphicsBitmapTests.cpp`: 11 tests over the texture side (`XNAPP-090`-`092`), `XnaFontDescriptionTests.cpp`: 10 over `XNAPP-093`, `XnaMaterialContentTests.cpp`: 15 over `XNAPP-094`, `XnaAnimationContentTests.cpp`: 13 over `XNAPP-097`, `XnaVertexCollectionTests.cpp`: 11 over the first half of `XNAPP-096` and `XnaNodeContentTests.cpp`: 15 over `XNAPP-095` and the second half of `XNAPP-096`, each comparing against `tests/reference/xna40/graphics/graphics-content-oracle.json` -- layouts, pixel data, conversions and the 22-type converter table exactly; resampled pixels and DXT blocks within the tolerances the row above names. The oracle driver is `tools/xna-pipeline-oracle/graphics/`. `XNAPP-096`'s `CreateVertexBuffer` is covered by `XNAPP-134`'s model tests in `XnaProcessorTests.cpp`. Every Phase 6 type is covered and this row had no remainder left to name. |
...
| `XNAPP-136` | `SoundEffectProcessor`/`SongProcessor` (`Quality`), `SoundEffectContent`, `SongContent`; `VideoProcessor` (`VideoSoundtrackType`). | [x] The two audio processors and both content types are done, over 8 more `audio-content-oracle.json` cases. Measured: both processors default to `Quality=Best`; `SoundEffectProcessor.Process` **converts its input in place** -- at `Best` the source is left exactly as it is, and at `Medium` and `Low` it becomes ADPCM at that quality, answering the same numbers the plain `convert/adpcm_*` cases do; both processors refuse a null input with `ArgumentNullException("input")`; and `SoundEffectContent` and `SongContent` declare **no public member at all**, so CNA's carry the converted audio behind CNAEXT accessors. `SongProcessor.Process` is `EXTERNAL_BLOCKED`: a song is Windows Media audio, whose encoder exists only on the platform that owns it and whose behaviour could not even be measured -- XNA's own never returns under the oracle's Wine prefix. `VideoProcessor` is done too, which is what this row was waiting for: `XNAPP-043`'s `VideoContent` is its input and its output, `VideoSoundtrackType=Music` is asserted against the measurement by `XnaProcessorDefaults` and gated by `processor_defaults_gate.py`, the enum's `Music=0 Dialog=1 MusicAndDialog=2` by `XnaVideoProcessor.TheEnumValuesMatchXna`, and the per-target answer by `XnaVideoProcessor.TheAnswerIsTheSameForEveryTargetXnaHas`. |
...
| `XNAPP-138` | For every processor: tests with explicit values, omitted values (XNA defaults), and property interactions; defaults asserted against §10. | [x] The three halves of this row are each covered, and one of them is now mechanical. **Omitted values** -- what a processor answers when a build sets nothing -- is `XnaProcessorDefaultsTests.cpp`: all twelve processors constructed and **all 47** properties asserted against what Microsoft's own classes answered, with `processor_defaults_gate.py` tying each assertion back to the inventory so the table cannot drift and cannot cover fewer than 47 (`XNAPP-023`). **Explicit values** are the per-extension `processor` legs the input matrix names for all eighteen extensions, which `inputs_matrix.py check` fails if any of them names a test that is not in the tree -- `XnaTextureExtensionTests.cpp` alone drives all seventeen `TextureProcessor` property cases against `textureprop/*`. **Interactions** are where the measurement found them and they are recorded in their own rows: colour-keying before premultiplication, `ResizeToPowerOfTwo` before mipmap generation, `PremultiplyAlpha` truncating rather than rounding, and `GenerateMipmaps` under a profile that refuses a non-power-of-two source. §10's table is no longer the thing asserted against -- the defaults are read from the runtime and §10 agrees with them, which is what the gate checks. |
| `XNAPP-139` | The `FontTextureProcessor` **route**: a sheet of glyph images named on an asset of its own, measured against genuine XNA and reproduced. | [x] The twelfth of XNA's twelve processors and the last one the canonical graph could not route: the class existed, a build could not name it, and a `.contentproj` asking for it was refused -- 44 assets across the public samples had nowhere to go. Two things this plan said about it were wrong and are corrected here. `XNAPP-250` said a `.contentproj` cannot name it on an asset of its own; **it can**, and genuine XNA builds one. `XNAPP-132` said what its `Process` produces is not comparable beyond its boundary, because `SpriteFontContent` publishes no member; **the built `.xnb` is comparable**, and CNA's is now byte-identical to XNA's on every ordinary sheet. Six sheets were built by the genuine pipeline and every rule is read off those files. **The separator is magenta**, fixed rather than taken from the image: a sheet whose cells are separated by transparent black is refused although it has perfectly good glyphs in it. **The scan starts at the first magenta texel**, which is the one rule that explains all six at once -- a 2x2 image of four ordinary colours has no magenta and is refused, and a sheet whose *corner* texel is white while the rest of its border is magenta still answers exactly its three glyphs rather than a spurious fourth. A glyph does **not** have to be enclosed: one running to the right edge with no separator column after it is still a glyph. Everything else follows from the glyphs -- the characters run consecutively from `FirstCharacter`, the line spacing is the tallest glyph, the spacing is zero, each cropping rectangle is the glyph's own size at the origin and each kerning triple is `(0, width, 0)`. Two things about the packer came out of it that the `.spritefont` route had wrong: **the gap between glyphs is two texels and the margin one**, read straight off XNA's own atlases, and **the placement order is tallest then widest**, which a sheet of 4x6, 7x6 and 3x6 cells settles. The gap is applied to the sheet route only, where it is measured; the description route keeps its own, because its packer is CNA's own either way and two texels there costs an atlas that is twice as tall. The width choice is the one thing two measurements do not agree on -- two 5x7 glyphs go into an 8-wide atlas and three into a 16-wide one, where minimising area would have put the three into 8x32 -- so that case is a recorded divergence and the three ordinary sheets are byte-identical. `TextureFormat` is honoured and its XNA default is `Color`: this processor does **not** compress its atlas, where `FontDescriptionProcessor` always writes DXT3, which is XNA's own difference and not an oversight. |
...
| `XNAPP-167` | Per-extension fixtures, importer/processor/XNB/CNB/malformed/target tests; differential comparison per §21. | [x] This row is the input matrix, and the matrix is closed: **18/18 `IMPLEMENTED+TESTED`**, every extension naming an importer test, a processor test, a source-to-XNB test, a source-to-CNB test, a malformed-input test and a target test, with `inputs_matrix.py check` failing if any named test or fixture is not in the tree (ctest `XnaPipelineInputParityMatrixIsCurrent`). The texture half is what this row was written for and is described above; the rest arrived with their own phases, and the last of them was `.xml` (`XNAPP-261`). Two of the source-to-CNB legs are documented refusals rather than builds -- `.fx` because a `.cnb` carrying Direct3D 9 bytecode is unloadable on every CNA renderer that is not Direct3D 9, and `.xml` because an `.xnb` is what an XNA object graph is -- and both are recorded with `ContentPipelineRegistry::DocumentAbsentWriter()` so the refusal says which of omission and decision it is. The §21 differential comparison runs over 105 corpus cases: **45 identical, 41 accepted with a recorded reason, 0 open**. |
...
| `XNAPP-182` | Atlas/glyph/kerning/line-spacing/baseline differential vs XNA for an OFL font registered in the Wine prefix. | [x] Seven `.spritefont` cases joined `corpus.json` -- five sizes of Liberation Mono, one with `<Spacing>`, `<UseKerning>false` and a `<DefaultCharacter>`, and one with two out-of-order regions -- and are compared field by field through the independent parser. **The kerning was already exact: the ABC advance triples agree for all 95 characters, and so does the cropping origin for 94 of them, which is the half of a font that decides where text lands.** Everything else needed work. **Line spacing was a pixel short**, and getting it right needed four faces: `ceil(ascender - descender + lineGap, scaled)` and `ceil(ascender) + ceil(descender)` both fit Liberation Mono's five sizes, and they disagree on Arial, whose line height is 21.465 px at size 14 where the separately rounded pair sums to 21 and XNA answers 22. The first rule holds for all eleven builds measured (Liberation Mono, Courier New, Arial, Georgia). FreeType's own `metrics.height` rounds to nearest, so the unrounded product is ceiled instead. **XNA hands the writer a DXT3 atlas**, which CNA wrote as `Color`: 16 KB against 128 KB for the same 95-glyph font. DXT3 rather than DXT1 or DXT5 because a glyph is a coverage mask and DXT3's four explicit alpha bits are what a mask wants. Compressed for `.xnb` only -- the `.cnb` container is CNA's own and keeps the lossless atlas -- which makes this the one route that looks at the container it is building for. **The sheet was the wrong shape.** XNA's atlases are 128x64, 128x128, 128x256, 256x256 and 256x512 across the five sizes; CNA chose a square and trimmed the height, transposing three of them. It now packs tallest-glyph-first (a shelf is as tall as its tallest member, and character order let one accented capital raise every shelf), tries every candidate width, and keeps the smallest area, breaking a tie towards the squarer sheet -- a 16x1024 atlas holds the same glyphs and is a poor texture everywhere. Five of the seven now agree exactly; one is transposed on a genuine tie and one is *half* XNA's size. **`<DefaultCharacter>` outside every region was refused**, where XNA adds it: `font_regions.spritefont` covers 42 characters in its regions and XNA's font carries 43, sorted, so the fallback lands where its code point belongs. The old refusal's reasoning -- that such a font could never draw its fallback -- was true only because the pipeline declined to put it there. **`<UseKerning>false` meant the wrong thing.** XNA drops both bearings and keeps the ink width as B, so the line advances by the glyph; CNA folded the whole advance into B and shifted the ink with the cropping rectangle, which keeps the text at its metric width. A description asking for tight text got loose text. What remains is recorded in `decisions.json`, path-scoped so a reason covers only what it explains: the two rasterizers disagree about a glyph's ink by a pixel (40 of 95 boxes identical, 50 one wider, four two wider, one wider and taller, never narrower) and everything derived from a box follows it; XNA's cropping *height* is one to five pixels more than its own line spacing by an amount no metric predicts (Courier New and Liberation Mono are metrically identical and answer 41 at size 24 where Arial, whose line height is larger, answers 40 -- eleven builds over four faces and no rule fits), which `SpriteFont.MeasureString()` reports, so a measured string is up to five pixels shorter here; and the space's cropping rectangle is XNA's degenerate one. |
...
| `XNAPP-191` | Genuine-compiler verification: build `.fx` fixtures through `BuildContent` under Wine (which uses the prefix's `d3dx9`), compare the Effect container CNA's route produces with a real `fxc`-compatible backend where one exists; record exactly which backend produced what. Closes or re-scopes `XNAP-A4`. | [x] Seventeen effects now go through the genuine `BuildContent` and CNA's own route, and every differing byte is accounted for: **195 inside the compiler's version string and 54 of padding, and nothing else at all.** XNA compiles in-process through d3dx9 and stamps `Microsoft (R) HLSL Shader Compiler 9.26.952.2844` into each shader blob; the June 2010 SDK's `fxc.exe` -- the newest legacy compiler that can write an `fx_2_0` container -- stamps 9.29.952.3111. The padding is the same coin's other side: d3dx9 leaves the bytes after each aligned string as whatever was in its heap (`but`, `st_`, fragments of other names) where fxc zeroes them. Neither is reproducible without running Microsoft's own compiler in-process, and the second should not be reproduced. Everything CNA decides is now XNA's, which took understanding the `0xBCF00BCF` header. It was documented here as *magic, offset, two zero dwords*, measured from the processor called directly -- and a build writes something else entirely. **It is one pair of dwords per pass**, so its length is `8 + 8 x passCount` and CNA's fixed sixteen bytes were the wrong size for every multi-pass effect. The first of each pair is a bitmask of the device state that pass assigns -- 1 blend, 2 depth-stencil, 4 rasterizer, which are three of XNA 4.0's four state objects -- with the second shifted up three places; the second is a bitmask of the sampler *registers* the pass's shaders bind, which a fixture pinning one sampler to `register(s3)` settled by answering 8 rather than 1. Reproducing it needed the container walked at build time: a pass's own state list gives the groups, and the large-object table records the technique and pass each shader blob belongs to, which is the attribution a per-pass sampler mask needs. The render-state numbering is dense from `D3DRS_ZENABLE` and is measured rather than derived -- three fixtures assign every state of one group and nothing else, and the genuine build answers 1, 2 and 4. A state outside the measured sets makes the summary refuse: an effect this build cannot describe keeps the zero header it always had rather than getting a guessed one. The walk is bounds-checked throughout and answers empty on anything unexpected. One harness gap closed with it: the semantic comparison was a script nobody ran. It now runs from `XnaDifferentialBuildTest` itself, after that test publishes the side it compares -- two gtest cases are two processes with no ordering between them, so it could not have been a test of its own. |
| `XNAPP-192` | Reach vs HiDef, includes/include dependencies, macros, techniques/passes/state/parameter/sampler reflection compared. | [x] Fifteen fixtures, each `fx_minimal.fx` plus exactly one feature so a difference in the container is attributable: an initialized parameter, a texture and sampler, two samplers, one sampler pinned to `register(s3)`, two techniques, two passes with state on both, an `#include` of a committed header, a `#ifdef` arm nothing defines, and one fixture per state group assigning every member of it. Reach against HiDef is `fx_source` built both ways. All fifteen agree with XNA to the byte outside the compiler version string and its padding (`XNAPP-191`), which covers the reflection this row asks about: the parameter table, the technique and pass graph, the per-pass state assignments, the sampler objects and their registers, and the shader blobs themselves are all compared by `compare.py` through the independent parser. Two things came out of it beyond the container. The differential harness did not copy an `.fxh` beside an `.fx`, so an effect with an `#include` was a case CNA refused and XNA built -- the include is a real build dependency and the corpus now carries one. And XNA's `EffectProcessor` has no parameter for preprocessor defines and always compiles with none, which `fx_macro.fx` pins: CNA's own `defines` parameter is an extension, and the two agree exactly when it is unset. |
...
| ID | Task | State |
|---|---|---|
| `XNAPP-210` | `WmvImporter`/`VideoProcessor`: measure what XNA emits (the `.xnb` is a header plus an external media file); implement the container semantics; codec availability per §26. | [x] `VideoContent`, `WmvImporter` and `VideoProcessor` are implemented and their measured behaviour reproduced. What the measurements settled: the **constructor is eager**, so the same missing file is a `FileNotFoundException` through the importer and an `InvalidContentException` through `VideoContent` itself, and a **null filename is not refused as null** -- it reaches the message as an empty name; every unreadable source carries the one sentence `Video file X is invalid. Please make sure that the video is not DRM protected and is a valid single-pass CBR encoded video file.`; `VideoProcessor` defaults to `VideoSoundtrackType.Music`, refuses a null input with `ArgumentNullException("input")`, and answers **its own input** rather than a copy. **The source-to-XNB leg is done**, which is what this row's remainder was: a `.wmv` builds to an `.xnb` naming the external media file, which the coordinator publishes beside it as a deployment file, and the independent parser reads the result as a `VideoReader` root (`XnaSourceToOutput.EveryRoutableSourceBuildsToXnb`, and the `.wmv` row of the input matrix). The one thing left is not local and is now its own row: `XNAPP-211`. || `XNAPP-220` | Measure XNA's own **successful** WMV decode -- the `VideoContent` a readable file produces under Microsoft's own importer -- and hold CNA's to it. | [!] **EXTERNAL: no Media Foundation under this Wine.** Split out of `XNAPP-210`, whose local half is done, so that a finished task does not own unfinished work. Every *refusal* XNA gives for a video was measured and is reproduced; the successful path could not be, because constructing a `VideoContent` needs Media Foundation and Wine 10.0 aborts on `mfplat.dll.MFCreateVideoMediaType` and faults inside `XnaMediaHelper_1.dll` for a 64x48 source (`docs/xna-content-pipeline-media.md` §6). Every `videoprocessor/target_*` case in the oracle is an `SEHException` for the same reason. CNA's own half is held to the container format instead -- the shape read out of the file, the same answer for every target -- and the `.wmv` row of the input matrix records `windowsPhone` and `xbox360` as `UNVERIFIED` rather than claiming them. What would close this is a Windows machine with Media Foundation, or a Wine with a working `mfplat`; nothing in this repository can. |
| `XNAPP-211` | Measure XNA's own **successful** WMV decode -- the `VideoContent` a readable file produces under Microsoft's own importer -- and hold CNA's to it. | [!] **EXTERNAL: no Media Foundation under this Wine.** Split out of `XNAPP-210`, whose local half is done, so that a finished task does not own unfinished work. Every *refusal* XNA gives for a video was measured and is reproduced; the successful path could not be, because constructing a `VideoContent` needs Media Foundation and Wine 10.0 aborts on `mfplat.dll.MFCreateVideoMediaType` and faults inside `XnaMediaHelper_1.dll` for a 64x48 source (`docs/xna-content-pipeline-media.md` §6). Every `videoprocessor/target_*` case in the oracle is an `SEHException` for the same reason. CNA's own half is held to the container format instead -- the shape read out of the file, the same answer for every target -- and the `.wmv` row of the input matrix records `windowsPhone` and `xbox360` as `UNVERIFIED` rather than claiming them. What would close this is a Windows machine with Media Foundation, or a Wine with a working `mfplat`; nothing in this repository can. |
...
| ID | Task | State |
|---|---|---|
| `XNAPP-240` | `.contentproj` parser → canonical build graph (§18 schema); `cna-content build Foo.contentproj`; unknown metadata refused, not guessed. | [x] `Tasks::ContentProject` reads the whole §18 schema -- `Compile` with `Name`, `Importer`, `Processor`, `ProcessorParameters_X` and `Link`; `Content`/`None` with `CopyToOutputDirectory`; `XnaPlatform` (with `Xbox 360` and `Windows Phone` normalized), `XnaProfile`, `XnaCompressContent`, `ContentRootDirectory`, `XnaFrameworkVersion`, `ProjectGuid`, and the references -- and fills a `BuildContent`, which runs the canonical coordinator. No second build engine and no second project format; `.cna-content.json` is untouched. **`Condition` is decided rather than guessed**: every condition the 170 sample projects use is one comparison of two quoted, property-expanded strings, evaluated in document order so a default-value guard means what it says, and anything else -- a function call, a numeric comparison -- is refused by name rather than silently adding or dropping assets. `UnroutableEXT()` names every asset whose importer or processor is not built in, all at once. **The two remaining pieces are done.** `cna-content build Foo.contentproj` is the command line: a `.contentproj` source is read, handed to `BuildContent` -- the task XNA's own `.targets` drives -- and that task runs this same coordinator with the project's platform, profile, compression and per-asset metadata, and with XNA's own leniency. One thing had to be added for it to be honest: `BuildContent` built its own registry, so a host that had registered components of its own would have had them silently absent; `setRegistryFactoryEXT` passes the caller's factory through, and the command line passes its own. And the `Content`/`None` items are **copied** as XNA's targets copy them, honouring `CopyToOutputDirectory` (`Never`, `PreserveNewest` and `Always` each meaning what they say on a second run) and `Link`, which decides where the copy lands rather than where the source is. Five tests in `XnaContentProjectCommandLineTests.cpp` cover the route, the copies, the project's own platform reaching the container byte, the refusal that names every missing component at once, and the leniency a project build takes where the plain tool refuses. |
...
| `XNAPP-242` | Build the public XNA samples' `.contentproj` files (108 available locally) through the parser and record which items route, which need a custom processor, and which fail and why. | [x] **170 projects on this machine, all 170 read**, carrying 8795 source assets; 115 route entirely with no custom component. The reading sweep is a test (`XnaContentProject.ThePublicSamplesProjectsAreReadAndTheirRoutesReported`) rather than a one-off, so a schema this reader stops handling fails the build. Two things it found that a written fixture would not have: **126 of the 170 use a `'$(Configuration)|$(Platform)' == 'Debug|x86'` property group**, which the first reader refused outright, and the `.fbx` and `.x` component names were still mapped to "not implemented in this build" after both importers had landed. What the remaining 55 need is a custom processor of the sample's own -- `SkinnedModelProcessor`, `SkyProcessor`, `SpriteSheetProcessor` and seven more -- which is `XNAPP-280`'s external-extensibility route and not a defect here. **They are built now, not only read**, and building them found a defect nothing else could have. Every one of the **149 FBX 6.1 ASCII files in the samples was refused** while all six fixtures written here were read: an Autodesk exporter wraps a long value list across lines and writes the comma that separates the last value on one line from the first on the next **at the start of the next line**, and the reader ended the list at the newline and then found a comma where a node name should be. `make_fbx_fixtures.py` writes `fbx_wrapped_values.fbx` in that shape now -- it reads as the byte-identical graph to the unwrapped mesh beside it -- and **all 197 FBX files in the samples import**, 149 ASCII, 30 binary 6100, 17 binary 7100 and one binary 6000. Over the whole sweep: **20 of the 170 build end to end**, 55 are refused by name for a component this build has not got, and the remaining 95 are blocked by this machine rather than by CNA -- 45 name a Windows font family that is not installed here (XNA resolves an installed family through GDI+; CNA resolves a font *file*), 40 name a source the sample snapshot does not carry, 6 name a pipeline assembly (C++ has no assembly loading), 2 need an `fxc` this machine has not got, and 2 name two sources that collide on one logical name, which XNA refuses too. `XnaContentProject.ThePublicSamplesProjectsAreBuiltEndToEnd` builds a bounded 24 of the routable ones on every run -- 8 build, 16 are blocked by one of those named classes -- and **fails on a failure outside them**, which is the shape that would have caught the FBX wrapping the day it was written. |
### Phase 19 — target platform and profile parity

| ID | Task | State |
|---|---|---|
| `XNAPP-250` | Windows: differential tests for every route (§23). | [x] Every route the corpus can name is built for Windows and compared: all ten importers and every processor an asset can name -- the three texture processors, `SoundEffectProcessor`, `SongProcessor`, `VideoProcessor`, `ModelProcessor` over both `.x` and `.fbx`, `EffectProcessor` over fifteen effects, `FontDescriptionProcessor` over seven descriptions, and `PassThroughProcessor`. `MaterialProcessor` is the one a `.contentproj` cannot name on an asset of its own -- its input is a `MaterialContent`, which no importer produces, so it is reached through `ModelProcessor`. `FontTextureProcessor` was recorded here as the other, and that was wrong: a project names it on a `.png` and XNA builds it, which is what `XNAPP-139` measured and closed. 61 of the corpus's 77 cases are Windows, over both profiles. |
| `XNAPP-251` | Windows Phone: `BuildContent` fixtures for `WindowsPhone`/`Reach`; texture format, profile, media, effect and model differences measured and implemented. | [x] Twelve cases across texture, audio, video, model, effect and font. The phone's own rule is one sentence and CNA did not have it: **`The Windows Phone platform does not support custom shaders.`** XNA refuses an `.fx` for that target at build time, where CNA compiled one happily -- a project that would fail on a device. Everything else the phone builds, it builds the same way Windows does: a `.wav` sound effect, an `.x` model and an `.fbx` model come out byte-identical, and the font atlas is DXT3 there too. What the sweep actually found was not phone-specific at all, because it was the first case to put `probe_4x4.png` through `TextureProcessor` without asking for DXT. **XNA's colour key is on by default, and keys magenta.** `ColorKeyEnabled` is True and `ColorKeyColor` is (255, 0, 255, 255) for all three texture processors -- measured long ago and recorded in the processor-default table, and never wired into the façade's mapping, so every magenta texel stayed opaque where XNA turns it transparent. **And the key writes transparent *black*,** not merely alpha zero, which only shows when premultiplication is off and the keyed texel keeps its colour. **And premultiplication truncates rather than rounds:** a channel of 30 at alpha 200 is 23 in XNA and was 24 here, which is every partially transparent texel of every texture. Two translation gaps came with them. `ColorKeyColor` is a `Color` and a project writes four components where the canonical parameter took three, so naming a key at all refused the build; and `PremultiplyAlpha`/`ResizeToPowerOfTwo` reached a processor that had never heard of those spellings, which -- now that an unknown parameter is a warning rather than an error (`XNAPP-267`) -- meant a project's own value was dropped and silently replaced by the default. The mapping carries the translation now. |
| `XNAPP-252` | Xbox 360: `BuildContent` fixtures for `Xbox360`; big-endian payload rules per type, texture tiling/swizzle, `SoundEffect` format, effect bytecode, compression; `RequireVerifiedPlatformPayload()` lifted per type only as each is proven against those fixtures. Device verification stays `EXTERNAL_BLOCKED`. | [~] Ten cases, every route the corpus can name, and **XNA builds all of them for the Xbox** -- there is no route it refuses for that target. The big-endian rule turned out to be one rule with a different unit per payload, and each unit was read off the genuine bytes rather than assumed: a `Color` texel is **reversed as one four-byte unit** (`00FF00FF` becomes `FF00FF00`, and the *texel order* is untouched), a DXT level is **reversed in sixteen-bit words** (`0aca ff79 fdfe` becomes `ca0a 79ff fefd`), a vertex buffer is reversed **by each element's own component** -- the eight `1.0f` and eight `-1.0f` of a model's buffer become big-endian while the nine floats the container itself writes stay little-endian, which is what says the container is little-endian everywhere and only the payloads swap -- and an index buffer by the index. Implemented and **verified byte-for-byte against XNA**: `xbox/png_texture`, `xbox/x_model` and `xbox/fbx_model` are now identical, and `RequireVerifiedPlatformPayload()` is lifted for Texture2D, VertexBuffer and IndexBuffer, each of which now refuses only a *format* it has no measured rule for rather than the platform. **There is no tiling.** This plan recorded that XNA tiles a texture for the Xbox and that CNA's untiled payload was the divergence; the byte swap explains that case entirely, and with it implemented `texture_png_sprite_xbox360` is identical. The decision claiming tiling has been removed -- `compare.py` caught it itself, because a decision whose case has become identical fails. What is left is `EXTERNAL_BLOCKED` on the type the plan expected: the console's shaders are a different instruction set, 980 bytes of Xenos bytecode against 804 of Direct3D 9 for the same source, produced by a compiler that ships only inside XNA Game Studio's Xbox toolchain. `RequireVerifiedPlatformPayload()` is also lifted for **SpriteFont**, whose two halves are separately measured: its atlas is a texture and swaps by the rule above, and its glyph, cropping and kerning lists go through the container's own object protocol, which stays little-endian -- `xbox/font_description`'s kerning triples read little-endian and equal CNA's. **Texture3D and TextureCube are lifted too**, once `XNAPP-255` built the route that makes a fixture possible: `xbox/dds_cube` and `xbox/dds_volume` are byte-identical to XNA's, which is the proof the plan asks for before a guard comes off. Every payload type this corpus can produce for the Xbox is now converted and verified. `[~]` rather than `[x]` for the two things that remain and cannot be finished here: the console's shader ISA, and device verification. |
| `XNAPP-253` | Reach/HiDef enforcement measured and implemented per §20. | [x] Nine cases over both profiles, and XNA turned out to enforce four rules on a Texture2D, in its own sentences and in its own order -- the size first, then the shape. **Reach refuses a texture larger than 2048** in either dimension and **HiDef one larger than 4096**; a 2048x1 builds in both. **HiDef also refuses an aspect ratio above 2048**, which is the rule a 2049x1 trips there while Reach is still refusing it for its size, and which a 4096x1 trips even though 4096 is a legal size. And **Reach refuses a mipmapped texture whose dimensions are not powers of two**, where HiDef builds one -- a non-power-of-two texture without mipmaps is fine in both, and DXT compression is allowed under Reach. Implemented on the final texture, after the resize, because that is the one the profile has to hold, and **only when the build is producing an `.xnb`**: a `.cnb` has no target profile at all, nothing in that container records one, and refusing a 4096-wide texture because a flag defaulted to Reach would invent a limit CNA's own format does not have. The effect side of §20 was settled by `XNAPP-191`, which refuses shader model 3 under Reach. Whether Reach carries the aspect rule too cannot be observed -- a Reach texture wide enough to break an aspect limit of 2048 is already wider than Reach's own 2048 -- so it is not asserted there. |
| `XNAPP-255` | A cube or volume DDS carried from source to a `TextureCube` / `Texture3D` output. | [x] Done, and **byte-identical to XNA on both platforms** -- `texture/dds_cube`, `texture/dds_volume`, `xbox/dds_cube` and `xbox/dds_volume` all read `same`. XNA's `TextureImporter` reads all three shapes out of a DDS and routes a cube to `TextureCubeContent` and a volume to `Texture3DContent`; the genuine build produces a `TextureCubeReader` of six 64-byte faces and a `Texture3DReader` of one 256-byte volume for the corpus's two fixtures, on Windows and on the Xbox alike (`texture/dds_cube`, `texture/dds_volume`, `xbox/*`). CNA *reads* both shapes already -- `ReadDdsSurfaces` answers exactly those types (`XNAPP-165`) -- and has no route that carries either to an output: the canonical image importer refuses with `DDS source is a cube map; this route builds a Texture2D`. The writers existed on both sides of the gap and what was missing was the middle, which turned out to be three separate things. The importer now **declares three output types and answers by shape**, as XNA's own does; the cube is built from `XNAPP-165`'s shared surface reader rather than the older cube decoder beside it, which accepts a narrower set of pixel formats and refused the corpus's own fixture. The mapping had to stop naming a processor for the three XNA texture processors: XNA has *one* processor over three content types and CNA has three, so no canonical name can stand in for it -- naming `CNA.TextureProcessor` for a cube resolves to a processor that does not accept one. A mapping that sets `chooseByImportedType` contributes its defaults and its parameter names and leaves the processor unnamed, which is what lets the graph pick by the type the importer actually produced. And the cube and volume processors take no parameters at all, where a `.contentproj` naming `TextureProcessor` carries three; their refusal is now the typed unknown-name one, so `--xna-compatible` drops them with a warning as XNA would rather than failing. **That last step found a defect in `XNAPP-267`'s leniency:** the drop-and-retry loop re-read the parameter count each pass, so the bound shrank along with the set and the last refusals went unhandled. It only appears once two parameters are wrong, which no earlier case had. Closing this closed the last two types of `XNAPP-252` with it: `xbox/dds_cube` and `xbox/dds_volume` prove the Texture3D and TextureCube byte order end to end, and both guards are lifted. |
| `XNAPP-254` | XNA's mip-reduction filter identified and implemented. | [x] Identified from its **impulse response**, which is what the fixture this row asked for turned out to be: a 16x16 image with four isolated white texels on black, whose reduced level shows the kernel directly. One texel becomes a 2x2 of 48, 7, 7, 1 out of 255, and that is the outer product of the separable four-tap **{1/16, 7/16, 7/16, 1/16}** -- not the 2x2 area average CNA took, which answers 64, 0, 0, 0. A step-edge fixture beside it confirms the weights and settles the edge rule: taps that fall outside are **clamped**, where wrapping is wrong by up to 24 units at a border. An **odd dimension** has no such alignment and XNA area-averages there instead, which a 3x2 confirms exactly on its alpha channel. Implemented, and every mipmapped case in the corpus is now **within one unit** of XNA: 61 bytes of 2944 differ, all by exactly 1, where before the levels differed by up to 29. The residual is not a fittable error -- sweeping the tap weight finds nothing that beats 1/16, and it falls in both directions, which is what a dither looks like. It is recorded in `decisions.json` with those numbers rather than chased further. One case that used to read `identical` no longer does: a 2x2 source's chain matched the old area average by coincidence, because at that size the two filters differ only in the rounding of one byte. Trading a coincidence for the right kernel is the trade this row is for. |
| `XNAPP-256` | XMA: the final audit the owner's decision requires, everything around the encoder that can be built here, and a build-time codec interface an external encoder attaches to without changing any public pipeline API. | [!] **XMA ENCODER EXTERNALLY UNAVAILABLE**, and the four conditions the owner set for calling it that are all met and recorded in [`docs/xma-encoder-backend.md`](../docs/xma-encoder-backend.md): no public specification sufficient to implement a conforming encoder (Microsoft published the *container* and never the bitstream; the open-source work is decoding, reverse-engineered from streams); no legally usable or licensable encoder (it ships inside the XDK and XNA Game Studio, under licences that permit a registered developer's own use and not redistribution); **none in FFmpeg** -- measured on this machine, `ffmpeg -codecs` lists `xma1` and `xma2` as `D.AIL.`, decode only, and `ffmpeg -h encoder=xma2` answers *"Codec 'xma2' is known to FFmpeg, but no encoders for it are available"*; and Microsoft's own cannot be a CNA dependency. Everything else was done. **The format was decomposed from the genuine file**: the Xbox 360's whole format block is big-endian, so the tag reads `0x0166` only when read that way and `0x6601` otherwise, and the 34-byte `XMA2WAVEFORMATEX` is swapped field by field -- `NumStreams` 1, `ChannelMask` 1, `SamplesEncoded` 22528, `BytesPerBlock` 32768, `PlayBegin` 0, `PlayLength` 22050, `LoopBegin` 384, `LoopLength` 22016, `LoopCount` 0, `EncoderVersion` 4, `BlockCount` 1, every one of them asserted by `XmaEncoderService.TheGenuineXboxSoundEffectDecomposesAsABigEndianXma2WaveFormatEx`. **The loop and duration semantics follow the codec's 128-sample subframe**: the sample rate is rounded down to a multiple of 128 (44100 becomes 44032 = 344x128), `SamplesEncoded` is the *padded* length and is longer than the source, `PlayLength` keeps the source's exact frame count, the loop region is quantized (384 = 3x128 and 22016 = 172x128 against the PCM builds' exact 0..22050, so a loop point moves by up to 127 frames), and the duration is unchanged because XNA computes it from `PlayLength` and the source rate rather than from the encoded size. The container around it is validated by the independent parser. **The seam is `CNA::Content::Pipeline::XmaEncoderService`**, deliberately the same shape as `EffectCompilerService` because it is the same problem: `--xma-encoder`, `--xma-encoder-launcher` and `--xma-encoder-arg` (with `CNA_XMA_ENCODER*` environment forms and no `PATH` search, because an encoder for this codec is attached on purpose), a contract every `.xma` writer already satisfies -- read a RIFF WAVE, write a RIFF whose `fmt ` is the `XMA2WAVEFORMATEX` and whose `data` is the payload -- and an `XmaEncoderIdentity` in the fingerprint so two encoders are two identities. Nine tests drive the whole seam with a stub the test writes: the contract, the argument template and the quality reaching it, the format block becoming the content's format, an output that is not a `.xma` refused by name, an encoder that fails reported in its own words, and the two refusal classes kept distinguishable. `AudioContent::ConvertFormat(Xma, ...)` -- XNA's own API for asking -- encodes through it when one is attached and otherwise refuses with a message beginning with the exact sentence, which `.wav`'s `xbox360` row now reads verbatim instead of `UNSUPPORTED`. The remaining work is one step, listed in the document: give the Xbox branch of the `.wav` route the encoder. It depends on nothing else. |
...
| ID | Task | State |
|---|---|---|
| `XNAPP-265` | `tools/xna-pipeline-oracle/differential/`: C# `BuildContent` driver (mcs + Wine), corpus manifest, per-case result JSON. | [x] `DifferentialOracle.cs` runs Microsoft's own `BuildContent` -- the task an XNA project's MSBuild invokes -- over the committed `corpus.json` (39 cases today), one case per (source, importer, processor, parameters, platform, profile), and records per case whether it built, the compiled bytes, the diagnostics and the files the task says it wrote, read from the task's own output properties. `XnaDifferentialBuildTests.cpp` builds the same corpus through CNA's own `BuildContent` façade -- the same ItemSpec, the same metadata, the same platform and profile, so both sides are asked the same question -- and compares outcomes. **All 38 now agree.** Getting there found four defects, every one of them invisible to the per-component differentials: XNA refuses to DXT-compress a texture whose dimensions are not multiples of four and CNA silently padded one (measured from both sides -- 2x2 and 3x2 refused, 4x4 built); `TaskItem` folded metadata names to lower case, so every `ProcessorParameters_Scale` arrived as `scale` and no processor recognised it; a mapping's default parameter beat the project's own explicit value, so a `.contentproj` asking for `DxtCompressed` got `Color`; and `ScopedEnvironment` restored a previously-unset variable as present-but-empty, which every later test in the process then saw. The corpus's own model fixture had to change too: `surface.png` was 2x2, which makes `quad_textured.x` a model XNA itself cannot build. |
| `XNAPP-266` | Extend `tools/xnb/xnb_conformance.py` to normalize every root type this plan emits; `compare.py` with the §24 semantics; decision file for accepted differences. | [x] `compare.py` reads both sides through the independent parser -- which shares no code with CNA's writer or reader, so agreeing with it is evidence rather than a tautology -- and sorts every difference into identical, accepted (`decisions.json`, with the reason) or open. A decision whose case has since become identical also fails, so a reason cannot outlive what it explained. **21 identical, 8 accepted, 0 open**, over 39 cases: every case either matches XNA exactly or differs for a reason written down in `decisions.json`. Feeding the parser XNA's own output fixed two of its own gaps first: XNA writes a *null* bone or mesh name when the source node had none (`.x` files' unnamed root frames are ordinary), and the Xbox 360's `WAVEFORMATEX` block is big-endian -- its own `cbSize` only validates when read that way. The comparison then found a runtime-visible defect: **a WAV with no `smpl` chunk got `loopLength` 0 from CNA and the whole frame count from XNA**, so a looped `SoundEffectInstance` built by CNA looped nothing. Fixed as a build-time policy (`SoundEffectLoopPolicy`), because a runtime reader must reproduce what its file declares rather than invent a region its author never wrote. Seven cases were open when the comparison first ran and all seven closed. Four cases had the mesh bounding sphere's first two components swapped, and the cause was not the pipeline: XNA's `BoundingSphere.CreateFromPoints` seeds the sphere from the *later* axis when two are equally wide and FNA -- which CNA followed -- keeps the earlier one, so a right triangle came out mirrored. Measured directly rather than inferred from a model (`framework-packing-oracle.json`, `boundingsphere/*`, including the three-way tie that makes the rule "the last axis wins") and fixed in `modules/math`. Two more wrote `''` where XNA writes a *null* bone name, which is one byte against a StringReader plus a zero-length string. `model_x_skinned` lost its skinning channels entirely -- `ProcessVertexChannel` checked that a weights channel existed and never converted it, so the `BoneWeightCollection` reached the buffer builder, which has no vertex element format for one and dropped it. It now becomes `BlendIndices` (`Byte4`) and `BlendWeight` (`Vector4`), indices being positions in the flattened skeleton rather than in the model's bone list, and the vertex buffer is now byte-identical to XNA's. `VertexBufferContent::SizeOf` had to learn the packed vector types for that, which XNA accepts and CNA refused -- measured rather than assumed (`modelprocessor/vertex_buffer_sizeof_packed`). `model_x_textured` needed three more: a colour channel is converted to packed `Color` before the buffer is built (XNA's own importer answers `Vector4`, so the conversion is the processor's), `VertexColorEnabled` is turned on for a batch that carries colours, and `BoundingSphere`'s Ritter expansion moves the centre along the direction by half the overshoot instead of taking a midpoint -- algebraically the same, and the midpoint form drifted a centre component that should stay exactly zero by one ulp. XNA's generated nested-asset names carry an index -- `surface_0`, not `surface`. One case could not say whether the index is a running counter or per derived name, so a model naming *two* different textures was added to the corpus and answered it: both come out `_0`. CNA generates the same names now, keyed by what was built so the same source built the same way twice still keeps one name and one asset. That two-texture case then found two more. **XNA merges a mesh's geometry batches into one vertex buffer and one index buffer**, giving each part a `vertexOffset` and a `startIndex` into them, where CNA wrote a separate pair per batch -- four shared resources against six, and a different number of buffers at run time; the merged index buffer is `[0,1,2,0,1,2]`, so the indices stay batch-relative and the vertex offset is what places them. And **a generated normal channel is appended after the file's own channels**, not put first: a `.x` declaring `MeshNormals` answers Normal, Color, TextureCoordinate, while one declaring none answers TextureCoordinate, Normal -- measured on the importer itself, because the order is the order of the elements in the vertex buffer. |
| `XNAPP-267` | Error-parity corpus: malformed/unsupported/impossible-parameter/missing-dependency/cyclic/unsupported-target/bad-declaration/missing-glyph/shader-failure cases; XNA and CNA failure classes compared. | [x] `errors.json` is the second corpus the XNAPP-265 oracle drives, 25 cases over thirteen failure classes with a control beside each rule, and `XnaErrorParityTests.cpp` compares three things per case, each able to fail on its own: that the manifest's `expect` still matches what the oracle recorded, that CNA's outcome matches XNA's, and that every token in `mentions` -- the parameter's name, the missing file, the undeclared identifier -- appears in *both* sides' diagnostics. The sentences are each implementation's own and cannot be compared; the subject belongs to the content. **All 25 agree.** The first thing it found was that CNA's task had no diagnostics at all: `BuildContent` drove the coordinator and reported `the content build reported status 1`, naming neither the asset nor the reason, because the coordinator's diagnostics are its streams and `--quiet` was suppressing half of them. The streams are now captured and reported with the coordinator's own three-way split -- a failure is what it wrote to the error stream, a warning is what it labelled one -- and `ContentTask` grew `WarningsEXT()`, because *built with a warning* is a different outcome from *built* and XNA chooses it far more often than a reimplementation expects. **XNA warns and builds where CNA refused, in three places.** A processor parameter naming a property the processor has not got, and one whose written value it cannot convert, are both warnings there -- the processor keeps its default and the asset is built. So is a `.spritefont` character region the font cannot cover: XNA draws those characters from the machine's own default font and warns that redistributing the result may not be licensed. CNA refused all three, which is right for `cna-content` (each is a mistake nothing else in a build will notice) and wrong for a façade standing in for XNA, so the difference became a build-time policy: `ContentStrictness` on the build environment, `--xna-compatible` on the command line, selected by the task and by nothing else. Leniency needed the two faults to be *distinguishable* rather than guessed from a message, so processors now throw `ContentParameterError` carrying which parameter and which of the two mistakes; every other refusal is untyped and still fails, because a missing DXT encoder defaulted past would ship a texture nobody asked for. The missing glyph is drawn with the font's own `.notdef` rather than a second font, because a content build must produce the same bytes on every machine, and the warning says so. **And XNA refuses two things CNA built.** An importer or processor name no assembly defines is `Cannot find importer "..."` there and was a *message* here, after which the build quietly took whatever route the extension suggested -- a project that works here and fails on a real toolchain. The mapping table now records whether XNA defines a name at all, which is a different question from whether CNA has a canonical component for it: `XmlImporter` and `PassThroughProcessor` are real XNA components the canonical graph reaches without being named. And an effect containing shader model 3 built under the Reach profile, which XNA refuses in one sentence. The compiler does not enforce it -- an `.fx` saying `compile vs_3_0` compiles happily under `fx_2_0` and then fails to load on a Reach device -- so the compiled effect is scanned for Direct3D 9 shader version tokens, which are DWORD-aligned in the container; verified against the corpus's two effects, which answer 2 and 3. That also needed `ContentBuildEnvironment` to be *populated*: the coordinator had been leaving it default-constructed, so `targetPlatform` and `targetProfile` never reached a processor at all. Two more measurements came out of the controls. XNA marks exactly two processors obsolete -- `SpriteTextureProcessor` and `ModelTextureProcessor`, both pointing at `TextureProcessor` -- and warns when a project names one; the mapping table carries the sentence and the task emits it. And the font cases only became measurable after two fixture repairs: an XML comment may not contain two hyphens in a row, which XNA's intermediate reader refuses outright and three committed `.spritefont` fixtures did, and installing the corpus's font into the Wine prefix *breaks* it -- Wine already exposes the host's copy, and the duplicate makes the family report its Regular style unavailable. |
...
| ID | Task | State |
|---|---|---|
| `XNAPP-290` | Fuzz targets: DDS, PFM/PPM, X (text+binary), FBX, intermediate XML, MP3 framing, `.contentproj`; sanitizer runs; explicit ceilings documented. | [x] **The two readers written in session 2 are cleared.** `tools/content/xna_model_fuzzer.cpp` has the same two shapes as the intermediate-XML harness beside it (standalone replay and mutation; a libFuzzer entry point on request), and `XnaModelReaderHardeningTests` runs a deterministic mutation pass on every build over the committed corpus, in three shapes because they break different things -- a flip corrupts a value in place, a truncation ends the document mid-structure, and a zero run turns a length or a count into something a reader might trust. The run they were actually cleared under is a standalone **ASAN + UBSAN** build of the two translation units (both depend on nothing but the standard library and zlib, so a sanitizer configuration of the whole project would be several hundred megabytes to prove the same thing about the same two files) over **1,159,999 mutated documents** -- 42,910 `.x` and 61,904 FBX parsed, 1,055,186 refused, no memory error, no undefined behaviour, no leak, and no exception escaping past each reader's own type. Every tree a mutated document produced is walked rather than merely held, so a reader answering a view of freed or out-of-range memory would be caught. Ceilings are checked on the declaration: a `.x` nested past its depth limit and an FBX array claiming four billion doubles are both refused without an allocation being attempted. **The rest are done.** `XnaTextureAndProjectHardeningTests.cpp` runs the same three-shape deterministic mutation over the DDS surface reader, the portable-float-map decoder, the Radiance picture decoder and the `.contentproj` reader -- 400 mutations per fixture for the texture readers and 200 per project over the committed acceptance project plus eight of the public samples' own -- and every one of them must answer a value or its own refusal, with everything it answers walked rather than merely held and its own dimensions checked against the buffer it returned. **It found a real one on the first run**: a mutated DDS header naming **1 929 379 840 mip levels** reached `resize` before anything checked it, and the reader asked for forty-odd gigabytes of empty vectors -- not a refusal, not a crash, not a diagnostic, but a build that stops answering. A texture has as many levels as halving its longest side takes and no more, so a header claiming more is describing something that cannot exist; that is checked on the declaration now, and the case that hung for fifteen minutes finishes in nine milliseconds. **Two readers are deliberately not fuzzed here**, for the same reason: a `.ppm`, `.png`, `.bmp`, `.tga` and `.jpg` are decoded by `stb_image` and an MP3, WMA or WMV by FFmpeg, both properly licensed third-party parsers with their own hardening, and a campaign against them here would be measuring somebody else's code. What is CNA's is the use of them -- what it accepts, what it refuses, what it does with the answer -- and that is measured against the genuine importers in the per-extension differential suites. The row's remaining item is therefore a campaign under a real driver rather than a fixed seed, which needs a clang sanitizer-fuzzer build; the entry points for it are already exported (`-DCNA_XNA_MODEL_FUZZER_ENTRY_POINT=ON`). |
### Phase 24 — performance / determinism

| ID | Task | State |
|---|---|---|
| `XNAPP-300` | Determinism: multi-worker, multi-process builds of the new routes byte-identical; tool identities in fingerprints. | [x] `XnaBuildDeterminismTests.cpp` builds one corpus of **21 sources over thirteen extensions** four times, in **four separate processes**, at 1, 2, 4 and 8 workers, and compares every byte of every file each build produced -- separate processes rather than four calls in one, because a single process can be deterministic by accident: a cache warmed by the first build, a static initialised once, an allocator handing back the same addresses. It does the same for `.cnb`, because the writer is the half that differs between the containers. Both are byte-identical at every worker count. The third case is the one that found a defect: **a node whose build starts a nested one never skipped**. `quad_textured.x` builds its material's texture as a second asset, so its manifest entry records that nested output's writer schema beside its own -- and the up-to-date check compared the entry's schema list with the *writer's* for equality, which for such a node can never hold. Every model with a texture, in every project, rebuilt on every build, for ever, reporting `direct fingerprint changed outside a persisted reason domain` -- which is the fallback label the tool reaches for when nothing else explains a rebuild, so the message said nothing about the cause. What that check is asking is whether the *route* is still the recorded one, so it asks whether the writer's declared schemas are still among the recorded ones rather than equal to them; anything extra belongs to a nested build, whose own inputs are fingerprinted separately. Tool identities in fingerprints were already there and are tested: `EffectSourceCommandLineTest.ChangingTheCompilerIdentityRebuilds` for the effect compiler and `XmaEncoderService.TwoEncodersHaveTwoIdentities` for the XMA encoder (`XNAPP-256`). |
| `XNAPP-301` | Representative large inputs per route with recorded measurements; no accidental quadratic behaviour. | [x] `XnaRouteScalingTests.cpp` builds four representative inputs at two sizes each, four times apart, with the sources authored by the test rather than downloaded, and fails when four times the input costs more than eight times the time -- which is the shape a quadratic route has and a linear one cannot reach. Each size is the fastest of three runs, because this is a shared machine and one slow run is not a property worth failing a build over. **The three single-asset routes are linear or better**: a `.tga` at 512x512 against 1024x1024 costs 3.3x, a `.wav` at 5 s against 20 s costs 2.8x, and an intermediate-XML `List[string]` of 25 000 elements against 100 000 costs 3.6x. A ratio below four is not a surprise -- process start, the manifest and the staging directory are a fixed cost the small build pays in full. **The fourth measurement is the one worth recording**: the coordinator's own cost per asset is not flat. It falls from 2.10 ms at 100 assets to 1.77 ms at 400 and then rises to 2.68 ms at 1600 and 4.13 ms at 3200, an exponent climbing 0.99, 1.19, 1.40, 1.63. Three measurements say where it is not: an **all-skip rebuild has the same shape** (542 ms at 400 against 3769 ms at 1600), so it is not in any importer, processor or writer; splitting the tree into subdirectories of fifty changes nothing, so it is not the directory's size; and it **parallelizes** (5068 ms at one worker against 2332 ms at three for 1600 assets), so it is per-node work rather than serial coordination. It stays sub-quadratic at every size measured. What it is exactly is not chased here and is not guessed at: no project in the sample corpus is within an order of magnitude of that size -- the largest is 8795 assets across all 170 of them -- and every figure above is in [`docs/content-pipeline-benchmark.md`](../docs/content-pipeline-benchmark.md) with the recipe that produced it. The model route's own size behaviour was already measured by `XNAP-93` (`LargeModelScalingTests`). |
### Phase 25 — gates, documentation, final qualification

| ID | Task | State |
|---|---|---|
| `XNAPP-310` | All §29 gates wired as ctests. | [x] Seven, in `cmake/XnaPipelineParityGates.cmake`. `XnaPipelineParityReportIsCurrent` (the committed report is byte for byte a regeneration) and `XnaPipelineInputParityMatrixIsCurrent` (the eighteen extensions, their statuses, and every test and fixture the matrix names existing in the tree) were already there. Added: **`XnaPipelineParityGateIsGreen`**, which is `parity_report.py --gate` -- the completion gate the plan had been running by hand because `MISSING` was non-zero, and which is a test now that it is zero; **`CnaProvenanceGate`** and **`CnaProvenanceGateSelfTest`** (`XNAPP-008`); **`XnaPipelineWhitespaceIsClean`**, `git diff --check` against the empty tree so it reads committed content rather than only an uncommitted change, scoped to the paths this plan owns because the repository carries around eight thousand whitespace complaints in prose written long before it and a gate that can never be green is one somebody turns off; and **`XnaPipelineGenuineRuntimeInterop`**, which runs `tests/interop/xna40/run-interop-harness.sh` against a genuine XNA 4.0 runtime under Wine and is registered with `SKIP_RETURN_CODE 3`, so a machine that has no Wine, no mono, no display or no legally installed XNA reports a skip rather than a failure -- the difference between *this machine cannot answer* and *the answer was wrong*. It **passes** here. The remaining §29 rows were already ctests and are named in §29 itself: the intermediate serializer, the custom extension, the source-to-XNB/CNB legs and the differential comparison inside `CnaContentPipelineTests`; the independent conformance parser as `CnaXnbSpecificationConformance` and `CnaXnbModelCorpusSweep`; the single-writer rule as `XnbArchitectureUniquenessTest`; the dependency boundary as `CnaXnbDependencyBoundary`. |
| `XNAPP-315` | Provenance gate (`XNAPP-008`) plus a MonoGame/FNA build-source grep. | [x] The gate is `XNAPP-008` and it is a ctest. The *grep* half of this row was reconsidered rather than written: a grep for `MonoGame` or `FNA` over this repository matches hundreds of legitimate lines -- every comment recording what FNA does differently, every measurement written against it, `plans/`, `docs/`, and the reference tree's own name -- so it would have to be either ignored or narrowed until it meant nothing. What a copy actually leaves behind is an attribution header and a path, and those are what the gate checks, on production sources only, with comments and prose out of scope by construction. Recorded here as a deliberate narrowing rather than as the row's literal words, with the reason. |
