...
**Current (2026-09-05, after Phase 5 and the texture, font, material and animation sides of
Phase 6): types 63/128, members 381/705, enum values 6/27, importers 0/10, extensions 0/18, processors 0/12,
...
| `XNAPP-098` | Compile-parity and behaviour tests for every Phase 6 type. | [~] `XnaGraphicsBitmapTests.cpp`: 11 tests over the texture side (`XNAPP-090`–`092`), `XnaFontDescriptionTests.cpp`: 10 over `XNAPP-093` `XnaMaterialContentTests.cpp`: 15 over `XNAPP-094` and `XnaAnimationContentTests.cpp`: 13 over `XNAPP-097`, each comparing against `tests/reference/xna40/graphics/graphics-content-oracle.json` -- layouts, pixel data, conversions and the 22-type converter table exactly; resampled pixels and DXT blocks within the tolerances the row above names. The oracle driver is `tools/xna-pipeline-oracle/graphics/`. The node and vertex types (`XNAPP-095`, `XNAPP-096`) still need both their implementation and their measurements. |
...
(`XNAPP-090`–`094`, `097`, `099`, `098` partial); its remaining types are `XNAPP-095` and
`XNAPP-096` (nodes and meshes, vertices), which need their measurements before their code -- extend
