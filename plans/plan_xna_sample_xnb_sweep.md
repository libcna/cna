...
| `XNASWEEP-145` | A scene with five meshes loses one of them, and lists the other four backwards. | [ ] **Open, measured, not fixed.** SAMPLE-047's `table.FBX` connects five `Mesh` models to `Model::Scene` -- `TableTop`, `BackRightLeg`, `BackLeftLeg`, `FrontLeftLeg`, `FrontRightLeg` -- and XNA's build answers a synthesized `RootNode` with exactly those five under it, in the file's connection order. CNA answers `RootNode` with **four**: `TableTop` is gone, and the four legs are listed in the reverse of the file's order. The root rule itself is right -- more than one top-level object means a synthesized root, which `XNASWEEP-111` measured -- so what is wrong is which top-level objects the importer finds and in what order it walks them, and an FBX 6 file identifies an object by its *name* rather than a number, which is a place two objects of one name can collide (the `Relations` block repeats every model as an empty stub). Four references. |
