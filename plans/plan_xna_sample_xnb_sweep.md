...
| `XNASWEEP-149` | `OptimizeForCache`, round two: ten more probes, and what they settle. | [ ] **Open; four facts it did not have before, and one it can now rule out.** `XNASWEEP-139` had ten probes, all of them plain strips, grids and UV spheres in file order. Ten more were added, each built to separate a part of the rule the first set could not: six **disjoint quads** (every strip is two faces, so every later choice is a restart from a cache whose contents are known exactly), **grids of side 3 and 5** beside the existing 2, 4 and 6, the 4x4 grid with its faces handed over **permuted** as `(7i)%32` and again **reversed**, a **closed fan** of eight, **two disjoint 3x3 grids**, the 4x4 grid with its *vertices* renumbered `v -> 24-v` and its faces left in place, the quads **permuted** as `(5i)%12`, and the 2x4 sphere **permuted** as `(7i)%16`. **(1)** The answer depends on the order the faces arrive in. The permuted quads and the permuted sphere both answer something the plain ones do not, so no purely topological rule can be right. The reversed grid is not evidence either way -- reversing a square grid's face list is exactly its 180-degree rotation, and the answer is the rotated image of the plain one -- and neither is the vertex renumbering, for the same reason. **(2)** Where the cache cannot discriminate, the next strip starts at the **highest-index unused face**. The permuted quads make it readable: in the permuted list the sequence is 11,6,10,3,9,4,8,1,7,2,5,0, whose strip seeds are 11,10,9,8,7,5 -- the highest unused every time. That is also why a mesh with no shared vertices comes back exactly reversed, which is the behaviour CNA was fitted to and mistook for the whole rule. **(3)** The first seed is the **highest-index face containing a minimum-valence vertex** on 13 of the 17 probes -- every strip, every grid, the quads, the fan, the two grids and both permutations. The four UV spheres are the exceptions: their minimum valence is at the two poles, and the seed is in the *south* fan where the rule says the north. What separates the two poles has not been found. **(4)** The walk is a **generalized triangle strip with an ordered leading edge**: from a face entered across `(p,q)` with new vertex `r`, the next face carries `(q,r)`, or `(p,r)` where the first is a dead end. That second case is not decoration -- `grid_4`'s second strip needs it to reach face 15 -- and reading the walk as an unordered shared edge, which is what `XNASWEEP-139` did, cannot express it. `grid_6`'s answer breaks into blocks of 7, 7, 4, 7, 5, 7, 5, 7, 5, 7, 5, 6, which reads as a **cap of seven faces** on a strip. **(5) Ruled out:** a global greedy over a Forsyth-style score, with or without the strip. Scored on its own it answers 3 of the 17 probes and diverges by the third face of every grid; with the strip-cap in front of it, a teacher-forced replay -- give it the truth's own history at every step and ask only for the next face -- gets **364 of 478** decisions. Two decisions it does get, and they are the two that matter: at the end of `grid_4`'s first seven-face strip the score prefers the continuation, and at the end of `grid_6`'s it prefers the far-away face XNA takes, which is what a cache-position-plus-valence score with a 12-entry cache does and what a hit-count-with-a-tie-break cannot. 141 references still turn on this. | **Round three, and the restart rule is now identified.** When a strip ends, the next face is the **highest-indexed unused face containing a vertex of minimum remaining valence** -- remaining meaning the unused faces that still name it, recomputed after every face. It reproduces every restart of `grid_4` by hand (after row 3, the only valence-1 vertex left with an unused face is `v15`, whose one face is 17, which is what XNA takes; then `v10` -> 9; then `v5` -> 1) and, simulated, it answers **9 of the 20 probes exactly**: all three strips, both quad meshes including the permuted one, `grid_2`, `grid_3`, the closed fan and the two disjoint grids -- with a strip cap of 8, the sequential lead preferred and the swap allowed. It replaces the cache-score restart, which answered 3. What it does not yet answer is a mesh whose strips are longer than the cap (`grid_4` stops at 16 of 32, `grid_5` and `grid_6` at 7) and the four spheres (1 of n): a closed surface's minimum valence is at its two poles, symmetric to each other, and XNA takes the *south* one where the grids' tie goes to the *higher* face -- the one contradiction that survives every formulation tried.
| `XNASWEEP-150` | SAMPLE-142's skeletons: XNA answers an identity root where the file gives it a quarter turn. | [ ] **Open, measured, not explained.** Every RobotGame mech differs from CNA's build in exactly one bone's sixteen numbers and nothing else -- same bone list, same names, same parents, same meshes, same vertex buffers, byte for byte. `Duskmas.FBX` is the clearest: its `Model::Root` is a `LimbNode` carrying `Lcl Translation (0, 1.49544, 5.96e-08)`, `Lcl Rotation (0,0,90)`, `PreRotation (-90,0,0)` and `RotationActive 1`, and CNA composes exactly that. XNA answers the identity, with residues: `[1,0,0,0, 0,1,-0,0, 0,-2.35e-41,1,0, 9.16e-17,5.61e-33,0,1]`. The residues are not noise -- `9.157e-17` is `1.49544 * 6.123e-17` and `5.607e-33` is `1.49544 * 6.123e-17^2`, so the translation went through the quarter turn -- but the third component is a clean zero where the same product would give `-1.49544`, and the linear part is the identity where no ordering of a -90 about X and a +90 about Z gives one. The node under it, `Pelvis`, whose own properties are the identity, comes back with the same translation and `+-6.123e-17` off its diagonal, and its child `R leg` is bit-identical to CNA's. What was ruled out: the project passes no processor parameter (plain `FbxImporter`/`ModelProcessor`, no rotation or scale), the file has no `Pose`, no `Deformer` and an empty `Takes`, its `UnitScaleFactor` is 1, and a `MeshHelper` similarity transform of the whole scene cannot be it because that changes every descendant and the descendants are unchanged. Around fifteen references, all in SAMPLE-142. | **A second file joins it, and it puts a number on the relation.** SAMPLE-138's `photograph.fbx` has one mesh, `cube1`, with `Lcl Scaling 0.01`, a `ScalingPivot` of about `-(1.06, 6.89, 6.43)` and a `ScalingOffset` that nearly cancels it. XNA answers an **identity** linear part -- not the 0.01 scaling the file gives it -- and the translation `(2.35153561e-06, 1.52507637e-05, -1.4228086e-05)`. That translation is exactly **one hundred times** what FBX's own formula gives, `-Sp*S + Sp + Soff` in double, which is `(2.3515356772918e-08, 1.5250763674e-07, -1.4228086265e-07)`: the transform is the local one with its scaling **divided out**, `M * S^-1`, which leaves the identity where the linear part was and multiplies the translation by `1/S`. `Duskmas.FBX` has the same shape with a rotation in place of the scaling -- an identity linear part where the file gives it two quarter turns -- but its translation is *not* `t * L^-1` for either composition order, so what divides the rotation out is not the plain inverse. And the rule cannot be general: SAMPLE-003's `Cube.fbx` reaches three of its four references with the *scaled* root the file describes, and the fourth, `Content-built/Cube.xnb`, with an identity one -- two genuine builds of one file that do not agree, which `XNASWEEP-121` already recorded. Whatever the switch is, it is not in the FBX.
...
| `XNASWEEP-153` | One number in `tank.fbx` comes back one float apart, and it is not the parser. | [ ] **Open; a hypothesis raised and then disproved by measurement, which is the useful part.** SAMPLE-074's `tank.fbx` writes its two back wheels' `Lcl Translation` Z as `-234.273040771484` -- fifteen digits of a float whose seventeen are `-234.273040771484375` -- and XNA's own build holds `-234.27302551269531`, the adjacent float, one unit of last place toward zero. Every other translation in that file, twenty-nine of them, comes back as `float(strtod(text))` exactly, and so does every component of the two wheels except this one. The obvious reading -- that the SDK narrows a decimal toward zero rather than to the nearest -- was tested and is **wrong**: `fbx_float_rounding.fbx` writes `234.273040771484`, its negation and `1.0000001` as a translation and as positions, and the genuine build answers `234.27304077148438`, `-234.27304077148438` and `1.0000001192092896`, which is round-to-nearest in all three. That fixture is now a differential-corpus case, so the question stays answered. Also ruled out on this value: five decimal-parsing routes (double, float mantissa over a float power of ten, digitwise in float, and two more), a float round trip through the parent's global translation, and a round trip through the file's `UnitScaleFactor` of 100 -- each reproduces the file's own float, not XNA's. Ten references, one component each. |
...
| `XNASWEEP-160` | One scene puts its children in an order none of the file's three lists gives. | [ ] **Open, one reference, and it is worth writing down because it is the one case `XNASWEEP-154` does not cover.** SAMPLE-142's `Hammer.FBX` connects two objects to `Model::Scene`: `Model::Root`, whose FBX type is the rare `"Root"`, and `Model:: Footsteps`, a `Null` whose name begins with a space. The `Connections` block names `Root` first, the `Objects` block declares it first (line 142 against 6957), and the `Relations` block lists it first -- and XNA's own build answers `RootNode` with ` Footsteps` **before** `Root`. So the order is not the connection order, the declaration order or the relations order; it is not alphabetical either, because `France.FBX`'s five come back as `Object04, Sandbag20, Box_Big03, Truck06, Barricade_11`; and it is not type-first, because that same file puts a `Mesh` before a `Null`. What is left is the model type `Root` itself, which no other file in the corpus uses at the top level -- a skeleton root ordered after the plain nodes would explain it, on one observation. Left open rather than implemented on one. |
