...
> `EnvironmentMapEffect`/`SkinnedEffect` classic lighting, Texture3D,
...
- After `SOFTWARE-113`, the complete `Software` label run is 70 passed, four existing MSAA/
  ...
  shared BasicEffect lighting contract passes 15/15 on each renderer; ten affected Software CTests
  and seven pre-existing EasyGL BasicEffect binaries are green. CTest's configured `DISPLAY=:0`
