...
| SOFTWARE-120 | Implement multiple render targets | ✅ | Completed 2026-09-08. Software now binds up to four ordered `RenderTarget2D` or `RenderTargetCube` face attachments and advertises MRT only with that real path. Clear/discard and resolve/mip finalization visit every color attachment; the first binding alone owns depth/stencil, so a depthless slot zero does not borrow later storage. All four color-write masks are retained, while classic stock effects correctly emit only `COLOR0` and leave higher attachments at their cleared/preserved contents instead of fabricating shader outputs. Per-attachment 4x planes resolve independently, and distinct faces of one cube may coexist with 2D targets. The renderer-neutral stock-effect contract passes 17/17 on Software and its common 15/15 subset on EasyGL; cube plural binding passes 14/14, bound-target lifetime 18/18, render-pass boundaries 41/41 and backbuffer ordering 29/29. Full Software `CnaGraphicsTests` passes 2,234 with 115 intentional skips out of 2,349; the non-display Software label passes 90 with four pre-existing supervisor skips. The same work removed Software's hidden depth/stencil storage from `DepthFormat::None` targets. EasyGL's cube-member refusal is now an explicit reference discrepancy under `SOFTWARE-135`; GDI remains externally blocked before CNA compilation by sharp-runtime's unconditional POSIX `<poll.h>`. |
...
| SOFTWARE-135 | Reconcile EasyGL's refusal of cube faces in plural render-target bindings | ⬜ | FNA's public `RenderTargetBinding`/`GraphicsDevice.SetRenderTargets` accepts both 2D targets and cube faces, while EasyGL currently refuses any cube member when count is greater than one. Software's `SOFTWARE-120` path already supports mixed 2D/cube bindings and distinct faces of one cube. Replace EasyGL's renderer-local refusal with valid FBO attachment handling, retain duplicate/dimension/MSAA validation, and run the shared mixed/cube-face contract on both renderers. |

### Current dependency order

`SOFTWARE-104` through `SOFTWARE-122` and `SOFTWARE-131..133` are complete foundations.
...
foundation. Supervisor reconciliation (`SOFTWARE-134`) and the EasyGL cube-MRT discrepancy
(`SOFTWARE-135`) can proceed independently of CPU instancing (`SOFTWARE-129`) and the high-level
