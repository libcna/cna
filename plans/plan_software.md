...
> MRT and instancing. The executable task table is in the new parity campaign
...
| SOFTWARE-104 | Make capability reporting truthful during the campaign | ✅ | Completed 2026-09-07 and kept current by `SOFTWARE-117`/`SOFTWARE-118`/`SOFTWARE-122` on 2026-09-08. Software reports false for the still-unimplemented MRT, instancing and arbitrary custom-effect paths. OcclusionQuery, anisotropic filtering and Texture3D storage were opted in only after exact CPU implementations and public proof landed. `Software_CapabilityContract` protects ten positive promises, three honest false boundaries and the real query/Texture3D factories in 16/16 checks. |
...
anisotropic sampling and its SpriteBatch plumbing. Remaining resource task `SOFTWARE-120` precedes the high-level
