...
> Texture3D, RenderTargetCube, MRT, anisotropic filtering and instancing. The executable task table is in the new parity campaign
...
| SOFTWARE-104 | Make capability reporting truthful during the campaign | ✅ | Completed 2026-09-07 and kept current by `SOFTWARE-122` on 2026-09-08. Software reports false for the still-unimplemented MRT, anisotropic, Texture3D, instancing and arbitrary custom-effect paths. OcclusionQuery was initially false/null, then opted back in only after its exact CPU implementation and public proof landed. `Software_CapabilityContract` now protects eight positive promises, five honest false boundaries and the query/Texture3D factory behavior in 16/16 checks. |
...
`SOFTWARE-122` now counts the surviving sample mask exactly. Resource tasks `SOFTWARE-118..120` precede the high-level
