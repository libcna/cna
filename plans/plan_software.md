...
> Texture3D, RenderTargetCube, MRT and instancing. The executable task table is in the new parity campaign
...
| SOFTWARE-104 | Make capability reporting truthful during the campaign | ✅ | Completed 2026-09-07 and kept current by `SOFTWARE-117`/`SOFTWARE-122` on 2026-09-08. Software reports false for the still-unimplemented MRT, Texture3D, instancing and arbitrary custom-effect paths. OcclusionQuery and anisotropic filtering were initially false, then opted back in only after exact CPU implementations and public proof landed. `Software_CapabilityContract` now protects nine positive promises, four honest false boundaries and the query/Texture3D factory behavior in 16/16 checks. |
...
anisotropic sampling and its SpriteBatch plumbing. Resource tasks `SOFTWARE-118..120` precede the high-level
