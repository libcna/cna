...
| SOFTWARE-133 | Reconcile stale Software capability/declaration unit oracles with completed parity work | ⬜ | The full Software `CnaGraphicsTests` run now executes 2335 tests: 2173 pass and 150 intentionally skip, while 12 failures are obsolete expectations from before `SOFTWARE-104/108/122`. Make capability fixtures expect false CustomEffects/MRT and true OcclusionQuery on Software, and make `VertexDeclarationLayoutTests` positively validate declaration-driven reordered/padded ordinary draws instead of demanding rejection; retain instancing skips until `SOFTWARE-129`. Acceptance is a green full `CnaGraphicsTests` run with only intentional skips and unchanged EasyGL expectations. |
