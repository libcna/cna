...
| SOFTWARE-134 | Reconcile stale Software render-target supervisor gates and oracles | ⬜ | Four shared supervisor CTests (`MsaaDepthContract`, `MsaaFirstReadback`, `MsaaMipReadback`, `InvalidMipLevel`) skip every child under their current video gate even though Software requires no video. Running them with `SDL_VIDEODRIVER=dummy` exposes stale `kTargetMipReadbackSupported`, MRT and cube-support constants plus a strict level-zero pixel mismatch. Make the harness run Software natively, replace obsolete v1 capability assumptions with positive contracts, and resolve the measured pixel discrepancy without widening tolerances. Acceptance: all four supervisors execute their applicable Software legs with zero harness skips and exact/current resource semantics. |
