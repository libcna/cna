...
> actually tested Linux/Vulkan and Windows/D3D12 configurations, every audited ordinary XNA 4.0
> graphics family is observably equivalent or is
...
> `SDLGPU-133` corrects the two depth-state oracle programs that still relied on the pre-132
> hardcoded depth attachment: both now request the normal XNA `GraphicsDeviceManager` default
> (`Depth24`) without weakening any pixel assertion, and pass on SDL GPU and EasyGL.
> `SDLGPU-134` performs the post-change closeout from one current tree: all 194 native SDL GPU
> registrations pass with validation diagnostics fatal, and the fail-closed inventories still
> classify all 266 renderer hooks and all 247 EasyGL examples.
> `SDLGPU-94` closes the remaining D3D12 platform gate on a private headless Mutter/Xwayland
> compositor with DRI3. Matched Proton/vkd3d-proton creates a real SDL_gpu D3D12 swapchain and
> passes 60-frame presentation, OS resize, null-acquire retry and hard release/re-claim recovery;
> the eight focused real-window executions pass 75/75 checks with no D3D12 debug-layer or Vulkan
> validation diagnostic. The smoke test now counts its platform-dependent ShaderEffect diagnostic
> correctly: 30/30 on Vulkan with the compiler and 31/31 on D3D12 without it.
> `SDLGPU-95` and `SDLGPU-96` remain open: Metal and Android/Vulkan still require native runtime
> evidence.
> Ten EasyGL defect findings (nine distinct behavior families) are deliberately not copied. The
> current full **194/194** SDL integration sweep, current 54/54 renderer-unit aggregate, new 5/5
> sampler-cache and 71/71 pipeline-cache focused targets, and 33/33 EasyGL
...
| Newly created tasks | 44 (`SDLGPU-91`–`SDLGPU-134`) |
| Completed / open | 42 / 2 (`SDLGPU-91`–`94`, `SDLGPU-97`–`134` complete; `SDLGPU-95`–`96` open) |
| Ending evidence commit | `0ad1e76bd07f736a983f4dd466b239546b1a5c10` (`SDLGPU-133` behavior/test); `SDLGPU-134` is the final plan-only closeout commit |
| Proven runtime configuration | Linux/Vulkan remains the complete behavioral baseline. D3D12 has a real no-window device, all-26-stock-shader construction, stock-pipeline exact-pixel proof, a public windowless `GraphicsDevice` with exact backbuffer/RT2D clear readback, the unchanged public Game/Texture2D/SpriteBatch 2D scene for 120 frames, all nine shared classic stock-effect fixtures, the complete ten-fixture state/sampler matrix, the 24-test texture/format/transfer matrix, all 33 render-target registrations, all 25 buffer/draw registrations, all five model oracles and the current 45-case compiled-effect corpus. The target evidence includes the 851-assertion mip/readback oracle and 40-assertion classic MRT matrix; the draw evidence includes instancing, multistream and declaration semantics. Six presentation/reset/resize programs, 73 individually isolated backbuffer/bound-target/Present lifecycle legs and 291/291 applicable constructor/lazy-resource rollback checks also pass through D3D12. `SDLGPU-94` adds a real D3D12 swapchain on private headless Mutter/Xwayland: eight presentation/acquire/resize/recovery executions pass 75/75, including 60 presented frames, an actual OS resize, a null acquisition retry and a forced release/re-claim recovery. There are 194 registered native and 261 registered Windows SDL integration tests; the newest shared public-format contract passes all 17 depth/capability/pixel/readback assertions on both available drivers. The pre-platform full sweep plus the focused portability gates remain the Linux behavioral baseline. The D3D12 portability probes pass 2/2, 3/3, 6/6 and 3/3; the five skinned-effect/PBR executables add 25/25 discriminating assertions. The expanded 45-case compiled-effect corpus passes Vulkan, focused ASan Vulkan and D3D12; the immutable-sampler target passes 5/5 on both drivers, while the immutable-pipeline target passes 71/71 on Vulkan and all 64 applicable classic checks on D3D12. The nine-case sampler-publication oracle also passes on both drivers. Unchanged dependencies were reused from the stable builds. |
...
| Display constraint | Routine Linux SDL tests use `SDL_VIDEODRIVER=offscreen`. Windows GUI evidence uses a private headless Mutter virtual monitor and its DRI3-capable Xwayland, never the host display. |
...
- **Result (2026-09-11):** complete. The stable MinGW tree is configured with the existing Windows SDL3 package and
  ...
  failure matrix at 291/291 applicable checks.

  Real swapchain presentation/acquire/minimize/recovery is now covered as well. A private GNOME
  headless Mutter session supplied a 1024x768 virtual monitor and DRI3-capable Xwayland `:2`;
  matched Proton Experimental plus vkd3d-proton 3.1.0 then created SDL_gpu's real D3D12 swapchain
  on the AMD Radeon 780M without touching a physical display. Eight focused real-window executions
  pass **75/75** checks: smoke **31/31** including 60 `Clear()+Present()` frames, hard acquisition
  failure plus `SDL_ReleaseWindowFromGPUDevice`/`SDL_ClaimWindowForGPUDevice` recovery **5/5**,
  documented null-texture/minimized retry **3/3**, real OS resize and `ClientSizeChanged` **4/4**,
  presentation-surface/zero-size/interval semantics **13/13**, GDM/direct backbuffer resize **12/12**,
  VSync forwarding **2/2**, and presentation parameters **5/5**. All eight processes report
  `debug mode enabled, swapchain`; their captured logs contain no `D3D12 ERROR/WARNING`, Vulkan
  validation diagnostic, failed assertion or failed check. The only initially red outcome was
  `31/30 PASS`: the compiler-unavailable D3D12 build deliberately performs one more diagnostic
  check than the compiler-enabled Vulkan build. The smoke oracle now derives the expected count
  from `CNA_SDL_GPU_SHADER_EFFECTS`; final native Vulkan remains **30/30** and D3D12 is **31/31**.

  The earlier 2026-09-11 retry used Wine 10.0 plus matching local DXVK v3.0.2-58 and vkd3d-proton 3.1.0
  ...
  Xvfb, including nested Xephyr/glamor, exposes no DRI3. That historical Xvfb result was an exact
  runner/display-stack limitation, not renderer success or failure; the installed GNOME Shell's
  headless Mutter mode supplied the missing DRI3 presentation path and removes the blocker.
...
- **Result (2026-09-11):** the stable incremental Vulkan tree passes **191/191** SDL GPU integration
  ...
  At this checkpoint the exact renderer-wide result was still verdict **B** because three platform
  evidence tasks were open: real D3D12 swapchain recovery needed DRI3 or a real Windows runner
  (`SDLGPU-94`), Metal needs an Apple SDK/device (`SDLGPU-95`),
  ...
  `SDLGPU-94` subsequently found and used the installed headless Mutter DRI3 route, so the D3D12
  part of this checkpoint is superseded; only `SDLGPU-95` and `SDLGPU-96` remain open.
...
### SDLGPU-134 — close the current 194-test and fail-closed parity sweep ✅

- **Problem/public behavior:** the summary's last single-HEAD native proof was still **191/191**
  even though the current suite registers 194 tests. Historical implementation rows 39, 44,
  46–50 and 53 also retained open/in-progress glyphs after later parity tasks had either completed
  or canonically replaced them, making a mechanical status scan contradict the authoritative
  76/2 ledger. The project owner's newer global build policy additionally supersedes the old
  build-directory names quoted throughout the historical log.
- **Evidence:** the first current 194-test run was **192/194** and exposed the invalid depth-buffer
  assumption closed by `SDLGPU-133`; accepting only the previously green 192 would have hidden it.
  The renderer-contract and EasyGL-corpus checkers are exact-signature/fail-closed, so any newly
  inherited hook or unclassified source is an error rather than an optimistic default.
- **Location:** final summary/status tables, historical task dispositions and this closeout row.
  No renderer, public API, test assertion or build configuration changes belong in this task.
- **Acceptance/test:** from the post-133 tree, require all **194/194** native SDL GPU registrations
  to pass sequentially with validation output fatal; require **266/266** classified renderer hooks
  with no `~`/`≠`/`∅`, and **247/247** classified EasyGL examples with zero unclassified or
  new-test-needed rows. Re-run the two modified sources on EasyGL, account for every adversarial
  ignored-argument/hardcoded-format/throw/null hit, and leave only canonical `SDLGPU-95/96` open.
- **Result (2026-09-11):** complete. The post-fix offscreen Vulkan sweep passes **194/194** from
  one current executable set in 183.71 seconds; its **115** parity-labelled registrations are a
  subset of that same run. Every registration retains fatal expressions for `Validation Error`,
  `Validation Warning`, bare `VUID-`, `D3D12 ERROR:` and `D3D12 WARNING:`, and none fired. The
  contract checker reports **266/266** (144 `=`, 117 `out`, five `⛔`) with zero unresolved states.
  The corpus checker reports **247/247** (128 broadly covered, 52 direct, two query limitations,
  three EasyGL defects, nine EasyGL-specific, 53 modern-out) with zero unclassified, duplicate or
  new-test-needed rows. The two renderer-neutral depth tests also pass directly on EasyGL under a
  fresh isolated Xvfb: **5/5** compare pixels and **2/2** write-enable pixels.

  The final source scan found no SDL GPU `TODO`/`FIXME`. Its ignored arguments are deliberate and
  already classified: render targets return their allocated applied depth format rather than
  re-echoing a later request; the backbuffer normalizes SDL's fixed RGBA8/BGRA8 transfer contract;
  fullscreen is owned by the common window path; test-hook-only driver parameters compile away;
  and CNA-specific `ShaderEffect` uniform names follow the same fixed-slot convention as sibling
  renderers. Format fallbacks are guarded by explicit classifiers, every ordinary factory argument
  reaches the format-bearing implementation, pipeline identity includes target/depth/sample and
  complete static render state, and sampler identity includes all seven XNA sampler fields. Named
  exceptions are public validation, the shared compiled-effect vertex-sampler boundary, or the
  proven `SDLGPU-80` query limitation; no new classic-XNA gap was found.

  Historical rows 39, 44, 46–50 and 53 now use `↪` and point at their canonical later owners rather
  than pretending to be additional open work. Under the owner-supplied global build policy,
  `/rv/cnaccache` is the single cache and new build/configure work may use only `build/`,
  `build-asan/`, `build-ubsan/`, `build-tsan/`, `build-probe/` or `build-consumer/`. Older
  `cmake-build-*` names below are immutable historical evidence, not instructions for a new build.
  This closeout created no build directory, ran no concurrent heavy build, and reused the two
...
| Ending parity-behavior/test commit | `0ad1e76bd07f736a983f4dd466b239546b1a5c10` (`SDLGPU-133`); the final plan-only reconciliation is `SDLGPU-134` |
...
| Tasks created by this audit | 36 (`SDLGPU-55`–`SDLGPU-90`); the later platform/adversarial re-audit creates 44 more (`SDLGPU-91`–`134`): **80 total** |
| Completed / open / proven unavoidable | This closed Linux/Vulkan phase completed 36 / 0 tasks; the current renderer-wide ledger is **78 / 2**. Three capability fields (`BlendState.MultiSampleMask`, exact half-rate `PresentInterval::Two`, and `OcclusionQuery`) are proven unavailable in current SDL_gpu; the first two are not separate tasks and the third is closed by `SDLGPU-80` |
| SDL GPU build | Debug, `CNA_GRAPHICS_RENDERER=SDL_GPU`, tests/examples and `CNA_SDL_GPU_COMPILED_EFFECTS` ON. Historical runs used `cmake-build-sdlgpu`; under the newer global policy that name is evidence only, not a template for new work. |
| EasyGL oracle build | Debug, `CNA_GRAPHICS_RENDERER=OPENGL33`, tests/examples ON. Historical runs used `cmake-build-debug`; the same policy caveat applies. |
| Runtime driver | SDL 3.5.0 SDL_gpu Vulkan on AMD Radeon 780M / Mesa RADV 25.0.7; Khronos validation layer 1.4.309 present |
| Display constraint | Sandboxed tests do not access the host display. All 194 native SDL GPU registrations explicitly use `SDL_VIDEODRIVER=offscreen`, `DISPLAY=` and `WAYLAND_DISPLAY=` while still creating the real Vulkan device/swapchain. EasyGL runs only in isolated Xvfb instances (the historical full oracle used `:179`; the final two-test replay used a fresh auto-selected display). D3D12's focused real-window evidence uses a separate private headless Mutter virtual monitor and DRI3-capable Xwayland. Native driver defaults are platform-specific rather than hard-coded to X11. |
| Final SDL GPU verification | **194/194** registered native SDL integration CTests pass sequentially from the post-`SDLGPU-133` tree in 183.71 seconds, including all **115** parity-labelled registrations. This single current sweep supersedes the prior 191 plus separately-run 192nd–194th evidence. The current focused renderer aggregate remains **54/54**; the compiled-effect subset is **45/45** on native Vulkan, focused ASan Vulkan and headless D3D12; the strict texture-filter oracle is **70/70** on Vulkan and D3D12; the depth-format contract is **17/17** on both; sampler/pipeline retention remains bounded and exact at **5/5** and **71/71**; and `SDLGPU-94` provides **75/75** real D3D12 swapchain checks across eight presentation/acquire/resize/recovery executions. |
| Validation | SDL GPU debug mode is enabled. All 194 integration registrations make `Validation Error`, `Validation Warning`, bare `VUID-`, `D3D12 ERROR:` or `D3D12 WARNING:` output fatal; the new format, native focused presentation and eight real-window D3D12 runs emitted none. |
| Final EasyGL/parity oracle | **33/33** registered EasyGL oracle CTests pass under Xvfb/llvmpipe. The two `SDLGPU-133`-modified sources additionally pass directly under a fresh isolated Xvfb (**5/5 + 2/2** pixel checks). The direct corpus executes all 32 shared sources under both renderers: 27 frames satisfy the strict byte policy, two use renderer-local discriminating invariants, and three stay within fixed measured line/edge coverage budgets. |
...
> **Historical-status rule (2026-09-11):** `↪ superseded by SDLGPU-*` means the original row was
> not retroactively claimed complete under its old acceptance text; a later parity task became its
> authoritative owner and records the current implementation and verification. Such rows are not
> open work. Only task headings in the current audit ledger determine the 78-complete/2-open count.
> In particular, the only open tasks are `SDLGPU-95` (Metal runtime certification) and
> `SDLGPU-96` (owner-deferred Android/Vulkan runtime certification).

> **Current build-policy rule (2026-09-11):** the project owner's newer global policy supersedes
> every historical build-directory recipe below. New work uses the single shared ccache at
> `/rv/cnaccache` and only `build/`, `build-asan/`, `build-ubsan/`, `build-tsan/`, `build-probe/`
> or `build-consumer/`. Names such as `cmake-build-sdlgpu` and `cmake-build-debug` are retained only
> as provenance for runs already performed; do not create new trees from those examples.
...
| # | Task | Status | Notes |
| --- | --- | --- | --- |
| SDLGPU-44 | Confirm and document that `SDL_gpu` has no occlusion-query API in the vendored SDL3 (`third_party/SDL/include/SDL3/SDL_gpu.h`, checked 2026-07-15 — no query-pool type exists). `CreateOcclusionQuery()` returns `nullptr`, matching `IGraphicsBackend`'s own documented default and the posture `Headless`/`Software` already take. | ↪ superseded by SDLGPU-80 | Not a gap to close by writing code — only re-check this row if a future SDL3 upgrade adds real query support. |
...
| # | Task | Status | Notes |
| --- | --- | --- | --- |
| SDLGPU-48 | Windows validation via `SDL_gpu`'s D3D12 driver. | ↪ superseded by SDLGPU-94/99–114 | Code paths only until run on real (or Wine/DXVK-class) Windows infrastructure, same caveat structure already used for `D3D11`/`D3D12`/WebGPU. |
| SDLGPU-49 | macOS/iOS validation via `SDL_gpu`'s Metal driver. | ↪ superseded by SDLGPU-95 | `needs_human` — no Apple hardware in this dev environment, same class of gate as `DX-90`/`DX-91`/`DX-114`. |
| SDLGPU-50 | Android validation via `SDL_gpu`'s Vulkan driver. | ↪ superseded by SDLGPU-96 | The Android prebuilt SDL3 package already exists (`.sdl-prebuilt-Android-aarch64/`) and already compiles `SDL_gpu.c` — check whether its Vulkan GPU driver was also compiled in before assuming this is free. |
