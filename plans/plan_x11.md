...
> X11-0165, input-device enumeration, X11-0166, gamepad motion sensors, and X11-0167, message boxes
> drawn with Xlib, are ✅ -- every Phase N row. See
...
| D15 | **`nativeFileDialog`, `tray`, `camera`, `haptics`, `sensors` are `false`.** | Core X11 has no standard facility for any of them, and shelling out to `zenity`/`kdialog` is not a native backend. Truthfulness beats checkbox count; the contract's rule is that a false capability *refuses*, which is exactly what these do. (`gamepad` and `joystick` were on this list until X11-0150; see D17. `messageBox` was until X11-0167: X has no dialog service, but a box is only a window, which the backend draws itself -- no program is started -- as SDL3's X11 backend draws its own once `zenity` has failed, a step CNA does not take.) |
...
| `haptics`, `sensors` | ❌ false | not an X11 facility |
...
| `X11EvdevVirtualDevice.AMotionSensorNodeIsPairedWithItsPad` on CI | enabled in the SDL-free X11 cell's test step; its result is recorded with the next CI run |
| every X11 and platform ctest entry | 10/10 |
