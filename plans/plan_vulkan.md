...
**Implementation under way.** Twenty-five tasks are ✅ (`VULKAN-004`, `-020`, `-021`, `-091`, `-097`,
`-130`, `-131`, `-144`, `-145`, `-146`, `-147`, `-148`, `-149`, `-150`, `-152`, `-153`, `-155`, `-156`,
`-158`, `-250`, `-332`, `-333`, `-346`, `-370`, `-390`), plus
...
Ten rows were opened by work that discovered something the plan had not, and one of them —
`VULKAN-150` — is already closed again:
...
distinguish GLSL source from SPIR-V bytecode), `VULKAN-151` (a
`Vector4`-spelled `BlendIndices` is bindable on EasyGL and not here), `VULKAN-153` and `VULKAN-157` (F-22, `CnaInputTests` can deadlock and `ctest`
...
| VULKAN-151 | Accept a `Vector4`-spelled `BlendIndices` on the skinned route | ⬜ | **Opened by `VULKAN-147`, which made the refusal explicit rather than creating it.** **Observed:** `skinned3d.vert.glsl` declares `layout(location = 4) in uvec4 aBoneIndices`, so only the integer spelling of `BlendIndices` is bindable -- a float attribute on an integer input is a Vulkan usage error, not a conversion. XNA allows both spellings and a content processor writes either (`plans/plan_fx.md` FX-127, whose own note says CustomModelAnimation's `SkinnedModelProcessor` writes `Vector4`); EasyGL accepts both through a dedicated bone-index binding helper. Vulkan now reports the float spelling as unrepresentable and refuses the draw **by name**, which is the right failure mode and not the right behaviour. **Before `VULKAN-147` the same content was mis-bound silently or refused for its stride**, so this is a narrowing of an existing gap, not a new one. **Intended:** either a second shader variant taking `vec4 aBoneIndices` (selected by the declaration, the way `VULKAN-146` selects between the lit and coloured programs), or a documented refusal with the conversion left to the content pipeline. The first is preferred: the model data is what it is. **Acceptance:** a skinned draw whose declaration spells `BlendIndices` as `Vector4` renders the same picture as the `Byte4` one -- asserted as equality of the two readbacks, not as each matching a constant -- and `Vulkan_VertexInputLayout`'s leg E2 is rewritten to the resolved contract. **Depends on:** `VULKAN-147`. |
