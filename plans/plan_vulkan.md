...
| VULKAN-145 | Build a vertex input layout from the declaration, and key a pipeline on it | ⬜ | **The shared half of `VULKAN-144`, and nothing else -- no renderer behaviour changes in this row.** A helper that takes a `DeclaredVertexLayout` and a `StockProgramInput` table and produces (a) the `VkVertexInputAttributeDescription` array, one entry per *consumed* shader input, at the matching element's own offset and `VulkanVertexFormatHelper` format, and (b) a stable 64-bit hash of that array for the pipeline key. Inputs the declaration does not supply are reported, not silently left unbound: Vulkan reads undefined data from an attribute with no description, so the caller must decide between refusing and falling back, and this helper must not decide for it. An empty declaration yields "no opinion" so the caller keeps its stride-derived layout. **Acceptance:** a registered test proving two declarations of **equal stride** and different element offsets produce different attribute arrays and different hashes; that declaration ORDER does not change the result (the same elements listed in either order hash the same), which is the property that makes this better than EasyGL's index-based binding; and that an unsupplied input is reported. **Non-goal:** touching any pipeline factory. |
| VULKAN-146 | Drive the BasicEffect pipelines from the declaration | ⬜ | Adopt `VULKAN-145`'s builder in `GetOrCreatePipeline3D`, `GetOrCreatePipelineFogColored3D`, `GetOrCreatePipelineFogTex3D` and `GetOrCreatePipelineLitTextured3D`, fold the layout hash into their pipeline keys, and narrow `RequireFaithfulDeclarationEXT` on those routes from the stride-table check to `RequireDeclarationMatchesStockProgram`. **Acceptance:** a test binds two declarations of equal stride whose colour element sits at different offsets and proves each draw reads its own; `GetGraphicsPipelineCacheEntryCountEXT` shows **two** entries, not one; every existing `Vulkan_BasicEffect_*` test stays green. **Depends on:** `VULKAN-145`. |
