...
**Implementation under way.** Twenty-two tasks are ✅ (`VULKAN-004`, `-020`, `-021`, `-091`, `-097`,
`-130`, `-131`, `-144`, `-145`, `-146`, `-147`, `-148`, `-149`, `-152`, `-153`, `-156`, `-250`, `-332`,
`-333`, `-346`, `-370`, `-390`), plus
...
legacy `DrawColoredPrimitives` pair binds a hard-coded stride), `VULKAN-156` (F-25, `SkinnedEffect`
...
**The Vulkan suite is 228/228** on `-R '^Vulkan_'`, from the 209/215 this plan opened with. It is green
...
| VULKAN-155 | The legacy `DrawColoredPrimitives` pair must not mis-read a non-16 stride (F-24) | ⬜ | **Found while closing `VULKAN-144`, by inventorying which of the fifteen pipeline factories now take a declaration-derived layout: eleven do, `GetOrCreatePipeline2D`/`2DMsaa` own their vertex format, `GetOrCreatePipelineDualTex3D` is `VULKAN-150`, and this one was the leftover.** **First step, before any code changes:** reproduce it -- a stride-20 `Position+Color` buffer through `DrawColoredPrimitives`, asserting the pixel, because F-24 is read from source and a plan that fixes an unreproduced defect is guessing. **Then one of two answers, and the choice is the substance of this row:** (a) refuse a stride this route cannot express, by name, which is what D3D9 does and what CNA's own capability philosophy prefers -- but the route has no declaration to check, only a stride, so the refusal is `stride != 16`; or (b) give it the same declaration-driven layout the other eleven have, which is more work and more capability, and is what EasyGL effectively does by binding the buffer's own layout. **(a) is the recommendation**: the method has no production caller, its interface documentation describes a colored-vertex convenience rather than a general draw, and an honest refusal on a legacy surface is worth more than a new binding path nothing calls. **Acceptance either way:** a permanent test that drives both entry points with a non-16 stride and asserts the chosen behaviour, and the stride-16 cases stay byte-identical. **Not blocked by anything.** |
