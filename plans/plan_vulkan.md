...
**Implementation under way.** Twenty-six tasks are ✅ (`VULKAN-004`, `-020`, `-021`, `-091`, `-097`,
`-130`, `-131`, `-144`, `-145`, `-146`, `-147`, `-148`, `-149`, `-150`, `-151`, `-152`, `-153`, `-155`,
`-156`, `-158`, `-250`, `-332`, `-333`, `-346`, `-370`, `-390`), plus
...
Ten rows were opened by work that discovered something the plan had not; `VULKAN-150` and
`VULKAN-151` are already closed again, and these are the rest:
...
distinguish GLSL source from SPIR-V bytecode), `VULKAN-153` and `VULKAN-157` (F-22, `CnaInputTests` can deadlock and `ctest`
would not reap it — the second half is bounded now, the deadlock is not), `VULKAN-154` (F-23, an X protocol error ends the whole test binary), `VULKAN-155` (F-24, the
legacy `DrawColoredPrimitives` pair drew nothing at any stride but 16), `VULKAN-156` (F-25, `SkinnedEffect`
drew nothing and reported success for every stride but two, and the conformance suite passed it
32/32 because it compares that family against itself), `VULKAN-158` (F-26, the declaration-driven
conversion moved the attribute offsets and left the vertex binding's stride baked) — plus the
...
the conversion could not answer are `VULKAN-150` and `VULKAN-151`.
...
| VULKAN-157 | Diagnose the `CnaInputTests` deadlock (F-22, the half `VULKAN-153` did not fix) | ⬜ | **Blocked on tooling, not on analysis.** **What is known:** the process is single-threaded, blocked on a **private glibc mutex with a NULL timeout** (`/proc/<pid>/syscall` = `202 <uaddr> 0x80 0x2 0x0 …`, i.e. `FUTEX_WAIT|FUTEX_PRIVATE`, value 2 = locked-with-waiters), with no children and 2 s of CPU after 45 minutes. Both observed occurrences had a **second copy of the same suite** running from another checkout, itself hung the same way. It does **not** reproduce on demand: `--gtest_shuffle --gtest_repeat=5` passed **5/5** directly (from both the repo root and `ctest`'s working directory) and the suite passed **221 s** under `ctest` alone, all on `:99`. **Suspects, in order:** a process-global SDL subsystem whose init/quit takes a lock a previous repeat left held (joystick/haptic enumeration walks udev and `/dev/input`, which is machine-global and would explain the two-copies correlation); a fork in a threaded process -- a gtest death test forking while an SDL thread holds a lock produces exactly this signature in the child; a static destructor ordering race across `--gtest_repeat`. **First step:** get a stack. There is no `gdb` on this machine and no `eu-stack`/`pstack` either; installing one, or building `CnaTests` with `-fsanitize=thread`, is the prerequisite and should be recorded here when done. **Acceptance:** either a fix with a regression that runs two copies of the input suite concurrently and both finish, or -- if the cause is a genuinely machine-global resource -- a `RESOURCE_LOCK` on the test and the reason recorded here, which makes it serial rather than lucky. **Not a Vulkan-renderer defect.** |
| VULKAN-154 | Do not let one X protocol error end a whole test binary (F-23) | ⬜ | **Opened by the `VULKAN-149` baseline.** Two pieces again, and the first is worth doing whether or not the second is ever diagnosed. **(a) Containment:** install an Xlib error handler (`XSetErrorHandler`) for test binaries -- or wherever CNA owns the X connection -- that records the failed request and lets the process continue, so a `BadWindow` costs the one test that provoked it instead of the 577 that had nothing to do with it. Xlib's default handler calls `exit`, which is why the failure arrives with no gtest verdict at all. Note this is **not** the same as swallowing the error: it must be logged loudly and the provoking test failed. **(b) The request itself:** `X_ChangeProperty` on a window id that is no longer valid, in a suite that creates and destroys a `GraphicsDevice` (and its window) per test. The suspects are a property set on a window between `SDL_DestroyWindow` and the connection catching up, and a window id cached across a device teardown. **Acceptance:** (a) a test that provokes a deliberate bad request and asserts the process survives with the error recorded; (b) if the stale-id path is found, a fix plus a device create/destroy loop that runs clean under it. **Not a Vulkan-renderer defect** -- the renderer is only what makes these tests build windows -- but it is on this board because it is what corrupted a baseline this plan had to take. |
