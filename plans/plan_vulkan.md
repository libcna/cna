...
**Implementation under way.** Twenty tasks are ✅ (`VULKAN-004`, `-020`, `-021`, `-091`, `-097`,
`-130`, `-131`, `-144`, `-145`, `-146`, `-147`, `-148`, `-149`, `-156`, `-250`, `-332`, `-333`, `-346`,
`-370`, `-390`), plus
...
`Vector4`-spelled `BlendIndices` is bindable on EasyGL and not here), `VULKAN-152` (F-15's tail:
what a shader input the caller never supplied should bind to, which GL answers with a constant
and Vulkan cannot), `VULKAN-153` (F-22, `CnaInputTests` can deadlock and `ctest` will not reap
...
the conversion could not answer are `VULKAN-150` and `VULKAN-152`.

**The Vulkan suite is 226/227** on `-R '^Vulkan_'`, from the 209/215 this plan opened with, and the
full `ctest` is **9060/9083**; the prefixed suite is **227/228** after `VULKAN-156`. §7.5 carries the running baseline, one line per
completed task. The single remaining `Vulkan_*` failure, `Vulkan_RenderTarget_EffectSource`, is a
capability gap that `VULKAN-148` **re-attributed**: it is not the declaration path at all — the
test draws `PbrEffect` from a 20-byte buffer with no declaration, so the shader's normal and
tangent have no source, and what it needs is the default-attribute mechanism now owned by
`VULKAN-152`. It fails cleanly rather than aborting the process (`VULKAN-346`). **Both numbers matter, and F-21 is why**: `-R '^Vulkan_'` is 215 of this
...
| VULKAN-152 | Decide what an unsupplied shader input binds to (the question `VULKAN-146` raised and `VULKAN-148` hit) | ⬜ | **The last thing between this renderer and a green suite, and a design decision rather than a conversion.** **Observed, twice:** `VertexDeclarationLayoutTests`' `positionColorPadded32` needed declaration-driven *program* selection to avoid it (`VULKAN-146` solved that case that way), and `rendertarget_effect_source_test.cpp` cannot be solved that way at all -- it draws `PbrEffect` from a 20-byte position+texture buffer with **no declaration**, so `pbr3d.vert.glsl`'s normal and tangent have no source of any kind. **The asymmetry:** an unbound GL attribute takes a constant `(0,0,0,1)`, which is why EasyGL renders both; a Vulkan shader input with no `VkVertexInputAttributeDescription` reads **undefined** data, which is why this renderer refuses. **Intended:** a renderer-owned defaults buffer bound at binding 1 with `VK_VERTEX_INPUT_RATE_INSTANCE`, holding one 16-byte record of float `{0, 0, 0, 1}` — every non-instanced draw has `instanceCount == 1`, so every vertex reads record 0 and the attribute is constant, and the record reproduces GL's own rule exactly (`vec2` reads `(0,0)`, `vec3` `(0,0,0)`, `vec4` `(0,0,0,1)`). Integer-format inputs need their own zero region rather than that record's `w`. **Two things to settle before implementing, not during:** (a) whether this is XNA's behaviour or GL's -- D3D9 leaves an unmapped input register undefined, and the shared fixtures are calibrated on GL, so the answer decides whether this is parity or a deliberate divergence to record in `docs/vulkan-renderer.md`; (b) it must not weaken the guard -- a *declared* element that the selected program cannot bind must still be refused, because that is a caller error, while a *missing* one is the caller declining to supply it. **Acceptance:** `Vulkan_RenderTarget_EffectSource` reaches 9/9 and the whole `Vulkan_*` suite is green; the defaults buffer's binding does not collide with the instanced route's binding 1 (`VULKAN-149`); and a test asserts the constant an unsupplied `vec3` input actually reads. **Depends on:** `VULKAN-148`. |
