...
**Planning complete; no `VULKAN-*` task implemented yet.** This document was produced by a static
audit of the working tree on 2026-09-04, a mechanical inventory of both renderers' registered
CTests, a build of both configurations, a full run of the Vulkan suite and a targeted EasyGL
cross-check — all in this session. Everything below marked ✅ in prose is
an *observation about the tree*, not a completed task. The task tables start at ⬜ with a single
exception: `VULKAN-011` is ✅ because this session actually ran it, and its result (§7.6) reshaped
two other rows.

Measured this session: the Vulkan suite is **209/215**, with six deterministic failures, four of
which pass on EasyGL from the same sources (§7.5, §7.6). Headline inventory numbers:
...
| ID | Task | Status | Acceptance criterion |
|---|---|---|---|
| VULKAN-020 | Replace `SupportsCapability`'s catch-all `default: return true` with an explicit case per `GraphicsCapability` member | ⬜ | Every member of `CNA::GraphicsCapability` gets its own `case` in `VulkanRenderer::SupportsCapability` (`VulkanRenderer.cpp:1739`), each answering from something real. A `default:` arm, if kept, must fail loudly (`static_assert` on the enum size, or an explicit "unknown capability" refusal) so a future enum member cannot be silently claimed. Test: a case per member asserting the answer matches an independently observable behaviour; plus a compile-time guard that adding a member breaks the build. Non-goal: changing any answer that is already correct. |
| VULKAN-021 | Derive `MultiSampleAntiAliasing` from the device instead of returning `true` unconditionally | ⬜ | The answer comes from `VkPhysicalDeviceLimits::framebufferColorSampleCounts` ∧ `framebufferDepthSampleCounts` (the same source `PickSampleCount` already reads), so a device offering only `VK_SAMPLE_COUNT_1_BIT` reports `false`. EasyGL's `GL_MAX_SAMPLES > 1` is the shape to match. Test: the reported capability agrees with `GetMultiSampleCount()` after requesting 4x, on whichever device the run selected. Depends on `VULKAN-020`. |
...
| VULKAN-091 | Root-cause and fix `Vulkan_DepthBias` | ⬜ | **Observed:** `DepthBias=-1e6 (flat)` renders red where green is expected. The renderer sets real dynamic `vkCmdSetDepthBias` and enables `depthBiasEnable` on ten pipeline creation sites. The failure was historically attributed to Mesa lavapipe; §7.6 removes that explanation, because `EasyGL_DepthBias` **passes** on the same virtual display and the same software rasteriser stack. **Classification:** implementation defect until proven otherwise. **Investigate in this order:** is `vkCmdSetDepthBias` actually recorded on the pipeline the failing draw uses (a deferred draw recorded before the state was set would explain it); is the sign convention right (XNA/D3D9 `DepthBias` is a normalised-depth offset, Vulkan's `depthBiasConstantFactor` is scaled by the format's minimum resolvable difference, and the two differ by more than a factor); is the flat (zero-slope) case reaching `depthBiasSlopeFactor` at all. **Test:** the existing `vulkan_depth_bias_test.cpp` goes green, plus a mutation check that forcing the bias to 0 makes it fail. **Do not** close by widening a tolerance or by re-attributing to the driver without reproducing the pass on a second driver. Re-check on RADV once `VULKAN-012` provides one. |
...
| VULKAN-097 | Root-cause and fix the shared pixel-centre / magnifying-filter divergence (F-18) | ⬜ | **Observed:** three shared-source tests pass on EasyGL and fail on Vulkan (§7.6) — a 1×1 XNA triangle covering 0 pixels, a non-integer 3×3→10×10 point magnification with 19/100 wrong texels, and 15/27 + 142/256 sampler-state legs that interpolate where the contract says they must reproduce a stored texel. **Classification:** implementation defect, single suspected root cause (a half-texel offset that only shows when a filter magnifies or a primitive is sub-pixel). **Intended behaviour:** whatever XNA does, established from §5's authority order — the D3D9 runtime evidence in `spikes/xna-pixel-center-spike/` and `xna4-decomp/`, not from EasyGL's output. CNA already diverges deliberately in places, so where the shared tests encode a CNA decision rather than an XNA one, the decision is cited rather than re-litigated. **Test:** the three existing Vulkan CTests go green without any tolerance being widened and without their expected values being changed; if an expected value genuinely must change, the change is justified by XNA evidence and applied to *both* renderers. **Split rule:** if the root cause turns out to be two (rasterisation rule vs. sampler coordinate), split into two rows before fixing. **Non-goals:** matching EasyGL's pixels; touching the sprite half-integer convention, which passes today. |
...
| ID | Task | Status | Acceptance criterion |
|---|---|---|---|
| VULKAN-130 | Fix the unchecked `VertexBuffer` upload (F-01) | ⬜ | **Observed:** `VulkanVertexBufferRenderer` allocates `max(1, vertex_capacity * 64)` bytes (`VulkanRenderer.cpp:1572`) while `SetData` does `memcpy(mappedPtr_, data, vertex_count * stride_in_bytes)` unchecked (`:1608`); the same file's `MakeExt3DKey` recognises strides 68, 76 and 80 (`:4753`–`:4760`). **Classification:** implementation bug (memory safety). **Intended behaviour:** the allocation is sized from the real stride, or the upload is refused with a named exception, and never writes past the mapping. Sizing at first `SetData` (the stride is known there) or reallocating on growth are both acceptable; silently truncating is not. **Test:** a `VertexBuffer` of a stride-80 layout (glTF skinned PBR + `COLOR_0`) with enough vertices that `count*80` exceeds `count*64` by more than one page — assert the drawn result is correct, assert `GetData` round-trips every byte, and assert zero validation messages. The test must fail on the current code. **Validation:** `ctest -R '^Vulkan_'` plus the new test with the layer enabled. **Non-goal:** changing `IVertexBufferRenderer`'s signature or EasyGL. |
| VULKAN-131 | Fix the unchecked `IndexBuffer` upload (F-02) | ⬜ | Same shape for `SetData16`/`SetData32` (`VulkanRenderer.cpp:1654`, `:1663`) against the `capacity_` the constructor allocated (`:1623`). Test: an `IndexBuffer` written with more indices than its capacity must raise a named exception rather than corrupt memory, and a correctly-sized 32-bit index buffer must still round-trip. Depends on `VULKAN-130` only for the shared bounds helper, if one is introduced. |
...
| VULKAN-144 | Bind an arbitrary `VertexDeclaration` instead of selecting the layout from the stride (F-15) | ⬜ | **Observed:** the renderer derives its `VkVertexInputAttributeDescription` set from a ten-entry stride table (`MakeExt3DKey`, `VulkanRenderer.cpp:4753`–`:4760`; the 2D table at `:4535`), so a declaration whose elements do not match the stride-implied layout is refused at draw time — `Vulkan_DualTextureSlotSamplerContract` skips its entire body over a 28-byte record with `TextureCoordinate0@12 Vector2`, and `Vulkan_RenderTarget_EffectSource` loses two legs to "requires vertex stride 48 or 60". **Classification:** implementation gap (capability), not a correctness bug — the refusal is explicit and by name, which is the right failure mode and must be preserved for anything still unsupported after this task. **Intended:** `SetVertexDeclaration` produces the attribute descriptions from the declaration's own elements (offset, format, usage, usage index) and the pipeline key is keyed on the declaration rather than on the stride, so two different declarations of equal stride get different pipelines. `VulkanVertexFormatHelper` already maps element formats. **Test:** `Vulkan_DualTextureSlotSamplerContract` runs its A..M body instead of skipping, `Vulkan_RenderTarget_EffectSource` reaches 9/9, and a new test binds two declarations with the same stride and different element offsets and proves each samples its own channel — plus a cardinality check that the two produce two pipeline entries, not one. **Depends on:** `VULKAN-130` (the allocation must be stride-correct first). **Non-goals:** multi-stream input (reported `false` deliberately, `REMED-GFX-201`); changing `IVertexBufferRenderer`'s signature; touching EasyGL. |
...
| ID | Task | Status | Acceptance criterion |
|---|---|---|---|
| VULKAN-250 | Declare the shader dialect the Vulkan renderer actually requires | ⬜ | **Observed:** `GetShaderDialectEXT()` is unoverridden, so `GraphicsDevice::GetShaderDialectEXT()` answers `Unknown` although `VulkanEffectRenderer::CompileProgram` accepts nothing but SPIR-V words (`VulkanRenderer.cpp:3475`). **Intended:** the renderer declares a dialect. `ShaderDialectEXT::GlslVulkan` is documented as "GLSL compiled to SPIR-V", which is the closest existing value; if the review concludes CNA's Vulkan `ShaderEffect` takes *already-compiled* SPIR-V and therefore needs its own enumerator, the enumerator is a C-ABI change and must be coordinated with `plans/plan_binding.md` (`MapShaderDialect`, `modules/c-api/src/CnaCApiGraphics.cpp:924`) — in which case this task records the finding, opens the coordination row, and declares the closest honest value meanwhile. **Test:** `GetShaderDialectEXT()` is not `Unknown` on Vulkan, and a test that skips-with-reason on a mismatched dialect (IGL's precedent, `igl_spritebatch_shadereffect_test.cpp:122`) genuinely takes the run branch here. |
...
| VULKAN-332 | Implement `SetSwapInterval` on Vulkan (F-05) | ⬜ | **Observed:** `swapInterval_` is honoured only at swapchain creation (`VulkanRenderer.cpp:2362`); the setter is not overridden, so `GraphicsDeviceManager.SynchronizeWithVerticalRetrace` + `ApplyChanges()` → `GraphicsDevice::Reset` reaches nothing. **Intended:** the setter records the request and recreates the swapchain with the matching present mode when it differs — the renderer already recreates swapchains on resize, so the mechanism exists. `IGraphicsRenderer`'s comment "renderers that cannot change VSync at runtime (e.g. Vulkan)" is a statement about the old implementation and is corrected by this task. **Test:** register `easygl_present_interval_test.cpp` and assert the present mode actually changed (`VK_PRESENT_MODE_FIFO_KHR` ↔ `IMMEDIATE`/`MAILBOX`), not just that nothing crashed. |
| VULKAN-333 | Implement `GetSwapIntervalEXT` on Vulkan | ⬜ | Returning `swapInterval_` makes the request checkable headlessly, which is the whole reason `REMED-GFX-243` added the getter — without it `EasyGL_GraphicsDeviceManager_Vsync` could not distinguish "CNA never asked" from "the driver declined". Register `easygl_graphicsdevicemanager_vsync_test.cpp` on Vulkan. Depends on `VULKAN-332`. |
...
| VULKAN-346 | Stop an unsupported combination from ending the process (F-16) | ⬜ | **Observed:** two Vulkan CTests abort *after* reporting their results — `Vulkan_RenderTarget_EffectSource` on an uncaught `std::runtime_error` ("Vulkan PbrEffect requires vertex stride 48 or 60", `[CRASH] leg B1: killed by signal 6`), and `Vulkan_DualTextureSlotSamplerContract` on `System::InvalidOperationException: Cannot present while render targets are bound` after printing `=== 5/5 PASS ===`. **Classification:** per case. The first is a harness fault — a leg that already recorded its failure re-enters the throwing path — and is fixed in the test. The second looks like a real teardown-ordering defect: a render target is still bound when the game exits and `Present()` refuses. **Intended:** a CTest reports its verdict through its exit code and never through `std::terminate`; if the second case is a renderer/`Game`-shutdown sequencing bug, it gets its own row rather than being papered over in the test. **Test:** both CTests exit with a code that matches their printed verdict, and a new check asserts that ending a frame with a render target bound is either legal or refused at the point of the mistake rather than at shutdown. **Depends on:** `VULKAN-144` removes the first case's trigger but not its uncaught-exception shape. |
...
| ID | Task | Status | Acceptance criterion |
|---|---|---|---|
| VULKAN-370 | Answer `PixelCountIsPreciseEXT` deliberately on Vulkan | ⬜ | Vulkan uses a real `VK_QUERY_TYPE_OCCLUSION` and inherits `true` by default. Confirm the count is genuinely exact on the selected device (`VkPhysicalDeviceFeatures::occlusionQueryPrecise`) and override the method so the answer is stated rather than inherited; if the feature is not enabled, the honest answer is `false` and the renderer must say so. Test: a partially occluded quad whose visible pixel count is known exactly. |
...
| ID | Task | Status | Acceptance criterion |
|---|---|---|---|
| VULKAN-390 | Make descriptor-pool exhaustion fail loudly instead of drawing white (F-06) | ⬜ | **Observed:** `GetOrCreateTexSamplerDescSet` returns `defaultWhiteDescSet_` when `vkAllocateDescriptorSets` fails (`VulkanRenderer.cpp:3424`); the pool holds 512 sets and entries are evicted only when a view dies. **Classification:** implementation bug — a silent wrong-pixel fallback. **Intended:** exhaustion either grows the pool (a second pool chained on demand) or raises a named exception; it must never substitute a resource that renders the wrong image. **Test:** a scene that creates more than `MaxDescriptorSets` distinct `(texture, sampler)` pairs and asserts every sprite draws its own texture. Note the measured result first: `Vulkan_DescriptorCapacityContract`'s capacity legs already pass at 256 simultaneously live textures (§7.5), so the exhaustion point has not been reached by any existing test — this task must *construct* it (more than 512 distinct pairs, or a test-only reduced pool) rather than assume it is reachable by accident. If exhaustion turns out to be unreachable through the public API, that is a valid close: record the bound and still remove the silent substitution. **Non-goal:** redesigning the descriptor strategy; a chained pool or an eviction policy is enough. |
