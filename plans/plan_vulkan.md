...
| `VULKAN-146` | **224/225** | the same one; the added test is `Vulkan_DeclaredVertexLayout`, which passes |
...
| VULKAN-146 | Drive the BasicEffect pipelines from the declaration | ✅ | **Done for `GetOrCreatePipelineFogColored3D`, `GetOrCreatePipelineFogTex3D`, `GetOrCreatePipelineLitTextured3D` and its `VertexLit` sibling.** Each now takes a `VulkanVertexInputLayoutEXT`, overwrites its baked attribute array with the declaration-derived one when the layout is complete, and folds `Hash()` into the pipeline key. The layout is built at **draw** time and snapshotted into `Pending3DDraw`, not read at replay: the record is replayed at `Present()`, by which point the buffer may carry a different declaration entirely. **The guard moved rather than went:** `RequireFaithfulDeclarationEXT` is for a route that infers its input from the stride, and a converted family with a complete declaration no longer does, so the call is now conditional on `!declaredLayout.IsComplete()`. Reaching that decision meant hoisting the family booleans above the guard -- they are pure functions of the stride and the draw params, and everything with a side effect (`NoteSampledSourcesEXT`, `EnsureDefaultWhiteTexture`) still runs after it, which is what keeps the hoist safe. **Three decisions recorded because a later row should not silently re-make them:** (1) the pipeline key gains a **field** (`PipelineKey::vl`) rather than more bits in `a` -- `a` already carries the stride bucket, topology, depth/stencil state, attachment count, depth format and a raw stride at bits 53..63, and a 64-bit hash has nowhere to hide in what is left; (2) an **empty** declaration produces hash 0 and no attribute override, so every unconverted family and every `VertexBuffer(device, count)` buffer keeps byte-identical keys and layouts; (3) which **shader** runs is still chosen by the stride. This row makes the OFFSETS come from the declaration, which is what F-15 actually complains about -- "`TextureCoordinate0@12` ... is not bound at all" is an offset the pipeline did not know, not a program it could not pick. Declaration-driven program *selection* is a separate question no row currently asks. **Evidence:** new CTest `Vulkan_DeclaredVertexLayout` (`vulkan_declared_vertex_layout_test.cpp`), **5/5**. Two **24-byte** declarations the stride table cannot tell apart -- colour at 12 vs colour at 20, each buffer filled to match its own -- draw their own colours, and the cache goes `0 -> 1 -> 2` entries, so they did not share a pipeline. A buffer with no declaration still draws through the stride path. **Two mutation probes, each isolating one half:** with the layout out of the *key*, legs B and C fail and B reads `(0,0,64)` -- the UV floats at offset 12 read as a colour, exactly the corruption the row exists to prevent; with the layout in the key but not applied to the *attributes*, only B fails. **Regression evidence:** the conversion touches the pipeline construction of every BasicEffect draw, and the suite went 223/224 -> **224/225** -- the new test, and nothing else moved. |
| VULKAN-147 | Drive the alpha-test, dual-texture, environment-map and skinned pipelines from the declaration | ⬜ | Same conversion for `GetOrCreatePipelineAlphaTest3D`, `GetOrCreatePipelineDualTexture3D`, `GetOrCreatePipelineEnvMap3D` and the two skinned factories. **Acceptance:** `Vulkan_DualTextureSlotSamplerContract` runs its whole `A..M` body instead of skipping it -- the 28-byte record carrying `TextureCoordinate0@12 Vector2` that §7.5 records as unbindable is exactly this row's case -- and the existing `Vulkan_SkinnedEffect_*` and `Vulkan_EnvironmentMapEffect_*` tests stay green. **Depends on:** `VULKAN-146`. |
