...
**Implementation under way.** Twenty-eight tasks are ✅ (`VULKAN-004`, `-020`, `-021`, `-091`, `-097`,
`-130`, `-131`, `-144`, `-145`, `-146`, `-147`, `-148`, `-149`, `-150`, `-151`, `-152`, `-153`, `-154`,
`-155`, `-156`, `-157`, `-158`, `-250`, `-332`, `-333`, `-346`, `-370`, `-390`), plus
...
Twelve rows were opened by work that discovered something the plan had not, and **nine of them
...
**Still open from that list:** `VULKAN-098` (F-19, the two renderers place the same XNA `z` at
different depths and EasyGL is the one that diverges from XNA), `VULKAN-264` (F-08's tail,
`ShaderDialectEXT::GlslVulkan` does not distinguish GLSL source from SPIR-V bytecode), and
`VULKAN-159` (F-28, the hazard underneath F-27: a platform destructor that blocks on a lock its
own thread may already hold, with today's only trigger now removed).
...
**The Vulkan suite is 231/231** on `-R '^Vulkan_'`, from the 209/215 this plan opened with. It is green
...
| VULKAN-159 | Make the platform destructor safe against `exit()` from inside its own lock (F-28) | ⬜ | **Opened by `VULKAN-157`'s diagnosis, and deliberately left open rather than folded into it.** `VULKAN-154` closed the one route that was actually firing; this row is about the hazard underneath, which outlives that route. **The shape of the problem:** `~Sdl3Platform` takes `SdlGlobalStateMutex()`, and eleven methods hold it across calls into SDL, so `exit()` reached from inside any of them deadlocks on a non-recursive mutex. **Three candidate answers, and the choice is the substance of this row.** (a) Track the owning thread beside the mutex and let the destructor skip re-locking when it already owns it -- explicit, documents the exception, and does not make recursion legal anywhere else. (b) Make the mutex `std::recursive_mutex` -- one line, and it silently sanctions re-entrancy in ten other methods that currently must not have it, which is a real loss. (c) Do not destroy the platform singleton at `exit()` at all (a deliberately leaked `IPlatform`), which is the usual answer for a process-global whose destructor can only run during teardown -- and would also stop `SDL_QuitSubSystem` running after the host may already be gone. **(a) or (c) are the real options; (b) is recorded so it is not rediscovered as a shortcut.** **Acceptance:** a test that calls `std::exit` (in a forked child, so the suite survives) from inside a platform call holding the mutex, and asserts the child terminates rather than hanging -- the exact shape F-27 produced, with a route CNA controls. **Owner:** the platform layer, `plans/plan_platform.md`; it sits here because this plan found it, and it should be moved rather than duplicated if that plan takes it. |
