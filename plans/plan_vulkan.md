...
**Implementation under way.** Fourteen tasks are ✅ (`VULKAN-004`, `-020`, `-021`, `-091`, `-097`,
`-130`, `-131`, `-145`, `-250`, `-332`, `-333`, `-346`, `-370`, `-390`), plus `VULKAN-011` from the
planning session. `VULKAN-146` was implemented, measured, **reverted** and re-scoped — read F-21
first; it found that this plan's own baseline cannot see the shared suite that already measures
F-15. Four rows were opened by work that discovered something the plan had not:
...
**The Vulkan suite is 223/224** on `-R '^Vulkan_'`, from the 209/215 this plan opened with, and
**8826/8856** on the rest of the configuration. §7.5 carries the running baseline, one line per
...
| VULKAN-146 | Drive the BasicEffect pipelines from the declaration | ⬜ | **Implemented, measured, and REVERTED (`beddf1575`, reverted). Re-scoped -- read F-21 before attempting it again.** The conversion itself worked: the four factories took the declaration-derived attribute array and the layout hash, two 24-byte declarations the stride table cannot tell apart drew their own colours and got their own pipelines, an empty declaration kept the stride path, and `ctest -R '^Vulkan_'` was **224/225** with no `Vulkan_*` regression. **What the `Vulkan_*` filter did not see, and this plan's baseline never had:** the shared, renderer-agnostic `VertexDeclarationLayoutTests.cpp` (REMED-GFX-216/234) is the cross-renderer statement of this very defect, it is registered in this configuration, and it went from **10 passing to 8 failing**. §7.5 measures `-R '^Vulkan_'` only -- 215 of the configuration's 9071 CTests -- so it cannot see a shared suite at all. That is F-21, and it is the more important half of this entry. **Why the failures were right to fail:** that suite has two mutually exclusive arms chosen by one boolean, `TranslatesDeclarations()`. The refusal arm asserts Vulkan **refuses** `colorPosition16` and `positionTextureColor24` -- exactly the declarations the conversion makes it render *correctly*. Moving Vulkan into the translating arm makes six of them pass and then fails the translating control on `positionColorPadded32`: `Position0@0 + Color0@12` at stride **32**, which selects the lit-textured program, whose `Normal0` and `TextureCoordinate0` the declaration does not supply. Refusing that is **correct** -- a Vulkan shader input with no attribute description reads undefined data -- so neither arm describes the renderer. **The state the conversion creates is a third one the shared contract cannot name:** *renders what it can bind, refuses what it cannot*. **What this row must now include, and did not:** (1) a decision on an input the declaration leaves unsupplied -- XNA/D3D9 defaults it and renders, Vulkan cannot leave it unbound, so this needs a real answer (a zero-filled default attribute, or a documented refusal), and it is the thing that decides whether `positionColorPadded32` renders; (2) how the shared DECL-GUARD suite describes a partially-translating renderer, agreed with whoever owns REMED-GFX-234 rather than by adding an identity to a list; (3) the conversion itself, unchanged from what was measured. **Kept:** `VULKAN-145`'s builder, which is inert, correct and proven (`Vulkan_VertexInputLayout`, 11/11). **Depends on:** `VULKAN-145`. |
