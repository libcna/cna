...
**Implementation under way.** Twenty-one tasks are ✅ (`VULKAN-004`, `-020`, `-021`, `-091`, `-097`,
`-130`, `-131`, `-144`, `-145`, `-146`, `-147`, `-148`, `-149`, `-152`, `-156`, `-250`, `-332`, `-333`,
`-346`, `-370`, `-390`), plus
...
Eight rows were opened by work that discovered something the plan had not:
...
`Vector4`-spelled `BlendIndices` is bindable on EasyGL and not here), `VULKAN-153` (F-22, `CnaInputTests` can deadlock and `ctest` will not reap
it), `VULKAN-154` (F-23, an X protocol error ends the whole test binary), `VULKAN-155` (F-24, the
...
| VULKAN-153 | Stop `CnaInputTests` from being able to hang a whole `ctest` run (F-22) | ⬜ | **Opened by `VULKAN-148`'s baseline, which could not finish until the process was killed.** Two separable pieces, and the second is worth doing even if the first is judged out of this plan's scope. **(a) The deadlock itself:** the suite blocks on a private glibc mutex with a NULL timeout, single-threaded, and both observed occurrences had a *second* copy of the same suite running from another checkout. The suspects, in order: a process-global SDL subsystem (joystick/haptic enumeration walks udev and `/dev/input`), a lock taken in a `SDL_Init`/`SDL_Quit` path that a previous repeat left held, and a fork-in-a-threaded-process (a gtest death test forking while an SDL thread holds a lock gives the child exactly this signature). Diagnosis needs a stack, and there is **no `gdb` on this machine** -- installing one, or building the suite with `-fsanitizer=thread`, is the first step. **(b) The un-reaped hang:** `add_test([=[CnaInputTests]=] …)` carries no `TIMEOUT` property and `ctest` let it run 45 minutes; every other long suite here is bounded. A `TIMEOUT` on this test converts an unbounded stall into a reported failure, which is what made the `VULKAN-148` baseline expensive to take. **Acceptance:** (b) a `TIMEOUT` set in `cmake/TestHelpers.cmake` (or on this test) generous enough for its measured 221 s and its 628 s worst case, proven by an `add_test` property assertion or a `CTestTestfile.cmake` diff; (a) either a fix with a test that runs two copies of the input suite concurrently and both finish, or -- if the cause is genuinely a shared machine resource that cannot be made concurrent -- a `RESOURCE_LOCK` on the test plus the reason recorded here. **Not a Vulkan-renderer defect**; it is on this plan's board because it is what stopped this plan's own measurement method. |
