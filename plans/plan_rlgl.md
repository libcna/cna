...
**Status:** Active — device, classic Texture2D/samplers, pipeline state, buffers, primitive submission, BasicEffect/AlphaTestEffect, and built-in SpriteBatch/SpriteFont slices complete
...
2. Device lifecycle, physical/logical presentation tracking, exact selective clears, viewport/scissor coordinates, swap interval, default-framebuffer restoration, complete backbuffer pipeline state, all 20 classic Texture2D formats, GL sampler objects, fixed-capacity vertex/index resources, every primitive topology, public bound/user primitive routes, BasicEffect/AlphaTestEffect, and built-in SpriteBatch/SpriteFont rendering are implemented. DualTexture/EnvironmentMap/Skinned/custom effects, multi-stream input, render targets, reset, and query resources remain unavailable and throw stable diagnostics where a required hook exists.
...
| Shaders | Textures/samplers | Implemented | Active texture, cubemap, sampler parameters | 🟨 | Texture2D unit zero, default white, independent sampler, and stale sampling-disable transitions pass; multi-unit/cubemap/reflection remain |
| Shaders | Vertex attributes | Semantic declaration mapping | Generic attributes plus integer/divisor variants | 🟨 | Position0/Normal0/Color0/TextureCoordinate0 plus all classic native formats pass; dual UV, blend inputs, multi-stream, and custom reflection remain |
| Shaders | Fog | Implemented in stock effects | General shader support | 🟨 | Lit/unlit BasicEffect and AlphaTestEffect use the exact forwarded view-space-Z fog vector with pixel evidence; remaining fog-capable stock families remain |
...
| RLGL-012 | 🟨 | RLGL-009, RLGL-011 | Shader/effect umbrella. RLGL-033 completes the shared unlit stock pipeline and AlphaTestEffect; RLGL-034–038 own lighting, dual texture, environment mapping, skinning, and custom/compiled effects. Only the completed subtasks count as available. |
...
| RLGL-035 | ⬜ | RLGL-033 | Implement DualTextureEffect with TextureCoordinate0/1 semantic binding, independent texture/sampler slots, vertex color, diffuse/alpha, and fog. Validate combine, UV, addressing, and slot-transition pixels. |
