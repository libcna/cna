...
**Status:** Active — device, classic Texture2D/samplers, pipeline state, buffers, primitive submission, and built-in SpriteBatch/SpriteFont slices complete
...
2. Device lifecycle, physical/logical presentation tracking, exact selective clears, viewport/scissor coordinates, swap interval, default-framebuffer restoration, complete backbuffer pipeline state, all 20 classic Texture2D formats, GL sampler objects, fixed-capacity vertex/index resources, every primitive topology, public bound/user primitive routes, and built-in SpriteBatch/SpriteFont rendering are implemented. Textured/lit/advanced stock effects, custom effects, multi-stream input, render targets, reset, and query resources remain unavailable and throw stable diagnostics where a required hook exists.
...
| Buffers | 16-bit index buffers | Implemented | EBO wrappers and unsigned-short indexed draw | ✅ | Fixed-capacity allocation and exact values/tails passed RLGL-030; drawing is RLGL-031 |
| Buffers | 32-bit index buffers | Implemented | Upload works; draw wrapper hardcodes unsigned short | ✅ | Exact values above 65535 passed native storage; private 32-bit draw remains RLGL-031 |
