...
**Status:** Active — device vertical slice, documentation/CI gate, and RGBA8 Texture2D baseline complete
...
2. Device lifecycle, physical/logical presentation tracking, exact selective clears, viewport/scissor coordinates, swap interval, default-framebuffer restoration, basic depth/blend/write toggles, and RGBA8 Texture2D storage/update/readback/binding are implemented. Other texture formats, sampler state, SpriteBatch, buffer, draw, effect, render-target, reset, and query resources remain unavailable and throw stable diagnostics where a required factory exists.
...
| RLGL-025 | ⬜ | RLGL-023 | Implement deterministic SamplerState mapping with per-slot GL sampler objects (filter, address U/V/W, LOD range/bias, anisotropy and comparison behavior where applicable), measured limits, cache transitions, and sampled-pixel tests. Avoid mutating shared texture-object parameters per slot. |
