...
**Status:** Active — device, classic Texture2D/samplers, pipeline state, and built-in SpriteBatch/SpriteFont slices complete
...
- Fixed-capacity rlgl vertex and 16/32-bit index resources are runtime-validated for public packed streams, every classic declaration format, exact native storage, and all dynamic update options. General primitive submission remains the separate RLGL-031 gate.
...
2. Device lifecycle, physical/logical presentation tracking, exact selective clears, viewport/scissor coordinates, swap interval, default-framebuffer restoration, complete backbuffer pipeline state, all 20 classic Texture2D formats, GL sampler objects, fixed-capacity vertex/index resources, and built-in SpriteBatch/SpriteFont rendering are implemented. Custom SpriteBatch effects plus general primitive draws, effect, render-target, reset, and query resources remain unavailable and throw stable diagnostics where a required hook exists.
...
| Buffers | Vertex declarations | Implemented semantic/type mapping | Generic attributes and divisors; CNA must map declarations | 🟨 | All twelve formats retain declaration metadata and exact bytes; GL attribute mapping/pixels remain RLGL-031 |
...
| RLGL-011 | 🟨 | RLGL-006 | Buffer/draw umbrella. RLGL-030 owns declaration-aware fixed-capacity resources and dynamic update hints; RLGL-031 owns primitive submission and pixel validation. User primitives share those paths through GraphicsDevice's transient resources. |
...
| RLGL-031 | ⬜ | RLGL-030, RLGL-013 | Implement non-indexed and indexed point/line/triangle list/strip submission, declaration format mapping, transforms, 16/32-bit indices, and public user-primitive paths. Validate topology, declaration, update, state-transition, and pixel matrices. |
