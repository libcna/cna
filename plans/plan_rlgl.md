...
**Status:** Active — device, classic Texture2D/samplers, pipeline state, buffers, primitive submission, unlit BasicEffect/AlphaTestEffect, and built-in SpriteBatch/SpriteFont slices complete
...
- The first reusable stock-effect pipeline is runtime-validated for unlit BasicEffect and AlphaTestEffect: Position0/Color0/TextureCoordinate0 bind by semantics, texture/default-white sampling, diffuse/emissive/alpha/vertex-color composition, every alpha comparison, view-space-Z fog, deterministic textured/untextured transitions, and EasyGL's measured XNA/D3D9 pixel-center correction all pass pixel gates.
...
2. Device lifecycle, physical/logical presentation tracking, exact selective clears, viewport/scissor coordinates, swap interval, default-framebuffer restoration, complete backbuffer pipeline state, all 20 classic Texture2D formats, GL sampler objects, fixed-capacity vertex/index resources, every primitive topology, public bound/user primitive routes, unlit BasicEffect/AlphaTestEffect, and built-in SpriteBatch/SpriteFont rendering are implemented. Lighting, DualTexture/EnvironmentMap/Skinned/custom effects, multi-stream input, render targets, reset, and query resources remain unavailable and throw stable diagnostics where a required hook exists.
...
| Shaders | BasicEffect | Implemented | Shader compile/link/uniform/attribute wrappers are sufficient | 🟨 | Unlit textured/untextured, vertex color, material color/alpha/emissive, fog, WVP, and pixel-center behavior pass RLGL-033; lighting variants remain RLGL-034 |
...
| Shaders | Shader constants/uniforms | Implemented | Scalar/vector/matrix/array setters | 🟨 | RLGL-031/033 validate scalar, vec3/vec4, sampler, and matrix stock uniforms; arrays/reflected custom parameters remain RLGL-037/038 |
| Shaders | Matrices | XNA conventions normalized by renderer shaders | Column-major rlgl matrix representation; raw uniform upload available | 🟨 | Baseline `world * view * projection` and column-major upload passed a translated-pixel oracle; every stock family remains RLGL-012 |
...
| Shaders | Vertex attributes | Semantic declaration mapping | Generic attributes plus integer/divisor variants | 🟨 | Position0/Color0/TextureCoordinate0 plus all classic native formats pass; normal, dual UV, blend inputs, multi-stream, and custom reflection remain |
| Shaders | Fog | Implemented in stock effects | General shader support | 🟨 | BasicEffect and AlphaTestEffect use the exact forwarded view-space-Z fog vector with pixel evidence; remaining fog-capable stock families remain |
...
| 3D | Meshes | Implemented through Model/Mesh draw paths | General indexed vertex draws available with bridge gaps | 🟨 | General indexed topology/resource machinery works; stock texture/normal/lighting semantics and sample corpus remain RLGL-012/RLGL-018 |
...
| RLGL-033 | ✅ | RLGL-031, RLGL-009, RLGL-025 | Generalized the baseline shader into one declaration-driven unlit stock pipeline with optional Color0/TextureCoordinate0, rlgl Texture2D/default-white binding, diffuse/emissive/alpha composition, FNA's four-value alpha-test encoding, view-space-Z fog, and the same 63/64-pixel clip correction EasyGL applies to non-MSAA stock draws. Unsupported lit/dual/cube/skinned/custom shapes still fail before submission. Validated 2026-09-12: `cna_test_rlgl_effect` passed 15 gates covering texture row orientation, material and vertex tint, null-texture fallback, textured→untextured reset, all eight `CompareFunction` variants at equality, AlphaTest diffuse/color/fog composition, BasicEffect fog, and explicit lit-effect refusal. CNA's existing renderer-neutral `xna_pixel_center_contract_test.cpp` passed its measured 1×1 XNA triangle and control triangle; `cna_test_rlgl_primitive` remained 24/24. Effect, pixel-center, and primitive targets passed rebuilt AddressSanitizer runs. |
| RLGL-034 | ⬜ | RLGL-033 | Implement BasicEffect per-vertex and per-pixel lighting variants, Position0/Normal0 with optional UV/color, non-uniform world normal matrices, ambient/emissive, three directional diffuse/specular lights, material specular power, eye position, fog, and untextured lighting. Validate against EasyGL/XNA lighting fixtures. |
