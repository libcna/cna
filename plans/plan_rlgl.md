...
**Status:** Active — device vertical slice, documentation/CI gate, every uncompressed classic Texture2D format, and sampler objects complete
...
- Every uncompressed classic-XNA Texture2D format is runtime-validated: rlgl wrappers own the exact layouts they expose, measured private GL calls cover its format gaps and arbitrary mip operations, exact XNA bytes survive full/partial native round-trips, and texture swizzles reproduce one-/two-channel and alpha-only sampling semantics. DXT1/3/5 remain the final Texture2D format group.
...
- `docs/rlgl-renderer.md` records the verified boundary, profile/platform status, build, offline dependency, update, licensing, and debugging procedures. A Linux compile-focused CI lane builds both the renderer and the focused smoke executable from an independently checked-out copy of the exact upstream commit without claiming hosted runtime coverage.
...
2. Device lifecycle, physical/logical presentation tracking, exact selective clears, viewport/scissor coordinates, swap interval, default-framebuffer restoration, basic depth/blend/write toggles, all 17 classic uncompressed Texture2D formats, and GL sampler objects are implemented. DXT textures, SpriteBatch, buffer, draw, effect, render-target, reset, and query resources remain unavailable and throw stable diagnostics where a required factory exists.
...
| Textures | Texture2D | Implemented | `rlLoadTexture`, update, bind, unload; several exact formats need a bridge | 🟨 | All 17 uncompressed classic formats passed RLGL-023/026/027; DXT1/3/5 remain RLGL-028 under RLGL-024 |
...
| Textures | Texture updates | Implemented | `rlUpdateTexture` supports level-zero subrect upload | 🟨 | Public full and partial `SetData` paths round-trip exact bytes for all 17 uncompressed classic formats, including odd-row alignment; DXT remains RLGL-028 |
| Textures | Readback | Implemented | Desktop `rlReadTexturePixels`; arbitrary levels/subrects and compressed readback unavailable | 🟨 | Private desktop bridge passed exact format-native readback for all uncompressed classic formats; DXT remains RLGL-028 |
...
| RLGL-009 | 🟨 | RLGL-006 | Texture2D and sampler umbrella, split after measuring the format and sampler boundaries into RLGL-023 through RLGL-028. Complete only when RLGL-023, RLGL-024, and RLGL-025 close. |
...
| RLGL-024 | 🟨 | RLGL-023 | Remaining-classic-Texture2D umbrella. The complete classic inventory is `Color`, `Bgr565`, `Bgra5551`, `Bgra4444`, DXT1/3/5, `NormalizedByte2/4`, `Rgba1010102`, `Rg32`, `Rgba64`, `Alpha8`, `Single`, `Vector2/4`, `HalfSingle`, `HalfVector2/4`, and `HdrBlendable`; CNAEXT formats 20–26 do not gate this task. Complete when the measured uncompressed and compressed subtasks RLGL-026 through RLGL-028 close. |
...
| RLGL-028 | ⬜ | RLGL-026 | Implement DXT1/DXT3/DXT5 storage through rlgl's capability-gated compressed path, including exact block-size validation, mip/subrectangle update constraints, truthful unsupported diagnostics, and a non-shadowed native validation strategy. Do not emulate successful compression. |
