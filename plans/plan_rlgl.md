...
**Status:** Active — device, classic Texture2D/samplers, pipeline state, buffers, primitive submission, BasicEffect/AlphaTestEffect/DualTextureEffect, and built-in SpriteBatch/SpriteFont slices complete
...
2. Device lifecycle, physical/logical presentation tracking, exact selective clears, viewport/scissor coordinates, swap interval, default-framebuffer restoration, complete backbuffer pipeline state, all 20 classic Texture2D formats, GL sampler objects, fixed-capacity vertex/index resources, every primitive topology, public bound/user primitive routes, BasicEffect/AlphaTestEffect/DualTextureEffect, and built-in SpriteBatch/SpriteFont rendering are implemented. EnvironmentMap/Skinned/custom effects, multi-stream input, render targets, reset, and query resources remain unavailable and throw stable diagnostics where a required hook exists.
...
| Shaders | Shader constants/uniforms | Implemented | Scalar/vector/matrix/array setters | 🟨 | RLGL-031/033/034 validate scalar, vec3/vec4, sampler, world/WVP matrices, and a mat3 represented through three vec3 columns; arrays/reflected custom parameters remain RLGL-037/038 |
| Shaders | Matrices | XNA conventions normalized by renderer shaders | Column-major rlgl matrix representation; raw uniform upload available | 🟨 | `world * view * projection`, full world position, large-world precision, and inverse-transpose non-uniform normal transforms pass; remaining stock families remain RLGL-012 |
...
| Shaders | Vertex attributes | Semantic declaration mapping | Generic attributes plus integer/divisor variants | 🟨 | Position0/Normal0/Color0/TextureCoordinate0/1 plus all classic native formats pass; blend inputs, multi-stream, and custom reflection remain |
| Shaders | Fog | Implemented in stock effects | General shader support | 🟨 | Lit/unlit BasicEffect, AlphaTestEffect, and DualTextureEffect use the exact forwarded view-space-Z fog vector with pixel evidence; remaining fog-capable stock families remain |
| Shaders | Lighting | Implemented in stock effects | General shader support | 🟨 | BasicEffect's full three-light, material, vertex/pixel-stage, normal-transform, and precision contract passes RLGL-034; EnvironmentMapEffect and SkinnedEffect remain |
...
| 3D | Lighting | Implemented | Shader capability sufficient | 🟨 | BasicEffect passes RLGL-034; EnvironmentMapEffect and SkinnedEffect remain RLGL-036/037 |
...
| RLGL-012 | 🟨 | RLGL-009, RLGL-011 | Shader/effect umbrella. RLGL-033 completes the shared unlit stock pipeline and AlphaTestEffect; RLGL-034/035 complete BasicEffect lighting and dual texture; RLGL-036–038 retain environment mapping, skinning, and custom/compiled effects. Only the completed subtasks count as available. |
...
| RLGL-037 | ⬜ | RLGL-034 | Implement SkinnedEffect with BlendWeight0/BlendIndices0 format handling, 72-matrix palette upload, 1/2/4 weights, per-vertex/per-pixel lighting, optional vertex color, texture, and fog. Measure GL 3.3 uniform limits and validate animated output. |
