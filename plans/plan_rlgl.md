...
**Status:** Active — device vertical slice, documentation/CI gate, six uncompressed texture formats, and sampler objects complete
...
- The Texture2D baseline is runtime-validated for `Color`, `Bgr565`, `Bgra5551`, `Bgra4444`, `NormalizedByte2`, and `NormalizedByte4`: rlgl wrappers own the formats they expose, measured private GL calls cover signed-normalized storage and arbitrary mip operations, exact XNA bytes survive full/partial native round-trips, and the maximum texture dimension is reported from the live context.
...
2. Device lifecycle, physical/logical presentation tracking, exact selective clears, viewport/scissor coordinates, swap interval, default-framebuffer restoration, basic depth/blend/write toggles, six classic uncompressed Texture2D formats, and GL sampler objects are implemented. The remaining high-precision/float and DXT texture formats, SpriteBatch, buffer, draw, effect, render-target, reset, and query resources remain unavailable and throw stable diagnostics where a required factory exists.
...
| Textures | Texture2D | Implemented | `rlLoadTexture`, update, bind, unload; signed-normalized storage needs a bridge | 🟨 | `Color` passed RLGL-023 and five packed/signed-normalized formats passed RLGL-026; remaining classic formats are RLGL-027/028 under RLGL-024 |
...
| Textures | Mipmaps | Implemented | `rlGenTextureMipmaps`; arbitrary allocation/update lacks public wrappers | 🟨 | Declared RGBA8 chains and exact non-zero-level upload/readback passed RLGL-023; generation/filtering remain |
| Textures | Texture updates | Implemented | `rlUpdateTexture` supports level-zero subrect upload | 🟨 | Public full and partial `SetData` paths round-trip exact bytes for six uncompressed formats, including odd-row alignment; remaining formats are RLGL-027/028 |
| Textures | Readback | Implemented | Desktop `rlReadTexturePixels`; arbitrary levels/subrects and compressed readback unavailable | 🟨 | Private desktop bridge passed exact format-native readback for six uncompressed formats; remaining formats are RLGL-027/028 |
