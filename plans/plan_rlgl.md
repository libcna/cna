...
**Status:** Active — device vertical slice, documentation/CI gate, RGBA8 textures, and sampler objects complete
...
- The first resource slice is runtime-validated: `SurfaceFormat::Color` Texture2D storage is created and bound through rlgl, declared mip levels are allocated, uploads and native readback preserve exact RGBA8 bytes and XNA row order, and the maximum texture dimension is reported from the live context.
...
| Textures | Texture2D | Implemented | `rlLoadTexture`, update, bind, unload | 🟨 | `SurfaceFormat::Color` allocation/upload/native readback/binding passed RLGL-023; remaining classic formats are RLGL-024 |
...
| Textures | Texture updates | Implemented | `rlUpdateTexture` supports level-zero subrect upload | 🟨 | Public full and partial `SetData` paths round-trip exact RGBA8 bytes; remaining formats are RLGL-024 |
| Textures | Readback | Implemented | Desktop `rlReadTexturePixels`; arbitrary levels/subrects and compressed readback unavailable | 🟨 | Private desktop bridge passed exact RGBA8 level/subrect and row-order checks; remaining formats are RLGL-024 |
...
| RLGL-009 | 🟨 | RLGL-006 | Texture2D and sampler umbrella, split after measuring the format and sampler boundaries into RLGL-023 through RLGL-025. Complete only when those tasks close. |
...
| RLGL-024 | ⬜ | RLGL-023 | Inventory and implement the remaining classic-XNA Texture2D `SurfaceFormat` mappings, including exact transfer sizes/types, capability-gated compressed storage, updates and truthful readback behavior. Add per-format data tests and document profile limitations without conversions that change XNA bytes. |
