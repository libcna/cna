...
and passes its whole registered suite -- `ctest -R Igl` is **70/70**, comprising 34 host-portable
unit cases in `CnaTests` and 36 example tests against Mesa llvmpipe (OpenGL/GLX) and lavapipe
(Vulkan) under Xvfb.

Run explicitly on **both** backends (all 27 example binaries x 2, not only the ones the ctest
registration forces onto Vulkan), the picture is:

* **OpenGL/GLX: 27 of 27 binaries pass, every check.**
* **Vulkan: 27 of 27.** Parity, as of 2026-08-18. `Igl_2D` and `Igl_Msaa` were casualties of the row
  order defects IGL-60/IGL-67 records; the three custom-`ShaderEffect` tests failed for a single
  documented reason (IGL-43: loose uniforms do not exist in Vulkan GLSL), which is now closed by a
  declared std140 block rather than worked around. All six pass here.

  What remains is upstream limits of IGL `v1.1.1`, not unfinished work: no occlusion query API at
  all (IGL-61), no sampler LOD-bias field (IGL-62), no way to name a resolving cube face (IGL-63),
  and no way to attach a 3D texture to a framebuffer, which is what makes volume readback impossible
  (IGL-17). Each was re-verified against the pinned source or established by attempting it, not
  restated.

  A custom `ShaderEffect` still needs two shader sources, and that too is a property of SPIR-V
  rather than a gap here: explicit `layout(location = N)` on every user in/out, and a different
  `#version`. `GraphicsDevice::GetShaderDialectEXT()` (IGL-72) is how an application asks which one
  to supply, and `docs/igl-renderer.md` carries the worked example.
...
| C | Resources (textures, buffers, targets) | 🔶 `Texture2D`, `RenderTarget2D`, depth and `RenderTarget2D.GetData` (`Igl_RenderTarget`), full 2- and 4-slot MRT plus the cube-face-in-MRT refusal (`Igl_Mrt`/`Igl_Mrt4`/`Igl_MrtCubeRefuse`), `TextureCube` including **readback at every declared mip level** (IGL-17), `RenderTargetCube` and `Texture3D` upload and sampling -- all verified on BOTH backends. The surface-format boundary and every transfer's byte arithmetic likewise (`Igl_SurfaceFormat`), with `Rg32` and `Single` promoted to the public API on the strength of the whole path, not storage alone (IGL-71). Still 🔶 for one reason, and it is upstream: volume READBACK is impossible at IGL `v1.1.1`, which cannot attach a 3D texture to a framebuffer at all (IGL-17, established by attempting it) |
| D | 2D pipeline (`SpriteBatch`) | ✅ verified on BOTH backends. `Igl_2D` passes 5/5 on Vulkan since IGL-60 Bug C was root-caused -- it was never a `SpriteBatch` defect but a readback one, `ReadBackbuffer` measuring the requested rectangle's y from the wrong edge. `Igl_SpriteBatchShaderEffect` drives a custom effect through `Begin(effect)` on both |
| E | 3D pipeline and stock effects | ✅ every stock effect (`BasicEffect` through `PbrEffect`, plus fog, alpha test, dual texture, skinning, stencil, MSAA and instancing) has a pixel test, and all of them pass on BOTH backends |
| F | Custom `ShaderEffect` | ✅ verified on both backends, parameters included (IGL-43). Compilation, uniform and texture paths on OpenGL/GLX; on Vulkan the same through a declared std140 block, with `Igl_ShaderEffectTexture3D_Vulkan`, `Igl_SpriteBatchShaderEffect_Vulkan`, `Igl_Instancing_Vulkan` and `Igl_CustomEffectBackend_Vulkan` guarding it. A shader that violates SPIR-V's own requirements is refused with glslang's line-and-reason text plus the requirement in words (IGL-70), and `GraphicsDevice::GetShaderDialectEXT()` (IGL-72) is how an application asks which dialect to supply |
| G | Tests, docs and gates | ✅ 70/70 registered tests pass; the five platform boundary gates and the four renderer-registry gates all run clean; `docs/graphics-renderer-feature-matrix.md` carries an `IGL` column (IGL-53) and `docs/igl-renderer.md` a worked custom-`ShaderEffect` example |
| H | Verification and hardening | ✅ IGL-56-59, IGL-60, IGL-64, IGL-65, IGL-66, IGL-67 and IGL-70 done; IGL-17's volume half, IGL-61, IGL-62 and IGL-63 are upstream limits of `v1.1.1`, each re-verified against the pinned source or established by attempting it rather than restated |
...
| IGL-2 | Classify the backend | ✅ | `GraphicsBackendCategory` = `TranslationLayer`, `GraphicsBackendMaturity` = `Experimental` — Exercised: `GraphicsRendererMetadataTests` and the identity gates pass with this classification.|
| IGL-3 | CMake selection | ✅ | `cmake/RendererSelection.cmake`: option, STRINGS, explicit-selection chain, `RENDERER_DIR`/`RENDERER_TARGET` mapping — Exercised: `-DCNA_GRAPHICS_RENDERER=IGL` configures and builds, and `check_renderer_identities`/`check_renderer_combinations` pass.|
| IGL-4 | Third-party integration | ✅ | `cmake/ThirdPartyIGL.cmake`: pinned `v1.1.1`, `CNA_IGL_ROOT` escape hatch, pruned dependency bootstrap, backend options — Exercised: the pinned `v1.1.1` tree builds both backends and 27 example binaries link against it.|
| IGL-5 | Module skeleton | ✅ | `modules/renderers/igl/{CMakeLists.txt,include,src,examples,tests}` — Exercised: the module builds as `cna_renderer_igl` and its descriptor compiles in every configuration (the descriptor gate).|
| IGL-6 | Backend selection | ✅ | `IglRendererSelection.{hpp,cpp}`, `CNA_IGL_BACKEND` — Exercised: `IglRendererSelectionTests` covers the override grammar, the compiled-in filter and the per-process stability of the answer; both backends are selected for real by the `_Vulkan` ctest entries.|
...
| IGL-8 | Identity gates | ✅ | the four `modules/graphics/tests/**` identity suites, `cmake/Tests/ModuleProbes.cmake` link-closure forbid lists — Exercised: the four renderer gates pass, `GraphicsRendererCompileDefinitionsTest.IglRendererIsReportedByName` included.|

### Phase B — device bring-up and presentation

| ID | Task | State | Notes |
|----|------|-------|-------|
| IGL-9 | OpenGL device on GLX | ✅ | adopt the platform `IPlatformGlContext`'s context via `glXGetCurrentContext`, wrap in `igl::opengl::glx::Context`/`Device` — Exercised: 27 example binaries render and read back real pixels on OpenGL/GLX.|
| IGL-10 | Vulkan device | ✅ | `igl::vulkan::HWDevice::createContext/queryDevices/create` from the X11 native handle — Exercised: the same 27 binaries on Vulkan against Mesa lavapipe.|
| IGL-11 | Swap surface and framebuffer | ✅ | `createTextureFromNativeDrawable`/`…Depth`, one `IFramebuffer` re-pointed per frame with `updateDrawable` — Exercised: every pixel test presents and reads the back buffer; `Igl_VulkanBackBufferBgr` pins the surface format specifically.|
| IGL-12 | Frame model | ✅ | lazily opened render pass; clears as load actions; `Present()` = end pass + present + submit — Exercised: every example test drives real frames; `Igl_ReadbackOrientation` depends on the flush boundary being right.|
| IGL-13 | Presentation policies | ✍️ | all five `CnaPresentationMode` values, resize, `GetDefaultViewportRect`, `TransformWindowToLogical`/`…LogicalToWindow` — **No dedicated test, deliberately noted rather than overlooked: presentation policy is observable only through a real window manager, which the Xvfb harness does not provide.**|
| IGL-14 | Swap interval | ✍️ | honoured on OpenGL; Vulkan reports `false` rather than pretending — **No dedicated test: `SetSwapInterval` returns false on Vulkan (the present mode is fixed at swap-chain creation) and a vsync-honouring OpenGL run is not observable under Xvfb.**|
| IGL-15 | Back-buffer MSAA | ✍️ | real on OpenGL (the platform GL visual); reported as 1 on Vulkan, whose swap-chain images are single-sample. Still unverified for the back buffer itself, but `igl_msaa_test.cpp` verified the sibling `RenderTarget2D` MSAA path this session and found a real bug: upstream IGL v1.1.1's OpenGL `Framebuffer::copyBytesColorAttachment` reads `colorAttachments[0]`'s own texture via `glReadPixels` with no check for a resolve texture (unlike its Vulkan counterpart, which does check), so reading back a multisampled `RenderTarget2D` failed with `GL_INVALID_OPERATION` -- OpenGL refuses `glReadPixels` against a multisample-attached framebuffer outright. Fixed in `IglRenderTargetRenderer::GetData()` (`IglResources.cpp`): when the target is multisampled, build a small ad-hoc single-attachment framebuffer around the already-resolved `color_` texture (populated by the end-of-pass `igl::StoreAction::MsaaResolve`, see design decision 3) and read from that instead of `framebuffer_` directly — **No dedicated test: back-buffer MSAA comes from the GL visual on OpenGL and is unavailable on Vulkan (IGL swap-chain images are single-sample), so `GetMultiSampleCount()` reports 0 there — the count genuinely in effect. `Igl_Msaa` covers render-target MSAA, which is the part that is real on both.**|
...
| IGL-17 | `TextureCube` / `Texture3D` | 🔶 | Real `igl::TextureType::Cube` / `ThreeD` resources, per-face and per-box uploads. `TextureCube` construction, per-face `SetData` and sampling verified by `Igl_EnvironmentMapEffect`; `Texture3D` construction and per-box `SetData` verified by `Igl_ShaderEffectTexture3D` (sampled through a real custom shader). **Cube readback implemented on 2026-08-18, at every declared mip level.** It had been refused on the grounds that "IGL exposes readback through `IFramebuffer`, not `ITexture`, and a plain `TextureCube` owns no framebuffer" -- true, and not a reason: a framebuffer is a cheap descriptor over an existing image, and `IglRenderTargetRenderer::GetData` was already building a throwaway one over another texture for its MSAA-resolve read. `IglTextureCubeRenderer::GetData` now does the same, selecting the face AND the mip through the range (`newCubeFace(..., level)`), which reaches the attachment on both backends -- OpenGL copies `range.mipLevel` into `AttachmentParams::mipLevel` for `glFramebufferTexture2D`, Vulkan passes it to `getImageData2D` as the copy level. Plain cube textures gained the `Attachment` usage bit, which costs nothing for an otherwise-sampled image. All 49 `TextureCubeTest` cases now pass on IGL, mips included, and the shared table's `CubeLevel0ReadbackSupported()` is an alias of storage again. **Volume readback is NOT implementable at `v1.1.1`, established by attempting it rather than asserted:** IGL cannot attach a 3D texture to a framebuffer at all. `opengl::TextureBufferBase::attach` branches on multisample, stereo and array-layer and otherwise calls `glFramebufferTexture2D`; a volume takes that last branch because `getNumLayers()` counts ARRAY layers, of which a volume has one, and the driver answers `GL_INVALID_OPERATION in glFramebufferTexture2D(invalid textarget GL_TEXTURE_3D)`. The Vulkan side is no better: `ivkGetBufferImageCopy2D` fixes `extent.depth` at 1 and `getVkLayer()` feeds a volume's slice index into `baseArrayLayer`, which is invalid for a 3D image. So there is no slice loop waiting to be written -- nothing underneath it to call. `Texture3DTests.cpp`, the XNB reader suite and `CnjTexture3DTests.cpp` gained a `VolumeReadbackSupported()` arm asserting the refusal instead of the round trip, which is what the previous audit had recorded as "no per-renderer table to extend"; there is one now |
| IGL-18 | Vertex / index buffers | ✅ | lazily created, grown on demand, re-upload flushes the pending frame first — Exercised: `Igl_3D`, `Igl_Instancing` and every indexed draw in the suite.|
| IGL-19 | Dynamic buffer pool | ✅ | 3-frame ring for `SpriteBatch`/`DrawUser*`/uniforms — Exercised: every `SpriteBatch` and user-primitive draw allocates from it; the uniform-chunk cap it needed on Vulkan is IGL-60 Bug B.|
...
| IGL-23 | Back-buffer readback | ✅ | `ReadBackbuffer` through the swap framebuffer, presentation-rect aware, nearest-sampled — Exercised: `Igl_ReadbackOrientation` pins its row order AND its row origin on both backends, which is what IGL-60 Bug C turned out to be.|
| IGL-24 | Render-target orientation | ✅ | design decision 6; `igl_rendertarget_test.cpp` is the discriminating test — Exercised, and it was WRONG until 2026-08-18: the off-screen Y flip was applied on both backends when only OpenGL needs it (IGL-67). `Igl_ReadbackOrientation` now pins the storage order for uploaded and rendered content alike, on both.|

### Phase D — 2D

| ID | Task | State | Notes |
|----|------|-------|-------|
| IGL-25 | `SpriteBatch` geometry | ✅ | rotation, origin, source rect, `SpriteEffects` flips, layer depth in Z — Exercised: `Igl_2D` places an opaque and an alpha-blended sprite and checks the geometry AND the blend arithmetic, 5/5 on both backends.|
| IGL-26 | Batching | ✍️ | flush on texture / sampler / transform / effect change, on `End()`, and per-quad in `SpriteSortMode::Immediate` — **No dedicated test: `Igl_2D` draws two sprites in one batch and both land correctly, but nothing measures the batching itself (draw-call counts are not observable through the public API).**|
| IGL-27 | Sampler and blend state | 🔶 | per-batch `SamplerState`; `BlendState` through the pipeline's own blend attachment — Exercised in part: `BlendState::Opaque` and `NonPremultiplied` are pixel-verified with the blended value checked against the arithmetic (`Igl_2D`), and `SamplerState::PointClamp` throughout. The full preset set, custom factors/equations and the address modes have no test here — see the feature matrix row, which says the same.|
| IGL-28 | `ColorMatrixEffect` | ✍️ | CNA's fixed 2D colour-matrix path, in the uber shader — **No test at all — `ColorMatrixEffect` is a CNAEXT effect with no IGL example.**|
...
| IGL-32 | `BasicEffect` | ✅ | three directional lights, per-vertex and per-pixel, ambient/emissive/specular, `PreferPerPixelLighting` — Exercised: `Igl_3D` renders and depth-sorts two `BasicEffect` quads on both backends, and `Igl_SkinnedEffect`/`Igl_Fog`/`Igl_AlphaTest` drive the same uber-shader through their own paths.|
| IGL-33 | `AlphaTestEffect` | ✅ | XNA's own weight rule, `discard` on a negative weight — Exercised: `Igl_AlphaTestEffect` — the passing quad is visible and the failing one is discarded, both backends.|
| IGL-34 | `DualTextureEffect` | ✅ | overlay × 2 — Exercised: `Igl_DualTextureEffect` — both textures and the diffuse tint reach the shader, both backends.|
...
| IGL-38 | Fog | ✅ | FNA's `fogVector` view-space formulation — Exercised: `Igl_Fog` checks three depths (nearly unfogged, blended midpoint, nearly fully fogged), both backends.|
| IGL-39 | Depth / stencil state | ✅ | full two-sided stencil, standalone `ReferenceStencil` — Exercised: depth compare through two overlapping quads (`Igl_3D`) and a real stencil stamp-then-test with `ReferenceStencil`, `StencilFunction` and `StencilPass`, where the unstamped half genuinely fails (`Igl_Stencil`). Both backends.|
| IGL-40 | Rasterizer state | ✍️ | cull, fill, scissor, depth bias / slope-scale bias — **No dedicated test: `CullNone` is exercised throughout and the front-face winding is derived per target and per backend (IGL-67), but cull-mode and fill-mode outcomes are not pixel-checked, and `WireFrame` is OpenGL-only since IGL does not request Vulkan's `fillModeNonSolid`.**|
| IGL-41 | Clip-space depth | ✅ | `z' = 2z − w` only when `getNormalizedZRange() == NegOneToOne` — Exercised, and it was WRONG on Vulkan until this was found: `getNormalizedZRange()` reports `NegOneToOne` on both backends because neither overrides IGL's base-class default, so the OpenGL-only `z' = 2z - w` remap was applied to Vulkan too and clipped every draw away. `IsVulkanBackend()` is checked directly instead — see IGL-60 Bug B, which `Igl_3D_Vulkan` and `Igl_RenderTarget_Vulkan` pin.|
...
| IGL-43 | Effect parameters | ✅ | Closed 2026-08-18, by deciding the contract rather than patching around it. Loose (non-block) uniforms do not exist in Vulkan GLSL and IGL leaves `bindUniform` unimplemented there, so a parameter has to travel in a std140 block -- and reaching a member of one means knowing its byte offset. **That mapping is DECLARED by the application, and that is a finding rather than a preference:** IGL `v1.1.1` returns an EMPTY reflection on Vulkan (`vulkan::RenderPipelineState` builds a default `RenderPipelineReflection`; its `getIndexByName` is `IGL_DEBUG_ASSERT_NOT_IMPLEMENTED`), so there is no name-to-offset information to be had from the API, and CNA parsing the author's GLSL to guess at it would be a worse contract than asking for it. `ShaderEffect::DeclareUniformBlockEXT(blockSizeBytes, names, offsets, count)` is that ask; it is a no-op on a backend whose uniforms are loose, so the same application code runs on both. CNA packs the recorded parameters into the declared block and binds it at `UniformBufferBinding::CustomEffect` (set 1, binding 2 -- free, since the generated shaders declare only 0 and 1). A parameter that is set but has no member in the declaration **fails by name**: dropping it would draw a zero where the caller set a value, which is the exact class of bug this path exists to prevent. The three custom-effect tests now carry SPIR-V variants of their shaders and pass on Vulkan, registered as `Igl_ShaderEffectTexture3D_Vulkan`, `Igl_SpriteBatchShaderEffect_Vulkan` and `Igl_Instancing_Vulkan`; `Igl_CustomEffectBackend` adds a parameter round trip on both backends with a decoy value overwritten before the draw. One asymmetry is real and documented rather than hidden: an int uniform naming a texture UNIT is an OpenGL idea, and on a SPIR-V target the sampler is bound by its own `layout(set, binding)` -- so `SetUniformInt("SomeSampler", 0)` is OpenGL-only |
...
| IGL-53 | Feature-matrix row | ✅ | Added 2026-08-18. Held open deliberately until the bar the other columns are held to was actually met, and until the two things that would have made a single column dishonest were resolved: a `SpriteBatch` pixel test failing on Vulkan that nobody had root-caused (IGL-60), and a custom `ShaderEffect` taking the process down there (IGL-68/IGL-70). Both are closed, so `docs/graphics-renderer-feature-matrix.md` now carries an `IGL` column across all five per-renderer tables -- 21 rows, every one backed by a named test rather than by intent. **An IGL cell describes BOTH backends unless it says otherwise**, which is the real difficulty of giving this renderer one column: IGL is itself an abstraction over OpenGL and Vulkan, so a claim that holds on only one of them is written out as such rather than averaged. 🟨 is used strictly for "implemented and partly exercised, but the row as written is broader than what was measured" -- `SpriteFont` and `TextureAddressMode::Wrap`/`Mirror` are ⬜ because no IGL test touches them, and occlusion queries are ❌ against the pinned source, not merely unattempted |
...
| IGL-71 | `Rg32` and `Single` promoted to the public texture API | ✅ | IGL-65 verified that this renderer can STORE a dozen non-`Color` formats and deliberately promoted none of them, answering `Defer` so the framework's `Color`-only rule stayed in charge. That was the right call and it named its own condition: promotion promises the typed `SetData`/`GetData` overloads, sampling, render-target use and the framework's four-byte transfer rule, of which storage is one third. `Igl_PublicSurfaceFormat` supplies the other two thirds through the PUBLIC API only -- `Texture2D.SetData` via each format's own typed overload, the texture sampled by a real `SpriteBatch` draw and checked against the colour its channel semantics imply, `GetData` back, and a `RenderTarget2D` in that format rendered into -- with `Color` running the same checks as a control. Both formats pass 9/9 on both backends, so `ClassifySurfaceFormatEXT` and `ClassifyRenderTargetFormatEXT` now answer `Supported` for them. **Chosen on a stated rule, not on appetite:** a promoted format must be a real XNA 4.0 member, must be stored natively by IGL with no substitution, and must have a texel that is a multiple of four bytes -- which is why `ByteEXT` (1), `UShortEXT` (2) and `HalfSingle` (2) stay deferred despite being perfectly storable: promoting them would satisfy this gate and then break `Texture::ValidateGetDataFormat`'s own rule one layer up. The gate is mutation-tested: un-promoting `Rg32` makes the test fail at the `Texture2D` constructor, by name |
| IGL-72 | A `ShaderEffect` author had no supported way to ask which dialect to write | ✅ | A `ShaderEffect` has always been renderer-specific source text, and the framework offered no way to ask which dialect to supply -- so the only route was to infer one from the build's renderer identity. That is wrong twice: in a multi-renderer build the identity is not the active renderer, and a renderer that chooses its native API per process (IGL, LLGL, Diligent) has no per-build answer at all. `GraphicsDevice::GetShaderDialectEXT()` answers at runtime, from the renderer, via a new `IGraphicsRenderer::GetShaderDialectEXT()` whose default is `Unknown` -- which means "this renderer has not stated one, do not guess", not "no shaders". Renderer-neutral on purpose: nothing about it is IGL-specific, and every renderer with a custom-shader path can declare its own. IGL answers `GlslVulkan` or `GlslDesktop` from the backend the process actually resolved |
