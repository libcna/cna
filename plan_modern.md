...
|---|---|---|---|
| MOD-1 | Create `modules/graphics-ext/include/CNA/Graphics/CNAEXT.hpp` master include (→ N05, §7) | ⬜ | Includes every current `CNA/Graphics/*.hpp`; a TU that includes only this header compiles with `-DCNA_CNAEXT=ON` and is empty (zero symbols) with it OFF. |
| MOD-2 | Add a `cnaext` CMake preset (`cmake-build-cnaext`, `OPENGLES3`, `CNA_CNAEXT=ON`, tests ON) | ⬜ | `cmake --preset cnaext` configures from a clean tree; documented in §11 of CNAEXT.md and in `docs/cnaext-engine-layer.md`. |
| MOD-3 | Verify a clean `CNA_CNAEXT=OFF` build is unaffected by the whole module (guard audit) | ⬜ | A script (`scripts/check_cnaext_guards.sh`) asserts every `modules/graphics-ext/{src,include}` file opens with `#ifdef CNA_CNAEXT` and closes with the matching `#endif // CNA_CNAEXT`; passes on the current tree and is run by `MOD-1740`. |
| MOD-4 | `docs/cnaext-engine-layer.md` skeleton — capability boundary, layer diagram, per-renderer support matrix stub | ⬜ | Doc exists, linked from `docs/README.md` and CLAUDE.md's renderer section; every later phase appends its own section instead of creating new files. |
...
| MOD-14 | `NEXT_modern.md` — running "what to pick up next" ledger for this plan | ⬜ | File exists, lists the current critical-path task and the last 5 completed; updated by every task commit (same rule as `NEXT_skia.md`). |
...
| MOD-104 | Renderer virtual `SupportsRenderTargetFormat(int surfaceFormat)` with a default that accepts only `Color` | ⬜ | Default preserves today's behavior for all 44 renderers; overridden per renderer in Phase 16. |
...
| MOD-1710 | Baseline capture: record today's full-suite result before Phase 1 lands | ⬜ | Numbers recorded in `NEXT_modern.md` so "no regressions" is checkable, not asserted. |
