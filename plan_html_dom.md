...
| HTMLDOM-84 | ⬜ | Colour tint (RGB × colour, the per-pixel maths in `CNA_HtmlDom_InstallTextureHelpers`'s `cnaDomGetVariant`) has never been checked against actual rendered pixels — every browser check so far used `Color::White` (tint = identity) or only checked alpha→opacity. Add a pixel-exact check: draw a known-colour opaque texel under a known tint, read back via a render target, assert the exact expected product. |
| HTMLDOM-85 | ⬜ | **The highest-risk item on this list.** `AlphaBlend`'s un-premultiply pass (`mode === 1` in `cnaDomGetVariant`) is the *exact* category of bug (divide/multiply-by-alpha algebra) that `CANVAS`'s own external review found genuinely wrong in the sibling backend — the logic here was written by the same reasoning, never independently pixel-verified. Add a check using genuinely premultiplied source data (RGB already scaled by alpha, mirroring `SDL_RENDERER`'s own Task 697 pixel test) under `BlendState.AlphaBlend`, read back, assert the un-premultiplied result composites correctly with no dark/light fringing. |
| HTMLDOM-86 | ⬜ | `Opaque`'s alpha-stripping (`mode === 2`, forces alpha to 255) has only ever been exercised with source data that was *already* fully opaque, so the strip was a no-op every time it ran. Add a check with genuinely semi-transparent source pixels under `BlendState.Opaque`, read back, assert alpha reads back as 255 and RGB is unchanged from the source (not blended with whatever was under it). |
| HTMLDOM-87 | ⬜ | `Additive` → `mix-blend-mode: plus-lighter` has only been checked for CSS-property presence (`JsSpriteBackgroundRepeat`-style string check), never that the composited *colour* is actually additive. Add a check: two overlapping opaque colours under `BlendState.Additive` on a render target, read back the overlap pixel, assert it equals the channel-wise sum (clamped to 255), not a plain overwrite or average. |
| HTMLDOM-88 | ⬜ | `SpriteFont` coverage is a single glyph (`'A'`) with no kerning variation, no `\n`, no rotation/scale/flip on `DrawString`. Extend the demo/smoke-test font to 3+ glyphs with distinct kerning triples, draw a two-line string (exercising `\n`'s line-spacing advance), and call the rotation/scale/flip `DrawString` overload — assert glyph *placement*, not just that elements appeared. |
