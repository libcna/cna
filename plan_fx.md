...
| FX-061 | Implement and gate SDL_GPU through the MojoShader SDL adapter | FX-030, FX-060 | Full shared suite and native lifecycle tests pass before capability becomes true. **Feasibility established and the build decoupled; the renderer implementation itself is not started.** Two existence gates now stand behind it: `cna_mojoshader_effect_probe` proves MojoShader parses committed effects with no FNA3D linked, and `cna_mojoshader_sdlgpu_probe` proves its SDL_GPU adapter compiles and binds those effects' shader pairs against a device this machine creates -- reporting `SDL_GPU_SHADERFORMAT_SPIRV`, and for `CnaConformanceEffect.fxb` the exact uniform and sampler counts its `.fx` source implies (P0's pixel shader 3 uniforms and 1 sampler; the flat pixel shader 1 uniform and no sampler). The adapter owns translation, shader creation, program linking and uniform-buffer mapping; what stays CNA's is pipeline creation, render-state mapping and texture/sampler binding -- the same split FNA3D's SDL driver uses. The build side is done: `cna_configure_mojoshader()` is separable from `cna_configure_fna3d()`, FNA3D's targets are `EXCLUDE_FROM_ALL`, and an `SDL_GPU` configure with `-DCNA_SDL_GPU_COMPILED_EFFECTS=ON` builds `libmojoshader.a` and no `libFNA3D.a` at all. Off by default, because it pulls a fetched dependency into a renderer that does not otherwise need one |
...
| SDL_GPU | SPIR-V profile plus the `mojoshader_sdlgpu.c` adapter. No longer a paper pairing: `cna_mojoshader_sdlgpu_probe` links and binds the committed fixtures' shader pairs on this machine's Vulkan-backed SDL_GPU device, with only MojoShader and SDL3 linked | `FX-061` |
