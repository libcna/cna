...
|---|---|---|
| Vulkan (`CNA_VULKAN_COMPILED_EFFECTS=ON`) | **true** | `FX-065`: CNA's own MojoShader SPIR-V backend, with a pipeline built per linked shader pair and vertex layout, the profile's four fixed descriptor sets, and per-frame uniform rings for a `Present()`-deferred replay. Refuses by name: vertex-stage sampling (`FX-109`), multi-stream vertex input, instancing, and `Texture3D` on a pixel sampler (`FX-110`) |
