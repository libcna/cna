...
- additional renderer implementations (`FX-061`–`FX-071`). SDL_GPU (`FX-061`, `FX-071`) and
  ...
  `Effect`/`GraphicsDevice` API, golden-pixel tests, `SupportsCompiledEffects()` true. Both needed a
  repair pass afterwards (`FX-080`-`FX-090`, see the section above this one) before that claim was
  true of *every* draw route rather than the two each had implemented. Still open:
...
|---|---|---|
| Vulkan (`CNA_VULKAN_COMPILED_EFFECTS=ON`) | **false, deliberately** | `FX-065`: the runtime exists and is verified against a real device, but the draw route does not, and a true capability would mean a compiled draw silently renders with a stock shader |
| FNA3D | true | Full shared suite including every draw section; multi-stream and instancing actually run (it advertises both); FNA reflection/state oracles; fuzz and sanitizer gates |
| SDL_GPU (`CNA_SDL_GPU_COMPILED_EFFECTS=ON`) | true | Full shared suite; multi-stream and instancing skip because the renderer advertises neither capability, so `GraphicsDevice` refuses those draws for compiled and stock effects alike |
| EasyGL family (`CNA_EASYGL_COMPILED_EFFECTS=ON`) | true | Full shared suite including multi-stream and instancing |
...
- DirectX 11 (`FX-063`), Metal (`FX-066`) and DirectX 9 (`FX-070`) are unwritten, and none of the
  three can be verified on this Linux machine at all.
- Vulkan (`FX-065`) has its runtime, verified against a real device, but not its draw route -- so its
  capability is still false. It is the one remaining renderer that CAN be finished and verified here.
...
- (closed 2026-08-18) `EffectParameter`'s numeric accessors now reject an object parameter (`FX-105`).
...
| A compiled sampler bound to a `TextureCube` | **Closed** -- all three sample one | works | works | works | `FX-110` |
| A compiled sampler bound to a `Texture3D` | **Case 1** on EasyGL (pinned MojoShader's GLSL ES output omits the required `sampler3D` precision qualifier, so the effect cannot be created); **Case 2** on SDL_GPU (its `Texture3D` has no lifetime-tracked native handle for the deferred replay) | works | refused, reason recorded | refused, reason recorded | `FX-110` |
| A texture whose dimension does not match the declared sampler | **Closed** -- refused on all three; it used to sample black silently | refused at assignment | refused at draw | refused at draw | `FX-110` |
