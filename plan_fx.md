...
- eight upstream crash classes found by that campaign and fixed in the managed MojoShader patch --
  ...
  unchecked shader-array selector index -- with the remaining preshader-interpreter exposure
  recorded rather than hidden (`FX-050`, `FX-051`);
...
| FX-051 | Build a libFuzzer/AFL-compatible constructor/reflection/clone harness with the fixture corpus | FX-050 | Harness, corpus and a deterministic mutation campaign **done** (`tools/graphics/compiled_effect_fuzzer.cpp`, `docs/fx-bytecode-fuzzing.md`). The campaign found eight crash classes, all in pinned MojoShader and none in CNA, now fixed by the managed patch; it reaches ~4,000 iterations before an out-of-bounds read inside MojoShader's preshader interpreter, which needs its own bounds pass. **Not yet clean**: this row stays open until the campaign runs dry |
