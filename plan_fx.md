...
| SDL_GPU | SPIR-V profile plus the `mojoshader_sdlgpu.c` adapter. Runtime implemented and tested; ordinary 3D draws now have a working route (`FX-071`), but the capability is still false pending SpriteBatch integration, the FX-060 shared suite and a golden-pixel test | `FX-061`, `FX-071` |
