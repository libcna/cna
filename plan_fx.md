...
| FX-061 | Implement and gate SDL_GPU through the MojoShader SDL adapter | FX-030, FX-060 | **Runtime done and tested; the capability stays false, deliberately.** Everything up to the draw is implemented: `cna_configure_mojoshader()` separates the dependency from FNA3D, the renderer-neutral translation moved to `modules/renderers/common/mojoshader` and is shared with FNA3D, and `SdlGpuCompiledEffect` creates, clones, reflects, selects techniques and passes, translates render and sampler states, and validates parameter and texture assignment against MojoShader's own SDL_GPU adapter. Two existence gates plus ten dedicated tests cover it, and one of those tests caught a real crash: several MojoShader parse failures are static sentinels rather than allocations, and deleting one walks static storage -- now guarded in the shared module for every backend. What is missing is the draw route, and it is not small: this renderer dispatches to one of eight built-in shader variants by vertex stride, each with a hand-written vertex layout, and there is no generic path from an XNA `VertexDeclaration` to SDL_GPU vertex attributes, which is exactly what an arbitrary compiled effect needs. `SupportsCompiledEffects()` therefore still reports false, because a true there would mean a game silently drew with a stock shader. Tracked as `FX-071` |
...
| FX-071 | Give the SDL_GPU renderer a compiled-effect draw route | FX-061 | Draws execute the applied pass's own shaders, and the FX-060 shared suite plus a golden-pixel test pass before `SupportsCompiledEffects()` becomes true. Needs three things this renderer does not have: a generic `VertexDeclaration`-to-`SDL_GPUVertexAttribute` builder matched against MojoShader's reported vertex attributes, a pipeline cache keyed on the effect's shader pair together with the declaration and the current state, and uniform upload through `MOJOSHADER_sdlMapUniformBufferMemory`. Each of the eight existing draw routes hard-codes its own layout, so none of it is reusable as it stands |
...
| SDL_GPU | SPIR-V profile plus the `mojoshader_sdlgpu.c` adapter. Runtime implemented and tested; the capability is still false because the draw route does not exist yet | `FX-061`, `FX-071` |
