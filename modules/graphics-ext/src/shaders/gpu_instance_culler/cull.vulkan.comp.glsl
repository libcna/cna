...
layout(std430, set = 0, binding = 0) readonly buffer CnaInstances
{
    CnaInstance cnaInstances[];
};
layout(std430, set = 0, binding = 1) writeonly buffer CnaVisible
{
    mat4 cnaVisible[];
};
layout(std430, set = 0, binding = 2) readonly buffer CnaPlanes
{
    vec4 cnaPlanes[];
};
layout(std430, set = 0, binding = 3) buffer CnaCommand
{
    uint cnaCommand[];
};
