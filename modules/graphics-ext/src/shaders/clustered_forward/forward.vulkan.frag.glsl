...
layout(set = 1, binding = 0) uniform sampler2D uCnaAreaBrdf;
layout(set = 1, binding = 1) uniform sampler2D uOpaqueFrame;
layout(set = 1, binding = 12, std140) uniform FloatArray
{
    float uClusterScalars[72];
};
layout(set = 1, binding = 14, std140) uniform Vec3Array
{
    vec3 uClusterVectors[72];
};
...
layout(set = 2, binding = 6, std430) readonly buffer CnaClusteredLights
{
    vec4 uCnaLights[];
};
layout(set = 2, binding = 7, std430) readonly buffer CnaClusterTable
{
    uvec2 uCnaClusters[];
};
layout(set = 2, binding = 8, std430) readonly buffer CnaLightIndices
{
    uint uCnaIndices[];
};

#define uProbeCoefficients uClusterVectors
#define uCameraPosition uClusterVectors[9]
#define uBaseColor uClusterVectors[10]
#define uAmbient uClusterVectors[11]
#define uSheenColor uClusterVectors[12]
#define uSubsurfaceColor uClusterVectors[13]
#define uAttenuationColor uClusterVectors[14]
#define uAreaPosition uClusterVectors[15]
#define uAreaRight uClusterVectors[16]
#define uAreaUp uClusterVectors[17]
#define uAreaColour uClusterVectors[18]

#define uHasProbe uClusterScalars[0]
#define uMetallic uClusterScalars[1]
#define uRoughness uClusterScalars[2]
#define uClearcoat uClusterScalars[3]
#define uClearcoatRoughness uClusterScalars[4]
#define uSheenRoughness uClusterScalars[5]
#define uSubsurfaceWrap uClusterScalars[6]
#define uIridescence uClusterScalars[7]
#define uIridescenceIor uClusterScalars[8]
#define uIridescenceThickness uClusterScalars[9]
#define uTransmission uClusterScalars[10]
#define uIor uClusterScalars[11]
#define uThickness uClusterScalars[12]
#define uAttenuationDistance uClusterScalars[13]
#define uAreaShape int(uClusterScalars[14])
#define uCnaAreaBrdfSize uClusterScalars[15]
#define uAreaRange uClusterScalars[16]
#define uAreaTwoSided uClusterScalars[17]
#define uCnaTilesX int(uClusterScalars[18])
#define uCnaTilesY int(uClusterScalars[19])
#define uCnaSliceCount int(uClusterScalars[20])
#define uCnaLightCount int(uClusterScalars[21])
#define uCnaGridNear uClusterScalars[22]
#define uCnaGridFar uClusterScalars[23]
#define uViewProjection uClusterMatrices[3]
...
CnaClusteredLight cnaLoadLight(int index)
{
    ...
    light.position = uCnaLights[base].xyz;
    light.range = uCnaLights[base].w;
    light.colour = uCnaLights[base + 1].xyz;
    light.isSpot = uCnaLights[base + 1].w;
    light.direction = uCnaLights[base + 2].xyz;
    light.cosOuter = uCnaLights[base + 2].w;
    light.cosInner = uCnaLights[base + 3].x;
    return light;
}
...
{
    return int(uCnaClusters[cluster].y);
}

int cnaClusterLightIndex(int cluster, int i)
{
    return int(uCnaIndices[uCnaClusters[cluster].x + uint(i)]);
}
...
vec3 cnaProbeIrradiance(vec3 coefficients[72], vec3 normal) {
