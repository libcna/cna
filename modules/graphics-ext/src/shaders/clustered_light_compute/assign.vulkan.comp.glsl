...
layout(std430, set = 0, binding = 0) readonly buffer CnaLights { vec4 uLights[]; };
layout(std430, set = 0, binding = 1) readonly buffer CnaMatrix { float uInverseProjection[]; };
layout(std430, set = 0, binding = 2) buffer CnaCounts { int uCounts[]; };
layout(std430, set = 0, binding = 3) buffer CnaIndices { int uIndices[]; };
layout(std140, set = 0, binding = 4) uniform ClusteredLightParameters
{
    ivec4 uGrid;
    ivec4 uOutput;
    vec4 uDepth;
