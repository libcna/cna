...
namespace CNA::Graphics {
    ...
    namespace {
        ...
        /// The indirect command is five words. Word five carries the offered count to the compute
        /// shader, keeping the package independent of renderer-specific scalar-uniform transport;
        /// two final words preserve the original eight-word alignment.
        ...
        [[nodiscard]] ShaderPackageEXT CreateCullShaderPackage()
        {
            using namespace detail::GpuInstanceCullerGenerated;
            return ShaderPackageEXT(
                {
                    ShaderCodeEXT(CNA::ShaderLanguageEXT::GlslEs,
                                  CNA::ShaderStageEXT::Compute, "main",
                                  "gpu_instance_culler/cull.es.comp.glsl",
                                  std::string(kCullEsComputeSource)),
                    ShaderCodeEXT(CNA::ShaderLanguageEXT::GlslDesktop,
                                  CNA::ShaderStageEXT::Compute, "main",
                                  "gpu_instance_culler/cull.desktop.comp.glsl",
                                  std::string(kCullDesktopComputeSource)),
                ...
                {CNA::ShaderStageEXT::Compute},
                {
                    ShaderBindingRequirementEXT(
                        "CnaInstances", kInstancesBinding,
                        ShaderBindingTypeEXT::StorageBuffer, CNA::ShaderStageEXT::Compute),
                    ShaderBindingRequirementEXT(
                        "CnaVisible", kVisibleBinding,
                        ShaderBindingTypeEXT::StorageBuffer, CNA::ShaderStageEXT::Compute),
                    ShaderBindingRequirementEXT(
                        "CnaPlanes", kPlanesBinding,
                        ShaderBindingTypeEXT::StorageBuffer, CNA::ShaderStageEXT::Compute),
                    ShaderBindingRequirementEXT(
                        "CnaCommand", kCommandBinding,
                        ShaderBindingTypeEXT::StorageBuffer, CNA::ShaderStageEXT::Compute),
                });
        }
        ...
        : device_(device)
    {
        if (!device.SupportsCapability(CNA::GraphicsCapability::ComputeShaders))
            ...
            unsupportedReason_ = "this renderer cannot execute a custom draw effect";
        ...
        const ShaderPackageEXT package = CreateCullShaderPackage();
        if (!package.selectFor(device).isUsable())
        {
            unsupportedReason_ = "this renderer has no usable portable culling shader variant";
            return;
        }
        try
        {
            program_ = std::make_unique<ComputeShader>(device, package);
        }
        catch (const std::exception& error)
        {
            unsupportedReason_ =
                "the portable culling program did not compile on this device: "
                                 ...
                                 const int indexCount, const int firstIndex, const int baseVertex)
    {
        ...
        command[4] = 0;
        command[5] = static_cast<std::uint32_t>(instanceCount_);
