...
// from level 0 at all.
//
// THE QUAD IS MAGNIFIED, which for THIS state is fine and worth saying explicitly: `MaxMipLevel`
// becomes `lodMinClamp`, a clamp applied to the computed level of detail whatever that computation
// produced, so it moves the selected level even while magnifying. A LOD *bias* does not -- see
// `parity_sampler_lod_bias`, whose texture has to be sized to its quad for exactly that reason.
//
