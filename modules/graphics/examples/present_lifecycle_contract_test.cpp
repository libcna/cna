...
#include <sys/wait.h>
#include <unistd.h>
#define CNA_GFX180_CAN_ISOLATE 1
#elif defined(_WIN32)
#include "common/WindowsProcessIsolation.hpp"
#define CNA_GFX180_CAN_ISOLATE 1
#else
#define CNA_GFX180_CAN_ISOLATE 0
#endif
    ...
    bool RunLegIsolated(const char* exePath, const LegSpec& leg, bool& skipped)
    {
        ...
        if (child.outcome != CNA::Examples::WindowsChildOutcome::Exited)
        {
            std::printf("[FAIL] supervisor: Win32 child operation failed for leg %s (error %lu)\n",
                        leg.id, static_cast<unsigned long>(child.systemError));
            std::fflush(stdout);
            return false;
        }
        if (leg.expectAbort)
        {
            const bool wanted = child.exitCode == 3;
            std::printf("[%s] leg %s: exited %lu%s\n", wanted ? "PASS" : "FAIL", leg.id,
                        static_cast<unsigned long>(child.exitCode),
                        wanted ? " -- the measured MinGW unhandled-exception termination"
                               : " -- expected MinGW termination code 3");
            std::fflush(stdout);
            return wanted;
        }
        ...
        std::printf("[%s] leg %s: exited %lu\n",
                    CNA::Examples::IsWindowsAbnormalExit(child.exitCode) ? "CRASH" : "FAIL",
                    leg.id, static_cast<unsigned long>(child.exitCode));
        std::fflush(stdout);
        return false;
#endif
    }
