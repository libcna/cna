...
protected:
    ...
    // GTEST_SKIP() only unwinds the function it is called from; called from an ordinary member
    // function like RequireInstancedRendering() below it cannot skip the test body that invokes
    // it. SetUp() is where GoogleTest itself checks for a skip, so the capability gate has to run
    // here too -- RequireInstancedRendering() keeps its own copy for the state-setup calls that
    // follow it, which only run once SetUp() has already let the test proceed.
    void SetUp() override
    {
