...
// plan_dx1.md: real DirectX 1 (DirectDraw v1) graphics renderer. 2D-only, like SDL_RENDERER/DX3 --
...
// a Dx1Renderer pointer. All real <ddraw.h> usage lives in Dx1Renderer.cpp behind the
// Impl pimpl below.
...
{
    /**
     * DX1 graphics renderer (plan_dx1.md): a CPU 2D compositor that uses a REAL
     ...
     * DDSCAPS_OFFSCREENPLAIN surfaces (Dx1TextureRenderer/Dx1RenderTargetRenderer, defined entirely in
     * Dx1Renderer.cpp -- neither is ever named outside it), and SetRenderTarget2D() redirects
     * Clear()/ReadBackbuffer() to whichever surface is currently bound. The SpriteBatch CPU
     * compositor (Phase O4, Dx1SpriteBatchRenderer) is ported verbatim from DX3's own
     ...
     */
    class Dx1Renderer final : public IGraphicsRenderer
    {
    public:
        explicit Dx1Renderer(const GraphicsRendererCreateArgs& args);
        ~Dx1Renderer() override;

        Dx1Renderer(const Dx1Renderer&) = delete;
        Dx1Renderer& operator=(const Dx1Renderer&) = delete;
