...
// this .cpp only -- see Dx1Renderer.hpp's own comment. plan_dx1.md section 1's discipline:
...
namespace CNA::Internal::Renderers::Dx1
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx1TextureRenderer and
        // Dx1RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
    ...
    // Common accessor so Dx1SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx1TextureRenderer/Dx1RenderTargetRenderer themselves.
    ...
    struct Dx1Renderer::Impl
    {
        ...
        // Phase O3: the offscreen surface owned by the currently-bound Dx1RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx1RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE rather than
        // a Dx1RenderTargetRenderer* so Dx1RenderTargetRenderer never needs to name this private Impl
    ...
    Dx1Renderer::Dx1Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx1Renderer initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx1Renderer: could not obtain a real HWND from the SDL window.");
    ...
    Dx1Renderer::~Dx1Renderer() = default;

    void Dx1Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx1Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx1Renderer::Present: ClientToScreen failed");
    ...
    void Dx1Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx1Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx1Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx1Renderer::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx1Renderer::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx1Renderer::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx1Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    class Dx1TextureRenderer : public ITextureRenderer, public Dx1SurfaceOwner
    {
    public:
        Dx1TextureRenderer(LPDIRECTDRAW dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx1TextureRenderer(LPDIRECTDRAW dd, const ImageData& data)
            : Dx1TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx1TextureRenderer() override { if (surface_) surface_->Release(); }

        Dx1TextureRenderer(const Dx1TextureRenderer&) = delete;
        Dx1TextureRenderer& operator=(const Dx1TextureRenderer&) = delete;
    ...
    class Dx1RenderTargetRenderer final : public IRenderTargetRenderer, public Dx1SurfaceOwner
    {
    public:
        Dx1RenderTargetRenderer(LPDIRECTDRAWSURFACE* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx1RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx1RenderTargetRenderer(const Dx1RenderTargetRenderer&) = delete;
        Dx1RenderTargetRenderer& operator=(const Dx1RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> Dx1Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx1TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> Dx1Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx1RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx1Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void Dx1Renderer::SetRenderTargets(
    ...
    // Never named outside this .cpp, same reasoning as Dx1TextureRenderer/Dx1RenderTargetRenderer.

    class Dx1SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        Dx1SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE()> getActiveSurface,
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx1SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx1SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx1SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx1SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx1SpriteBatchRenderer::Draw: texture renderer is not a DX1 surface (created by "
    ...
    std::unique_ptr<ISpriteBatchRenderer> Dx1Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx1SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx1Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void Dx1Renderer::ClearColorAndDepth(float, float, float, float, float) { ThrowNo3D("ClearColorAndDepth"); }
    void Dx1Renderer::ClearDepth(float) { ThrowNo3D("ClearDepth"); }
    void Dx1Renderer::ClearStencil(int) { ThrowNo3D("ClearStencil"); }
    void Dx1Renderer::ClearDepthAndStencil(float, int) { ThrowNo3D("ClearDepthAndStencil"); }
    void Dx1Renderer::ClearColorAndStencil(float, float, float, float, int) { ThrowNo3D("ClearColorAndStencil"); }
    void Dx1Renderer::ClearColorDepthAndStencil(float, float, float, float, float, int) { ThrowNo3D("ClearColorDepthAndStencil"); }
    void Dx1Renderer::SetDepthTestEnabled(bool)  { ThrowNo3D("SetDepthTestEnabled"); }
    void Dx1Renderer::SetBlendEnabled(bool)      { ThrowNo3D("SetBlendEnabled"); }
    void Dx1Renderer::SetDepthWriteEnabled(bool) { ThrowNo3D("SetDepthWriteEnabled"); }

    std::unique_ptr<IVertexBufferRenderer> Dx1Renderer::CreateVertexBuffer(int)
    {
        ThrowNo3D("CreateVertexBuffer");
    }

    std::unique_ptr<IIndexBufferRenderer> Dx1Renderer::CreateIndexBuffer16(int)
    {
        ThrowNo3D("CreateIndexBuffer16");
    }

    void Dx1Renderer::DrawColoredPrimitives(const IVertexBufferRenderer&,
                                                   const Matrix&, const Matrix&, const Matrix&,
                                                   PrimitiveType, int) { ThrowNo3D("DrawColoredPrimitives"); }

    void Dx1Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer&,
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<Dx1::Dx1Renderer>(args);
    }
#endif
}
