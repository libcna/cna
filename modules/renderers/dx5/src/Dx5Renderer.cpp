...
// <windows.h> it pulls in) is contained to this .cpp only -- see Dx5Renderer.hpp's own
...
// IDirect3D3/IDirect3DDevice3 are not forbidden -- this renderer's 3D layer (a port of DX3's own
...
namespace CNA::Internal::Renderers::Dx5
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx5TextureRenderer and
        // Dx5RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
    ...
    // Common accessor so Dx5SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx5TextureRenderer/Dx5RenderTargetRenderer themselves.
    ...
    struct Dx5Renderer::Impl
    {
        ...
        // Phase O3: the offscreen surface owned by the currently-bound Dx5RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx5RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE4 rather than
        // a Dx5RenderTargetRenderer* so Dx5RenderTargetRenderer never needs to name this private Impl
    ...
    Dx5Renderer::Dx5Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx5Renderer initialized with null window.");
        impl_->window = args.window;
        impl_->presentationMode = args.presentationMode;

        // Design decision 3: a real Win32 HWND, obtained the same way D3D9Renderer.cpp does
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx5Renderer: could not obtain a real HWND from the SDL window.");
    ...
    Dx5Renderer::~Dx5Renderer() = default;

    void Dx5Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx5Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx5Renderer::Present: ClientToScreen failed");
    ...
    void Dx5Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx5Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx5Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx5Renderer::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx5Renderer::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx5Renderer::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx5Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    // DX6 anyway, per docs/directx-legacy-renderers-analysis.md section 3.1's table).

    class Dx5VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit Dx5VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx5VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx5VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class Dx5IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        Dx5IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx5IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx5IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    /// (not cached) rather than stored on Dx5TextureRenderer: a handle is only valid for the device
    ...
    D3DTEXTUREHANDLE Dx5ResolveTextureHandle(const ITextureRenderer* texture, LPDIRECT3DDEVICE3 device3)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const Dx5SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx5Renderer: texture0 is not a DX5 surface (created by a different graphics renderer?)");
                                   ...
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx5Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx5Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx5Renderer::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class Dx5TextureRenderer : public ITextureRenderer, public Dx5SurfaceOwner
    {
    public:
        Dx5TextureRenderer(LPDIRECTDRAW4 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx5TextureRenderer(LPDIRECTDRAW4 dd, const ImageData& data)
            : Dx5TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx5TextureRenderer() override { if (surface_) surface_->Release(); }

        Dx5TextureRenderer(const Dx5TextureRenderer&) = delete;
        Dx5TextureRenderer& operator=(const Dx5TextureRenderer&) = delete;
    ...
    class Dx5RenderTargetRenderer final : public IRenderTargetRenderer, public Dx5SurfaceOwner
    {
    public:
        Dx5RenderTargetRenderer(LPDIRECTDRAWSURFACE4* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx5RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx5RenderTargetRenderer(const Dx5RenderTargetRenderer&) = delete;
        Dx5RenderTargetRenderer& operator=(const Dx5RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> Dx5Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx5TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> Dx5Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx5RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx5Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void Dx5Renderer::SetRenderTargets(
    ...
    // Never named outside this .cpp, same reasoning as Dx5TextureRenderer/Dx5RenderTargetRenderer.

    class Dx5SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        Dx5SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE4()> getActiveSurface,
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx5SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx5SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx5SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx5SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx5SpriteBatchRenderer::Draw: texture renderer is not a DX5 surface (created by "
            ...
            // origin, then scale) reuses the exact formula SoftwareSpriteBatchRenderer::Draw()/DX3
    ...
    std::unique_ptr<ISpriteBatchRenderer> Dx5Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx5SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx5Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void Dx5Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    void Dx5Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
    ...
    void Dx5Renderer::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
    ...
    void Dx5Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }
    ...
    void Dx5Renderer::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void Dx5Renderer::ClearStencil(int) {}
    void Dx5Renderer::ClearDepthAndStencil(float depth, int) { ClearDepth(depth); }
    void Dx5Renderer::ClearColorAndStencil(float r, float g, float b, float a, int) { Clear(r, g, b, a); }
    void Dx5Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int)
    {
        ClearColorAndDepth(r, g, b, a, depth);
    }
    void Dx5Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void Dx5Renderer::SetBlendEnabled(bool) {}

    void Dx5Renderer::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferRenderer> Dx5Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx5VertexBufferRenderer>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx5Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx5IndexBufferRenderer>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx5Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx5IndexBufferRenderer>(index_capacity, true);
    }

    void Dx5Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   ...
                                                   PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx5Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx5VertexBufferRenderer&>(vb);
    ...
    void Dx5Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          ...
                                                          PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx5Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx5VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const Dx5IndexBufferRenderer&>(ib);
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DX5", route);
    }

    void Dx5Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        ...
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx5Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx5Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx5VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx5Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
    ...
    void Dx5Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        ...
        Dx5CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx5Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx5Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx5Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<Dx5::Dx5Renderer>(args);
    }
#endif
}
