...
// with no seam aliasing) into a mipMap=true 64x64 RenderTarget2D via SpriteBatch, unbind (triggers
// RecordAndSubmitFrame()'s new GenerateMips() call), then read levels back directly:
//
...
// Check E -- GetData() beyond LevelCount (level 7 on a 7-level target) returns false.
// Check F -- a NON-mipmapped RenderTarget2D still only has level 0 (LevelCount == 1) and
//   GetData(1, ...) on it also returns false -- mip-mapping stays opt-in.
//
...
namespace
{
    ...
    // 1x8: rows 0..6 red, row 7 blue -- point-stretched to fill kRTSize (an integer multiple of 8)
...
class LlglRenderTarget2DMipTest : public CNA::Examples::PixelTestGame
{
public:
    void RunTest() override
    {
        ...
        try
        {
            renderTarget = std::make_unique<RenderTarget2D>(
                device, kRTSize, kRTSize, /*mipMap=*/true, SurfaceFormat::Color, DepthFormat::None);
        }
        catch (const std::exception&)
        {
            renderTarget.reset();
        ...
        if (!ExpectTrue("mipMap=true RenderTarget2D(64,64) construction succeeds with LevelCount=7",
                        renderTarget != nullptr && renderTarget->getLevelCountProperty() == 7))
        {
            return;
        }
        ...
        spriteBatch.Draw(*patternTexture, Rectangle(0, 0, kRTSize, kRTSize), Color::White);
        spriteBatch.End();
        // Unbinding is what triggers RecordAndSubmitFrame()'s GenerateMips() call, once this
        // render pass (and any MSAA resolve, though none is requested here) has finished.
        device.SetRenderTarget(nullptr);

        // --- Check B: level 0, well inside each region -- must stay crisp, unblended. ---
        Color level0Top(0, 0, 0, 0), level0Bottom(0, 0, 0, 0);
        const Rectangle level0TopRegion(kRTSize / 2, 16, 1, 1);
        const Rectangle level0BottomRegion(kRTSize / 2, 60, 1, 1);
        renderTarget->GetData(0, &level0TopRegion, &level0Top, 0, 1);
        renderTarget->GetData(0, &level0BottomRegion, &level0Bottom, 0, 1);
        ...
        try
        {
            ...
            renderTarget->GetData(3, &level3TopRegion, &level3Top, 0, 1);
            renderTarget->GetData(3, &level3BottomRegion, &level3Bottom, 0, 1);
        }
        catch (const std::exception&)
        {
            level3Threw = true;
        ...
        try
        {
            ...
            renderTarget->GetData(6, &coarsestRegion, &coarsest, 0, 1);
        }
        catch (const std::exception&)
        {
            coarsestThrew = true;
        ...
        // --- Check E: a level beyond LevelCount (7 on a 7-level target) is rejected, not a
        // garbage read. ---
        ...
        try
        {
            ...
            renderTarget->GetData(7, &beyondRegion, &beyond, 0, 1);
        }
        catch (const std::exception&)
        {
            beyondThrew = true;
        }
        ExpectTrue("GetData() beyond LevelCount (level 7 on a 7-level target) throws, not a "
                  "garbage read", beyondThrew);

        // --- Check F: mip-mapping stays opt-in -- a plain (mipMap=false, the default)
        // RenderTarget2D still only has one level. ---
        std::unique_ptr<RenderTarget2D> plainTarget;
        try
        {
            plainTarget = std::make_unique<RenderTarget2D>(device, kRTSize, kRTSize);
        }
        catch (const std::exception&)
        {
            plainTarget.reset();
        }
        if (ExpectTrue("mipMap=false RenderTarget2D still has LevelCount=1",
                       plainTarget != nullptr && plainTarget->getLevelCountProperty() == 1))
        {
            ...
            try
            {
                ...
                plainTarget->GetData(1, &plainLevel1Region, &plainLevel1, 0, 1);
            }
            catch (const std::exception&)
            {
                plainLevel1Threw = true;
            }
            ExpectTrue("GetData(1, ...) on a non-mipmapped RenderTarget2D throws",
                      plainLevel1Threw);
        }
    }
};
...
int main()
{
    return CNA::Examples::RunPixelTest<LlglRenderTarget2DMipTest>();
}
