...
{
    /**
     * @brief SDL-windowed retro text/glyph-grid graphics renderer ("ASCII").
     *
     * Not a real terminal/TTY renderer -- see plan_ascii.md. Architecturally a thin decorator
     * around SDL_RENDERER's own SdlRenderer (design decision 2): supported rendering
     * methods forward to a wrapped, fully real SdlRenderer::SdlRenderer instance, which
     ...
     */
    class AsciiRenderer : public IGraphicsRenderer
    {
    public:
        explicit AsciiRenderer(const GraphicsRendererCreateArgs& args);
        ~AsciiRenderer() override = default;

        /// Sets the quantization mode used by Present() (design decision 5). Callable at any
        /// time, including before Game::Run(), same as HeadlessRenderer::SetMode().
        ...
        /// No depth/stencil buffer on this renderer either -- inherited from the same underlying
        ...
        /**
         * @brief Reports that this 2D-only renderer supports none of the enumerated capabilities.
         *
         * REMED-GFX-130. This class inherits `IGraphicsRenderer` directly rather than wrapping
         * `SdlRenderer`'s own override, so it was still answering
         * `IGraphicsRenderer::SupportsCapability`'s `return true` default for everything -- including
         ...
         * capability report honest. Same answer as the wrapped SDL_RENDERER renderer gives.
         *
         * @return Always false.
         */
        ...
        // 3D pipeline: NOT supported, same as the wrapped SDL_RENDERER renderer. Calls throw by
        // default or become warn-once no-ops with safe null-object resources under
        // Unsupported3DGraphicsCallBehavior::WarnAndStub. Forwards to SdlRenderer's own
    ...
    private:
        /// The real, wrapped SDL_RENDERER renderer instance that does all the actual compositing
        /// (design decision 2). Never exposed outside this class.
        std::unique_ptr<SdlRenderer::SdlRenderer> inner_;
        ...
        std::unique_ptr<IRenderTargetRenderer> gameTarget_;
        ...
        std::unique_ptr<ISpriteBatchRenderer> presentSpriteBatch_;
        /// The Phase G2 glyph atlas, uploaded once at construction as a plain renderer texture
        ...
        std::unique_ptr<ITextureRenderer> fontAtlasTexture_;
        int virtualWidth_ = 0;
