...
// plan_dx6.md: real DirectX 6 graphics backend -- DirectDraw v4 (IDirectDraw4/
// IDirectDrawSurface4/DDSURFACEDESC2/DDSCAPS2) + Direct3D v3 (IDirect3D3/IDirect3DDevice3/
// IDirect3DViewport3), the EXACT SAME COM interfaces DX5 already uses -- DX6 introduces no new
// interface revision at all (confirmed by inspection: no IDirect3D4/IDirect3DDevice4 exists).
// DX6's own delta is purely new render states on that same interface: real stencil buffer
...
namespace CNA::Internal::Backends::Dx6
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx6TextureBackend and
        // Dx6RenderTargetBackend (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
        ...
        // Dx6_Smoke/Dx6_Blend/etc.'s Clear()+GetBackBufferData() checks all still passed), but a
        ...
        void DetectChannelLayout(LPDIRECTDRAWSURFACE4 surface)
        {
        ...
        LPDIRECTDRAWSURFACE4 CreateOffscreenSurface(LPDIRECTDRAW4 dd, int width, int height)
        {
            DDSURFACEDESC2 desc{};
            desc.dwSize = sizeof(DDSURFACEDESC2);
            ...
            if (FAILED(hr)) ThrowHr("IDirectDraw4::CreateSurface(offscreen)", hr);
        ...
        void WriteSurfacePixels(LPDIRECTDRAWSURFACE4 surface, int width, int height,
                                const uint8_t* rgba, int stride)
        {
            ...
            if (FAILED(hr)) ThrowHr("IDirectDrawSurface4::Lock(write)", hr);
            ...
            surface->Unlock(nullptr);  // IDirectDrawSurface4::Unlock takes LPRECT, not the locked pointer
        }
        ...
        // is currently active.
        void ReadSurfacePixels(LPDIRECTDRAWSURFACE4 surface, int x, int y, int w, int h, uint8_t* pixels)
        {
            DDSURFACEDESC2 desc{};
            desc.dwSize = sizeof(DDSURFACEDESC2);
            const HRESULT hr = surface->Lock(nullptr, &desc, DDLOCK_WAIT, nullptr);
            if (FAILED(hr)) ThrowHr("IDirectDrawSurface4::Lock(read)", hr);
                              ...
                              uint8_t r, uint8_t g, uint8_t b, uint8_t a)
        {
            ...
            desc.dwSize = sizeof(DDSURFACEDESC2);
            const HRESULT hr = surface->Lock(nullptr, &desc, DDLOCK_WAIT, nullptr);
            if (FAILED(hr)) ThrowHr("IDirectDrawSurface4::Lock(clear)", hr);
        ...
        [[noreturn]] void ThrowMipLevelUnsupported(int level)
        {
            throw std::runtime_error(
                "DX6 (DirectDraw v4) does not support mip-level texture uploads (level " +
                std::to_string(level) + "): IDirectDrawSurface4 has no native mip chain or "
        ...
        enum class Dx6BlendMode { Opaque, AlphaBlend, NonPremultiplied, Additive };
        ...
        // found-and-fixed bug).
        Dx6BlendMode DetectBlendMode(int colorSrc, int alphaSrc, int colorDst, int alphaDst,
                                     int colorFunc, int alphaFunc)
        {
            ...
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 5 && alphaDst == 5) return Dx6BlendMode::NonPremultiplied;
            // Additive: ColorSrc=SourceAlpha, AlphaSrc=SourceAlpha, ColorDst=One, AlphaDst=One
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 0 && alphaDst == 0) return Dx6BlendMode::Additive;
            // Opaque: ColorSrc=One, AlphaSrc=One, ColorDst=Zero, AlphaDst=Zero
            if (bothAdd && colorSrc == 0 && alphaSrc == 0 && colorDst == 1 && alphaDst == 1) return Dx6BlendMode::Opaque;
            ...
            return Dx6BlendMode::AlphaBlend;
        }
        ...
        DWORD Dx6CompareFunctionToD3D(int compareFunction)
        {
        ...
        DWORD Dx6StencilOperationToD3D(int stencilOperation)
        {
        ...
        DWORD Dx6CullModeToD3D(int cullMode)
        {
        ...
        DWORD Dx6FillModeToD3D(int fillMode)
        {
        ...
        DWORD Dx6TextureAddressModeToD3D(int addressMode)
        {
        ...
        struct Dx6FilterPair { DWORD mag; DWORD min; };
        ...
        Dx6FilterPair Dx6TextureFilterToD3D(int textureFilter)
        {
        ...
        struct LockedSurfaceCache
        {
            ...
            void EnsureLocked(LPDIRECTDRAWSURFACE4 s, const char* what)
            {
        ...
        void CompositeQuad(uint8_t* dstBase, int dstPitch, int dstW, int dstH,
                           ...
                           Dx6BlendMode blendMode, int filter, int addressU, int addressV)
        {
            ...
            for (int y = y0; y < y1; ++y)
            {
                for (int x = x0; x < x1; ++x)
                {
                    ...
                    for (const auto& tri : kTriangles)
                    {
                        ...
                        case Dx6BlendMode::NonPremultiplied:
                        {
    ...
    // Common accessor so Dx6SpriteBatchBackend can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx6TextureBackend/Dx6RenderTargetBackend themselves.
    class Dx6SurfaceOwner
    {
    public:
        virtual ~Dx6SurfaceOwner() = default;
        [[nodiscard]] virtual LPDIRECTDRAWSURFACE4 Surface() const = 0;
    };

    struct Dx6GraphicsBackend::Impl
    {
        ...
        // this backend makes, and every surface it creates, goes through v4 from here on
        // (LPDIRECTDRAWSURFACE4/DDSURFACEDESC2/DDSCAPS2 throughout), spike-confirmed (DX5-0a) to
        // behave identically to earlier versions for all of them.
        LPDIRECTDRAW4 dd = nullptr;
        ...
        // This backend's own "shadow backbuffer": a Lockable offscreen surface, sized to the
        ...
        // Phase O3: the offscreen surface owned by the currently-bound Dx6RenderTargetBackend, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx6RenderTargetBackend::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE4 rather than
        // a Dx6RenderTargetBackend* so Dx6RenderTargetBackend never needs to name this private Impl
        // type (it is defined later in this file, after Impl).
        LPDIRECTDRAWSURFACE4 currentTargetSurface = nullptr;
        ...
        Dx6BlendMode currentBlendMode = Dx6BlendMode::AlphaBlend;
        ...
        [[nodiscard]] LPDIRECTDRAWSURFACE4 ActiveSurface() const
        {
        ...
        // Tears down device3/viewport3 (but not d3d3 or zbuffer -- callers that recreate the
        ...
        void Release3DDevice()
        {
            if (viewport3)
            {
                if (device3) device3->DeleteViewport(viewport3);
                viewport3->Release();
                viewport3 = nullptr;
            }
            if (device3) { device3->Release(); device3 = nullptr; }
        }
        ...
        void Create3DDevice(int width, int height)
        {
            ...
            if (!d3d3)
            {
                const HRESULT hr = dd->QueryInterface(IID_IDirect3D3, reinterpret_cast<void**>(&d3d3));
            ...
            // D24S8-equivalent shape) instead of DX5's depth-only 16-bit surface -- spike-confirmed
            // (DX6-0a) real stencil write+test behavior against exactly this format.
            ...
            if (FAILED(hr)) ThrowHr("IDirectDraw4::CreateSurface(z-buffer)", hr);
            ...
            // that nullptr is sufficient here, same as every other CNA backend's COM aggregation.
            hr = d3d3->CreateDevice(IID_IDirect3DRGBDevice, backBuffer, &device3, nullptr);
            if (FAILED(hr)) ThrowHr("IDirect3D3::CreateDevice(IID_IDirect3DRGBDevice)", hr);

            hr = d3d3->CreateViewport(&viewport3, nullptr);
            if (FAILED(hr)) ThrowHr("IDirect3D3::CreateViewport", hr);
            hr = device3->AddViewport(viewport3);
            if (FAILED(hr)) ThrowHr("IDirect3DDevice3::AddViewport", hr);
            ...
            hr = viewport3->SetViewport2(&vp);
            if (FAILED(hr)) ThrowHr("IDirect3DViewport3::SetViewport2", hr);
            hr = device3->SetCurrentViewport(viewport3);
            if (FAILED(hr)) ThrowHr("IDirect3DDevice3::SetCurrentViewport", hr);

            device3->SetRenderState(D3DRENDERSTATE_LIGHTING, FALSE);
            ...
            device3->SetRenderState(D3DRENDERSTATE_CULLMODE, D3DCULL_NONE);
            ...
            device3->SetRenderState(D3DRENDERSTATE_ZENABLE, D3DZB_TRUE);
            device3->SetRenderState(D3DRENDERSTATE_ZFUNC, D3DCMP_LESSEQUAL);
            device3->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, TRUE);
        }
        ...
        void CreatePrimary()
        {
            ...
            if (FAILED(hr)) ThrowHr("IDirectDraw4::CreateSurface(primary)", hr);
        }
        ...
        void CreateBackBuffer(int width, int height)
        {
            ...
            if (FAILED(hr)) ThrowHr("IDirectDraw4::CreateSurface(shadow backbuffer)", hr);
    ...
    Dx6GraphicsBackend::Dx6GraphicsBackend(const GraphicsBackendCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx6GraphicsBackend initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx6GraphicsBackend: could not obtain a real HWND from the SDL window.");
        ...
        hr = ddV1->QueryInterface(IID_IDirectDraw4, reinterpret_cast<void**>(&impl_->dd));
        ddV1->Release();
        if (FAILED(hr)) ThrowHr("IDirectDraw::QueryInterface(IID_IDirectDraw4)", hr);
        ...
        // E_NOTIMPL in windowed mode here, exactly as expected -- dead code for this backend
        // family regardless of DirectDraw version).
        hr = impl_->dd->SetCooperativeLevel(impl_->hwnd, DDSCL_NORMAL);
        if (FAILED(hr)) ThrowHr("IDirectDraw4::SetCooperativeLevel", hr);
    ...
    Dx6GraphicsBackend::~Dx6GraphicsBackend() = default;

    void Dx6GraphicsBackend::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx6GraphicsBackend::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx6GraphicsBackend::Present: ClientToScreen failed");
        ...
        if (FAILED(hr)) ThrowHr("IDirectDrawSurface4::Blt(present)", hr);
    }

    void Dx6GraphicsBackend::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx6GraphicsBackend::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx6GraphicsBackend::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx6GraphicsBackend::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx6GraphicsBackend::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx6GraphicsBackend::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx6GraphicsBackend::TransformLogicalToWindow(float logX, float logY,
    ...
    class Dx6VertexBufferBackend final : public IVertexBufferBackend
    {
    public:
        explicit Dx6VertexBufferBackend(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx6VertexBufferBackend::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx6VertexBufferBackend::SetData: stride_in_bytes must be > 0");
    ...
    class Dx6IndexBufferBackend final : public IIndexBufferBackend
    {
    public:
        Dx6IndexBufferBackend(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx6IndexBufferBackend: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx6IndexBufferBackend: SetData bit-width does not match the buffer's declared width");
    ...
    Dx6ClipVertex Dx6LerpClipVertex(const Dx6ClipVertex& a, const Dx6ClipVertex& b, float t)
    {
        Dx6ClipVertex out;
        out.x = a.x + t * (b.x - a.x);
        out.y = a.y + t * (b.y - a.y);
    ...
    int Dx6ClipTriangleNearPlane(const Dx6ClipVertex verts[3], Dx6ClipVertex out[4])
    {
        constexpr float kNearEpsilon = 1e-5f;
        int count = 0;
        for (int i = 0; i < 3; ++i)
        {
            const Dx6ClipVertex& cur = verts[i];
            const Dx6ClipVertex& prev = verts[(i + 2) % 3];
            const bool curIn = cur.w > kNearEpsilon;
            const bool prevIn = prev.w > kNearEpsilon;
            if (curIn != prevIn)
            {
                const float t = (kNearEpsilon - prev.w) / (cur.w - prev.w);
                out[count++] = Dx6LerpClipVertex(prev, cur, t);
            }
            if (curIn)
                out[count++] = cur;
        }
    ...
    Dx6ClipVertex Dx6BuildPositionColorClipVertex(const uint8_t* raw, const Matrix& combined)
    {
        Vector3 position;
    ...
    /// Result of Dx6ComputeVertexLighting: the diffuse (base vertex color) and specular (additive,
    ...
    Dx6LitColor Dx6ComputeVertexLighting(const Vector3& localPosition, const Vector3& localNormal,
    ...
    /// normal (32/52), Dx6ComputeVertexLighting() replaces the raw/white vertex color with real
    ...
    Dx6ClipVertex Dx6BuildGenericClipVertex(const uint8_t* raw, std::size_t stride, const Matrix& combined,
                                            const Matrix& world, const GpuDrawParams& params)
    {
        Vector3 position;
        std::memcpy(&position, raw, sizeof(Vector3));
        ...
        else if (stride == 32 || stride == 52)
        {
            ...
            if (params.lightingEnabled)
            {
                ...
                const Dx6LitColor litColor = Dx6ComputeVertexLighting(position, normal, world, params);
    ...
    D3DTLVERTEX Dx6ClipVertexToD3DTLVERTEX(const Dx6ClipVertex& cv, int viewportWidth, int viewportHeight)
    {
    ...
    /// (not cached) rather than stored on Dx6TextureBackend: a handle is only valid for the device
    /// it was obtained against, and this backend's device3 is torn down and recreated whenever the
    ...
    /// device2 view is obtained here and released immediately after GetHandle returns.
    D3DTEXTUREHANDLE Dx6ResolveTextureHandle(const ITextureBackend* texture, LPDIRECT3DDEVICE3 device3)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const Dx6SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx6GraphicsBackend: texture0 is not a DX6 surface (created by a different graphics backend?)");

        LPDIRECT3DTEXTURE2 tex2 = nullptr;
        HRESULT hr = owner->Surface()->QueryInterface(IID_IDirect3DTexture2, reinterpret_cast<void**>(&tex2));
        if (FAILED(hr)) ThrowHr("IDirectDrawSurface4::QueryInterface(IID_IDirect3DTexture2)", hr);

        LPDIRECT3DDEVICE2 device2ForHandle = nullptr;
        hr = device3->QueryInterface(IID_IDirect3DDevice2, reinterpret_cast<void**>(&device2ForHandle));
        if (FAILED(hr))
        {
            tex2->Release();
            ThrowHr("IDirect3DDevice3::QueryInterface(IID_IDirect3DDevice2)", hr);
        }

        D3DTEXTUREHANDLE handle = 0;
        hr = tex2->GetHandle(device2ForHandle, &handle);
        device2ForHandle->Release();
        tex2->Release();
        if (FAILED(hr)) ThrowHr("IDirect3DTexture2::GetHandle", hr);
        return handle;
    }

    /// Shared core for all 4 draw entry points: clips each triangle (Dx6ClipTriangleNearPlane),
    /// packs the result into D3DTLVERTEX (Dx6ClipVertexToD3DTLVERTEX), and submits via a single
    ...
    void SubmitDx6Primitives(LPDIRECT3DDEVICE3 device3, int viewportWidth, int viewportHeight,
                             const std::function<Dx6ClipVertex(int)>& fetchVertex,
                             int primitiveCount, D3DTEXTUREHANDLE texHandle, bool specularEnabled)
    {
        ...
        for (int i = 0; i < primitiveCount; ++i)
        {
            ...
            Dx6ClipVertex clipped[4];
            const int clippedCount = Dx6ClipTriangleNearPlane(cv, clipped);
            if (clippedCount == 0) continue;

            const auto baseIdx = static_cast<WORD>(verts.size());
            for (int k = 0; k < clippedCount; ++k)
                verts.push_back(Dx6ClipVertexToD3DTLVERTEX(clipped[k], viewportWidth, viewportHeight));
        ...
        HRESULT hr = device3->SetRenderState(D3DRENDERSTATE_TEXTUREHANDLE, texHandle);
        if (FAILED(hr)) ThrowHr("IDirect3DDevice2::SetRenderState(TEXTUREHANDLE)", hr);
        hr = device3->SetRenderState(D3DRENDERSTATE_TEXTUREMAPBLEND, D3DTBLEND_MODULATE);
        if (FAILED(hr)) ThrowHr("IDirect3DDevice2::SetRenderState(TEXTUREMAPBLEND)", hr);
        hr = device3->SetRenderState(D3DRENDERSTATE_SPECULARENABLE, specularEnabled ? TRUE : FALSE);
        if (FAILED(hr)) ThrowHr("IDirect3DDevice2::SetRenderState(SPECULARENABLE)", hr);

        hr = device3->BeginScene();
        if (FAILED(hr)) ThrowHr("IDirect3DDevice2::BeginScene", hr);
        hr = device3->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, D3DFVF_TLVERTEX,
    ...
    /// is scoped to the default backbuffer only -- device3 is never bound to a custom render
    /// target's surface).
    void Dx6CheckDrawPreconditions(const char* who, bool customRenderTargetBound,
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx6GraphicsBackend::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx6GraphicsBackend::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx6GraphicsBackend::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class Dx6TextureBackend : public ITextureBackend, public Dx6SurfaceOwner
    {
    public:
        Dx6TextureBackend(LPDIRECTDRAW4 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx6TextureBackend(LPDIRECTDRAW4 dd, const ImageData& data)
            : Dx6TextureBackend(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx6TextureBackend() override { if (surface_) surface_->Release(); }

        Dx6TextureBackend(const Dx6TextureBackend&) = delete;
        Dx6TextureBackend& operator=(const Dx6TextureBackend&) = delete;
        ...
        [[nodiscard]] LPDIRECTDRAWSURFACE4 Surface() const override { return surface_; }

    protected:
        int width_ = 0;
        int height_ = 0;
        LPDIRECTDRAWSURFACE4 surface_ = nullptr;
    };

    class Dx6RenderTargetBackend final : public IRenderTargetBackend, public Dx6SurfaceOwner
    {
    public:
        Dx6RenderTargetBackend(LPDIRECTDRAWSURFACE4* currentTargetSlot, int* currentTargetWidthSlot,
                               int* currentTargetHeightSlot, LPDIRECTDRAW4 dd,
        ...
        ~Dx6RenderTargetBackend() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx6RenderTargetBackend(const Dx6RenderTargetBackend&) = delete;
        Dx6RenderTargetBackend& operator=(const Dx6RenderTargetBackend&) = delete;
    ...
    private:
        LPDIRECTDRAWSURFACE4* currentTargetSlot_;
    ...
    std::unique_ptr<ITextureBackend> Dx6GraphicsBackend::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx6TextureBackend>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetBackend> Dx6GraphicsBackend::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx6RenderTargetBackend>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx6GraphicsBackend::SetRenderTarget2D(IRenderTargetBackend* rt)
    {
        if (rt)
    ...
    void Dx6GraphicsBackend::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        // DirectDraw has no multi-render-target concept -- single active surface only.
        if (count > 1)
            throw std::runtime_error(
                "DX6 (DirectDraw v4) does not support multiple simultaneous render targets (MRT): "
                "requested " + std::to_string(count) + ", but IDirectDrawSurface4 supports exactly "
                "one active render target at a time.");
        if (count > 0 && renderTargets[0].IsRenderTargetCubeFace())
            throw std::runtime_error(
                "DX6 (DirectDraw v4) does not support RenderTargetCube face bindings.");
    ...
    // Never named outside this .cpp, same reasoning as Dx6TextureBackend/Dx6RenderTargetBackend.

    class Dx6SpriteBatchBackend final : public ISpriteBatchBackend
    {
    public:
        Dx6SpriteBatchBackend(std::function<LPDIRECTDRAWSURFACE4()> getActiveSurface,
                              std::function<void(int&, int&)> getActiveSurfaceSize,
                              std::function<Dx6BlendMode()> getBlendMode)
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx6SpriteBatchBackend::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx6SpriteBatchBackend::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        void SetCustomEffect(Effect* effect) override
        {
            if (effect != nullptr)
                throw std::runtime_error(
                    "DX6 (DirectDraw v4) does not support custom SpriteBatch Effects: no "
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx6SpriteBatchBackend::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx6SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx6SpriteBatchBackend::Draw: texture backend is not a DX6 surface (created by "
                    "a different graphics backend?)");
            LPDIRECTDRAWSURFACE4 srcSurface = owner->Surface();
            LPDIRECTDRAWSURFACE4 dstSurface = getActiveSurface_();
            int dstW = 0, dstH = 0;
            ...
            if (isIdentityGeometry && isWhiteTint && getBlendMode_() == Dx6BlendMode::Opaque)
            {
                ...
                if (FAILED(hr)) ThrowHr("IDirectDrawSurface4::BltFast(identity)", hr);
                return;
            }
            ...
            dstLock_.EnsureLocked(dstSurface, "IDirectDrawSurface4::Lock(compositor dst)");
            srcLock_.EnsureLocked(srcSurface, "IDirectDrawSurface4::Lock(compositor src)");
    ...
    private:
        std::function<LPDIRECTDRAWSURFACE4()> getActiveSurface_;
        std::function<void(int&, int&)> getActiveSurfaceSize_;
        std::function<Dx6BlendMode()> getBlendMode_;
        bool begun_ = false;
    ...
    std::unique_ptr<ISpriteBatchBackend> Dx6GraphicsBackend::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx6SpriteBatchBackend>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx6GraphicsBackend::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
                                             ...
                                             const BlendWriteState& /*writeState*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_ALPHABLENDENABLE, TRUE);
        impl_->device3->SetRenderState(D3DRENDERSTATE_SRCBLEND, Dx6BlendToD3D(colorSrcBlend));
        impl_->device3->SetRenderState(D3DRENDERSTATE_DESTBLEND, Dx6BlendToD3D(colorDstBlend));
    }

    void Dx6GraphicsBackend::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    {
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZENABLE, depthEnable ? D3DZB_TRUE : D3DZB_FALSE);
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, depthWriteEnable ? TRUE : FALSE);
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZFUNC, Dx6CompareFunctionToD3D(depthFunc));
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILENABLE, stencilEnable ? TRUE : FALSE);
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILFUNC, Dx6CompareFunctionToD3D(stencilFunc));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILFAIL, Dx6StencilOperationToD3D(stencilFail));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILZFAIL, Dx6StencilOperationToD3D(stencilDepthFail));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILPASS, Dx6StencilOperationToD3D(stencilPass));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILREF, static_cast<DWORD>(referenceStencil));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILMASK, static_cast<DWORD>(stencilMask));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILWRITEMASK, static_cast<DWORD>(stencilWriteMask));
    }

    void Dx6GraphicsBackend::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
                                                  float /*depthBias*/, float /*slopeScaleDepthBias*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_CULLMODE, Dx6CullModeToD3D(cullMode));
        impl_->device3->SetRenderState(D3DRENDERSTATE_FILLMODE, Dx6FillModeToD3D(fillMode));
    }

    void Dx6GraphicsBackend::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
        ...
        const Dx6FilterPair filterPair = Dx6TextureFilterToD3D(filter);
        impl_->device3->SetRenderState(D3DRENDERSTATE_TEXTUREMAG, filterPair.mag);
        impl_->device3->SetRenderState(D3DRENDERSTATE_TEXTUREMIN, filterPair.min);
        impl_->device3->SetRenderState(D3DRENDERSTATE_TEXTUREADDRESS, Dx6TextureAddressModeToD3D(addressU));
    ...
    // shadow backbuffer ever has a Z-buffer/3D device attached, matching decision 4's scope.
    void Dx6GraphicsBackend::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }
    ...
    // count=0/rects=nullptr clears nothing at all (silently) -- always pass a real full-surface
    // D3DRECT with count=1.
    void Dx6GraphicsBackend::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
        const HRESULT hr = impl_->viewport3->Clear2(1, &fullRect, D3DCLEAR_ZBUFFER, 0, depth, 0);
    ...
    void Dx6GraphicsBackend::ClearStencil(int stencil)
    {
        ...
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
        const HRESULT hr = impl_->viewport3->Clear2(1, &fullRect, D3DCLEAR_STENCIL, 0, 0.0f,
                                                     static_cast<DWORD>(stencil));
        if (FAILED(hr)) ThrowHr("IDirect3DViewport3::Clear2(STENCIL)", hr);
    }
    void Dx6GraphicsBackend::ClearDepthAndStencil(float depth, int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
        const HRESULT hr = impl_->viewport3->Clear2(1, &fullRect, D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL,
                                                     0, depth, static_cast<DWORD>(stencil));
        if (FAILED(hr)) ThrowHr("IDirect3DViewport3::Clear2(ZBUFFER|STENCIL)", hr);
    }
    void Dx6GraphicsBackend::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        ClearStencil(stencil);
    }
    void Dx6GraphicsBackend::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
        Clear(r, g, b, a);
        ClearDepthAndStencil(depth, stencil);
    }
    void Dx6GraphicsBackend::SetDepthTestEnabled(bool enabled)
    {
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZENABLE, enabled ? D3DZB_TRUE : D3DZB_FALSE);
    }
    ...
    void Dx6GraphicsBackend::SetBlendEnabled(bool) {}

    void Dx6GraphicsBackend::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferBackend> Dx6GraphicsBackend::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx6VertexBufferBackend>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferBackend> Dx6GraphicsBackend::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx6IndexBufferBackend>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferBackend> Dx6GraphicsBackend::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx6IndexBufferBackend>(index_capacity, true);
    }

    void Dx6GraphicsBackend::DrawColoredPrimitives(const IVertexBufferBackend& vb,
                                                   const Matrix& world, const Matrix& view, const Matrix& projection,
                                                   PrimitiveType primitive, int primitiveCount)
    {
        Dx6CheckDrawPreconditions("DrawColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx6GraphicsBackend::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx6VertexBufferBackend&>(vb);
        ...
        SubmitDx6Primitives(impl_->device3, vw, vh,
            [&](int i) { return Dx6BuildPositionColorClipVertex(base + static_cast<std::size_t>(i) * stride, combined); },
            primitiveCount, 0, false);
    }

    void Dx6GraphicsBackend::DrawIndexedColoredPrimitives(const IVertexBufferBackend& vb,
                                                          const IIndexBufferBackend& ib,
                                                          const Matrix& world, const Matrix& view, const Matrix& projection,
                                                          PrimitiveType primitive, int primitiveCount)
    {
        Dx6CheckDrawPreconditions("DrawIndexedColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx6GraphicsBackend::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx6VertexBufferBackend&>(vb);
        const auto& dxIb = static_cast<const Dx6IndexBufferBackend&>(ib);
        ...
        SubmitDx6Primitives(impl_->device3, vw, vh,
            [&](int i) {
                ...
                return Dx6BuildPositionColorClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, combined);
            },
            primitiveCount, 0, false);
    }
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferBackend& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::BackendRefusesIt, "DX6", route);
    }

    void Dx6GraphicsBackend::DrawPrimitivesEx(const IVertexBufferBackend& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-nonindexed");
        Dx6CheckDrawPreconditions("DrawPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx6GraphicsBackend::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx6GraphicsBackend::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx6VertexBufferBackend&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx6GraphicsBackend::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        const D3DTEXTUREHANDLE texHandle = params.textureEnabled
            ? Dx6ResolveTextureHandle(params.texture0, impl_->device3) : 0;
        const int vertexStart = params.vertexStart;

        SubmitDx6Primitives(impl_->device3, vw, vh,
            [&](int i) { return Dx6BuildGenericClipVertex(base + static_cast<std::size_t>(vertexStart + i) * stride,
                                                          stride, combined, world, params); },
            primitiveCount, texHandle, params.lightingEnabled);
    }

    void Dx6GraphicsBackend::DrawIndexedPrimitivesEx(const IVertexBufferBackend& vb, const IIndexBufferBackend& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-indexed");
        Dx6CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx6GraphicsBackend::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx6GraphicsBackend::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx6GraphicsBackend::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        SubmitDx6Primitives(impl_->device3, vw, vh,
            [&](int i) {
                ...
                return Dx6BuildGenericClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, stride,
...
namespace CNA::Internal::Backends
{
    ...
    {
        return std::make_unique<Dx6::Dx6GraphicsBackend>(args);
    }
#endif
}
