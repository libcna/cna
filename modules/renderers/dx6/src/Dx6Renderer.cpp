...
// real <windows.h> it pulls in) is contained to this .cpp only -- see Dx6Renderer.hpp's
...
namespace CNA::Internal::Renderers::Dx6
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx6TextureRenderer and
        // Dx6RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
    ...
    // Common accessor so Dx6SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx6TextureRenderer/Dx6RenderTargetRenderer themselves.
    ...
    struct Dx6Renderer::Impl
    {
        ...
        // Phase O3: the offscreen surface owned by the currently-bound Dx6RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx6RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE4 rather than
        // a Dx6RenderTargetRenderer* so Dx6RenderTargetRenderer never needs to name this private Impl
    ...
    Dx6Renderer::Dx6Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx6Renderer initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx6Renderer: could not obtain a real HWND from the SDL window.");
    ...
    Dx6Renderer::~Dx6Renderer() = default;

    void Dx6Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx6Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx6Renderer::Present: ClientToScreen failed");
    ...
    void Dx6Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx6Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx6Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx6Renderer::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx6Renderer::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx6Renderer::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx6Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    class Dx6VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit Dx6VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx6VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx6VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class Dx6IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        Dx6IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx6IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx6IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    /// (not cached) rather than stored on Dx6TextureRenderer: a handle is only valid for the device
    ...
    D3DTEXTUREHANDLE Dx6ResolveTextureHandle(const ITextureRenderer* texture, LPDIRECT3DDEVICE3 device3)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const Dx6SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx6Renderer: texture0 is not a DX6 surface (created by a different graphics renderer?)");
                                   ...
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx6Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx6Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx6Renderer::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class Dx6TextureRenderer : public ITextureRenderer, public Dx6SurfaceOwner
    {
    public:
        Dx6TextureRenderer(LPDIRECTDRAW4 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx6TextureRenderer(LPDIRECTDRAW4 dd, const ImageData& data)
            : Dx6TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx6TextureRenderer() override { if (surface_) surface_->Release(); }

        Dx6TextureRenderer(const Dx6TextureRenderer&) = delete;
        Dx6TextureRenderer& operator=(const Dx6TextureRenderer&) = delete;
    ...
    class Dx6RenderTargetRenderer final : public IRenderTargetRenderer, public Dx6SurfaceOwner
    {
    public:
        Dx6RenderTargetRenderer(LPDIRECTDRAWSURFACE4* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx6RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx6RenderTargetRenderer(const Dx6RenderTargetRenderer&) = delete;
        Dx6RenderTargetRenderer& operator=(const Dx6RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> Dx6Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx6TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> Dx6Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx6RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx6Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void Dx6Renderer::SetRenderTargets(
    ...
    // Never named outside this .cpp, same reasoning as Dx6TextureRenderer/Dx6RenderTargetRenderer.

    class Dx6SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        Dx6SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE4()> getActiveSurface,
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx6SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx6SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx6SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx6SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx6SpriteBatchRenderer::Draw: texture renderer is not a DX6 surface (created by "
    ...
    std::unique_ptr<ISpriteBatchRenderer> Dx6Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx6SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx6Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void Dx6Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    void Dx6Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
    ...
    void Dx6Renderer::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
    ...
    void Dx6Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }
    ...
    void Dx6Renderer::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void Dx6Renderer::ClearStencil(int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
    ...
    void Dx6Renderer::ClearDepthAndStencil(float depth, int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
    ...
    void Dx6Renderer::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        ClearStencil(stencil);
    }
    void Dx6Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
        Clear(r, g, b, a);
        ClearDepthAndStencil(depth, stencil);
    }
    void Dx6Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void Dx6Renderer::SetBlendEnabled(bool) {}

    void Dx6Renderer::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferRenderer> Dx6Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx6VertexBufferRenderer>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx6Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx6IndexBufferRenderer>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx6Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx6IndexBufferRenderer>(index_capacity, true);
    }

    void Dx6Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   ...
                                                   PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx6Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx6VertexBufferRenderer&>(vb);
    ...
    void Dx6Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          ...
                                                          PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx6Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx6VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const Dx6IndexBufferRenderer&>(ib);
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DX6", route);
    }

    void Dx6Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        ...
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx6Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx6Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx6VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx6Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
    ...
    void Dx6Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        ...
        Dx6CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx6Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx6Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx6Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<Dx6::Dx6Renderer>(args);
    }
#endif
}
