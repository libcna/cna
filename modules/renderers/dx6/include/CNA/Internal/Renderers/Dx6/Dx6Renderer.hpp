...
// a Dx6Renderer pointer. All real <ddraw.h> usage lives in Dx6Renderer.cpp behind the
// Impl pimpl below.
...
namespace CNA::Internal::Renderers::Dx6
{
    /**
     * DX6 graphics renderer (plan_dx6.md): a verbatim port of Dx5Renderer (plan_dx5.md,
     * itself a port of Dx3Renderer/Dx2Renderer) -- DirectDraw v4
     ...
     * DX6 introduces no new COM interface revision at all. Device/window bring-up:
     * DirectDrawCreate -> QueryInterface(IID_IDirectDraw4) -> SetCooperativeLevel(DDSCL_NORMAL,
     * against a real Win32 HWND obtained from CNA's own already-existing SDL_Window via
     ...
     * and empirically at DX1-0/DX2-0/DX30-0/DX5-0/DX6-0, all of which this renderer's history
     * inherits). The primary surface is desktop-sized (the real historical DirectDraw model: the
     * primary IS the display, not "this window") -- so this renderer never composites directly
     * onto it. Instead it owns a second, Lockable offscreen "shadow backbuffer" surface
     * (DDSCAPS_OFFSCREENPLAIN | DDSCAPS_3DDEVICE, so a Z-buffer can be attached and a Direct3D
     * device created against this same surface), sized to the logical/virtual resolution, that
     * Clear()/SpriteBatch draws always target; Present() letterbox-scales that shadow buffer onto
     * the primary via a single Blt(), with the destination rect recomputed every frame from the
     * window's real client area.
     *
     * Textures and render targets are real, private offscreen DDSCAPS_OFFSCREENPLAIN v4 surfaces
     * (Dx6TextureRenderer/Dx6RenderTargetRenderer, defined entirely in Dx6Renderer.cpp --
     ...
     * (Dx6SpriteBatchRenderer): IDirectDrawSurface4::Blt/BltFast has never supported rotation in any
     ...
     * real depth-test occlusion, real texture sampling, full per-draw state mapping, and Phase
     * O9's CPU-side BasicEffect lighting (ambient + directional Lambertian/Blinn-Phong specular
     * via real D3DRENDERSTATE_SPECULARENABLE) -- all ported from DX5's own already-proven 3D
     ...
     */
    class Dx6Renderer final : public IGraphicsRenderer
    {
    public:
        explicit Dx6Renderer(const GraphicsRendererCreateArgs& args);
        ~Dx6Renderer() override;

        Dx6Renderer(const Dx6Renderer&) = delete;
        Dx6Renderer& operator=(const Dx6Renderer&) = delete;
        ...
        // ignored (no scissor test or depth-bias render state exists in d3dtypes.h at this era --
        // confirmed by inspection). ApplySamplerState only acts on slot 0 (D3D v1-v3 has exactly
        // one texture stage in this renderer's own scope, decision 6) and honors filter/addressU/
        ...
        // Phase O5 (design decision 8): plain CPU-side storage (Dx6VertexBufferRenderer/
        // Dx6IndexBufferRenderer, defined entirely in Dx6Renderer.cpp), matching the
