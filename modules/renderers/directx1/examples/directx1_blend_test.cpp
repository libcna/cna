...
// Check E confirms DX1-44's fallback: a custom (non-preset) BlendState combination produces the
...
class DirectX1BlendTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
...

        {
            ...
            check(r == 200 && g == 0, "Opaque: direct overwrite, ignores source alpha (DX1-40)");
        }
        {
            const auto [r, g] = DrawAndReadRG(dev, sb, BlendState::AlphaBlend);
            check(r == 200 && g == 30, "AlphaBlend: premultiplied formula (src + dst*(1-srcAlpha)) (DX1-41)");
        }
        {
            const auto [r, g] = DrawAndReadRG(dev, sb, BlendState::NonPremultiplied);
            check(r == 78 && g == 30, "NonPremultiplied: straight-alpha formula (src*srcAlpha + dst*(1-srcAlpha)) (DX1-42)");
        }
        {
            const auto [r, g] = DrawAndReadRG(dev, sb, BlendState::Additive);
            check(r == 78 && g == 50, "Additive: saturating add, dst unattenuated (DX1-43)");
        }
        {
            BlendState custom;
            ...
            check(r == 200 && g == 30,
                  "Custom (non-preset) BlendState falls back to AlphaBlend behavior (DX1-44)");
        }
        {
            ...
            check(r == 200 && g == 30,
                  ...
                  "AlphaBlend, not misdetected as Opaque (DX1-44)");
        }
