...
// plans/plan_dx1.md Phase O3 (DX1-20..DX1-28): texture and render-target renderer tests for the DIRECTX1
...
//   ...) round-trips through DirectX1TextureRenderer::UpdatePixels (Lock()/memcpy/Unlock()) without
//   throwing (DX1-20/21).
...
//   from the render target's OWN surface, not the shadow backbuffer (DX1-23/25/26).
...
//   Clear()/ReadBackbuffer() back to Impl::backBuffer (DX1-26).
// Check F -- RenderTargetUsage::DiscardContents auto-clears to black on every (re)bind -- comes
//   for free from shared GraphicsDevice.cpp once bind/Clear/read are wired correctly (DX1-25).
// Check G -- SetRenderTargets() with 2 bindings throws: no MRT on IDirectDrawSurface (DX1-27).
...
class DirectX1TextureRenderTargetTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        // Check H (DX1-28): real finding, spike-confirmed rather than assumed -- unlike DIRECTX3's
        ...
        // (plans/plan_dx1.md DX1-28) succeeded up to 16384x16384 offscreen surfaces and only failed at
        ...
        // CNA_RENDERER_DIRECTX9-only) -- an honest, documented gap for DIRECTX1, not silently dropped.
        {
            bool threw = false;
...
public:
    DirectX1TextureRenderTargetTest()
    {
...
int main()
{
    DirectX1TextureRenderTargetTest game;
    game.Run();
