...
// plans/plan_dx1.md Phase O5 (DX1-45/DX1-46): TextureFilter (nearest vs. bilinear) and
// TextureAddressMode (Wrap/Mirror/Clamp) sampling tests for the DIRECTX1 (DirectDraw, via the
...
// destination's prior content -- this isolates the sampling logic itself (DX1-45/46) from any
// blend-formula concern (DX1-40..44, already covered by DirectX1_Blend).
//
...
//   Red/Green tiling patterns (DX1-46).
//
...
class DirectX1SamplingTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        // ---- Checks A/B: TextureFilter (DX1-45) ----
        ...
        // a clear difference between the two filter modes.
        {
            ...
            check(pointVal == 0 || pointVal == 255,
                  "TextureFilter::Point samples a pure endpoint color at the texel boundary (DX1-45)");
            ...
            check(linearVal > 0 && linearVal < 255,
                  "TextureFilter::Linear (bilinear) blends between texels at the boundary (DX1-45)");
        }

        // ---- Checks C/D/E: TextureAddressMode (DX1-46) ----
