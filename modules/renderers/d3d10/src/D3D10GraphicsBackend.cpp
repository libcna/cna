// plan_d3d10.md: real Direct3D 10 graphics backend implementation. See the header's own class-level
...
namespace CNA::Internal::Backends::D3D10
{
    ...
    namespace
    {
        ...
        // re-expressed against D3D10_* types since this backend deliberately stays self-contained
    ...
    // render target polymorphically (mirrors DX8's own Dx8TextureOwner pattern).
    ...
    class D3D10TextureBackend final : public ITextureBackend, public D3D10TextureOwner
    {
    public:
        D3D10TextureBackend(ID3D10Device* device, const ImageData& data)
    ...
    class D3D10RenderTargetBackend final : public IRenderTargetBackend, public D3D10TextureOwner
    {
    public:
        D3D10RenderTargetBackend(ID3D10Device* device, int w, int h, bool wantDepthStencil)
        ...
        ~D3D10RenderTargetBackend() override
        {
    ...
    class D3D10VertexBufferBackend final : public IVertexBufferBackend
    {
    public:
        D3D10VertexBufferBackend(ID3D10Device* device, int vertexCapacity)
            : device_(device), capacity_(vertexCapacity)
        {
        }
        ~D3D10VertexBufferBackend() override { if (buffer_) buffer_->Release(); }
    ...
    class D3D10IndexBufferBackend final : public IIndexBufferBackend
    {
    public:
        D3D10IndexBufferBackend(ID3D10Device* device, bool thirtyTwoBit)
            : device_(device), thirtyTwoBit_(thirtyTwoBit)
        {
        }
        ~D3D10IndexBufferBackend() override { if (buffer_) buffer_->Release(); }
    ...
    class D3D10SpriteBatchBackend final : public ISpriteBatchBackend
    {
    public:
        // viewportWidthPtr/viewportHeightPtr point at the owning D3D10GraphicsBackend::Impl's own
        ...
        D3D10SpriteBatchBackend(ID3D10Device* device, const int* viewportWidthPtr, const int* viewportHeightPtr)
            : device_(device), viewportWidthPtr_(viewportWidthPtr), viewportHeightPtr_(viewportHeightPtr)
        {
            ...
            // winding order and gets silently culled unless this backend explicitly disables
        ...
        ~D3D10SpriteBatchBackend() override
        {
        ...
        void Begin() override
        {
            if (begun_) throw std::runtime_error("D3D10SpriteBatchBackend::Begin: already begun");
            begun_ = true;
            pendingVertices_.clear();
            pendingIndices_.clear();
            currentTexture_ = nullptr;
            // Real bug found via CTest (not present in DX8's own Begin(), which this was modeled
        ...
        void End() override
        {
            if (!begun_) throw std::runtime_error("D3D10SpriteBatchBackend::End: End() called before Begin()");
            FlushBatch();
            begun_ = false;
        }
        ...
        {
            if (!begun_) throw std::runtime_error("D3D10SpriteBatchBackend::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const D3D10TextureOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "D3D10SpriteBatchBackend::Draw: texture backend is not a D3D10 texture");
    ...
    struct D3D10GraphicsBackend::Impl
    {
        ...
        D3D10RenderTargetBackend* boundSingleTarget = nullptr; // for SetRenderTarget2D(nullptr) restore + ReadBackbuffer sizing
    ...
    D3D10GraphicsBackend::D3D10GraphicsBackend(const GraphicsBackendCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("D3D10GraphicsBackend initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("D3D10GraphicsBackend: could not obtain a real HWND from the SDL window.");
    ...
    D3D10GraphicsBackend::~D3D10GraphicsBackend() = default;

    void D3D10GraphicsBackend::Clear(float r, float g, float b, float a)
    {
    ...
    void D3D10GraphicsBackend::Present()
    {
        const HRESULT hr = impl_->swapChain->Present(0, 0);
        if (FAILED(hr)) ThrowHr("IDXGISwapChain::Present", hr);
    }

    void D3D10GraphicsBackend::GetViewportSize(int& width, int& height)
    {
        if (impl_->window)
        {
    ...
    void D3D10GraphicsBackend::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
        if (w <= 0 || h <= 0) return;
    ...
    void D3D10GraphicsBackend::SetVirtualResolution(int width, int height)
    {
        // Inert bookkeeping (design decision 6) -- the swap chain always matches the real window
        // size, matching D3D11GraphicsBackend's own simpler convention (unlike DX8's own
    ...
    void D3D10GraphicsBackend::SetPresentationMode(int mode)
    {
        impl_->presentationMode = static_cast<CnaPresentationMode>(mode);
    }

    SDL_Window* D3D10GraphicsBackend::GetWindowInternal() const { return impl_->window; }

    std::unique_ptr<ITextureBackend> D3D10GraphicsBackend::CreateTexture(const ImageData& data)
    {
        return std::make_unique<D3D10TextureBackend>(impl_->device, data);
    }

    std::unique_ptr<IRenderTargetBackend> D3D10GraphicsBackend::CreateRenderTarget2D(
        int w, int h, int depthFormat, bool /*preserveContents*/, bool /*mipMap*/, int /*multiSampleCount*/)
    {
        const bool wantDepth = depthFormat != 0; // DepthFormat::None == 0
        return std::make_unique<D3D10RenderTargetBackend>(impl_->device, w, h, wantDepth);
    }

    void D3D10GraphicsBackend::SetRenderTarget2D(IRenderTargetBackend* rt)
    {
        if (!rt)
        {
            impl_->BindDefaultTargets();
        ...
        auto* d3dRt = dynamic_cast<D3D10RenderTargetBackend*>(rt);
        if (!d3dRt)
            throw std::runtime_error("D3D10GraphicsBackend::SetRenderTarget2D: not a D3D10 render target");
        ...
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        ...
        if (count > Impl::kMaxTargets)
            throw std::runtime_error("D3D10GraphicsBackend::SetRenderTargets: too many render targets requested");
    ...
    std::unique_ptr<ISpriteBatchBackend> D3D10GraphicsBackend::CreateSpriteBatch()
    {
        return std::make_unique<D3D10SpriteBatchBackend>(impl_->device, &impl_->currentTargetWidth,
    ...
    void D3D10GraphicsBackend::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    void D3D10GraphicsBackend::ApplyRasterizerState(int cullMode, int fillMode, bool scissorTestEnable,
    ...
    void D3D10GraphicsBackend::ApplySamplerState(int slot, int filter, int addressU, int addressV,
                                                 int maxAnisotropy)
    {
    ...
    void D3D10GraphicsBackend::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        if (impl_->currentDSV) impl_->device->ClearDepthStencilView(impl_->currentDSV, D3D10_CLEAR_DEPTH, depth, 0);
    }
    void D3D10GraphicsBackend::ClearDepth(float depth)
    {
        if (impl_->currentDSV) impl_->device->ClearDepthStencilView(impl_->currentDSV, D3D10_CLEAR_DEPTH, depth, 0);
    }
    void D3D10GraphicsBackend::ClearStencil(int stencil)
    {
    ...
    void D3D10GraphicsBackend::ClearDepthAndStencil(float depth, int stencil)
    {
    ...
    void D3D10GraphicsBackend::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        if (impl_->currentDSV) impl_->device->ClearDepthStencilView(impl_->currentDSV, D3D10_CLEAR_STENCIL, 1.0f, static_cast<UINT8>(stencil));
    }
    void D3D10GraphicsBackend::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
        Clear(r, g, b, a);
        if (impl_->currentDSV) impl_->device->ClearDepthStencilView(impl_->currentDSV, D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL, depth, static_cast<UINT8>(stencil));
    }

    void D3D10GraphicsBackend::SetDepthTestEnabled(bool /*enabled*/) {}
    void D3D10GraphicsBackend::SetBlendEnabled(bool /*enabled*/) {}
    void D3D10GraphicsBackend::SetDepthWriteEnabled(bool /*enabled*/) {}

    std::unique_ptr<IVertexBufferBackend> D3D10GraphicsBackend::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<D3D10VertexBufferBackend>(impl_->device, vertex_capacity);
    }
    std::unique_ptr<IIndexBufferBackend> D3D10GraphicsBackend::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<D3D10IndexBufferBackend>(impl_->device, false);
    }
    std::unique_ptr<IIndexBufferBackend> D3D10GraphicsBackend::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<D3D10IndexBufferBackend>(impl_->device, true);
    }

    void D3D10GraphicsBackend::DrawColoredPrimitives(
        const IVertexBufferBackend& vb, const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount)
    {
        const auto& d3dVb = static_cast<const D3D10VertexBufferBackend&>(vb);
    ...
    void D3D10GraphicsBackend::DrawIndexedColoredPrimitives(
        const IVertexBufferBackend& vb, const IIndexBufferBackend& ib,
        const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount)
    {
        const auto& d3dVb = static_cast<const D3D10VertexBufferBackend&>(vb);
        const auto& d3dIb = static_cast<const D3D10IndexBufferBackend&>(ib);
        const std::size_t stride = d3dVb.GetStrideEXT() > 0 ? d3dVb.GetStrideEXT() : 16;
...
namespace CNA::Internal::Backends
{
    ...
    {
        return std::make_unique<D3D10::D3D10GraphicsBackend>(args);
    }
}
