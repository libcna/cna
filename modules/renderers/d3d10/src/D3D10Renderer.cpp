...
namespace CNA::Internal::Renderers::D3D10
{
    ...
    {
    public:
        // viewportWidthPtr/viewportHeightPtr point at the owning D3D10Renderer::Impl's own
    ...
    D3D10Renderer::D3D10Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("D3D10Renderer initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("D3D10Renderer: could not obtain a real HWND from the SDL window.");
    ...
    D3D10Renderer::~D3D10Renderer() = default;
    ...
    void D3D10Renderer::GetViewportSize(int& width, int& height)
    {
        if (impl_->window)
        {
    ...
    void D3D10Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
        if (w <= 0 || h <= 0) return;
    ...
    void D3D10Renderer::SetVirtualResolution(int width, int height)
    {
        // Inert bookkeeping (design decision 6) -- the swap chain always matches the real window
        // size, matching D3D11Renderer's own simpler convention (unlike DX8's own
    ...
    void D3D10Renderer::SetPresentationMode(int mode)
    {
        impl_->presentationMode = static_cast<CnaPresentationMode>(mode);
    }

    SDL_Window* D3D10Renderer::GetWindowInternal() const { return impl_->window; }

    std::unique_ptr<ITextureRenderer> D3D10Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<D3D10TextureRenderer>(impl_->device, data);
    }

    std::unique_ptr<IRenderTargetRenderer> D3D10Renderer::CreateRenderTarget2D(
    ...
    void D3D10Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (!rt)
        {
            impl_->BindDefaultTargets();
        ...
        if (!d3dRt)
            throw std::runtime_error("D3D10Renderer::SetRenderTarget2D: not a D3D10 render target");
    ...
    void D3D10Renderer::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        if (count <= 0) { impl_->BindDefaultTargets(); return; }
        if (count > Impl::kMaxTargets)
            throw std::runtime_error("D3D10Renderer::SetRenderTargets: too many render targets requested");
        ...
        for (int i = 0; i < count; ++i)
        {
            if (renderTargets[i].IsRenderTargetCubeFace())
                throw std::runtime_error(
                    "D3D10Renderer::SetRenderTargets: RenderTargetCube face bindings are not supported");
            auto* d3dRt = dynamic_cast<D3D10RenderTargetRenderer*>(renderTargets[i].GetRenderTarget2D());
            if (!d3dRt)
                throw std::runtime_error("D3D10Renderer::SetRenderTargets: not a D3D10 render target");
            rtvs[i] = d3dRt->RTV();
    ...
    std::unique_ptr<ISpriteBatchRenderer> D3D10Renderer::CreateSpriteBatch()
    {
        return std::make_unique<D3D10SpriteBatchRenderer>(impl_->device, &impl_->currentTargetWidth,
                                                           &impl_->currentTargetHeight);
    }

    void D3D10Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void D3D10Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    void D3D10Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool scissorTestEnable,
    ...
    void D3D10Renderer::ApplySamplerState(int slot, int filter, int addressU, int addressV,
                                                 int maxAnisotropy)
    {
    ...
    void D3D10Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        if (impl_->currentDSV) impl_->device->ClearDepthStencilView(impl_->currentDSV, D3D10_CLEAR_DEPTH, depth, 0);
    }
    void D3D10Renderer::ClearDepth(float depth)
    {
        if (impl_->currentDSV) impl_->device->ClearDepthStencilView(impl_->currentDSV, D3D10_CLEAR_DEPTH, depth, 0);
    }
    void D3D10Renderer::ClearStencil(int stencil)
    {
    ...
    void D3D10Renderer::ClearDepthAndStencil(float depth, int stencil)
    {
    ...
    void D3D10Renderer::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        if (impl_->currentDSV) impl_->device->ClearDepthStencilView(impl_->currentDSV, D3D10_CLEAR_STENCIL, 1.0f, static_cast<UINT8>(stencil));
    }
    void D3D10Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
        Clear(r, g, b, a);
        if (impl_->currentDSV) impl_->device->ClearDepthStencilView(impl_->currentDSV, D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL, depth, static_cast<UINT8>(stencil));
    }

    void D3D10Renderer::SetDepthTestEnabled(bool /*enabled*/) {}
    void D3D10Renderer::SetBlendEnabled(bool /*enabled*/) {}
    void D3D10Renderer::SetDepthWriteEnabled(bool /*enabled*/) {}

    std::unique_ptr<IVertexBufferRenderer> D3D10Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<D3D10VertexBufferRenderer>(impl_->device, vertex_capacity);
    }
    std::unique_ptr<IIndexBufferRenderer> D3D10Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<D3D10IndexBufferRenderer>(impl_->device, false);
    }
    std::unique_ptr<IIndexBufferRenderer> D3D10Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<D3D10IndexBufferRenderer>(impl_->device, true);
    }

    void D3D10Renderer::DrawColoredPrimitives(
    ...
    void D3D10Renderer::DrawIndexedColoredPrimitives(
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<D3D10::D3D10Renderer>(args);
    }
}
