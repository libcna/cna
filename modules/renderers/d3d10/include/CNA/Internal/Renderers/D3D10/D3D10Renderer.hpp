...
// plan_d3d10.md: real Direct3D 10 graphics renderer. Unlike DX1..DX8, D3D10 removed the
...
// precedent (D3D9EffectRenderer.cpp/D3D11EffectRenderer.cpp), not DX1..DX8's CPU-transform-and-submit
...
// color (XNA's own graceful degradation), just without D3D11Renderer's own full 10-stock-
...
// D3D10Renderer.cpp behind the Impl pimpl below.
...
namespace CNA::Internal::Renderers::D3D10
{
     ...
     * the real window size, matching D3D11Renderer's own simpler, already-established
     * convention.
     *
     ...
     * D3D10Renderer.cpp), created via CreateTexture2D against an explicit
     ...
     * every DX1..DX8 renderer (DirectDraw/early-Direct3D has exactly one active render target).
     */
    class D3D10Renderer final : public IGraphicsRenderer
    {
    public:
        explicit D3D10Renderer(const GraphicsRendererCreateArgs& args);
        ~D3D10Renderer() override;

        D3D10Renderer(const D3D10Renderer&) = delete;
        D3D10Renderer& operator=(const D3D10Renderer&) = delete;
        ...
        // DX1..DX8 renderer), up to D3D10_SIMULTANEOUS_RENDER_TARGET_COUNT (8).
        ...
        [[nodiscard]] bool SupportsCapability(CNA::GraphicsCapability capability) const override
        {
            ...
            // MultipleRenderTargets: real (unlike every DX1..DX8 renderer) -- D3D10 has genuine
        ...
        // defined entirely in D3D10Renderer.cpp).
