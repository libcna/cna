...
// plan_d3d10.md: real Direct3D 10 graphics backend. Unlike DX1..DX8, D3D10 removed the
...
// precedent (D3D9EffectBackend.cpp/D3D11EffectBackend.cpp), not DX1..DX8's CPU-transform-and-submit
...
//
// MVP scope (plan_d3d10.md design decision 3, mirroring DX1's own "baseline first, richness
// later" precedent): DrawColoredPrimitives/DrawIndexedColoredPrimitives (required by
// IGraphicsBackend) are real shader draws (world*view*projection transform + vertex-color
// passthrough, no lighting). DrawPrimitivesEx/DrawIndexedPrimitivesEx are intentionally NOT
// overridden -- IGraphicsBackend's own safe default falls back to the colored-primitives path, so
// draws requesting lighting/texturing via GpuDrawParams still render correctly as flat vertex
// color (XNA's own graceful degradation), just without D3D11GraphicsBackend's own full 10-stock-
// shader-variant richness. CreateEffectBackend/occlusion query/3D-cube-textures/instancing are
// likewise left at IGraphicsBackend's own safe defaults.
//
// This header intentionally does NOT include <d3d10.h> (matching D3D9/D3D11/D3D12/DX1..DX8's own
...
{
    /**
     * D3D10 graphics backend (plan_d3d10.md): real Direct3D 10 (ID3D10Device), delivered via
     ...
     * chain -- no logical-resolution render target or letterboxing (unlike DX8's own addition,
     * forced there by D3D8 having no scaled-blit primitive at all): the swap chain always matches
     * the real window size, matching D3D11GraphicsBackend's own simpler, already-established
     * convention.
     *
     * Textures and render targets are real ID3D10Texture2D objects
     * (D3D10TextureBackend/D3D10RenderTargetBackend, defined entirely in
     * D3D10GraphicsBackend.cpp), created via CreateTexture2D against an explicit
     ...
     *
     * The 2D SpriteBatch layer (D3D10SpriteBatchBackend) is real GPU-rendered textured quads
     ...
     * setters like DX1..DX8's D3DRENDERSTATE_ / D3DRS_ constants -- there is no equivalent of
     ...
     * every DX1..DX8 backend (DirectDraw/early-Direct3D has exactly one active render target).
     */
    class D3D10GraphicsBackend final : public IGraphicsBackend
    {
    public:
        explicit D3D10GraphicsBackend(const GraphicsBackendCreateArgs& args);
        ~D3D10GraphicsBackend() override;

        D3D10GraphicsBackend(const D3D10GraphicsBackend&) = delete;
        D3D10GraphicsBackend& operator=(const D3D10GraphicsBackend&) = delete;
        ...
        // ---- IGraphicsBackend: textures and render targets (real ID3D10Texture2D) ----
        ...
        [[nodiscard]] bool SupportsCapability(CNA::GraphicsCapability capability) const override
        {
            ...
            // MultipleRenderTargets: real (unlike every DX1..DX8 backend) -- D3D10 has genuine
        ...
        // Real ID3D10Buffer-backed storage (D3D10VertexBufferBackend/D3D10IndexBufferBackend,
        // defined entirely in D3D10GraphicsBackend.cpp).
        ...
        // Sutherland-Hodgman needed, unlike DX1..DX8).
