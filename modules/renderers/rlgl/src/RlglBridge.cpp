...
void main()
{
    ...
    if (skinned > 0.5)
    {
        ...
        // XNA's SkinnedEffect treats bone transforms as rigid/uniform and applies them directly
        // to the normal. Only the outer World transform uses the inverse-transpose matrix below.
        vec3 skinNormal = skinMatrix * vertexNormal;
...
uniform float texturePresent;
uniform float dualTexture;
uniform float texture1Present;
uniform float environmentMapping;
uniform float environmentMapPresent;
...
void main()
{
    ...
    vec4 sampled = (textureEnabled > 0.5)
        ? ((texturePresent > 0.5)
            ? texture(texture0, textureCoordinate0) : vec4(0.0, 0.0, 0.0, 1.0))
        : vec4(1.0);
    if (dualTexture > 0.5)
    {
        sampled.rgb *= 2.0;
        sampled *= (texture1Present > 0.5)
            ? texture(texture1, textureCoordinate1) : vec4(0.0, 0.0, 0.0, 1.0);
    }
    if (environmentMapping > 0.5)
    {
        ...
        vec4 environmentSample = (environmentMapPresent > 0.5)
            ? texture(environmentMap, reflect(-eye, normal)) : vec4(0.0, 0.0, 0.0, 1.0);
    ...
    finalColor.rgb = mix(fogColor, finalColor.rgb, fragmentFogFactor);
        ...
        try
        {
            ...
            pipeline.texturePresentLocation =
                rlGetLocationUniform(pipeline.program, "texturePresent");
            pipeline.dualTextureLocation =
                rlGetLocationUniform(pipeline.program, "dualTexture");
            pipeline.texture1PresentLocation =
                rlGetLocationUniform(pipeline.program, "texture1Present");
            pipeline.environmentMappingLocation =
                rlGetLocationUniform(pipeline.program, "environmentMapping");
            pipeline.environmentMapPresentLocation =
                rlGetLocationUniform(pipeline.program, "environmentMapPresent");
            ...
            if (pipeline.worldViewProjectionLocation < 0 ||
                ...
                pipeline.textureEnabledLocation < 0 || pipeline.texturePresentLocation < 0 ||
                pipeline.dualTextureLocation < 0 || pipeline.texture1PresentLocation < 0 ||
                pipeline.environmentMappingLocation < 0 ||
                pipeline.environmentMapPresentLocation < 0 ||
        ...
        const bool thirtyTwoBitIndices)
    {
        ...
        SetCachedUniform(
            ...
            RL_SHADER_UNIFORM_FLOAT, 1);
        const float texturePresentFlag = params.texture0 != nullptr ? 1.0f : 0.0f;
        SetCachedUniform(
            pipeline, uniformCacheSlot,
            pipeline.texturePresentLocation, &texturePresentFlag,
            RL_SHADER_UNIFORM_FLOAT, 1);
        ...
        SetCachedUniform(
            ...
            RL_SHADER_UNIFORM_FLOAT, 1);
        const float texture1PresentFlag = params.texture1 != nullptr ? 1.0f : 0.0f;
        SetCachedUniform(
            pipeline, uniformCacheSlot,
            pipeline.texture1PresentLocation, &texture1PresentFlag,
            RL_SHADER_UNIFORM_FLOAT, 1);
        const float environmentMappingFlag = params.envMapping ? 1.0f : 0.0f;
        const float environmentMapPresentFlag = params.envMap != nullptr ? 1.0f : 0.0f;
        ...
        SetCachedUniform(
            pipeline, uniformCacheSlot,
            pipeline.environmentMapPresentLocation, &environmentMapPresentFlag,
            RL_SHADER_UNIFORM_FLOAT, 1);
        SetCachedUniform(
            pipeline, uniformCacheSlot,
