...
namespace CNA::Internal::Renderers::Rlgl
{
    namespace
    {
        ...
        public:
                ...
                , lifetime_(std::move(lifetime))
            {
                ...
                try
                {
                    ...
                    registered = true;
                    if (compressed_ || recoveryRegistered_)
                    {
            ...
            {
                if (!compressed_ || recoverySubresources_.empty()) return false;
                ...
                const int regionBlockColumns = (width + 3) / 4;
                const int regionBlockRows = (height + 3) / 4;
                const std::size_t required =
                    static_cast<std::size_t>(regionBlockColumns) * regionBlockRows * blockBytes_;
                if (dataLength < 0 || static_cast<std::size_t>(dataLength) < required)
                    return false;

                const std::size_t index = SubresourceIndex(face, level);
                if (!definedSubresources_[index]) return false;
                const auto& blocks = recoverySubresources_[index];
                auto* destination = static_cast<std::uint8_t*>(data);
                for (int row = 0; row < regionBlockRows; ++row)
                {
                    const std::size_t sourceOffset =
                        (static_cast<std::size_t>(y / 4 + row) * levelBlockColumns + x / 4) *
                        blockBytes_;
                    const std::size_t destinationOffset =
                        static_cast<std::size_t>(row) * regionBlockColumns * blockBytes_;
                    std::memcpy(destination + destinationOffset,
                                blocks.data() + sourceOffset,
                                static_cast<std::size_t>(regionBlockColumns) * blockBytes_);
                }
                return true;
            }
                ...
                const void* data, const int dataLength) override
            {
                ...
                std::vector<std::uint8_t> recoveryCopy;
                if (!recoverySubresources_.empty())
                {
        ...
        private:
            ...
            {
                if (recoverySubresources_.empty()) return;
                const std::size_t index = SubresourceIndex(face, level);
