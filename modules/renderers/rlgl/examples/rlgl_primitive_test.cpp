...
protected:
    void RunTest() override
    {
        ...
        device.SetVertexBuffers({
            ...
            VertexBufferBinding(&publicColor, 0, 0)});
        bool partialSemanticCollisionAccepted = true;
        try
        {
            device.DrawPrimitives(PrimitiveType::TriangleList, 0, 1);
        }
        ...
        {
            partialSemanticCollisionAccepted = false;
        }
        Check(partialSemanticCollisionAccepted,
              "a partially duplicate stream remaps its colliding Position0 and leaves unused "
              "TextureCoordinate0 unbound");
        ...
        Check(IsColor(read(kWidth / 2, kHeight / 2), Color(255, 128, 0, 255)) &&
              ...
              "a remapped partial collision leaves exact per-stream VBO state recoverable");
