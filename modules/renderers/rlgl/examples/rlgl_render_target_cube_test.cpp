...
protected:
    void RunTest() override
    {
        ...
        RenderTargetCube twoFaceCube(
            device, kTargetSize, false, SurfaceFormat::Color,
            DepthFormat::None, 4, RenderTargetUsage::PreserveContents);
        bool duplicateCubeRejected = false;
        try
        {
            device.SetRenderTargets({
                RenderTargetBinding(
        ...
        {
            duplicateCubeRejected = true;
        }
        Check(duplicateCubeRejected && device.GetRenderTargets().empty(),
              "two faces of one cube cannot occupy distinct MRT slots");
    }
