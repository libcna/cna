...
namespace
{
    ...
    [[nodiscard]] std::array<VertexPositionColorTexture, 3> ColoredTexturedTriangle(
        const float z, const Color& color, const Vector2& textureCoordinate)
    {
        return {
            VertexPositionColorTexture(
                Vector3(-0.75f, -0.75f, z), color, textureCoordinate),
            VertexPositionColorTexture(
                Vector3(0.75f, -0.75f, z), color, textureCoordinate),
            VertexPositionColorTexture(
                Vector3(0.0f, 0.75f, z), color, textureCoordinate)};
    }
}
...
protected:
    void RunTest() override
    {
        ...
        device.DrawUserPrimitives(PrimitiveType::TriangleList, textured.data(), 0, 1);
        ExpectPixel(
            "a null BasicEffect texture samples XNA opaque black",
            center, Color::Black);
        ...
        const auto alphaFogged = ColoredTexturedTriangle(
            -0.5f, Color(128, 255, 255, 255), Vector2::Zero);
        device.Clear(Color::Black);
        alphaTest.Apply();
        device.DrawUserPrimitives(
            PrimitiveType::TriangleList, alphaFogged.data(), 0, 1);
        ExpectPixel(
            "a null AlphaTestEffect texture samples opaque black before fog",
            center, Color(0, 0, 128, 255), 2);
