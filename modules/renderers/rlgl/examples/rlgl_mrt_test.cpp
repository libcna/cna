...
protected:
    void RunTest() override
    {
        ...
        const unsigned int activeBeforeResolve =
        ...
        Check(allCleared && activeBeforeResolve == previousFramebuffer &&
                  masksAfterClear.colorWriteMasks == std::array<int, 4>{1, 2, 4, 8},
              "Clear ignores all indexed write masks, restores them, and MRT resolves on unbind");
        ...
        RenderTarget2D packed(
            device, kTargetSize, kTargetSize, false, SurfaceFormat::Rgba1010102,
            DepthFormat::None, 0, RenderTargetUsage::PreserveContents);
        device.setDepthStencilStateProperty(DepthStencilState::None);
        device.SetRenderTargets(Bindings({&slot2, &packed}));
        const auto mixedNative = Bridge::GetBoundMrtFramebufferSnapshotForTesting();
        const auto packedResource = Rlgl::GetRenderTargetResourceSnapshotForTesting(
            *packed.GetRenderTargetRenderer());
        const Color mixedClear(64, 128, 192, 255);
        device.Clear(mixedClear);
        device.SetRenderTargets({});
        PackedVector::Rgba1010102 packedPixel;
        const Rectangle center(kTargetSize / 2, kTargetSize / 2, 1, 1);
        packed.GetData(0, &center, &packedPixel, 0, 1);
        const Vector4 packedVector = packedPixel.ToVector4();
        Check(mixedNative.complete &&
                  mixedNative.colorObjects[0] == resources[2].colorTexture &&
                  mixedNative.colorObjects[1] == packedResource.colorTexture &&
                  Matches(ReadPixel(slot2), mixedClear) &&
                  std::fabs(packedVector.X - 64.0f / 255.0f) < 0.002f &&
                  std::fabs(packedVector.Y - 128.0f / 255.0f) < 0.002f &&
                  std::fabs(packedVector.Z - 192.0f / 255.0f) < 0.002f &&
                  std::fabs(packedVector.W - 1.0f) < 0.002f,
              "equal-width mixed Color/Rgba1010102 attachments keep format-native storage");
