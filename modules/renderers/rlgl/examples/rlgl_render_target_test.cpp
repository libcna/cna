...
protected:
    void RunTest() override
    {
        ...
        const bool firstResolvedRead =
            Matches(ReadPixel(multisampled), Color(37, 113, 229, 73));
        device.SetRenderTarget(&multisampled);
        device.Clear(Color(205, 31, 77, 93));
        device.SetRenderTarget(static_cast<RenderTarget2D*>(nullptr));
        const bool repeatedResolvedRead =
            Matches(ReadPixel(multisampled), Color(205, 31, 77, 93));
        Check(firstResolvedRead && repeatedResolvedRead,
              "repeated MSAA pass boundaries resolve exactly and preserve later drawing");
        ...
        RasterizerState unscissored = scissored;
        unscissored.setScissorTestEnableProperty(false);
        device.setRasterizerStateProperty(unscissored);
        device.SetRenderTarget(static_cast<RenderTarget2D*>(nullptr));
        ...
        const auto depthAttachmentRejects = [&](RenderTarget2D& target)
        {
            ...
            device.SetRenderTarget(&target);
            DepthStencilState depthAlways = depthState;
