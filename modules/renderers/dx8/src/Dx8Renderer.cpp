...
// primary" trick DX1..DX7 all needed. Scope decision (made with the project owner before any code
// was written): fixed-function 3D only, matching DX1..DX7's own CPU-transform-and-submit shape --
...
namespace CNA::Internal::Renderers::Dx8
{
    ...
    namespace
    {
        ...
        // upload, matching DX2-DX7's own identical precedent.
        [[noreturn]] void ThrowMipLevelUnsupported(int level)
        {
            throw std::runtime_error(
                "DX8 does not support mip-level texture uploads (level " + std::to_string(level) +
        ...
        DWORD Dx8CompareFunctionToD3D(int compareFunction)
        {
        ...
        DWORD Dx8StencilOperationToD3D(int stencilOperation)
        {
        ...
        DWORD Dx8CullModeToD3D(int cullMode)
        {
        ...
        DWORD Dx8FillModeToD3D(int fillMode)
        {
        ...
        DWORD Dx8TextureAddressModeToD3D(int addressMode)
        {
        ...
        struct Dx8FilterPair { DWORD mag; DWORD min; };
        ...
        Dx8FilterPair Dx8TextureFilterToD3D(int textureFilter)
        {
        ...
        // detection dance is needed at all, unlike DX2..DX7's own DetectChannelLayout.
    ...
    // Common accessor so Dx8SpriteBatchRenderer/the 3D draw path can reach the underlying
    ...
    class Dx8TextureOwner
    {
    public:
        virtual ~Dx8TextureOwner() = default;
        [[nodiscard]] virtual IDirect3DTexture8* Texture() const = 0;
    };

    struct Dx8Renderer::Impl
    {
        ...
        // Dx8RenderTargetRenderer) and Present() draws THAT texture as a single full-screen quad
        // through the same fixed-function pipeline onto the real swap chain, letterbox-scaled --
        // matching DX1..DX7's own logical/virtual resolution + letterbox guarantee exactly, just
        ...
        // The offscreen surface owned by the currently-bound Dx8RenderTargetRenderer, or nullptr
        // when no custom render target is bound (the logical render target is active). Set
        // directly by Dx8RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a
    ...
    Dx8Renderer::Dx8Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx8Renderer initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx8Renderer: could not obtain a real HWND from the SDL window.");
    ...
    Dx8Renderer::~Dx8Renderer() = default;

    void Dx8Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx8Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->virtualWidth;
        height = impl_->virtualHeight;
    }

    void Dx8Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
        if (w <= 0 || h <= 0) return;
    ...
    void Dx8Renderer::SetVirtualResolution(int width, int height)
    {
        // Unlike D3D9Renderer's own vestigial handling of this field (its swap chain always
        // matches the real window, so nothing needs resizing), Dx8Renderer owns a REAL
    ...
    void Dx8Renderer::SetPresentationMode(int mode)
    {
        impl_->presentationMode = static_cast<CnaPresentationMode>(mode);
    }

    SDL_Window* Dx8Renderer::GetWindowInternal() const
    {
        return impl_->window;
    }
    ...
    class Dx8TextureRenderer final : public ITextureRenderer, public Dx8TextureOwner
    {
    public:
        Dx8TextureRenderer(IDirect3DDevice8* device, const ImageData& data)
        ...
        ~Dx8TextureRenderer() override { if (texture_) texture_->Release(); }
    ...
    class Dx8RenderTargetRenderer final : public IRenderTargetRenderer, public Dx8TextureOwner
    {
    public:
        Dx8RenderTargetRenderer(IDirect3DSurface8** currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx8RenderTargetRenderer() override
        {
    ...
    std::unique_ptr<ITextureRenderer> Dx8Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx8TextureRenderer>(impl_->device8, data);
    }

    std::unique_ptr<IRenderTargetRenderer> Dx8Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx8RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->device8, w, h, multiSampleCount);
    }

    void Dx8Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void Dx8Renderer::SetRenderTargets(
    ...
    // resize is correct on the very next call -- same convention DX1..DX7's own ComputeLetterbox
    ...
    void Dx8Renderer::Present()
    {
        ...
        // to Dx8SpriteBatchRenderer::Draw's own quad corners).
    ...
    bool Dx8Renderer::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx8Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    class Dx8SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        Dx8SpriteBatchRenderer(IDirect3DDevice8* device, std::function<void(int&, int&)> getActiveSurfaceSize)
        ...
        void Begin() override
        {
            if (begun_) throw std::runtime_error("Dx8SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        void End() override
        {
            if (!begun_) throw std::runtime_error("Dx8SpriteBatchRenderer::End: End() called without a matching Begin()");
            FlushBatch();
            begun_ = false;
        }
        // Applied as a point transform (z=0) directly on the already-screen-space quad corners,
        // same convention SOFTWARE/DX2..DX7's own SetTransformMatrix uses.
        void SetTransformMatrix(const Matrix& m) override { transformMatrix_ = m; }
        void SetCustomEffect(Effect* effect) override
        {
            if (effect != nullptr)
                throw std::runtime_error(
                    "DX8 (fixed-function) does not support custom SpriteBatch Effects: no "
        ...
        {
            if (!begun_) throw std::runtime_error("Dx8SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx8TextureOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx8SpriteBatchRenderer::Draw: texture renderer is not a DX8 texture (created by "
            ...
            const auto placeCorner = [&](float px, float py) -> Vector2
            {
                ...
                // corner, same convention SOFTWARE/DX2..DX7 already established.
    ...
    private:
        void FlushBatch()
        {
            ...
            const auto* owner = dynamic_cast<const Dx8TextureOwner*>(currentTexture_);
            ...
            const Dx8FilterPair filterPair = Dx8TextureFilterToD3D(filter_);
            ...
            device_->SetTextureStageState(0, D3DTSS_ADDRESSU, Dx8TextureAddressModeToD3D(addressU_));
            device_->SetTextureStageState(0, D3DTSS_ADDRESSV, Dx8TextureAddressModeToD3D(addressV_));
            ...
            hr = device_->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0,
                ...
                pendingIndices_.data(), D3DFMT_INDEX16, pendingVertices_.data(), sizeof(Dx8TLVertex));
            if (FAILED(hr))
            {
                device_->EndScene();
        ...
        std::vector<Dx8TLVertex> pendingVertices_;
    ...
    std::unique_ptr<ISpriteBatchRenderer> Dx8Renderer::CreateSpriteBatch()
    {
        return std::make_unique<Dx8SpriteBatchRenderer>(impl_->device8,
    ...
    void Dx8Renderer::ApplyBlendState(int colorSrcBlend, int /*alphaSrcBlend*/,
    ...
    void Dx8Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
                                                    ...
                                                    int /*ccwStencilFail*/, int /*ccwStencilDepthFail*/)
    {
        ...
        impl_->device8->SetRenderState(D3DRS_ZFUNC, Dx8CompareFunctionToD3D(depthFunc));

        impl_->device8->SetRenderState(D3DRS_STENCILENABLE, stencilEnable ? TRUE : FALSE);
        impl_->device8->SetRenderState(D3DRS_STENCILFUNC, Dx8CompareFunctionToD3D(stencilFunc));
        impl_->device8->SetRenderState(D3DRS_STENCILFAIL, Dx8StencilOperationToD3D(stencilFail));
        impl_->device8->SetRenderState(D3DRS_STENCILZFAIL, Dx8StencilOperationToD3D(stencilDepthFail));
        impl_->device8->SetRenderState(D3DRS_STENCILPASS, Dx8StencilOperationToD3D(stencilPass));
    ...
    void Dx8Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
                                                  float /*depthBias*/, float /*slopeScaleDepthBias*/)
    {
        impl_->device8->SetRenderState(D3DRS_CULLMODE, Dx8CullModeToD3D(cullMode));
        impl_->device8->SetRenderState(D3DRS_FILLMODE, Dx8FillModeToD3D(fillMode));
    }

    void Dx8Renderer::ApplySamplerState(int slot, int filter, int addressU, int addressV,
                                               int maxAnisotropy)
    {
        if (slot != 0) return; // fixed-function scope: exactly one texture stage
        const Dx8FilterPair filterPair = Dx8TextureFilterToD3D(filter);
        ...
        impl_->device8->SetTextureStageState(0, D3DTSS_ADDRESSU, Dx8TextureAddressModeToD3D(addressU));
        impl_->device8->SetTextureStageState(0, D3DTSS_ADDRESSV, Dx8TextureAddressModeToD3D(addressV));
        impl_->device8->SetTextureStageState(0, D3DTSS_MAXANISOTROPY, static_cast<DWORD>(maxAnisotropy));
    }

    void Dx8Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
    ...
    void Dx8Renderer::ClearDepth(float depth)
    {
    ...
    void Dx8Renderer::ClearStencil(int stencil)
    {
    ...
    void Dx8Renderer::ClearDepthAndStencil(float depth, int stencil)
    {
    ...
    void Dx8Renderer::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
    ...
    void Dx8Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
    ...
    void Dx8Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void Dx8Renderer::SetBlendEnabled(bool) {}
    void Dx8Renderer::SetDepthWriteEnabled(bool enabled)
    {
    ...
    class Dx8VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit Dx8VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}
        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx8VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx8VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class Dx8IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        Dx8IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx8IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx8IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    std::unique_ptr<IVertexBufferRenderer> Dx8Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx8VertexBufferRenderer>(vertex_capacity);
    }
    std::unique_ptr<IIndexBufferRenderer> Dx8Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx8IndexBufferRenderer>(index_capacity, false);
    }
    std::unique_ptr<IIndexBufferRenderer> Dx8Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx8IndexBufferRenderer>(index_capacity, true);
    }

    // ---- CPU transform + near-plane clip pipeline, fixed-function submission (decisions 6/8) ----
    // Ported conceptually from DX7's own Dx7ClipVertex/Dx7ClipTriangleNearPlane/
    // Dx7BuildPositionColorClipVertex/Dx7BuildGenericClipVertex/Dx7ClipVertexToD3DTLVERTEX -- same
    // math, re-expressed against Dx8TLVertex/DrawIndexedPrimitiveUP instead of
    // D3DTLVERTEX/DrawIndexedPrimitive.

    struct Dx8ClipVertex
    {
    ...
    Dx8ClipVertex Dx8LerpClipVertex(const Dx8ClipVertex& a, const Dx8ClipVertex& b, float t)
    {
        Dx8ClipVertex out;
    ...
    int Dx8ClipTriangleNearPlane(const Dx8ClipVertex verts[3], Dx8ClipVertex out[4])
    {
        constexpr float kNearEpsilon = 1e-5f;
        int count = 0;
        for (int i = 0; i < 3; ++i)
        {
            const Dx8ClipVertex& cur = verts[i];
            const Dx8ClipVertex& prev = verts[(i + 2) % 3];
            const bool curIn = cur.w > kNearEpsilon;
            const bool prevIn = prev.w > kNearEpsilon;
            if (curIn != prevIn)
            {
                const float t = (kNearEpsilon - prev.w) / (cur.w - prev.w);
                out[count++] = Dx8LerpClipVertex(prev, cur, t);
            }
    ...
    Dx8ClipVertex Dx8BuildPositionColorClipVertex(const uint8_t* raw, const Matrix& combined)
    {
        Vector3 position;
    ...
    Dx8LitColor Dx8ComputeVertexLighting(const Vector3& localPosition, const Vector3& localNormal,
    ...
    Dx8ClipVertex Dx8BuildGenericClipVertex(const uint8_t* raw, std::size_t stride, const Matrix& combined,
                                            const Matrix& world, const GpuDrawParams& params)
    {
        Vector3 position;
        std::memcpy(&position, raw, sizeof(Vector3));
        ...
        else if (stride == 32 || stride == 52)
        {
            ...
            if (params.lightingEnabled)
            {
                ...
                const Dx8LitColor litColor = Dx8ComputeVertexLighting(position, normal, world, params);
    ...
    Dx8TLVertex Dx8ClipVertexToDx8TLVertex(const Dx8ClipVertex& cv, int viewportWidth, int viewportHeight)
    {
    ...
    IDirect3DTexture8* Dx8ResolveTexture(const ITextureRenderer* texture)
    {
        if (!texture) return nullptr;
        const auto* owner = dynamic_cast<const Dx8TextureOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx8Renderer: texture0 is not a DX8 texture (created by a different graphics renderer?)");
        return owner->Texture();
    }

    /// Shared core for all 4 draw entry points: clips each triangle, packs into Dx8TLVertex, and
    ...
    void SubmitDx8Primitives(IDirect3DDevice8* device, int viewportWidth, int viewportHeight,
                             const std::function<Dx8ClipVertex(int)>& fetchVertex,
                             int primitiveCount, IDirect3DTexture8* texture, bool specularEnabled)
    {
        std::vector<Dx8TLVertex> verts;
        ...
        for (int i = 0; i < primitiveCount; ++i)
        {
            ...
            Dx8ClipVertex clipped[4];
            const int clippedCount = Dx8ClipTriangleNearPlane(cv, clipped);
            if (clippedCount == 0) continue;

            const auto baseIdx = static_cast<WORD>(verts.size());
            for (int k = 0; k < clippedCount; ++k)
                verts.push_back(Dx8ClipVertexToDx8TLVertex(clipped[k], viewportWidth, viewportHeight));
        ...
        hr = device->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, static_cast<UINT>(verts.size()),
            static_cast<UINT>(indices.size() / 3), indices.data(), D3DFMT_INDEX16, verts.data(), sizeof(Dx8TLVertex));
        if (FAILED(hr))
        {
            device->EndScene();
    ...
    void Dx8CheckDrawPreconditions(const char* who, bool customRenderTargetBound,
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx8Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx8Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx8Renderer::") + who + ": only TriangleList is supported");
    }

    void Dx8Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   const Matrix& world, const Matrix& view, const Matrix& projection,
                                                   PrimitiveType primitive, int primitiveCount)
    {
        Dx8CheckDrawPreconditions("DrawColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx8Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx8VertexBufferRenderer&>(vb);
        ...
        SubmitDx8Primitives(impl_->device8, vw, vh,
            [&](int i) { return Dx8BuildPositionColorClipVertex(base + static_cast<std::size_t>(i) * stride, combined); },
            primitiveCount, nullptr, false);
    }

    void Dx8Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          const IIndexBufferRenderer& ib,
                                                          const Matrix& world, const Matrix& view, const Matrix& projection,
                                                          PrimitiveType primitive, int primitiveCount)
    {
        Dx8CheckDrawPreconditions("DrawIndexedColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx8Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx8VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const Dx8IndexBufferRenderer&>(ib);
        ...
        SubmitDx8Primitives(impl_->device8, vw, vh,
            [&](int i) {
                ...
                return Dx8BuildPositionColorClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, combined);
            },
            primitiveCount, nullptr, false);
    }
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DX8", route);
    }

    void Dx8Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                             const Matrix& world, const Matrix& view, const Matrix& projection,
                                             PrimitiveType primitive, int primitiveCount,
                                             const GpuDrawParams& params)
    {
        ...
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx8Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx8Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx8VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx8Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported)");

        const uint8_t* base = dxVb.Data().data();
        const Matrix combined = world * view * projection;
        int vw = 0, vh = 0;
        impl_->ActiveSurfaceSize(vw, vh);
        IDirect3DTexture8* texture = params.textureEnabled ? Dx8ResolveTexture(params.texture0) : nullptr;
        const int vertexStart = params.vertexStart;

        SubmitDx8Primitives(impl_->device8, vw, vh,
            [&](int i) { return Dx8BuildGenericClipVertex(base + static_cast<std::size_t>(vertexStart + i) * stride,
                                                          stride, combined, world, params); },
            primitiveCount, texture, params.lightingEnabled);
    }

    void Dx8Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        ...
        Dx8CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx8Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx8Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx8Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported)");
        ...
        SubmitDx8Primitives(impl_->device8, vw, vh,
            [&](int i) {
                ...
                return Dx8BuildGenericClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, stride,
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<Dx8::Dx8Renderer>(args);
    }
}
