...
// plan_dx8.md: real DirectX 8 graphics renderer -- architecturally very different from DX1..DX7.
...
// business leaking into every translation unit that merely wants to hold a Dx8Renderer
// pointer. All real <d3d8.h> usage lives in Dx8Renderer.cpp behind the Impl pimpl below.
...
{
    /**
     * DX8 graphics renderer (plan_dx8.md): real DirectX 8 (IDirect3D8/IDirect3DDevice8), delivered
     ...
     * modeled on D3D9Renderer's own shape rather than DX1..DX7's DirectDraw-based one): a
     ...
     *
     * Textures and render targets are real Direct3D8 resources (Dx8TextureRenderer/
     * Dx8RenderTargetRenderer, defined entirely in Dx8Renderer.cpp -- neither is ever named
     ...
     * to whichever surface is currently bound. The SpriteBatch compositor (Dx8SpriteBatchRenderer,
     ...
     */
    class Dx8Renderer final : public IGraphicsRenderer
    {
    public:
        explicit Dx8Renderer(const GraphicsRendererCreateArgs& args);
        ~Dx8Renderer() override;

        Dx8Renderer(const Dx8Renderer&) = delete;
        Dx8Renderer& operator=(const Dx8Renderer&) = delete;
        ...
        // Design decision 11: real GPU-rendered textured quads (Dx8SpriteBatchRenderer), not a
        ...
        // Design decision 10: plain CPU-side storage (Dx8VertexBufferRenderer/
        // Dx8IndexBufferRenderer, defined entirely in Dx8Renderer.cpp), matching this
