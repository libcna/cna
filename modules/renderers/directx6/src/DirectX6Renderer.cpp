...
namespace CNA::Internal::Renderers::DirectX6
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both DirectX6TextureRenderer and
        // DirectX6RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
        ...
        // DirectX6_Smoke/DirectX6_Blend/etc.'s Clear()+GetBackBufferData() checks all still passed), but a
        ...
        [[noreturn]] void ThrowMipLevelUnsupported(int level)
        {
            throw std::runtime_error(
                "DIRECTX6 (DirectDraw v4) does not support mip-level texture uploads (level " +
        ...
        enum class DirectX6BlendMode { Opaque, AlphaBlend, NonPremultiplied, Additive };
        ...
        // found-and-fixed bug).
        DirectX6BlendMode DetectBlendMode(int colorSrc, int alphaSrc, int colorDst, int alphaDst,
                                     int colorFunc, int alphaFunc)
        {
            ...
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 5 && alphaDst == 5) return DirectX6BlendMode::NonPremultiplied;
            // Additive: ColorSrc=SourceAlpha, AlphaSrc=SourceAlpha, ColorDst=One, AlphaDst=One
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 0 && alphaDst == 0) return DirectX6BlendMode::Additive;
            // Opaque: ColorSrc=One, AlphaSrc=One, ColorDst=Zero, AlphaDst=Zero
            if (bothAdd && colorSrc == 0 && alphaSrc == 0 && colorDst == 1 && alphaDst == 1) return DirectX6BlendMode::Opaque;
            ...
            return DirectX6BlendMode::AlphaBlend;
        }
        ...
        DWORD DirectX6BlendToD3D(int blend)
        {
        ...
        DWORD DirectX6CompareFunctionToD3D(int compareFunction)
        {
        ...
        DWORD DirectX6StencilOperationToD3D(int stencilOperation)
        {
        ...
        DWORD DirectX6CullModeToD3D(int cullMode)
        {
        ...
        DWORD DirectX6FillModeToD3D(int fillMode)
        {
        ...
        DWORD DirectX6TextureAddressModeToD3D(int addressMode)
        {
        ...
        struct DirectX6FilterPair { DWORD mag; DWORD min; };
        ...
        DirectX6FilterPair DirectX6TextureFilterToD3D(int textureFilter)
        {
        ...
        void CompositeQuad(uint8_t* dstBase, int dstPitch, int dstW, int dstH,
                           ...
                           DirectX6BlendMode blendMode, int filter, int addressU, int addressV)
        {
                ...
                for (int x = x0; x < x1; ++x)
                {
                    ...
                    for (const auto& tri : kTriangles)
                    {
                        ...
                        case DirectX6BlendMode::Opaque:
                            ...
                            break;
                        case DirectX6BlendMode::AlphaBlend:
                        {
                        ...
                        case DirectX6BlendMode::NonPremultiplied:
                        {
                        ...
                        case DirectX6BlendMode::Additive:
    ...
    // Common accessor so DirectX6SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as DirectX6TextureRenderer/DirectX6RenderTargetRenderer themselves.
    class DirectX6SurfaceOwner
    {
    public:
        virtual ~DirectX6SurfaceOwner() = default;
        [[nodiscard]] virtual LPDIRECTDRAWSURFACE4 Surface() const = 0;
    };

    struct DirectX6Renderer::Impl
    {
        explicit Impl(const RendererSurfaceInfo& surfaceInfo)
            : surface(surfaceInfo, "DirectX6Renderer")
        {
        }
        ...
        // Phase O3: the offscreen surface owned by the currently-bound DirectX6RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by DirectX6RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE4 rather than
        // a DirectX6RenderTargetRenderer* so DirectX6RenderTargetRenderer never needs to name this private Impl
        ...
        DirectX6BlendMode currentBlendMode = DirectX6BlendMode::AlphaBlend;
        ...
        : impl_(std::make_unique<Impl>(args.surface))
    {
        ...
        // DIRECTX3's more limited "upgrade only the top object" change) for everything from this point
        // on.
    ...
    DirectX6Renderer::~DirectX6Renderer() = default;

    void DirectX6Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void DirectX6Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("DirectX6Renderer::Present: ClientToScreen failed");
    ...
    void DirectX6Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void DirectX6Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void DirectX6Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void DirectX6Renderer::SetPresentationMode(int mode)
    {
    ...
    void DirectX6Renderer::OnSurfaceChanged(const RendererSurfaceInfo& surface)
    {
    ...
    bool DirectX6Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    class DirectX6VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit DirectX6VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("DirectX6VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("DirectX6VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class DirectX6IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        DirectX6IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("DirectX6IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("DirectX6IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    DirectX6ClipVertex DirectX6LerpClipVertex(const DirectX6ClipVertex& a, const DirectX6ClipVertex& b, float t)
    {
        DirectX6ClipVertex out;
        out.x = a.x + t * (b.x - a.x);
        out.y = a.y + t * (b.y - a.y);
        out.z = a.z + t * (b.z - a.z);
        out.w = a.w + t * (b.w - a.w);
    ...
    int DirectX6ClipTriangleNearPlane(const DirectX6ClipVertex verts[3], DirectX6ClipVertex out[4])
    {
        constexpr float kNearEpsilon = 1e-5f;
        int count = 0;
        for (int i = 0; i < 3; ++i)
        {
            const DirectX6ClipVertex& cur = verts[i];
            const DirectX6ClipVertex& prev = verts[(i + 2) % 3];
            const bool curIn = cur.w > kNearEpsilon;
            const bool prevIn = prev.w > kNearEpsilon;
            if (curIn != prevIn)
            {
                const float t = (kNearEpsilon - prev.w) / (cur.w - prev.w);
                out[count++] = DirectX6LerpClipVertex(prev, cur, t);
            }
            if (curIn)
                out[count++] = cur;
        }
    ...
    DirectX6ClipVertex DirectX6BuildPositionColorClipVertex(const uint8_t* raw, const Matrix& combined)
    {
        Vector3 position;
    ...
    /// Result of DirectX6ComputeVertexLighting: the diffuse (base vertex color) and specular (additive,
    ...
    DirectX6LitColor DirectX6ComputeVertexLighting(const Vector3& localPosition, const Vector3& localNormal,
    ...
    /// normal (32/52), DirectX6ComputeVertexLighting() replaces the raw/white vertex color with real
    ...
    DirectX6ClipVertex DirectX6BuildGenericClipVertex(const uint8_t* raw, std::size_t stride, const Matrix& combined,
                                            const Matrix& world, const GpuDrawParams& params)
    {
        Vector3 position;
        std::memcpy(&position, raw, sizeof(Vector3));
        ...
        else if (stride == 32 || stride == 52)
        {
            ...
            if (params.lightingEnabled)
            {
                ...
                const DirectX6LitColor litColor = DirectX6ComputeVertexLighting(position, normal, world, params);
    ...
    /// (not cached) rather than stored on DirectX6TextureRenderer: a handle is only valid for the device
    /// it was obtained against, and this renderer's device3 is torn down and recreated whenever the
    ...
    D3DTEXTUREHANDLE DirectX6ResolveTextureHandle(const ITextureRenderer* texture, LPDIRECT3DDEVICE3 device3)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const DirectX6SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "DirectX6Renderer: texture0 is not a DIRECTX6 surface (created by a different graphics renderer?)");
    ...
    /// Shared core for all 4 draw entry points: clips each triangle (DirectX6ClipTriangleNearPlane),
    /// packs the result into D3DTLVERTEX (DirectX6ClipVertexToD3DTLVERTEX), and submits via a single
    ...
    void SubmitDx6Primitives(LPDIRECT3DDEVICE3 device3, int viewportWidth, int viewportHeight,
                             const std::function<DirectX6ClipVertex(int)>& fetchVertex,
                             int primitiveCount, D3DTEXTUREHANDLE texHandle, bool specularEnabled)
    {
        ...
        {
            DirectX6ClipVertex cv[3];
            for (int k = 0; k < 3; ++k)
                cv[k] = fetchVertex(i * 3 + k);

            DirectX6ClipVertex clipped[4];
            const int clippedCount = DirectX6ClipTriangleNearPlane(cv, clipped);
            if (clippedCount == 0) continue;

            const auto baseIdx = static_cast<WORD>(verts.size());
            for (int k = 0; k < clippedCount; ++k)
                verts.push_back(DirectX6ClipVertexToD3DTLVERTEX(clipped[k], viewportWidth, viewportHeight));
    ...
    /// target's surface).
    void DirectX6CheckDrawPreconditions(const char* who, bool customRenderTargetBound,
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("DirectX6Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("DirectX6Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("DirectX6Renderer::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class DirectX6TextureRenderer : public ITextureRenderer, public DirectX6SurfaceOwner
    {
    public:
        DirectX6TextureRenderer(LPDIRECTDRAW4 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        DirectX6TextureRenderer(LPDIRECTDRAW4 dd, const ImageData& data)
            : DirectX6TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~DirectX6TextureRenderer() override { if (surface_) surface_->Release(); }

        DirectX6TextureRenderer(const DirectX6TextureRenderer&) = delete;
        DirectX6TextureRenderer& operator=(const DirectX6TextureRenderer&) = delete;
    ...
    class DirectX6RenderTargetRenderer final : public IRenderTargetRenderer, public DirectX6SurfaceOwner
    {
    public:
        DirectX6RenderTargetRenderer(LPDIRECTDRAWSURFACE4* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~DirectX6RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        DirectX6RenderTargetRenderer(const DirectX6RenderTargetRenderer&) = delete;
        DirectX6RenderTargetRenderer& operator=(const DirectX6RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> DirectX6Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<DirectX6TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> DirectX6Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<DirectX6RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void DirectX6Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void DirectX6Renderer::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        // DirectDraw has no multi-render-target concept -- single active surface only.
        if (count > 1)
            throw std::runtime_error(
                "DIRECTX6 (DirectDraw v4) does not support multiple simultaneous render targets (MRT): "
        ...
        if (count > 0 && renderTargets[0].IsRenderTargetCubeFace())
            throw std::runtime_error(
                "DIRECTX6 (DirectDraw v4) does not support RenderTargetCube face bindings.");
    ...
    // Never named outside this .cpp, same reasoning as DirectX6TextureRenderer/DirectX6RenderTargetRenderer.

    class DirectX6SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        DirectX6SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE4()> getActiveSurface,
                              std::function<void(int&, int&)> getActiveSurfaceSize,
                              std::function<DirectX6BlendMode()> getBlendMode)
        ...
        {
            if (begun_)
                throw std::runtime_error("DirectX6SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("DirectX6SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        void SetCustomEffect(Effect* effect) override
        {
            if (effect != nullptr)
                throw std::runtime_error(
                    "DIRECTX6 (DirectDraw v4) does not support custom SpriteBatch Effects: no "
        ...
        {
            if (!begun_)
                throw std::runtime_error("DirectX6SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const DirectX6SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "DirectX6SpriteBatchRenderer::Draw: texture renderer is not a DIRECTX6 surface (created by "
            ...
            if (isIdentityGeometry && isWhiteTint && getBlendMode_() == DirectX6BlendMode::Opaque)
            {
    ...
    private:
        ...
        std::function<DirectX6BlendMode()> getBlendMode_;
        bool begun_ = false;
    ...
    std::unique_ptr<ISpriteBatchRenderer> DirectX6Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<DirectX6SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void DirectX6Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
                                             ...
                                             const BlendWriteState& /*writeState*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_SRCBLEND, DirectX6BlendToD3D(colorSrcBlend));
        impl_->device3->SetRenderState(D3DRENDERSTATE_DESTBLEND, DirectX6BlendToD3D(colorDstBlend));
    }

    void DirectX6Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
                                                    ...
                                                    int /*ccwStencilFail*/, int /*ccwStencilDepthFail*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILFUNC, DirectX6CompareFunctionToD3D(stencilFunc));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILFAIL, DirectX6StencilOperationToD3D(stencilFail));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILZFAIL, DirectX6StencilOperationToD3D(stencilDepthFail));
        impl_->device3->SetRenderState(D3DRENDERSTATE_STENCILPASS, DirectX6StencilOperationToD3D(stencilPass));
    ...
    void DirectX6Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
                                                  float /*depthBias*/, float /*slopeScaleDepthBias*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_CULLMODE, DirectX6CullModeToD3D(cullMode));
        impl_->device3->SetRenderState(D3DRENDERSTATE_FILLMODE, DirectX6FillModeToD3D(fillMode));
    }

    void DirectX6Renderer::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
        ...
        const DirectX6FilterPair filterPair = DirectX6TextureFilterToD3D(filter);
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_TEXTUREADDRESS, DirectX6TextureAddressModeToD3D(addressU));
    ...
    void DirectX6Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }
    ...
    void DirectX6Renderer::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void DirectX6Renderer::ClearStencil(int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
    ...
    void DirectX6Renderer::ClearDepthAndStencil(float depth, int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
    ...
    void DirectX6Renderer::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        ClearStencil(stencil);
    }
    void DirectX6Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
        Clear(r, g, b, a);
        ClearDepthAndStencil(depth, stencil);
    }
    void DirectX6Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void DirectX6Renderer::SetBlendEnabled(bool) {}

    void DirectX6Renderer::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferRenderer> DirectX6Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<DirectX6VertexBufferRenderer>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> DirectX6Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<DirectX6IndexBufferRenderer>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> DirectX6Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<DirectX6IndexBufferRenderer>(index_capacity, true);
    }

    void DirectX6Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   const Matrix& world, const Matrix& view, const Matrix& projection,
                                                   PrimitiveType primitive, int primitiveCount)
    {
        DirectX6CheckDrawPreconditions("DrawColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "DirectX6Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX6VertexBufferRenderer&>(vb);
        ...
        SubmitDx6Primitives(impl_->device3, vw, vh,
            [&](int i) { return DirectX6BuildPositionColorClipVertex(base + static_cast<std::size_t>(i) * stride, combined); },
            primitiveCount, 0, false);
    }

    void DirectX6Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          const IIndexBufferRenderer& ib,
                                                          const Matrix& world, const Matrix& view, const Matrix& projection,
                                                          PrimitiveType primitive, int primitiveCount)
    {
        DirectX6CheckDrawPreconditions("DrawIndexedColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "DirectX6Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX6VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const DirectX6IndexBufferRenderer&>(ib);
        ...
        SubmitDx6Primitives(impl_->device3, vw, vh,
            [&](int i) {
                ...
                return DirectX6BuildPositionColorClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, combined);
            },
            primitiveCount, 0, false);
    }
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DIRECTX6", route);
    }

    void DirectX6Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-nonindexed");
        DirectX6CheckDrawPreconditions("DrawPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("DirectX6Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "DirectX6Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX6VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "DirectX6Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        const D3DTEXTUREHANDLE texHandle = params.textureEnabled
            ? DirectX6ResolveTextureHandle(params.texture0, impl_->device3) : 0;
        const int vertexStart = params.vertexStart;

        SubmitDx6Primitives(impl_->device3, vw, vh,
            [&](int i) { return DirectX6BuildGenericClipVertex(base + static_cast<std::size_t>(vertexStart + i) * stride,
                                                          stride, combined, world, params); },
            primitiveCount, texHandle, params.lightingEnabled);
    }

    void DirectX6Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-indexed");
        DirectX6CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("DirectX6Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "DirectX6Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "DirectX6Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        SubmitDx6Primitives(impl_->device3, vw, vh,
            [&](int i) {
                ...
                return DirectX6BuildGenericClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, stride,
