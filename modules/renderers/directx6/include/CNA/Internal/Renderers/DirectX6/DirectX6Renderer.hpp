...
// a DirectX6Renderer pointer. All real <ddraw.h> usage lives in DirectX6Renderer.cpp behind the
// Impl pimpl below.
...
namespace CNA::Internal::Renderers::DirectX6
{
     ...
     * via the D3DFVF_TLVERTEX FVF bitmask) -- the EXACT SAME interfaces DIRECTX5 already uses, since
     * DIRECTX6 introduces no new COM interface revision at all. Device/window bring-up:
     * DirectDrawCreate -> QueryInterface(IID_IDirectDraw4) -> SetCooperativeLevel(DDSCL_NORMAL,
     * against a real Win32 HWND supplied in RendererSurfaceInfo) -> primary CreateSurface. No
     * SetDisplayMode call is ever
     ...
     * (DirectX6TextureRenderer/DirectX6RenderTargetRenderer, defined entirely in DirectX6Renderer.cpp --
     ...
     * (DirectX6SpriteBatchRenderer): IDirectDrawSurface4::Blt/BltFast has never supported rotation in any
     ...
     * genuine write-then-test behavior -- resolving the "no stencil until DIRECTX6" boundary
     * DIRECTX2/DIRECTX3/DIRECTX5 all documented. Multitexture stays accepted-and-ignored (design decision 6 --
     ...
     */
    class DirectX6Renderer final : public IGraphicsRenderer
    {
    public:
        explicit DirectX6Renderer(const GraphicsRendererCreateArgs& args);
        ~DirectX6Renderer() override;

        DirectX6Renderer(const DirectX6Renderer&) = delete;
        DirectX6Renderer& operator=(const DirectX6Renderer&) = delete;
        ...
        // Phase O5 (design decision 8): plain CPU-side storage (DirectX6VertexBufferRenderer/
        // DirectX6IndexBufferRenderer, defined entirely in DirectX6Renderer.cpp), matching the
