...
namespace CNA::Internal::Renderers::Skia
{
    ...
    SkiaEffectRenderer::~SkiaEffectRenderer() = default;

    const char* SkiaEffectRenderer::UniformKindName(UniformKind kind) noexcept
    {
        switch (kind)
        {
    ...
    int SkiaEffectRenderer::UniformTypeOrdinal(UniformKind kind) noexcept
    {
        using Type = SkRuntimeEffect::Uniform::Type;
        switch (kind)
        {
    ...
    void SkiaEffectRenderer::Fail(std::string message)
    {
        effect_.reset();
        uniformBytes_.clear();
        ...
        boundCubeRenderer_.reset();
        boundVolumeRenderer_.reset();
    }

    bool SkiaEffectRenderer::CompileProgram(const std::string& vertSrc, const std::string& fragSrc)
    {
        Fail({});
    ...
    void SkiaEffectRenderer::Unbind()
    {
        bound_ = false;
    }

    void SkiaEffectRenderer::SetUniformData(
    ...
    void SkiaEffectRenderer::SetUniformFloat(const char* name, float value)
    {
        SetUniformData(name, UniformKind::Float, false, 0, &value, sizeof(value), "SetUniformFloat");
    }

    void SkiaEffectRenderer::SetUniformInt(const char* name, int value)
    {
    ...
    void SkiaEffectRenderer::SetUniformVec2(const char* name, float x, float y)
    {
        const float value[2] = {x, y};
        SetUniformData(name, UniformKind::Float2, false, 0, value, sizeof(value), "SetUniformVec2");
    }

    void SkiaEffectRenderer::SetUniformVec3(const char* name, float x, float y, float z)
    {
        const float value[3] = {x, y, z};
        SetUniformData(name, UniformKind::Float3, false, 0, value, sizeof(value), "SetUniformVec3");
    }

    void SkiaEffectRenderer::SetUniformVec4(const char* name, float x, float y, float z, float w)
    {
        const float value[4] = {x, y, z, w};
        SetUniformData(name, UniformKind::Float4, false, 0, value, sizeof(value), "SetUniformVec4");
    }

    void SkiaEffectRenderer::SetUniformMat4(const char* name, const float* matrix)
    {
        SetUniformData(name, UniformKind::Float4x4, false, 0, matrix, sizeof(float) * 16u,
                       "SetUniformMat4");
    }

    void SkiaEffectRenderer::SetUniformFloatArray(const char* name, const float* values, int count)
    {
    ...
    void SkiaEffectRenderer::SetUniformVec2Array(const char* name, const float* values, int count)
    {
        const std::size_t byteCount = count >= 0
    ...
    void SkiaEffectRenderer::BindTexture(int unit, ITextureRenderer* texture)
    {
        if (!effect_)
        ...
        if (childIndexByUnit_[static_cast<std::size_t>(unit)] < 0)
            throw std::invalid_argument(std::string("Skia effect does not declare `cnaTexture")
                                        + std::to_string(unit) + "`.");
        if (!texture)
            throw std::invalid_argument("Skia BindTexture received a null Texture2D renderer.");
        ...
        std::weak_ptr<ITextureRenderer> weakTexture = texture->weak_from_this();
        if (weakTexture.expired())
            throw std::invalid_argument("Skia BindTexture requires a shared, lifetime-tracked Texture2D renderer.");
        // Retain no raw pointer or immutable image. Draw locks this weak renderer and snapshots its
    ...
    void SkiaEffectRenderer::BindTextureCube(int unit, ITextureCubeRenderer* texture)
    {
        if (!effect_)
        ...
        if (!texture)
            throw std::invalid_argument("Skia BindTextureCube received a null TextureCube renderer.");
        std::weak_ptr<ITextureCubeRenderer> weakTexture = texture->weak_from_this();
        if (weakTexture.expired())
        {
            throw std::invalid_argument(
                "Skia BindTextureCube requires a shared, lifetime-tracked TextureCube renderer.");
        }
        // Retain no raw pointer or immutable image. Draw locks this weak renderer and reads its
        ...
        boundCubeRenderer_ = std::move(weakTexture);
    }

    void SkiaEffectRenderer::BindTexture3D(int unit, ITexture3DRenderer* texture)
    {
        if (!effect_)
        ...
        if (!texture)
            throw std::invalid_argument("Skia BindTexture3D received a null Texture3D renderer.");
        std::weak_ptr<ITexture3DRenderer> weakTexture = texture->weak_from_this();
        if (weakTexture.expired())
        {
            throw std::invalid_argument(
                "Skia BindTexture3D requires a shared, lifetime-tracked Texture3D renderer.");
        }
        ...
        boundVolumeRenderer_ = std::move(weakTexture);
    }

    void SkiaEffectRenderer::ValidateSpriteBindingsEXT() const
    {
        if (!effect_)
        ...
        if (cubeFaceChildIndices_[0] >= 0 && boundCubeRenderer_.expired())
        {
            throw std::runtime_error(
                "Skia SkSL effect calls cnaSampleCubeEXT and requires SetTexture(1, TextureCube) "
                "before SpriteBatch::Begin.");
        }
        if (volumeAtlasChildIndex_ >= 0 && boundVolumeRenderer_.expired())
        {
            throw std::runtime_error(
    ...
    sk_sp<SkShader> SkiaEffectRenderer::MakeSpriteShaderEXT(
        sk_sp<SkShader> primaryTexture, const float tint[4]) const
    {
        ...
        for (int unit = 1; unit <= kSkiaSkslMaxTextureUnitEXT; ++unit)
        {
            ...
            if (childIndex < 0)
                continue;
            const std::shared_ptr<ITextureRenderer> texture
        ...
        // faces fresh on every draw (via the shared ITextureCubeRenderer::GetData contract, which
        ...
        // between SetTexture(1, ...) and this draw is visible, and an expired weak renderer fails
        // this draw rather than sampling stale or fabricated pixels.
        if (cubeFaceChildIndices_[0] >= 0)
        {
            const std::shared_ptr<ITextureCubeRenderer> cube = boundCubeRenderer_.lock();
        ...
        if (volumeAtlasChildIndex_ >= 0)
        {
            const std::shared_ptr<ITexture3DRenderer> volume = boundVolumeRenderer_.lock();
