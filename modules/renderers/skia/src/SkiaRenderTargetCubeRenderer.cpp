...
namespace CNA::Internal::Renderers::Skia
{
    ...
    SkiaRenderTargetCubeRenderer::SkiaRenderTargetCubeRenderer(
    ...
    SkiaSurface* SkiaRenderTargetCubeRenderer::FaceSurface(int face, int level) noexcept
    {
    ...
    const SkiaSurface* SkiaRenderTargetCubeRenderer::FaceSurface(int face, int level) const noexcept
    {
        if (face < 0 || face >= 6 || level < 0 || level >= static_cast<int>(levels_.size()))
            return nullptr;
        return levels_[static_cast<std::size_t>(level)].faces[static_cast<std::size_t>(face)].get();
    ...
    void SkiaRenderTargetCubeRenderer::BindAsRenderTargetFace(int face)
    {
        if (face < 0 || face >= 6)
        ...
        if (!binding)
            throw std::runtime_error("Skia RenderTargetCube cannot bind after renderer destruction.");
    ...
    void SkiaRenderTargetCubeRenderer::UnbindAsRenderTarget()
    {
        const std::shared_ptr<SkiaRenderTargetBinding> binding = binding_.lock();
        if (!binding)
            throw std::runtime_error("Skia RenderTargetCube cannot unbind after renderer destruction.");
        if (binding->ActiveTarget() == this)
            FinalizeWriteEXT();
        binding->UnbindToBackbuffer();
    }

    SkiaSurface* SkiaRenderTargetCubeRenderer::BoundSurfaceEXT() noexcept
    {
        return FaceSurface(boundFace_, 0);
    }

    const SkiaSurface* SkiaRenderTargetCubeRenderer::BoundSurfaceEXT() const noexcept
    {
        return FaceSurface(boundFace_, 0);
    }

    void SkiaRenderTargetCubeRenderer::BeforeWriteEXT() noexcept
    {
    ...
    void SkiaRenderTargetCubeRenderer::FinalizeWriteEXT()
    {
        if (boundFace_ < 0 || boundFace_ >= 6
    ...
    bool SkiaRenderTargetCubeRenderer::SetData(int face, int level, int x, int y,
    ...
    bool SkiaRenderTargetCubeRenderer::GetData(int face, int level, int x, int y,
                                               int width, int height,
                                               void* data, int dataLength) const
    {
        ...
        if (dataLength < 0 || static_cast<std::size_t>(dataLength) < required)
            return false;
        if (mipDirty_[static_cast<std::size_t>(face)])
            const_cast<SkiaRenderTargetCubeRenderer*>(this)->SynchronizeRenderedFace(face);
    ...
    void SkiaRenderTargetCubeRenderer::SynchronizeRenderedFace(int face)
    {
    ...
    void SkiaRenderTargetCubeRenderer::GenerateMipChain(int face)
    {
    ...
    sk_sp<SkImage> SkiaRenderTargetCubeRenderer::SnapshotFaceEXT(int face, int level) const
    {
        if (face < 0 || face >= 6 || level < 0 || level >= static_cast<int>(levels_.size()))
            return nullptr;
        if (mipDirty_[static_cast<std::size_t>(face)])
    ...
    void SkiaRenderTargetCubeRenderer::InvalidateFaceSnapshotEXT(int face) const noexcept
    {
        if (face < 0 || face >= 6)
            return;
        const auto index = static_cast<std::size_t>(face);
    ...
    void SkiaRenderTargetCubeRenderer::InvalidateAllFaceSnapshotsEXT() noexcept
    {
