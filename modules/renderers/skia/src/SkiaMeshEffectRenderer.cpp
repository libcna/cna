...
namespace CNA::Internal::Renderers::Skia
{
    namespace
    {
        // Mirrors SkiaEffectRenderer's own ParseTextureChild exactly (docs/skia-vertices-2d-effect-
    ...
    const char* SkiaMeshEffectRenderer::UniformKindName(UniformKind kind) noexcept
    {
        switch (kind)
        {
    ...
    int SkiaMeshEffectRenderer::UniformTypeOrdinal(UniformKind kind) noexcept
    {
        using Type = SkRuntimeEffect::Uniform::Type;
        switch (kind)
        {
    ...
    void SkiaMeshEffectRenderer::Fail(std::string message)
    {
        compiled_.reset();
        uniformBytes_.clear();
    ...
    bool SkiaMeshEffectRenderer::CompileProgram(
        const std::string& marker, const std::string& source, SkiaMeshEffectCacheEXT& cache)
    {
        ...
        // Every renderer instance owns its own independent uniform buffer and texture bindings even
    ...
    void SkiaMeshEffectRenderer::SetUniformData(
    ...
    void SkiaMeshEffectRenderer::SetUniformFloat(const char* name, float value)
    {
        SetUniformData(name, UniformKind::Float, false, 0, &value, sizeof(value), "SetUniformFloat");
    }

    void SkiaMeshEffectRenderer::SetUniformInt(const char* name, int value)
    {
    ...
    void SkiaMeshEffectRenderer::SetUniformVec2(const char* name, float x, float y)
    {
        const float value[2] = {x, y};
        SetUniformData(name, UniformKind::Float2, false, 0, value, sizeof(value), "SetUniformVec2");
    }

    void SkiaMeshEffectRenderer::SetUniformVec3(const char* name, float x, float y, float z)
    {
        const float value[3] = {x, y, z};
        SetUniformData(name, UniformKind::Float3, false, 0, value, sizeof(value), "SetUniformVec3");
    }

    void SkiaMeshEffectRenderer::SetUniformVec4(const char* name, float x, float y, float z, float w)
    {
        const float value[4] = {x, y, z, w};
        SetUniformData(name, UniformKind::Float4, false, 0, value, sizeof(value), "SetUniformVec4");
    }

    void SkiaMeshEffectRenderer::SetUniformMat4(const char* name, const float* matrix)
    {
        SetUniformData(name, UniformKind::Float4x4, false, 0, matrix, sizeof(float) * 16u,
                       "SetUniformMat4");
    }

    void SkiaMeshEffectRenderer::SetUniformFloatArray(const char* name, const float* values, int count)
    {
    ...
    void SkiaMeshEffectRenderer::SetUniformVec2Array(const char* name, const float* values, int count)
    {
        const std::size_t byteCount = count >= 0
    ...
    void SkiaMeshEffectRenderer::BindTexture(int unit, ITextureRenderer* texture)
    {
        if (!compiled_)
        ...
        if (compiled_->childIndexByUnit[static_cast<std::size_t>(unit)] < 0)
            throw std::invalid_argument(std::string("Skia mesh effect does not declare `cnaTexture")
                                        + std::to_string(unit) + "`.");
        if (!texture)
            throw std::invalid_argument("Skia mesh BindTexture received a null Texture2D renderer.");
        ...
        if (weakTexture.expired())
            throw std::invalid_argument("Skia mesh BindTexture requires a shared, lifetime-tracked Texture2D renderer.");
        ...
        // identical contract to SkiaEffectRenderer::BindTexture.
        boundTextureBackends_[static_cast<std::size_t>(unit)] = std::move(weakTexture);
    }

    void SkiaMeshEffectRenderer::ValidateMeshBindingsEXT() const
    {
        if (!compiled_)
    ...
    sk_sp<SkShader> SkiaMeshEffectRenderer::MakeMeshShaderEXT() const
    {
        if (!compiled_)
            return nullptr;
    ...
    {
        return renderer_.MakeMeshShaderEXT();
    }
} // namespace CNA::Internal::Renderers::Skia
