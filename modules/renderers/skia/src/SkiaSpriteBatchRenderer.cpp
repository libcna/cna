...
namespace CNA::Internal::Renderers::Skia
{
    ...
    std::shared_ptr<SkiaRenderTargetBinding> SkiaSpriteBatchRenderer::LockBinding(
    ...
    void SkiaSpriteBatchRenderer::Begin()
    {
    ...
    void SkiaSpriteBatchRenderer::End()
    {
    ...
    void SkiaSpriteBatchRenderer::SetTransformMatrix(const Matrix& matrix)
    {
        (void)LockBinding("SpriteBatch::SetTransformMatrix");
        transformMatrix_ = matrix;
    }

    void SkiaSpriteBatchRenderer::SetCustomEffect(Effect* effect)
    {
        (void)LockBinding("SpriteBatch::SetCustomEffect");
        customEffect_ = nullptr;
        if (!effect || effect->IsExactStockSpriteEffectEXT())
        {
            // CNA's exact stock SpriteEffect owns no renderer program; EasyGL also falls back to
        ...
        auto* skiaEffect = dynamic_cast<SkiaEffectRenderer*>(effect->GetEffectRendererPtr());
    ...
    void SkiaSpriteBatchRenderer::SetSamplerFilter(int textureFilter)
    {
    ...
    void SkiaSpriteBatchRenderer::SetSamplerAddressMode(int addressU, int addressV)
    {
        ...
        // shared renderer seam. A shader is selected lazily by Draw only when its tiled path is
    ...
    void SkiaSpriteBatchRenderer::Draw(const ITextureRenderer& texture, float x, float y)
    {
    ...
    void SkiaSpriteBatchRenderer::Draw(const ITextureRenderer& texture, const Rectangle& destinationRectangle,
                                      ...
                                      const Vector2& origin, SpriteEffects effects, float layerDepth)
    {
        ...
        (void)layerDepth; // Shared SpriteBatch ordering determines call order before this renderer is invoked.
    ...
    void SkiaSpriteBatchRenderer::DrawMeshEXT(
        ...
        int vertexCount, const std::uint16_t* indices, int indexCount)
    {
        ...
        auto* adapter = dynamic_cast<SkiaMeshEffectAdapterEXT*>(effect.GetEffectRendererPtr());
