...
// plan_dx7.md: real DirectX 7 graphics backend -- genuinely new interfaces vs DX6:
// IDirectDraw7/IDirectDrawSurface7 (created via DirectDrawCreateEx, not the old
// DirectDrawCreate+QueryInterface chain) and IDirect3D7/IDirect3DDevice7. DX7 REMOVES the whole
// viewport-object concept this backend family has carried since DX2-0 -- there is no
...
// a Dx7GraphicsBackend pointer. All real <ddraw.h> usage lives in Dx7GraphicsBackend.cpp behind the
// Impl pimpl below.
...
namespace CNA::Internal::Backends::Dx7
{
     ...
     * 3) -- -> SetCooperativeLevel(DDSCL_NORMAL, against a real Win32 HWND obtained from CNA's own
     * already-existing SDL_Window via SDL_PROP_WINDOW_WIN32_HWND_POINTER) -> primary CreateSurface.
     ...
     * all of which this backend's history inherits). The primary surface is desktop-sized (the
     * real historical DirectDraw model: the primary IS the display, not "this window") -- so this
     * backend never composites directly onto it. Instead it owns a second, Lockable offscreen
     ...
     * (Dx7TextureBackend/Dx7RenderTargetBackend, defined entirely in Dx7GraphicsBackend.cpp --
     ...
     * (Dx7SpriteBatchBackend): IDirectDrawSurface7::Blt/BltFast has never supported rotation in any
     ...
     * D3DRENDERSTATE_SPECULARENABLE) -- all ported from DX6's own already-proven 3D layer. NEW at
     ...
     * D3DRENDERSTATE_TEXTUREMAPBLEND render state DX2..DX6 all used -- a real API restriction found
     ...
     * PASS/REF/MASK/WRITEMASK against the same combined depth+stencil Z-buffer surface DX6
     ...
     * adopted (design decision 9) -- this backend still submits CPU-pre-transformed-and-lit
     * D3DTLVERTEX vertices, matching every prior backend in this family.
     */
    class Dx7GraphicsBackend final : public IGraphicsBackend
    {
    public:
        explicit Dx7GraphicsBackend(const GraphicsBackendCreateArgs& args);
        ~Dx7GraphicsBackend() override;

        Dx7GraphicsBackend(const Dx7GraphicsBackend&) = delete;
        Dx7GraphicsBackend& operator=(const Dx7GraphicsBackend&) = delete;
        ...
        // from DX6, spike-confirmed the real write+test behavior survives the DX7 API flattening
        ...
        // inspection). ApplySamplerState only acts on slot 0 (this backend's own scope has exactly
        ...
        // buffers -- gone entirely since DX5; see plan_dx5.md section 1). Device/Z-buffer bring-up
        ...
        // pixel-verified. Unlike DX1 (which throws PERMANENTLY -- DirectX 1 shipped no Direct3D at
        ...
        // genuinely unavailable at this DirectX era; stencil is real since DX6, ported verbatim). ----
        ...
        // Phase O5 (design decision 8): plain CPU-side storage (Dx7VertexBufferBackend/
        // Dx7IndexBufferBackend, defined entirely in Dx7GraphicsBackend.cpp), matching the
        ...
        // indirection at all, unlike every prior backend in this family). Overridden explicitly
        // rather than relying on the shared IGraphicsBackend default (which falls back to
