...
// .cpp only -- see Dx7Renderer.hpp's own comment. This file's 2D and 3D layers are
...
namespace CNA::Internal::Renderers::Dx7
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx7TextureRenderer and
        // Dx7RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
    ...
    // Common accessor so Dx7SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx7TextureRenderer/Dx7RenderTargetRenderer themselves.
    ...
    struct Dx7Renderer::Impl
    {
        ...
        // Phase O3: the offscreen surface owned by the currently-bound Dx7RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx7RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE7 rather than
        // a Dx7RenderTargetRenderer* so Dx7RenderTargetRenderer never needs to name this private Impl
    ...
    Dx7Renderer::Dx7Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx7Renderer initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx7Renderer: could not obtain a real HWND from the SDL window.");
    ...
    Dx7Renderer::~Dx7Renderer() = default;

    void Dx7Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx7Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx7Renderer::Present: ClientToScreen failed");
    ...
    void Dx7Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx7Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx7Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx7Renderer::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx7Renderer::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx7Renderer::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx7Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    // DX7 anyway, per docs/directx-legacy-renderers-analysis.md section 3.1's table).

    class Dx7VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit Dx7VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx7VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx7VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class Dx7IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        Dx7IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx7IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx7IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    LPDIRECTDRAWSURFACE7 Dx7ResolveTextureSurface(const ITextureRenderer* texture)
    {
        if (!texture) return nullptr;
        const auto* owner = dynamic_cast<const Dx7SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx7Renderer: texture0 is not a DX7 surface (created by a different graphics renderer?)");
                                   ...
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx7Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx7Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx7Renderer::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class Dx7TextureRenderer : public ITextureRenderer, public Dx7SurfaceOwner
    {
    public:
        Dx7TextureRenderer(LPDIRECTDRAW7 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx7TextureRenderer(LPDIRECTDRAW7 dd, const ImageData& data)
            : Dx7TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx7TextureRenderer() override { if (surface_) surface_->Release(); }

        Dx7TextureRenderer(const Dx7TextureRenderer&) = delete;
        Dx7TextureRenderer& operator=(const Dx7TextureRenderer&) = delete;
    ...
    class Dx7RenderTargetRenderer final : public IRenderTargetRenderer, public Dx7SurfaceOwner
    {
    public:
        Dx7RenderTargetRenderer(LPDIRECTDRAWSURFACE7* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx7RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx7RenderTargetRenderer(const Dx7RenderTargetRenderer&) = delete;
        Dx7RenderTargetRenderer& operator=(const Dx7RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> Dx7Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx7TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> Dx7Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx7RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx7Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void Dx7Renderer::SetRenderTargets(
    ...
    // Never named outside this .cpp, same reasoning as Dx7TextureRenderer/Dx7RenderTargetRenderer.

    class Dx7SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        Dx7SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE7()> getActiveSurface,
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx7SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx7SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx7SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx7SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx7SpriteBatchRenderer::Draw: texture renderer is not a DX7 surface (created by "
    ...
    std::unique_ptr<ISpriteBatchRenderer> Dx7Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx7SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx7Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void Dx7Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    void Dx7Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
    ...
    void Dx7Renderer::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
    ...
    void Dx7Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }
    ...
    void Dx7Renderer::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void Dx7Renderer::ClearStencil(int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
    ...
    void Dx7Renderer::ClearDepthAndStencil(float depth, int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
    ...
    void Dx7Renderer::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        ClearStencil(stencil);
    }
    void Dx7Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
        Clear(r, g, b, a);
        ClearDepthAndStencil(depth, stencil);
    }
    void Dx7Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void Dx7Renderer::SetBlendEnabled(bool) {}

    void Dx7Renderer::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device7->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferRenderer> Dx7Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx7VertexBufferRenderer>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx7Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx7IndexBufferRenderer>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx7Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx7IndexBufferRenderer>(index_capacity, true);
    }

    void Dx7Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   ...
                                                   PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx7Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx7VertexBufferRenderer&>(vb);
    ...
    void Dx7Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          ...
                                                          PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx7Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx7VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const Dx7IndexBufferRenderer&>(ib);
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DX7", route);
    }

    void Dx7Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        ...
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx7Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx7Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx7VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx7Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
    ...
    void Dx7Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        ...
        Dx7CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx7Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx7Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx7Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<Dx7::Dx7Renderer>(args);
    }
#endif
}
