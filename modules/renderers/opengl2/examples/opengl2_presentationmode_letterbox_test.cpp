...
// content with 32px bars left+right.
//
...
// Overscan/Stretch twins for the other two modes.
//
// Check A -- left/right physical window edges are background (bars); centre is the drawn
//   sprite's own colour.
...
class OpenGL2PresentationModeLetterboxTest : public Game
{
...
protected:
    ...
    void Draw(const GameTime&) override
    {
        ...
        if (frame_ == 2)
        {
            const Color left = ReadPixel(5, kWindowH / 2);
            const Color right = ReadPixel(kWindowW - 5, kWindowH / 2);
            const Color centre = ReadPixel(kWindowW / 2, kWindowH / 2);
            Check("Letterbox: left bar=" + ColorStr(left) + " right bar=" + ColorStr(right) + " centre=" + ColorStr(centre),
                  Matches(left, Color::Black, 10) && Matches(right, Color::Black, 10) && Matches(centre, Color(0, 255, 0, 255), 10));
        }
...
int main()
{
    ...
    OpenGL2PresentationModeLetterboxTest game;
    game.Run();
