...
// SpriteBatch rotation-around-origin pixel oracle for the OPENVG (ShivaVG) renderer.
//
// Direct port of Task 671's SDL_Renderer test (sdlrenderer_spritebatch_rotation_test.cpp), itself
// a port of Task 417's EasyGL test -- same methodology, same geometry, same expected pixel
// outcomes. This is the decisive test for OpenVgSpriteBatchRenderer's matrix composition (device
// Y-flip, then SetTransformMatrix, then per-sprite translate/rotate/scale/origin -- see that
// class's own header comment): a wrong flip, a wrong rotation sign/units (vgRotate takes degrees,
// XNA rotation is radians), or a wrong origin/offset term all produce a wrong pixel here.
//
// Requires PresentationMode::NativeBackBuffer (same reason SDL_Renderer's own port needed it):
// ReadBackbuffer operates in physical output coordinates, and OPENVG's default presentation mode
...
class OpenVgSpriteBatchRotationTest : public CNA::Examples::PixelTestGame
{
...
int main()
{
    return CNA::Examples::RunPixelTest<OpenVgSpriteBatchRotationTest>();
}
