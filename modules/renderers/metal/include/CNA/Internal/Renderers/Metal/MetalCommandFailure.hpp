...
namespace CNA::Internal::Renderers::Metal
{
    ...
    {
        /** @brief The owning renderer is active and no asynchronous failure is pending. */
        Active,
        /** @brief The owner is active, but its command-failure latch is pending cleanup. */
        PendingCommandFailure,
        /** @brief The owning renderer has begun teardown and must no longer be dereferenced. */
        OwnerInactive
    };
     ...
     * observe a pending failure, but only the live renderer can consume it and perform the required
    ...
    public:
        ...
        /** @brief Marks the owning renderer inactive before its native teardown begins. */
        ...
        void RequireOwnerActive() const
        {
            if(!ownerActive_.load(std::memory_order_acquire))
                throw std::runtime_error(
                    "Metal: resource operation rejected after owning renderer teardown");
        }

        /**
         * @brief Rejects native resource work when its owner is failed or inactive.
         *
         * A pending failure remains latched for the owning renderer's common cleanup path.
         */
        void RequireUsable() const
        {
            ...
            if(state==MetalResourceHealthState::PendingCommandFailure)
                throw std::runtime_error(
                    "Metal: a command failure is pending owning-renderer cleanup");
            throw std::runtime_error(
                "Metal: resource operation rejected after owning renderer teardown");
        }
     ...
     *
     * @tparam TOwner Renderer implementation type retained for the duration of the operation.
     ...
     *
     * @tparam TOwner Renderer implementation type that owns active binding state.
