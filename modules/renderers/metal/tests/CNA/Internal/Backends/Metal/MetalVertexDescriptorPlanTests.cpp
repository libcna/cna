...
// object construction itself, which stays in MetalGraphicsBackend.mm. Only reads VertexElement
// (plain XNA framework type) and returns plain C++ structs, zero Objective-C dependency, so this
// is genuinely unit-tested on this Linux machine. No #if defined(CNA_BACKEND_METAL) gate,
// deliberately.
...
TEST(MetalVertexDescriptorPlan, LocationMatchesElementIndexNotOffsetOrder)
{
    ...
    // offset -- matching EasyGLGraphicsBackend::ApplyLayout()'s own documented convention exactly
...
{
    // This backend has no per-instance / multi-stream binding yet (Phase 9's own note: real
...
{
    // Cross-validates against MetalGraphicsBackend.mm's own existing hand-written stride-48
