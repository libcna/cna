...
namespace CNA::Internal::Renderers::DirectX5
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both DirectX5TextureRenderer and
        // DirectX5RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
        ...
        // DirectX5_Smoke/DirectX5_Blend/etc.'s Clear()+GetBackBufferData() checks all still passed), but a
        ...
        [[noreturn]] void ThrowMipLevelUnsupported(int level)
        {
            throw std::runtime_error(
                "DIRECTX5 (DirectDraw v4) does not support mip-level texture uploads (level " +
        ...
        enum class DirectX5BlendMode { Opaque, AlphaBlend, NonPremultiplied, Additive };
        ...
        // found-and-fixed bug).
        DirectX5BlendMode DetectBlendMode(int colorSrc, int alphaSrc, int colorDst, int alphaDst,
                                     int colorFunc, int alphaFunc)
        {
            ...
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 5 && alphaDst == 5) return DirectX5BlendMode::NonPremultiplied;
            // Additive: ColorSrc=SourceAlpha, AlphaSrc=SourceAlpha, ColorDst=One, AlphaDst=One
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 0 && alphaDst == 0) return DirectX5BlendMode::Additive;
            // Opaque: ColorSrc=One, AlphaSrc=One, ColorDst=Zero, AlphaDst=Zero
            if (bothAdd && colorSrc == 0 && alphaSrc == 0 && colorDst == 1 && alphaDst == 1) return DirectX5BlendMode::Opaque;
            ...
            return DirectX5BlendMode::AlphaBlend;
        }
        ...
        DWORD DirectX5BlendToD3D(int blend)
        {
        ...
        DWORD DirectX5CompareFunctionToD3D(int compareFunction)
        {
        ...
        DWORD DirectX5CullModeToD3D(int cullMode)
        {
        ...
        DWORD DirectX5FillModeToD3D(int fillMode)
        {
        ...
        DWORD DirectX5TextureAddressModeToD3D(int addressMode)
        {
        ...
        struct DirectX5FilterPair { DWORD mag; DWORD min; };
        ...
        DirectX5FilterPair DirectX5TextureFilterToD3D(int textureFilter)
        {
        ...
        void CompositeQuad(uint8_t* dstBase, int dstPitch, int dstW, int dstH,
                           ...
                           DirectX5BlendMode blendMode, int filter, int addressU, int addressV)
        {
                ...
                for (int x = x0; x < x1; ++x)
                {
                    ...
                    for (const auto& tri : kTriangles)
                    {
                        ...
                        case DirectX5BlendMode::Opaque:
                            ...
                            break;
                        case DirectX5BlendMode::AlphaBlend:
                        {
                        ...
                        case DirectX5BlendMode::NonPremultiplied:
                        {
                        ...
                        case DirectX5BlendMode::Additive:
    ...
    // Common accessor so DirectX5SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as DirectX5TextureRenderer/DirectX5RenderTargetRenderer themselves.
    class DirectX5SurfaceOwner
    {
    public:
        virtual ~DirectX5SurfaceOwner() = default;
        [[nodiscard]] virtual LPDIRECTDRAWSURFACE4 Surface() const = 0;
    };

    struct DirectX5Renderer::Impl
    {
        ...
        // Phase O3: the offscreen surface owned by the currently-bound DirectX5RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by DirectX5RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE4 rather than
        // a DirectX5RenderTargetRenderer* so DirectX5RenderTargetRenderer never needs to name this private Impl
        ...
        DirectX5BlendMode currentBlendMode = DirectX5BlendMode::AlphaBlend;
    ...
    DirectX5Renderer::DirectX5Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("DirectX5Renderer initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("DirectX5Renderer: could not obtain a real HWND from the SDL window.");
    ...
    DirectX5Renderer::~DirectX5Renderer() = default;

    void DirectX5Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void DirectX5Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("DirectX5Renderer::Present: ClientToScreen failed");
    ...
    void DirectX5Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void DirectX5Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void DirectX5Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void DirectX5Renderer::SetPresentationMode(int mode)
    {
    ...
    bool DirectX5Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    class DirectX5VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit DirectX5VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("DirectX5VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("DirectX5VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class DirectX5IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        DirectX5IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("DirectX5IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("DirectX5IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    DirectX5ClipVertex DirectX5LerpClipVertex(const DirectX5ClipVertex& a, const DirectX5ClipVertex& b, float t)
    {
        DirectX5ClipVertex out;
        out.x = a.x + t * (b.x - a.x);
        out.y = a.y + t * (b.y - a.y);
        out.z = a.z + t * (b.z - a.z);
        out.w = a.w + t * (b.w - a.w);
    ...
    int DirectX5ClipTriangleNearPlane(const DirectX5ClipVertex verts[3], DirectX5ClipVertex out[4])
    {
        constexpr float kNearEpsilon = 1e-5f;
        int count = 0;
        for (int i = 0; i < 3; ++i)
        {
            const DirectX5ClipVertex& cur = verts[i];
            const DirectX5ClipVertex& prev = verts[(i + 2) % 3];
            const bool curIn = cur.w > kNearEpsilon;
            const bool prevIn = prev.w > kNearEpsilon;
            if (curIn != prevIn)
            {
                const float t = (kNearEpsilon - prev.w) / (cur.w - prev.w);
                out[count++] = DirectX5LerpClipVertex(prev, cur, t);
            }
            if (curIn)
                out[count++] = cur;
        }
    ...
    DirectX5ClipVertex DirectX5BuildPositionColorClipVertex(const uint8_t* raw, const Matrix& combined)
    {
        Vector3 position;
    ...
    /// Result of DirectX5ComputeVertexLighting: the diffuse (base vertex color) and specular (additive,
    ...
    DirectX5LitColor DirectX5ComputeVertexLighting(const Vector3& localPosition, const Vector3& localNormal,
    ...
    /// normal (32/52), DirectX5ComputeVertexLighting() replaces the raw/white vertex color with real
    ...
    DirectX5ClipVertex DirectX5BuildGenericClipVertex(const uint8_t* raw, std::size_t stride, const Matrix& combined,
                                            const Matrix& world, const GpuDrawParams& params)
    {
        Vector3 position;
        std::memcpy(&position, raw, sizeof(Vector3));
        ...
        else if (stride == 32 || stride == 52)
        {
            ...
            if (params.lightingEnabled)
            {
                ...
                const DirectX5LitColor litColor = DirectX5ComputeVertexLighting(position, normal, world, params);
    ...
    /// (not cached) rather than stored on DirectX5TextureRenderer: a handle is only valid for the device
    ...
    D3DTEXTUREHANDLE DirectX5ResolveTextureHandle(const ITextureRenderer* texture, LPDIRECT3DDEVICE3 device3)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const DirectX5SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "DirectX5Renderer: texture0 is not a DIRECTX5 surface (created by a different graphics renderer?)");
    ...
    /// Shared core for all 4 draw entry points: clips each triangle (DirectX5ClipTriangleNearPlane),
    /// packs the result into D3DTLVERTEX (DirectX5ClipVertexToD3DTLVERTEX), and submits via a single
    ...
    void SubmitDx5Primitives(LPDIRECT3DDEVICE3 device3, int viewportWidth, int viewportHeight,
                             const std::function<DirectX5ClipVertex(int)>& fetchVertex,
                             int primitiveCount, D3DTEXTUREHANDLE texHandle, bool specularEnabled)
    {
        ...
        {
            DirectX5ClipVertex cv[3];
            for (int k = 0; k < 3; ++k)
                cv[k] = fetchVertex(i * 3 + k);

            DirectX5ClipVertex clipped[4];
            const int clippedCount = DirectX5ClipTriangleNearPlane(cv, clipped);
            if (clippedCount == 0) continue;

            const auto baseIdx = static_cast<WORD>(verts.size());
            for (int k = 0; k < clippedCount; ++k)
                verts.push_back(DirectX5ClipVertexToD3DTLVERTEX(clipped[k], viewportWidth, viewportHeight));
    ...
    /// target's surface).
    void DirectX5CheckDrawPreconditions(const char* who, bool customRenderTargetBound,
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("DirectX5Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("DirectX5Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("DirectX5Renderer::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class DirectX5TextureRenderer : public ITextureRenderer, public DirectX5SurfaceOwner
    {
    public:
        DirectX5TextureRenderer(LPDIRECTDRAW4 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        DirectX5TextureRenderer(LPDIRECTDRAW4 dd, const ImageData& data)
            : DirectX5TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~DirectX5TextureRenderer() override { if (surface_) surface_->Release(); }

        DirectX5TextureRenderer(const DirectX5TextureRenderer&) = delete;
        DirectX5TextureRenderer& operator=(const DirectX5TextureRenderer&) = delete;
    ...
    class DirectX5RenderTargetRenderer final : public IRenderTargetRenderer, public DirectX5SurfaceOwner
    {
    public:
        DirectX5RenderTargetRenderer(LPDIRECTDRAWSURFACE4* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~DirectX5RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        DirectX5RenderTargetRenderer(const DirectX5RenderTargetRenderer&) = delete;
        DirectX5RenderTargetRenderer& operator=(const DirectX5RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> DirectX5Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<DirectX5TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> DirectX5Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<DirectX5RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void DirectX5Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void DirectX5Renderer::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        // DirectDraw has no multi-render-target concept -- single active surface only.
        if (count > 1)
            throw std::runtime_error(
                "DIRECTX5 (DirectDraw v4) does not support multiple simultaneous render targets (MRT): "
        ...
        if (count > 0 && renderTargets[0].IsRenderTargetCubeFace())
            throw std::runtime_error(
                "DIRECTX5 (DirectDraw v4) does not support RenderTargetCube face bindings.");
    ...
    // Never named outside this .cpp, same reasoning as DirectX5TextureRenderer/DirectX5RenderTargetRenderer.

    class DirectX5SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        DirectX5SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE4()> getActiveSurface,
                              std::function<void(int&, int&)> getActiveSurfaceSize,
                              std::function<DirectX5BlendMode()> getBlendMode)
        ...
        {
            if (begun_)
                throw std::runtime_error("DirectX5SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("DirectX5SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        void SetCustomEffect(Effect* effect) override
        {
            if (effect != nullptr)
                throw std::runtime_error(
                    "DIRECTX5 (DirectDraw v4) does not support custom SpriteBatch Effects: no "
        ...
        {
            if (!begun_)
                throw std::runtime_error("DirectX5SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const DirectX5SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "DirectX5SpriteBatchRenderer::Draw: texture renderer is not a DIRECTX5 surface (created by "
            ...
            if (isIdentityGeometry && isWhiteTint && getBlendMode_() == DirectX5BlendMode::Opaque)
            {
    ...
    private:
        ...
        std::function<DirectX5BlendMode()> getBlendMode_;
        bool begun_ = false;
    ...
    std::unique_ptr<ISpriteBatchRenderer> DirectX5Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<DirectX5SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void DirectX5Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
                                             ...
                                             const BlendWriteState& /*writeState*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_SRCBLEND, DirectX5BlendToD3D(colorSrcBlend));
        impl_->device3->SetRenderState(D3DRENDERSTATE_DESTBLEND, DirectX5BlendToD3D(colorDstBlend));
    }

    void DirectX5Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
                                                    ...
                                                    int /*ccwStencilFail*/, int /*ccwStencilDepthFail*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZFUNC, DirectX5CompareFunctionToD3D(depthFunc));
    }

    void DirectX5Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
                                                  float /*depthBias*/, float /*slopeScaleDepthBias*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_CULLMODE, DirectX5CullModeToD3D(cullMode));
        impl_->device3->SetRenderState(D3DRENDERSTATE_FILLMODE, DirectX5FillModeToD3D(fillMode));
    }

    void DirectX5Renderer::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
        ...
        const DirectX5FilterPair filterPair = DirectX5TextureFilterToD3D(filter);
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_TEXTUREADDRESS, DirectX5TextureAddressModeToD3D(addressU));
    ...
    void DirectX5Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }
    ...
    void DirectX5Renderer::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void DirectX5Renderer::ClearStencil(int) {}
    void DirectX5Renderer::ClearDepthAndStencil(float depth, int) { ClearDepth(depth); }
    void DirectX5Renderer::ClearColorAndStencil(float r, float g, float b, float a, int) { Clear(r, g, b, a); }
    void DirectX5Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int)
    {
        ClearColorAndDepth(r, g, b, a, depth);
    }
    void DirectX5Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void DirectX5Renderer::SetBlendEnabled(bool) {}

    void DirectX5Renderer::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferRenderer> DirectX5Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<DirectX5VertexBufferRenderer>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> DirectX5Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<DirectX5IndexBufferRenderer>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> DirectX5Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<DirectX5IndexBufferRenderer>(index_capacity, true);
    }

    void DirectX5Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   const Matrix& world, const Matrix& view, const Matrix& projection,
                                                   PrimitiveType primitive, int primitiveCount)
    {
        DirectX5CheckDrawPreconditions("DrawColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "DirectX5Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX5VertexBufferRenderer&>(vb);
        ...
        SubmitDx5Primitives(impl_->device3, vw, vh,
            [&](int i) { return DirectX5BuildPositionColorClipVertex(base + static_cast<std::size_t>(i) * stride, combined); },
            primitiveCount, 0, false);
    }

    void DirectX5Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          const IIndexBufferRenderer& ib,
                                                          const Matrix& world, const Matrix& view, const Matrix& projection,
                                                          PrimitiveType primitive, int primitiveCount)
    {
        DirectX5CheckDrawPreconditions("DrawIndexedColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "DirectX5Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX5VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const DirectX5IndexBufferRenderer&>(ib);
        ...
        SubmitDx5Primitives(impl_->device3, vw, vh,
            [&](int i) {
                ...
                return DirectX5BuildPositionColorClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, combined);
            },
            primitiveCount, 0, false);
    }
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DIRECTX5", route);
    }

    void DirectX5Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-nonindexed");
        DirectX5CheckDrawPreconditions("DrawPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("DirectX5Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "DirectX5Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX5VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "DirectX5Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        const D3DTEXTUREHANDLE texHandle = params.textureEnabled
            ? DirectX5ResolveTextureHandle(params.texture0, impl_->device3) : 0;
        const int vertexStart = params.vertexStart;

        SubmitDx5Primitives(impl_->device3, vw, vh,
            [&](int i) { return DirectX5BuildGenericClipVertex(base + static_cast<std::size_t>(vertexStart + i) * stride,
                                                          stride, combined, world, params); },
            primitiveCount, texHandle, params.lightingEnabled);
    }

    void DirectX5Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-indexed");
        DirectX5CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("DirectX5Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "DirectX5Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "DirectX5Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        SubmitDx5Primitives(impl_->device3, vw, vh,
            [&](int i) {
                ...
                return DirectX5BuildGenericClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, stride,
...
namespace CNA::Internal::Renderers
{
    ...
    std::unique_ptr<IGraphicsRenderer> DirectX5::CreateGraphicsRenderer(const GraphicsRendererCreateArgs& args)
    {
        return std::make_unique<DirectX5::DirectX5Renderer>(args);
    }
#endif
}
