// plan_dx.md Phase DX2/DX4: D3D11 renderer skeleton + device/swap-chain/back-buffer.
...
namespace CNA::Internal::Renderers::D3D11
{
    ...
    D3D11Renderer::D3D11Renderer(const GraphicsRendererCreateArgs& args)
        : window_(args.window)
    ...
    {
        if (!window_)
            throw std::runtime_error("D3D11Renderer: no window available to create a swap chain for");

        HWND hwnd = static_cast<HWND>(SDL_GetPointerProperty(
            SDL_GetWindowProperties(window_), SDL_PROP_WINDOW_WIN32_HWND_POINTER, nullptr));
        if (!hwnd)
            throw std::runtime_error("D3D11Renderer: could not obtain HWND from SDL window");
    ...
    void D3D11Renderer::CreateWindowSizeDependentViews()
    {
    ...
    void D3D11Renderer::ReleaseWindowSizeDependentViews()
    {
    ...
    void D3D11Renderer::EnsureSwapChainSize()
    {
    ...
    void D3D11Renderer::CheckDeviceRemoved(HRESULT hr) const
    {
    ...
    void D3D11Renderer::GetViewportSize(int& width, int& height)
    {
        if (window_)
        {
    ...
    void D3D11Renderer::SetVirtualResolution(int width, int height)
    {
        virtualWidth_ = width;
        virtualHeight_ = height;
    }

    void D3D11Renderer::SetPresentationMode(int mode)
    {
        (void)mode; // presentation-mode scaling: not yet implemented (out of Phase DX4's scope)
    }

    void D3D11Renderer::SetSwapInterval(int interval)
    {
    ...
    void D3D11Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void D3D11Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
    ...
    void D3D11Renderer::ClearStencil(int stencil)
    {
        if (currentDSV_)
    ...
    void D3D11Renderer::ClearDepthAndStencil(float depth, int stencil)
    {
        if (currentDSV_)
            context_->ClearDepthStencilView(
    ...
    void D3D11Renderer::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        if (currentDSV_)
            context_->ClearDepthStencilView(
    ...
    void D3D11Renderer::ClearColorDepthAndStencil(
    ...
    void D3D11Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void D3D11Renderer::SetDepthWriteEnabled(bool enabled)
    {
    ...
    void D3D11Renderer::SetBlendEnabled(bool enabled) { (void)enabled; }

    std::unique_ptr<ITextureRenderer> D3D11Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<D3D11TextureRenderer>(device_.Get(), context_.Get(), data);
    }

    std::unique_ptr<ITexture3DRenderer> D3D11Renderer::CreateTexture3D(
        int w, int h, int depth, bool mipMap, int surfaceFormat)
    {
        return std::make_unique<D3D11Texture3DRenderer>(device_.Get(), context_.Get(), w, h, depth, mipMap, surfaceFormat);
    }

    std::unique_ptr<ITextureCubeRenderer> D3D11Renderer::CreateTextureCube(
        int size, bool mipMap, int surfaceFormat)
    {
        return std::make_unique<D3D11TextureCubeRenderer>(device_.Get(), context_.Get(), size, mipMap, surfaceFormat);
    }

    std::unique_ptr<IRenderTargetRenderer> D3D11Renderer::CreateRenderTarget2D(
    ...
    void D3D11Renderer::FlushPendingMRTResolveEXT()
    {
    ...
    void D3D11Renderer::FlushPendingCubeResolveEXT()
    {
    ...
    void D3D11Renderer::SetRenderTargetCubeFace(IRenderTargetCubeRenderer* rt, int face)
    {
    ...
    void D3D11Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
    ...
    std::unique_ptr<IRenderTargetCubeRenderer> D3D11Renderer::CreateRenderTargetCube(
        ...
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        ...
        for (int i = 0; i < count; ++i)
            if (renderTargets[i].IsRenderTargetCubeFace())
                throw std::runtime_error(
                    "D3D11Renderer::SetRenderTargets: cube faces in a multi-target "
    ...
    void D3D11Renderer::ApplySamplerState(int slot, int filter, int addressU, int addressV, int maxAnisotropy)
    {
    ...
    std::unique_ptr<IOcclusionQueryRenderer> D3D11Renderer::CreateOcclusionQuery()
    {
        return std::make_unique<D3D11OcclusionQueryRenderer>(device_.Get(), context_.Get());
    }

    std::unique_ptr<IEffectRenderer> D3D11Renderer::CreateEffectRenderer(
    ...
    void D3D11Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void D3D11Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable,
                                                       int depthFunc,
    ...
    void D3D11Renderer::RebindDepthStencilState()
    {
    ...
    void D3D11Renderer::ApplyRasterizerState(int cullMode, int fillMode,
    ...
    void D3D11Renderer::SetBlendFactor(float r, float g, float b, float a)
    {
    ...
    void D3D11Renderer::SetReferenceStencil(int value)
    {
    ...
    void D3D11Renderer::SetViewport(int x, int y, int w, int h, float minDepth, float maxDepth)
    {
        D3D11_VIEWPORT vp{};
    ...
    void D3D11Renderer::TrackCurrentRenderTargetEXT(
    ...
    void D3D11Renderer::RestoreBackBufferRenderTargetEXT()
    {
        ID3D11RenderTargetView* rtv = backBufferRTV_.Get();
        context_->OMSetRenderTargets(1, &rtv, depthStencilView_.Get());
    ...
    std::unique_ptr<ISpriteBatchRenderer> D3D11Renderer::CreateSpriteBatch()
    {
        return std::make_unique<D3D11SpriteBatchRenderer>(this);
    }

    std::unique_ptr<IVertexBufferRenderer> D3D11Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<D3D11VertexBufferRenderer>(device_.Get(), context_.Get(), vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> D3D11Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<D3D11IndexBufferRenderer>(device_.Get(), context_.Get(), index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> D3D11Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<D3D11IndexBufferRenderer>(device_.Get(), context_.Get(), index_capacity, true);
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreatePerDrawConstantBufferEXT()
    {
        if (!perDrawConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: PerDraw constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return perDrawConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreateFogConstantBufferEXT()
    {
        if (!fogConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: Fog constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return fogConstantBuffer_.Get();
    }

    void D3D11Renderer::UpdateDynamicConstantBufferEXT(
        ID3D11Buffer* buffer, const void* data, std::size_t byteCount)
    {
        ...
        if (FAILED(hr))
            throw std::runtime_error("D3D11Renderer: constant buffer Map failed, hr=" + FormatHr(hr));
        std::memcpy(mapped.pData, data, byteCount);
        context_->Unmap(buffer, 0);
    }

    void D3D11Renderer::DrawColoredPrimitives(
        ...
        PrimitiveType primitive, int primitiveCount)
    {
        ...
        {
            throw std::runtime_error(
                "D3D11Renderer::DrawColoredPrimitives: only stride-16 (VertexPositionColor) "
    ...
    void D3D11Renderer::DrawIndexedColoredPrimitives(
        ...
        PrimitiveType primitive, int primitiveCount)
    {
        ...
        {
            throw std::runtime_error(
                "D3D11Renderer::DrawIndexedColoredPrimitives: only stride-16 "
    ...
    ID3D11Buffer* D3D11Renderer::GetOrCreateLightingConstantBufferEXT()
    {
        if (!lightingConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: Lighting constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return lightingConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreateAlphaTestConstantBufferEXT()
    {
        if (!alphaTestConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: AlphaTest constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return alphaTestConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreateDualTexFogConstantBufferEXT()
    {
        if (!dualTexFogConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: DualTex Fog constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return dualTexFogConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreateEnvMapPerDrawConstantBufferEXT()
    {
        if (!envMapPerDrawConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: EnvMap PerDraw constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return envMapPerDrawConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreateEnvMapConstantBufferEXT()
    {
        if (!envMapConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: EnvMap constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return envMapConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreateBoneConstantBufferEXT()
    {
        if (!boneConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: Bone constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return boneConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreateSkinnedExtraConstantBufferEXT()
    {
        if (!skinnedExtraConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: Skinned Extra constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return skinnedExtraConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreatePbrPerDrawConstantBufferEXT()
    {
        if (!pbrPerDrawConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: Pbr PerDraw constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return pbrPerDrawConstantBuffer_.Get();
    }

    ID3D11Buffer* D3D11Renderer::GetOrCreatePbrLightsConstantBufferEXT()
    {
        if (!pbrLightsConstantBuffer_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: Pbr Lights constant buffer creation failed, hr=" + FormatHr(hr));
        }
        return pbrLightsConstantBuffer_.Get();
    }

    ID3D11ShaderResourceView* D3D11Renderer::GetOrCreateDefaultWhiteSrvEXT()
    {
        if (!defaultWhiteSrv_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: default white texture creation failed, hr=" + FormatHr(hr));
            hr = device_->CreateShaderResourceView(defaultWhiteTexture_.Get(), nullptr, defaultWhiteSrv_.ReleaseAndGetAddressOf());
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: default white SRV creation failed, hr=" + FormatHr(hr));
        }
        return defaultWhiteSrv_.Get();
    }

    ID3D11ShaderResourceView* D3D11Renderer::GetOrCreateDefaultFlatNormalSrvEXT()
    {
        if (!defaultFlatNormalSrv_)
        {
            ...
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: default flat-normal texture creation failed, hr=" + FormatHr(hr));
            hr = device_->CreateShaderResourceView(defaultFlatNormalTexture_.Get(), nullptr, defaultFlatNormalSrv_.ReleaseAndGetAddressOf());
            if (FAILED(hr))
                throw std::runtime_error("D3D11Renderer: default flat-normal SRV creation failed, hr=" + FormatHr(hr));
        }
        return defaultFlatNormalSrv_.Get();
    }

    ID3D11InputLayout* D3D11Renderer::GetOrCreateInstancedInputLayoutEXT(UINT instanceStepRate)
    {
    ...
    void D3D11Renderer::DrawPrimitivesExImpl(
        const IVertexBufferRenderer& vb, const IIndexBufferRenderer* ib,
        const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount, const GpuDrawParams& params)
    {
        ...
        if (needsAlphaTest && stride != 20 && stride != 24)
            throw std::runtime_error(
                "D3D11Renderer::DrawPrimitivesEx: AlphaTestEffect (alpha_test3d) only "
        ...
        if (needsDualTex && stride != 20)
            throw std::runtime_error(
                "D3D11Renderer::DrawPrimitivesEx: DualTextureEffect (dual_texture3d) only "
        ...
        if (needsEnvMap && stride != 32)
            throw std::runtime_error(
                "D3D11Renderer::DrawPrimitivesEx: EnvironmentMapEffect (env_map3d) requires "
        ...
        if (needsSkinned && stride != 52 && stride != 56)
            throw std::runtime_error(
                "D3D11Renderer::DrawPrimitivesEx: SkinnedEffect (skinned3d) requires stride "
        ...
        if (needsPbr && !params.skinned && stride != 48)
            throw std::runtime_error(
                "D3D11Renderer::DrawPrimitivesEx: PbrEffect (pbr3d) requires stride 48 "
                "(VertexPositionNormalTangentTexture)");
        if (needsPbr && params.skinned && stride != 68)
            throw std::runtime_error(
                "D3D11Renderer::DrawPrimitivesEx: SkinnedPbrEffect (pbr_skinned3d) requires "
                "stride 68 (VertexPositionNormalTangentTextureSkinned)");
        ...
        else
        {
            ...
            default:
                throw std::runtime_error(
                    "D3D11Renderer::DrawPrimitivesEx: unsupported vertex stride " +
    ...
    void D3D11Renderer::DrawPrimitivesEx(
        const IVertexBufferRenderer& vb, const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount, const GpuDrawParams& params)
    {
    ...
    void D3D11Renderer::DrawIndexedPrimitivesEx(
        const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
        const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount, const GpuDrawParams& params)
    {
    ...
    void D3D11Renderer::DrawInstancedPrimitivesEx(
        const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
        const Matrix& world, const Matrix& view, const Matrix& projection,
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<D3D11::D3D11Renderer>(args);
    }
}
