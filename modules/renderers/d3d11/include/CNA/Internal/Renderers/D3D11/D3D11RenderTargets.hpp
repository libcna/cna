...
// plan_dx.md Phase DX6 (DX-43/DX-45): real D3D11 offscreen render-target renderers.
//
// Both classes hold a non-owning D3D11Renderer* (owner_) -- unlike D3D11Buffers/Textures,
...
namespace CNA::Internal::Renderers::D3D11
{
    ...
    {
    public:
        D3D11RenderTargetRenderer(D3D11Renderer* owner, ID3D11Device* device, ID3D11DeviceContext* context,
         ...
         * D3D11Renderer::ReadBackbuffer already uses: copy the requested subresource region
        ...
        /// extracted so D3D11Renderer::SetRenderTargets()'s MRT (N>1) path can finalize
    ...
    {
    public:
        D3D11RenderTargetCubeRenderer(D3D11Renderer* owner, ID3D11Device* device, ID3D11DeviceContext* context,
