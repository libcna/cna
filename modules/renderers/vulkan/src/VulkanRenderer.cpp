...
// Checked at runtime in CreateInstance(); flipped to false if unavailable.
    ...
    static const CNA::Internal::Graphics::StockProgramInput* BasicFamilyStockInputsEXT(
        bool useFogTex3D, bool useLitTextured, std::size_t stride, std::size_t& countOut)
    {
        countOut = 0;
        if (useLitTextured) {
            countOut = std::size(StockInputs::kLitTextured);
            return StockInputs::kLitTextured;
        }
        if (!useFogTex3D) return nullptr;
        switch (stride) {
            case 16: countOut = std::size(StockInputs::kColored);     return StockInputs::kColored;
            case 20: countOut = std::size(StockInputs::kTextured);    return StockInputs::kTextured;
            case 24: countOut = std::size(StockInputs::kColTextured); return StockInputs::kColTextured;
        ...
        const DepthStencilKeyParams& dsParams, const BlendKeyParams& blendParams, VkFormat targetDepthFmt, const VulkanVertexInputLayoutEXT& vertexLayout)
    {
        ...
        PipelineKey key = { FoldDepthFormatIntoKey(Make3DKey(topo, depthTest, depthWrite, blend, cullMode, colorAttachmentCount, wireframe, msaa, dsParams), targetDepthFmt), PackBlendBits(blend, blendParams), PackColorWriteBits(blendParams), blendParams.sampleMask, vertexLayout.Hash() };
        ...
        PrimitiveType primitive, int primitiveCount, const GpuDrawParams& params)
    {
        ...
        VulkanVertexInputLayoutEXT declaredLayout;
        {
            ...
            if (const auto* inputs = BasicFamilyStockInputsEXT(
                    usesFogTex3D, needsLitTextured, stride, inputCount))
