...
// plans/plan_vulkan.md VULKAN-224: the per-vertex-lit half of the lit family, made instanceable.
//
// A copy of lit_textured3d_vertexlit.vert.glsl with the four per-instance matrix columns added.
// Same push constant, same LitLightParams UBO, same outputs -- so the same fragment stage, pipeline
// layout and descriptor set. Both lit variants exist because XNA's real default is
// PreferPerPixelLighting=false, and an instanced draw must not silently switch a game to the other
// one (Task 1103).
...
void main() {
    // VULKAN-219: the per-instance matrix applies inside the effect's own world transform.
    ...
    vec3 worldPos = (lp.world * objPos).xyz;
