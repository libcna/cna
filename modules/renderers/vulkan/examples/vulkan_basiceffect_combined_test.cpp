...
// EmissiveColor) × Alpha. Uses a real 2×2 multi-texel texture with 4 separate draws, each holding
// UV constant across the whole quad to sample one exact texel (at 0.25/0.75 texel centers,
// avoiding bilinear-filtering ambiguity).
//
...
class VulkanBasicEffectCombinedTest : public Game
{
    ...
    void Draw(const GameTime&) override
    {
        ...
        for (const auto& s : kSamples)
        {
            ...
            for (int i = 0; i < 20; ++i)
            {
                ...
                // standard NDC quad winding used throughout this pixel-test family is culled once
                // the real default RasterizerState reaches the GPU.
...
int main()
{
    VulkanBasicEffectCombinedTest game;
    game.Run();
