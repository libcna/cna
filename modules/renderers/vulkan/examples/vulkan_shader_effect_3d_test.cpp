    ...
    void Draw(const GameTime&) override
    {
        ...
        }
        {
            ...
            std::string what;
            {
                ...
                dev.SetRenderTarget(&rt);
                fx_->Apply();
                // A REAL per-instance stream, because without one DrawInstancedPrimitives falls
                // back to an ordinary indexed draw before this renderer ever sees it -- which is
                // correct, and would have made this leg pass for the wrong reason.
                ...
                const float instData[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
                instVb.SetDataRaw(instData, 2, 16);
                dev.SetVertexBuffers({ VertexBufferBinding(vb.get(), 0, 0),
                                       VertexBufferBinding(&instVb, 0, 1) });
                dev.setIndicesProperty(ib_.get());
                try {
                    dev.DrawInstancedPrimitives(PrimitiveType::TriangleList, 0, 0, 4, 0, 2, 2);
                }
                catch (const System::NotSupportedException& e) { threw = true; what = e.what(); }
                catch (const std::exception& e) {
                    what = std::string("the wrong exception type: ") + e.what();
            ...
            check(threw,
                  "E DrawInstancedPrimitives with a ShaderEffect is refused rather than drawn "
                  "with a stock shader: " +
                      (threw ? what
                             : (what.empty() ? std::string("NOT REFUSED, nothing thrown") : what)));
        }
