...
// REMED-GFX-018 / REMED-GFX-129: public GraphicsDevice.Clear/ClearOptions contract.
//
...
//   * a complete initialization/teardown cycle.
//
// Written renderer-agnostic; modules/renderers/vulkan/examples/CMakeLists.txt registers it for
// Vulkan.
...
namespace
{
    ...
    float ClipDepthForFramebufferDepth(float depth)
    {
        ...
        //
        // REMED-GFX-129 false positive: this used to read an environment variable only one other
        // renderer's CTest routes ever set. On any OTHER renderer it was absent, so the OpenGL
        // conversion was applied regardless of that renderer's real convention -- and on
...
class GraphicsDeviceClearOptionsTest final : public Game
{
    enum class Suite
    {
        Backbuffer,
    ...
    void RunBackbufferSuite(GraphicsDevice& device)
    {
        ...
        // distinct pre-existing values earlier in THIS frame, then exercise the renderer's ordered
        // clear handling and take one snapshot of the resulting frame.
        StampBaseline(
    ...
    void RunIsolationTest(GraphicsDevice& device)
    {
        ...
        // already-recorded clear through a reused native pass.
        device.SetRenderTarget(targetA_.get());
        ApplyRestrictedClearState(device, kTargetWidth, kTargetHeight);
...
{
    int result = 1;
    std::printf("[INFO] GFX-018 initialization begins\n");
    {
        GraphicsDeviceClearOptionsTest game;
        game.Run();
        result = game.GetResult();
    }
    std::printf("[PASS] GFX-018 teardown completed\n");
    return result;
}
