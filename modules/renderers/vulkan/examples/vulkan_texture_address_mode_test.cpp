...
// Task 665: Vulkan integration test — SpriteBatch TextureAddressMode edge sampling.
//
// Direct Vulkan port of Task 269's EasyGL test (examples/easygl_texture_address_mode_test.cpp).
// Draws a 2x1 (Red|Blue) texture via SpriteBatch with a sourceRectangle twice the texture's
// width, so the sampled U coordinate spans [0,2] -- exercising sampling past the texture edge,
// the classic XNA scrolling/tiling-background technique (FNA's real SpriteBatch never clamps
// sourceRectangle-derived UVs to [0,1]).
//
...
// could never take effect either way.
//
...
class VulkanTextureAddressModeTest : public Game
{
...
int main()
{
    VulkanTextureAddressModeTest game;
    game.Run();
