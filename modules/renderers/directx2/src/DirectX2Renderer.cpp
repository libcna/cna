#include "CNA/Internal/Renderers/DirectX2/DirectX2Renderer.hpp"
...
// this .cpp only -- see DirectX2Renderer.hpp's own comment. This file's 2D layer (ported from
// DIRECTX1, plans/plan_dx1.md section 1) may only name v1 DirectDraw symbols (IDirectDraw/IDirectDrawSurface/
// DDSURFACEDESC/DirectDrawCreate) -- never IDirectDraw2+/IDirectDrawSurface2+/DDSURFACEDESC2 (that
// boundary is permanent, plans/plan_dx2.md section 1). Unlike DIRECTX1, <d3d.h>/IDirect3D2/IDirect3DDevice2
// are NOT forbidden here -- this renderer's own 3D layer (Phase O3/O4) is built on them -- only the
...
// forbidden, asserted by the DirectX2_ExecuteBufferDiscipline CTest
// (scripts/check-directx2-execute-buffer-discipline.sh, plans/plan_dx2.md design decision 12).
#include <ddraw.h>
#include <d3d.h>
...
namespace CNA::Internal::Renderers::DirectX2
{
    ...
    namespace
    {
        ...
        // ---- 3D pipeline: NOT YET implemented (this is Phase O1/O2, plans/plan_dx2.md -- 2D layer
        // only). Every 3D entry point is wired to this same throw for now, identical to
        // DirectX1Renderer's own (permanent, for DIRECTX1) ThrowNo3D -- a later phase (O3/O4) replaces
        // these with real IDirect3D2/IDirect3DDevice2::DrawPrimitive-based implementations, unlike
        // DIRECTX1 where this throw is permanent (DirectX 1 shipped no Direct3D to call at all). ----
        ...
        {
            throw std::runtime_error(std::string("DIRECTX2 (DirectDraw v1, 3D not yet implemented) does not support 3D: ") + methodName);
        }

        // ---- Phase O3: shared offscreen-surface helpers, used by both DirectX2TextureRenderer and
        // DirectX2RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
        ...
        // DirectX2_Smoke/DirectX2_Blend/etc.'s Clear()+GetBackBufferData() checks all still passed), but a
        ...
        [[noreturn]] void ThrowMipLevelUnsupported(int level)
        {
            throw std::runtime_error(
                "DIRECTX2 (DirectDraw v1) does not support mip-level texture uploads (level " +
        ...
        enum class DirectX2BlendMode { Opaque, AlphaBlend, NonPremultiplied, Additive };
        ...
        // found-and-fixed bug).
        DirectX2BlendMode DetectBlendMode(int colorSrc, int alphaSrc, int colorDst, int alphaDst,
                                     int colorFunc, int alphaFunc)
        {
            ...
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 5 && alphaDst == 5) return DirectX2BlendMode::NonPremultiplied;
            // Additive: ColorSrc=SourceAlpha, AlphaSrc=SourceAlpha, ColorDst=One, AlphaDst=One
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 0 && alphaDst == 0) return DirectX2BlendMode::Additive;
            // Opaque: ColorSrc=One, AlphaSrc=One, ColorDst=Zero, AlphaDst=Zero
            if (bothAdd && colorSrc == 0 && alphaSrc == 0 && colorDst == 1 && alphaDst == 1) return DirectX2BlendMode::Opaque;
            ...
            return DirectX2BlendMode::AlphaBlend;
        }
        ...
        DWORD DirectX2BlendToD3D(int blend)
        {
        ...
        DWORD DirectX2CompareFunctionToD3D(int compareFunction)
        {
        ...
        DWORD DirectX2CullModeToD3D(int cullMode)
        {
        ...
        DWORD DirectX2FillModeToD3D(int fillMode)
        {
        ...
        DWORD DirectX2TextureAddressModeToD3D(int addressMode)
        {
        ...
        struct DirectX2FilterPair { DWORD mag; DWORD min; };
        ...
        DirectX2FilterPair DirectX2TextureFilterToD3D(int textureFilter)
        {
        ...
        void CompositeQuad(uint8_t* dstBase, int dstPitch, int dstW, int dstH,
                           ...
                           DirectX2BlendMode blendMode, int filter, int addressU, int addressV)
        {
                ...
                for (int x = x0; x < x1; ++x)
                {
                    ...
                    for (const auto& tri : kTriangles)
                    {
                        ...
                        case DirectX2BlendMode::Opaque:
                            ...
                            break;
                        case DirectX2BlendMode::AlphaBlend:
                        {
                        ...
                        case DirectX2BlendMode::NonPremultiplied:
                        {
                        ...
                        case DirectX2BlendMode::Additive:
    ...
    // Common accessor so DirectX2SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as DirectX2TextureRenderer/DirectX2RenderTargetRenderer themselves.
    class DirectX2SurfaceOwner
    {
    public:
        virtual ~DirectX2SurfaceOwner() = default;
        [[nodiscard]] virtual LPDIRECTDRAWSURFACE Surface() const = 0;
    };

    struct DirectX2Renderer::Impl
    {
        explicit Impl(const RendererSurfaceInfo& surfaceInfo)
            : surface(surfaceInfo, "DirectX2Renderer")
        {
        }
        ...
        // Phase O3: the offscreen surface owned by the currently-bound DirectX2RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by DirectX2RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE rather than
        // a DirectX2RenderTargetRenderer* so DirectX2RenderTargetRenderer never needs to name this private Impl
        ...
        DirectX2BlendMode currentBlendMode = DirectX2BlendMode::AlphaBlend;
    ...
    DirectX2Renderer::DirectX2Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>(args.surface))
    {
        ...
        if (!CNA::Platform::TryGetWin32(impl_->surface.GetNativeHandle(), nativeWindow))
            throw std::runtime_error("DirectX2Renderer requires a Win32 native window.");
    ...
    DirectX2Renderer::~DirectX2Renderer() = default;

    void DirectX2Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void DirectX2Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("DirectX2Renderer::Present: ClientToScreen failed");
    ...
    void DirectX2Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void DirectX2Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void DirectX2Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void DirectX2Renderer::SetPresentationMode(int mode)
    {
    ...
    void DirectX2Renderer::OnSurfaceChanged(const RendererSurfaceInfo& surface)
    {
    ...
    bool DirectX2Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    class DirectX2VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit DirectX2VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("DirectX2VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("DirectX2VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class DirectX2IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        DirectX2IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("DirectX2IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("DirectX2IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    DirectX2ClipVertex DirectX2LerpClipVertex(const DirectX2ClipVertex& a, const DirectX2ClipVertex& b, float t)
    {
        DirectX2ClipVertex out;
        out.x = a.x + t * (b.x - a.x);
        out.y = a.y + t * (b.y - a.y);
        out.z = a.z + t * (b.z - a.z);
        out.w = a.w + t * (b.w - a.w);
    ...
    int DirectX2ClipTriangleNearPlane(const DirectX2ClipVertex verts[3], DirectX2ClipVertex out[4])
    {
        constexpr float kNearEpsilon = 1e-5f;
        int count = 0;
        for (int i = 0; i < 3; ++i)
        {
            const DirectX2ClipVertex& cur = verts[i];
            const DirectX2ClipVertex& prev = verts[(i + 2) % 3];
            const bool curIn = cur.w > kNearEpsilon;
            const bool prevIn = prev.w > kNearEpsilon;
            if (curIn != prevIn)
            {
                const float t = (kNearEpsilon - prev.w) / (cur.w - prev.w);
                out[count++] = DirectX2LerpClipVertex(prev, cur, t);
            }
            if (curIn)
                out[count++] = cur;
        }
    ...
    DirectX2ClipVertex DirectX2BuildPositionColorClipVertex(const uint8_t* raw, const Matrix& combined)
    {
        Vector3 position;
    ...
    /// Result of DirectX2ComputeVertexLighting: the diffuse (base vertex color) and specular (additive,
    ...
    DirectX2LitColor DirectX2ComputeVertexLighting(const Vector3& localPosition, const Vector3& localNormal,
    ...
    /// normal (32/52), DirectX2ComputeVertexLighting() replaces the raw/white vertex color with real
    ...
    DirectX2ClipVertex DirectX2BuildGenericClipVertex(const uint8_t* raw, std::size_t stride, const Matrix& combined,
                                            const Matrix& world, const GpuDrawParams& params)
    {
        Vector3 position;
        std::memcpy(&position, raw, sizeof(Vector3));
        ...
        else if (stride == 32 || stride == 52)
        {
            ...
            if (params.lightingEnabled)
            {
                ...
                const DirectX2LitColor litColor = DirectX2ComputeVertexLighting(position, normal, world, params);
    ...
    D3DTLVERTEX DirectX2ClipVertexToD3DTLVERTEX(const DirectX2ClipVertex& cv, int viewportWidth, int viewportHeight)
    {
    ...
    /// (not cached) rather than stored on DirectX2TextureRenderer: a handle is only valid for the device
    ...
    D3DTEXTUREHANDLE DirectX2ResolveTextureHandle(const ITextureRenderer* texture, LPDIRECT3DDEVICE2 device2)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const DirectX2SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "DirectX2Renderer: texture0 is not a DIRECTX2 surface (created by a different graphics renderer?)");
    ...
    /// Shared core for all 4 draw entry points: clips each triangle (DirectX2ClipTriangleNearPlane),
    /// packs the result into D3DTLVERTEX (DirectX2ClipVertexToD3DTLVERTEX), and submits via a single
    ...
    void SubmitDx2Primitives(LPDIRECT3DDEVICE2 device2, int viewportWidth, int viewportHeight,
                             const std::function<DirectX2ClipVertex(int)>& fetchVertex,
                             int primitiveCount, D3DTEXTUREHANDLE texHandle, bool specularEnabled)
    {
        ...
        {
            DirectX2ClipVertex cv[3];
            for (int k = 0; k < 3; ++k)
                cv[k] = fetchVertex(i * 3 + k);

            DirectX2ClipVertex clipped[4];
            const int clippedCount = DirectX2ClipTriangleNearPlane(cv, clipped);
            if (clippedCount == 0) continue;

            const auto baseIdx = static_cast<WORD>(verts.size());
            for (int k = 0; k < clippedCount; ++k)
                verts.push_back(DirectX2ClipVertexToD3DTLVERTEX(clipped[k], viewportWidth, viewportHeight));
    ...
    /// target's surface).
    void DirectX2CheckDrawPreconditions(const char* who, bool customRenderTargetBound,
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("DirectX2Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("DirectX2Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("DirectX2Renderer::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class DirectX2TextureRenderer : public ITextureRenderer, public DirectX2SurfaceOwner
    {
    public:
        DirectX2TextureRenderer(LPDIRECTDRAW dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        DirectX2TextureRenderer(LPDIRECTDRAW dd, const ImageData& data)
            : DirectX2TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~DirectX2TextureRenderer() override { if (surface_) surface_->Release(); }

        DirectX2TextureRenderer(const DirectX2TextureRenderer&) = delete;
        DirectX2TextureRenderer& operator=(const DirectX2TextureRenderer&) = delete;
    ...
    class DirectX2RenderTargetRenderer final : public IRenderTargetRenderer, public DirectX2SurfaceOwner
    {
    public:
        DirectX2RenderTargetRenderer(LPDIRECTDRAWSURFACE* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~DirectX2RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        DirectX2RenderTargetRenderer(const DirectX2RenderTargetRenderer&) = delete;
        DirectX2RenderTargetRenderer& operator=(const DirectX2RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> DirectX2Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<DirectX2TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> DirectX2Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<DirectX2RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void DirectX2Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void DirectX2Renderer::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        // DirectDraw has no multi-render-target concept -- single active surface only.
        if (count > 1)
            throw std::runtime_error(
                "DIRECTX2 (DirectDraw v1) does not support multiple simultaneous render targets (MRT): "
        ...
        if (count > 0 && renderTargets[0].IsRenderTargetCubeFace())
            throw std::runtime_error(
                "DIRECTX2 (DirectDraw v1) does not support RenderTargetCube face bindings.");
    ...
    // Never named outside this .cpp, same reasoning as DirectX2TextureRenderer/DirectX2RenderTargetRenderer.

    class DirectX2SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        DirectX2SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE()> getActiveSurface,
                              std::function<void(int&, int&)> getActiveSurfaceSize,
                              std::function<DirectX2BlendMode()> getBlendMode)
        ...
        {
            if (begun_)
                throw std::runtime_error("DirectX2SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("DirectX2SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        void SetCustomEffect(Effect* effect) override
        {
            if (effect != nullptr)
                throw std::runtime_error(
                    "DIRECTX2 (DirectDraw v1) does not support custom SpriteBatch Effects: no "
        ...
        {
            if (!begun_)
                throw std::runtime_error("DirectX2SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const DirectX2SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "DirectX2SpriteBatchRenderer::Draw: texture renderer is not a DIRECTX2 surface (created by "
            ...
            if (isIdentityGeometry && isWhiteTint && getBlendMode_() == DirectX2BlendMode::Opaque)
            {
    ...
    private:
        ...
        std::function<DirectX2BlendMode()> getBlendMode_;
        bool begun_ = false;
    ...
    std::unique_ptr<ISpriteBatchRenderer> DirectX2Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<DirectX2SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void DirectX2Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
                                             ...
                                             const BlendWriteState& /*writeState*/)
    {
        ...
        impl_->device2->SetRenderState(D3DRENDERSTATE_SRCBLEND, DirectX2BlendToD3D(colorSrcBlend));
        impl_->device2->SetRenderState(D3DRENDERSTATE_DESTBLEND, DirectX2BlendToD3D(colorDstBlend));
    }

    void DirectX2Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
                                                    ...
                                                    int /*ccwStencilFail*/, int /*ccwStencilDepthFail*/)
    {
        ...
        impl_->device2->SetRenderState(D3DRENDERSTATE_ZFUNC, DirectX2CompareFunctionToD3D(depthFunc));
    }

    void DirectX2Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
                                                  float /*depthBias*/, float /*slopeScaleDepthBias*/)
    {
        ...
        impl_->device2->SetRenderState(D3DRENDERSTATE_CULLMODE, DirectX2CullModeToD3D(cullMode));
        impl_->device2->SetRenderState(D3DRENDERSTATE_FILLMODE, DirectX2FillModeToD3D(fillMode));
    }

    void DirectX2Renderer::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
        ...
        const DirectX2FilterPair filterPair = DirectX2TextureFilterToD3D(filter);
        ...
        impl_->device2->SetRenderState(D3DRENDERSTATE_TEXTUREADDRESS, DirectX2TextureAddressModeToD3D(addressU));
    ...
    void DirectX2Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }

    void DirectX2Renderer::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void DirectX2Renderer::ClearStencil(int) {}
    void DirectX2Renderer::ClearDepthAndStencil(float depth, int) { ClearDepth(depth); }
    void DirectX2Renderer::ClearColorAndStencil(float r, float g, float b, float a, int) { Clear(r, g, b, a); }
    void DirectX2Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int)
    {
        ClearColorAndDepth(r, g, b, a, depth);
    }
    void DirectX2Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void DirectX2Renderer::SetBlendEnabled(bool) {}

    void DirectX2Renderer::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device2->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferRenderer> DirectX2Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<DirectX2VertexBufferRenderer>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> DirectX2Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<DirectX2IndexBufferRenderer>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> DirectX2Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<DirectX2IndexBufferRenderer>(index_capacity, true);
    }

    void DirectX2Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   const Matrix& world, const Matrix& view, const Matrix& projection,
                                                   PrimitiveType primitive, int primitiveCount)
    {
        DirectX2CheckDrawPreconditions("DrawColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "DirectX2Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX2VertexBufferRenderer&>(vb);
        ...
        SubmitDx2Primitives(impl_->device2, vw, vh,
            [&](int i) { return DirectX2BuildPositionColorClipVertex(base + static_cast<std::size_t>(i) * stride, combined); },
            primitiveCount, 0, false);
    }

    void DirectX2Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          const IIndexBufferRenderer& ib,
                                                          const Matrix& world, const Matrix& view, const Matrix& projection,
                                                          PrimitiveType primitive, int primitiveCount)
    {
        DirectX2CheckDrawPreconditions("DrawIndexedColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "DirectX2Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX2VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const DirectX2IndexBufferRenderer&>(ib);
        ...
        SubmitDx2Primitives(impl_->device2, vw, vh,
            [&](int i) {
                ...
                return DirectX2BuildPositionColorClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, combined);
            },
            primitiveCount, 0, false);
    }
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DIRECTX2", route);
    }

    void DirectX2Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-nonindexed");
        DirectX2CheckDrawPreconditions("DrawPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("DirectX2Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "DirectX2Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const DirectX2VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "DirectX2Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        const D3DTEXTUREHANDLE texHandle = params.textureEnabled
            ? DirectX2ResolveTextureHandle(params.texture0, impl_->device2) : 0;
        const int vertexStart = params.vertexStart;

        SubmitDx2Primitives(impl_->device2, vw, vh,
            [&](int i) { return DirectX2BuildGenericClipVertex(base + static_cast<std::size_t>(vertexStart + i) * stride,
                                                          stride, combined, world, params); },
            primitiveCount, texHandle, params.lightingEnabled);
    }

    void DirectX2Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-indexed");
        DirectX2CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("DirectX2Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "DirectX2Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "DirectX2Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        SubmitDx2Primitives(impl_->device2, vw, vh,
            [&](int i) {
                ...
                return DirectX2BuildGenericClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, stride,
...
namespace CNA::Internal::Renderers
{
    ...
    namespace DirectX2 { std::unique_ptr<IGraphicsRenderer> CreateGraphicsRenderer(const GraphicsRendererCreateArgs& args); }

    std::unique_ptr<IGraphicsRenderer> DirectX2::CreateGraphicsRenderer(const GraphicsRendererCreateArgs& args)
    {
        return std::make_unique<DirectX2::DirectX2Renderer>(args);
    }
#endif
}
