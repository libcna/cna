...
// a DirectX2Renderer pointer. All real <ddraw.h> usage lives in DirectX2Renderer.cpp behind the
// Impl pimpl below.
...
namespace CNA::Internal::Renderers::DirectX2
{
     ...
     * private offscreen DDSCAPS_OFFSCREENPLAIN surfaces (DirectX2TextureRenderer/DirectX2RenderTargetRenderer,
     * defined entirely in DirectX2Renderer.cpp -- neither is ever named outside it), and
     * SetRenderTarget2D() redirects Clear()/ReadBackbuffer() to whichever surface is currently
     * bound. The SpriteBatch CPU compositor (Phase O4, DirectX2SpriteBatchRenderer, ported from DIRECTX1's
     ...
     */
    class DirectX2Renderer final : public IGraphicsRenderer
    {
    public:
        explicit DirectX2Renderer(const GraphicsRendererCreateArgs& args);
        ~DirectX2Renderer() override;

        DirectX2Renderer(const DirectX2Renderer&) = delete;
        DirectX2Renderer& operator=(const DirectX2Renderer&) = delete;
        ...
        // Phase O5 (design decision 8): plain CPU-side storage (DirectX2VertexBufferRenderer/
        // DirectX2IndexBufferRenderer, defined entirely in DirectX2Renderer.cpp), matching the
