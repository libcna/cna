...
// plan_dx3.md: real DirectX 3 graphics renderer (temporarily named DX3 -- see plan_dx3.md's own
// status note for why -- pending a still-not-executed rename of the existing, shipping
// ../free-direct-backed DX3 renderer to FREE_DIRECT). <ddraw.h> (and the real <windows.h> it pulls
// in) is contained to this .cpp only -- see Dx3Renderer.hpp's own comment. This file's 2D
...
namespace CNA::Internal::Renderers::Dx3
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx3TextureRenderer and
        // Dx3RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
    ...
    // Common accessor so Dx3SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx3TextureRenderer/Dx3RenderTargetRenderer themselves.
    ...
    struct Dx3Renderer::Impl
    {
        ...
        // Phase O3: the offscreen surface owned by the currently-bound Dx3RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx3RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE rather than
        // a Dx3RenderTargetRenderer* so Dx3RenderTargetRenderer never needs to name this private Impl
    ...
    Dx3Renderer::Dx3Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx3Renderer initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx3Renderer: could not obtain a real HWND from the SDL window.");
    ...
    Dx3Renderer::~Dx3Renderer() = default;

    void Dx3Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx3Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx3Renderer::Present: ClientToScreen failed");
    ...
    void Dx3Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx3Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx3Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx3Renderer::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx3Renderer::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx3Renderer::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx3Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    class Dx3VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit Dx3VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx3VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx3VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class Dx3IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        Dx3IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx3IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx3IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    /// (not cached) rather than stored on Dx3TextureRenderer: a handle is only valid for the device
    ...
    D3DTEXTUREHANDLE Dx3ResolveTextureHandle(const ITextureRenderer* texture, LPDIRECT3DDEVICE2 device2)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const Dx3SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx3Renderer: texture0 is not a DX3 surface (created by a different graphics renderer?)");
                                   ...
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx3Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx3Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx3Renderer::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class Dx3TextureRenderer : public ITextureRenderer, public Dx3SurfaceOwner
    {
    public:
        Dx3TextureRenderer(LPDIRECTDRAW2 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx3TextureRenderer(LPDIRECTDRAW2 dd, const ImageData& data)
            : Dx3TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx3TextureRenderer() override { if (surface_) surface_->Release(); }

        Dx3TextureRenderer(const Dx3TextureRenderer&) = delete;
        Dx3TextureRenderer& operator=(const Dx3TextureRenderer&) = delete;
    ...
    class Dx3RenderTargetRenderer final : public IRenderTargetRenderer, public Dx3SurfaceOwner
    {
    public:
        Dx3RenderTargetRenderer(LPDIRECTDRAWSURFACE* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx3RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx3RenderTargetRenderer(const Dx3RenderTargetRenderer&) = delete;
        Dx3RenderTargetRenderer& operator=(const Dx3RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> Dx3Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx3TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> Dx3Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx3RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx3Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void Dx3Renderer::SetRenderTargets(
    ...
    // Never named outside this .cpp, same reasoning as Dx3TextureRenderer/Dx3RenderTargetRenderer.

    class Dx3SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        Dx3SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE()> getActiveSurface,
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx3SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx3SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx3SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx3SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx3SpriteBatchRenderer::Draw: texture renderer is not a DX3 surface (created by "
    ...
    std::unique_ptr<ISpriteBatchRenderer> Dx3Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx3SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx3Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void Dx3Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    void Dx3Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
    ...
    void Dx3Renderer::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
    ...
    void Dx3Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }

    void Dx3Renderer::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void Dx3Renderer::ClearStencil(int) {}
    void Dx3Renderer::ClearDepthAndStencil(float depth, int) { ClearDepth(depth); }
    void Dx3Renderer::ClearColorAndStencil(float r, float g, float b, float a, int) { Clear(r, g, b, a); }
    void Dx3Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int)
    {
        ClearColorAndDepth(r, g, b, a, depth);
    }
    void Dx3Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void Dx3Renderer::SetBlendEnabled(bool) {}

    void Dx3Renderer::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device2->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferRenderer> Dx3Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx3VertexBufferRenderer>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx3Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx3IndexBufferRenderer>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx3Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx3IndexBufferRenderer>(index_capacity, true);
    }

    void Dx3Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   ...
                                                   PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx3Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx3VertexBufferRenderer&>(vb);
    ...
    void Dx3Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          ...
                                                          PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx3Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx3VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const Dx3IndexBufferRenderer&>(ib);
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DX3", route);
    }

    void Dx3Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        ...
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx3Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx3Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx3VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx3Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
    ...
    void Dx3Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        ...
        Dx3CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx3Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx3Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx3Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<Dx3::Dx3Renderer>(args);
    }
#endif
}
