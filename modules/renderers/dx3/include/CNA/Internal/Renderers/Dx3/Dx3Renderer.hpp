...
// did not exist until DX2), this renderer's 3D layer is real, built on IDirect3D2/IDirect3DDevice2
...
// a Dx3Renderer pointer. All real <ddraw.h> usage lives in Dx3Renderer.cpp behind the
// Impl pimpl below.
...
namespace CNA::Internal::Renderers::Dx3
{
    /**
     * DX2 graphics renderer (plan_dx2.md): its 2D layer is a verbatim port of DX1's own
     ...
     * and empirically at DX1's own DX1-0 spike, which this renderer inherits unchanged). DX1-0 also
     ...
     * private offscreen DDSCAPS_OFFSCREENPLAIN surfaces (Dx3TextureRenderer/Dx3RenderTargetRenderer,
     * defined entirely in Dx3Renderer.cpp -- neither is ever named outside it), and
     * SetRenderTarget2D() redirects Clear()/ReadBackbuffer() to whichever surface is currently
     * bound. The SpriteBatch CPU compositor (Phase O4, Dx3SpriteBatchRenderer, ported from DX1's
     ...
     */
    class Dx3Renderer final : public IGraphicsRenderer
    {
    public:
        explicit Dx3Renderer(const GraphicsRendererCreateArgs& args);
        ~Dx3Renderer() override;

        Dx3Renderer(const Dx3Renderer&) = delete;
        Dx3Renderer& operator=(const Dx3Renderer&) = delete;
        ...
        // Phase O5 (design decision 8): plain CPU-side storage (Dx3VertexBufferRenderer/
        // Dx3IndexBufferRenderer, defined entirely in Dx3Renderer.cpp), matching the
