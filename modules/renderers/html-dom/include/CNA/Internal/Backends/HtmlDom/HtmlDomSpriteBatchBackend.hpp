...
namespace CNA::Internal::Backends::HtmlDom
{
     ...
     * @throws std::runtime_error for an addressing mode this backend cannot reproduce; see
    ...
    /**
     * @brief `SpriteBatch` backend that emits pooled, CSS-transformed `<div>` elements.
     *
     ...
     */
    class HtmlDomSpriteBatchBackend final : public ISpriteBatchBackend
    {
    public:
        /** @brief Creates an empty batch; no DOM element is allocated until the first flush. */
        HtmlDomSpriteBatchBackend() = default;
    ...
    private:
        void QueueDraw(const ITextureBackend& texture,
                       const Rectangle& destinationRectangle,
                       const Rectangle& sourceRectangle,
                       const Color& color,
                       float rotation,
