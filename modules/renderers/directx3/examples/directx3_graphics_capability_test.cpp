...
// CNA::GraphicsCapability: verifies GraphicsDevice::SupportsCapability() correctly reports DIRECTX3's
// capability set. Unlike an earlier version of this test (mirroring a 2D-only renderer's
// capability test, matching DIRECTX1/SDL_RENDERER/CANVAS), DIRECTX3's 3D pipeline is now genuinely real
...
// confirmed D3DFILL_WIREFRAME genuinely renders edge-only output here (see DirectX3Renderer.hpp's
...
class DirectX3GraphicsCapabilityTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        auto& renderer = static_cast<CNA::Internal::Renderers::DirectX3::DirectX3Renderer&>(dev.GetRenderer());
        ...
        check(!renderer.SupportsStencilBuffer(), "standalone stencil hook reports depth-only DIRECTX3 truthfully");
        check(dev.SupportsCapability(GraphicsCapability::StencilBuffer) == renderer.SupportsStencilBuffer(),
        ...
        // above) is a real DIRECTX3 (and DIRECTX2, its own porting source) boundary: DirectDraw has exactly one active render target.
...
public:
    DirectX3GraphicsCapabilityTest()
    {
...
int main()
{
    DirectX3GraphicsCapabilityTest game;
    game.Run();
