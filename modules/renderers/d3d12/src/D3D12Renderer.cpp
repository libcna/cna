...
// stubs -- see D3D12Renderer.hpp's class doc comment for exactly why and what's next.
...
namespace CNA::Internal::Renderers::D3D12
{
    ...
    namespace
    {
        ...
        /// DX-111: same per-PrimitiveType vertex-count formula D3D11Renderer.cpp's own
    ...
    void D3D12Renderer::NotYetImplemented(const char* what)
    {
    ...
    D3D12Renderer::D3D12Renderer(const GraphicsRendererCreateArgs& args)
        : window_(args.window)
        , virtualWidth_(args.virtualWidth)
        , virtualHeight_(args.virtualHeight)
    {
        // DX-116: mirrors D3D11Renderer's own constructor exactly.
    ...
    void D3D12Renderer::CreateCommandQueueResources()
    {
    ...
    void D3D12Renderer::CreateDescriptorHeapResources()
    {
    ...
    void D3D12Renderer::CreateCommandListResources()
    {
        for (int i = 0; i < kFramesInFlight; ++i)
        {
    ...
    void D3D12Renderer::CreateFenceResources()
    {
    ...
    void D3D12Renderer::CreateSwapChainResources()
    {
    ...
    void D3D12Renderer::CreateWindowSizeDependentViews()
    {
        ...
        if (FAILED(hr))
            throw std::runtime_error("D3D12Renderer: back-buffer depth-stencil CreateCommittedResource failed, hr=" + FormatHr(hr));
    ...
    void D3D12Renderer::ReleaseWindowSizeDependentViews()
    {
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE D3D12Renderer::AllocateRtvDescriptorEXT()
    {
        return heaps_->rtv.Allocate();
    }

    void D3D12Renderer::FreeRtvDescriptorEXT(D3D12_CPU_DESCRIPTOR_HANDLE handle)
    {
        if (heaps_) heaps_->rtv.Free(handle);
    }

    D3D12_CPU_DESCRIPTOR_HANDLE D3D12Renderer::AllocateDsvDescriptorEXT()
    {
        return heaps_->dsv.Allocate();
    }

    void D3D12Renderer::FreeDsvDescriptorEXT(D3D12_CPU_DESCRIPTOR_HANDLE handle)
    {
        if (heaps_) heaps_->dsv.Free(handle);
    }

    std::uint32_t D3D12Renderer::CreateCbvSrvUavDescriptorEXT(
    ...
    void D3D12Renderer::FreeCbvSrvUavDescriptorEXT(std::uint32_t index)
    {
        if (heaps_) heaps_->cbvSrvUav.Free(index);
    }

    D3D12_GPU_DESCRIPTOR_HANDLE D3D12Renderer::GetCbvSrvUavGpuHandleEXT(std::uint32_t index) const
    {
        return heaps_ ? heaps_->cbvSrvUav.GpuHandle(index) : D3D12_GPU_DESCRIPTOR_HANDLE{};
    }

    D3D12_GPU_DESCRIPTOR_HANDLE D3D12Renderer::GetSamplerGpuHandleEXT(int slot)
    {
    ...
    void D3D12Renderer::ApplySamplerState(int slot, int filter, int addressU, int addressV,
                                                 int maxAnisotropy)
    {
    ...
    void D3D12Renderer::ExecuteCommandListAndWaitEXT(ID3D12CommandList* commandList)
    {
    ...
    std::uint64_t D3D12Renderer::SignalAndWaitForFrameEXT(int frameIndex)
    {
        const std::uint64_t valueToSignal = nextFenceValue_++;
        HRESULT hr = commandQueue_->Signal(fence_.Get(), valueToSignal);
        if (FAILED(hr))
    ...
    void D3D12Renderer::CheckDeviceRemovedEXT(HRESULT hr) const
    {
    ...
    void D3D12Renderer::RecreateDeviceEXT()
    {
    ...
    void D3D12Renderer::BindOffscreenColorTargetEXT(ID3D12Resource* resource,
    ...
    void D3D12Renderer::UnbindOffscreenColorTargetEXT()
    {
    ...
    void D3D12Renderer::SetViewport(int x, int y, int w, int h, float minDepth, float maxDepth)
    {
    ...
    D3D12_VIEWPORT D3D12Renderer::GetEffectiveViewportEXT() const
    {
    ...
    void D3D12Renderer::BindOffscreenColorTargetsEXT(ID3D12Resource* const* resources,
    ...
    void D3D12Renderer::RestoreBackBufferRenderTargetEXT()
    {
        if (!swapChainAvailable_)
        {
            UnbindOffscreenColorTargetEXT();
    ...
    void D3D12Renderer::CreateUploadConstantBuffer(UINT byteWidth, ComPtr<ID3D12Resource>& outResource,
                                                           void*& outMapped)
    {
        D3D12_HEAP_PROPERTIES heapProps{};
        ...
        if (FAILED(hr))
            throw std::runtime_error("D3D12Renderer: constant buffer CreateCommittedResource failed, hr=" + FormatHr(hr));

        const D3D12_RANGE readRange{0, 0}; // never read back on the CPU
        hr = outResource->Map(0, &readRange, &outMapped);
        if (FAILED(hr))
            throw std::runtime_error("D3D12Renderer: constant buffer Map failed, hr=" + FormatHr(hr));
    }

    ID3D12Resource* D3D12Renderer::GetOrCreatePerDrawConstantBufferEXT()
    {
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreateFogConstantBufferEXT()
    {
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreateLightingConstantBufferEXT()
    {
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreateAlphaTestConstantBufferEXT()
    {
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreateBoneConstantBufferEXT()
    {
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreateSkinnedExtraConstantBufferEXT()
    {
    ...
    D3D12_GPU_DESCRIPTOR_HANDLE D3D12Renderer::GetSrvGpuHandleForTextureEXT(const ITextureRenderer* tex) const
    {
        ...
        // DX-117: two-concrete-type resolution, mirroring D3D11Renderer::GetSrvForTextureEXT
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreateEnvMapPerDrawConstantBufferEXT()
    {
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreateEnvMapConstantBufferEXT()
    {
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreatePbrPerDrawConstantBufferEXT()
    {
    ...
    ID3D12Resource* D3D12Renderer::GetOrCreatePbrLightsConstantBufferEXT()
    {
    ...
    D3D12_GPU_DESCRIPTOR_HANDLE D3D12Renderer::GetSrvGpuHandleForTextureCubeEXT(const ITextureCubeRenderer* tex) const
    {
        D3D12_GPU_DESCRIPTOR_HANDLE handle{};
        if (tex == nullptr) return handle;
    ...
    ITextureRenderer* D3D12Renderer::GetOrCreateDefaultWhiteTextureEXT()
    {
    ...
    ITextureRenderer* D3D12Renderer::GetOrCreateDefaultFlatNormalTextureEXT()
    {
    ...
    std::unique_ptr<IRenderTargetRenderer> D3D12Renderer::CreateRenderTarget2D(
    ...
    void D3D12Renderer::FlushPendingCubeResolveEXT()
    {
    ...
    void D3D12Renderer::SetRenderTargetCubeFace(IRenderTargetCubeRenderer* rt, int face)
    {
    ...
    void D3D12Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        ...
        // and mip-chain generation never ran. Mirrors D3D11Renderer::SetRenderTarget2D()'s
    ...
    std::unique_ptr<IRenderTargetCubeRenderer> D3D12Renderer::CreateRenderTargetCube(
    ...
    void D3D12Renderer::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        ...
        // D3D11Renderer::SetRenderTargets()'s own currentCustomRT_ handling.
        ...
        for (int i = 0; i < count; ++i)
            if (renderTargets[i].IsRenderTargetCubeFace())
                throw std::runtime_error(
                    "D3D12Renderer::SetRenderTargets: cube faces in a multi-target "
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            if (!rt)
                throw std::runtime_error("D3D12Renderer::SetRenderTargets: target is not a real D3D12RenderTargetRenderer");
    ...
    void D3D12Renderer::Clear(float r, float g, float b, float a)
    {
        ...
        if (FAILED(hr))
            throw std::runtime_error("D3D12Renderer::Clear: command list Close failed, hr=" + FormatHr(hr));
    ...
    void D3D12Renderer::Present()
    {
        ...
        if (boundColorResource_ == currentBackBuffer)
        {
            ...
            HRESULT hr = cmdList->Close();
            if (FAILED(hr))
                throw std::runtime_error("D3D12Renderer::Present: command list Close failed, hr=" + FormatHr(hr));
            ExecuteCommandListAndWaitEXT(cmdList);
        }
    ...
    void D3D12Renderer::SetSwapInterval(int interval)
    {
        // DX-116: mirrors D3D11Renderer::SetSwapInterval exactly -- sync interval is
        // renderer state applied at the next Present(), not a direct D3D12 API call ahead of time.
        vsyncEnabled_ = interval > 0;
    }

    void D3D12Renderer::GetViewportSize(int& width, int& height)
    {
        width = virtualWidth_;
        height = virtualHeight_;
    }

    void D3D12Renderer::SetVirtualResolution(int width, int height)
    {
        virtualWidth_ = width;
        virtualHeight_ = height;
    }

    void D3D12Renderer::SetPresentationMode(int) { /* no-op until DX-106 onward */ }

    std::unique_ptr<ITextureRenderer> D3D12Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<D3D12TextureRenderer>(this, data);
    }

    std::unique_ptr<ITextureCubeRenderer> D3D12Renderer::CreateTextureCube(
        int size, bool mipMap, int surfaceFormat)
    {
        return std::make_unique<D3D12TextureCubeRenderer>(this, size, mipMap, surfaceFormat);
    }

    std::unique_ptr<ITexture3DRenderer> D3D12Renderer::CreateTexture3D(
        int w, int h, int depth, bool mipMap, int surfaceFormat)
    {
        return std::make_unique<D3D12Texture3DRenderer>(this, w, h, depth, mipMap, surfaceFormat);
    }

    std::unique_ptr<ISpriteBatchRenderer> D3D12Renderer::CreateSpriteBatch()
    {
        return std::make_unique<D3D12SpriteBatchRenderer>(this);
    }

    std::unique_ptr<IOcclusionQueryRenderer> D3D12Renderer::CreateOcclusionQuery()
    {
        return std::make_unique<D3D12OcclusionQueryRenderer>(this);
    }

    std::unique_ptr<IEffectRenderer> D3D12Renderer::CreateEffectRenderer(
    ...
    // six near-identical copies. Mirrors D3D11Renderer's own Clear*/ClearDepthStencilView
    ...
    // behavior identical.
    void D3D12Renderer::ClearImpl(bool clearColor, float r, float g, float b, float a,
                                          ...
                                          const char* what)
    {
        ...
        HRESULT hr = cmdList->Close();
        if (FAILED(hr))
            throw std::runtime_error(std::string("D3D12Renderer::") + what +
                                      ": command list Close failed, hr=" + FormatHr(hr));
        ExecuteCommandListAndWaitEXT(cmdList);
    }

    void D3D12Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
    ...
    void D3D12Renderer::ClearStencil(int stencil)
    {
        ClearImpl(false, 0, 0, 0, 0, D3D12_CLEAR_FLAG_STENCIL, 1.0f, stencil, "ClearStencil");
    }

    void D3D12Renderer::ClearDepthAndStencil(float depth, int stencil)
    {
    ...
    void D3D12Renderer::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        ClearImpl(true, r, g, b, a, D3D12_CLEAR_FLAG_STENCIL, 1.0f, stencil, "ClearColorAndStencil");
    }

    void D3D12Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a,
    ...
    // deliberate no-op, matching D3D11Renderer's own established convention for all three:
    ...
    void D3D12Renderer::SetDepthTestEnabled(bool enabled) { currentDepthEnable_ = enabled; }
    void D3D12Renderer::SetBlendEnabled(bool enabled) { (void)enabled; }
    void D3D12Renderer::SetDepthWriteEnabled(bool enabled) { currentDepthWriteEnable_ = enabled; }

    void D3D12Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void D3D12Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable,
                                                        int depthFunc,
    ...
    void D3D12Renderer::ApplyRasterizerState(int cullMode, int fillMode,
    ...
    std::unique_ptr<IVertexBufferRenderer> D3D12Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<D3D12VertexBufferRenderer>(this, vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> D3D12Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<D3D12IndexBufferRenderer>(this, index_capacity, /*thirtyTwoBit=*/false);
    }

    std::unique_ptr<IIndexBufferRenderer> D3D12Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<D3D12IndexBufferRenderer>(this, index_capacity, /*thirtyTwoBit=*/true);
    }

    void D3D12Renderer::DrawColoredPrimitives(
        ...
        PrimitiveType primitive, int primitiveCount)
    {
        ...
        {
            throw std::runtime_error(
                "D3D12Renderer::DrawColoredPrimitives: only stride-16 (VertexPositionColor) "
    ...
    void D3D12Renderer::DrawIndexedColoredPrimitives(
        ...
        PrimitiveType primitive, int primitiveCount)
    {
        ...
        {
            throw std::runtime_error(
                "D3D12Renderer::DrawIndexedColoredPrimitives: only stride-16 (VertexPositionColor) "
                "is implemented so far (plan_dx.md DX-111); other strides are a follow-up");
        }
    ...
    // D3D11Renderer::DrawPrimitivesExImpl's own priority-chain shape (alpha-test > lit-textured
    ...
    void D3D12Renderer::DrawPrimitivesExImpl(
        const IVertexBufferRenderer& vb, const IIndexBufferRenderer* ib,
        const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount, const GpuDrawParams& params)
    {
        ...
        // D3D11Renderer::DrawPrimitivesExImpl exactly (alpha-test/dual-tex/env-map still
        ...
        if (needsEnvMap && stride != 32)
            throw std::runtime_error(
                "D3D12Renderer::DrawPrimitivesEx: EnvironmentMapEffect (env_map3d) requires "
        ...
        if (needsDualTex && stride != 20)
            throw std::runtime_error(
                "D3D12Renderer::DrawPrimitivesEx: DualTextureEffect (dual_texture3d) only "
        ...
        if (needsSkinned && stride != 52 && stride != 56)
            throw std::runtime_error(
                "D3D12Renderer::DrawPrimitivesEx: SkinnedEffect (skinned3d) requires stride "
        ...
        if (needsAlphaTest && stride != 20 && stride != 24)
            throw std::runtime_error(
                "D3D12Renderer::DrawPrimitivesEx: AlphaTestEffect (alpha_test3d) only "
        ...
        if (needsPbr && !params.skinned && stride != 48)
            throw std::runtime_error(
                "D3D12Renderer::DrawPrimitivesEx: PbrEffect (pbr3d) requires stride 48 "
                "(VertexPositionNormalTangentTexture)");
        if (needsPbr && params.skinned && stride != 68)
            throw std::runtime_error(
                "D3D12Renderer::DrawPrimitivesEx: SkinnedPbrEffect (pbr_skinned3d) requires "
        ...
        {
            // Mirrors D3D11Renderer::DrawPrimitivesExImpl's own needsPbr branch exactly.
        ...
        else
        {
            ...
            default:
                throw std::runtime_error(
                    "D3D12Renderer::DrawPrimitivesEx: unsupported vertex stride " +
        ...
        // D3D11Renderer::DrawPrimitivesExImpl's own srvs[5] sizing.
        ...
        {
            // Mirrors D3D11Renderer::DrawPrimitivesExImpl's own needsEnvMap branch exactly --
        ...
        {
            // Mirrors D3D11Renderer::DrawPrimitivesExImpl's own needsSkinned branch exactly --
    ...
    void D3D12Renderer::DrawPrimitivesEx(
        const IVertexBufferRenderer& vb, const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount, const GpuDrawParams& params)
    {
    ...
    void D3D12Renderer::DrawIndexedPrimitivesEx(
        const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
        const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount, const GpuDrawParams& params)
    {
    ...
    ID3D12PipelineState* D3D12Renderer::GetOrCreateInstancedPsoEXT(
        ID3D12RootSignature* rootSig, UINT instanceStepRate)
    {
        ...
        if (!vsBytes || !psBytes)
            throw std::runtime_error("D3D12Renderer: missing instanced3d DXBC bytecode");

        // Mirrors D3D11Renderer::GetOrCreateInstancedInputLayoutEXT()'s own element list
        ...
        if (FAILED(hr))
            throw std::runtime_error("D3D12Renderer: instanced3d CreateGraphicsPipelineState failed, hr=" + FormatHr(hr));
        auto inserted = instancedPsos_.emplace(instanceStepRate, std::move(pso));
        return inserted.first->second.Get();
    }

    void D3D12Renderer::DrawInstancedPrimitivesEx(
        const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
        const Matrix& world, const Matrix& view, const Matrix& projection,
        PrimitiveType primitive, int primitiveCount, int instanceCount, const GpuDrawParams& params)
    {
        // Matches D3D11Renderer::DrawInstancedPrimitivesEx's own fallback -- no per-instance
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<D3D12::D3D12Renderer>(args);
    }
}
