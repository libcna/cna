...
// this .cpp only -- see Dx2Renderer.hpp's own comment. This file's 2D layer (ported from
...
namespace CNA::Internal::Renderers::Dx2
{
    ...
    namespace
    {
        ...
        // Dx1Renderer's own (permanent, for DX1) ThrowNo3D -- a later phase (O3/O4) replaces
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx2TextureRenderer and
        // Dx2RenderTargetRenderer (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
    ...
    // Common accessor so Dx2SpriteBatchRenderer can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx2TextureRenderer/Dx2RenderTargetRenderer themselves.
    ...
    struct Dx2Renderer::Impl
    {
        ...
        // Phase O3: the offscreen surface owned by the currently-bound Dx2RenderTargetRenderer, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx2RenderTargetRenderer::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE rather than
        // a Dx2RenderTargetRenderer* so Dx2RenderTargetRenderer never needs to name this private Impl
    ...
    Dx2Renderer::Dx2Renderer(const GraphicsRendererCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx2Renderer initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx2Renderer: could not obtain a real HWND from the SDL window.");
    ...
    Dx2Renderer::~Dx2Renderer() = default;

    void Dx2Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx2Renderer::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx2Renderer::Present: ClientToScreen failed");
    ...
    void Dx2Renderer::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx2Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx2Renderer::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx2Renderer::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx2Renderer::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx2Renderer::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx2Renderer::TransformLogicalToWindow(float logX, float logY,
    ...
    class Dx2VertexBufferRenderer final : public IVertexBufferRenderer
    {
    public:
        explicit Dx2VertexBufferRenderer(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx2VertexBufferRenderer::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx2VertexBufferRenderer::SetData: stride_in_bytes must be > 0");
    ...
    class Dx2IndexBufferRenderer final : public IIndexBufferRenderer
    {
    public:
        Dx2IndexBufferRenderer(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx2IndexBufferRenderer: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx2IndexBufferRenderer: SetData bit-width does not match the buffer's declared width");
    ...
    /// (not cached) rather than stored on Dx2TextureRenderer: a handle is only valid for the device
    ...
    D3DTEXTUREHANDLE Dx2ResolveTextureHandle(const ITextureRenderer* texture, LPDIRECT3DDEVICE2 device2)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const Dx2SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx2Renderer: texture0 is not a DX2 surface (created by a different graphics renderer?)");
                                   ...
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx2Renderer::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx2Renderer::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx2Renderer::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class Dx2TextureRenderer : public ITextureRenderer, public Dx2SurfaceOwner
    {
    public:
        Dx2TextureRenderer(LPDIRECTDRAW dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx2TextureRenderer(LPDIRECTDRAW dd, const ImageData& data)
            : Dx2TextureRenderer(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx2TextureRenderer() override { if (surface_) surface_->Release(); }

        Dx2TextureRenderer(const Dx2TextureRenderer&) = delete;
        Dx2TextureRenderer& operator=(const Dx2TextureRenderer&) = delete;
    ...
    class Dx2RenderTargetRenderer final : public IRenderTargetRenderer, public Dx2SurfaceOwner
    {
    public:
        Dx2RenderTargetRenderer(LPDIRECTDRAWSURFACE* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx2RenderTargetRenderer() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx2RenderTargetRenderer(const Dx2RenderTargetRenderer&) = delete;
        Dx2RenderTargetRenderer& operator=(const Dx2RenderTargetRenderer&) = delete;
    ...
    std::unique_ptr<ITextureRenderer> Dx2Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx2TextureRenderer>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetRenderer> Dx2Renderer::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx2RenderTargetRenderer>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx2Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (rt)
    ...
    void Dx2Renderer::SetRenderTargets(
    ...
    // Never named outside this .cpp, same reasoning as Dx2TextureRenderer/Dx2RenderTargetRenderer.

    class Dx2SpriteBatchRenderer final : public ISpriteBatchRenderer
    {
    public:
        Dx2SpriteBatchRenderer(std::function<LPDIRECTDRAWSURFACE()> getActiveSurface,
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx2SpriteBatchRenderer::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx2SpriteBatchRenderer::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx2SpriteBatchRenderer::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx2SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx2SpriteBatchRenderer::Draw: texture renderer is not a DX2 surface (created by "
    ...
    std::unique_ptr<ISpriteBatchRenderer> Dx2Renderer::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx2SpriteBatchRenderer>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx2Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void Dx2Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    void Dx2Renderer::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
    ...
    void Dx2Renderer::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
    ...
    void Dx2Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }

    void Dx2Renderer::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void Dx2Renderer::ClearStencil(int) {}
    void Dx2Renderer::ClearDepthAndStencil(float depth, int) { ClearDepth(depth); }
    void Dx2Renderer::ClearColorAndStencil(float r, float g, float b, float a, int) { Clear(r, g, b, a); }
    void Dx2Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int)
    {
        ClearColorAndDepth(r, g, b, a, depth);
    }
    void Dx2Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void Dx2Renderer::SetBlendEnabled(bool) {}

    void Dx2Renderer::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device2->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferRenderer> Dx2Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx2VertexBufferRenderer>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx2Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx2IndexBufferRenderer>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> Dx2Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx2IndexBufferRenderer>(index_capacity, true);
    }

    void Dx2Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                   ...
                                                   PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx2Renderer::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx2VertexBufferRenderer&>(vb);
    ...
    void Dx2Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb,
                                                          ...
                                                          PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx2Renderer::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx2VertexBufferRenderer&>(vb);
        const auto& dxIb = static_cast<const Dx2IndexBufferRenderer&>(ib);
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferRenderer& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::RendererRefusesIt, "DX2", route);
    }

    void Dx2Renderer::DrawPrimitivesEx(const IVertexBufferRenderer& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        ...
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx2Renderer::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx2Renderer::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx2VertexBufferRenderer&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx2Renderer::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
    ...
    void Dx2Renderer::DrawIndexedPrimitivesEx(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        ...
        Dx2CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx2Renderer::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx2Renderer::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx2Renderer::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<Dx2::Dx2Renderer>(args);
    }
#endif
}
