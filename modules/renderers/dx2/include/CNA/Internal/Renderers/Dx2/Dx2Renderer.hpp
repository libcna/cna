...
// a Dx2Renderer pointer. All real <ddraw.h> usage lives in Dx2Renderer.cpp behind the
// Impl pimpl below.
...
namespace CNA::Internal::Renderers::Dx2
{
     ...
     * private offscreen DDSCAPS_OFFSCREENPLAIN surfaces (Dx2TextureRenderer/Dx2RenderTargetRenderer,
     * defined entirely in Dx2Renderer.cpp -- neither is ever named outside it), and
     * SetRenderTarget2D() redirects Clear()/ReadBackbuffer() to whichever surface is currently
     * bound. The SpriteBatch CPU compositor (Phase O4, Dx2SpriteBatchRenderer, ported from DX1's
     ...
     */
    class Dx2Renderer final : public IGraphicsRenderer
    {
    public:
        explicit Dx2Renderer(const GraphicsRendererCreateArgs& args);
        ~Dx2Renderer() override;

        Dx2Renderer(const Dx2Renderer&) = delete;
        Dx2Renderer& operator=(const Dx2Renderer&) = delete;
        ...
        // Phase O5 (design decision 8): plain CPU-side storage (Dx2VertexBufferRenderer/
        // Dx2IndexBufferRenderer, defined entirely in Dx2Renderer.cpp), matching the
