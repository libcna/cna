...
// D3D9Renderer.hpp -- the latter drags in d3d9.h -> windows.h, which #defines a plain
...
#include "CNA/Internal/Renderers/D3D9/D3D9OcclusionQuery.hpp"
...
namespace CNA::Internal::Renderers::D3D9
{
    ...
    D3D9Renderer::D3D9Renderer(const GraphicsRendererCreateArgs& args)
        : window_(args.window)
    ...
    void D3D9Renderer::EnsureRenderReadyEXT() const
    {
        ThrowIfDeviceLost();
    ...
    void D3D9Renderer::InjectNextNativeFailureEXT(D3D9NativeOperationEXT operation,
                                                          HRESULT result)
    {
        if (!FAILED(result))
    ...
    void D3D9Renderer::ClearNativeFailureInjectionEXT()
    {
        nativeFailureInjection_.active = false;
    }

    bool D3D9Renderer::HasNativeFailureInjectionEXT() const
    {
        return nativeFailureInjection_.active;
    }

    std::uint64_t D3D9Renderer::GetNativeCallCountEXT(
        D3D9NativeOperationEXT operation) const
    {
        return nativeCallCounts_[NativeOperationIndex(operation)];
    }

    HRESULT D3D9Renderer::TakeInjectedResultEXT(D3D9NativeOperationEXT operation)
    {
    ...
    void D3D9Renderer::MarkStateUnsafeEXT(unsigned int safetyBit)
    {
        unsafeRenderStateMask_ |= safetyBit;
    }

    void D3D9Renderer::MarkStateKnownEXT(unsigned int safetyBit)
    {
        unsafeRenderStateMask_ &= ~safetyBit;
    }

    void D3D9Renderer::MarkDeviceLostFromScopedFailureEXT()
    {
    ...
    [[noreturn]] void D3D9Renderer::ThrowScopedFailureEXT(
    ...
    void D3D9Renderer::SetRenderStateCheckedEXT(
    ...
    void D3D9Renderer::SetViewportCheckedEXT(
    ...
    void D3D9Renderer::SetScissorRectCheckedEXT(
    ...
    HRESULT D3D9Renderer::InvokeSetRenderTargetEXT(DWORD slot, IDirect3DSurface9* surface)
    {
    ...
    HRESULT D3D9Renderer::InvokeSetDepthStencilSurfaceEXT(IDirect3DSurface9* surface)
    {
    ...
    HRESULT D3D9Renderer::InvokeGetRenderTargetEXT(DWORD slot, IDirect3DSurface9** surface)
    {
    ...
    HRESULT D3D9Renderer::InvokeGetDepthStencilSurfaceEXT(IDirect3DSurface9** surface)
    {
    ...
    HRESULT D3D9Renderer::InvokeGetBackBufferEXT(IDirect3DSurface9** surface)
    {
    ...
    HRESULT D3D9Renderer::InvokeSetViewportEXT(const D3DVIEWPORT9& viewport)
    {
    ...
    HRESULT D3D9Renderer::InvokeSetScissorRectEXT(const RECT& rect)
    {
    ...
    HRESULT D3D9Renderer::InvokeCreateDepthStencilSurfaceEXT(
    ...
    D3DPRESENT_PARAMETERS D3D9Renderer::BuildPresentParameters() const
    {
    ...
    void D3D9Renderer::CreateDeviceResources(const GraphicsRendererCreateArgs&)
    {
        if (!window_)
            throw std::runtime_error("D3D9Renderer: no window available to create a device for");

        HWND hwnd = static_cast<HWND>(SDL_GetPointerProperty(
            SDL_GetWindowProperties(window_), SDL_PROP_WINDOW_WIN32_HWND_POINTER, nullptr));
        if (!hwnd)
            throw std::runtime_error("D3D9Renderer: could not obtain HWND from SDL window");
    ...
    void D3D9Renderer::EnsureDeviceSize()
    {
        if (!window_) return;
    ...
    void D3D9Renderer::UpdatePresentationFormatEXT(int backBufferFormat, int depthStencilFormat,
                                                           bool isFullScreen)
    {
    ...
    void D3D9Renderer::RegisterDefaultPoolResourceEXT(ID3D9DefaultPoolResourceEXT* resource)
    {
        defaultPoolResources_.push_back(resource);
    }

    void D3D9Renderer::UnregisterDefaultPoolResourceEXT(ID3D9DefaultPoolResourceEXT* resource)
    {
        std::erase(defaultPoolResources_, resource);
    }

    void D3D9Renderer::PerformResetRecovery()
    {
    ...
    void D3D9Renderer::PollDeviceLost()
    {
    ...
    void D3D9Renderer::ThrowIfDeviceLost() const
    {
        if (deviceLost_)
        {
    ...
    void D3D9Renderer::GetViewportSize(int& width, int& height)
    {
        width = width_;
        height = height_;
    }

    void D3D9Renderer::SetVirtualResolution(int width, int height)
    {
        virtualWidth_ = width;
        virtualHeight_ = height;
    }

    void D3D9Renderer::SetPresentationMode(int mode)
    {
        presentationMode_ = mode;
    }

    void D3D9Renderer::Clear(float r, float g, float b, float a)
    {
    ...
    void D3D9Renderer::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        EnsureRenderReadyEXT();
    ...
    void D3D9Renderer::ClearDepth(float depth)
    {
        EnsureRenderReadyEXT();
    ...
    void D3D9Renderer::ClearStencil(int stencil)
    {
        EnsureRenderReadyEXT();
    ...
    void D3D9Renderer::ClearDepthAndStencil(float depth, int stencil)
    {
        EnsureRenderReadyEXT();
        if (!HasDepthBuffer(depthStencilFormatOrdinal_)) return;
    ...
    void D3D9Renderer::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        EnsureRenderReadyEXT();
        DWORD flags = D3DCLEAR_TARGET;
        if (HasStencilBuffer(depthStencilFormatOrdinal_)) flags |= D3DCLEAR_STENCIL;
    ...
    void D3D9Renderer::ClearColorDepthAndStencil(float r, float g, float b, float a,
    ...
    void D3D9Renderer::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
        EnsureRenderReadyEXT();
        if (w <= 0 || h <= 0) return;
    ...
    void D3D9Renderer::SetDepthTestEnabled(bool enabled)
    {
    ...
    void D3D9Renderer::SetBlendEnabled(bool) {}

    void D3D9Renderer::SetDepthWriteEnabled(bool enabled)
    {
        SetRenderStateCheckedEXT(D3DRS_ZWRITEENABLE, enabled ? TRUE : FALSE,
                                 kUnsafeDepthState, "SetDepthWriteEnabled(D3DRS_ZWRITEENABLE)");
    }

    std::unique_ptr<IVertexBufferRenderer> D3D9Renderer::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<D3D9VertexBufferRenderer>(*this, device_.Get(), vertex_capacity);
    }

    std::unique_ptr<IIndexBufferRenderer> D3D9Renderer::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<D3D9IndexBufferRenderer>(*this, device_.Get(), index_capacity, false);
    }

    std::unique_ptr<IIndexBufferRenderer> D3D9Renderer::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<D3D9IndexBufferRenderer>(*this, device_.Get(), index_capacity, true);
    }

    IDirect3DVertexDeclaration9* D3D9Renderer::GetOrCreateVertexDeclarationEXT(std::size_t strideInBytes)
    {
        ...
        if (!elements || count == 0)
            throw std::runtime_error(
                "D3D9Renderer: no vertex declaration for stride " + std::to_string(strideInBytes));

        ComPtr<IDirect3DVertexDeclaration9> decl;
        const HRESULT hr = device_->CreateVertexDeclaration(elements, decl.GetAddressOf());
        if (FAILED(hr))
            throw std::runtime_error("D3D9Renderer: CreateVertexDeclaration failed, hr=" + FormatHr(hr));
    ...
    void D3D9Renderer::DrawColoredPrimitives(const IVertexBufferRenderer& vb,
                                                     ...
                                                     PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (stride != 16)
            throw std::runtime_error(
                "D3D9Renderer::DrawColoredPrimitives: only stride-16 (VertexPositionColor) "
    ...
    void D3D9Renderer::DrawIndexedColoredPrimitives(const IVertexBufferRenderer& vb, const IIndexBufferRenderer& ib,
                                                            const Matrix& world, const Matrix& view, const Matrix& projection,
                                                            PrimitiveType primitive, int primitiveCount)
    {
        EnsureRenderReadyEXT();
        ...
        if (stride != 16)
            throw std::runtime_error(
                "D3D9Renderer::DrawIndexedColoredPrimitives: only stride-16 "
    ...
    std::unique_ptr<ITextureRenderer> D3D9Renderer::CreateTexture(const ImageData& data)
    {
        return std::make_unique<D3D9TextureRenderer>(device_.Get(), data);
    }

    std::unique_ptr<ITextureCubeRenderer> D3D9Renderer::CreateTextureCube(int size, bool mipMap, int surfaceFormat)
    {
    ...
    std::unique_ptr<ITexture3DRenderer> D3D9Renderer::CreateTexture3D(int w, int h, int depth, bool mipMap, int surfaceFormat)
    {
    ...
    std::unique_ptr<IRenderTargetRenderer> D3D9Renderer::CreateRenderTarget2D(
        int w, int h, int depthFormat, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<D3D9RenderTargetRenderer>(*this, device_.Get(), w, h, depthFormat, multiSampleCount);
    }

    std::unique_ptr<IRenderTargetCubeRenderer> D3D9Renderer::CreateRenderTargetCube(
    ...
    int D3D9Renderer::ClampMultiSampleCountEXT(D3DFORMAT format, int requested) const
    {
    ...
    void D3D9Renderer::RestoreBackBufferRenderTargetEXT()
    {
        ComPtr<IDirect3DSurface9> backBuffer;
    ...
    void D3D9Renderer::CacheDefaultDepthStencilSurfaceEXT()
    {
        defaultDepthStencilSurface_.Reset();
        if (!HasDepthBuffer(depthStencilFormatOrdinal_)) return;
    ...
    void D3D9Renderer::CreateDepthStencilSurfaceEXT(
        UINT width, UINT height, D3DFORMAT format, D3DMULTISAMPLE_TYPE multiSample, DWORD quality,
    ...
    void D3D9Renderer::BindRenderTargetSurfacesEXT(
    ...
    void D3D9Renderer::SetRenderTarget2D(IRenderTargetRenderer* rt)
    {
        if (!rt)
        {
    ...
    void D3D9Renderer::SetRenderTargetCubeFace(IRenderTargetCubeRenderer* rt, int face)
    {
        if (!rt)
        {
            if (currentCustomRT_) currentCustomRT_->ResolveForTransitionEXT();
    ...
    void D3D9Renderer::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        ...
        for (int i = 0; i < count; ++i)
            if (renderTargets[i].IsRenderTargetCubeFace())
                throw std::runtime_error(
                    "D3D9Renderer::SetRenderTargets: cube faces in a multi-target "
        ...
        {
            throw std::runtime_error(
                "D3D9Renderer::SetRenderTargets: requested " + std::to_string(count) +
    ...
    std::unique_ptr<IOcclusionQueryRenderer> D3D9Renderer::CreateOcclusionQuery()
    {
    ...
    std::unique_ptr<ISpriteBatchRenderer> D3D9Renderer::CreateSpriteBatch()
    {
        return std::make_unique<D3D9SpriteBatchRenderer>(this);
    }

    std::unique_ptr<IEffectRenderer> D3D9Renderer::CreateEffectRenderer(
    ...
    void D3D9Renderer::SetSwapInterval(int interval)
    {
    ...
    void D3D9Renderer::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
    ...
    void D3D9Renderer::SetBlendFactor(float r, float g, float b, float a)
    {
        SetRenderStateCheckedEXT(D3DRS_BLENDFACTOR, D3DCOLOR_COLORVALUE(r, g, b, a),
                                 kUnsafeBlendState, "SetBlendFactor(D3DRS_BLENDFACTOR)");
    }

    void D3D9Renderer::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable,
                                                      int depthFunc,
    ...
    void D3D9Renderer::SetScissorRect(int x, int y, int w, int h)
    {
        RECT rect{};
        rect.left = x;
        rect.top = y;
    ...
    void D3D9Renderer::ApplySamplerState(int slot, int filter, int addressU, int addressV, int maxAnisotropy)
    {
    ...
    void D3D9Renderer::SetViewport(int x, int y, int w, int h, float minDepth, float maxDepth)
    {
        D3DVIEWPORT9 vp{};
    ...
    void D3D9Renderer::SetContextRecoveryEnabled(bool)
    {
        NotYetImplemented("D3D9", "SetContextRecoveryEnabled");
    }

    void D3D9Renderer::SetStringMarkerEXT(const char*)
    {
        NotYetImplemented("D3D9", "SetStringMarkerEXT");
    }

    void D3D9Renderer::DebugSimulateContextLoss()
    {
    ...
    void D3D9Renderer::DebugRestoreContext()
    {
...
namespace CNA::Internal::Renderers
{
    ...
    {
        return std::make_unique<D3D9::D3D9Renderer>(args);
    }
}
