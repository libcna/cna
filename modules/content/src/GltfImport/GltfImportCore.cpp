...
namespace CNA::Internal::GltfImport
{
    namespace
    {
        ...
        /// One vertex's outgoing flat-normal group: the incident faces that agree on orientation.
        struct FlatNormalGroupEXT
        {
            /// The group's orientation as a unit vector; zero while only degenerate faces joined.
            Microsoft::Xna::Framework::Vector3 unit;
            /// Sum of the group's un-normalized face normals, so a merge is area-weighted.
            Microsoft::Xna::Framework::Vector3 accumulated;
            /// How many faces joined this group.
            std::size_t faces = 0;
            /// False only for a group opened by a face with no computable orientation.
            bool oriented = false;
            /// True once two faces whose unit normals are not identical joined the same group.
            bool averaged = false;
        };

        /// The vertex split §3.7.2.1's flat normals need, with the rewritten connectivity.
        struct FlatNormalGeometryEXT
        {
            /// New vertex index -> the source vertex it was gathered from. Identity when nothing split.
            std::vector<std::uint32_t> sourceVertex;
            /// The triangle list rewritten onto the new vertex numbering.
            std::vector<std::uint32_t> indices;
            /// Three floats per new vertex: that vertex's face normal.
            std::vector<float> normals;
            /// New vertices beyond the source count -- the price of exact flat shading.
            std::size_t duplicatedVertexCount = 0;
            /// New vertices whose normal averages faces that are parallel only within tolerance.
            std::size_t averagedVertexCount = 0;
        };

        /// Gathers a per-vertex float stream onto a new vertex numbering.
        ///
        /// @param source Interleaved @p components floats per source vertex; empty passes through.
        /// @param map New vertex index -> source vertex index.
        /// @param components Floats per vertex.
        /// @return The gathered stream, or empty when @p source is.
        std::vector<float> GatherVertexStreamEXT(const std::vector<float>& source,
                                                 const std::vector<std::uint32_t>& map,
                                                 std::size_t components)
        {
            if (source.empty() || components == 0) { return source; }
            std::vector<float> gathered(map.size() * components, 0.0f);
            for (std::size_t v = 0; v < map.size(); ++v)
            {
                const std::size_t from = static_cast<std::size_t>(map[v]) * components;
                if (from + components > source.size()) { continue; }
                for (std::size_t c = 0; c < components; ++c)
                {
                    gathered[v * components + c] = source[from + c];
                }
            }
            return gathered;
        }
        ...
        /// average such a vertex instead and count it, which is a *smoothed* normal presented as a
        /// flat one; this splits it, which is the answer the specification asks for.
        ///
        /// Faces are grouped per vertex, so the duplication is the minimum the geometry forces: a
        /// mesh whose author already split its edges, and a mesh whose shared vertices lie on
        /// coplanar faces, both come back with their original vertex count. New indices are handed
        /// out in source-vertex order, so an unsplit primitive keeps its numbering exactly and the
        /// generated bytes are unchanged.
        ///
        /// Two faces whose unit normals agree to within `kParallelFaceDotEXT` share a group, and
        /// the group's normal is their area-weighted sum. That tolerance exists so a pair of
        ...
        /// corpus non-reproducible. Any vertex whose group really did merge non-identical faces is
        /// counted in `averagedVertexCount`, so the residue is measured rather than assumed.
        ///
        /// @param positions Three floats per source vertex, in source (pre-split) numbering.
        /// @param indices A triangle list; every index must already be in range.
        /// @param vertexCount Source vertex count.
        /// @param splitEveryCorner When true every triangle corner gets its own vertex, whatever
        ///        its neighbours' orientation. §3.7.2.2 requires flat normals to be recomputed for
        ///        each morph target, and two faces that are coplanar at rest need not stay so once
        ///        positions move — only a fully split primitive can carry an exact per-face normal
        ///        at every reachable pose.
        /// @return The split plan, connectivity and normals.
        FlatNormalGeometryEXT BuildFlatNormalGeometryEXT(const std::vector<float>& positions,
                                                          const std::vector<std::uint32_t>& indices,
                                                          std::size_t vertexCount,
                                                          bool splitEveryCorner)
        {
            using Microsoft::Xna::Framework::Vector3;
            // ~0.081 degrees. See the doc comment: this is a reproducibility floor, not a
            // smoothing threshold.
            constexpr float kParallelFaceDotEXT = 1.0f - 1e-6f;
            constexpr float kDegenerateFaceLength = 1e-12f;

            const auto positionAt = [&](std::uint32_t index) {
                const std::size_t o = static_cast<std::size_t>(index) * 3;
                return Vector3(positions[o], positions[o + 1], positions[o + 2]);
            };

            std::vector<std::vector<FlatNormalGroupEXT>> groups(vertexCount);
            std::vector<std::uint32_t> cornerGroup(indices.size(), 0);

            for (std::size_t f = 0; f + 2 < indices.size(); f += 3)
            {
                const std::uint32_t corner[3] = {indices[f], indices[f + 1], indices[f + 2]};
                if (corner[0] >= vertexCount || corner[1] >= vertexCount || corner[2] >= vertexCount)
                {
                    continue;
                }
                const Vector3 a = positionAt(corner[0]);
                const Vector3 b = positionAt(corner[1]);
                const Vector3 c = positionAt(corner[2]);
                // Un-normalized on purpose: its length is twice the triangle's area, which is the
                // weight a large face should carry when a group does merge more than one.
                const Vector3 weighted = Vector3::Cross(b - a, c - a);
                const float length = weighted.Length();
                const bool oriented = length > kDegenerateFaceLength;
                const Vector3 unit = oriented
                    ? Vector3(weighted.X / length, weighted.Y / length, weighted.Z / length)
                    : Vector3(0.0f, 0.0f, 0.0f);

                for (std::size_t k = 0; k < 3; ++k)
                {
                    std::vector<FlatNormalGroupEXT>& vertexGroups = groups[corner[k]];
                    std::size_t chosen = vertexGroups.size();
                    if (!oriented)
                    {
                        // A degenerate face has no orientation to disagree with, so it never forces
                        // a duplicate: it joins whatever group the vertex already has.
                        chosen = vertexGroups.empty() ? vertexGroups.size() : 0u;
                    }
                    else if (!splitEveryCorner)
                    {
                        for (std::size_t g = 0; g < vertexGroups.size(); ++g)
                        {
                            if (!vertexGroups[g].oriented) { chosen = g; break; }
                            if (Vector3::Dot(vertexGroups[g].unit, unit) >= kParallelFaceDotEXT)
                            {
                                chosen = g;
                                break;
                            }
                    ...
                    if (chosen == vertexGroups.size()) { vertexGroups.emplace_back(); }

                    FlatNormalGroupEXT& group = vertexGroups[chosen];
                    if (oriented && group.oriented &&
                        (group.unit.X != unit.X || group.unit.Y != unit.Y || group.unit.Z != unit.Z))
                    {
                        group.averaged = true;
                    }
                    if (oriented && !group.oriented)
                    ...
                    group.accumulated = group.accumulated + weighted;
                    ++group.faces;
                    cornerGroup[f + k] = static_cast<std::uint32_t>(chosen);
                }
            }

            FlatNormalGeometryEXT result;
            // New indices are handed out in source-vertex order so that an unsplit primitive keeps
            // its numbering byte-for-byte.
            std::vector<std::vector<std::uint32_t>> groupVertex(vertexCount);
            result.sourceVertex.reserve(vertexCount);
            for (std::size_t v = 0; v < vertexCount; ++v)
            {
                // A vertex no face touches still occupies a slot: the file declared it, and
                // dropping it would renumber every index for no gain.
                const std::size_t slots = std::max<std::size_t>(groups[v].size(), 1u);
                for (std::size_t g = 0; g < slots; ++g)
                {
                    groupVertex[v].push_back(static_cast<std::uint32_t>(result.sourceVertex.size()));
                    result.sourceVertex.push_back(static_cast<std::uint32_t>(v));
                }
            }
            result.duplicatedVertexCount = result.sourceVertex.size() - vertexCount;

            result.normals.assign(result.sourceVertex.size() * 3, 0.0f);
            for (std::size_t v = 0; v < vertexCount; ++v)
            {
                for (std::size_t g = 0; g < groupVertex[v].size(); ++g)
                {
                    // A vertex touched by no face, or only by degenerate ones, has no computable
                    // normal. glTF states no default for that case, so the packing loop's own
                    // placeholder is the honest answer and is reproduced here.
                    Vector3 normal(0.0f, 0.0f, 1.0f);
                    if (g < groups[v].size())
                    {
                        const Vector3 accumulated = groups[v][g].accumulated;
                        const float length = accumulated.Length();
                        if (length > kDegenerateFaceLength)
                        {
                            normal = Vector3(accumulated.X / length, accumulated.Y / length,
                                             accumulated.Z / length);
                        }
                        if (groups[v][g].averaged) { ++result.averagedVertexCount; }
                    }
                    const std::size_t o = static_cast<std::size_t>(groupVertex[v][g]) * 3;
                    result.normals[o] = normal.X;
                    result.normals[o + 1] = normal.Y;
                    result.normals[o + 2] = normal.Z;
                }
            }
            ...
            for (std::size_t f = 0; f + 2 < indices.size(); f += 3)
            {
                for (std::size_t k = 0; k < 3; ++k)
                {
                    const std::uint32_t source = indices[f + k];
                    if (source >= vertexCount) { continue; }
                    result.indices[f + k] =
                        groupVertex[source][cornerGroup[f + k]];
                }
            }
            return result;
        }
                                 ...
                                 const std::string& subject, bool countPrimitive)
    {
        ...
        if (mesh.flatNormalDuplicatedVertexCountEXT > 0)
        {
            AddImportDiagnosticEXT(
                destination, "flat-normal-vertices-split",
                GltfImportDiagnosticKindEXT::GeneratedData,
                "Vertices shared by differently oriented faces were duplicated so each face "
                "carries its own flat normal, as §3.7.2.1 requires.",
                mesh.flatNormalDuplicatedVertexCountEXT, 0.0, subject);
        }
        if (mesh.flatNormalMergedVertexCountEXT > 0)
        {
            AddImportDiagnosticEXT(
                destination, "flat-normal-faces-merged",
                GltfImportDiagnosticKindEXT::Approximation,
                "Generated normals on these vertices average faces that are parallel only within "
                "the split's reproducibility tolerance rather than exactly.",
                mesh.flatNormalMergedVertexCountEXT, 0.0, subject);
        }
        if (mesh.morphedFlatNormalsEXT)
        {
            AddImportDiagnosticEXT(
                destination, "morphed-flat-normals", GltfImportDiagnosticKindEXT::GeneratedData,
                "The primitive authored no NORMAL and carries morph targets, so every corner was "
                "split and its flat normals are recomputed from the morphed positions.",
                1, 0.0, subject);
        }
        if (mesh.ignoredTangentForGeneratedNormalsEXT)
        {
            AddImportDiagnosticEXT(
                destination, "authored-tangent-ignored", GltfImportDiagnosticKindEXT::DroppedData,
                "§3.7.2.1 requires an authored tangent basis to be ignored when the primitive "
                "authors no NORMAL; a basis was generated from the computed normals instead.",
                1, 0.0, subject);
        }
        if (!mesh.ignoredMorphAttributesEXT.empty())
        {
            AddImportDiagnosticEXT(
                destination, "morph-attributes-ignored", GltfImportDiagnosticKindEXT::DroppedData,
                "These morph-target attributes were decoded by nobody: §3.7.2.2 makes morphed "
                "TEXCOORD_n and COLOR_n optional and CNA carries only POSITION, NORMAL and TANGENT.",
                mesh.ignoredMorphAttributesEXT.size(), 0.0, subject,
                mesh.ignoredMorphAttributesEXT);
        }
        if (mesh.ignoredMorphNormalDeltasForGeneratedNormalsEXT)
        {
            AddImportDiagnosticEXT(
                destination, "morph-normal-deltas-ignored",
                GltfImportDiagnosticKindEXT::DroppedData,
                "A morph target declared NORMAL deltas while the base primitive authors no NORMAL, "
                "which §3.7.2.2 does not permit; the flat normals of each morphed pose are computed "
                "instead.", 1, 0.0, subject);
        }
                         ...
                         const SkeletonResult* skel, float unitScale)
    {
        ...
        // arrived as a BasicEffect with no Normal slot at all, so it could not be lit, let alone
        // shaded: the file's normals, its tangent basis, every PBR factor and every PBR map were
        ...
        // GLTF-462: extracted for a coloured primitive as well, because a coloured metallic-
        // roughness primitive is now shaded by PbrEffect and PbrEffect samples a real occlusion
        // map. The DualTextureEffect approximation stays gated on uncoloured, unskinned, non-PBR --
        // SkinnedEffect has no Texture2 slot and the stride-24 colour layout has no second UV.
        out.material.occlusionImage =
            FindOcclusionImage(prim, &out.unsupportedTextureSourcesEXT);
        out.useDualTexture = (!out.usePbr) && (!out.skinned) && (!out.colored) &&
        ...
        // vertex is shared across differing faces and a *smoothed* normal wherever one is.
        //
        // GLTF-461 removes that residue: a vertex whose incident faces disagree is DUPLICATED, once
        // per orientation, which is what flat shading means. Everything the old comment said was
        // impossible -- the vertex count changing and with it every per-vertex stream including the
        // morph deltas -- is what the split plan below does, through one remap applied to every
        // stream rather than a special case per stream.
        //
        ...
        //
        // `vertexSource` stays empty when nothing split, which is both the common case and the
        // reason an unsplit primitive's bytes are unchanged: new indices are handed out in
        // source-vertex order, so identity is what an unsplit mesh gets.
        std::vector<std::uint32_t> vertexSource;
        cgltf_size packedVertexCount = vertexCount;
        const auto gatherFloats = [&](std::vector<float>& stream, std::size_t components) {
            if (!vertexSource.empty())
            {
                stream = GatherVertexStreamEXT(stream, vertexSource, components);
            }
        };
        if (normals.empty() && strideHasNormalSlot && ProducesTriangles(sourceTopology) &&
            !positions.empty() && indices.size() >= 3)
        {
            // §3.7.2.2: "When the base mesh primitive does not specify normals, client
            // implementations MUST calculate flat normals for each morph target". A pair of faces
            // that is coplanar at rest need not stay coplanar once a POSITION delta moves one of
            // their vertices, so a rest-pose split cannot serve every reachable pose -- only a
            // fully split primitive can. That is the whole reason the split is unconditional here
            // and minimal otherwise: this is the case where the geometry is not fixed.
            const bool morphed = prim.targets_count > 0;
            const FlatNormalGeometryEXT flat = BuildFlatNormalGeometryEXT(
                positions, indices, static_cast<std::size_t>(vertexCount), morphed);

            normals = flat.normals;
            indices = flat.indices;
            out.generatedNormalsEXT = true;
            out.flatNormalMergedVertexCountEXT = flat.averagedVertexCount;
            out.flatNormalDuplicatedVertexCountEXT = flat.duplicatedVertexCount;
            // The runtime must recompute the flat normals of a MORPHED primitive from the morphed
            // positions: §3.7.2.2 makes an original attribute a precondition for a target
            // attribute, so a base without NORMAL cannot legally carry NORMAL deltas at all, and a
            // normal blended from deltas that cannot exist is a normal that never changes.
            out.morphedFlatNormalsEXT = morphed;
            if (flat.duplicatedVertexCount > 0)
            {
                vertexSource = flat.sourceVertex;
                out.vertexSourceEXT = flat.sourceVertex;
                packedVertexCount = static_cast<cgltf_size>(vertexSource.size());
                ReserveOrRefuse(out.vertexBytes, static_cast<std::size_t>(packedVertexCount),
                                 static_cast<std::size_t>(out.stride), name);
                positions = GatherVertexStreamEXT(positions, vertexSource, 3);
            }
        }
        ...
        std::vector<float> uvs1 = uv1Acc
            ...
            : std::vector<float>();
        // GLTF-461: gathered here, before anything reads them, because a flat-normal split renumbers
        // every per-vertex stream. Tangent generation and the packing loop below both index by the
        // packed vertex, so a stream left in source numbering would silently pair the wrong UV with
        // the wrong position.
        gatherFloats(uvs, 2);
        gatherFloats(uvs1, 2);
        ...
        // GLTF-461: gathered after the influence and renormalisation sweeps above, which count and
        // report PER SOURCE VERTEX -- gathering first would multiply a split vertex's own report
        // entry by the number of copies the geometry happened to need.
        gatherFloats(weights, 4);
        std::vector<float> joints = out.skinned
            ? unpackSemantic(cgltf_attribute_type_joints, 0, jointsAcc, 4, "JOINTS_0") : std::vector<float>();
        gatherFloats(joints, 4);
        ...
        std::vector<float> colors = out.colored
        ...
        if (out.usePbr)
        {
            ...
            // so keeping it pairs a tangent with a normal it is not orthogonal to and normal
            // mapping lights the surface through a skewed frame. CNA used the authored tangent
            // regardless, which is that exact case.
            const bool ignoreAuthoredTangent = out.generatedNormalsEXT;
            if (tangentAcc != nullptr && ignoreAuthoredTangent)
            {
                out.ignoredTangentForGeneratedNormalsEXT = true;
            }
            if (tangentAcc != nullptr && !ignoreAuthoredTangent)
            {
                std::vector<float> raw = unpackSemantic(cgltf_attribute_type_tangent, 0, tangentAcc, 4, "TANGENT");
                gatherFloats(raw, 4);
            ...
            else
            {
                ...
                tangents = ComputeTangentsEXT(
                    positions, normals, tangentUvs, indices, packedVertexCount);
            }
        }

        for (cgltf_size i = 0; i < packedVertexCount; ++i)
        {
            ...
            if (out.usePbr)
            {
                // stride 48 / 60 (unskinned) and 68 / 76 / 80 (skinned): Position + Normal +
                // Tangent + TextureCoordinate [+ BlendWeight + BlendIndices] [+ TextureCoordinate1]
                // [+ Color].
                ...
                // The second UV slot exists whenever the record has one at all, which a colour
                // forces even when the material samples a single set: a stride is a layout, not a
                // promise that every slot is used, and the per-map selector mask already tells a
                // shader which channel to read. Its value is the packer's own absent-stream fill
                // when the file authors no second set.
                if (out.hasSecondTexcoordEXT || out.colored)
                {
                    AppendFloat(out.vertexBytes, u1); AppendFloat(out.vertexBytes, v1);
                }
                if (out.colored)
                {
                    // Rigid: stride 60's four reserved discriminator bytes. Skinned: stride 80's own
                    // appended slot. One rule, two records.
                    appendColor();
                }
                else if (out.hasSecondTexcoordEXT && !out.skinned)
                {
                    // The naturally 56-byte rigid record collides with skinned+colour, so the
                    // canonical stride-60 layout spends four bytes on a discriminator. Those bytes
                    // are the colour slot (GLTF-462), and OPAQUE WHITE is its identity value: a
                    // renderer that samples it for an uncoloured primitive multiplies by one instead
                    // of darkening the surface to nothing, which is what a zero fill would do.
                    out.vertexBytes.push_back(255); out.vertexBytes.push_back(255);
                    out.vertexBytes.push_back(255); out.vertexBytes.push_back(255);
                }
                continue;
            }
            ...
            {
                // stride 24: Position + Color + TextureCoordinate. Reached only by a primitive whose
                // material declares KHR_materials_unlit, which has no lighting to lose a Normal to.
                appendColor();
                AppendFloat(out.vertexBytes, u); AppendFloat(out.vertexBytes, v);
                continue;
            }
        ...
        out.use32BitIndices = packedVertexCount > 65535;
        ...
        for (cgltf_size ti = 0; ti < prim.targets_count; ++ti)
        {
            ...
            // GLTF-461: a morph delta array is a per-vertex stream like any other, so it is
            // gathered through the same split remap. The pre-split accessor length is what
            // cgltf_validate checked against POSITION's count, so the read below stays in the
            // source numbering and the gather does the renumbering exactly once.
            const auto deltaAt = [&](const std::vector<float>& deltas, cgltf_size v) {
                const std::size_t source = vertexSource.empty()
                    ? static_cast<std::size_t>(v)
                    : static_cast<std::size_t>(vertexSource[static_cast<std::size_t>(v)]);
                const std::size_t o = source * 3;
                return (o + 2 < deltas.size()) ? Vector3(deltas[o], deltas[o + 1], deltas[o + 2])
                                               : Vector3::Zero;
            };
            ...
            // indistinguishable from a bug. Named once per distinct semantic across all targets.
            for (cgltf_size a = 0; a < target.attributes_count; ++a)
            {
                ...
                const std::string semantic = attribute.name != nullptr ? attribute.name
                                                                       : "<unnamed>";
                if (std::find(out.ignoredMorphAttributesEXT.begin(),
                              out.ignoredMorphAttributesEXT.end(), semantic) ==
                    out.ignoredMorphAttributesEXT.end())
                {
                    out.ignoredMorphAttributesEXT.push_back(semantic);
                }
            }
            ...
            if (posDeltaAcc)
            {
                out.morphPositionDeltas[ti].resize(packedVertexCount);
                const std::vector<float> deltas = UnpackAccessor(posDeltaAcc, 3, "morph target POSITION delta");
                for (cgltf_size v = 0; v < packedVertexCount; ++v)
                {
                    out.morphPositionDeltas[ti][v] = deltaAt(deltas, v) * unitScale;
                }
            }
            ...
            // GLTF-461: §3.7.2.2 makes an original attribute a precondition for a target attribute,
            // so a base primitive without NORMAL cannot legally carry NORMAL deltas -- and if a file
            // does anyway, "MUST calculate flat normals for each morph target" says to compute them
            // rather than blend the deltas. Ignoring them here is what keeps the runtime's
            // recomputation from being fought by a delta blend applied on top of it.
            if (normDeltaAcc != nullptr && out.generatedNormalsEXT)
            {
                out.ignoredMorphNormalDeltasForGeneratedNormalsEXT = true;
            }
            else if (normDeltaAcc)
            {
                const std::vector<float> deltas = UnpackAccessor(normDeltaAcc, 3, "morph target NORMAL delta");
                out.morphNormalDeltas[ti].resize(packedVertexCount);
                for (cgltf_size v = 0; v < packedVertexCount; ++v)
                {
                    out.morphNormalDeltas[ti][v] = deltaAt(deltas, v);
                }
            }
            ...
            // GLTF-461: "the provided tangents and their displacements (if present) MUST be
            // ignored" when the base authors no NORMAL. The tangent basis CNA generated belongs to
            // the geometry, not to the file, and displacing it by deltas that were authored against
            // a normal set the file never supplied is exactly the skewed frame §3.7.2.1 forbids.
            if (tangentDeltaAcc != nullptr && out.generatedNormalsEXT)
            {
                out.ignoredTangentForGeneratedNormalsEXT = true;
            }
            else if (tangentDeltaAcc)
            {
                const std::vector<float> deltas =
                    UnpackAccessor(tangentDeltaAcc, 3, "morph target TANGENT delta");
                out.morphTangentDeltas[ti].resize(packedVertexCount);
                for (cgltf_size v = 0; v < packedVertexCount; ++v)
                {
                    out.morphTangentDeltas[ti][v] = deltaAt(deltas, v);
                }
            }
        }
    ...
    const std::vector<VertexLayoutRuleEXT>& VertexLayoutTableEXT()
    {
        static const std::vector<VertexLayoutRuleEXT> table = {
            ...
            // Unskinned, coloured. GLTF-462: a metallic-roughness primitive takes the rigid PBR
            // layout and carries its COLOR_0 in the four bytes GLTF-182 had reserved as stride 60's
            // discriminator -- so the authored Normal, the tangent basis, every PBR factor and map
            // and the vertex colour all survive together. Stride 24 is now reached only by a
            // primitive whose material declares KHR_materials_unlit, where a Normal has nothing to
            // light and losing it costs nothing.
            {{false, true, true, false, true}, 60, ""},
            {{false, true, true, false, false}, 60, ""},
            {{false, true, false, false, false}, 24,
             "the Normal stream: stride 24 (Position+Color+TextureCoordinate) has no normal slot, "
             "which an unlit material does not need"},
