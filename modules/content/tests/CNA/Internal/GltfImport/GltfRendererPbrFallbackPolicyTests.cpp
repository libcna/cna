...
TEST(GltfRendererPbrFallbackPolicy, EverySkinnedPbrRendererEitherBindsTheStride80RecordOrRefusesIt)
{
    ...
    constexpr std::array<Stride80Audit, 10> binds{{
    ...
    constexpr std::array<const char*, 6> refuses{{
        "directx9", "llgl",
...
TEST(GltfRendererPbrFallbackPolicy, VertexColourReachesTheBaseColourProductOnlyWhereItIsImplemented)
{
    ...
    constexpr std::array<VertexColourPbrAudit, 10> alphaProduct{{
    ...
    constexpr std::array<VertexColourPbrAudit, 9> upload{{
    ...
    constexpr std::array<OpenVertexColourRenderer, 6> notYet{{
        {"directx9", "vs_3_0/ps_3_0 bytecode needs the pinned native d3dcompiler_47.dll prefix"},
        {"llgl", "its committed SPIR-V was built by a glslang this environment cannot reproduce"},
...
TEST(GltfRendererPbrFallbackPolicy, EveryPbrRendererEitherAppliesVertexColourOrRefusesTheDrawExplicitly)
{
    ...
    constexpr std::array<const char*, 11> applies{{
        ...
        "magnum", "diligent", "bgfx",
    }};
    // Refuses the draw through the shared guard. Two shapes of renderer are here for two different
    // reasons, and both end at the same behaviour: llgl has a stride-60 layout and WOULD have drawn
    // the asset with the identity, so for it the guard is a behavioural fix;
