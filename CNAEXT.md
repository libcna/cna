...
| `KHR_materials_ior` / `KHR_materials_specular` | ⚠️ | Raw factors survive both loaders and `.cnj`; shader-ready dielectric F0/F90 are analytically verified at L6 and consumed by both EasyGL PBR programs with an OPENGLES3 pixel witness. Other PBR renderers and the specular extension's two texture inputs remain absent, so neither extension is claimed. |
