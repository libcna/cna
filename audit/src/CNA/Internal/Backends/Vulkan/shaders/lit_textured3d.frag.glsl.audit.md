# Audit: src/CNA/Internal/Backends/Vulkan/shaders/lit_textured3d.frag.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/lit_textured3d.frag.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: BasicEffect per-pixel-lit fragment shader — 3-light Lambert+Blinn-Phong specular (FNA Lighting.fxh ComputeLights), EmissiveColor added post-multiply (correct, matches FNA), fog mix.
...
BasicEffect per-pixel-lit fragment shader — 3-light Lambert+Blinn-Phong specular (FNA Lighting.fxh ComputeLights), EmissiveColor added post-multiply (correct, matches FNA), fog mix.
...
BasicEffect per-pixel-lit fragment shader — 3-light Lambert+Blinn-Phong specular (FNA Lighting.fxh ComputeLights), EmissiveColor added post-multiply (correct, matches FNA), fog mix. Fog mix (`mix(FogColor, color, fogFactor)`) is correct in isolation; the mirrored-formula bug (already recorded) lives in the paired vertex shader's fog-factor computation, not here.
