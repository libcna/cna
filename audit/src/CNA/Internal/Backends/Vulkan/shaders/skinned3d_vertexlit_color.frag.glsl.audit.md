# Audit: src/CNA/Internal/Backends/Vulkan/shaders/skinned3d_vertexlit_color.frag.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/skinned3d_vertexlit_color.frag.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: SkinnedEffect stride-56 per-vertex-lit +Color fragment shader — same correct vertex-color-after-specular ordering as skinned3d_color.frag.glsl.
...
SkinnedEffect stride-56 per-vertex-lit +Color fragment shader — same correct vertex-color-after-specular ordering as skinned3d_color.frag.glsl.
...
SkinnedEffect stride-56 per-vertex-lit +Color fragment shader — same correct vertex-color-after-specular ordering as skinned3d_color.frag.glsl. Fog mix (`mix(FogColor, color, fogFactor)`) is correct in isolation; the mirrored-formula bug (already recorded) lives in the paired vertex shader's fog-factor computation, not here.
