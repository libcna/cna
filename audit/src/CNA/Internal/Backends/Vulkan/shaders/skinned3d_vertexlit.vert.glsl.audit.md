# Audit: src/CNA/Internal/Backends/Vulkan/shaders/skinned3d_vertexlit.vert.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/skinned3d_vertexlit.vert.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: SkinnedEffect per-vertex-lit sibling of skinned3d (PreferPerPixelLighting=false).
...
SkinnedEffect per-vertex-lit sibling of skinned3d (PreferPerPixelLighting=false).
