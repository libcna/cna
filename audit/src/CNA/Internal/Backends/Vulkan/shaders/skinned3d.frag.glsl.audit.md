# Audit: src/CNA/Internal/Backends/Vulkan/shaders/skinned3d.frag.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/skinned3d.frag.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: SkinnedEffect per-pixel-lit fragment shader — 3-light Lambert+specular, AmbientColor consumed (always zero due to the upstream SkinnedEffect::FillGpuDrawParams() gap, see AUDIT_CROSS_CUTTING_FINDINGS.md), no EmissiveColor field at all in this shader's UBO (structural, not just unset).
...
SkinnedEffect per-pixel-lit fragment shader — 3-light Lambert+specular, AmbientColor consumed (always zero due to the upstream SkinnedEffect::FillGpuDrawParams() gap, see AUDIT_CROSS_CUTTING_FINDINGS.md), no EmissiveColor field at all in this shader's UBO (structural, not just unset).
...
SkinnedEffect per-pixel-lit fragment shader — 3-light Lambert+specular, AmbientColor consumed (always zero due to the upstream SkinnedEffect::FillGpuDrawParams() gap, see AUDIT_CROSS_CUTTING_FINDINGS.md), no EmissiveColor field at all in this shader's UBO (structural, not just unset). Fog mix (`mix(FogColor, color, fogFactor)`) is correct in isolation; the mirrored-formula bug (already recorded) lives in the paired vertex shader's fog-factor computation, not here.
