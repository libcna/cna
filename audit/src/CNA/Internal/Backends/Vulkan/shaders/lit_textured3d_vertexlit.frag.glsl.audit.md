# Audit: src/CNA/Internal/Backends/Vulkan/shaders/lit_textured3d_vertexlit.frag.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/lit_textured3d_vertexlit.frag.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: BasicEffect per-vertex-lit fragment shader — consumes interpolated per-vertex lit/specular RGB, texture sample, fog mix.
...
BasicEffect per-vertex-lit fragment shader — consumes interpolated per-vertex lit/specular RGB, texture sample, fog mix.
...
BasicEffect per-vertex-lit fragment shader — consumes interpolated per-vertex lit/specular RGB, texture sample, fog mix. Fog mix (`mix(FogColor, color, fogFactor)`) is correct in isolation; the mirrored-formula bug (already recorded) lives in the paired vertex shader's fog-factor computation, not here.
