# Audit: src/CNA/Internal/Backends/Vulkan/shaders/skinned3d_vertexlit.frag.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/skinned3d_vertexlit.frag.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: SkinnedEffect per-vertex-lit fragment shader — consumes interpolated per-vertex lit/specular RGB, fog mix.
...
SkinnedEffect per-vertex-lit fragment shader — consumes interpolated per-vertex lit/specular RGB, fog mix.
...
SkinnedEffect per-vertex-lit fragment shader — consumes interpolated per-vertex lit/specular RGB, fog mix. Fog mix (`mix(FogColor, color, fogFactor)`) is correct in isolation; the mirrored-formula bug (already recorded) lives in the paired vertex shader's fog-factor computation, not here.
