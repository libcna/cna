# Audit: src/CNA/Internal/Backends/Vulkan/shaders/lit_textured3d.vert.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/lit_textured3d.vert.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: BasicEffect stride-32 (VertexPositionNormalTexture) per-pixel-lit vertex shader (PreferPerPixelLighting=true).
...
BasicEffect stride-32 (VertexPositionNormalTexture) per-pixel-lit vertex shader (PreferPerPixelLighting=true).
