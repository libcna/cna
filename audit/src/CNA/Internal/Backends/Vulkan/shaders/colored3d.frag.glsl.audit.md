# Audit: src/CNA/Internal/Backends/Vulkan/shaders/colored3d.frag.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/colored3d.frag.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: Colored3D fragment shader — flat tint + fog mix, no texture sampling.
...
Colored3D fragment shader — flat tint + fog mix, no texture sampling.
...
Colored3D fragment shader — flat tint + fog mix, no texture sampling. Fog mix (`mix(FogColor, color, fogFactor)`) is correct in isolation; the mirrored-formula bug (already recorded) lives in the paired vertex shader's fog-factor computation, not here.
