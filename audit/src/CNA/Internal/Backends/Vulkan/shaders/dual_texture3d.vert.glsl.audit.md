# Audit: src/CNA/Internal/Backends/Vulkan/shaders/dual_texture3d.vert.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/dual_texture3d.vert.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: DualTextureEffect stride-20 (VertexPositionTexture) vertex shader, dedicated (not shared with textured3d since Task 899).
...
DualTextureEffect stride-20 (VertexPositionTexture) vertex shader, dedicated (not shared with textured3d since Task 899).
