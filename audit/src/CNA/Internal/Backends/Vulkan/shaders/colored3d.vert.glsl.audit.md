# Audit: src/CNA/Internal/Backends/Vulkan/shaders/colored3d.vert.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/colored3d.vert.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: Colored3D (VertexPositionColor) vertex shader — flat-color 3D geometry, DrawColoredPrimitives' modern (post-Task-899, fog-capable) path.
...
Colored3D (VertexPositionColor) vertex shader — flat-color 3D geometry, DrawColoredPrimitives' modern (post-Task-899, fog-capable) path.
