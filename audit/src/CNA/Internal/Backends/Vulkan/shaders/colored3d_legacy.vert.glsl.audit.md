# Audit: src/CNA/Internal/Backends/Vulkan/shaders/colored3d_legacy.vert.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/colored3d_legacy.vert.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: Legacy, zero-descriptor-set Colored3D vertex shader predating Task 899's fog support — kept for DrawColoredPrimitives()/DrawIndexedColoredPrimitives() backward compatibility; unique to Vulkan, no equivalent in any other backend.
...
Legacy, zero-descriptor-set Colored3D vertex shader predating Task 899's fog support — kept for DrawColoredPrimitives()/DrawIndexedColoredPrimitives() backward compatibility; unique to Vulkan, no equivalent in any other backend.
