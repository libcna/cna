# Audit: src/CNA/Internal/Backends/Vulkan/shaders/dual_texture3d.frag.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/dual_texture3d.frag.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: DualTextureEffect fragment shader — 2-texture multiplicative blend (tex1*2.0*tex2*tint, matching FNA's DualTextureEffect convention) + fog mix.
...
DualTextureEffect fragment shader — 2-texture multiplicative blend (tex1*2.0*tex2*tint, matching FNA's DualTextureEffect convention) + fog mix.
...
DualTextureEffect fragment shader — 2-texture multiplicative blend (tex1*2.0*tex2*tint, matching FNA's DualTextureEffect convention) + fog mix. Fog mix (`mix(FogColor, color, fogFactor)`) is correct in isolation; the mirrored-formula bug (already recorded) lives in the paired vertex shader's fog-factor computation, not here.
