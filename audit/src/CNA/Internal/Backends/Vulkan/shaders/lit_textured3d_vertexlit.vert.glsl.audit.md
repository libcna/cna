# Audit: src/CNA/Internal/Backends/Vulkan/shaders/lit_textured3d_vertexlit.vert.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/lit_textured3d_vertexlit.vert.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: BasicEffect per-vertex-lit sibling of lit_textured3d (PreferPerPixelLighting=false, XNA's real default) — lighting evaluated once per vertex, Gouraud-interpolated.
...
BasicEffect per-vertex-lit sibling of lit_textured3d (PreferPerPixelLighting=false, XNA's real default) — lighting evaluated once per vertex, Gouraud-interpolated.
