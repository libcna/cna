# Audit: src/CNA/Internal/Backends/Vulkan/shaders/colored_textured3d.frag.glsl

## Metadata

- Source file: `src/CNA/Internal/Backends/Vulkan/shaders/colored_textured3d.frag.glsl`
- Audit status: AUDITED
- Subsystem: `backend-vulkan` shard
- File type: GLSL shader source (compiled to SPIR-V via `compile_shaders.py`/`shaderc`)
- XNA/FNA relevance: BasicEffect stride-24 fragment shader — texture sample + vertex-color-gated tint + fog mix.
...
BasicEffect stride-24 fragment shader — texture sample + vertex-color-gated tint + fog mix.
...
BasicEffect stride-24 fragment shader — texture sample + vertex-color-gated tint + fog mix. Fog mix (`mix(FogColor, color, fogFactor)`) is correct in isolation; the mirrored-formula bug (already recorded) lives in the paired vertex shader's fog-factor computation, not here.
