# Audit: src/CNA/Internal/Backends/D3DCommon/shaders/alpha_test_colored3d.vert.hlsl

## Metadata

- Source file: `src/CNA/Internal/Backends/D3DCommon/shaders/alpha_test_colored3d.vert.hlsl`
- Audit status: AUDITED
- Subsystem: `backend-d3dcommon` shard (shared D3D11/D3D12 HLSL source)
- File type: HLSL shader source
- XNA/FNA relevance: AlphaTestEffect vertex stage, VertexColorEnabled variant (stride-24 VertexPositionColorTexture)
- Graphics backend relevance: compiled into both the D3D11 and D3D12 backends via this shared directory
- FNA reference: AlphaTestEffect.fx / Lighting.fxh (fog only)
- Main related tests: covered indirectly by `examples-tests-generic`/backend-specific example shards where a
  cross-backend test happens to register on D3D11/D3D12; no D3D11/D3D12-specific shader unit test found

## Purpose

Sibling of `alpha_test3d.vert.hlsl` adding a per-vertex Color input, mixed with DiffuseColor when `VertexColorEnabled` is set.

## Executive Verdict

**Needs attention (shares the systemic fog bug).**

## Checklist Results

### API / XNA parity
`VertexColorEnabled` gating (`input.Color * DiffuseColor` vs `DiffuseColor` alone) is correct and matches the same convention as `colored_textured3d.vert.hlsl`.

### Systematic FNA parity gaps
**MEDIUM — shares the cross-cutting D3DCommon fog-formula bug.** Line 50: `output.FogFactor = (FogEnabled > 0.5) ? saturate((FogEnd - input.Position.z) / max(FogEnd - FogStart, 1e-6)) : 1.0;` — algebraically
`(FogEnd - z) / (FogEnd - FogStart)`, the mirror-image of FNA's real `SetFogVector`/`ComputeFogFactor` formula
(`(z + FogEnd) / (FogEnd - FogStart)`), already proven wrong by this project's own XNA-oracle diff (commit
`74ad3bae`) and fixed in EasyGL but never ported here. Confirmed identical in all 15 fog-capable D3DCommon vertex
shaders (see `AUDIT_CROSS_CUTTING_FINDINGS.md`, "Systematic FNA parity gaps" — D3D11+D3D12 is the widest single
instance of this bug in the audit so far, affecting literally every fog-capable shader in the shared source).

## Detailed Findings

**F1 (MEDIUM):** mirrored fog formula, line 50.

## Cross-File Observations

Shares the fog bug with all fog-capable D3DCommon siblings.

## Missing or Weak Tests

No dedicated D3D11/D3D12 test found exercising this exact variant's fog behavior.

## Positive Findings

Correct division of responsibility with the fragment shader (this file computes/forwards the combined Tint; `alpha_test_colored3d.frag.hlsl` never re-reads vertex color directly).

## Final Assessment

One confirmed MEDIUM finding (shared fog-formula bug), otherwise correct.
