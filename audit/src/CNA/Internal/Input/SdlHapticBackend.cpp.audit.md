# Audit: src/CNA/Internal/Input/SdlHapticBackend.cpp

## Metadata

- Source file: `src/CNA/Internal/Input/SdlHapticBackend.cpp`
...
Implements RealSdlHapticBackend: a thin 1:1 forwarding layer over the real SDL3 haptic API.
...
Every one of the 21 interface methods forwards directly to its named SDL3 function with no transformation, including correct `SDL_free()` after copying `SDL_GetHaptics()`'s heap-allocated array into a `std::vector` (`GetHaptics()`).
