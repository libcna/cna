...
**Status:** PARTIALLY FIXED. Twenty-two crash classes are fixed by
`cmake/patches/mojoshader-6333f74-effect-parser-robustness.patch`. The campaign is now clean on
FNA3D's OpenGL driver across three independent seeds and on the SDL_GPU driver for 3,000
iterations; the SPIR-V emitter still validates with `assert()` in about fifty places.
...
(`tools/graphics/compiled_effect_fuzzer.cpp --campaign`) found twenty-two distinct ways it crashed the
