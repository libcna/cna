...
//
// Task REMED-GFX-001: regression coverage for the EasyGLGraphicsBackend constructor's
...
// entry in IGraphicsBackend's static window registry, because the destructor of a
...
using CNA::Internal::Backends::EasyGL::EasyGLGraphicsBackend;
using Microsoft::Xna::Framework::Input::Mouse;

TEST(EasyGLGraphicsBackendConstructor, FailedContextCreationLeavesNoDanglingRegistryEntry)
{
    ...
    // deterministically, reproducing the exact throwing path (EasyGLGraphicsBackend.cpp's
    // "SDL_GL_CreateContext failed" branch) the fix must survive without leaking a registry entry.
    SDL_Window* window = SDL_CreateWindow("EasyGLGraphicsBackendTests", 64, 64, SDL_WINDOW_HIDDEN);
    if (!window)
    {
    ...
    EXPECT_THROW(
        (EasyGLGraphicsBackend(window, 64, 64, CnaPresentationMode::FixedHeightDynamicWidth, false, 1, 1)),
        std::runtime_error);
    ...
    EXPECT_EQ(IGraphicsBackend::GetForWindow(window), nullptr);

    // Dispatch a real public-API mouse event against the same window. Mouse::SetPosition()
    // resolves the window's registered backend via IGraphicsBackend::GetForWindow() -- pre-fix,
