...
// and runs on any platform CnaTests already builds for, no #if defined(CNA_BACKEND_METAL) gate.
//
// plan_metal.md METAL-13: PointListEXT previously fell through to the *3 (triangle) default -- a
// real, previously-shipping bug, already fixed in the Metal backend's own source. This test locks
// that fix in against a future regression, and cross-checks every formula against
// EasyGLGraphicsBackend's own already-tested equivalent switch (src/CNA/Internal/Backends/EasyGL/
// EasyGLGraphicsBackend.cpp), not just re-derived from memory.
