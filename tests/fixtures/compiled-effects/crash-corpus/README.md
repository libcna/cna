...
| `multiple-preshaders.fxb` | How many preshaders a shader declares is content, and `parse_preshader` guarded it with an assert -- upstream's own `// !!! FIXME: can you have more than one?` is still unanswered. A second declaration also overwrote the first and leaked it |
