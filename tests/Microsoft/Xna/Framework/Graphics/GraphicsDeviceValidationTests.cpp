...
#if defined(CNA_BACKEND_SDL_RENDERER) || defined(CNA_BACKEND_ASCII) || defined(CNA_BACKEND_DX3) || defined(CNA_BACKEND_DX1) || defined(CNA_BACKEND_DX2)
    // Task 709 (SDL_Renderer) / DX3-27 (DirectDraw, plan_dx3.md) / DX1-27 (real DirectDraw v1,
    // plan_dx1.md) / DX2-84 (same DirectDraw v1 2D layer, plan_dx2.md): all four support exactly
    // one active render target at a time -- unlike the other, real-MRT-capable backends, binding
    // more than one target here must throw clearly rather than silently rendering to only the
    // first. 4 is still within the MAX_RENDERTARGET_BINDINGS cap this test's name/history (Task
    // 881) refers to, so the throw here comes entirely from the backend's own single-target
