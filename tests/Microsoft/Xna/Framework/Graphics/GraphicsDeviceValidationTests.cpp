...
TEST(GraphicsDeviceValidationTest, SetRenderTargets_FourTargets_DoesNotThrow)
{
...
#if defined(CNA_BACKEND_SDL_RENDERER) || defined(CNA_BACKEND_ASCII) || defined(CNA_BACKEND_DX3) || defined(CNA_BACKEND_DX1) || defined(CNA_BACKEND_DX2) || defined(CNA_BACKEND_DX30)
    ...
    // layer, now DirectDraw v2): all five support exactly one active render target at a time --
    // unlike the other, real-MRT-capable backends, binding more than one target here must throw
    // clearly rather than silently rendering to only the first. 4 is still within the
    // MAX_RENDERTARGET_BINDINGS cap this test's name/history (Task 881) refers to, so the throw
    // here comes entirely from the backend's own single-target limitation, not the cap check.
    // ASCII (plan_ascii.md) forwards SetRenderTargets straight to the same real SdlGraphicsBackend
    // instance it wraps, so it inherits this exact throw too.
