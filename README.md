...
- **`D3D12` renderer:** Windows-only native Direct3D 12 renderer, cross-compiled via MinGW-w64 and verified through Wine+vkd3d-proton on a real GPU, **off-screen only** (`D3D12_Smoke` CTest, 80/80 checks) — device/queue/heaps/command-lists/fences/barriers/PSOs/root-signatures are real, all 10 stock HLSL shader variants (same DXBC as `D3D11`) and a real `SpriteBatch` are pixel-verified off-screen, and device-removed recovery is real and functionally proven. Swap-chain presentation is a known, real, unresolved gap on this dev loop (genuine Wine/vkd3d-proton `dxgi.dll` architecture mismatch, not a CNA bug); runtime-settable blend/depth-stencil/rasterizer state, per-slot `SamplerState`, render targets, `Texture3D`, occlusion queries, and real-Windows hardware verification are all still open. See [`docs/d3d12-renderer.md`](docs/d3d12-renderer.md) and `plan_dx.md`.
...
- `DX1` (Windows-only)
- `DX2` (Windows-only)
- `DX3` (Windows-only; CNA's real DirectX 3 renderer -- see `plan_dx3.md`)
- `DX5` (Windows-only; CNA's real DirectX 5 renderer -- DirectDraw v4 + Direct3D v3 FVF `DrawPrimitive`)
- `DX6` (Windows-only; CNA's real DirectX 6 renderer -- same interfaces as DX5, plus real stencil buffer operations)
- `DX7` (Windows-only; CNA's real DirectX 7 renderer -- new IDirectDraw7/IDirect3D7/IDirect3DDevice7, viewport object removed, direct texture binding)
- `DX8` (Windows-only; CNA's real DirectX 8 renderer -- no DirectDraw at all, real IDirect3D8/IDirect3DDevice8 via DXVK's D8VK, fixed-function 3D only)
- `D3D10` (Windows-only; CNA's real Direct3D 10 renderer -- no fixed-function pipeline at all, real HLSL vs_4_0/ps_4_0 shaders via Wine's own d3d10.dll + DXVK's d3d10core.dll, real MRT)
...
- **Direct2D renderer (Windows-only 2D path)**
    ...
    - Explicitly excludes 3D/depth/MRT/custom shaders; use `D3D11` for those features.
...
see [`docs/d3d9-renderer.md`](docs/d3d9-renderer.md) for what that means and why. Developed and
...
[`docs/d3d9-renderer.md`](docs/d3d9-renderer.md) and [`plan_dx9.md`](plan_dx9.md) for full detail.
...
(`docs/d3d9-renderer.md` has full setup steps); CTest wires this in automatically:
...
[`docs/d3d11-renderer.md`](docs/d3d11-renderer.md) and [`plan_dx.md`](plan_dx.md) for full detail.
...
Running the resulting `.exe`s needs a Wine + DXVK dev-loop (`docs/d3d11-renderer.md` has full setup
...
a known, real, unresolved gap on this dev loop (see `docs/d3d12-renderer.md`). See
[`docs/d3d12-renderer.md`](docs/d3d12-renderer.md) and [`plan_dx.md`](plan_dx.md) for full detail.
...
D3D11's own (`docs/d3d12-renderer.md` has full setup steps); CTest wires this in automatically:
...
| Linux → Windows (cross) | MinGW-w64 | D3D9 | ✅ verified building + `D3D9` CTest suite (14 tests) under Wine+DXVK on a real GPU — 0/31 oracle scenes diverge from real XNA 4.0 at `--tolerance 0`; real Windows hardware verification still open, see `docs/d3d9-renderer.md` |
| Linux → Windows (cross) | MinGW-w64 | D3D11 | ✅ verified building + `D3D11` CTest suite (6 tests, 96+ checks) under Wine+DXVK on a real GPU — real Windows hardware verification still open, see `docs/d3d11-renderer.md` |
| Linux → Windows (cross) | MinGW-w64 | D3D12 | ✅ verified building + `D3D12` CTest suite (1 test, 80/80 checks, off-screen only) under Wine+vkd3d-proton on a real GPU — swap-chain presentation and real Windows hardware verification both still open, see `docs/d3d12-renderer.md` |
