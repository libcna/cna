...
- **`DIRECT2D` backend:** Windows-only hardware-accelerated **2D-only** backend using `ID2D1DeviceContext`; D3D11/DXGI only host the Direct2D device and swap chain. Texture/RT mips, SpriteBatch transforms/tint/flip/addressing, native anisotropic filtering, exact Direct2D Porter–Duff modes, logical presentation/readback, resize and 2D resource recovery are implemented. Wine/Proton cover the portable matrix; built-in effects and non-source-over composite pixels remain gated on native Windows. See [`docs/direct2d-backend.md`](docs/direct2d-backend.md) and `plan_direct2d.md`.
