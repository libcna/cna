...
cmake -S . -B build -DCNA_GRAPHICS_BACKEND=OPENGLES
...
- **`LLGL` backend:** Experimental backend on [LLGL](https://github.com/LukasBanana/LLGL), the only CNA backend that picks its native API at **runtime** rather than at build time (`CNA_LLGL_RENDERER=auto|vulkan|opengl|null`, default Vulkan then OpenGL). The current baseline is 2D only: swap chain and clears, virtual-resolution presentation, `Texture2D` upload/readback, and `SpriteBatch` with real blend and sampler state — pixel-verified end to end through the Vulkan module (orientation, tint, flip, alpha blending). The 3D pipeline, render targets, cube/volume textures and custom effects are not implemented and fail loudly rather than silently. **The OpenGL module has a known open bug: it clears but draws nothing** (`plan_llgl.md` `LLGL-17`). See [`docs/llgl-backend.md`](docs/llgl-backend.md) and `plan_llgl.md`.
...
- **`GLIDE` backend:** 32-bit Windows native Glide 3.x call path for a separately supplied `glide3x.dll` runtime, with no rendering fallback. `SpriteBatch` textures are native ARGB4444 TMU uploads and compatible quads are submitted through `grDrawVertexArrayContiguous`. The deliberately constrained real 3D path includes clipped color/textured triangle lists/strips, TMU0 mipmapping, native Z/cull/alpha-test state and the fixed-function per-vertex BasicEffect subset; it is not shader emulation or full EasyGL parity. See [`docs/glide-backend.md`](docs/glide-backend.md) and [`plan_glide.md`](plan_glide.md).
...
- **Automatic CI is Linux-only and partial** (see `.github/workflows/`): it currently runs the `Input` and `Devices`/`Sensors` gtest suites, not the full ~4,370-test unit suite or the complete GPU pixel matrix. A separate manual-dispatch Windows MSVC workflow covers D3D11/D3D12/Direct2D backend CTests; its Direct2D leg records native debug-layer, WARP and runtime/adapter artifacts. There is no automatic Windows, macOS, or Android gate.
...
| src/CNA/Internal/Backends/{SdlRenderer,EasyGL,Vulkan,Skia} |
+-----------------------------------------------------------+
```
...
This keeps game code stable while allowing backend-specific optimizations in SDL renderer,
...
- `OPENGL2` (native desktop OpenGL 2.1 compatibility profile, GLSL 1.10 -- independent of the EasyGL-implemented GL family and of the other GL backends)
...
- **Direct2D backend (Windows-only 2D path)**
...
- **Skia backend**
    - Select with `CNA_GRAPHICS_BACKEND=SKIA`; requires the explicitly pinned external raster
...
### Build (Linux — OPENGLES backend, default)
...
### Build (Windows cross-compilation — Glide 3.x backend)
...
[`docs/glide-backend.md`](docs/glide-backend.md) for runtime setup and limitations.
