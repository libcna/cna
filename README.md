...
- **`SOKOL` backend:** Experimental backend on `sokol_gfx`, a single-header GPU abstraction (OpenGL 4.1 core today; GLES3/D3D11/Metal/WebGPU are wired but unimplemented). The current baseline covers context/pass lifecycle, the full `Clear` family, back-buffer read-back, `Texture2D`, vertex/index buffers, a pixel-verified `SpriteBatch` with real `BlendState`/`SamplerState` support, vertex-coloured/textured/lit 3D geometry (`BasicEffect` with real depth testing, face culling, ambient + up to 3 per-pixel directional lights, specular, emissive, alpha test and fog), and `RenderTarget2D` (real colour + depth-stencil attachments, correct sampling orientation, immediate first-use, no direct `GetData` readback). Dual-texture, environment-mapped, skinned and PBR shading, custom effects, `RenderTargetCube`/MRT/render-target MSAA/mip chains, and cube/volume textures are not implemented and fail loudly. See [`docs/sokol-backend.md`](docs/sokol-backend.md) and `plan_sokol.md`.
