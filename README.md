...
- **`GLIDE` backend:** 32-bit Windows native Glide 3.x call path for a separately supplied `glide3x.dll` emulator (normally dgVoodoo2), with no rendering fallback. `SpriteBatch` textures are native ARGB4444 TMU uploads and each quad is two actual `grDrawTriangle` calls. A deliberately constrained real 3D path provides untextured `VertexPositionColor` triangle lists/strips and the native Z buffer; it is not shader emulation or full EasyGL parity. See [`docs/glide-backend.md`](docs/glide-backend.md) and [`plan_glide.md`](plan_glide.md).
...
2D `SpriteBatch` draws and a constrained fixed-function `VertexPositionColor` 3D path, both
submitted as real Glide triangles. See
