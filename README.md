...
- **`METAL` backend (macOS only, experimental):** Direct native `MTLDevice`/`CAMetalLayer` rendering with runtime-compiled MSL shaders. Its supported and evidence-backed boundary is documented in [`docs/metal-backend.md`](docs/metal-backend.md) and `plan_metal.md`; iOS and tvOS remain unvalidated and are not claimed.
