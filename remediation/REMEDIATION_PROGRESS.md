...
| REMED-GFX-068 | P3 | NOT STARTED (recorded during GFX-064) | | feature/audit | SdlGpu applies `ScissorRectangle` once PER PASS from the live state at `Present()` time (`ApplyScissorForPass`), so when a game sets a scissor while an RT is bound and then unbinds (which resets `ScissorRectangle` to the full **backbuffer** at the GraphicsDevice layer), the deferred RT pass replays with the full-backbuffer scissor and no longer clips. Same deferred-model shape GFX-013 fixed for Vulkan scissor and GFX-064 fixed for SdlGpu viewport (per-draw capture). Surfaced by GFX-064's viewport∩scissor frame; separable (scissor, not viewport). Not fixed. See detail below. |
...
### REMED-GFX-068 — SdlGpu: per-pass `ScissorRectangle` reset to the full backbuffer on RT unbind (NEW, discovered during GFX-064, NOT fixed)

| Field | Value |
|---|---|
| Sev / Pri / Owner / Status | LOW / P3 / GRAPHICS / **NOT STARTED (recorded, deferred)** |
| Root cause | SdlGpu applies `ScissorRectangle` **once per pass** from the live member state at `Present()` time (`ApplyScissorForPass(pass, targetW, targetH)`, gated on `scissorEnabled_`), unlike the **per-draw** viewport GFX-064 just wired. In this deferred backend, a game that sets a scissor while a RenderTarget2D is bound and then unbinds it triggers `GraphicsDevice::ResetViewportAndScissorForRenderTarget` → `setScissorRectangleProperty(0,0,backbufferW,backbufferH)` — so by the time the RT pass replays at Present, the scissor rect is the full **backbuffer** (larger than the RT), and the scissor no longer clips. Same deferred-model shape GFX-013 fixed for Vulkan scissor (per-draw capture). |
| Evidence | GFX-064's SdlGpu bring-up: a viewport∩scissor frame with the standard `SetRenderTarget(rt); scissor; draw; SetRenderTarget(nullptr); GetData(rt)` sequence left the scissored-out probe RED (scissor did not clip). SdlGpu also aborts (`SIGABRT`) on `GetData` while a target is still bound, so the scissor cannot be isolated via RT readback without the unbind — which is exactly what triggers the reset. `RasterizerState.ScissorTestEnable` itself works (the same pattern passes on Vulkan GFX-062 Frame 3). |
| Fix shape (not done) | Capture the scissor per draw into `QueuedDrawRef` and apply `SDL_SetGPUScissor` per draw in `RenderQueuedDraws` — exactly the pattern GFX-064 established for the viewport (and GFX-013 for Vulkan). Straightforward but separable (scissor, not viewport), so left for its own task. |
| Affected files | `src/CNA/Internal/Backends/SdlGpu/SdlGpuGraphicsBackend.cpp` |
| Backends / Platforms | SdlGpu / ALL |
| Why separable / not fixed here | Distinct dynamic state (scissor) from GFX-064's viewport; the existing per-pass scissor is a pre-existing SdlGpu design (documented in `ApplyScissorForPass`), not introduced or worsened by GFX-064. GFX-064's viewport change does not touch scissor state (regression-safe: SdlGpu shard 28/28). |
| Suggested test | Extend `sdlgpu_rendertarget_viewport_test` (or a new scissor test) with a per-draw RT scissor case once the per-draw scissor capture lands — mirror `vulkan_rendertarget_scissor_test` (GFX-013). |
