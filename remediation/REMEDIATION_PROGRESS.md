...
| REMED-GFX-070 | P3 | NOT STARTED (recorded during GFX-069 Phase 20) | | feature/audit | Vulkan declares `VK_DYNAMIC_STATE_BLEND_CONSTANTS` on its pipelines but calls `vkCmdSetBlendConstants` exactly **once per frame** — at the backbuffer-pass begin in `RecordCommandBuffer` (line ~6926), reading the live `blendFactorR_/G_/B_/A_` members. Consequence (source-identified, NOT runtime-verified): RT-pass (Phase 1) constant-color blends run with an undefined/stale blend constant (the dynamic state is never set before them that frame), and multiple `GraphicsDevice.BlendFactor` values within one backbuffer frame collapse to the last one (per-frame single set, not per-draw). This is the Vulkan analog of GFX-069 — viewport/scissor were made per-draw by GFX-062/013 but blend constants were not. Separable — recorded, NOT fixed. Needs its own task with a runtime reproduction (mirror `SdlGpu_RenderTargetBlendFactor` Frame 4 for Vulkan). See detail below. |
...
### REMED-GFX-070 — Vulkan: blend constants set once per frame despite `VK_DYNAMIC_STATE_BLEND_CONSTANTS` (NEW, source-identified during GFX-069 Phase 20, NOT fixed)

| Field | Value |
|---|---|
| Sev / Pri / Owner / Status | LOW / P3 / GRAPHICS / **NEW — source-identified, NOT runtime-verified, NOT fixed** |
| Evidence | Vulkan pipelines declare `VK_DYNAMIC_STATE_BLEND_CONSTANTS` (e.g. `VulkanGraphicsBackend.cpp:2438,2624,2958,...`), so the constant MUST be set per command buffer via `vkCmdSetBlendConstants`. But there is **exactly one** `vkCmdSetBlendConstants` call in the whole backend — `RecordCommandBuffer` line ~6926, at the **backbuffer** pass begin (Phase 2), reading the live `blendFactorR_/G_/B_/A_` members. |
| Consequence (unverified) | (a) **RT passes (Phase 1) run before that single set** → RenderTarget2D/Cube/MRT constant-color blends execute with an undefined/stale blend constant (the dynamic state is never set that frame before them). (b) The backbuffer pass sets the constant **once from the live member**, so multiple `GraphicsDevice.BlendFactor` values within one frame collapse to the last one set (last-wins), not per-draw. This is the Vulkan analog of GFX-069: viewport/scissor were converted to per-draw dynamic capture by GFX-062/013, but blend constants were left as a single per-frame set. |
| Why separable / not fixed here | Different backend, different codebase (`RecordCommandBuffer`'s per-draw `computeViewport`/`computeScissor` are the natural place to add a per-draw `vkCmdSetBlendConstants`, using a per-draw/per-batch captured factor like GFX-062/013 do for viewport/scissor). Out of GFX-069's SdlGpu scope. Needs its own task with a runtime reproduction — mirror `SdlGpu_RenderTargetBlendFactor` Frame 4 (three BlendFactors in one pass) + an RT-pass constant-color case for Vulkan, then convert the single per-frame set to per-draw capture. |
| Backends / Platforms | Vulkan / ALL |
