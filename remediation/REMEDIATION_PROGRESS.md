...
### REMED-GFX-064 — SdlGpu + D3D12: `GraphicsDevice.Viewport` never wired to the GPU (NEW, discovered during GFX-062 parity, NOT fixed)

| Field | Value |
|---|---|
| Sev / Pri / Owner / Status | MEDIUM / P2 / GRAPHICS / **NOT STARTED (recorded, deferred)** |
| Root cause | Neither backend overrides the **no-op** base `IGraphicsBackend::SetViewport(...)` (`IGraphicsBackend.hpp:687`, an empty `{}`). **SdlGpu** never calls `SDL_SetGPUViewport` anywhere (only `ComputeLogicalViewport()`, which is derived from `physicalWidth_/Height_`, the full target); **D3D12** hardcodes `D3D12_VIEWPORT{0,0,boundColorWidth_,boundColorHeight_,0,1}` at all four `RSSetViewports` sites. A custom `GraphicsDevice.Viewport` is a total no-op on both — for the **backbuffer as well as** render targets (broader than GFX-062, which only affected Vulkan's RT path). |
| Evidence | `grep` across each backend dir: zero `SetViewport(int` override; SdlGpu has no `SDL_SetGPUViewport`; D3D12's four viewport sites all hardcode the full bound-target size. Source-established, not pixel-verified here (D3D12/SdlGpu pixel readback need Wine/special env). |
| Relationship to GFX-062 / GFX-014 | Same "viewport not honored" family, but a **broader, different** defect (backbuffer + RT, and no state storage at all). D3D12 is the same MVP-level backend whose scissor is inert (**GFX-014**). |
| Affected files | `src/CNA/Internal/Backends/SdlGpu/SdlGpuGraphicsBackend.cpp`, `src/CNA/Internal/Backends/D3D12/D3D12GraphicsBackend.cpp` |
| Backends / Platforms | SdlGpu, D3D12 / ALL |
| Why separable / not fixed here | Two distinct backends, backbuffer-level wiring absent (not the RT-specific deferred-capture pattern GFX-062 addresses); GFX-062 is Vulkan-scoped. Not inseparable. |
| Suggested test | Backbuffer + RT custom-`Viewport` pixel tests per backend once each backend's `SetViewport` is wired (SdlGpu: `SDL_SetGPUViewport` per pass; D3D12: store + `RSSetViewports` with the captured rect). |
