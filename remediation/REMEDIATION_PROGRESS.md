...
| REMED-GFX-071 | P2 | **NEW — source-identified during GFX-070 Phase 16, NOT fixed** | | feature/audit | Vulkan's 2D **sprite** pipeline (`GetOrCreatePipeline2D`/`GetOrCreatePipeline2DMsaa`, blend attachment hardcoded at `VulkanGraphicsBackend.cpp:2427-2434`) fixes `srcColorBlendFactor=SRC_ALPHA`/`dstColorBlendFactor=ONE_MINUS_SRC_ALPHA` and takes no blend params, so `SpriteBatch.Begin(sortMode, blendState)` has **no effect on the actual blend equation** — `Additive`/`NonPremultiplied`/`BlendFactor`/separate factors/functions are all silent no-ops for sprites. The 3D path was fixed by Task 868 (`FillBlendAttachmentState`+`draw.blendParams`, honors `CONSTANT_COLOR` — proven by `Vulkan_BlendState_BlendFactor` PASS and GFX-070 Frames 0-4), but the 2D path was never plumbed. Discovered because GFX-070's per-batch `vkCmdSetBlendConstants` is correctly wired yet inert (the sprite pipeline never selects `CONSTANT_COLOR`). Broader than a blend-constant bug — the whole BlendState is dropped for sprites. Fix = plumb `BlendKeyParams` into the 2D pipeline cache key + `FillBlendAttachmentState` (mirror the 3D path), out of GFX-070 scope. Note: the stale `cmake/Tests/VulkanTests.cmake:174` comment ("`Vulkan_BlendState_BlendFactor` expected to FAIL per Task 868") is factually wrong today (that 3D-path test PASSES) and was corrected to point here. |
