...
| REMED-GFX-079 | P3 | NOT STARTED (recorded during GFX-073) | | feature/audit | The Software backend's **3D** raster path (`DrawColoredPrimitives`/`DrawIndexedColoredPrimitives`/`DrawPrimitivesEx`/`DrawIndexedPrimitivesEx`) ignores a custom `GraphicsDevice.Viewport`: each calls `GetViewportSize(vw, vh)` — which returns the **full current framebuffer** size (`CurrentFramebuffer().width/height`), NOT the stored viewport — and feeds it to `ClipVertexToRasterVertex`, which maps NDC→screen with **no `Viewport.X/Y` offset, no `Width/Height` sub-scaling, no viewport clip, and no `MinDepth/MaxDepth` depth-range remap** (`out.depth = ndcZ` directly). Correct for the default full-target viewport (all existing Software 3D tests use it, so they stay green); a custom sub-viewport is silently ignored. This corrects the original GFX-073 finding's assumption that "the 3D path does consume the viewport". Sibling of GFX-073 — the `viewportSet_`/`GetActiveViewport` state added there can be reused; the work is a real per-vertex transform change (offset + W/H scale + depth-range) plus a viewport clip on `RasterizeTriangle`/`RasterizeTriangleShaded` (the shaded one already takes a `RasterClipRect`). Source-verified, NOT fixed. |
