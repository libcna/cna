...
### REMED-GFX-078 — Bgfx: generic 3D-effect texture binding static_cast is UB for RenderTarget2D sources (NEW, discovered during GFX-067 Phase 27, NOT fixed)

| Field | Value |
|---|---|
| Sev / Pri / Owner / Status | MEDIUM / P2 / GRAPHICS / **NOT STARTED (recorded, needs own fix)** |
| Root cause | The Bgfx 3D draw path (`DrawPrimitivesEx`/`DrawInstancedPrimitivesEx`/`FillGpuDrawParams` dispatch) binds every effect sampler slot with `static_cast<const BgfxTextureBackend&>(*params.textureN)` and reads `.textureHandle` (texture0/1, PBR normal/metallic-roughness/emissive/occlusion maps, dual-texture, env-map's 2D texture). A `RenderTarget2D`'s backend is a `BgfxRenderTargetBackend` — an unrelated sibling of `BgfxTextureBackend` (no `textureHandle` member; its handle is `colorTex`). Setting a RenderTarget2D as a generic effect texture (`effect.Texture = renderTarget`, legal XNA — RenderTarget2D is-a Texture2D) makes this static_cast **undefined behaviour**, reading the wrong pooled handle. |
| Evidence | Verified by source read of the cast sites (`BgfxGraphicsBackend.cpp` texture0/1/PBR/dual/env dispatch). Identical bug shape to Task 873 (fixed for `SpriteBatch` via `IBgfxSamplable`) and Task 907 (fixed for cube env-map via `IBgfxCubeSamplable`) — never fixed for the 3D **2D-texture** path. |
| Relationship to GFX-067 | GFX-067's orientation fix is scoped to `SpriteBatch` precisely because the generic path can't safely sample an RT at all; the two are independent (cast safety vs V-orientation). Once this is fixed, RT samples on the generic path would ALSO need the GFX-067 `originBottomLeft` V-flip handled at their UV source. |
| Affected files | `src/CNA/Internal/Backends/Bgfx/BgfxGraphicsBackend.cpp` (all `static_cast<const BgfxTextureBackend&>(*params.textureN)` sites) |
| Backends / Platforms | Bgfx / ALL |
| Suggested fix | Resolve the sampleable handle through `dynamic_cast<const IBgfxSamplable*>(params.textureN)->GetBgfxTextureHandle()` (as `SpriteBatch::Draw` already does), and apply the GFX-067 origin V-flip for RT sources where the shader samples them. |
