...
| REMED-GFX-005 | P1 | IN PROGRESS | 58a7d0bb | feature/audit | **Vulkan slice DONE** (16 shaders, 7/8 fog tests pass — see "Wave 3 — GRAPHICS shader campaign" below). Bgfx + D3DCommon (D3D11/D3D12) slices PENDING (bytecode regen blocked in this sandbox: no shaderc/fxc/dxc binary). Cross-backend fog conformance test still to build. |
...
- **Numerical/pixel verification (Vulkan/llvmpipe, Xvfb :99):** 7/8 pass 3/3 — `Vulkan_BasicEffect_Fog`,
  ...
  `Vulkan_EnvironmentMapEffect_Fog`.
...
- **Interaction with REMED-GFX-052:** the 8th test `Vulkan_SkinnedEffect_Fog` still fails, but with
  **black skinned geometry in the fog-OFF case too** → a skinned-lighting defect, not fog. Its fog
  scene was corrected here so it will pass once the skinned-lighting issue (GFX-006/-008) lands. See
  the updated GFX-052 row.
