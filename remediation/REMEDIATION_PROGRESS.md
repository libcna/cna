...
| REMED-GFX-080 | P3 | NOT STARTED (recorded during GFX-073) | | feature/audit | `SoftwareGraphicsBackend::SetScissorRect(int,int,int,int)` is a **no-op** (`{}`) and no scissor state is stored, so `GraphicsDevice.ScissorRectangle` (with `RasterizerState.ScissorTestEnable`) never clips the Software 2D sprite path or the 3D raster path — the whole target is always writable. Independent of GFX-073's viewport clip (a distinct raster-state gap); the natural implementation is a centralized `effective clip = current target ∩ Viewport ∩ (ScissorRectangle when enabled)` reusing GFX-073's `RasterClipRect`. Source-verified, NOT fixed. |
...
| Software | CPU raster at raw framebuffer pixels; `SetViewport` no-op | ignored | **SEPARATE_FINDING (GFX-073 — now DONE; spawned GFX-079 3D-path, GFX-080 scissor)** |
