...
| REMED-GFX-072 | P3 | **NEW — source-identified during GFX-019 Phase 11/22, NOT fixed** | | feature/audit | CNA's `SpriteBatch` does not thread `GraphicsDevice.Viewport` into the sprite clip-space bake: the backend divides the destination rectangle by the full render-target (or backbuffer) size, then the rasterizer viewport squishes the result into any custom sub-`Viewport`. So a **custom sub-`Viewport` set on a bound render target mis-scales sprites** — XNA/FNA build the SpriteBatch ortho from `Viewport.Width/Height` (viewport-relative; sprite (0,0)=viewport top-left), but EasyGL/Vulkan/Bgfx/SdlGpu **and** the now-fixed WebGPU are all full-target-relative. Only **D3D11** is XNA-correct here (its `ViewportSize` constant is filled from the live `RSGetViewports()`, so its NDC divide already uses the custom viewport). Default viewport (auto-reset to the full target on `SetRenderTarget`) is unaffected — the common case is correct everywhere, which is why this has gone unnoticed. Pre-existing on the backbuffer path too, cross-backend (5/6), not the GFX-019 backbuffer-vs-RT-size defect; surfaced by the cross-backend parity survey, source-identified only (no runtime repro yet). Fix would thread the active `Viewport` W/H into each backend's sprite NDC (or CPU bake) — a shared-layer/multi-backend change, out of GFX-019 scope. |
