...
| `REMED-GFX-072` | SpriteBatch clip space ignores a custom `GraphicsDevice.Viewport` sub-region: CNA's SpriteBatch does not thread `Viewport.Width/Height` into the sprite NDC bake, so a custom sub-viewport on a bound target squishes sprites instead of making them viewport-relative. XNA/FNA build the SpriteBatch ortho from `Viewport` (D3D11 is correct via live `RSGetViewports`); EasyGL/Vulkan/Bgfx/SdlGpu **and** the now-fixed WebGPU are all full-target-relative. Default viewport (auto-reset to full target on `SetRenderTarget`) is correct everywhere. Pre-existing, cross-backend (5/6), source-identified during GFX-019 Phase 11/22; NOT fixed | MEDIUM | GRAPHICS | NEW |
