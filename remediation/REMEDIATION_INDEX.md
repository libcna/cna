...
| `REMED-GFX-073` | Software (CPU-raster) backend SpriteBatch ignores `GraphicsDevice.Viewport`: sprites rasterize at raw framebuffer pixels and `SetViewport` is a no-op, so a custom viewport neither positions nor clips 2D (the 3D path does use it). Not the GFX-072 mis-scale — a broader Software-2D viewport gap. Surfaced by the GFX-072 backend survey; source-identified, NOT fixed | LOW | GRAPHICS | NEW |
| `REMED-GFX-074` | Vulkan deferred SpriteBatch-into-RenderTarget2D lifetime/flush bug: a sprite drawn into an RT is only replayed at Present, so `GetData` before Present returns the cleared RT, and destroying the RT before Present leaves a dangling `batchRT` pointer in `activeBatches_` → use-after-free at the next Present. Surfaced while building the GFX-072 harness; NOT fixed | MEDIUM | GRAPHICS | NEW |
