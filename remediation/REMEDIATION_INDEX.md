...
| `REMED-GFX-081` | `SpriteBatch::Begin(...)` silently **drops its `rasterizerState` argument** on every backend: all three `Begin` overloads forward to the 7-arg overload which names the parameter `/*rasterizerState*/` and never applies it (unlike `blendState`/`depthStencilState`/`samplerState`, which it does re-apply — the latter fixed by Task 803). So the idiomatic XNA way to scissor-clip sprites — `spriteBatch.Begin(..., rasterizerState: rsWithScissorTestEnable)` — has no effect; a game must instead set `GraphicsDevice.RasterizerState` directly before `Begin`. FNA's `SpriteBatch.Begin` stores the state and applies it in `PrepRenderState`. Cross-backend (shared `SpriteBatch.cpp`), independent of GFX-080's backend fix. Surfaced + source-verified during GFX-080; NOT fixed. | MEDIUM | GRAPHICS | NEW |
