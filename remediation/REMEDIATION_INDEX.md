...
| `REMED-GFX-084` | Bgfx `SpriteBatch` `setViewTransform` last-wins for two batches that share the SAME `GraphicsDevice.Viewport` but pass DIFFERENT `SpriteBatch.Begin(transformMatrix)` in one frame: `EnsureViewState` bakes `transformMatrix·ortho` into the shared view's (per-view) `setViewTransform`, so the last batch's transform wins for both. GFX-065's ordered view segmentation keys segments on the VIEWPORT only (a viewport change starts a new segment), so same-viewport/different-transform batches share a segment and still collapse. Residual case of the same bgfx view-global-state last-wins class GFX-065 fixed for the viewport; would need the segment key to also include the sprite transform (or a per-batch sprite view). Very rare in practice (same device viewport + different Begin transform + same frame). Surfaced during the GFX-065 Phase-14 analysis; source-identified, NOT fixed. | LOW | GRAPHICS | NEW |
