...
| `REMED-NA-007` | `EffectMaterial::Clone()` preserves type identity, unlike FNA (which returns a plain `Effect`) | **SUPERSEDED by SOFTWARE-260.** Recovered Microsoft XNA confirms FNA's behavior is the platform contract rather than an upstream oversight. CNA's added override was removed and both noncompiled and authentic compiled-effect runtime types are regression-tested. |
