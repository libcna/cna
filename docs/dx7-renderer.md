# DX7 (real `IDirectDraw7` + `IDirect3D7`, flattened device model) Renderer — Completeness Status

`DX7` is architecturally `DX6` (`docs/dx6-renderer.md`) with a real structural change: genuinely new
...
result in `docs/dx6-renderer.md` (and, further back, `docs/dx5-renderer.md`/`docs/dx3-renderer.md`/
`docs/dx2-renderer.md`) applies identically here (same 2D compositor, same 3D pipeline, same
...
- **`IDirectDraw7`/`IDirectDrawSurface7`, created via `DirectDrawCreateEx`** — a genuinely new entry
  ...
  every `DX2`..`DX6` renderer used (spike-confirmed both paths empirically work, `DX7-0a1`/`DX7-0a2`
...
- **`IDirect3D7::CreateDevice` drops the trailing `IUnknown* outer` parameter** `DX5`/`DX6`'s
  ...
  `DX7` — a real signature oscillation across this renderer family's own history).
...
several of DX7's own forbidden symbols legitimately appear in this renderer's own explanatory
...
See `docs/dx6-renderer.md` (and, further back, `docs/dx5-renderer.md`/`docs/dx3-renderer.md`/
`docs/dx2-renderer.md`) for the full completeness table — every row applies to `DX7` identically,
since its own code is a port with only the deltas described above.

## See also

- `plan_dx7.md` — this renderer's own implementation plan.
- `plan_dx6.md`, `docs/dx6-renderer.md` — the renderer this one ports.
