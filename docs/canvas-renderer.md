...
| `Texture2D::FromStream` (PNG/JPEG) and `DDSFromStreamEXT` decode | ✅ | Confirmed fully renderer-agnostic (`DxtUtil`/vendored `stb_image`) before ever reaching this renderer's `putImageData` upload. |
