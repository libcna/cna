...
| MOD-2219 | Supplied | `FilmGrainPass` selects generated GLSL ES/desktop GLSL/SPIR-V variants and packs size, intensity and time into the existing custom-effect `vec4` slot. Determinism, temporal change and luminance weighting pass on EasyGL, RADV and llvmpipe. |
...
| MOD-2239 | Supplied | The shared internal post-process package starts with chromatic aberration across GLSL ES/desktop GLSL/SPIR-V. Vulkan uses the existing SpriteBatch source and scalar push slot; the three-case cross-renderer pixel oracle passes without a source-execution skip on EasyGL, RADV and llvmpipe. |
| MOD-2239a | Supplied | `TonemapPass` packages all five operators and output-space debanding for GLSL ES/desktop GLSL/SPIR-V. Mode, exposure, inverse gamma and dither amplitude share the existing `vec4` slot; 17 tests plus the seven-case pipeline application pass on EasyGL, RADV and llvmpipe. |
| MOD-2239b | Supplied | `LensFlarePass` packages its existing thresholded ghost path for GLSL ES/desktop GLSL/SPIR-V, with four parameters in the existing `vec4` slot. Its four-case optical-direction, threshold, disabled and settings oracle passes on EasyGL, RADV and llvmpipe; the separate missing-halo bound remains. |
| MOD-2239c | Supplied | `HdrDisplayOutput` packages sRGB pass-through, linear scRGB scaling and Rec. 2020/PQ HDR10 encoding for GLSL ES/desktop GLSL/SPIR-V. Its non-symmetric pixel oracle exposed and corrected the shared Vulkan fullscreen Y transform; 11 HDR tests and the complete 43-case portable post-process regression set pass on EasyGL, RADV and llvmpipe. This is texture/file encoding, not HDR swap-chain support. |
| MOD-2239d | Supplied | `SpatialUpscalePass` packages its edge-adaptive upsample and neighbourhood-clamped sharpen for GLSL ES/desktop GLSL/SPIR-V. Source size, sharpness and edge-adaptive state use the existing `vec4`, while exact 1:1 identity uses the scalar slot. Its eight-case oracle and the four-check GPU-driven application pass on EasyGL, RADV and llvmpipe. |
| MOD-2239e | Supplied | `LightShaftPass` packages its thresholded 24-step radial walk for GLSL ES/desktop GLSL/SPIR-V. Light position, threshold and intensity use the existing `vec4`, with decay in the scalar slot; a rendered-target oracle proves accumulation, occlusion shape and off-screen falloff on EasyGL, RADV and llvmpipe. |
| MOD-2239f | Supplied | `BloomPass` packages extraction, separable blur, progressive upsample/add and final composite for GLSL ES/desktop GLSL/SPIR-V. The two dual-input stages bind real render targets through the common Vulkan sampled-view contract and use clamp addressing; the expanded 20-case Bloom oracle passes on EasyGL, RADV and llvmpipe. |
| MOD-2239g | Supplied | `ColorGradePass` packages its filtered strip, explicit trilinear/tetrahedral strip and true 3D-LUT routes for GLSL ES/desktop GLSL/SPIR-V. Its existing 2D/3D descriptor ranges and one packed parameter vector are sufficient; all 31 grading/LUT tests and the 107-case portable post-process regression set pass on EasyGL, RADV and llvmpipe. |
| MOD-2239h | Supplied | `HeightFogPass` packages its analytic depth/camera-based integral for GLSL ES/desktop GLSL/SPIR-V. The existing depth sampler and typed float/vec3/mat4 array blocks carry its five scalars, colour and two matrices without a new Vulkan contract; the expanded 9-case oracle and 116-case portable post-process regression set pass on EasyGL, RADV and llvmpipe. |
| MOD-2239i | Supplied | `DepthOfFieldPass` packages its thin-lens sixteen-tap gather for GLSL ES/desktop GLSL/SPIR-V. The existing depth sampler and float-array block carry its six scalar inputs without a new Vulkan contract; its 14-case oracle and the 130-case portable post-process regression set pass on EasyGL, RADV and llvmpipe. |
| MOD-2239j | Supplied | `MotionBlurPass` packages camera reprojection and its optional per-object velocity override for GLSL ES/desktop GLSL/SPIR-V. Existing depth/velocity sampler slots plus float/mat4 array blocks carry every input; its expanded 7-case oracle and the 137-case portable post-process regression set pass on EasyGL, RADV and llvmpipe. |
| MOD-2239k | Supplied | `SsaoPass` packages its 64-sample estimate and 5×5 blur/composite for GLSL ES/desktop GLSL/SPIR-V. Existing depth/normal/noise sampler slots and float/vec2/vec3 array blocks carry every input; its 15 standalone cases and the 152-case portable post-process regression set pass on EasyGL, RADV and llvmpipe. Its three real-prepass cases are supplied by `MOD-2239l` below. |
| MOD-2239l | Supplied | `DepthNormalPrepass` packages rigid/skinned depth, normal and optional per-object velocity production for GLSL ES/desktop GLSL/SPIR-V. Vulkan reuses typed scalar/bone arrays and expands the engine-owned matrix block to six matrices; exact two- and three-output fragments match the attachment count. Deferred readback dependency closure now recognizes each constituent produced through an MRT proxy. The 27-case portable core passes on EasyGL, RADV and llvmpipe, so SSAO and motion blur can consume real prepass images on Vulkan. |
| MOD-2239m | Supplied | `SsrPass` packages its depth/normal-guided view-space ray march for GLSL ES/desktop GLSL/SPIR-V. Existing depth/normal sampler slots plus typed float/vec2/mat4 arrays carry every input; Vulkan explicitly bridges its top-left texture coordinates to XNA camera NDC. Its 22-case oracle and the 174-case portable post-process regression set pass on EasyGL, RADV and llvmpipe. |
| MOD-2239n | Supplied | `ContactShadowPass` packages its short, world-unit view-space ray march for GLSL ES/desktop GLSL/SPIR-V. Existing depth sampling plus typed float/vec2/vec3/mat4 arrays carry every input; a render-target-based vertical oracle pins Vulkan's texture-UV/XNA-camera-NDC bridge. EasyGL passes all 24 cases, Vulkan passes 23 with only the arbitrary-source diagnostic skipped, and the 197-case portable execution set passes on all three paths. |
| MOD-2239o | Supplied | `DecalPass` packages its depth-reconstructed box projection for GLSL ES/desktop GLSL/SPIR-V. Depth, decal and optional normal sampling plus typed float/vec3/mat4 arrays preserve volume rejection, slope rejection and straight-alpha composition; asymmetric-texture and tint/opacity oracles expand the shared suite to 10/10 on EasyGL, RADV and llvmpipe. |
| MOD-2239p | Supplied | `AerialPerspectivePass` packages the sky model's finite-path scattering and transmittance for GLSL ES/desktop GLSL/SPIR-V. Depth plus typed float/vec3/mat4 arrays carry the complete camera/atmosphere contract; a vertical camera-ray oracle caught and corrected the legacy EasyGL Y inversion. EasyGL passes 15/15, while RADV and llvmpipe pass 14 with only the deliberate arbitrary-source diagnostic skipped. |
| MOD-2239q | Supplied | `AtmosphericSky` selects a standalone generated GLSL ES/desktop GLSL/SPIR-V package using the existing typed matrix/vec3/float custom-effect blocks. A two-row oracle compares every channel with `AtmosphericSky::radiance` at CPU-reconstructed world rays; the expanded suite passes 10/10 on EasyGL, RADV and llvmpipe. |
| MOD-2239r | Supplied | `VolumetricFogPass` packages both its shadow-aware froxel-atlas build and depth-limited resolve for GLSL ES/desktop GLSL/SPIR-V. Existing sampler and typed float/vec3/mat4 blocks carry the complete contract; a vertical oracle additionally pins logical versus render-target storage UVs. The expanded suite passes 7/7 on EasyGL, RADV and llvmpipe. |
