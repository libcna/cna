...
| MOD-2244 | Partial | Vulkan has the dedicated format-qualified `rgba8` storage image and the zero-copy storage-capable `Color` `RenderTarget2D` bridge. Optional extended formats, an ordinary-`Texture2D` bridge where allocation permits it, and the complete lifetime matrix remain open. |
...
| MOD-2253 | Partial | Compute descriptors and per-timer query pools are bounded/reused, timing and sampled-image transitions add no routine global wait; broader modern allocators and the off-screen readback boundary remain open. |
