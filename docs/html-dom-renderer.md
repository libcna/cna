...
`<canvas>`. See `plans/plan_html_dom.md` for the design decisions and task breakdown this document
summarizes.
...
| `host-integration` (reuses the smoke page's own `.html`, driven by `scripts/htmldom-host-integration-test.mjs`) | A host page's own pre-existing canvas `opacity` value is captured and restored rather than assumed unset; a pre-existing, host-page-owned element with a colliding id is detected and refused rather than silently adopted (HTMLDOM-115) | 2/2 |
...
the layout and pointer hit testing at `opacity:0`, because SDL keeps sizing it and delivering input
through it). Every sprite is a pooled child `<div>`:
...
2. **Drawing a texture costs a PNG encode the first time, lazily** — by design; see design
   decision 6 in `plans/plan_html_dom.md` (HTMLDOM-117: the encode is paid on first DRAW, not upload).
