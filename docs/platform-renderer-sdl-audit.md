...
| `sdl-free` | 4 | No SDL references at all. |

## Per-family detail

| Family | Identities | Verdict | SDL refs (all / in code) | Platform services needed | Presentation calls |
|---|---|---|---:|---|---|
| `sdl-gpu` | SDL_GPU | `sdl-native` | 1702 / 1509 | `window` | — |
| `sdl-renderer` | SDL_RENDERER | `sdl-native` | 305 / 201 | `window` | `SDL_CreateRenderer`, `SDL_CreateTexture`, `SDL_DestroyRenderer`, `SDL_DestroyTexture`, `SDL_GetRenderLogicalPresentationRect`, `SDL_GetRenderOutputSize`, `SDL_RenderClear`, `SDL_RenderPresent`, `SDL_RenderReadPixels`, `SDL_RenderTexture`, `SDL_SetRenderClipRect`, `SDL_SetRenderDrawColor`, `SDL_SetRenderLogicalPresentation`, `SDL_SetRenderTarget`, `SDL_SetRenderVSync`, `SDL_SetTextureBlendMode`, `SDL_SetTextureScaleMode`, `SDL_UpdateTexture` |
| `freedirect` | FREEDIRECT | `sdl-upstream` | 19 / 3 | `window` | — |
| `fna3d` | FNA3D | `sdl-upstream` | 7 / 5 | `window` | — |
| `skia` | SKIA | `cpu-presentation` | 93 / 79 | `gl-vulkan-interop`, `window` | `SDL_CreateRenderer`, `SDL_CreateTexture`, `SDL_DestroyRenderer`, `SDL_DestroyTexture`, `SDL_GetRenderOutputSize`, `SDL_RenderClear`, `SDL_RenderPresent`, `SDL_RenderTexture`, `SDL_SetRenderDrawColor`, `SDL_SetRenderLogicalPresentation`, `SDL_SetRenderVSync`, `SDL_UpdateTexture` |
| `blend2d` | BLEND2D | `cpu-presentation` | 47 / 38 | — | `SDL_CreateRenderer`, `SDL_CreateTexture`, `SDL_DestroyRenderer`, `SDL_DestroyTexture`, `SDL_GetRenderOutputSize`, `SDL_RenderClear`, `SDL_RenderPresent`, `SDL_RenderTexture`, `SDL_SetRenderDrawColor`, `SDL_SetRenderLogicalPresentation`, `SDL_SetRenderVSync`, `SDL_UpdateTexture` |
| `easygl` | OPENGL33 OPENGLES2 OPENGLES3 WEBGL1 WEBGL2 | `migratable` | 56 / 49 | `gl-vulkan-interop`, `window` | — |
| `opengl1` | OPENGL1 | `migratable` | 56 / 44 | `gl-vulkan-interop`, `window` | — |
| `sokol` | SOKOL | `migratable` | 56 / 41 | `gl-vulkan-interop`, `window` | — |
| `opengles1` | OPENGLES1 | `migratable` | 55 / 50 | `gl-vulkan-interop`, `window` | — |
| `opengl2` | OPENGL2 | `migratable` | 46 / 35 | `gl-vulkan-interop`, `window` | — |
| `opengl4` | OPENGL4 | `migratable` | 44 / 33 | `gl-vulkan-interop`, `window` | — |
| `diligent` | DILIGENT | `migratable` | 41 / 31 | `native-handle`, `gl-vulkan-interop`, `window`, `display` | — |
| `openvg` | OPENVG | `migratable` | 41 / 33 | `gl-vulkan-interop`, `window` | — |
| `webgpu` | WEBGPU | `migratable` | 37 / 36 | `native-handle`, `gl-vulkan-interop`, `window`, `display` | — |
| `bgfx` | BGFX | `migratable` | 36 / 33 | `native-handle`, `window`, `display` | — |
| `gdi` | GDI | `migratable` | 36 / 33 | `native-handle`, `event`, `window` | — |
| `magnum` | MAGNUM | `migratable` | 31 / 30 | `gl-vulkan-interop`, `window` | — |
| `svg-dom` | SVG_DOM | `migratable` | 25 / 22 | `native-handle`, `window`, `filesystem` | — |
| `vulkan` | VULKAN | `migratable` | 25 / 19 | `gl-vulkan-interop`, `window` | — |
| `llgl` | LLGL | `migratable` | 22 / 19 | `native-handle`, `window`, `display` | — |
| `direct2d` | DIRECT2D | `migratable` | 14 / 11 | `native-handle`, `window` | — |
...
| `canvas` | CANVAS | `migratable` | 12 / 7 | `window` | — |
