...
> now `CNA_GRAPHICS_RENDERER=FREEDIRECT`. The `DIRECTX3` identity that the name was handed to is
> itself retired (see `docs/removed-renderers.md`); no renderer answers to it today. Historical
> `DX3-*` task IDs are unchanged.
...
|---|---|---|
| `CNA_GRAPHICS_RENDERER=FREEDIRECT` CMake selection + `../free-direct` sibling wiring | ✅ | Mirrors `../easy-gl`'s exact dependency pattern. Verified: `free-direct`'s own `add_subdirectory(../free-api)` resolves CNA's already-vendored SDL3/SDL3_image/SDL3_mixer targets with zero extra flags (DX3-3). |
...
| `TextureAddressMode::Wrap`/`Mirror` | ✅ | **A real capability win over `SDL_RENDERER`**, which has this ⛔ BLOCKED (see its own completeness doc §11) — FREEDIRECT's compositor already samples per-source-pixel for every non-identity draw, so `Wrap`/`Mirror` cost nothing extra to implement for real (design decision 7) (DX3-46). |
...
| `CreateOcclusionQuery()` | ⚪-internal factory returns `nullptr` | **Renderer seam:** the Phase X1/X2 skeleton incorrectly threw, so DX3-66 restored `IGraphicsRenderer`'s null factory. **Public seam (corrected by SOFTWARE-199):** recovered Microsoft XNA rejects construction when the profile or renderer does not support queries; it does not expose a permanently incomplete null-backed object. |
...
| Optional-capability factories (`Texture3D`/`TextureCube`/`RenderTargetCube`/custom `Effect`) | ⚪ internal factories degrade gracefully to `nullptr`; `OcclusionQuery` is the exception at the public seam and rejects unsupported construction per recovered XNA (SOFTWARE-199) |
