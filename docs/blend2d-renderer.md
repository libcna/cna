...
(`modules/renderers/blend2d/examples/blend2d_renderer_correctness_test.cpp`, 125 pixel-level
...
| `Opaque`/`AlphaBlend`/`NonPremultiplied`/`Additive` blend presets | `Opaque`/`AlphaBlend` direct exact `BLCompOp`; `NonPremultiplied`/`Additive` native `BLCompOp` colour + a bounded CPU-assisted independent-alpha correction | `Opaque` -> `BL_COMP_OP_SRC_COPY` (a literal copy, source alpha included -- it does not force full opacity, matching real XNA). `AlphaBlend` -> `BL_COMP_OP_SRC_OVER`, exact: XNA's `Dca'=Sca+Dca*(1-Sa)`, `Da'=Sa+Da*(1-Sa)` is byte-identical to native Porter-Duff over. `NonPremultiplied`/`Additive` set `AlphaSourceBlend=SourceAlpha` (not `One`), which XNA evaluates as an *independent* alpha equation -- `Da'=Sa*Sa+Da*(1-Sa)` for `NonPremultiplied`, `Da'=Sa*Sa+Da` for `Additive` -- that native `SRC_OVER`/`PLUS` (which only ever multiply by `Sa` once) cannot reproduce, even though their COLOUR channel *is* byte-identical to the native op (`Sc*Sa` is algebraically the source's own stored premultiplied byte either way). The colour bytes are therefore drawn through the ordinary native `BLCompOp` blit; a second bounded pass then renders the same draw onto a transparent scratch canvas via `SRC_OVER` (which collapses to exactly `Sa`, recovering Blend2D's true per-pixel effective source alpha including AA/clip/coverage) and combines it with a whole-surface pre-draw alpha snapshot to compute and write back the true independent alpha equation over just the alpha byte, leaving the already-correct colour bytes untouched (`Blend2DSpriteBatchRenderer.cpp`'s `ApplyIndependentAlphaCorrectionEXT`). A stored premultiplied colour byte can legitimately exceed the new, smaller `NonPremultiplied`/`Additive` alpha byte -- a genuine consequence of the independent alpha equation on premultiplied-native storage, not a bug -- so the premultiplied-to-straight unpack (`Blend2DPixelConvert.hpp`) clamps rather than wraps. All four verified against a semi-transparent source over a semi-transparent destination, with alpha checked in isolation from colour, not just opaque colours. | `Blend2D_Correctness` (`TestBlendPresets`, `TestTintChain`) |
