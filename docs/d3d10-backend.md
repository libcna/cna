# D3D10 (real `ID3D10Device`, real HLSL shaders, DXVK-delivered) Backend — Completeness Status

`D3D10` is architecturally very different from every backend in the `DX1`..`DX8` family: Direct3D 10
...
ceiling). This backend is architecturally much closer to this project's own `D3D9`/`D3D11` backends
...
**This document covers a genuinely new v1 backend, not a port.** Scope is deliberately bounded
...
- **Real MRT support** (`OMSetRenderTargets` takes an array) — a genuine, real difference from every
  `DX1`..`DX8` backend (DirectDraw/early-Direct3D has exactly one active render target).
...
- **Winding order flips at 180° rotation — a real bug found via CTest.** D3D10's default rasterizer
  ...
  for its own quads. Fixed: `D3D10SpriteBatchBackend` binds a dedicated cull-none rasterizer state
...
- **`SetTransformMatrix` real bug found via CTest**: the public `SpriteBatch::Begin(...)` call order
  is `backend_->SetTransformMatrix(transformMatrix_)` THEN `backend_->Begin()` — an initial draft
  of `D3D10SpriteBatchBackend::Begin()` unconditionally reset `transformMatrix_` back to identity,
  silently discarding whatever the caller's own `Begin(..., transformMatrix)` overload had just set
  moments earlier. Fixed by removing that reset (matching `Dx8SpriteBatchBackend::Begin()`'s own
...
- `DrawColoredPrimitives`/`DrawIndexedColoredPrimitives` (**required** by `IGraphicsBackend`): real
...
- `DrawPrimitivesEx`/`DrawIndexedPrimitivesEx` (**optional**, has a safe default fallback to the
  ...
  without `D3D11GraphicsBackend`'s own full 10-stock-shader-variant richness.
- `CreateEffectBackend` (custom `ShaderEffect`), occlusion query, 3D/cube textures, instancing: not
  implemented in this v1 — all have safe `IGraphicsBackend` base-class defaults (return
  `nullptr`/throw), matching every backend in this family at its own MVP stage.
...
`D3D10_MultipleRenderTargets`, plus the shared, backend-agnostic EasyGL-authored
...
difference from every `DX1`..`DX8` backend). `OcclusionQuery`/`CustomEffects` report `false` (out
...
gap every prior legacy-scope backend in this family shows expected, not a new regression).
...
backend's source — a real, automated proof this backend genuinely has no fixed-function-era
...
- `plan_d3d10.md` — this backend's own implementation plan and design-decision record.
- `dx10-spike/README.md` — the full `DX10-0` spike record plus the environment-bug investigations.
- `docs/dx8-backend.md` — the last backend in the `DX1`..`DX8` legacy family; `D3D10` diverges from
