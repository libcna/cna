...
```sh
ctest --test-dir build-skia -L 'Audit|Raster' --parallel 8 --output-on-failure
```
...
xvfb-run -a bash -c '
  ...
  ctest --test-dir build-skia -L Skia --parallel 8 --output-on-failure
'
```
...
There is no accelerated Skia binary or runtime fallback in this revision. The pinned GN arguments
...
cmake -S . -B build-sdl-renderer -G Ninja \
  ...
  -DCNA_BUILD_TESTS=ON
cmake --build build-sdl-renderer --parallel 8
```
