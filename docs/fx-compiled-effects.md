...
- CNA applies one narrow, versioned robustness patch to that exact MojoShader revision
  (`cmake/patches/mojoshader-6333f74-effect-parser-robustness.patch`), which turns thirty-one ways
  untrusted bytecode could crash the process into ordinary parser errors. Thirty of the thirty-one were
