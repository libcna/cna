# Stub (no-op) graphics backend

## Status

The Stub backend is a **deliberately minimal no-op graphics backend**, verified 2026-07-19:
...
needing a real backend this one deliberately doesn't provide, 1 a pre-existing `EASYGL`-only test
unrelated to this backend; see `plan_stub.md` for the full breakdown). Select it with:

```bash
cmake -S . -B cmake-build-stub \
  -DCNA_GRAPHICS_BACKEND=STUB \
  -DCMAKE_BUILD_TYPE=Debug \
  -DCNA_BUILD_TESTS=ON
cmake --build cmake-build-stub -j
```

No extra dependencies are needed — like `HEADLESS`/`SOFTWARE`, this backend never touches SDL's
video subsystem, OpenGL, Vulkan, or any GPU library. It only needs the same SDL3/SDL3_image/
SDL3_mixer and `../sharp-runtime` checkout every other backend already requires.

## What this backend is for (and isn't)

Two backends already exist that avoid the GPU entirely: `HEADLESS` (real argument validation, a
...
Stub is neither. It is the smallest possible complete `IGraphicsBackend` implementation: every
...
- **A minimal reference implementation.** Reading `StubGraphicsBackend.hpp`/`.cpp` end-to-end shows
  exactly which `IGraphicsBackend` members must be implemented (pure virtual) vs. already have a
...
- **The fastest "does the game loop even run" smoke check** — nothing to maintain, nothing to get
  subtly wrong.
- **A genuinely dependency-free placeholder backend** for build configurations that need *some*
  backend to link without pulling in SDL video, a GPU library, or Headless/Software's own overhead.
...
bookkeeping"), not this backend — see `docs/headless-backend.md`.

## Naming

Named `Stub`, not `Null` — `NULL` is a `<cstddef>`/`<cstdlib>` macro (`0`/`nullptr` depending on
context), so a bare `CNA_GRAPHICS_BACKEND=NULL`/`NullGraphicsBackend` name risks silent macro
substitution (e.g. in a `#if CNA_GRAPHICS_BACKEND == NULL` comparison) on top of just reading
...
backend selected at CMake configure time. See `examples/stub_smoke_test.cpp` for a full working
...
- **5 `CnaTests` failures are the direct consequence of the `Texture3D`/`TextureCube`/custom-
  ...
  expect a real GPU-backed `Texture3D`/`TextureCube`/custom `Effect`, which this backend does not
...
- **`ReadBackbuffer()` throws** (the shared `IGraphicsBackend` default) — there is no real pixel
...
- **`CreateEffectBackend()`/`CreateOcclusionQuery()`/`CreateTexture3D()`/`CreateTextureCube()`/
  ...
  `IGraphicsBackend` defaults) — a custom `ShaderEffect`, an occlusion query, or a 3D/cube texture
  or render target simply isn't available under this backend.
...
- **Not a column in `docs/graphics-backend-feature-matrix.md`** — with zero real rendering, none of
  that matrix's pixel/behavior rows are meaningful for this backend, the same reasoning that
