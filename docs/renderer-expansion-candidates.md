...
| A10 | `SIXEL` | libsixel / DEC SIXEL | 2D presentation | **True pixels in a terminal.** Distinct from the removed `ASCII` identity in kind, not degree: that was glyph quantization (now a post-process effect), this is a real framebuffer transport. Note that after the curation **no surviving renderer implements `needsSurfacePresenter`**, so a terminal transport has no CPU-frame producer to attach to; that gap would have to be closed first. | sixel-capable TTY | S | low | MIT |
