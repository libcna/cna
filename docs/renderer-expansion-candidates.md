...
**42 public renderer identities**, mechanically verified by `scripts/check_renderer_identities.py`
against `modules/core/include/CNA/GraphicsRendererType.hpp` and `cmake/RendererSelection.cmake`
(`OK: 42 public renderer identities preserved in both registries`). Selected at configure time via
