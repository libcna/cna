...
| A2 | `TINYGL` | C-Chads/tinygl (Bellard lineage) | CPU fixed-function | **DELIVERED 2026-08-13.** Fixed-function GL 1.x **on CPU**. `OPENGL1` needs a driver; `PORTABLEGL` is shader-era; `SOFTWARE` is CNA's own rasterizer. See `tinygl-renderer.md` / `../plan_tinygl.md`. | none | S | low | zlib-style, acknowledgment required |
