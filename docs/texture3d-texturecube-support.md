...
> Covers: EasyGL, Vulkan renderers. SDL_Renderer has no 3D texture support at all
...
EasyGL and Vulkan still silently reduce every format request to 32-bit RGBA. Metal's adapted
...
| Vulkan  | ✅ — real staged readback. Both `VulkanTexture3DRenderer::GetData` and `VulkanTextureCubeRenderer::GetData` are implemented, copying through a host-visible staging buffer. Pixel-verified by `Vulkan_Texture3D_Slices_RoundTrip`, `Vulkan_TextureCube_PartialRect_RoundTrip` and `Vulkan_TextureCube_Mip_RoundTrip`. **What this cell said before, and why it is kept:** "total no-op — neither renderer overrides `GetData`, so the caller's buffer is left completely untouched, no exception, no error, no log message." That was true when written and is not now; the row is corrected rather than deleted so a reader who remembers the old claim can see it was measured false rather than quietly dropped (`plans/plan_vulkan.md` D-01, `VULKAN-481`). |
...
actually returns. Running the full suite against `cmake-build-vulkan` reports
...
is why this went unnoticed until this task cross-checked the Vulkan renderer source directly.

Not fixed here — this is a real feature gap (Vulkan needs a staging-buffer-based
`vkCmdCopyImageToBuffer` readback path), tracked as new `plans/plan_graphics.md` Task 865.
...
and Task 864 (Vulkan, both types — new).
...
`TextureCube.cs` — so this is a CNA safety extra. Both renderers already guarded against an invalid
...
| Use case | EasyGL | Vulkan | Metal |
|---|:---:|:---:|:---:|
| `TextureCube` in `EnvironmentMapEffect` | ✅ (pre-existing, reconfirmed) | ✅ (pre-existing, reconfirmed) | 🔍 real, world-space cube-map reflection (flat + Fresnel-weighted blend, lit+fogged), landed and source-complete (`plans/plan_metal.md` Phase 6, `METAL-64`–`71`) — not independently pixel-verified against a known-reflective scene |
...
| Fix Vulkan's mip-level allocation for both `Texture3D` and `TextureCube` (`mipLevels` hardcoded to 1) | Task 864 (new) |
| Implement real GPU readback for `Texture3D`/`TextureCube::GetData` on Vulkan (staging-buffer `vkCmdCopyImageToBuffer`) | Task 865 (new) |
