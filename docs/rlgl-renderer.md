...
| Recovery registry and CPU shadows | ✅ | A registry subset measures exact defined Texture2D mip, Texture3D volume-mip, and TextureCube face/mip bytes, buffer bytes/declarations, target descriptions, pipelines, queries, and optional effect state. `SetContextRecoveryEnabled(false)` gates future shadows while teardown remains intact; render targets retain no pixels and are classified for the existing `ContentLost` contract (`RLGL-055`/`062`) |
...
| Registered child-resource restoration | ✅ | Texture2D mip, Texture3D volume-mip, and TextureCube face/mip contents, buffer bytes/declarations, target attachments, SpriteBatch/query pipelines, and optional compiled effects are rebuilt transactionally before active 2D/cube/MRT rebinding. Injected child failure rolls every replacement name back for retry; two complete cycles, target `ContentLost`, stock/SpriteBatch/compiled pixels, native DXT, decoded fallback, and exact volume bytes passed (`RLGL-057`/`062`) |
...
| `Texture3D` | ✅ | `SurfaceFormat::Color` volumes allocate complete width/height-derived mip chains while halving depth per level; exact full/partial slice-major upload/readback, live extent reporting, GL binding, independent `AddressW`, source and compiled sampling, recovery shadows, and context recreation passed `RLGL-062`. Other labels are refused because CNA's current public volume API exposes Color transfers rather than their format-native payloads. |
...
| Compiled XNA effects | ✅ | With `CNA_RLGL_COMPILED_EFFECTS=ON`, the pinned MojoShader path executes the same 17 shared contracts as DirectX11: reflection/state, ordinary/multi-stream/instanced indexed and non-indexed draws, SpriteBatch, reflected attributes/uniforms, fragment and vertex 2D/cube/volume samplers, pass selection, render-target orientation, stock-compatible depth, switching, stress/truncation, and context restoration (`RLGL-047`/`048`/`049`/`051`/`062`). `GraphicsCapability::CompiledEffects` is true in this build. |
| CNAEXT source `ShaderEffect` | ✅ | RLGL-050 compiles and links caller desktop GLSL, reports structured diagnostics, maps declaration-order attributes, uploads every loose uniform/array form, binds Texture2D/Texture3D/TextureCube, drives indexed/non-indexed 3D and SpriteBatch draws, survives source/stock transitions, and rebuilds retained source/uniform state after context recreation. RLGL-062 proves real volume slices and independent W addressing. |
...
`ThreeD`, `Texture3D`, and `WireFrame` are true after their representative workload and pixel gates passed.
...
run. Texture2D, Texture3D, and TextureCube sampler paths pass their pixel oracles; texture resources,
sampler objects, and compiled effect state are all rebuilt during context recreation.
