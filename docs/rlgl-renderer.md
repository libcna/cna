...
| Stock/custom effects and normal 3D workloads | ❌ | The primitive baseline intentionally rejects texture, lighting, fog, alpha-test, skinning, custom effects, multi-stream input, and instancing until their shader/input tasks; `GraphicsCapability::ThreeD` remains false |
...
`Rlgl_Sampler`, `Rlgl_State`, `Rlgl_SpriteBatch`, `Rlgl_Buffer`, and `Rlgl_Primitive`, labelled
...
independently checked-out copy of the exact upstream commit and compiles the renderer plus the
  smoke, texture, sampler, state, SpriteBatch, buffer, and primitive executables. It intentionally
  does not yet claim hosted runtime coverage; promotion to a CI runtime gate belongs to
  `RLGL-021` after the runner's context path is proven.
