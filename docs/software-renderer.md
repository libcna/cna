...
- **Classic compiled XNA Effects are not yet executable** (`SOFTWARE-161`). The public bytecode
  ...
  explicitly texture-free instruction. SOFTWARE-379 adds the destination-stage `TEXREG2AR` and
  `TEXREG2GB` loads to both paths, including CPU mip footprints derived from the selected AR/GB
  components rather than the ignored raw x/y pair. SOFTWARE-380 executes the sampled legacy
  `TEXM3X2PAD`/`TEXM3X2TEX` pair and derives CPU implicit LOD from its two matrix products rather
  than the untransformed source register. SOFTWARE-381 adds `TEXM3X3TEX`, transformed cube/volume
  footprints and the Shader Model 1.x CTAB sampler map that distinguishes volume from cube in both
  ...
  suspicion required tests and documentation but no renderer repair. SOFTWARE-441 then enforces
  Microsoft's immediate-constant definition contract: pixel `DEF` and both-stage `DEFI`/`DEFB`
  ...
  only implicit/explicit XYZ. SOFTWARE-462 then extends exact strict-profile temporary-component
  validation from `MOV` to the component-wise arithmetic family. Its destination mask and each
  source swizzle determine the actual components consumed, including unary, binary and ternary
  operands in `ps_1_4`, `ps_2_0` and `vs_1_1`. The dependency series now contains 87 patches;
  scalar, dot, matrix and other fixed-component opcode rules remain SOFTWARE-164/165 work.
  ...
  proves the same runtime gradients for volumes and converts cube direction gradients into the
  selected face's local texel footprint. SOFTWARE-390 closes a separate ps_1_4 ordering hole:
