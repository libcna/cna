...
`Dxt5` with an exact padded compressed-block CPU chain and a bounded decoded sampling image, whose
descendant mip levels are never generated and must be explicitly authored. The three remaining
compressed rows (`Dxt5SrgbEXT`, `Bc7EXT`, `Bc7SrgbEXT`) and every non-`Color` render target remain
refused until their independent SKIA-141–143 gates pass.
