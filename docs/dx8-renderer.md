# DX8 (real `IDirect3D8`/`IDirect3DDevice8`, DXVK-delivered, fixed-function) Renderer — Completeness Status

`DX8` is architecturally very different from every renderer in this family so far (`DX1`..`DX7`, all
...
own swap chain, the same device-bring-up shape as this project's own `D3D9Renderer`. Delivered
...
project, not the Wine-native pattern `DX1`..`DX7` use. Scope is **fixed-function 3D only**, matching
...
**This document covers what's specific to `DX8`.** Unlike `DX3`..`DX7` (each a mechanical port of
its predecessor with a documented delta), `DX8`'s 2D layer, device bring-up, and readback path are
...
## 2. Real architectural deltas vs. `DX1`..`DX7`
...
- **`D3DTLVERTEX`/`D3DFVF_TLVERTEX` are gone from the real headers.** D3D8 introduced the generic
  ...
  `Dx8TLVertex` struct reproduces the exact same 32-byte layout the old macro implied.
...
- **`DrawPrimitive`/`DrawIndexedPrimitive` require a bound vertex buffer** (`SetStreamSource`) — the
  CPU-memory-pointer submission `DX2`..`DX7` all used is `DrawPrimitiveUP`/`DrawIndexedPrimitiveUP`
...
- **No scaled-blit primitive at all.** No `StretchRect` (D3D9-only) and `CopyRects` is same-size-copy
  ...
  reusing `DX7`'s own letterboxing test. Solved with a **logical-resolution render target**: all
  ...
  logical resolution, matching `DX1`..`DX7`'s own letterbox convention — NOT D3D9's own simpler
  "swap chain always matches the window" choice.
- **2D `SpriteBatch` is a redesign, not a port.** `DX1`..`DX7` all used DirectDraw's `Blt`/`BltFast`
  for a CPU-side pixel compositor. With no DirectDraw at all, `Dx8SpriteBatchRenderer` renders real
  ...
  blending, real rotation/scale) over `DX1`..`DX7`'s CPU-approximated-per-blend-mode-formula
  approach, matching the pattern every non-DirectDraw-based CNA renderer (D3D9/EasyGL/Vulkan) uses.
- **Real hardware blending, no preset-detection fallback.** `DX2`..`DX7`'s CPU-emulated
  ...
  hardware to fall back to). `DX8`'s `ApplyBlendState` sets `D3DRS_SRCBLEND`/`D3DRS_DESTBLEND`
  ...
  real, documented DirectX-8-era boundary, verified by `Dx8_Blend`'s own Check F.
- **`AnisotropicFiltering` reports `true`** — unlike every prior renderer in this family (`DX2`..`DX7`,
  all confirmed-absent on their shared software rasterizer), `DX8` runs on a real GPU via DXVK, so
  this is a genuine, distinct capability here.
- **No native mip chain**, same as `DX2`..`DX7`: `Texture2D::SetData(level>0, ...)` throws honestly
  (`Dx8TextureRenderer`/`Dx8RenderTargetRenderer::UpdatePixelsLevel`) rather than silently discarding
  the upload.
- Everything else (CPU transform/clip pipeline — Sutherland-Hodgman near-plane clip, perspective
  ...
  conceptually the same math as `DX7`'s own `Dx7ClipVertex`/`Dx7ClipTriangleNearPlane`/
  `Dx7ComputeVertexLighting`, re-expressed against the new `Dx8TLVertex` struct and
...
1. **A real `Dx8Renderer::SetVirtualResolution` bug.** The logical render target (texture +
...
2. **Two environment-specific Wine/DXVK/AMD-RADV bugs, not code defects** — both reproduced in
   ...
   - SDL3's own internal, dynamic `LoadLibrary("dxgi.dll")` probe (for HDR/colorimetry detection,
     ...
     for the rest of the process. Fixed by giving `DX8` its own dedicated `~/.wine-cna-dx8` Wine
   ...
   - A real AMD RADV driver bug: the SECOND consecutive `Present()` call on this sandbox's GPU fails
     ...
     software (`llvmpipe`) Vulkan device instead avoids it entirely. `scripts/run-wine-dx8.sh` sets
...
3. **A real D3D8 XYZRHW half-texel alignment bug.** D3D8's pre-transformed (`D3DFVF_XYZRHW`)
   ...
   shift into its own orthographic projection matrix). Since `Dx8SpriteBatchRenderer`'s quad is
   already in screen space with no such matrix, the same -0.5 shift is applied directly to the
   final screen-space quad corners instead (`Dx8SpriteBatchRenderer::Draw` and `Present()`'s own
   full-screen quad). Found via `Dx8_SpriteBatch`'s `SpriteEffects::FlipHorizontally` check (a
   1:1 texel:pixel mapping, maximally sensitive to any half-texel misalignment) and confirmed to
   also fix `Dx8_AddressMode`'s `TextureAddressMode::Mirror` check, which failed for the same
   underlying reason.

## 4. CTest results

**20/20 `DX8`-labeled CTests pass**: `Dx8_LegacyInterfaceDiscipline`, `Dx8_Smoke`,
`Dx8_TextureRenderTarget`, `Dx8_SpriteBatch`, `Dx8_Blend`, `Dx8_AddressMode`, `Dx8_SpriteFont`,
`Dx8_GraphicsCapability`, `Dx8_LogicalTransform`, `Dx8_Device3DSmoke`, `Dx8_VertexIndexBuffer`,
`Dx8_ColoredPrimitives`, `Dx8_IndexedPrimitives`, `Dx8_ZTest`, `Dx8_Texture3D`, `Dx8_Clipping`,
`Dx8_RemainingDefaults`, `Dx8_Lighting`, `Dx8_WireframeAniso`, `Dx8_Stencil`.
...
The legacy-interface-discipline check (`scripts/check-dx8-legacy-interface-discipline.sh`) forbids
...
- `docs/dx7-renderer.md` — the last DirectDraw-based renderer in this family; `DX8` diverges from it
