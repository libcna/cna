...
| MSAA (`MultiSampleCount`, real resolve) | ✅ (fixed, Task 337) | ✅ — real multisampling with resolve, pixel-verified by `Vulkan_MSAA_4x_Readback`, `Vulkan_MRT_MsaaResolve` and `Vulkan_MsaaDepthContract`; the applied count is device-clamped and reported truthfully (`VULKAN-347`), and it is **per instance** since `VULKAN-216` (`Vulkan_RenderTargetOwnMsaa`) -- a target asking for 4x gets 4x on a back buffer that has none, where Task 878/879's "piggyback on the renderer's own `sampleCount_`" made it report `0` and render one sample. **Previously** "honestly reports `0`, not implemented (Task 879)"; corrected by `VULKAN-481` (D-02) | 🔶 same as Vulkan (Task 879) | ❌ deliberately unsupported: requested counts clamp to `0`, capability is false, and native attachments remain single-sample; the historical sample-count-four path produced a binary edge |
| `SetRenderTarget`/`SetRenderTargets` Viewport/ScissorRectangle reset on a real binding change; identical sets are no-ops | ✅ (fixed, Task 338 / SOFTWARE-222) | ✅ (fixed, Task 338 / SOFTWARE-222) | ✅ (fixed, Task 338 / SOFTWARE-222; shared code, not independently pixel-tested) | ✅ shared `GraphicsDevice`-level code, renderer-independent |
