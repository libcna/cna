...
| `GetWindowInternal()`/`GetRendererInternal()` | ✅ | Returns the real `SDL_Window*`; `GetRendererInternal()` is `nullptr` (no `SDL_Renderer*` exists on this backend, same as `OPENGLES`). |
