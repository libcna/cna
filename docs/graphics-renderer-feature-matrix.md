...
GPU," not "verified on real Windows hardware"** — see `docs/d3d11-renderer.md`/`docs/d3d12-renderer.md`'s
...
`docs/d3d12-renderer.md`. So a `D3D12` ✅ below means "real off-screen GPU pixel/behavior proof,"
...
open). See `docs/d3d9-renderer.md` and `docs/d3d9-divergence-report.md` for the full detail this
...
| Pixel/query correctness (visible vs. occluded) | ✅ verified both directions (Tasks 445/446) | ✅ verified both directions, plus a multi-draw-span check (Task 854) — this sandbox's software Vulkan driver (Mesa Lavapipe) reports fully accurate, discriminating pixel counts (4096 visible / 0 occluded on a 64×64 quad) | ⚠️ can't verify in this sandbox's software GL2.1 driver; dedicated-view architecture gap open (Task 917) | N/A | N/A — no query support exists | ✅ **stale row, corrected 2026-07-15** — `plan_dx.md` `DX-147` closed this: the same `ID3D11Query` object reused around a genuinely visible (viewport-covering) draw reports a real positive `PixelCount()`, then around a fully off-screen draw reports exactly 0, both-directions discriminating (`D3D11_Smoke` 91→93/93). This row previously described the pre-`DX-147` state | ✅ **stale row, corrected 2026-07-15** — real, and closed earlier than the row above: `DX-120`'s own landing already included this exact both-directions proof (Checks `AA3`/`AA4`, `plan_dx.md` `DX-147`'s own row explicitly credits `DX-120` for closing the D3D12 half) — this row's "no query support exists" contradicted the very next table up (`OcclusionQuery` "wired to real GPU work" row, same file), which already correctly described `D3D12OcclusionQueryRenderer` as real |
...
renderer. See `docs/d3d11-renderer.md`. The same link failure applies equally under `D3D12` (same
root cause, same example binaries, same shared code) — not independently re-confirmed but expected
identical, see `docs/d3d12-renderer.md`.
...
See `docs/d3d9-renderer.md` for the full writeup and `docs/d3d9-divergence-report.md` for the raw
...
- `docs/d3d9-renderer.md`, `docs/d3d9-divergence-report.md` — the `D3D9` column's full detail: what
