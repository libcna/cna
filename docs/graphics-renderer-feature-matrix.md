...
host. It covers every renderer directory that implements
...
provider/rejecter partition; adding a renderer without a disposition fails L0. The public
...
implementations and the D3D11 count/native mappers select their APIs' real point-list
...
The `SKIA` identity was retired on 2026-08-30 and is not restored; this document previously
carried a 23-row "Skia CPU-raster 2D companion matrix" beside the tables below. It was removed on
2026-09-17 with the rest of the retired-renderer documentation: every cell's evidence link pointed
at a `docs/skia-*.md` page or `plans/plan_skia.md` that was deleted with the renderer, so the
matrix could no longer be checked against anything. `docs/removed-renderers.md` is the tombstone,
and the removal commit is the record.
...
885-900's fog/lighting/specular fixes on Vulkan and still show some of these rows as gaps —
...
### Shared MRT attachment limit (Task 775)
...
this is the practical, enforced limit on every renderer regardless of what the underlying device
...
outright, unlike GL's more forgiving capability model). Confirmed the device-creation code
...
| Feature | EasyGL | Vulkan | SDL_Renderer | D3D9 | D3D11 | D3D12 |
|---|---|---|---|---|---|---|
| Wired to real GPU work (`Begin`/`End`) | ✅ | ✅ **FIXED (Task 447, 2026-07-10)** — real per-draw-call query correlation via `Pending3DDraw::occlusionQuery` tagging + `vkCmdBeginQuery`/`vkCmdEndQuery` recording in `RecordCommandBuffer()` | N/A — throws at construction (Task 727) | ⬜ **not built at all** — `plans/plan_dx9.md`'s task list has no `IDirect3DQuery9`-based occlusion-query task; `CreateOcclusionQuery()` falls through to `IGraphicsRenderer`'s own silent `nullptr` default, same gap as D3D12 | ✅ real `ID3D11Query(D3D11_QUERY_OCCLUSION)`, `Begin`/`End`/`GetData` wired (`DX-47`) | ✅ real `D3D12OcclusionQueryRenderer` (`ID3D12QueryHeap`+readback buffer, `DX-120`) — a full-viewport visible triangle reports a real positive `PixelCount()`, the SAME query object reused around off-screen/clipped geometry reports exactly 0, a genuine visible-vs-invisible discrimination, off-screen (Checks AA1–AA4) |
| Pixel/query correctness (visible vs. occluded) | ✅ verified both directions (Tasks 445/446) | ✅ verified both directions, plus a multi-draw-span check (Task 854) — this sandbox's software Vulkan driver (Mesa Lavapipe) reports fully accurate, discriminating pixel counts (4096 visible / 0 occluded on a 64×64 quad) | N/A | N/A — no query support exists | ✅ **stale row, corrected 2026-07-15** — `plans/plan_dx.md` `DX-147` closed this: the same `ID3D11Query` object reused around a genuinely visible (viewport-covering) draw reports a real positive `PixelCount()`, then around a fully off-screen draw reports exactly 0, both-directions discriminating (`DirectX11_Smoke` 91→93/93). This row previously described the pre-`DX-147` state | ✅ **stale row, corrected 2026-07-15** — real, and closed earlier than the row above: `DX-120`'s own landing already included this exact both-directions proof (Checks `AA3`/`AA4`, `plans/plan_dx.md` `DX-147`'s own row explicitly credits `DX-120` for closing the D3D12 half) — this row's "no query support exists" contradicted the very next table up (`OcclusionQuery` "wired to real GPU work" row, same file), which already correctly described `D3D12OcclusionQueryRenderer` as real |
...
`ReferenceStencil` gap (Task 872) remains open only on EasyGL. The 2D
...
Unlike the other columns, `DIRECTX9`'s ✅ cells above are not "implemented and plausible" — they are
...
- ~~`PreferPerPixelLighting` and real specular highlights are structurally unreachable~~ — **fixed
  ...
  fixed EasyGL/Vulkan/WebGPU(`BasicEffect`-only)/D3D11/D3D12 too. Software subsequently gained
...
- Per-effect docs (`docs/basiceffect-support.md` etc.) — largely predate Tasks 885-900's
  fog/lighting/specular fixes on Vulkan; this matrix reflects the current state, those
