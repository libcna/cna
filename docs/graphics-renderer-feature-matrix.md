...
(`../plans/plan_dx9.md`, `D9-130`) 2026-07-15, once each renderer's feature set was broad enough for a
meaningful row-by-row comparison. The experimental WebGPU renderer is intentionally tracked
separately in [`webgpu-renderer.md`](webgpu-renderer.md) and `../plans/plan_webgpu.md` until its feature
surface is broad enough for meaningful parity columns. The experimental LLGL renderer
([`llgl-renderer.md`](llgl-renderer.md), `../plans/plan_llgl.md`) is tracked separately for the same
...
An **`IGL`** column was added 2026-08-18 (`../plans/plan_igl.md` IGL-53), once the same bar the other
columns are held to was actually met: 27 example binaries pass on OpenGL/GLX and 24 on Vulkan, both
through Mesa (llvmpipe and lavapipe) under Xvfb, covering 2D, 3D, depth, stencil, MRT, render
targets, cube targets, MSAA, instancing, five stock effects, custom `ShaderEffect`s and the surface-
format boundary. This row was held open deliberately until then, and until the two things that
would have made a single column dishonest were resolved: a `SpriteBatch` pixel test failing on
Vulkan for reasons nobody had root-caused (IGL-60), and a custom `ShaderEffect` taking the process
down there (IGL-68/IGL-70). Both are closed.

**An `IGL` cell describes BOTH of its backends unless it says otherwise.** That is the whole
difficulty of giving this renderer one column — IGL is itself an abstraction over OpenGL and Vulkan,
chosen per process by `CNA_IGL_BACKEND` — so a claim that holds on only one of them is written out
as such rather than averaged. Every ✅ below names the test that produced it, and 🟨 is used for
"implemented and partly exercised, but the row as written is broader than what was measured" rather
than as a soft ✅.
...
own "Known limitations" for the real-Windows gate (`plans/plan_dx.md` `DX-90`/`DX-91` for D3D11, `DX-114`
for D3D12) that still applies on top of every ✅ below. The 2026-09-09 DX17 corpus has **265 parity
declarations, 262 shared**, and passes **266/266 D3D11** plus **264/264 D3D12**. Most D3D12 fixtures use
the off-screen public device; `BackbufferResize`, `RealWindowResize` and `ViewportResetAfterResize`
are registered CTests that use the matched Proton real-window wrapper on private virtual display
`:4`. A cell is only marked ✅ if a real,
...
The **Headless** renderer (`CNA_GRAPHICS_RENDERER=HEADLESS`, tracked in `../plans/plan_headless.md`) is deliberately
...
`plans/plan_gltf.md`; it does not turn a boundary-only result into pixel support.
...
implementations and the D3D10/D3D11 count/native mappers select their APIs' real point-list
topologies. D3D12 now maps `LineList`, `LineStrip` and `PointListEXT` too, keys its PSO by the required
triangle/line/point topology class, and counts one vertex per point (`DX-208`). The shared validation
fixture accepts all five public primitive types while rejecting an invalid ordinal, and D3D12 pixel
proof distinguishes a 64-pixel line from two one-pixel points. No Direct3D mapper retains an unnamed
default that can silently turn a future topology into a triangle list.

The **Wicked Engine** renderer (`CNA_GRAPHICS_RENDERER=WICKED`, tracked in `../plans/plan_wicked.md`) is
...
logic is unit tested; that is all. Revisit once `plans/plan_wicked.md` `WICKED-18`/`WICKED-74` have been
...
|---|---|---|---|
| `SpriteBatch` overloads, sorting, rotation/flip/scale/crop | ✅ | Direct `SkCanvas` 2D route; no renderer-local public-overload shortcuts. | [SKIA-31–40](../plans/plan_skia.md), [2D registrations](skia-2d-easygl-registration.md), [XNA oracle](skia-xna-oracle.md) |
| Custom `Effect` in `SpriteBatch.Begin` | ⚠️ | Exact stock `SpriteEffect` plus explicit `CNA_SKIA_SKSL_V1` fragment SkSL for ordinary sprite quads, and a separate bounded `CNA_SKIA_SKSL_MESH_V1` per-vertex mesh ABI reached via `SpriteBatch.DrawMeshEXT` (`SkVertices`-based, `SpriteSortMode.Immediate` only) that renders `DualTextureEffect`'s core formula both hand-written and through a restricted GLSL-to-SkSL translator; arbitrary EasyGL GLSL, custom vertex attributes beyond position/UV/colour, and stock 3D effects reject. | [SKIA-89–94, SKIA-144–158](../plans/plan_skia.md), [effects boundary](skia-effects.md), [mesh contract](skia-vertices-2d-effect-contract.md), `Skia_SkSL_UniformTexture`, `Skia_MeshEffect_PublicApi` |
| `SpriteFont` properties, glyph layout, fallback, newline and effects | ✅ | Shared font-atlas layout drawn through the direct sprite route; Skia text APIs are deliberately not substituted. | [SKIA-38, SKIA-109](../plans/plan_skia.md), `Skia_Contract_SpriteFontProperties`, five `Skia_SpriteFont_*` tests |
| Clamp/Wrap/Mirror U/V and point/linear filtering | ✅ | Direct Skia tile/filter modes, independently selected per axis. | [SKIA-43–46, SKIA-106, SKIA-108](../plans/plan_skia.md), `Skia_TextureAddressAxes` |
| `RenderTarget2D` colour/readback/sampling/usage | ✅ | Direct level-0 `SkSurface`; target switching and lifetime are checked. Mipmapped targets additionally own stable per-level surfaces, exact transfer shadows and one-shot dirty descendant resolve. | [SKIA-61–75, SKIA-131–132](../plans/plan_skia.md), `Skia_RenderTarget2D_Golden`, `Skia_RenderTarget2D_MipStorage`, `Skia_RenderTarget2D_MipGeneration` |
| `RenderTargetCube` face rendering/readback/usage/mips | ⚠️ | Six independent 2D raster surfaces with CPU-generated box mips; no cube sampler, depth or MSAA claim. | [SKIA-85–86](../plans/plan_skia.md), `Skia_RenderTargetCube_Policy` and four shared cube contracts |
| Multiple render targets | ❌ | `SkCanvas` has one colour result; replay emulation was evaluated and cannot reproduce distinct outputs for slots 0–3. Empty or one-target plural binding works. | [SKIA-87–88](../plans/plan_skia.md), `Skia_MRT_Rejection` |
| MSAA backbuffer/targets | ❌ | Selected raster `SkSurface` owns zero physical samples. Backbuffer requests from zero through oversized values clamp to 0; real `RenderTarget2D` requests reject. A fake resolve or multisample mask is not exposed. | [surface-mode ADR](skia-surface-mode-adr.md), [SKIA-56, SKIA-76–77](../plans/plan_skia.md), `Skia_RenderTarget2D_MsaaPolicy` |
| Mutable `RenderTarget2D` mip chain | ✅ | CNA-owned stable per-level surfaces and canonical shadows implement exact transfer and sampling. Canvas writes or parent uploads dirty their suffix; unbind/readback/sampling resolves every dirty descendant exactly once with deterministic odd/NPOT area boxes, matching the EasyGL resolve lifecycle. | [SKIA-70, SKIA-131–132](../plans/plan_skia.md), `Skia_RenderTarget2D_MipStorage`, `Skia_RenderTarget2D_MipGeneration`, `Skia_EasyGL_RenderTarget2D_MipComplete` |
| `Texture2D` level-0 Set/Get/partial/NPOT sampling | ✅ | Exact CPU shadow plus direct raster image path; transfer and validation contracts share sources with EasyGL. | [SKIA-22–30, SKIA-106, SKIA-109](../plans/plan_skia.md), [API contract comparison](skia-api-contract-comparison.md) |
| `Texture2D` mip levels above zero | ✅ | Public `mipMap=true` construction owns a complete checked CNA chain; transfer is exact, changed parents area-box-generate only unauthored descendants, all nine TextureFilter ordinals select/interpolate the real levels, and DDS/XNB loaders preserve every declared complete-chain level without fabricating absent mips. | [SKIA-125–130](../plans/plan_skia.md), [resource policy](skia-successor-resource-oracles.md), `Skia_Texture2D_MipGeneration`, `Skia_MipSampling_Raster`, `Skia_Sampler_MipmapFilterPolicy`, `Skia_Texture2D_ContentMips` |
| `TextureCube`/`Texture3D` Set/Get including mips | ⚠️ | Exact bounded CPU face/voxel transfer storage; this capability does not imply shader sampling (see the separate bounded sampling row below). | [SKIA-80–84](../plans/plan_skia.md), [texture storage policy](skia-texture-storage.md), shared 56-check contracts |
| Cube/volume texture sampling | ⚠️ | Bounded direction/coordinate sampling through a dedicated fragment-only SkSL extension (`cnaSampleCubeEXT`/`cnaSampleVolumeEXT`, reachable via `SetTexture(1, TextureCube)` or `SetTexture(1, Texture3D)`); no general `samplerCUBE`/`sampler3D` GLSL translation and no stock 3D effect route -- CPU transfer storage still cannot emulate geometry-driven directional/volume sampling. | [SKIA-144–151](../plans/plan_skia.md), [cube/volume sampling contract](skia-cube-volume-sampling-contract.md), `Skia_CubeVolume_Effect_Binding`, `Skia_CubeVolume_Sampling_Oracle` |
| Non-`Color` surface formats | ⚠️ | Texture2D directly stores/samples packed, BGRA, alpha-only, R8/R16, RG16 and RGBA16 UNORM routes; `Bgra4444` retains exact words with a bounded RGBA conversion, while `ColorSrgbEXT` retains encoded bytes and decodes once into linear-sRGB. The other 16 formats, all non-Color targets, and cube/volume non-Color storage remain refused. | [SKIA-25, SKIA-109, SKIA-134–137](../plans/plan_skia.md), `Skia_Texture2D_PackedFormats`, `Skia_Texture2D_ColourFormats`, `Skia_Texture2D_UnormFormats`, `Skia_Contract_SurfaceFormat` |
| Blend presets/custom state and colour-write masks | ✅ | All 714,025 valid selector tuples (13 factors⁴ × 5 functions²), independent RGB/alpha equations, live blend constants, and all target-0 masks use established or generated runtime-blender routes. Invalid raw selectors, target-1/2/3 masks, and non-default multisample masks reject atomically. | [SKIA-47–57, SKIA-108, SKIA-119–124](../plans/plan_skia.md), [generated blender](skia-generated-blender.md), `Skia_BlendMapping_Raster`, `Skia_GeneratedBlend_PublicCorpus` |
| Depth/stencil state and clears | ❌ | Disabled `None` is valid 2D state, but there is no attachment. CPU depth/stencil prototypes passed in isolation; product emulation was rejected as a separate software renderer. | [SKIA-67, SKIA-97–102](../plans/plan_skia.md), [3D ADR](skia-3d-emulation-adr.md), `Skia_3D_Refusal` |
| Rasterizer state | ⚠️ | Solid fill and scissor are meaningful in 2D. Wireframe, culling/depth bias as 3D features reject or stay unadvertised after geometry-emulation evaluation. | [SKIA-41–42, SKIA-58, SKIA-99–102](../plans/plan_skia.md), `Skia_RasterizerState_Policy` |
| Sampler slots/anisotropy/mip filtering | ⚠️ | Sprite Texture2D sampling implements all nine min/mag/mip combinations plus Clamp/Wrap/Mirror. Anisotropic is an exact complete-Linear fallback with capability false; cube/volume sampler binding is now a bounded SkSL-only extension (SKIA-144–151, see the cube/volume texture sampling row above) and general 3D sampler binding remains unavailable. | [surface-mode ADR](skia-surface-mode-adr.md), [SKIA-43–46, SKIA-70, SKIA-78–79, SKIA-102, SKIA-129, SKIA-144–151](../plans/plan_skia.md), `Skia_MipSampling_Raster`, `Skia_Sampler_MipmapFilterPolicy` |
| `ReferenceStencil` and stencil-only clear | ❌ | Zero is accepted only with disabled state; nonzero/active stencil rejects atomically. No attachment exists, and CPU-stencil product emulation was rejected by the 3D ADR. | [SKIA-67, SKIA-98, SKIA-101–102](../plans/plan_skia.md), `Skia_3D_Refusal` |
| `OcclusionQuery` | ❌ | Raster final pixels cannot distinguish positive from zero coverage. Framebuffer diff, replay and hidden-GPU routes were evaluated and rejected. | [SKIA-104–105](../plans/plan_skia.md), [feasibility report](skia-occlusion-query-feasibility.md) |
| 3D primitive/model rendering and stock 3D effects | ❌ | Direct selected-raster support is absent. SkVertices plus CPU depth/stencil/geometry/effect prototypes were evaluated; completing them would duplicate a software renderer. | [SKIA-95–103](../plans/plan_skia.md), [3D ADR](skia-3d-emulation-adr.md), `Skia_3D_Refusal` |
| Presentation recovery and resource lifetime | ✅ | Presenter-only reconstruction retains CPU surfaces/resources and emits Resetting→Reset; 64 repeated cycles are sanitizer-checked. No GPU `DeviceLost` event is fabricated. | [SKIA-16, SKIA-28, SKIA-65, SKIA-74, SKIA-110](../plans/plan_skia.md), [sanitizer validation](skia-sanitizer-validation.md) |
...
| Feature | EasyGL | Vulkan | Bgfx | SDL_Renderer | D3D9 | D3D11 | D3D12 | IGL |
|---|---|---|---|---|---|---|---|---|
| All `Draw` overloads, sort modes, rotation/flip/scale/crop | ✅ | ✅ — covered by the Vulkan SpriteBatch suite: `Vulkan_SpriteBatch_LayerDepthOrder` and `_MultiBeginEnd` for sort modes and batching, `_TransformMatrix` for the transform, `Vulkan_SpriteEffects_Flip` for flip. **Previously** "not separately re-audited (Task 861)", which was honest phrasing for an unmeasured cell rather than a claim; it is measured now (`VULKAN-481`, D-05) | not separately re-audited | ✅ (2 real bugs fixed: rotation pivot, `transformMatrix`) | ✅ real Microsoft `SpriteEffect.fx`, oracle-verified 0-divergence for rotation (`sprite_rotated_quad`), flip (`sprite_flipped_quad`), multi-texture batching (`sprite_multitexture_quad`), and 3 of 5 `SpriteSortMode` values (`Deferred`/`BackToFront`/`FrontToBack`, `D9-93`); `.Immediate`/`.Texture` confirmed not oracle-viable, not a coverage gap — see `docs/directx9-renderer.md` | ✅ destination-rect placement + `SpriteEffects::FlipHorizontally` (`DX-70`) plus rotation/scale/source-crop-rect, real geometrically-derived pixel proof (`DX-131`, Checks Y2–Y4); `SpriteSortMode` deliberately has no dedicated test — its ordering logic lives entirely in shared, renderer-agnostic `SpriteBatch.cpp` | ✅ same rotation/scale/crop proof, off-screen (`DX-131`, Checks R4–R6) — needed widening the fixture to 2 texels per color to account for D3D12's default bilinear sampler; `SpriteSortMode` same shared-code reasoning as D3D11 | 🟨 destination-rect placement with a tint, opaque and alpha-blended, pixel-verified on both backends (`Igl_2D`, 5/5). Rotation, flip, source-crop and the `SpriteSortMode` values have no test on this renderer, so the row is not ✅ — the code path is shared, but that is an argument, not a measurement |
| Custom `Effect` via `Begin(effect)` | ✅ | ✅ | ✅ | ❌ throws by design (no shader stage, 2D-only renderer) | ✅ **row corrected 2026-07-16 — was stale.** Real color-inversion custom-GLSL shader via `D3D9EffectRenderer` (runtime `D3DCompile()`), through the real public `SpriteBatch.Begin(effect)` API (`D9-110`/`D9-111`/`D9-112`, Phase D9-11 closed 2026-07-15) — this cell previously said "not started," written before that phase landed | ✅ real color-inversion custom-HLSL pixel test (`DX-71`) | ✅ same color-inversion methodology (byte-for-byte identical HLSL contract), through the real public XNA API off-screen via a windowless `GraphicsDevice` (`DX-121`, Checks NN0/NN1) | ✅ **both backends**, parameters included. A real custom-GLSL tint through the public `Begin(sortMode, blend, sampler, depthStencil, rasterizer, effect)` API, proving the CUSTOM shader ran rather than the vertex-colour path (`Igl_SpriteBatchShaderEffect` and its `_Vulkan` twin). Vulkan needs its own SPIR-V source and the effect's std140 block declared through `ShaderEffect::DeclareUniformBlockEXT` — a property of SPIR-V, documented rather than papered over (IGL-43/IGL-72) |
| SpriteFont — glyph placement/spacing/newline/fallback/flip | ✅ pixel-verified (Tasks 424-429) | ✅ — `Vulkan_SpriteFont_SingleGlyph`, `_MultiGlyphSpacing`, `_Newline` and `_DefaultCharacterFallback` cover placement, spacing, newline and fallback; flip is covered for sprites by `Vulkan_SpriteEffects_Flip` rather than by a SpriteFont-specific test. **Previously** "not separately re-audited (Task 861)" (`VULKAN-481`, D-05) | not separately re-audited | ✅ (1 real cross-renderer bug found and fixed, Task 694) | ⬜ not attempted this renderer | ✅ exact glyph placement (checked inside + all 4 edge midpoints), per-glyph advance, newline line-drop+x-reset, `FlipVertically` via an asymmetric glyph — real pixel proof through the back buffer (`DX-127`) | ✅ same proof, through a windowless `GraphicsDevice` (`PresentationParameters::HeadlessEXT`) + a real `RenderTarget2D` (`DX-132`) | ⬜ not attempted on this renderer |
| `TextureAddressMode::Wrap`/`Mirror` via SpriteBatch | ✅ | ✅ | ✅ | ⛔ **BLOCKED** (Tasks 686/687) | ✅ real `D3DSAMP_ADDRESSU`/`V`, oracle-verified: distinct 4-band tiling (Wrap) vs. symmetric mirroring pattern (Mirror), proven genuinely different from each other, not just "some non-Clamp behavior" (`D9-92`, `sprite_wrap_quad`/`sprite_mirror_quad`) | ✅ real `D3D11SamplerCache`-backed Wrap/Mirror with discriminating probe pixels (`DX-72`) | ✅ real `D3D12SamplerCache`-backed Wrap/Mirror, same probe-pixel methodology as D3D11 (`DX-133`) — found and fixed a real bug along the way: `SpriteBatch` draws previously silently used whatever sampler an unrelated prior 3D draw left bound, `SetSamplerFilter`/`SetSamplerAddressMode` were stored but inert | ⬜ not attempted — every IGL 2D test uses `SamplerState::PointClamp` |
...
| Feature | EasyGL | Vulkan | Bgfx | SDL_Renderer | D3D9 | D3D11 | D3D12 | IGL |
|---|---|---|---|---|---|---|---|---|
| Texture2D `SetData`/`GetData`/`FromStream`/`SaveAsPng`/NPOT | ✅ | ✅ | ✅ | ✅ (4 real bugs found and fixed) | ✅ real `D3DPOOL_MANAGED` upload, exercised (and oracle-verified byte-exact) by every textured scene in the corpus (`texturepixel=` used 64×); `FromStream`/`SaveAsPng`/NPOT-on-`Reach` not separately tested | ✅ `SetData`/`GetData` byte-exact GPU round-trip verified (`DX-40`); NPOT (a genuinely non-power-of-two 5×3 texture) and `FromStream`/`SaveAsPng` (real PNG encode/decode round-trip, every pixel of a varying pattern) both real (`DX-140`) | ✅ same coverage, off-screen — NPOT also exercises the `D3D12_TEXTURE_DATA_PITCH_ALIGNMENT` row-pitch edge case a 5×3 width creates; `FromStream`/`SaveAsPng` needed `PresentationParameters::HeadlessEXT` (a windowless `GraphicsDevice`) to reach at all (`DX-140`, Checks GG0–GG2/KK7–KK9) | 🟨 `SetData`/`GetData` verified byte-exactly across five texel sizes and both backends (`Igl_SurfaceFormat`), with row order and row origin pinned separately (`Igl_ReadbackOrientation`). `FromStream`, `SaveAsPng` and NPOT have no IGL-specific test |
| Texture2D mip-level `SetData` (level > 0) | ✅ | ✅ — `VulkanTextureRenderer::UpdatePixelsLevel` performs a real staged upload per level. **Previously** "❌ silent no-op (Task 867)", true when written, corrected by `VULKAN-481` (D-03) | ❌ silent no-op (Task 867) | ❌ throws by design (Task 681) | ⬜ not attempted — no oracle scene exercises mip level > 0 | ✅ real, deterministic direct-GPU-readback proof — `UpdatePixelsLevel(1,...)` round-trips exact bytes, and level 0's own content is independently confirmed unaffected (`DX-126`) | ✅ same direct-readback proof, off-screen (`DX-141`, Checks HH0–HH2) | 🟨 `UpdatePixelsLevel` uploads a real level and tracks which levels are defined; the per-level pixel proof that exists is on cube faces (IGL-17), not on `Texture2D` |
| Texture3D/TextureCube `SetData`/`GetData`, incl. mip | ✅ | ✅ | ✅ (needed a new `GetData` readback path, Task 914) | ⛔ **BLOCKED** — construction succeeds silently with a null renderer, 94-test blast radius (Task 725) | 🟨 split: `TextureCube` `SetData` real, oracle-verified byte-exact via `envmap_quad`/`envmap_fresnel_quad`; `Texture3D` has only its `GraphicsProfile` size-ceiling mutation-verified (`D9-103`) — no oracle scene or GPU round-trip test exists for `Texture3D` at all | ✅ byte-exact GPU round-trip verified for both types at mip 0 (`DX-41`/`DX-42`) | ✅ real `D3D12Texture3DRenderer` (`DX-122`, byte-exact off-center sub-volume round-trip); real `D3D12TextureCubeRenderer::GetData()` readback (`DX-123`, no longer a no-op — proven per-face-distinct and genuinely reads live GPU content, not CPU poison values) | 🟨 **cube: ✅ both, at every level** (IGL-17 — `GetData` builds a throwaway framebuffer over the cube image and selects face and mip through the range). **Volume: `SetData` ✅, `GetData` ❌ and not implementable** — IGL `v1.1.1` cannot attach a 3D texture to a framebuffer at all, which is its only readback route; established by attempting it (`GL_INVALID_OPERATION … invalid textarget GL_TEXTURE_3D`), not assumed |
| Texture3D/TextureCube sampled in shaders | ❌ don't inherit `Texture` (Task 863, architectural) | ❌ | ❌ | N/A | 🟨 `TextureCube` sampled and oracle-verified byte-exact via `EnvironmentMapEffect` (`envmap_quad`/`envmap_fresnel_quad`); `Texture3D` has no consuming shader variant tested | 🟨 `TextureCube` sampled and pixel-verified via `env_map3d` (`DX-66`); `Texture3D` has no consuming shader variant, unverified in a shader | 🟨 `TextureCube` sampled and pixel-verified via `env_map3d` (`DX-111`, Check U1); `Texture3D` has a real renderer (`DX-122`) but still no consuming shader variant, unverified in a shader | ✅ both: a volume sampled at two Z slices through a real custom shader (`Igl_ShaderEffectTexture3D`) and a cube sampled as an environment map, including the Fresnel term (`Igl_EnvironmentMapEffect`, `Igl_EnvironmentMapEffectFresnel`) |
| Non-`Color` `SurfaceFormat` for real GPU texture data | ⛔ **BLOCKED** (Task 732) | same shared-code limitation | same | same | same shared-code limitation | ✅ `DX-214`/`DX-225`: all 17 uncompressed core-XNA formats plus DXT1/3/5 retain native DXGI storage in 2D/cube/3D, with format-aware mip, partial-upload and readback proof | ✅ same native format set and behavioral proof, including block-compressed mip tails and D3D12 placed-footprint row alignment | 🟨 `Rg32` and `Single` are **public**, each with the whole path verified on both backends — typed `SetData`, sampling in a real draw checked against the format's channel semantics, `GetData`, and render-target use (`Igl_PublicSurfaceFormat`, IGL-71). Ten more formats are stored natively and verified byte-exactly but stay deferred to the framework's `Color`-only rule; `ByteEXT`/`UShortEXT`/`HalfSingle` on principle, their texels not being a multiple of four bytes |

## GraphicsDevice state objects

| Feature | EasyGL | Vulkan | Bgfx | SDL_Renderer | D3D9 | D3D11 | D3D12 | IGL |
|---|---|---|---|---|---|---|---|---|
| `BlendState` (all presets + custom factors/equations) | ✅ | ✅ **FIXED (Task 868, 2026-07-09)** — real per-`Blend`/`BlendFunction` mapping across all 9 3D pipeline-creation sites; was "almost entirely fake" (hardcoded one blend equation regardless of request, confirmed 5× via pixel tests) before this fix | ✅ | ✅ (2 real bugs fixed) | ✅ real `D3DRS_SRCBLEND`/`DESTBLEND` per-preset dispatch, `Opaque`/`AlphaBlend` pixel-behavior-verified (`DirectX9_BlendState_Opaque`/`_AlphaBlend`) and separately mutation-verified (src/dst swap caught, `D9-122`) | ✅ full shared state corpus covers every preset, separate colour/alpha factors and functions, blend factor, write masks, SpriteBatch/3D paths and reset (`DX-204`, `DX-227`) | ✅ same shared corpus drives real PSO keys and blend constants, including reset/state-leakage checks (`DX-204`, `DX-227`) | 🟨 `Opaque` and `NonPremultiplied` pixel-verified, the blended result checked against the arithmetic rather than merely "something changed" (`Igl_2D`). The full preset set and custom factors/equations are not audited on this renderer |
| `DepthStencilState` (compare func + full stencil ops) | ✅ | ✅ (Task 870 — real per-pipeline compare-op + stencil) | not separately re-confirmed this pass | ✅ never throws (deliberate no-op, matches FNA's renderer-agnostic-until-drawn model) | ✅ real `D3DRS_STENCILENABLE`/compare-func dispatch, pixel-verified (`DirectX9_DepthStencilState_StencilEnable`) and mutation-verified (forced-false caught, `D9-122`) | ✅ shared corpus proves depth compare/write, stencil enable/functions/three ops, masks, two-sided faces and state reset (`DX-202`, `DX-203`, `DX-227`) | ✅ same full behavior through real PSOs/DSVs, including two-sided face convention and SpriteBatch consumption (`DX-202`, `DX-203`, `DX-210`, `DX-227`) | ✅ depth compare through two overlapping quads at different Z, both backends (`Igl_3D`), and a real stencil stamp-then-test with `StencilFunction`, `StencilPass` and a reference value, where the unstamped half genuinely fails the test (`Igl_Stencil`) |
| `RasterizerState` | ✅ | ✅ — cull modes, fill mode and scissor are covered by `Vulkan_FillMode_WireFrame`, `Vulkan_SpriteBatch_BeginRasterizerState` and `Vulkan_Deferred_Scissor`; `DepthBias` drives real dynamic `vkCmdSetDepthBias` (§7.4). **Previously** "not separately re-confirmed this pass" (`VULKAN-481`, D-05) | not separately re-confirmed this pass | ✅ never throws | ✅ real `D3DRS_CULLMODE` winding-order dispatch, pixel-verified (`DirectX9_RasterizerState_CullMode`) and mutation-verified (forced-none caught, `D9-122`); depth-bias not separately tested — see this doc's own "Remaining genuine D3D9 limitations" section for `D9-62`'s own outstanding depth-bias oracle-proof attempt (investigated 2026-07-16, not closed: no observable effect at any tested magnitude, suspected environment/driver limitation shared with `Vulkan_DepthBias`) | ✅ cull/winding, scissor, constant normalized depth bias and slope-scale bias are pixel-proven; bias converts against the active D16/D24 target (`DX-201`, `DX-206`, `DX-227`, `DX-256`) | ✅ same shared rasterizer corpus through PSO keys, including cull/winding, scissor and both depth-bias terms (`DX-201`, `DX-206`, `DX-227`, `DX-256`) | 🟨 `CullNone` is exercised throughout, and the front-face winding is derived per render target and per backend (IGL-67). Cull-mode and fill-mode outcomes have no dedicated pixel test, and `WireFrame` is OpenGL-only — IGL does not request Vulkan's `fillModeNonSolid` |
| Per-slot `SamplerState` (16 slots) | ✅ | ✅ | ✅ | ✅ (1 real bug fixed) | 🟨 real `D3DSAMP_ADDRESSU`/`V`/`MAGFILTER`/`MINFILTER` wiring, Wrap/Mirror pixel-behavior oracle-verified via SpriteBatch (`D9-92`); not tested across all 16 slots simultaneously | ✅ 16 independent slots plus filter/address behavior; differential mip colours prove `MaxMipLevel`, LOD bias, independent `AddressW` and reset (`DX-142`, `DX-216`, `DX-257`) | ✅ same shared sampler behavior through dynamic descriptor heaps, including all LOD and W-axis fields (`DX-154`, `DX-216`, `DX-257`) | 🟨 two slots verified simultaneously through a real dual-texture draw (`Igl_DualTextureEffect`); the 16-slot fan-out is not audited |
| `GraphicsDevice.ReferenceStencil` | ✅ shared stamp/test fixture passes on EasyGL (`GraphicsDevice_ReferenceStencil`, remeasured 2026-09-09) | ✅ **FIXED** — connected via `vkCmdSetStencilReference`, an undocumented side effect of Task 870 (corrected 2026-07-09) | ❌ **no renderer connection** (Task 872, open) | N/A | ⬜ not attempted — no D3D9 test exercises `SetReferenceStencil()` | ✅ the reference value makes only the stamped half pass through a real DSV (`DX-203`, `DX-227`) | ✅ same discriminating public fixture through `OMSetStencilRef` (`DX-203`, `DX-227`) | ✅ the reference value is what makes the stamped half pass and the unstamped half fail (`Igl_Stencil`) |
| `Clear` honors `ClearOptions::Stencil` | ✅ `GraphicsDevice_ClearStencil` and `ClearOverloads` pass on EasyGL (remeasured 2026-09-09) | ❌ | ❌ | ⚠️ emulated | ⬜ not attempted — `DirectX9_Smoke` covers plain `Clear(r,g,b,a)` only, not the `ClearOptions` combo variants | ✅ shared clear-stencil fixture clears, stamps and tests the DSV; overload combinations are covered (`DX-227`) | ✅ same shared fixture proves stencil clear on public 2D/cube targets through real DSVs (`DX-209`, `DX-227`, `DX-255`) | ✅ `Igl_Stencil` clears target, depth and stencil together, then depends on the cleared stencil before stamping and testing |
...
| Feature | EasyGL | Vulkan | Bgfx | SDL_Renderer | D3D9 | D3D11 | D3D12 | IGL |
|---|---|---|---|---|---|---|---|---|
| Wired to real GPU work (`Begin`/`End`) | ✅ | ✅ **FIXED (Task 447, 2026-07-10)** — real per-draw-call query correlation via `Pending3DDraw::occlusionQuery` tagging + `vkCmdBeginQuery`/`vkCmdEndQuery` recording in `RecordCommandBuffer()` | ✅ (Task 448) | N/A — throws at construction (Task 727) | ⬜ **not built at all** — `plans/plan_dx9.md`'s task list has no `IDirect3DQuery9`-based occlusion-query task; `CreateOcclusionQuery()` falls through to `IGraphicsRenderer`'s own silent `nullptr` default, same gap as D3D12 | ✅ real `ID3D11Query(D3D11_QUERY_OCCLUSION)`, `Begin`/`End`/`GetData` wired (`DX-47`) | ✅ real `D3D12OcclusionQueryRenderer` (`ID3D12QueryHeap`+readback buffer, `DX-120`) — a full-viewport visible triangle reports a real positive `PixelCount()`, the SAME query object reused around off-screen/clipped geometry reports exactly 0, a genuine visible-vs-invisible discrimination, off-screen (Checks AA1–AA4) | ❌ IGL `v1.1.1` exposes no query object, no `IDevice` factory for one and no encoder call to begin or end one, on any backend — re-verified against the pinned source, not restated (IGL-61). `SupportsCapability(OcclusionQuery)` reports false |
| Pixel/query correctness (visible vs. occluded) | ✅ verified both directions (Tasks 445/446) | ✅ verified both directions, plus a multi-draw-span check (Task 854) — this sandbox's software Vulkan driver (Mesa Lavapipe) reports fully accurate, discriminating pixel counts (4096 visible / 0 occluded on a 64×64 quad) | ⚠️ can't verify in this sandbox's software GL2.1 driver; dedicated-view architecture gap open (Task 917) | N/A | N/A — no query support exists | ✅ **stale row, corrected 2026-07-15** — `plans/plan_dx.md` `DX-147` closed this: the same `ID3D11Query` object reused around a genuinely visible (viewport-covering) draw reports a real positive `PixelCount()`, then around a fully off-screen draw reports exactly 0, both-directions discriminating (`DirectX11_Smoke` 91→93/93). This row previously described the pre-`DX-147` state | ✅ **stale row, corrected 2026-07-15** — real, and closed earlier than the row above: `DX-120`'s own landing already included this exact both-directions proof (Checks `AA3`/`AA4`, `plans/plan_dx.md` `DX-147`'s own row explicitly credits `DX-120` for closing the D3D12 half) — this row's "no query support exists" contradicted the very next table up (`OcclusionQuery` "wired to real GPU work" row, same file), which already correctly described `D3D12OcclusionQueryRenderer` as real | ❌ same; a query completes immediately with 0 rather than spinning forever |
...
| Content-pipeline loading (`ModelTypeReader`) | 🟨 the custom `.model.json` format is not XNB-compatible, but hierarchy/parent bones, textures/effects, skeleton animation and 32-bit indices are implemented and covered (`DX-231`) | ⬜ not separately tested against this renderer | ✅ all nine public model/content fixtures pass, including `Content.Load<Model>()`, hierarchy, texture, skeleton, animation and 32-bit indices (`DX-231`) | ✅ the same nine fixtures pass through the off-screen public D3D12 device (`DX-231`) |
...
`cna_reference_dump` and `cna_demo_2d` now link under both DirectX renderers. The former static-library
cycle around `Effect::Apply()` is declared explicitly in CMake, and the demo no longer depends on an
undeclared raw SDL include; the previous warning here was obsolete.
...
- **Vulkan**: **superseded. Current baseline (2026-09-06, `plans/plan_vulkan.md` §7.5): the
  dedicated suite `ctest -R '^Vulkan_'` is **254/254** with no skips, and the full `ctest` is
  **9095/9111** with 16 failures, every one of which reproduces under EasyGL and is therefore not
  this renderer's (`VULKAN-475`).** The line below is kept because two of its claims were wrong
  in ways worth seeing rather than deleting. *What it said:* "verified 2026-07-11: `CnaTests`
  4371/4373 (2 hardware skips), `ctest` 126/127 — 1 remaining failure, `Vulkan_DepthBias`."
  *Why it is superseded:* the counts are from a much smaller suite; and `Vulkan_DepthBias` was
  **not** a renderer defect at all — §7.4 measured it as a *test* written against OpenGL's depth
  range, while the renderer drives real dynamic `vkCmdSetDepthBias` from ten pipeline sites.
  *And the device:* those numbers came from **Mesa lavapipe**, a software driver, which the
  original line did not say (`VULKAN-481`, D-07). Both the 5×
...
- **Constant `DepthBias` (Task 767): FIXED, 2026-07-10.** Project-owner decision received: bgfx's
  ...
  **`SlopeScaleDepthBias` remains an intentionally undone gap** (project-owner decision, not
  ...
  `plans/plan_graphics.md` entry). Only Vulkan implements real hardware depth bias (`vkCmdSetDepthBias`,
...
- ~~`PreferPerPixelLighting` and real specular highlights are structurally unreachable~~ — **fixed
  2026-07-16** (`plans/plan_graphics.md` Phase 80, Task 1101): `GpuDrawParams::preferPerPixelLighting`/
