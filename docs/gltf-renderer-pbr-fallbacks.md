...
| Diligent | binds it | refuses | **refuses** (`GLTF-465`): its runtime-substituted HLSL is tractable; the renderer is now buildable here, so this is the next one to implement |
...
| SDL GPU | no layout | refuses | **refuses** (`GLTF-465`): needs precompiled SPIR-V variants |
...
**How far each "applies" is verified, precisely.** Four of the nine are proven at the pixel level,
...
product. **OpenGL 2**, **OpenGL 4**, **DirectX 12** and **Magnum** are compiled and source-verified but have no L7
...
mind before treating this table as nine equal rows.
