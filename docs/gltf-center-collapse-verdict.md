...
| Generated fixtures today | **24** — `GLTF-021`/`GLTF-023` added the first container and robustness fixtures, and `GLTF-267` added §11.4's `xf-scale-nonuniform` |
| Planned corpus (`plan_gltf.md` §24.2) | **135** distinct assets — completed by `GLTF-399` |
| L5 goldens | 17 of 21 at the time of writing; **22 of 24 today** — the four point/line topologies gained theirs with `GLTF-073`/`GLTF-078`, and the two remaining assets are the rejection fixtures, which have no L5 by construction |
