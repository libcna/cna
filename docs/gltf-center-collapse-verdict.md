...
| Generated fixtures today | **25** — `GLTF-021`/`GLTF-023` added the first container and robustness fixtures, `GLTF-267` added §11.4's `xf-scale-nonuniform`, and `GLTF-137` added `skin-plus-static-mesh` |
...
| L5 goldens | 17 of 21 at the time of writing; **23 of 25 today** — the four point/line topologies gained theirs with `GLTF-073`/`GLTF-078`, and the two remaining assets are the rejection fixtures, which have no L5 by construction |
