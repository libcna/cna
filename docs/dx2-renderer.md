# DX2 (real DirectDraw v1 + Direct3D v2 `DrawPrimitive`) Renderer — Completeness Status

`DX2` is CNA's first legacy-DirectX renderer with a **real, working 3D pipeline** — unlike `DX1`
...
**Status legend** (matches `docs/dx1-renderer.md`'s own convention)
...
|---|---|---|
| `CNA_GRAPHICS_RENDERER=DX2` CMake selection, Windows-only gate, MinGW cross-compile | ✅ | Same `FATAL_ERROR` gate `D3D9`/`D3D11`/`D3D12`/`DX1` already share. |
...
Identical to `docs/dx1-renderer.md` §3/§4 — `CompositeQuad`, all 4 blend-mode formulas,
...
| Feature | Status | Notes |
|---|---|---|
| `Dx2VertexBufferRenderer`/`Dx2IndexBufferRenderer` (16- and 32-bit) | ✅ | Plain CPU-side `std::vector<uint8_t>` storage, matching the `Software` renderer's own identical approach — the CPU transform pipeline reads directly from these buffers each draw. `CreateIndexBuffer32` explicitly overridden with real 32-bit storage (not the shared default's delegate-to-16-bit fallback). `Dx2_VertexIndexBuffer` CTest (5/5). |
| CPU transform + near-plane clip | ✅ | `Dx2ClipVertex`/`Dx2ClipTriangleNearPlane`/`Dx2BuildPositionColorClipVertex`/`Dx2BuildGenericClipVertex`, ported from `SoftwareRenderer.cpp`'s own math, simplified (envMap/skinning out of scope, see §7; lighting is real for stride 32/52 as of Phase O9, see §6a). `Dx2_Clipping` CTest (2/2): a straddling triangle clips to a visible partial fragment, a fully-behind-camera triangle renders nothing. |
...
| `DrawPrimitivesEx`/`DrawIndexedPrimitivesEx` | ✅ | Stride-dispatched vertex layouts (16/20/24/32/52 bytes, matching `Software`'s own set); real texture0 modulation. Honors `GpuDrawParams::vertexStart`/`startIndex`/`baseVertex` (a real gap found in the `Software` reference renderer — it doesn't honor these at all, `EasyGL` does — DX2 matches `EasyGL`). |
...
| Feature | Status | Notes |
|---|---|---|
| `CreateOcclusionQuery` | ⚪-degrades-to-nullptr | Occlusion queries are DX9-only. Never overridden — the shared `IGraphicsRenderer` default applies directly, same as `DX1`/`DX3`. |
...
A CNA game built with `CNA_GRAPHICS_RENDERER=DX2` and MinGW-cross-compiled: creates a real window,
...
`docs/directx-legacy-renderers-analysis.md`'s original analysis-level "~15%" estimate for DX2/3
...
2. `gtest_discover_tests` never set a `WORKING_DIRECTORY`, defaulting to the build directory
   ...
   renderer run this way, not a DX2 regression**.
...
**Final 19 failures, categorized against `DX1-88`'s own precedent** (`docs/dx1-renderer.md` §7a):
...
`CNA_RENDERER_DX2`, now passes) and its 3 already-documented ungated sibling failures above
...
- `plan_dxold.md` — the roadmap this renderer is row 2 of (DX1/2/3/5/6/7/8/10).
- `docs/dx1-renderer.md` — the 2D architecture this renderer ports verbatim wherever the 3D layer
...
- `docs/directx-legacy-renderers-analysis.md` — the feasibility analysis that authorized this whole
  renderer family; §3.1's DX2/3 capability estimate is superseded by this renderer's actual,
  empirically-verified result.
- `scripts/run-wine-dx2.sh`, `scripts/check-dx2-execute-buffer-discipline.sh` — this renderer's own
