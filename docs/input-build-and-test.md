...
git -C .. clone <easy-gl-url> easy-gl        # only needed for -DCNA_GRAPHICS_BACKEND=OPENGLES
...
cmake -S . -B cmake-build-input-easygl -G Ninja -DCNA_GRAPHICS_BACKEND=OPENGLES -DCNA_BUILD_TESTS=ON
...
git submodule update --init --recursive
cmake -S . -B build -G Ninja -DCNA_GRAPHICS_BACKEND=OPENGLES -DCNA_BUILD_TESTS=ON
...
| CMake configure fails: `the OPENGLES backend requires … 'easy-gl'` | `easy-gl` sibling missing | clone it next to `cna_input`, or use `-DCNA_GRAPHICS_BACKEND=VULKAN` (or BGFX / SDL_RENDERER) |
