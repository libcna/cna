...
| `SdlGpu` | ✅ measured | `plans/plan_sdlgpu_modern_graphics.md`: **931 / 0 / 31** of the 962 engine-layer cases on SDL 3.5's Vulkan backend (RADV). Checked-in SPIR-V `ShaderEffect`s, compute and storage resources, indirect draws, instancing through custom effects, and directional/cascade/point/spot shadow generation and reception (the stock lit, skinned and PBR shaders compile the Vulkan renderer's own `shadow_sampling.glsl`). Since `plans/plan_street_sdlgpu.md`: image-based lighting (`STREETS-0002`, `CNAEXT_ImageBasedLighting` 8/8) and every stock family on the hardware-instancing route, lit and skinned included (`STREETS-0001`). Not implemented: `Texture2DArray`. Unavailable in SDL_gpu itself: GPU timers and occlusion queries (no query API). Every remaining skip is inline GLSL ES the SPIR-V intake cannot take, a timer, or a refusal path for a renderer without a capability this one has. Needs `libshaderc-dev` to build. |
