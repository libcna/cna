...
| `RenderPipeline` + post-process passes | ✅ | 🟨 portable tonemap/deband, chromatic aberration, FXAA and film grain run; remaining source-only passes copy through | ⬜ | The passes need `GraphicsCapability::CustomEffects`; without it each copies its input and the frame still renders |
...
main HDR tonemap/deband step. The remaining post-process paths still provide source alone and
remain unavailable on Vulkan. Portable packages use the language/stage query to select a payload;
