...
|---|---|---|---|---|
| Legacy post-process effects (`DepthEffect`, `CRTEffect`) | ✅ GLSL | 🟨 `CRTEffect` selects a portable GLSL/SPIR-V package; `DepthEffect` remains GLSL-only | ⬜ | `AsciiPostProcessEffect` is CPU-side and runs everywhere |
...
| `RenderPipeline` + post-process passes | ✅ | 🟨 eighteen built-in portable pass consumers plus portable `CRTEffect` through `EffectPass` now run, including aerial perspective, volumetric fog, contact shadows, SSR, SSAO, Bloom, colour grading, depth of field, motion blur, analytic height fog, light shafts, tonemap/deband and texture/file HDR display encoding; remaining source-only passes copy through | ⬜ | The passes need `GraphicsCapability::CustomEffects`; without it each copies its input and the frame still renders |
