...
| `RenderPipeline` + post-process passes | ✅ | 🟨 portable chromatic aberration, FXAA and film grain run; remaining source-only passes copy through | ⬜ | The passes need `GraphicsCapability::CustomEffects`; without it each copies its input and the frame still renders |
...
with chromatic aberration, and `MOD-2218`/`MOD-2219` add FXAA and film grain. The remaining
