...
generation and stock-effect reception, a portable skybox, fourteen portable post-process consumers,
...
`MOD-2239k` adds the SSAO estimate plus blur/composite, and `MOD-2239l` adds the shared rigid/skinned
depth/normal/velocity producer. The remaining
...
| `Vulkan` | 🟨 measured | Verified on RADV and llvmpipe: exact float/HDR 2D and cube targets, instancing, stock-PBR IBL, portable skybox, fourteen portable post-process consumers plus their depth/normal/velocity prepass producer, directional/cascade/point/spot shadow generation and reception, compute/storage resources, indirect draws and GPU timers work. Remaining source-authored post-process paths are unavailable because Vulkan consumes packaged SPIR-V rather than this layer's GLSL; presentation remains sRGB-only. |
