...
| `SdlGpu` | ✅ measured | `plans/plan_sdlgpu_modern_graphics.md`: **931 / 0 / 31** of the 962 engine-layer cases on SDL 3.5's Vulkan backend (RADV). Checked-in SPIR-V `ShaderEffect`s, compute and storage resources, indirect draws, instancing through custom effects, and directional/cascade/point/spot shadow generation and reception (the stock lit, skinned and PBR shaders compile the Vulkan renderer's own `shadow_sampling.glsl`). Not implemented: IBL, `Texture2DArray`, lighting on the stock hardware-instancing route. Unavailable in SDL_gpu itself: GPU timers and occlusion queries (no query API). Every remaining skip is inline GLSL ES the SPIR-V intake cannot take, a timer, or a refusal path for a renderer without a capability this one has. Needs `libshaderc-dev` to build. |
