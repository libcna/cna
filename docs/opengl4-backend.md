...
- Dependencies: the platform's own GL library (`find_package(OpenGL REQUIRED)` —
  ...
  1.1 are resolved by the backend's own small hand-rolled loader (`GL4Loader.hpp/.cpp`) — zero
...
- **Multi-stream vertex input** (`GraphicsCapability::MultiStreamVertexInput` = `false`):
  ...
  throws `System::NotSupportedException` if a multi-stream draw reaches the backend directly.
...
- `ApplyMultiSampleCount` after construction is not overridden (inherited no-op) — MSAA is fixed
  at backend construction, EasyGL's own documented limitation.
...
corpus under `CNA_GRAPHICS_BACKEND=OPENGL4` and an ASan/UBSan run of the representative backend
