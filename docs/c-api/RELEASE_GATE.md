...
- ❌ **No public C++ symbol is unaccounted for** — 35 public symbols are still unmapped.
...
| ✅ | **No unreviewed ABI break** | Every struct layout, field offset, scalar width, constant value and exported symbol is recorded, and a change to any of them arrives as a reviewable diff. | 172 struct layouts and 2796 exported symbols recorded |
| ❌ | **No public C++ symbol is unaccounted for** | The generated inventory has no `planned` row, and every partially mapped row carries an owner-approved disposition: every public declaration is mapped, partially mapped with its subset named and its limitation approved, or recorded as having no C form with the reason. | 35 public symbols are still unmapped |
