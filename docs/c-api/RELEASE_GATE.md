...
| ✅ | **No unreviewed ABI break** | Every struct layout, field offset, scalar width, constant value and exported symbol is recorded, and a change to any of them arrives as a reviewable diff. | 166 struct layouts and 2721 exported symbols recorded |
| ✅ | **No public C++ symbol is unaccounted for** | The generated inventory has no `planned` row: every public declaration is mapped, partially mapped with its subset named, or recorded as having no C form with the reason. | 6072 implemented, 23 partial, 0 planned |
| ✅ | **A known-limitations matrix a reader can act on** | What the ABI does not do is published, grouped by reason, with no unclassified reason and no deferral owned by a finished task. | docs/c-api/LIMITATIONS.md is current: 23 partially mapped symbols in 10 groups, 320 unmapped in 64 groups |
