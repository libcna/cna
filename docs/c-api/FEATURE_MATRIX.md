...
| Effect metadata and parameters | Fixed parameter class/type and value/texture-overload identities; own immutable annotations; own mutable parameters and stable collection aliases; all scalar/array/matrix-transpose/string/texture overloads; nested element/member/annotation views; count/index/name/semantic lookup | Boolean/Int32 annotation inputs use native four-byte bit storage; parameter arrays use atomic caller buffers; assigned texture handles are retained per overload slot; techniques, passes and effects remain planned |
