...
| Effects, metadata, parameters, techniques and passes | Own base-adapter, EffectMaterial, ShaderEffect and SpriteEffect game children; same-type clone/dispose/apply; current parameter/technique/pass views; exact type/source strings; shader validity, uniforms, textures and world/view/projection matrices; fixed parameter identities and complete metadata/value/collection operations | Compiled XNA `.fx` bytecode is explicit `NOT_SUPPORTED`; custom-shader renderer availability is queried; renderer pointers stay private; descendants retain the native effect/game/texture lifetime after parent-handle destruction; remaining stock effects are planned |
