...
| Effect metadata, parameters, techniques and passes | Fixed parameter class/type and value/texture identities; own annotations/parameters/techniques/passes and stable collection aliases; all parameter scalar/array/matrix-transpose/string/texture overloads; nested views; technique identities and canonical pass Apply; count/index/name/semantic lookup | Parameter arrays use atomic caller buffers; assigned textures are retained per overload slot; ownerless passes Apply as the native no-op, while future effect-owned views use the same current-technique-validating dispatch; effect lifecycles remain planned |
...
| Effect techniques, passes and collections | Default/named construction, canonical P0, identities, Apply dispatch, nesting, stable aliases and thread/error paths tested | Same renderer-independent strict-C suite tested | Ownerless Apply is a native no-op; effect-owned validation/output evidence arrives with effect lifecycle coverage |
...
- remaining 3D resources and draws, consumption of the completed vertex/index-buffer and binding
  values, models, meshes, effects and shaders;
