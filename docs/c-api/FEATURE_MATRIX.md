...
| Effect metadata | Fixed parameter class/type identities; own immutable annotations from copied metadata/raw storage; all scalar/vector/matrix/string getters; own copied annotation collections with count/index/name lookup | Boolean/Int32 annotation inputs use native four-byte bit storage; collection lookups return independent owned copies; parameters, techniques, passes and effects remain planned |
