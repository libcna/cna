...
| Values and 3D identities | ABI layouts for Color, Point, Vector2/3/4, Quaternion, Matrix, Plane, Ray, bounding volumes, all 17 PackedVector raw values and VertexElement; all MathHelper constants/scalar operations; complete Point/Rectangle, Vector2/3/4, Quaternion, Matrix, Plane and Ray construction/property/math/operator/hash/string operations plus transforms/factories/decomposition and explicit optional intersections; stable containment, plane, curve, buffer, index, primitive, SetData and vertex identities | Bounding-volume operations and other value families still expose layouts/identities only; their constructors, math/geometry/packing operations and resource behavior remain planned |
