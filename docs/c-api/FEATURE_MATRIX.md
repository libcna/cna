...
| Values and 3D identities | ABI layouts for Color, Point, Vector2/3/4, Quaternion, Matrix, Plane, Ray, bounding volumes, all 17 PackedVector raw values and VertexElement; all MathHelper constants/scalar operations; complete Point/Rectangle, Vector2/3/4 and Quaternion construction/property/math/operator/hash/string operations plus vector matrix/quaternion/normal bulk transforms and quaternion rotation factories/interpolation; stable containment, plane, curve, buffer, index, primitive, SetData and vertex identities | Other value families still expose layouts/identities only; their constructors, constants, math/geometry/packing operations and resource behavior remain planned |
