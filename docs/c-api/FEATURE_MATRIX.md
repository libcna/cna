...
| Model mesh parts | Own default/parameterized parts; exact signed geometry state; optional retained Effect, VertexBuffer and IndexBuffer assignments; opaque 64-bit C tag; retained snapshot collections with count/index aliases | Creation-thread affine; assigned resources must share one graphics device and cannot be disposed/destroyed while retained; top-level Model aggregation remains planned |
