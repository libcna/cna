...
| `KHR_materials_specular` | **IMPLEMENTED_WITH_A_NAMED_LIMIT** | no | Factor and colour are consumed by all 15 PBR renderers. Both optional texture inputs survive direct import and offline `.cnj`, including independent UV, transform, sampler and colour-space state; EasyGL, OpenGL2 and OpenGL4 sample them, while 12 PBR renderer bindings remain pending. Required use therefore remains refused and optional use is warned by name. | `GLTF-344` |
...
transform and sampler; the colour map also retains its sRGB declaration. EasyGL, OpenGL2 and
OpenGL4 apply the extension's multiply-before-clamp Fresnel equation. Validation continues
