...
| `KHR_materials_ior` | **PARSED_BUT_IGNORED** | no | The factor reaches PBR effect state and shader-ready dielectric F0/F90; EasyGL consumes both endpoints, but other PBR renderers do not yet. | `GLTF-343` |
| `KHR_materials_specular` | **PARSED_BUT_IGNORED** | no | Factor and colour reach PBR effect state and EasyGL's rigid/skinned shaders. The two optional textures are not imported and other PBR renderers do not consume the endpoints yet. | `GLTF-344` |
...
| A sampler whose input times need resampling to a shared track | Resampled, with the track count and the duplicate-input-time count recorded. Equal adjacent times are kept (an exporter writes them for a hard cut); a **decreasing** time is refused outright, because it has two authored values for one time. | `resampledTrackCount`, `duplicateInputTimeCount` | `GLTF-310`, `GLTF-313` |
...
| A material map selecting a different `TEXCOORD` set than the base colour's | Sampled with the base colour's coordinates, because both PBR effects read one shared UV channel. The maps that ended up on the wrong set are listed by name. | `uvSetMismatchedMapsEXT` | `GLTF-181`, `GLTF-188` |
| A second `KHR_texture_transform` on another map | One transform can be baked into the one UV channel, and CNA bakes the base colour's. Any other map declaring a *different* transform is named. | `unbakedTextureTransformsEXT` | `GLTF-184`, `GLTF-336` |
| `KHR_materials_transmission` above 0 | `alpha = 1 - transmissionFactor`, multiplied into the material's own alpha, with `alphaMode` forced to `Blend`. Explicitly not physical: no refraction, no roughness-driven blur, no thickness, and no view-dependent Fresnel. | `transmissionApproximatedEXT`, `transmissionFactorEXT`, `transmissionHasTextureEXT` | `GLTF-339` |
...
| An authored `TANGENT` on a non-PBR layout | Only strides 48 and 68 carry a tangent, and those are the PBR layouts. | `droppedTangentForStrideEXT` | `GLTF-086` |
...
| A second UV set (`TEXCOORD_1`) | No stride carries two UV channels; see §2's `uvSetMismatchedMapsEXT` row for what happens to a map that selects it. | `uvSetMismatchedMapsEXT` | `GLTF-181` |
...
| `KHR_materials_ior.ior` | `PbrEffect::getIorEXTProperty()` and `GpuDrawParams::pbrDielectricF0` | EasyGL consumes it; other PBR renderers still use their core dielectric F0. | `GLTF-343` |
