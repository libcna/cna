...
| `KHR_materials_specular` | **IMPLEMENTED_WITH_A_NAMED_LIMIT** | no | Factor and colour are consumed by all 15 PBR renderers. Both optional texture inputs survive direct import and offline `.cnj`, including independent UV, transform, sampler and colour-space state; EasyGL, OpenGL2, OpenGL4 and DirectX9/11/12 sample them, while 9 PBR renderer bindings remain pending. Required use therefore remains refused and optional use is warned by name. | `GLTF-344` |
...
OpenGL4 and DirectX9/11/12 apply the extension's multiply-before-clamp Fresnel equation.
