...
| A different `KHR_texture_transform` on another map | CNA currently bakes one reference transform into packed coordinates and carries no per-map transform matrices. Any map declaring a *different* transform is named even when its UV set is carried correctly. | `unbakedTextureTransformsEXT` | `GLTF-184`, `GLTF-336` |
