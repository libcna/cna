...
| `ITextureBackend::UpdatePixels/2` | Replaces level zero with a GL upload. | Rebuilds both alpha-labelled CPU images and regenerates only dirty unauthored descendants. | `implemented` | SKIA-22–24, SKIA-128; `Skia_Texture2D_MipGeneration` |
| `ITextureBackend::UpdatePixelsLevel/4` | Uploads a requested GL mip level. | Textures upload every valid level into stable checked CNA storage with authored barriers/generation; targets upload the addressed isolated surface and exact shadow without aliasing. Invalid levels/dimensions reject before mutation. | `implemented` | SKIA-27, SKIA-125–131; `Skia_Texture2D_MipGeneration`, `Skia_RenderTarget2D_MipStorage` |
| `ITextureBackend::HasDefinedMipLevel/1` | EasyGL has no separate initialized-level query; caller-authored CPU shadows define ordinary mip content. | Reports every allocated CNA texture or target mip as deterministic readable storage, allowing backend-defined levels without a duplicate public shadow. | `internal` | SKIA-128, SKIA-131; `Skia_Texture2D_MipGeneration`, `Skia_RenderTarget2D_MipStorage` |
...
| `ITextureBackend::GetData/7` | Reads GL target pixels when no CPU shadow exists. | Exact full/partial readback for every Texture2D and RenderTarget2D mip; rendered target level zero synchronizes into its canonical shadow at pass boundaries. | `implemented` | SKIA-23–24, SKIA-62, SKIA-127, SKIA-131; `Skia_Texture2D_MipTransfer`, `Skia_RenderTarget2D_MipStorage` |
...
| `IGraphicsBackend::CreateTexture/1` | Allocates an EasyGL 2D texture. | Allocates the complete checked zeroed 2D mip chain and level-zero alpha-labelled Skia images; every level transfers, generates, and samples through bounded zero-copy raster views. | `bounded` | SKIA-22–30, SKIA-125–129 |
...
| `IGraphicsBackend::CreateRenderTarget2D/6` | Allocates GL FBO with requested attachments. | Color targets own a bindable level-zero surface; mip requests add complete stable per-level surfaces and exact shadows. Depth and MSAA remain bounded refusals. | `bounded` | SKIA-61–79, SKIA-131; `Skia_RenderTarget2D_MipStorage` |
...
| `IGraphicsBackend::ApplySamplerState/5` | Applies per-slot GL filter/address/anisotropy. | Sprite Texture2D slot implements all min/mag/mip combinations and addressing; Anisotropic is a complete-Linear fallback, not an advertised device feature. | `bounded` | SKIA-43–46, SKIA-78–79, SKIA-129 |
