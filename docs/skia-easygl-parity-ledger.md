...
| `ITextureBackend::UpdatePixelsLevel/4` | Uploads a requested GL mip level. | Uploads every valid level into stable checked CNA mip storage, promotes it to an ownership barrier, and regenerates only following unauthored descendants; invalid levels/dimensions reject before mutation. | `implemented` | SKIA-27, SKIA-125–128; `Skia_Texture2D_MipTransfer`, `Skia_Texture2D_MipGeneration` |
| `ITextureBackend::HasDefinedMipLevel/1` | EasyGL has no separate initialized-level query; caller-authored CPU shadows define ordinary mip content. | Reports every allocated CNA mip as deterministic readable storage, allowing generated levels without a duplicate public shadow. | `internal` | SKIA-128; `Skia_Texture2D_MipGeneration` |
