...
Current as of the 21-lane reconciliation on 2026-08-09 plus the `SVG_DOM` addition. CNA exposes
exactly **42 public renderer identities**. EasyGL is an internal implementation shared by four
public GL profiles and does not add a public identity. Internal renderer/API choices made by bgfx,
Skia, Sokol, Diligent, LLGL, or another abstraction likewise do not add CNA identities.
...
| 42 | `SvgDom` | `SVG_DOM` | `CNA_RENDERER_SVG_DOM` | SVG DOM / `SvgDomRenderer` | Emscripten |
...
- **No renderer:** `STUB` is a no-op; `HEADLESS` is validation/trace-oriented and makes no pixel
  fidelity claim.
- **2D-oriented:** `SDL_RENDERER`, `CANVAS`, `HTML_DOM`, `SVG_DOM`, `SKIA`, `ASCII`, `FREEDIRECT`,
