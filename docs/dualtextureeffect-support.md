...
**Task 383 found and fixed a real bug on both renderers.** FNA's `DualTextureEffect.fx`
...
CNA's EasyGL and Vulkan dual-texture fragment shaders all computed
...
`base.rgb *= 2.0` to both renderers' dual-texture fragment shaders. Fixing it also required
...
compensated with `DiffuseColor=0.5`.
...
`FillGpuDrawParams()`'s output, plus real `BlendState.AlphaBlend` pixel tests on EasyGL
...
SOFTWARE-302 closes both slots for the Software/EasyGL parity target; the other renderer
...
C++ pattern). **At the time, not fixed here** (tracked as Task 888): Vulkan had zero fog
GPU implementation for any 3D effect. **Since fixed by Task 899** (closed 2026-07-07) — real fog is
now implemented across every 3D shader on Vulkan, including `DualTextureEffect`'s.
...
opened Task 889** (`VertexColorEnabled` is a total no-op on both renderers — each renderer's
...
|---|---|---|
| Property defaults (9/9) | ✅ Task 381/382 | ✅ (shared C++) |
| `color.rgb *= 2` doubling factor | ✅ fixed Task 383 | ✅ fixed Task 383 |
| Basic two-texture multiply (magenta×yellow) | ✅ Task 133/384 | ✅ Task 135/384 |
| `Alpha` premultiplication | ✅ Task 385 | ✅ Task 385 (alpha-channel-only) |
| First texture (`Texture`) null-fallback | ✅ Task 386 | ✅ Task 386 |
| Second texture (`Texture2`) null-fallback | ✅ Task 387 | ✅ Task 387 |
| Fog (`FogEnabled`/`FogColor`/`FogStart`/`FogEnd`) | ✅ fixed Task 388 | ✅ fixed Task 899 (2026-07-07) |
| Independent `TextureCoordinate0` / `TextureCoordinate1` | ✅ fixed SAMPLE-073 | not audited here |
| `VertexColorEnabled` | ❌ Task 889 | ❌ Task 889 |
| Cross-renderer pixel consistency | ✅ Task 389 | ✅ Task 389 |
...
- **Task 887** (opened by Task 377, `AlphaTestEffect`) — Vulkan alpha-test vertex-color
...
- ~~**Task 888**~~ (opened by Task 378, `AlphaTestEffect`) — **fixed by Task 899** (closed
  2026-07-07): real fog now implemented on Vulkan, project-wide, for every 3D effect
  including `DualTextureEffect`.
- **Task 889** (opened by Task 389) — `DualTextureEffect.VertexColorEnabled` is a total no-op on
  both renderers. Needs a new vertex-color-carrying stride variant and `vin.Color` multiplication
  into the forwarded diffuse on each renderer's dual-texture shader (mirroring FNA's
  `VSDualTextureVc`/`VSDualTextureVcNoFog` shader variants) — a genuinely new multi-shader-file
  feature addition, not fixed here.
