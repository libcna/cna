...
`ID3D11DeviceContext` fairly directly, going through `D3D11Renderer::GetDeviceEXT()` (a
...
```cpp
// 1. Create (or reuse) a real device via D3D11Renderer::GetDeviceEXT(), or construct one
...
- **`cna_reference_dump`'s `undefined reference to Effect::Apply()` link failure is fixed** (found
  ...
  dependency — `D3D11SpriteBatch.cpp` (in `cna_renderer_d3d11`) calls back into
  `Effect::Apply()` (defined in `CNA` itself), and MinGW's single-pass archive resolution never
  revisited `libCNA.a` once `libcna_renderer_d3d11.a` created the need. Fixed by declaring
