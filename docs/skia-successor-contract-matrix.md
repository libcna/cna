...
| `CAP-MultiStreamVertexInput` | REMED-GFX-201's several-bindings-per-input-rate contract. | refused | SKIA-170 | Postdates the baseline enum; there is no vertex-stream pipeline here to split, so the answer is a structural false rather than a deferred feature. |
| `CAP-Instancing` | REMED-GFX-202's hardware instancing contract. | refused | SKIA-170 | Postdates the baseline enum; the instanced route refuses through `Ensure3DSupported()` and remains inside the rejected 3D geometry scope. |
...
| `EFFECT-VERTEX` | Custom 2D vertex attributes, transform and varyings. | refused | SKIA-153–154 | `SkMeshSpecification` proved non-functional on raster Skia (SKIA-153); its `SkVertices` replacement has a fixed, non-programmable attribute set (position/texCoord/colour only), so custom vertex attributes/varyings remain refused by API limitation, not pending proof. |
| `EFFECT-FRAGMENT` | Single-output programmable fragment color. | bounded | SKIA-154–158 | Explicit SkSL v1 mesh ABI (`CNA_SKIA_SKSL_MESH_V1`) works end-to-end through the public API (SKIA-157); the restricted GLSL-to-SkSL translator (SKIA-155) also reaches it for its one proven formula (`dual_textured`). Broader ABI coverage (arbitrary GLSL, more formulas, helper functions) remains open. |
