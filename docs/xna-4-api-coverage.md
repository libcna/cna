...
| **Not yet implemented / open decision** | RPC-only release timing (`maxRpcReleaseTime`, no authored `fadeOutMS`) is unimplemented — RPC volume/pitch are now continuously re-evaluated (`P9-XACT-016`), but this specifically needs the built-in `"ReleaseTime"` envelope variable (elapsed-release-time tracking), which CNA doesn't have (`P9-STOP-010`) • the `"AttackTime"`/`"ReleaseTime"` built-in RPC variables generally, and RPCs targeting filter frequency/Q or a DSP preset, remain unevaluated (no elapsed-playback-time tracking, no DSP preset system) |
