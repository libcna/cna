...
- **Status:** Implemented, with a small set of accepted deviations documented in `CHECKLIST.md`
  ...
  not enforced) — none of these are "unimplemented," they are deliberate, reasoned SDL3_mixer-
  backend trade-offs. `Cue::IsPlaying`/`IsPaused` mutual exclusivity was one such deviation but is
  now fixed (`P9-LIFECYCLE-013`, resolved) — pausing no longer clears `IsPlaying`, matching real
  FACT's independent bits.
...
| **Not yet implemented / open decision** | XACT category `instanceLimit`/`fadeInMS`/`fadeOutMS` are parsed but never enforced • RPC-only release timing (`maxRpcReleaseTime`, no authored `fadeOutMS`) is unimplemented, tied to the RPC-evaluated-once-not-continuously deviation above (`P9-STOP-010`) |
