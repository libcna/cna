...
- **Status:** Implemented, with a small set of accepted deviations documented in `CHECKLIST.md`
  ...
  `REPLACE_QUIETEST` category-instance-limit behaviors all collapsing to "evict oldest," cue-level
  `instanceLimit` unenforced) — none of these are "unimplemented," they are deliberate, reasoned
  SDL3_mixer-backend trade-offs (or, for the `maxInstanceBehavior` collapse, matching FAudio's own
  acknowledged shipped behavior exactly). `Cue::IsPlaying`/`IsPaused` mutual exclusivity was one
  such deviation but is now fixed (`P9-LIFECYCLE-013`, resolved) — pausing no longer clears
  `IsPlaying`, matching real FACT's independent bits. XACT category `instanceLimit`/
  `maxInstanceBehavior`/fade in/out are now real and enforced (`P9-CATEGORY-005..010`, resolved).
...
| `Audio (XACT)` — AudioEngine/SoundBank/WaveBank/Cue | ~95 % | Real `.xgs`/`.xsb`/`.xwb` parser + SDL3_mixer playback; category/lifecycle/3D/instance-limit+fade all real; gaps are documented accepted deviations (no HRTF/elevation, cue-level instanceLimit unenforced), not missing implementation |
