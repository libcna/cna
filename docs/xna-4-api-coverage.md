...
| **Not yet implemented / open decision** | XACT category `instanceLimit`/`fadeInMS`/`fadeOutMS` are parsed but never enforced • `Cue::Stop(AsAuthored)`'s release-tail duration is the wave's own remaining length, not an authored `fadeOutMS`/RPC-release curve (`P9-STOP-010`, decision on whether to fix pending) |
