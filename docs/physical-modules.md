...
| modules/graphics | base | `cna_graphics_core` (`CNA::GraphicsCore`) | math, core; private: input (cycle), selected backend (factory edge) | Core.Base, IO, Collections.Core, Text |
...
- **`CNA`** — the historical full-framework INTERFACE umbrella: all modules + the selected
  backend + the shared build flags. Existing `target_link_libraries(game CNA)` consumers keep
...
pinned by `scripts/check_renderer_identities.py` over `CNA/GraphicsBackendType.hpp` (core
module) + `cmake/BackendSelection.cmake`; family directories are implementation structure,
not identities. Exactly one family's `${BACKEND_TARGET}` is configured per build
(`BACKEND_DIR = modules/renderers/<family>`); each family's `CMakeLists.txt` owns its
...
- **modules/renderers/common/d3d** — `cna_backend_graphics_d3dcommon`, consumed by the d3d11
...
- **sdl-renderer as ASCII's core** — under `CNA_GRAPHICS_BACKEND=ASCII` the sdl-renderer
  module builds its sources as `cna_backend_graphics_sdl_renderer_core` (the accepted
  decorator design); under SDL_RENDERER it builds the backend itself.
...
- **d3d9 effect sub-target** — `cna_backend_graphics_d3d9_effect` (the isolated
...
- **metal + glide header interfaces** — `cna_renderer_metal_headers` /
  ...
  suites deliberately compile into the CnaTests corpus on every backend, so their
  `CNA/Internal/Backends/<X>` policy headers stay reachable in every configuration.
...
- **graphics ↔ selected renderer** — the factory edge plus the unconditional reverse edges
  (`${BACKEND_TARGET}` → graphics/core/math), declared by
  `cna_renderer_backend_common_setup()` in `modules/renderers/CMakeLists.txt`.
...
- `modularization/tools/check_include_reachability.py` — every `#include "CNA/..."`
  ...
  the declared module graph; config-gated backend includes are attributed to their backend.
