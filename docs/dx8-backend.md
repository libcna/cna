# DX8 (real `IDirect3D8`/`IDirect3DDevice8`, DXVK-delivered, fixed-function) Backend — Completeness Status

`DX8` is architecturally very different from every backend in this family so far (`DX1`..`DX7`, all
DirectDraw-based): DirectDraw and Direct3D merge into a single API in DirectX 8, so this backend has
**no DirectDraw at all** — a single `IDirect3D8::CreateDevice` call creates both the device and its
own swap chain, the same device-bring-up shape as this project's own `D3D9GraphicsBackend`. Delivered
...
- **No DirectDraw.** No `IDirectDrawX`/`IDirectDrawSurfaceX` object anywhere in this backend — a
...
- **`D3DTLVERTEX`/`D3DFVF_TLVERTEX` are gone from the real headers.** D3D8 introduced the generic
  ...
  FVF *values* (`D3DFVF_XYZRHW`/`DIFFUSE`/`SPECULAR`/`TEX1`) are unchanged, so this backend's
...
- **2D `SpriteBatch` is a redesign, not a port.** `DX1`..`DX7` all used DirectDraw's `Blt`/`BltFast`
  for a CPU-side pixel compositor. With no DirectDraw at all, `Dx8SpriteBatchBackend` renders real
  ...
  approach, matching the pattern every non-DirectDraw-based CNA backend (D3D9/EasyGL/Vulkan) uses.
...
- **`AnisotropicFiltering` reports `true`** — unlike every prior backend in this family (`DX2`..`DX7`,
...
- **No native mip chain**, same as `DX2`..`DX7`: `Texture2D::SetData(level>0, ...)` throws honestly
  (`Dx8TextureBackend`/`Dx8RenderTargetBackend::UpdatePixelsLevel`) rather than silently discarding
  the upload.
...
1. **A real `Dx8GraphicsBackend::SetVirtualResolution` bug.** The logical render target (texture +
   surface + depth-stencil) is created once at construction using whatever `virtualWidth`/
   `virtualHeight` the backend happened to be constructed with (the SDL window's own initial/default
...
3. **A real D3D8 XYZRHW half-texel alignment bug.** D3D8's pre-transformed (`D3DFVF_XYZRHW`)
   ...
   same classic D3D8/9 convention `D3D9SpriteBatchBackend` compensates for by baking a half-texel
   shift into its own orthographic projection matrix). Since `Dx8SpriteBatchBackend`'s quad is
   already in screen space with no such matrix, the same -0.5 shift is applied directly to the
   final screen-space quad corners instead (`Dx8SpriteBatchBackend::Draw` and `Present()`'s own
...
`SupportsCapability(GraphicsCapability::WireFrame)` reports `true`; unlike every prior backend in
...
Cross-backend regression: `GraphicsBackendCompileDefinitionsTest.ExactlyOneGraphicsBackendIsSelected`,
...
own header comment documents it as only ever building against a fully 3D-capable backend) — zero new
regressions.
...
backend's source — a real, automated proof this backend genuinely has no DirectDraw and uses only
...
shaders/`CreateEffectBackend` (project-owner scope decision), no multitexture, no DXTn compressed
...
- `plan_dx8.md` — this backend's own implementation plan and design-decision record.
- `dx8-spike/README.md` — the full `DX8-0` spike record plus the two further runtime-bug
  investigations (§3 above).
- `docs/dx7-backend.md` — the last DirectDraw-based backend in this family; `DX8` diverges from it
  architecturally rather than porting it.
- `plan_dxold.md` — the roadmap this backend is a row of.
