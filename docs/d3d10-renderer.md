...
`D3D10` is architecturally very different from every renderer in the `DX1`..`DX8` family: Direct3D 10
...
(both already compile real HLSL via `D3DCompile`) than to the `DX1`..`DX8` CPU-transform family.
...
(plan_d3d10.md design decision 3, mirroring `DX1`'s own "baseline first, richness later" precedent
and `DX8`'s own "fixed-function only" scope-bounding) — see §3 below for exactly what's in and out.
...
## 2. Real architectural deltas vs. `DX1`..`DX8`
...
- **Real state OBJECTS, not per-call render states.** `ID3D10BlendState`/`ID3D10DepthStencilState`/
  ...
  equivalent of `DX1`..`DX8`'s `SetRenderState(D3DRS_xxx, value)` at all.
...
- **Real MRT support** (`OMSetRenderTargets` takes an array) — a genuine, real difference from every
  `DX1`..`DX8` renderer (DirectDraw/early-Direct3D has exactly one active render target).
- **Real readback**: `CreateTexture2D(D3D10_USAGE_STAGING)` + `CopyResource` + `Map(D3D10_MAP_READ)`
  — yet another distinct mechanism from `DX1`..`DX7`'s `Blt`, `DX8`'s
...
- **No logical-resolution render target / letterboxing** — unlike `DX8`'s own addition (forced by
  ...
  matches the real window size, matching `D3D11Renderer`'s own simpler, already-established
...
- **`SpriteBatch` is real GPU-quad rendering through a real `vs_4_0`/`ps_4_0` shader** (position +
  ...
  (unlike `DX8`/`D3D9`'s own pre-D3D10 pixel-center convention): D3D10 places texel/pixel centers at
...
- **`SetTransformMatrix` real bug found via CTest**: the public `SpriteBatch::Begin(...)` call order
  ...
  moments earlier. Fixed by removing that reset (matching `Dx8SpriteBatchRenderer::Begin()`'s own
...
- `DrawPrimitivesEx`/`DrawIndexedPrimitivesEx` (**optional**, has a safe default fallback to the
  ...
  without `D3D11Renderer`'s own full 10-stock-shader-variant richness.
...
**10/10 `D3D10`-labeled CTests pass**: `D3D10_LegacyInterfaceDiscipline`, `D3D10_Smoke`,
`D3D10_Device3DSmoke`, `D3D10_SpriteBatch`, `D3D10_GraphicsCapability`,
`D3D10_MultipleRenderTargets`, plus the shared, renderer-agnostic EasyGL-authored
`D3D10_BlendState_Opaque`/`D3D10_BlendState_AlphaBlend`/`D3D10_DepthStencilState_StencilEnable`/
`D3D10_RasterizerState_CullMode` (the same sources `D3D11`/`Vulkan` already reuse verbatim, testing
...
difference from every `DX1`..`DX8` renderer). `OcclusionQuery`/`CustomEffects` report `false` (out
...
The legacy-interface-discipline check (`scripts/check-d3d10-legacy-interface-discipline.sh`) forbids
...
- `docs/dx8-renderer.md` — the last renderer in the `DX1`..`DX8` legacy family; `D3D10` diverges from
