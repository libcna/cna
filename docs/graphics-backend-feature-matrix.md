...
| Non-`Color` surface formats | ⚠️ | Texture2D directly stores/samples `Bgr565`, `Rgba1010102`, and `ColorBgraEXT`; `Bgra4444` retains exact words with a bounded RGBA conversion, while `ColorSrgbEXT` retains encoded bytes and decodes once into linear-sRGB. The other 21 formats, all non-Color targets, and cube/volume non-Color storage remain refused. | [SKIA-25, SKIA-109, SKIA-134–136](../plan_skia.md), `Skia_Texture2D_PackedFormats`, `Skia_Texture2D_ColourFormats`, `Skia_Contract_SurfaceFormat` |
