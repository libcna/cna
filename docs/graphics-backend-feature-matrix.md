...
| Custom `Effect` via `Begin(effect)` | ✅ | ✅ | ✅ | ❌ throws by design (no shader stage, 2D-only backend) | ✅ real color-inversion custom-HLSL pixel test (`DX-71`) | 🟨 `D3D12EffectBackend`'s runtime `D3DCompile()` custom-`ShaderEffect` path is real and proven (`DX-121`) — the `SpriteBatch::Begin(effect)`-specific integration itself is real code but not independently CTest-proven |
