...
| `RenderTarget2D` colour/readback/sampling/usage | ✅ | Direct level-0 `SkSurface`; target switching and lifetime are checked. Mipmapped targets additionally own stable isolated per-level surfaces and exact transfer shadows. | [SKIA-61–75, SKIA-131](../plan_skia.md), `Skia_RenderTarget2D_Golden`, `Skia_RenderTarget2D_MipStorage` |
...
| Mutable `RenderTarget2D` mip chain | ⚠️ | CNA-owned stable per-level surfaces and canonical shadows implement construction, transfer, sampling and isolation. Rendered-level descendant generation remains pending SKIA-132, so the complete EasyGL resolve contract is not yet promoted. | [SKIA-70, SKIA-131–132](../plan_skia.md), `Skia_RenderTarget2D_MipStorage` |
...
| Sampler slots/anisotropy/mip filtering | ⚠️ | Sprite Texture2D sampling implements all nine min/mag/mip combinations plus Clamp/Wrap/Mirror. Anisotropic is an exact complete-Linear fallback with capability false; cube/volume and 3D sampler binding remain unavailable. | [surface-mode ADR](skia-surface-mode-adr.md), [SKIA-43–46, SKIA-70, SKIA-78–79, SKIA-102, SKIA-129](../plan_skia.md), `Skia_MipSampling_Raster`, `Skia_Sampler_MipmapFilterPolicy` |
