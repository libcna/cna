...
| `Texture2D` mip levels above zero | ⚠️ | Public `mipMap=true` construction now owns and reports a complete checked zeroed CNA chain, including odd/NPOT and one-dimensional levels. Higher-level Set/Get remains an explicit SKIA-127 boundary; generation and mip-filter sampling remain SKIA-128/129. | [SKIA-125–126](../plan_skia.md), [resource policy](skia-successor-resource-oracles.md), `Skia_Texture2D_MipConstruction` |
...
| Sampler slots/anisotropy/mip filtering | ⚠️ | Sprite slot point/linear plus Clamp/Wrap/Mirror are exact. Level-zero Anisotropic is an exact tested Linear fallback with capability false; mip filters and 3D sampler binding reject. | [surface-mode ADR](skia-surface-mode-adr.md), [SKIA-43–46, SKIA-70, SKIA-78–79, SKIA-102](../plan_skia.md), `Skia_Sampler_MipmapFilterPolicy` |
