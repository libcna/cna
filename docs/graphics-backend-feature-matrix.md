...
| Non-`Color` surface formats | ⚠️ | Texture2D directly stores/samples `Bgr565` and `Rgba1010102`; `Bgra4444` retains exact words with a bounded RGBA working conversion. The other 23 formats, all non-Color targets, and cube/volume non-Color storage remain refused. | [SKIA-25, SKIA-109, SKIA-134–135](../plan_skia.md), `Skia_Texture2D_PackedFormats`, `Skia_Contract_SurfaceFormat` |
