...
| Unlit `BasicEffect` subset: `VertexColorEnabled`, `DiffuseColor`, `Alpha` (`FragColor = vc * DiffuseColor`, the same formula the other CNA stock unlit programs use) | `PortableGL_EffectState` |
...
| Lit / textured / fogged / alpha-tested `BasicEffect`, `DualTextureEffect`, `EnvironmentMapEffect`, `SkinnedEffect`, `PbrEffect`, `AlphaTestEffect` | Only the unlit `VertexPositionColor` program exists. | `Draw*PrimitivesEx` refuses the configuration before any native draw. `PortableGL_EffectState` check G. |
| Vertex layouts other than `VertexPositionColor` (stride 16) | The colored program declares exactly Position0 `Vector3` @0 and Color0 `Color` @12. | The draw is refused; a **different** declaration that happens to share the 16-byte stride is refused too, through the shared `RequireFaithfulVertexDeclaration` guard, rather than reinterpreted. `PortableGL_Rejection` checks B–C. |
