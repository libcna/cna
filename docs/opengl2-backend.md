...
- **Multi-stream vertex input** — capability `false`, `GraphicsDevice` refusal, plus a
  backend-level `System::NotSupportedException` backstop on all three `Draw*Ex` routes.
- **Instancing without the ARB extensions** — `SupportsCapability(Instancing)` answers from the
  ...
  extensions) is the shared base class's deterministic refusal. `Instancing` is this backend's
  own `GraphicsCapability` member: unlike GL 3.3+/D3D11-class backends, instancing here is an
