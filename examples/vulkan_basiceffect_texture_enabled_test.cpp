...
// TextureColor*DiffuseColor*Alpha, component-wise (Common.fxh's ComputeCommonVSOutput() sets
// vout.Diffuse = DiffuseColor; PSBasicTxNoFog returns
// SAMPLE_TEXTURE(Texture, pin.TexCoord) * pin.Diffuse, with no vertex-color multiply anywhere in
// this shader variant). Uses the same (200,100,50) texture color / (0.8,0.4,0.6) DiffuseColor pair
// as Task 365's vertex-color test, so the correct product (160,40,30) is the same numeric target,
...
class VulkanBasicEffectTextureEnabledTest : public Game
{
...
public:
    VulkanBasicEffectTextureEnabledTest()
    {
...
int main()
{
    VulkanBasicEffectTextureEnabledTest game;
    game.Run();
