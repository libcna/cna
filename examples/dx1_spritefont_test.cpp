...
// plan_dx1.md Phase O6 (DX1-50..DX1-54): SpriteFont / DrawString tests for the DX1 (real
...
// plain sprites. This phase therefore needs no new renderer code (DX1-50/51 confirm this
...
// a color-match tolerance to work around that renderer's own physical/logical scaling), DX1's CPU
// compositor is exact-pixel, so every check here asserts an exact color match.
//
// Check A (DX1-50) -- a single glyph lands at exactly the expected destination rect.
// Check B (DX1-51) -- two glyphs with spacing land at their correctly kerning-advanced positions,
...
// Check D (DX1-53) -- an unresolvable character falls back to `defaultCharacter` when set, and
//   throws std::invalid_argument when not set.
// Check E (DX1-54) -- SpriteEffects::FlipHorizontally with DrawString mirrors the WHOLE glyph
...
class Dx1SpriteFontTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        // Check A (DX1-50): single glyph lands at exactly the expected destination rect.
        {
            ...
            check(ok, "Single glyph 'A' lands at exactly (4,4,8,8) (DX1-50)");
        }

        // Check B (DX1-51): two glyphs with spacing -- 'A' at x=4, 'B' at x=4+8+spacing=14, gap
        // between them (x=12,13) stays background.
        {
            ...
            check(ok, "Two glyphs 'AB' with spacing land correctly, gap stays background (DX1-51)");
        }
        ...
        // Check C (DX1-52): '\n' advances to the next line by exactly lineSpacing (8) pixels.
        {
            ...
            check(ok, "'\\n' advances to the next line by exactly lineSpacing (DX1-52)");
        }

        // Check D (DX1-53): unresolvable character falls back to defaultCharacter when set, and
        // throws when not set.
        {
            ...
            check(fallbackOk && threw,
                  "Unresolvable char uses defaultCharacter when set, throws when not (DX1-53)");
        }

        // Check E (DX1-54): SpriteEffects::FlipHorizontally mirrors the whole glyph sequence
        // (Task 694's shared-code fix) -- 'B' (green) ends up on the LEFT, 'A' (red) on the RIGHT.
        {
            ...
            check(ok, "SpriteEffects::FlipHorizontally mirrors the whole glyph sequence (DX1-54)");
        }
