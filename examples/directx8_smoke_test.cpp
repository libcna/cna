...
// the DIRECTX8 (real DirectDraw v4, run under Wine -- no ../free-direct anywhere in this renderer)
...
// actually happened: DirectX8Renderer's constructor unconditionally throws if
...
//
// Check A -- GetWindowInternal() returns a real, non-null window (DIRECTX8 needs a genuine Win32 HWND,
...
//   (RGB and alpha), read from DIRECTX8's own Lockable shadow-backbuffer surface (design decision 4).
...
using namespace CNA::Internal::Renderers::DirectX8;

static constexpr int kCanvasSize = 64;

class DirectX8SmokeTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        auto& renderer = static_cast<DirectX8Renderer&>(dev.GetRenderer());

        // Check A: real window.
        check(renderer.GetWindowInternal() != nullptr, "GraphicsDevice has a real window under the DIRECTX8 renderer");
