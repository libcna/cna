...
// plan_dx1.md Phase O4 (DX1-30..DX1-39): CPU compositor / SpriteBatch draw path tests for the DX1
...
//   BltFast straight copy -- exact pixel match (DX1-31).
...
//   CPU compositor itself, not just its blend math (DX1-32).
...
//   (DX1-40's precursor).
// Check E -- SpriteEffects::FlipHorizontally swaps sampled texels left-right (DX1-34).
// Check F -- Scale (dest rect != source rect) samples the correct texel per screen region (DX1-35).
// Check G -- Rotation by pi around the texture's center origin maps the top-left source texel to
//   the bottom-right screen quadrant and vice versa (DX1-33).
// Check H -- SetTransformMatrix() (a translation) shifts an otherwise-identity draw by exactly the
//   translation offset (DX1-36).
// Check I -- SpriteSortMode is fully handled by shared SpriteBatch.cpp -- Begin(sortMode, ...)
//   with a non-Deferred mode draws without throwing or needing renderer-specific code (DX1-37).
// Check J -- Begin(..., a non-null custom Effect) throws: no programmable shader stage exists on
//   this renderer (DX1-38).
//
// Source-rectangle cropping (DX1-39) is exercised implicitly by every Draw() call above (all use
...
class Dx1SpriteBatchTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        // Check J: custom Effect throws.
        {
            ...
            check(threw, "Begin() with a non-null custom Effect throws (no shader stage on DX1)");
        }
