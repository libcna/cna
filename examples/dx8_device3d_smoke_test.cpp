...
// real now -- pixel-verified 3D rendering is covered by dx8_colored_primitives_test.cpp/
// dx8_ztest_test.cpp/etc, and real stencil write-then-test is covered by dx8_stencil_test.cpp, not
...
//   stencil) with all three flags) does not throw and clears color correctly -- DX8's stencil
//   clear is real (unchanged from DX6, via a direct device-level Clear(D3DCLEAR_STENCIL) call, no
//   viewport object involved), but this smoke test only checks the color result;
//   dx8_stencil_test.cpp is the dedicated real stencil write/test proof.
// Check D -- SetDepthTestEnabled/SetDepthWriteEnabled no longer throw -- a smoke-level check;
//   Dx8_ZTest covers real depth-test pixel behavior.
//
...
using namespace CNA::Internal::Backends::Dx8;

static constexpr int kCanvasSize = 64;

class Dx8Device3DSmokeTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        auto& backend = static_cast<Dx8GraphicsBackend&>(dev.GetBackend());
        ...
        // color correctly; DX8's stencil clear is real (see dx8_stencil_test.cpp for the
