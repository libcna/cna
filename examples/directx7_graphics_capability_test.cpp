...
// CNA::GraphicsCapability: verifies GraphicsDevice::SupportsCapability() correctly reports DIRECTX7's
// capability set. Unlike an earlier version of this test (mirroring a 2D-only renderer's
// capability test, matching DIRECTX1/SDL_RENDERER/CANVAS), DIRECTX7's 3D pipeline is now genuinely real
...
// confirmed D3DFILL_WIREFRAME genuinely renders edge-only output here (see DirectX7Renderer.hpp's
...
class DirectX7GraphicsCapabilityTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        auto& renderer = static_cast<CNA::Internal::Renderers::DirectX7::DirectX7Renderer&>(dev.GetRenderer());
        ...
        // above) is a real DIRECTX7 (and DIRECTX2/DIRECTX3, its own porting sources) boundary: DirectDraw has exactly one active render target.
...
public:
    DirectX7GraphicsCapabilityTest()
    {
...
int main()
{
    DirectX7GraphicsCapabilityTest game;
    game.Run();
