...
// Task 425: Pixel test -- draw multiple glyphs with spacing on EasyGL.
//
...
// (x in [10,14)) that must stay background -- proving the spacing constant is genuinely applied
// (not zero, not some other value), and that each glyph is placed at its own distinct index's
// colour (ruling out an index mix-up between the two glyphs).
//
...
class EasyGLSpriteFontMultiGlyphSpacingTest : public Game
{
...
int main()
{
    EasyGLSpriteFontMultiGlyphSpacingTest game;
    game.Run();
