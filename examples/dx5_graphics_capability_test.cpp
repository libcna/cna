...
// CNA::GraphicsCapability: verifies GraphicsDevice::SupportsCapability() correctly reports DX5's
// capability set. Unlike an earlier version of this test (mirroring a 2D-only backend's
// capability test, matching DX1/SDL_RENDERER/CANVAS), DX5's 3D pipeline is now genuinely real
...
// confirmed D3DFILL_WIREFRAME genuinely renders edge-only output here (see Dx5GraphicsBackend.hpp's
...
class Dx5GraphicsCapabilityTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        auto& backend = static_cast<CNA::Internal::Backends::Dx5::Dx5GraphicsBackend&>(dev.GetBackend());
        ...
        // above) is a real DX5 (and DX2/DX30, its own porting sources) boundary: DirectDraw has exactly one active render target.
...
int main()
{
    Dx5GraphicsCapabilityTest game;
    game.Run();
