...
// Task 258: EasyGL pixel-readback test for DrawUserIndexedPrimitives<VertexPositionColor>
// (32-bit index overload).
//
// Draws a full-NDC red quad via DrawUserIndexedPrimitives (typed VPC + uint32_t overload)
...
class DrawUserIndexedPrimitives32Test : public Game
{
    ...
    void testBasic(GraphicsDevice& dev)
    {
        ...
        check(isRed(got), "DrawUserIndexedPrimitives 32-bit offset=0", got);
    }
    ...
    void testOffsets(GraphicsDevice& dev)
    {
        ...
        const std::uint32_t indices[7] = { 0 /* dummy */, 0, 1, 2, 0, 2, 3 };
        ...
        const Color got = readCenter(dev);
        check(isRed(got), "DrawUserIndexedPrimitives 32-bit offset=1", got);
    }
...
int main()
{
    DrawUserIndexedPrimitives32Test game;
    game.Run();
