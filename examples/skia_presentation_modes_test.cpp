...
namespace
{
    ...
    [[nodiscard]] bool QuerySizes(SkiaGraphicsBackend& backend,
                                  int& windowWidth, int& windowHeight,
                                  int& outputWidth, int& outputHeight)
    {
        SDL_GetWindowSize(backend.GetWindowInternal(), &windowWidth, &windowHeight);
        return windowWidth > 0 && windowHeight > 0
            && SDL_GetRenderOutputSize(backend.GetRendererInternal(), &outputWidth, &outputHeight)
            && outputWidth > 0 && outputHeight > 0;
    }

    void CheckRoundTrip(SkiaGraphicsBackend& backend, float logicalX, float logicalY,
                        const std::string& label)
    {
        ...
        Check(backend.TransformLogicalToWindow(logicalX, logicalY, windowX, windowY)
                  && backend.TransformWindowToLogical(windowX, windowY, roundTripX, roundTripY)
                  && Near(roundTripX, logicalX) && Near(roundTripY, logicalY),
              label + " coordinate round trip is exact");
    }

    void CheckMode(SkiaGraphicsBackend& backend, CnaPresentationMode mode,
                   ...
                   const char* name)
    {
        ...
        float stepYX = 0.0f;
        float stepY = 0.0f;
        const bool transformed = backend.TransformLogicalToWindow(0.0f, 0.0f, originX, originY)
            && backend.TransformLogicalToWindow(10.0f, 0.0f, stepX, stepXY)
            && backend.TransformLogicalToWindow(0.0f, 10.0f, stepYX, stepY);
        ...
        CheckRoundTrip(backend, 7.25f, 19.5f, name);
        ...
        backend.ReadBackbuffer(kVirtualWidth - 1, kVirtualHeight - 1, 1, 1, actual.data());
        backend.Present();
...
int main()
{
    ...
    try
    {
        ...
        int outputWidth = 0;
        int outputHeight = 0;
        Check(QuerySizes(backend, windowWidth, windowHeight, outputWidth, outputHeight),
        ...
        CheckMode(backend, CnaPresentationMode::Letterbox,
                  ...
                  "Letterbox");
        CheckMode(backend, CnaPresentationMode::Overscan,
                  overscanScale, overscanScale,
                  (windowWidth - kVirtualWidth * overscanScale) * 0.5f,
                  (windowHeight - kVirtualHeight * overscanScale) * 0.5f,
                  "Overscan");
        CheckMode(backend, CnaPresentationMode::Stretch,
                  static_cast<float>(windowWidth) / kVirtualWidth,
                  static_cast<float>(windowHeight) / kVirtualHeight,
                  0.0f, 0.0f, "Stretch");
        CheckMode(backend, CnaPresentationMode::NativeBackBuffer,
        ...
        int dynamicWidth = 0;
        int dynamicHeight = 0;
        backend.GetViewportSize(dynamicWidth, dynamicHeight);
        ...
        CheckRoundTrip(backend, dynamicWidth * 0.25f, kVirtualHeight * 0.75f,
        ...
        for (int attempt = 0; attempt < 200; ++attempt)
        {
            ...
            if (currentWindowWidth == resizedWindowWidth && currentWindowHeight == windowHeight
                && SDL_GetRenderOutputSize(backend.GetRendererInternal(),
        ...
        CheckRoundTrip(backend, dynamicWidth * 0.5f, kVirtualHeight * 0.5f,
                       "FixedHeightDynamicWidth after resize");

        bool invalidRejected = false;
        try
        {
            backend.SetPresentationMode(99);
        }
        ...
        backend.GetViewportSize(widthAfterReject, heightAfterReject);
