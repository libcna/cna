...
// CNA::GraphicsCapability: verifies GraphicsDevice::SupportsCapability() correctly reports DIRECTX5's
// capability set. Unlike an earlier version of this test (mirroring a 2D-only renderer's
// capability test, matching DIRECTX1/SDL_RENDERER/CANVAS), DIRECTX5's 3D pipeline is now genuinely real
...
// confirmed D3DFILL_WIREFRAME genuinely renders edge-only output here (see DirectX5Renderer.hpp's
...
class DirectX5GraphicsCapabilityTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        auto& renderer = static_cast<CNA::Internal::Renderers::DirectX5::DirectX5Renderer&>(dev.GetRenderer());
        ...
        // above) is a real DIRECTX5 (and DIRECTX2/DIRECTX3, its own porting sources) boundary: DirectDraw has exactly one active render target.
...
public:
    DirectX5GraphicsCapabilityTest()
    {
...
int main()
{
    DirectX5GraphicsCapabilityTest game;
    game.Run();
