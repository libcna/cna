...
class BgfxSkinnedEffectCombinedTest : public Game
{
    Texture2D tex_;
    bool      done_   = false;
    int       result_ = 1;
...
protected:
    ...
    Color renderAndRead(GraphicsDevice& device, SkinnedEffect& fx, VertexBuffer& vb, int triCount, const Rectangle& reg)
    {
        Color got(0, 0, 0, 0);
        for (int i = 0; i < 20; ++i)
        {
            ...
            // this draws a fixed set of full-screen-height quads at z=0 every frame.
    ...
    void Draw(const GameTime&) override
    {
        ...
        const int triCount = static_cast<int>(verts.size() / 3);
        ...
        const Color aPx = renderAndRead(device, fx, vb, triCount, aReg);
        const Color bPx = renderAndRead(device, fx, vb, triCount, bReg);
        const Color cPx = renderAndRead(device, fx, vb, triCount, cReg);
...
int main()
{
    BgfxSkinnedEffectCombinedTest game;
    game.Run();
