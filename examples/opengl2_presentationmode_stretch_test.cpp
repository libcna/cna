...
// plan_opengl2.md: pixel-exact proof for real CnaPresentationMode::Stretch semantics --
// previously fell back to the virtual size verbatim (no actual non-uniform scaling), a documented
// shared gap with EasyGL. Mirrors SdlGpuGraphicsBackend::ComputeLogicalViewport's algorithm (the
// established "this backend actually does it right" reference elsewhere in this codebase).
//
// Uses a deliberately non-square window (128x64, resized in after startup -- see below) against a
// square virtual resolution (64x64): Stretch always fills the window exactly with a NON-UNIFORM
// scale (2x horizontally, 1x vertically here) -- unlike Overscan, nothing is cropped either.
//
...
// Letterbox/Overscan twins for the other two modes.
//
// Check A -- left/right physical window edges AND the centre are all the drawn sprite's own
//   colour -- fills exactly, no bars (like Overscan, but via non-uniform stretch, not cropping).
...
class OpenGL2PresentationModeStretchTest : public Game
{
...
protected:
    ...
    void Draw(const GameTime&) override
    {
        ...
        if (frame_ == 2)
        {
            ...
            Check("Stretch: no bars, full coverage: left=" + ColorStr(left) + " right=" + ColorStr(right) + " centre=" + ColorStr(centre),
...
int main()
{
    ...
    OpenGL2PresentationModeStretchTest game;
    game.Run();
