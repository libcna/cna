...
public:
    SdlGpu3DTest()
    {
        ...
        // See sdlgpu_smoke_test.cpp's own comment: GraphicsDeviceManager.SynchronizeWithVerticalRetrace
        // does not currently reach the backend at all (pre-existing, backend-independent gap --
        // see plan_sdlgpu.md), so this calls the backend directly to avoid VSync's ~1s/frame wait
        // on this test's virtual display.
