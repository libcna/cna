...
// CNA::GraphicsCapability: verifies GraphicsDevice::SupportsCapability() correctly reports DX6's
// capability set. Unlike an earlier version of this test (mirroring a 2D-only backend's
// capability test, matching DX1/SDL_RENDERER/CANVAS), DX6's 3D pipeline is now genuinely real
...
// confirmed D3DFILL_WIREFRAME genuinely renders edge-only output here (see Dx6GraphicsBackend.hpp's
...
class Dx6GraphicsCapabilityTest : public Game
{
...
protected:
    void Draw(const GameTime&) override
    {
        ...
        auto& backend = static_cast<CNA::Internal::Backends::Dx6::Dx6GraphicsBackend&>(dev.GetBackend());
        ...
        // above) is a real DX6 (and DX2/DX30, its own porting sources) boundary: DirectDraw has exactly one active render target.
...
int main()
{
    Dx6GraphicsCapabilityTest game;
    game.Run();
