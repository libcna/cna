...
| # | Task | Status | Notes |
|---|---|---|---|
| CANVAS-30 | `CanvasSpriteBatchBackend : ISpriteBatchBackend` skeleton; `Begin()`/`End()` | ✅ | `End()` resets `ctx.setTransform` to identity (belt and suspenders alongside `Clear()`'s own defensive reset — see CANVAS-36). CANVAS-86 later added a compact deferred command buffer so a normal SpriteBatch crosses wasm→JS only once; Immediate mode still replays each draw directly. |
...
| CANVAS-86 | Fix real-game sprite scaling and remove Canvas2D's per-sprite CPU/readback bottleneck | ✅ | Mobile Eggbert exposed two independent defects. The in-bounds/clamp path applied `destination/source` once with `ctx.scale` and again to `drawImage`'s local destination rectangle, so any source/destination size mismatch was squared (notably tiny scaled text and sprites). Local clamp geometry now remains in source-pixel units. Separately, every AlphaBlend sprite synchronously read its source rectangle, looped over pixels, resized a scratch canvas and uploaded it again; a texture's full straight-alpha variant is now built once and cached until mutation, with render targets correctly recognized as already straight. Deferred SpriteBatch commands are replayed through one wasm→JS call instead of one crossing per sprite, and non-`copy` blends skip an unnecessary per-sprite clip path. The browser smoke test now pixel-checks the exact destination boundary and asserts one variant build/one bulk replay for repeated draws; emitted-JS diagnostics exercise the same invariants against the Mobile Eggbert build. Its Canvas example targets also dropped a stale direct dependency on the removed monolithic `SHARP_RUNTIME`, allowing the smoke executable to link against the modular runtime closure already supplied by `CNA`. |
