...
| CANVAS-11 | `Clear(r,g,b,a)`: real `ctx.fillStyle` + `ctx.fillRect(0,0,w,h)` (or `ctx.clearRect` for alpha=0, matching FNA's `Clear` semantics) against the currently-bound context (main canvas or a bound render-target's offscreen canvas — see Phase C3) | ✅ | Render-target-side (Phase C3) still TODO; main-canvas path done. |
