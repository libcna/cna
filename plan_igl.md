...
also attempted against Mesa lavapipe: device bring-up, the smoke test, `Igl_3D` and
`Igl_RenderTarget` all now genuinely pass (three real bugs found this session, two fixed: a
back-buffer BGRA/RGBA channel swap, and a `NormalizedZRange` misreport that was silently applying
OpenGL's clip-space depth remap to Vulkan too). `Igl_2D` (`SpriteBatch`) alone still fails its pixel
checks on Vulkan, but a follow-up session found this is very likely a *readback* bug, not a
rendering bug: a full-surface diagnostic copy shows both sprites rendering at the correct place with
the correct colour, while `ReadBackbuffer()`'s normal small-rectangle GPU copy returns the clear
colour at those exact same coordinates. No safe fix was found (a "read the whole surface and crop"
workaround fixes `Igl_2D` but breaks the previously-passing `Igl_RenderTarget_Vulkan`, so it was
reverted) -- see IGL-60 for the full account. So **the Vulkan backend is close but its own pixel
test harness cannot yet fully confirm it**: 3D geometry, depth testing and render-target readback
are verified correct; `SpriteBatch` is very likely correct too but not verified, because this
renderer's own readback path is unreliable in a scene-dependent way that has not yet been isolated.
The rest of Phase H is still open. Tasks below still marked ✍️ are code paths that exist but have
not yet been individually exercised.
...
| C | Resources (textures, buffers, targets) | 🔶 `Texture2D`/`RenderTarget2D`/depth/`RenderTarget2D.GetData` (`Igl_RenderTarget`), full 2- and 4-slot MRT plus the cube-face-in-MRT refusal (`Igl_Mrt`/`Igl_Mrt4`/`Igl_MrtCubeRefuse`), `TextureCube`/`RenderTargetCube` (`Igl_EnvironmentMapEffect`/`Igl_RenderTargetCube`) and `Texture3D` (`Igl_ShaderEffectTexture3D`) verified; back-buffer readback beyond the smoke test's own use remains untested |
...
modules/renderers/igl/
    ...
    examples/{CMakeLists.txt,igl_smoke_test.cpp,igl_2d_test.cpp,igl_3d_test.cpp,igl_rendertarget_test.cpp,
        ...
        igl_vulkan_backbuffer_bgr_test.cpp}
