...
**Branch:** `feature/igl`. **Audited and repaired on 2026-08-17** (see IGL-65, IGL-66, IGL-67 and
the rewritten IGL-7): the renderer builds, runs and passes its whole registered suite --
`ctest -R Igl` is **61/61**, comprising 34 host-portable unit cases in `CnaTests` and 27 example
tests against Mesa llvmpipe (OpenGL/GLX) and lavapipe (Vulkan) under Xvfb.

Run explicitly on **both** backends (all 24 example binaries x 2, not only the four the ctest
registration forces onto Vulkan), the picture is:

* **OpenGL/GLX: 24 of 24 binaries pass, every check.** This is the backend the renderer is verified
  on.
* **Vulkan: 20 of 24.** The four that do not are all previously-known or newly-characterised
  Vulkan-only limits, none of them regressions: `Igl_2D` (IGL-60's unresolved readback defect, 3/5
  checks pass), `Igl_Msaa` (3/4 -- a `RenderTarget2D`'s MSAA resolve produces a hard edge rather
  than a blended one on lavapipe; newly observed, since nothing had run this test on Vulkan before),
  and the three custom-`ShaderEffect` tests (`Igl_ShaderEffectTexture3D`, `Igl_Instancing`,
  `Igl_SpriteBatchShaderEffect`), which do not fail a check but **abort the process** inside IGL:
  `igl::glslang::compileShader` calls `IGL_ABORT` when `glslang_shader_parse()` rejects a shader,
  and these tests' custom effects are written in desktop-GL GLSL that glslang will not accept for
  SPIR-V. That is a wider limit than design decision 8's "loose uniforms are refused by name" --
  a custom effect on Vulkan must be written as Vulkan GLSL, and if it is not, upstream IGL takes
  the process down rather than returning an error CNA could report.
...
channel swap and a `NormalizedZRange` misreport (both fixed), plus `Igl_2D`'s own failure, which a
follow-up session narrowed to a *readback* defect rather than a rendering one: a full-surface
diagnostic copy shows both sprites rendering at the correct place with the correct colour, while
`ReadBackbuffer()`'s normal small-rectangle GPU copy returns the clear colour at those exact
coordinates. No safe fix was found (a "read the whole surface and crop" workaround fixes `Igl_2D`
but breaks the previously-passing `Igl_RenderTarget_Vulkan`, so it was reverted) -- see IGL-60.
...
| IGL-60 | Vulkan backend run | 🔶 | attempted this session against Mesa lavapipe. Found and fixed a real crash: `IglDynamicBufferPool` sized every chunk (vertex, index, *and* uniform) to a shared 256 KiB `kDynamicChunkBytes`, but Vulkan only guarantees `maxUniformBufferRange >= 65536` and IGL's Vulkan backend asserts a uniform buffer never exceeds the device's actual limit (lavapipe reports exactly the guaranteed minimum) -- `IglResources.cpp` now gives uniform-typed chunks their own 64 KiB `kDynamicUniformChunkBytes` cap. After that fix `Igl_Smoke` passes 8/8 on Vulkan. **Three real bugs found this session, two fixed, `Igl_3D` and `Igl_RenderTarget` now genuinely pass on Vulkan (verified via new `Igl_3D_Vulkan`/`Igl_RenderTarget_Vulkan` ctest entries reusing the same binaries with `CNA_IGL_BACKEND=vulkan`); `Igl_2D` alone remains open, narrowed to a specific, different cause:**<br><br>**Bug A -- back-buffer red/blue channel swap, FIXED.** `igl::IFramebuffer::copyBytesColorAttachment()` copies an attachment's raw physical bytes with no channel reordering on either backend, and the Vulkan back buffer's real physical format is whatever the platform window-system surface natively offers -- `BGRA_UNorm8` on this renderer's Linux/X11/Mesa target, confirmed by the swapchain's own startup log and by direct experiment (a pure-red clear (255,0,0) read back as pure blue (0,0,255); a pure-green clear was unaffected, since swapping two already-zero channels is a no-op -- exactly what let this hide behind a vaguer "everything reads back wrong" symptom for a while). Every other CNA-created texture requests an R-first format explicitly and was never affected. Fixed with `IglConversions.hpp`'s `SwapRedBlueIfBgrOrdered()`, checking the ACTUAL reported format rather than hardcoding "swap on Vulkan" -- applied in `IglRenderer::ReadBackbuffer()`. Verified by `igl_vulkan_backbuffer_bgr_test.cpp` (`Igl_VulkanBackBufferBgr`, the first test in this plan forced onto Vulkan via its own ctest `ENVIRONMENT` entry).<br><br>**Bug B -- `NormalizedZRange` misreported for Vulkan, FIXED, the dominant root cause.** `igl::IDevice::getNormalizedZRange()` defaults to `NegOneToOne` in IGL's own base class, and **neither IGL's OpenGL nor Vulkan backend overrides it** (confirmed: zero matches for that method/enum anywhere in IGL v1.1.1's Vulkan source tree). That default is coincidentally correct for OpenGL (whose native clip-space Z genuinely is [-1, 1]) but silently wrong for Vulkan, whose native Z range is [0, 1] like Direct3D. `IglDraw.cpp`'s `FillEffectUniforms` trusted the reported value to decide whether to apply `ApplyNegativeOneToOneDepth` (the `z' = 2z - w` OpenGL-only remap), so it was wrongly applied to Vulkan too, corrupting every draw's Z values into the wrong range -- Z=0.25 became clip-space -0.5, outside Vulkan's valid [0, w] volume. Fixed by checking `IsVulkanBackend()` directly rather than trusting the API's own (for this IGL version, unreliable) report. **This one fix resolved what earlier investigation this session had mistaken for a separate, deeper "second draw after any readback is invisible" bug** -- a from-scratch isolated repro (two full-screen quads, a readback between them, no explicit depth state) that had survived seven other ruled-out theories (GPU sync/`waitIdle` timing, command-buffer/encoder identity, framebuffer/image identity, depth-buffer presence, `DepthStencilState` settings, vertex/index buffer C++ lifetime, partial/scissor-clipped visibility) turned out to be this Z-range corruption all along: with Z values pushed outside the valid clip volume, the "invisible" draw was being clipped away, not skipped or racing. Confirmed directly: `Igl_3D`'s exact scene (nearer quad z=0.25, farther z=0.75, `DepthStencilState.Default`, a real depth buffer) went from failing all 3 checks to passing all 3 the moment this fix landed; `Igl_RenderTarget` went from 2/6 to 6/6 the same way. Verified permanently via `Igl_3D_Vulkan`/`Igl_RenderTarget_Vulkan` (new ctest entries, same binaries as the OpenGL versions, forced onto Vulkan).<br><br>**Bug C -- `Igl_2D` (`SpriteBatch`) still fails on Vulkan, UNRESOLVED, narrowed to a specific remaining cause.** With Bugs A and B both fixed, `Igl_2D`'s two sprite draws still render nothing visible (both the opaque and alpha-blended sprite areas read back as the background clear colour). Ruled out concretely this session, each with direct evidence, not guesswork: (1) vertex data -- mapped the sprite pool's actual GPU buffer bytes right before each draw; both sprites' position/colour are uploaded correctly; (2) culling -- `DrawSpriteBatchEXT` already forces `igl::CullMode::Disabled` in its own `PipelineKey` regardless of the game's `RasterizerState` (confirmed by reading the code, not assumed); (3) front-face winding -- empirically inverted it for the Vulkan back buffer as a blunt test, no change (also inconsistent with IGL's own documented, and now source-confirmed, negative-viewport-height Y-flip trick in `RenderCommandEncoder::bindViewport`, which already keeps winding consistent between backends); (4) depth/stencil state -- both sprite draws run with `depthEnable=false`/`writeEnable=false` (this test never touches `DepthStencilState`, and the device's own resolved default is disabled for a plain 2D-only test), so this is not a depth-test-vs-missing-buffer mismatch; (5) viewport/scissor -- logged as the full 64x64 surface for both draws, matching the passing 3D tests exactly; (6) pass/framebuffer identity -- only one `BeginPass()` covers both sprite draws (no frame-boundary crossing at all in this test, since both `SpriteBatch::Begin()/Draw()/End()` pairs run before the first `GetBackBufferData()` call), ruling out Bug B's own class of issue entirely for this case. (7) texture sampling -- **ruled out**: a throwaway diagnostic drew a textured `BasicEffect` quad (both `VertexPositionTexture` and, matching `IglSpriteVertex`'s own 24-byte shape exactly, `VertexPositionColorTexture`) through the *general* 3D `SubmitDraw` path with an identity projection, and it sampled its real texture correctly on Vulkan -- `SAMPLE_TEXTURE`/`uTexture0` genuinely works there, ruling out a Bug-A-like texture-format sibling as the explanation. (8) orthographic projection -- inconclusive, not attributable to Vulkan specifically: the same diagnostic quad, switched to `SpriteBatch`'s own exact `Matrix::CreateOrthographicOffCenter(0, W, H, 0, 0, 1)` projection and pixel-space vertex coordinates (matching `DrawSpriteBatchEXT`'s own call into the same `FillEffectUniforms` both draws share) instead of an identity one, failed to render **on OpenGL too** -- so whatever is wrong with that specific combination is not the Vulkan-only bug this entry is tracking, and was not investigated further to avoid conflating two different questions (this combination is not exercised by any other test or real code path in this plan; `SpriteBatch` itself, which uses the identical projection, renders correctly on OpenGL via its own dedicated `DrawSpriteBatchEXT` code path). **What remains unexplained (as of the previous session):** `Igl_2D`'s two sprite draws still produce zero visible output on Vulkan (confirmed via a 32x32-sample grid scan finding pixel 0/1024 of the expected colour, same as Bug B's own now-resolved symptom shape) despite ruling out vertex data, culling, winding, depth/stencil state, viewport/scissor, pass/framebuffer identity, and texture sampling in general.<br><br>**Follow-up session: reframed as a readback bug, not a rendering bug -- still unfixed, workaround reverted as unsafe.** (9) the dynamic vertex/index buffer pool -- **ruled out**: `DrawSpriteBatchEXT` was temporarily made to bypass `dynamicVertexPool_`/`dynamicIndexPool_` entirely, uploading through fresh persistent `igl::IBuffer`s instead (env-gated diagnostic, fully reverted); `Igl_2D` still failed identically, so the pool mechanism itself is not the cause. (10) texture sampling specifically inside `DrawSpriteBatchEXT` -- **ruled out**: forcing `params.textureEnabled = false` (vertex colour only, no sampling at all) still produced zero output. (11) **the actual breakthrough**: a one-time diagnostic full-surface `copyBytesColorAttachment(0, 0, fullWidth, fullHeight)` (rather than the small per-pixel rectangle `ReadBackbuffer()` normally requests) was added right where `ReadBackbuffer()` already does its real per-pixel copy, and it showed **both sprites genuinely rendered, at the geometrically correct location and with the correct colour** -- a 512-pixel non-background region split into two exact 16x16 blocks whose physical (bottom-first) rows convert, via the exact same `surfaceHeight - y - h` formula `ReadBackbuffer()` already uses, to precisely the sprites' correct logical positions (8,8)-(23,23) and (40,40)-(55,55), with the alpha-blended sprite's raw bytes decoding (after the existing BGR swap) to `(127,0,128,191)` -- matching the test's expected `(127,0,128,*)` almost exactly. **So the sprites are not invisible: `SpriteBatch` renders correctly on Vulkan.** The bug is in `ReadBackbuffer()`'s own small-rectangle GPU copy: a direct 1x1 `copyBytesColorAttachment` at the exact same physical coordinate the full copy read correctly instead returned the clear colour, for every one of `Igl_2D`'s sprite-covered sample points. (12) **the workaround that seemed obvious turned out to be unsafe**: changing `ReadBackbuffer()` to always copy the whole attachment once and crop the requested rectangle out of it in software (Vulkan only, OpenGL untouched) made `Igl_2D` pass 5/5 on Vulkan -- but running the *same* full-ctest-suite check against `Igl_RenderTarget_Vulkan` (a scene this renderer already gets right, verified back on the clean baseline: `direct sub-rect copy = correct, full-image copy = wrong`, i.e. the **exact opposite** polarity from `Igl_2D`) turned two previously-passing checks into an exact top/bottom colour swap. Side-by-side instrumentation (computing both the full-image crop and a direct sub-rect copy for the same coordinate, in both orders) confirmed this is not a staging-buffer-reuse ordering artifact -- swapping which copy ran first changed nothing -- and an explicit `VulkanContext::waitIdle()` immediately before the copy also changed nothing, ruling out a plain missing-barrier race. **Net effect: neither "always full-copy" nor "always direct sub-rect" is a universally correct readback strategy against this exact IGL v1.1.1 + Mesa lavapipe combination**, and no discriminating factor between the two scenes was found this session (physical Y values 47/15 for the failing case vs. 55/7 for the passing one; scene structure differs only in that `Igl_RenderTarget` renders to an off-screen target and samples it back before touching the back buffer). The workaround was fully reverted (`git checkout --` on `IglRenderer.cpp`); the committed baseline is unchanged from before this follow-up session, confirmed via the full `ctest -R Igl` suite (40/40 passing, unaffected either way). **Conclusion for whoever picks this up next:** treat IGL-60 as a Vulkan *readback* defect (`igl::vulkan::VulkanStagingDevice::getImageData2D` / `Framebuffer::copyBytesColorAttachment`'s sub-rectangle path), not a SpriteBatch rendering defect -- the rendering itself is very likely correct. A real fix probably needs either an upstream IGL fix, RenderDoc-level frame capture against lavapipe to see what the driver actually did differently between a full- and partial-extent `vkCmdCopyImageToBuffer`, or a scene-agnostic readback strategy neither attempted approach here provides (e.g. reading via a dedicated blit/resolve path instead of `copyBytesColorAttachment`, which was not tried) |
...
| IGL-67 | Uploaded render-target contents read back flipped on Vulkan | ⬜ | Found by `Igl_SurfaceFormat_Vulkan` (IGL-65's new test, run on both backends deliberately because IGL's two backends have genuinely different format tables). `igl::vulkan::Framebuffer::copyBytesColorAttachment` passes `flipImageVertical = true` to `VulkanStagingDevice::getImageData2D` **unconditionally** -- its OpenGL counterpart needs no such flip because `glReadPixels` already reads a texture-backed attachment from the same end. For content this renderer *rendered* the two flips cancel (design decision 6's own projection flip is the other one), which is why `Igl_RenderTarget_Vulkan` reads back correctly; for content a caller *uploaded* into a target with `RenderTarget2D.SetData` there is no second flip, so `GetData` returns the rows reversed on Vulkan and identically on OpenGL. Left open rather than papered over: the obvious fix (flip the rows inside `IglRenderTargetRenderer::UpdatePixels` on the Vulkan backend) is a guess about which of the two storage conventions the *sampling* path agrees with, and confirming that needs a test that uploads into a target and then samples it in a draw -- which does not exist yet on either backend. Documented in `docs/igl-renderer.md`'s gap table |
...
| IGL-69 | The shared capability and instancing suites had no IGL arm | ✅ | Two shared, renderer-keyed test tables never gained an `Igl` case, so an IGL build was asserted against the default profile and failed on behaviour this plan documents as correct. `GraphicsDeviceCapabilityTests.cpp`'s `ExpectedCapabilities()` default claims occlusion-query support, which IGL-61 records IGL v1.1.1 cannot have -- IGL now has its own arm, `{multipleRenderTargets = true, occlusionQuery = false, customEffects = true}`. `InstancedDrawMultiStreamTests.cpp`'s final `else` asserts that the renderer implements *no* instanced path and must refuse a mixed-stream draw; IGL implements one (IGL-31, `Igl_Instancing`), so it now has its own measured-outcome arm asserting acceptance. Deliberately NOT added to that file's `MultiStreamOracle()` pixel-measurement set: nothing has measured this renderer's mixed-frequency multi-stream result, and IGL-31's fixed usage-to-slot limit is a real reason to expect it would need its own analysis first |
...
| Uploading pixels into a `RenderTarget2D` on Vulkan | IGL's Vulkan readback always flips vertically, which cancels only for content this renderer rendered (IGL-67) | `SetData` followed by `GetData` returns the rows reversed on Vulkan; correct on OpenGL |
...
6. **Two shared test tables had no IGL arm (IGL-69).** The capability suite asserted an
   occlusion-query capability this renderer documents it cannot have, and the instancing suite
   asserted that it implements no instanced path at all.
