...
OpenGL's clip-space depth remap to Vulkan too). `Igl_2D` (`SpriteBatch`) alone still fails, narrowed
to a specific remaining cause -- likely `Texture2D` sampling correctness, not yet verified on this
backend at all (see IGL-60) -- so **the Vulkan backend is close but not yet fully working**: 3D
geometry, depth testing and render-target readback are real; textured 2D drawing is not yet proven.
