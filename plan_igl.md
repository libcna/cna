...
also attempted against Mesa lavapipe: device bring-up and the smoke test are real and pass, and a
real back-buffer channel-swap bug (BGRA vs. RGBA byte order) has since been found and fixed with a
permanent regression test, but the three pixel-conformance tests still fail on a second, deeper bug
now narrowed to a minimal repro (see IGL-60) -- **the Vulkan backend is not yet considered
working**, only "boots, clears, and reads clears back correctly." The rest of Phase H is still
open. Tasks below still marked ✍️ are code paths that exist but have not yet been individually
exercised.
...
| IGL-60 | Vulkan backend run | 🔶 | attempted this session against Mesa lavapipe. Found and fixed a real crash: `IglDynamicBufferPool` sized every chunk (vertex, index, *and* uniform) to a shared 256 KiB `kDynamicChunkBytes`, but Vulkan only guarantees `maxUniformBufferRange >= 65536` and IGL's Vulkan backend asserts a uniform buffer never exceeds the device's actual limit (lavapipe reports exactly the guaranteed minimum) -- `IglResources.cpp` now gives uniform-typed chunks their own 64 KiB `kDynamicUniformChunkBytes` cap. After that fix `Igl_Smoke` passes 8/8 on Vulkan (device bring-up, buffers, texture creation, 60 frames of Clear+Present all real). `Igl_2D`, `Igl_3D` and `Igl_RenderTarget` do not, and the OpenGL/GLX backend (IGL-56-59) remains the one this plan calls verified. **Two separate bugs, not one, now that this has been investigated further:**<br><br>**Bug A -- back-buffer red/blue channel swap, FOUND AND FIXED this session.** `igl::IFramebuffer::copyBytesColorAttachment()` copies an attachment's raw physical bytes with no channel reordering on either backend, and the Vulkan back buffer's real physical format is whatever the platform window-system surface natively offers -- `BGRA_UNorm8` on this renderer's Linux/X11/Mesa target, confirmed by the swapchain's own startup log line and by direct experiment (a pure-red clear (255,0,0) read back as pure blue (0,0,255); a pure-green clear was unaffected, since swapping two already-zero channels is a no-op -- exactly the kind of thing that looks like "sometimes it works" until reduced to a plain two-clears-no-draws repro). Every other CNA-created texture (`RenderTarget2D`, `Texture2D`, ...) requests an R-first format explicitly and was never affected. Fixed with a new portable helper, `IglConversions.hpp`'s `SwapRedBlueIfBgrOrdered()`, checking the ACTUAL texture format (`igl::TextureFormat::BGRA_UNorm8`) rather than hardcoding "swap on Vulkan" -- applied in `IglRenderer::ReadBackbuffer()` after the raw copy. Verified with a new permanent regression test, `igl_vulkan_backbuffer_bgr_test.cpp` (`Igl_VulkanBackBufferBgr`, the first test in this plan forced onto the Vulkan backend via its own ctest `ENVIRONMENT` entry rather than whatever `CNA_IGL_BACKEND` the invocation happens to set) -- both a pure-red and a mixed-channel clear now read back byte-exact. Confirmed via full-suite regression (`ctest -R Igl`, 38/38 pass) that this fix is a genuine no-op on OpenGL/GLX (format check never matches there).<br><br>**Bug B -- a draw issued after ANY readback (`GetBackBufferData`/render-target `GetData`) since the last `Clear`/present boundary never visually appears, STILL UNRESOLVED, but narrowed to a minimal, 100%-reproducible repro this session** (a throwaway diagnostic test, not committed): draw a full-screen green quad, read it back (correctly reads green after Bug A's fix), draw a full-screen blue quad on top with no depth test and no intervening `Clear()`, read again -- the screen is still green everywhere (a 16x16-sample grid scan found blue at 0/256 points, ruling out a partial/scissor-clipped visibility issue; the draw is not partially wrong, it is entirely invisible). This is very likely the actual root cause behind `Igl_2D`/`Igl_3D`/`Igl_RenderTarget`'s own failures (each has at least one draw following an earlier readback in the same run), now separated cleanly from Bug A's channel corruption, which was previously obscuring it (a "reads back blue" symptom was, before the fix, ambiguous between "channel-swapped correct data" and "genuinely wrong data" -- it turned out to be both, layered). **Ruled out, with concrete evidence, not just theory:** (1) GPU synchronization/timing -- inserting an explicit `VulkanContext::waitIdle()` (via `getVulkanContext()`, the same escape hatch `IglPlatformSurface.cpp` already uses for device bring-up) immediately after every `FlushPendingFrameEXT()` submit, before the next frame's command buffer is even created, made no difference; (2) command-buffer/encoder identity -- logged pointers confirm each `BeginPass()` gets a genuinely fresh `igl::ICommandBuffer` (`GetCommandQueue().createCommandBuffer()` returns a new object each time, confirmed by distinct addresses); (3) framebuffer/back-buffer image identity -- the `igl::IFramebuffer`/colour-attachment-texture pointers are IDENTICAL across repeated `BeginPass()`/`ReadBackbuffer()` calls within one process, ruling out "acquired a different, uninitialized swapchain image" as the cause; (4) depth-buffer presence and `DepthStencilState` -- the repro fails identically with no depth-stencil format requested at all and the device's own default `DepthStencilState` (never set to `Default`), so this is not a depth/stencil-format or depth-test-against-a-missing-buffer problem; (5) `VertexBuffer`/`IndexBuffer` C++ object lifetime -- kept the second draw's buffers alive for the whole test function (well past its own readback) instead of letting them go out of scope immediately after the draw call, with no change; (6) render-pass load action -- confirmed via direct log (`igl::LoadAction` values) that the second pass correctly requests `Load` (preserve), not `Clear` or `DontCare`, so this is not attachment content being discarded between passes; (7) partial/scissor-clipped visibility, per the grid-scan result above. **What remains genuinely untested and is the natural next lead:** whether the SECOND draw's own vertex-attribute or uniform-block GPU memory actually contains the values it was uploaded with by the time the GPU reads them -- none of the ruled-out theories inspected the actual buffer bytes (as opposed to identity/pointers/timing), which would need either a RenderDoc-style GPU capture or hand-written host-visible-memory-mapping diagnostic code, not attempted this session. Left unresolved rather than guessed at |
