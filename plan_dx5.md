...
- **New backend: `CNA_GRAPHICS_BACKEND=DX5`.** No temporary-naming concern this time (unlike
  `DX30`) — `DX5` is not already claimed by any shipping backend.
- **DX5 (1997) is the release where execute buffers disappear entirely.** `IDirect3DDevice3` only
  ever exposes `DrawPrimitive`/`DrawIndexedPrimitive` — matching `DX2`/`DX30`'s own already-proven
...
- **More invasive port than `DX30`'s.** `DX30` upgraded only the top `IDirectDraw` object to v2,
...
- **3D layer: `IDirect3D3`/`IDirect3DDevice3`/`IDirect3DViewport3`.** Texture object stays
  ...
  Phase O9 CPU lighting, ported from `DX30`, just resubmitted through the new parameter shape.
- **Real capability upgrade used, not just carried forward as a workaround:**
  `IDirect3DViewport3::Clear2` takes explicit color/z/stencil *values* — `DX2-24`/`DX30` had to
...
|---|---|---|---|
| DirectDraw (2D) | `IDirectDraw4`, `IDirectDrawSurface4`, `DDSURFACEDESC2`, `DDSCAPS2` | DX5 SDK | `IDirectDraw`/`IDirectDraw2`/`IDirectDraw3` (skipped, `DX30` already used v2 — DX5 goes straight to v4, the real DX5-SDK-introduced revision), `IDirectDrawSurface`/`IDirectDrawSurface2`/`IDirectDrawSurface3`, `IDirectDraw7`+ |
...
(interface number "2") is the genuine DX3-SDK interface (`DX30`'s own plan already established
...
| `DX5-0c` | `IDirect3DViewport3` creation, `SetViewport2` (same `D3DVIEWPORT2` struct as `DX2`/`DX30`) | Whether viewport setup is unchanged | ✅ Works |
...
architecturally identical to `DX30`'s already-proven one, needing only the FVF-parameter swap and
...
1. **Platform gate, same as `DX1`/`DX2`/`DX30`.** Same Windows-native-or-MinGW-cross-compile
   `FATAL_ERROR` gate.

2. **2D layer: every surface upgrades to v4** (`IDirectDrawSurface4`, `DDSURFACEDESC2`,
   `DDSCAPS2`), not just the top `IDirectDraw` object (unlike `DX30`'s decision 2, which kept
   surfaces at v1). `Impl::dd` is `LPDIRECTDRAW4` (upgraded via `QueryInterface` immediately after
   `DirectDrawCreate`, releasing the v1 pointer, same pattern as `DX30`); `Impl::primary`,
   ...
   all take** — every `surface->Unlock(desc.lpSurface)` call ported from `DX30` had to become
   `surface->Unlock(nullptr)`.

3. **3D device bring-up: `IDirect3D3`/`IDirect3DDevice3`/`IDirect3DViewport3`, verbatim port of
   `DX30`'s bring-up sequence otherwise.** `IDirect3D3::CreateDevice` takes an extra trailing
   ...
   struct as `DX30`'s `IDirect3DViewport2::SetViewport2` — no new viewport struct exists.

4. **CPU transform/clip pipeline, `D3DTLVERTEX` packing, and Phase O9 CPU lighting: verbatim port
   of `DX30`'s own (itself a verbatim port of `DX2`'s post-Phase-O9 code), with exactly one
...
5. **Real `Clear2`-based depth clearing, replacing `DX2`/`DX30`'s manual Z-buffer `Lock()`
   ...
   pass a real rect with `count=1`. `Clear(r,g,b,a)`'s own COLOR clear stays exactly as `DX2`/`DX30`
   ...
   therefore still `Clear(r,g,b,a)` + `ClearDepth(depth)`, matching `DX2`/`DX30`'s own structure —
   only `ClearDepth`'s own internal mechanism changed. `ClearStencil`/`ClearDepthAndStencil`/
   `ClearColorAndStencil`/`ClearColorDepthAndStencil` are unchanged from `DX2`/`DX30` (no real
...
7. **`VertexBuffer`/`IndexBuffer`: plain CPU-side storage, unchanged from `DX30`.** A real
   ...
   `DX2`/`DX30` already use is reused verbatim, matching their own decision 8/7 reasoning: the
...
8. **Lighting/fog/multitexture/envMap/skinning/stencil/MRT/instancing/occlusion-query/volume-cube
   textures/custom-effects: identical boundary to `DX30`/`DX2` post-Phase-O9.** Fixed-function
   ...
   unavailable at this DirectX era, same as `DX2`/`DX30`.
...
9. **32-bit surfaces only, `DirectSound`/`DirectInput`/`DirectPlay` out of scope, header
   ...
   `plan_dx30.md`'s equivalent decisions, ported without change.
...
10. **CMake integration**: add `"DX5"` to `CNA_GRAPHICS_BACKEND`'s `STRINGS` property + a
    ...
    `dxguid` + `SDL3::SDL3` — identical to `DX1`/`DX2`/`DX30`.

11. **Testing: `scripts/run-wine-dx5.sh`**, modeled on `scripts/run-wine-dx30.sh` — same
...
13. **Full CTest suite is a straight rename/port of `DX30`'s own 19 `DX30`-labeled CTests to
    `DX5`-labeled equivalents**, plus updated expectations anywhere `Clear2`'s real depth/color
    values are now directly verifiable (where `DX2`/`DX30`'s own tests could only verify the Z-test
...
3. **Phase Q2** (2D layer: port from `Dx30GraphicsBackend`, upgraded to v4 throughout, decision 2).
4. **Phase Q3** (3D device bring-up: verbatim port from `Dx30GraphicsBackend`, upgraded to
...
| `DX5-2` | `cna_backend_graphics_dx5` static library target; confirm minimal link set empirically | ✅ | Same link set as `DX1`/`DX2`/`DX30` (`SDL3::SDL3 ddraw dxguid`). |
| `DX5-3` | `include/CNA/Internal/Backends/Dx5/Dx5GraphicsBackend.hpp` + `src/CNA/Internal/Backends/Dx5/Dx5GraphicsBackend.cpp`: port of `Dx30GraphicsBackend`'s files, upgraded to v4/`IDirect3D3` throughout | ✅ | |
...
| `DX5-14` | `TransformWindowToLogical`/`TransformLogicalToWindow`/letterbox present math | ✅ | Port unchanged from `DX30`. |
...
| `DX5-81` | `docs/dx5-backend.md`: mirror `docs/dx30-backend.md`'s structure | ✅ | |
...
Identical to `plan_dx30.md`'s own Boundaries section: execute-buffer Direct3D (permanently
...
`DX2`/`DX30`'s own identical stencil boundary); `IDirectDraw7`+/`IDirectDrawSurface[23567]`+
...
- `plan_dx30.md`, `docs/dx30-backend.md` — the backend this plan ports (2D layer upgraded further
