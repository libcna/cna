...
rejects 3D entry points, depth buffers, MSAA, cube/volume textures, occlusion queries and custom effects;
an 8-bit SpriteBatch-only stencil plane is the documented exception for 2D masking.
...
| GDI-025 | Add optional CPU multi-sample anti-aliasing/supersampling. | ✅ | The GDI backbuffer accepts exactly opt-in 4x MSAA (`ApplyMultiSampleCount(4)` / `PresentationParameters.MultiSampleCount=4`), with four 2x2 coverage samples, per-sample blending and a CPU resolve before `Present()`/readback. All other requests honestly return 0; `RenderTarget2D` remains single-sample and reports 0. `cna_test_gdi_2d_regression` pixel-tests a rotated one-quarter-covered edge resolving to red=63 and both count transitions. `cna_bench_gdi_2d --frames 1` now reports 0x/4x pairs: Wine reference 800×600/12 sprites 7.64→18.04 ms, 800×600/50 33.57→62.34 ms, 1280×720/20 13.08→31.04 ms. Extra sample-colour memory is 16 B/pixel (7.32 MiB at 800×600, 14.06 MiB at 1280×720); it remains opt-in accordingly. |
| GDI-026 | Add a CPU stencil buffer for 2D clipping/masking. | ✅ | Every CPU framebuffer (backbuffer and `RenderTarget2D`) owns an independent 8-bit stencil plane. GDI forwards the clockwise stencil state while forcibly disabling depth: clear, compare/read mask, write mask, all eight reachable `StencilOperation` values and masked SpriteBatch output are pixel-tested in `cna_test_gdi_2d_regression`. `StencilDepthBufferFail` and two-sided faces have no reachable 2D path. `DepthStencilBuffer` remains false because it denotes a complete depth+stencil attachment, whereas this is explicitly stencil-only. |
| GDI-027 | Decide separately whether a real depth buffer is useful for 2D layering. | ✅ | Formally unsupported: 2D layering uses SpriteBatch submission/sort order, not a depth resource. GDI forcibly disables depth even though the shared Software core owns one for its separate 3D backend; its independent 2D stencil plane does not alter that decision. `DepthStencilBuffer` stays false, depth clears still reject, and the GDI regression proves a later SpriteBatch draw is not depth-occluded. |
| GDI-028 | Implement anisotropic texture filtering in the CPU sampler, or continue to map it to linear filtering. | ✅ | Decision: keep the existing explicit `Anisotropic → Linear+mip-linear` CPU mapping and keep `AnisotropicFiltering=false`. A real anisotropic filter requires several directional texture footprints per pixel (and per MSAA sample), which would multiply the CPU raster cost for little 2D compatibility value. The GDI regression pixel-tests anisotropic output byte-for-byte against Linear on non-uniform texels. `cna_bench_gdi_2d --frames 1 --linear` and `--anisotropic` run the same sampler path; current Wine 800×600/50 MSAA-off frames were 44.76 ms and 39.83 ms respectively, ordinary run-to-run variance rather than a feature difference. |
| GDI-029 | Audit and update `SupportsCapability()` as each optional feature becomes real. | ✅ | GDI advertises `WireFrame` and `MultiSampleAntiAliasing`: wireframe SpriteBatch quads are CPU-rasterized and GDI-025 supplies an honest optional 4x backbuffer path. The real GDI-026 buffer is stencil-only, so `DepthStencilBuffer` correctly remains false rather than over-advertising a depth attachment. It explicitly reports false for 3D, MRT, anisotropy, occlusion, arbitrary custom effects and Texture3D; their existing construction/draw paths reject rather than pretend to provide those features. |
...
3. GDI-025 through GDI-029 are implemented and covered by hidden regression/benchmark work. The
   only unfinished in-scope items are native-display confirmations (GDI-004, GDI-012–016), which
   require a visible Windows run rather than another CPU-rasterizer change.
