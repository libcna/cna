...
> **`BasicEffect`'s textured, tinted, fogged and alpha-tested paths landed next** (`LLGL-25`,
> partial): one texture, `DiffuseColor`, `Alpha`, vertex-colour modulation, fog, and
> `AlphaTestEffect` — `Llgl_BasicEffect` and `Llgl_BasicEffect_OpenGL`, 13/13 checks each.
> Lighting, `DualTextureEffect`, `EnvironmentMapEffect`, `SkinnedEffect` and `PbrEffect` are still
> not implemented, nor are render targets,
> cube and volume textures, custom `ShaderEffect`s and occlusion queries.
...
| LLGL-25 | `BasicEffect` family (`AlphaTestEffect`, `DualTextureEffect`, `EnvironmentMapEffect`, `SkinnedEffect`), depth/stencil state, cull and fill modes. | 🟨 | **Textures, `DiffuseColor`, `Alpha`, vertex-colour modulation, fog and `AlphaTestEffect` done 2026-07-31** — `examples/llgl_basiceffect_test.cpp`, `Llgl_BasicEffect` (+ `_OpenGL`), 13/13 on both modules. All 3D shaders now share one 128-byte uniform block (documented in `shaders/effect3d_common.glsl.inc`) and one fragment-shader pair, textured and untextured, which apply the alpha test and fog identically so the two cannot drift. The vertex-shader variant is chosen from what the vertex LAYOUT carries, not from what the effect asked for -- a shader declaring an input the buffer does not supply reads undefined data on Vulkan -- and a textured effect drawn from a layout with no texture coordinates is refused by name. Depth/stencil state (depth half), cull and fill modes were done in `LLGL-24`. **Still open: lighting, `DualTextureEffect`, `EnvironmentMapEffect`, `SkinnedEffect`, `PbrEffect`** -- each fails through `NotYetImplemented()` naming itself. |
