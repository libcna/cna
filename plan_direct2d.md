...
| D2D-64 | Definuj zdroj přežívající zánik backendu/GraphicsDevice. | 🟨 | Prozkoumáno: `Direct2DTextureBackend`/`Direct2DRenderTargetBackend`'s vlastní destruktory (`if (owner_) owner_->Unregister...(this);`) by šly bezpečně "osiřit" (nastavit `owner_=nullptr` při zániku backendu), ALE skutečný scénář "Texture2D/RenderTarget2D přežije svůj GraphicsDevice" narazí dřív na SDÍLENOU (ne Direct2D-specifickou) chybu: `GraphicsResource::Dispose(bool)` (`src/Microsoft/Xna/Framework/Graphics/GraphicsResource.cpp`) i `Texture::Dispose(bool)`/`RenderTarget2D::Dispose(bool)` bezpodmínečně dereferencují `graphicsDevice_` (`graphicsDevice_->OnResourceDestroyed(...)`, `graphicsDevice_->RemoveResourceReference(this)`, `graphicsDevice_->GetRenderTargets()`) bez ohledu na to, jaký backend je použit — to samé UAF riziko existuje na EasyGL, D3D11 a všech ostatních backendech stejně. Oprava jen na Direct2D úrovni by tento pád nezabránila a end-to-end test by spadl na jiném místě, než which ověřuje. Skutečná oprava patří do sdíleného `GraphicsResource`/`Texture` lifecycle kódu, mimo rozsah `plan_direct2d.md` — nekódováno zde, aby nedošlo k neověřené/nebezpečné změně destruktoru bez schopnosti ji otestovat. |
