...
| # | Úkol | Stav | Akceptace |
|---|---|---|---|
| D2D-20 | Zpřesni a pixelově otestuj nativní mapování Opaque, AlphaBlend, NonPremultiplied a Additive, včetně Color.A, premultiplied textur a render-target zdrojů. | 🟨 | Nedekorované sprite zdroje používají `DrawImage` s explicitním `SOURCE_OVER`/`PLUS`/`SOURCE_COPY`, nikoli `DrawBitmap` závislý na mutable primitive blend state. `Direct2D_2DParity` ve Wine ověřuje premultiplied AlphaBlend, straight-alpha NonPremultiplied, `Color.A` pro oba a RT jako AlphaBlend zdroj (premultiplied pixel je do RT zapsán přes AlphaBlend nad transparentním pozadím). Nativní Windows větev doplňuje Additive/Opaque + `Color.A`, RT zdroj pro všechny čtyři presety a `Color.A`/NonPremultiplied effect graph RT zdroje. WineD3D ignoruje `PLUS`/`SOURCE_COPY` a neregistruje `ColorMatrix`, proto jejich byte-exact dokončení zůstává nativní Windows gate. |
