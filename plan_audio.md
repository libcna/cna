...
* [ ] P10-AUDIT-002/003: Full per-member (property/method/constructor/enum-value/exception)
  ...
  *Status:* **Open, not attempted at full granularity in this pass.** What already exists and
  partially satisfies this: `docs/xna-4-api-coverage.md`'s Audio section has a **class-level**
  compatibility table (Implemented / Approximate / Intentionally unsupported / Not yet
  implemented) and a **property-level** table specifically for `Apply3D`'s three positional
  effects (pan/attenuation/Doppler). Neither is a full per-member table for every class (e.g.
  `AudioEngine`'s dozen-odd members, `SoundEffect`'s five constructors, etc.). Building that full
  table is a genuinely large, mechanical task on its own; recorded here as future work rather than
  fabricated. Recommended approach when picked up: one row per public member, generated by walking
  each header in P10-AUDIT-001's table alongside FNA's matching `.cs` file.
