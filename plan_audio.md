...
- [ ] **AUD-02-007 [P0]** Check and propagate `SDL_CreateAudioStream` failure. **Acceptance:** No public object enters a usable/playing state with a null stream.
- [ ] **AUD-02-008 [P0]** Check and propagate every `SDL_PutAudioStreamData` failure. **Acceptance:** Failed chunks are not counted as submitted; state and callback behavior remain consistent.
- [ ] **AUD-02-009 [P0]** Check and propagate every `MIX_PlayTrack` failure. **Acceptance:** Public state remains Stopped and failure includes SDL error context.
...
- [ ] **AUD-06-001 [P0]** Create an authoritative XNB SoundEffect format support matrix by XNA target platform/profile. **Acceptance:** Every accepted/rejected combination has a reference fixture and rationale.
- [ ] **AUD-06-002 [P0]** Preserve `nAvgBytesPerSec`, `nBlockAlign`, and full format extension data. **Acceptance:** Decoder receives complete WAVEFORMATEX metadata.
- [ ] **AUD-06-003 [P0]** Refactor XNB SoundEffect construction away from the PCM16-only raw constructor. **Acceptance:** A format-aware internal path owns metadata and encoded bytes safely.
- [ ] **AUD-06-004 [P0]** Support 8-bit PCM XNB SoundEffect where reference behavior requires it. **Acceptance:** Included fixture decodes with exact frame count and correct unsigned-to-signed conversion.
- [ ] **AUD-06-005 [P0]** Support 16-bit PCM XNB SoundEffect without regression. **Acceptance:** Existing fixture remains sample/duration correct.
- [ ] **AUD-06-006 [P0]** Support MS ADPCM XNB SoundEffect where reference behavior requires it. **Acceptance:** Block alignment, samples/block, duration, loops, and decoded output are verified.
- [ ] **AUD-06-007 [P0]** Determine and implement IMA ADPCM XNB compatibility policy. **Acceptance:** Decision is based on real XNA/MonoGame/FNA content, with fixture and explicit behavior.
- [ ] **AUD-06-008 [P0]** Determine and implement IEEE float XNB compatibility policy. **Acceptance:** Float fixture either loads correctly or fails with documented profile-compatible reason.
- [ ] **AUD-06-009 [P0]** Determine and implement XMA2 compatibility strategy. **Acceptance:** Native decode, conversion-at-build, optional decoder, or explicit unsupported result is tested and documented.
- [ ] **AUD-06-010 [P0]** Use stored XNB duration as a validation oracle. **Acceptance:** Large decoded-duration disagreement fails with asset-specific diagnostics.
...
- [ ] **AUD-07-001 [P0]** Define whether one DynamicSoundEffectInstance may switch between S16 and float modes. **Acceptance:** Behavior is verified against FNA extension semantics and documented.
- [ ] **AUD-07-002 [P0]** Prevent S16 submission into an F32-configured stream. **Acceptance:** Regression test covers float→stop→integer submission.
...
- [ ] **AUD-07-004 [P0]** Validate constructor sample rate and channels. **Acceptance:** Invalid metadata fails before stream creation.
...
- [ ] **AUD-07-007 [P0]** Check `SDL_CreateAudioStream` and preserve backend error. **Acceptance:** Null stream cannot proceed to track setup.
...
- [ ] **AUD-07-009 [P0]** Check `SDL_PutAudioStreamData`; retain failed chunks or fail deterministically. **Acceptance:** No data loss and no false submitted count.
...
- [ ] **AUD-11-008 [P0]** Golden-test MS ADPCM wave-bank entries. **Acceptance:** Decode, block alignment, tail samples, duration, and loops pass.
