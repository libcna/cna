...
- [ ] **AUD-06-013 [P0]** Validate sample rate, channels, block align, and bits coherently. **Acceptance:** Impossible WAVEFORMATEX combinations are rejected before decoding. **Partial evidence, left unchecked:** `nChannels` is already validated (only mono/stereo accepted, pre-existing). New test `Pcm8WithZeroSampleRateFailsCleanlyRatherThanCrashing` confirms `nSamplesPerSec=0` for a WAV-wrapped format fails deterministically (`System::NotSupportedException`, propagated cleanly from SDL3's own WAV loader validation, "Invalid sample rate") rather than crashing or silently constructing a garbage `SoundEffect` -- matches FNA's own reader, which has no C#-level sample-rate check either (same resolved-decision pattern as `AUD-05-001`). Not yet done: an explicit `nBlockAlign`-vs-`channels`/`wBitsPerSample` coherence check at the CNA layer (SDL3's own WAV validation likely also catches gross mismatches here, but this hasn't been specifically probed/tested the way sample rate was).
