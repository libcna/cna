...
> ✅ **WebGPU's native 2D baseline is complete, and it now has real GPU readback (2026-07-12).**
> `WEBGPU-124`–`WEBGPU-131` (2D baseline) and `WEBGPU-88`–`91` (automated CTest coverage + reusable
> readback) are all ✅. `WebGPUGraphicsBackend::ReadBackbuffer()` no longer throws — it renders any
> pending `Clear()`/`SpriteBatch` work on demand (a `Present()` refactor, `EnsureFrameRendered()`)
> and maps a row-aligned staging buffer, matching Vulkan/Bgfx's own on-demand-submit readback
> semantics; new `WebGPU_Clear_Readback` CTest, git-stash-verified discriminating. `WEBGPU-92`
> (pixel-asserted 2D tests) is 🟨 partial: colour/alpha=0/source-rectangle are pixel-asserted;
> partial-alpha blend formula and sampler address-mode still are not. This is an autonomous long
> session (started 2026-07-12): continuing into `WEBGPU-92`/`99` and, time permitting, the 3D
> backlog (Phases 57–66) per `plan_webgpu.md`'s own active execution order. Still treat
> browser/Emscripten support as unstarted, and 3D/effects/render-targets/MRT as not yet implemented
> until their own tasks close. mobile-eggbert commit `dcdb648` (WEBGPU-130) remains **local, not
> pushed** — see §3.
