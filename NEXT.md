...
> `Clear()`/`Present()`/draws remain honest throws. **Next concrete step: `DX-111`** (port D3D11's
> own Phase DX8 shader/effect variant set — should be substantially cheaper since the shading math
> and cbuffer layouts are already solved).
...
> vanilla Wine's own `dxgi.dll` — see `plan_dx.md`'s `DX-100` row for the full evidence.
> **`DX-101` (full D3D12 CMake wiring) also closed 2026-07-14**: `CNA_GRAPHICS_BACKEND=D3D12` is
> now a real, working CMake option (`cmake-build-d3d12`, MinGW-w64 cross-build, `CNA` links clean —
> real build proof, not assumed); links only `d3d12`+`dxgi` (`DX-100`'s own confirmed minimum, no
> `dxguid`/`d3dcompiler`). New `D3D12GraphicsBackend` skeleton implements all 21 `IGraphicsBackend`
> pure virtuals with honest throws — no real D3D12 API calls yet. `DX-102` (device/command-queue/
> swap-chain, off-screen-first per `DX-100`'s own recommendation) is the next concrete step.
...
| `cmake-build-d3d12` | D3D12 (Windows cross-compile, MinGW-w64) | **Updated 2026-07-14 (DX-109/110)**: `CNA` and both D3D12 backend targets build clean. Device-lifetime resources (DX-102-105), resource-barrier state tracking, a PSO cache, a root-signature cache (DX-106-108), and now real `D3D12VertexBufferBackend`/`D3D12IndexBufferBackend`/`D3D12TextureBackend` (DX-109, buffers+2D textures only — render targets/cube/3D textures honestly deferred) plus a real, tested `RecreateDeviceEXT()` device-removed recovery path (DX-110) are all REAL — `Clear()`/`Present()`/draws still honestly throw (`DX-111` onward: shader/effect variant port). `D3D12_Smoke` CTest: **48/48 checks pass** through Wine+vkd3d-proton (off-screen only), 3 consecutive runs all green. Swap-chain creation is implemented but crashes under this Wine setup (real, evidenced, see `plan_dx.md` `DX-102`'s row) — a by-hand diagnostic exists (`cna_diag_d3d12_swapchain`, not a CTest) to reproduce it. |
...
| D3D12 | Not applicable yet (no `CnaTests` target for this backend) | **1/1 pass, 48/48 checks** (`D3D12_Smoke`: off-screen device/queue/heaps/command-lists/fence + resource-barrier tracking + root-signature cache + pipeline-state-object cache + real vertex/index/texture round-trips + device-removed recreation), verified 2026-07-14 via `ctest --test-dir cmake-build-d3d12 -R D3D12` |
...
- **D3D11**: **Every stock 3D shader variant now has a real, pixel-verified draw pipeline**
  ...
  gap; a real, tested device-removed recreation path) are all closed 2026-07-14 too; `DX-111` (port
  D3D11's own Phase DX8 shader/effect variant set) is next, see §8/§9.
...
- **D3D12**: `DX-100`–`DX-110` are all closed (device/queue/heaps/command-lists/fence/barriers/PSOs/
  root-signatures/vertex-index-buffers/2D-textures/device-removed-recovery all real, `D3D12_Smoke`
  48/48 checks); render targets/cube/3D textures (`DX-109`'s honest partial) and `DX-111` onward
  (shader/effect variants, needed before any real draw) are still unimplemented — `Clear()`/
  `Present()`/draws still honestly throw.
...
`dxgi.dll` — see `plan_dx.md`'s `DX-100` row for the full evidence. **`DX-101` through `DX-110` are
all closed (2026-07-14)** — `CNA_GRAPHICS_BACKEND=D3D12` builds clean (`cmake-build-d3d12`); real
device/queue/descriptor-heaps/command-lists/fence (`DX-102`–`105`); real per-resource
`D3D12_RESOURCE_STATES` barrier tracking, a real root-signature cache, and a real pipeline-state-
object cache — the first real `ID3D12PipelineState` this backend has created (`DX-106`–`108`); real
vertex/index buffers + 2D textures with byte-exact GPU round-trip proof, and a real, tested
device-removed recreation path (`DX-109`/`DX-110`, honest 🟨 — render targets/cube/3D textures
deliberately triaged out). `D3D12_Smoke` CTest: 48/48 checks. `Clear()`/`Present()`/draws still
honestly throw. **The concrete next step is `DX-111`** (port D3D11's own Phase DX8 shader/effect
variant set, reusing `D3DCommon`'s HLSL/DXBC — should be substantially cheaper than D3D11's own
Phase DX8 since the shading math and cbuffer layouts are already solved), continuing to build
off-screen/swap-chain-free per `DX-100`'s own recommendation, and deferring swap-chain/`Present()`
proof to a real-Windows/CI-VM pass much earlier than D3D11's `DX-90` did. Outside the D3D plan, the standing backlog
...
1. **Phase DX12 (D3D12), continue with `DX-111`** (`plan_dx.md`) — `DX-100` through `DX-110` are all
   closed (2026-07-14): real `ID3D12Device`/`ID3D12CommandQueue`/descriptor heaps/command-lists/fence
   (`DX-102`–`105`), a real per-resource `D3D12ResourceStateTracker`, a real `D3D12RootSignatureCache`
   (keyed by binding shape), a real `D3D12PipelineStateCache` — **the first real `ID3D12PipelineState`
   this backend has created**, for `colored3d`/stride-16 (`DX-106`–`108`) — and now real
   `D3D12VertexBufferBackend`/`D3D12IndexBufferBackend`/`D3D12TextureBackend` (byte-exact GPU
   round-trip proven via a `D3D12_HEAP_TYPE_READBACK` buffer) plus a real, tested `RecreateDeviceEXT()`
   device-removed recovery path (`DX-109`/`DX-110`, honest 🟨 — render targets/cube/3D textures
   deliberately triaged out, they're not `DX-111`'s prerequisite) — all proven via `D3D12_Smoke`
   (48/48 checks, off-screen only, 3 consecutive green runs). Swap-chain creation is implemented for
   real but crashes under this Wine setup (a null-pointer read inside Wine's own `dxgi.dll`,
   reproduced with a full backtrace via the by-hand `cna_diag_d3d12_swapchain` diagnostic — not a
   CTest, since it always crashes here). **Next step: `DX-111`** (port the same shader/effect variant
   set D3D11 lands in Phase DX8, reusing `D3DCommon`'s HLSL sources and DXBC bytecode — should be
   substantially cheaper than D3D11's own Phase DX8 since the shading math/cbuffer layout are already
   solved, this is D3D12 command/resource plumbing only), then `DX-112` (SpriteBatch, matching D3D11's
   `DX-70`–`72`). Already authorized — no go-ahead needed, but see §9 for what's still off-limits.
...
this work. Phase DX12 (D3D12) is authorized and underway -- DX-100 through DX-110 are all closed
...
honest partial: render targets/cube/3D textures deliberately triaged out; swap-chain creation
crashes/fails under vanilla Wine's dxgi.dll, see DX-100's/DX-102's own row Notes;
CNA_GRAPHICS_BACKEND=D3D12 builds clean, D3D12_Smoke passes 48/48 checks). Continue with DX-111
(port D3D11's own Phase DX8 shader/effect variant set, reusing D3DCommon's HLSL/DXBC) and proceed
