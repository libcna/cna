...
(a shared `HasRealDepthBuffer` backend query) — continue at Task 709, and/or the untriaged 421–500 range of
...
different, Task 707 confirmed `GetBackBufferData` genuinely reads the real backbuffer after
unbinding (both no bugs found), and Task 708 decided SDL_Renderer silently ignores requested
`DepthFormat` — found+fixed one more real bug along the way — continue at Task 709 — write pixel tests for
