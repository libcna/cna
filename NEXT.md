...
permission-classifier stops in the prior session — see section 3). **Tasks 11.1-11.4
(skeleton, body, materials, facial morphs) are done; Task 11.5 (`generate_hair.py`/
`generate_clothes.py`) is next** — see section 8.
...
- **Task 11.4 done (this session):** `tools/avatar_builder/generate_morphs.py` adds
...
`plan_net.md`. Tasks 11.1-11.4 (canonical skeleton, procedural body, placeholder materials,
facial morphs) are done — `tools/avatar_builder/` has `generate_skeleton.py`,
`generate_body.py`, `generate_materials.py`, `generate_morphs.py`, and `README.md`. The next
session's job is to begin Task 11.5 (`generate_hair.py` + `generate_clothes.py`).
...
4. ~~**Task 11.4 — `tools/avatar_builder/generate_morphs.py`.**~~ **Done this session.**
...
5. **Task 11.5 — `tools/avatar_builder/generate_hair.py` + `generate_clothes.py`. (next task)**
   Goal: simple placeholder hair (basic cap/helmet-like shape) and clothes (shirt/pants/shoes as
   offset shells over the body). Expected to look crude — a known, accepted limitation of this
   first pass, not a bug to chase.
   Files: new `tools/avatar_builder/generate_hair.py`, `tools/avatar_builder/generate_clothes.py`.
   Verify: run headless, confirm the new mesh objects exist and are parented/positioned
   reasonably over the body.

6. **Task 11.6 — `tools/avatar_builder/generate_animations.py`.**
...
Task 11.5 (tools/avatar_builder/generate_hair.py and generate_clothes.py do not exist yet -
this is a from-scratch first step; generate_skeleton.py/generate_body.py/
generate_materials.py/generate_morphs.py, Tasks 11.1-11.4, are done - import
build_skeleton()/build_body()/build_materials()/build_morphs() from them rather than
re-deriving skeleton/body/material/morph data). Do not refactor unrelated code. Do not
re-attempt MakeHuman or CharMorph automation. Make one small, verified improvement (Task
11.5 from plan_net.md's Phase 11), run the verification command for that task, and update
NEXT.md after finishing.

Build: cmake --build cmake-build-debug --target CnaTests
Test:  cmake-build-debug/CnaTests
Blender: blender --background --python tools/avatar_builder/generate_hair.py
         blender --background --python tools/avatar_builder/generate_clothes.py
```
