...
permission-classifier stops in the prior session — see section 3). **Tasks 11.1 (canonical
skeleton) and 11.2 (procedural body) are done; Task 11.3 (`generate_materials.py`) is next** —
see section 8.
...
- **Task 11.2 done (this session):** `tools/avatar_builder/generate_body.py` builds one
...
`plan_net.md`. Tasks 11.1 (canonical skeleton) and 11.2 (procedural body) are done —
`tools/avatar_builder/` has `generate_skeleton.py`, `generate_body.py`, and `README.md`. The
next session's job is to begin Task 11.3 (`generate_materials.py`).
...
2. ~~**Task 11.2 — `tools/avatar_builder/generate_body.py`.**~~ **Done this session.**
...
3. **Task 11.3 — `tools/avatar_builder/generate_materials.py`. (next task)**
   Goal: minimal PBR materials (skin, hair, shirt, pants, shoes), each a simple base-color
   parameter, no texture painting yet.
   Files: new `tools/avatar_builder/generate_materials.py`.
   Verify: run headless, confirm the expected named materials exist and are assigned to the body
   mesh (or the relevant sub-regions/material slots).

4. **Task 11.4 — `tools/avatar_builder/generate_morphs.py`.**
...
Task 11.3 (tools/avatar_builder/generate_materials.py does not exist yet - this is a
from-scratch first step; generate_skeleton.py/generate_body.py, Tasks 11.1/11.2, are done -
import build_skeleton()/build_body() from them rather than re-deriving skeleton/body geometry).
Do not refactor unrelated code. Do not re-attempt MakeHuman or CharMorph automation. Make one
small, verified improvement (Task 11.3 from plan_net.md's Phase 11), run the verification
...
Blender: blender --background --python tools/avatar_builder/generate_materials.py
```
