...
| Confirmed bug, not fixed | `SkinnedEffect` `DirectionalLight1`/`DirectionalLight2` are unforwarded; has zero specular implementation; `WeightsPerVertex` is a GPU no-op. | 893/894/895 |
...
2. ~~Fix Task 890~~ — **done**, see §3. **Fix Task 893/894/895** (`SkinnedEffect` multi-light/
   specular/`WeightsPerVertex` gaps — same family as the now-closed Tasks 889/890/891, see §2
   "Does NOT work yet").
