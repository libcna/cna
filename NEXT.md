...
- **Current development phase:** Phase 32 (Texture2D completeness, Tasks 261–270) is **fully
  ...
  tests), 274 (`Texture3D` partial box x/y/z readback tests), and 275 (`TextureCube` partial rect +
  startIndex tests, all six faces) done. Phases 30 and 31 were already complete. A new Task 663 was
  added (unnumbered-sequence, matching existing precedent) for a severe finding from Task 272 —
  see §3. Next up: Task 276 (`TextureCube` mip-level tests for all six faces).
...
663) — not a build/test blocker, but worth flagging prominently since it fails silently.
