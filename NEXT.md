...
permission-classifier stops in the prior session — see section 3). **Tasks 11.1-11.3
(skeleton, body, materials) are done; Task 11.4 (`generate_morphs.py`) is next** — see
section 8.
...
- **Task 11.3 done (this session):** `tools/avatar_builder/generate_materials.py` builds
...
`plan_net.md`. Tasks 11.1-11.3 (canonical skeleton, procedural body, placeholder materials) are
done — `tools/avatar_builder/` has `generate_skeleton.py`, `generate_body.py`,
`generate_materials.py`, and `README.md`. The next session's job is to begin Task 11.4
(`generate_morphs.py`).
...
3. ~~**Task 11.3 — `tools/avatar_builder/generate_materials.py`.**~~ **Done this session.**
...
4. **Task 11.4 — `tools/avatar_builder/generate_morphs.py`. (next task)**
   Goal: at least two shape keys, `Smile` and `Blink`, plus documented notes on adding more later.
   Files: new `tools/avatar_builder/generate_morphs.py`.
   Verify: run headless, confirm both shape keys exist on the body mesh.
...
Task 11.4 (tools/avatar_builder/generate_morphs.py does not exist yet - this is a from-scratch
first step; generate_skeleton.py/generate_body.py/generate_materials.py, Tasks 11.1-11.3, are
done - import build_skeleton()/build_body()/build_materials() from them rather than
re-deriving skeleton/body/material data). Do not refactor unrelated code. Do not re-attempt
MakeHuman or CharMorph automation. Make one small, verified improvement (Task 11.4 from
plan_net.md's Phase 11), run the verification command for that task, and update NEXT.md
after finishing.

Build: cmake --build cmake-build-debug --target CnaTests
Test:  cmake-build-debug/CnaTests
Blender: blender --background --python tools/avatar_builder/generate_morphs.py
```
