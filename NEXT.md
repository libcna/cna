...
permission-classifier stops in the prior session — see section 3). **Task 11.1 (canonical
skeleton) is done; Task 11.2 (`generate_body.py`) is next** — see section 8.
...
- **Task 11.1 done (this session):** `tools/avatar_builder/generate_skeleton.py` builds
...
`plan_net.md`. Task 11.1 (canonical skeleton) is done — `tools/avatar_builder/` exists with
`generate_skeleton.py` and `README.md`. The next session's job is to begin Task 11.2
(`generate_body.py`).
...
1. ~~**Task 11.1 — `tools/avatar_builder/generate_skeleton.py`.**~~ **Done this session.**
...
2. **Task 11.2 — `tools/avatar_builder/generate_body.py`. (next task)**
...
Task 11.2 (tools/avatar_builder/generate_body.py does not exist yet - this is a from-scratch
first step; import build_skeleton()/BONES from generate_skeleton.py, Task 11.1, done). Do not
refactor unrelated code. Do not re-attempt MakeHuman or CharMorph automation. Make one small,
verified improvement (Task 11.2 from plan_net.md's Phase 11), run the verification command for
that task, and update NEXT.md after finishing.

Build: cmake --build cmake-build-debug --target CnaTests
Test:  cmake-build-debug/CnaTests
Blender: blender --background --python tools/avatar_builder/generate_body.py
```
