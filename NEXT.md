...
- **Software backend Phase S9 closed** (`SOFTWARE-80`–`84`, 2026-07-13) — a separate track from the
  WebGPU/main-graphics work above; full detail lives in `plan_software.md`/`docs/software-backend.md`,
  not here. `CNA_GRAPHICS_BACKEND=SOFTWARE` (the CPU rasterizer, Phases S1–S8 already landed
  2026-07-13) gained: real bilinear texture sampling (`SOFTWARE-80`); real backface culling
  respecting `RasterizerState.CullMode`, including on `SpriteBatch` quads (`SOFTWARE-81`); real
  Sutherland-Hodgman near-plane polygon clipping replacing whole-triangle culling (`SOFTWARE-83`);
  `DualTextureEffect`/`EnvironmentMapEffect` (real cube-map storage + reflection vector, new
  `SoftwareTextureCubeBackend`)/`SkinnedEffect` (real bone-transform skinning, new stride-32/52
  vertex dispatch) support, deliberately without any per-light diffuse lighting for any effect
  (`SOFTWARE-82`); and a manually-run cross-backend diagnostic tool confirming `SOFTWARE` matches
  `EASYGL` within a max per-channel pixel diff of 1 on a canonical scene, closing the `SOFTWARE-61`
  stretch goal (`SOFTWARE-84`). 6 CTests, 29 checks, all green; full `CnaTests` unchanged at
  4371/4373 after every step. `SOFTWARE-85` (blit-to-window) stays explicitly declined,
  `SOFTWARE-86` (performance) stays explicitly out of scope. Along the way, fixed 2 stale
  `plan_software.md` rows (`SOFTWARE-22` left at 🟨 since Phase S2; the "Boundaries" section still
  claiming these 3 effects were out of scope after `SOFTWARE-82` had already implemented them).
...
| Commit | Task | Summary |
|---|---|---|
| `630b95fb`/`12e9992c` | SOFTWARE-22 (doc) | Docs-only: fixed `plan_software.md`'s "Boundaries" section, which still claimed `DualTextureEffect`/`EnvironmentMapEffect`/`SkinnedEffect` and cube textures were "explicitly out of scope for v1" after `SOFTWARE-82` (below) had already implemented all three — a real contradiction, not just an oversight. Also closed a stale `SOFTWARE-22` row left at 🟨 since Phase S2. |
| `7ab79456` | SOFTWARE-84 | Software backend **cross-backend diagnostic tool**, closing the `SOFTWARE-61` stretch goal. `CNA_GRAPHICS_BACKEND` is a compile-time choice, so this isn't a single `ctest` — a shared, backend-agnostic scene source (`examples/cross_backend_diagnostic_scene.cpp`, an unlit vertex-color triangle) is built once per backend (`cna_diag_software`/`cna_diag_easygl`), dumping a raw 64x64 RGBA8 file; a standalone comparator (`cna_diag_compare`, no CNA dependency) diffs two dumps within a tolerance. Actually run end-to-end this session (real desktop `:0` session): `SOFTWARE` vs. `EASYGL` gave a max per-channel diff of 1 (mean 0.139) — effectively identical. Comparator verified to genuinely fail on a deliberately corrupted dump (max diff 255). See `docs/software-backend.md`'s "Cross-backend diagnostic" section for the exact invocation. |
| `b4dd22e0` | SOFTWARE-82 | Software backend **`DualTextureEffect`/`EnvironmentMapEffect`/`SkinnedEffect` support**, deliberately without any per-light diffuse lighting for any of the three (kept consistent with this backend's own existing "no lighting engine in v1" stance) — the "lit" base color stays `vertexColor*diffuseColor*texture0` for all of them, same as the plain `BasicEffect` path. `DualTextureEffect`: real second-texture sampling, FNA's `color.rgb*=2; color*=overlay*diffuse` formula, both textures reusing the same UV (matches this codebase's own Vulkan `dual_texture3d` shader precedent). `EnvironmentMapEffect`: a new `SoftwareTextureCubeBackend` gives `CreateTextureCube` real 6-face RGBA8 storage for the first time; a new 32-byte `VertexPositionNormalTexture` stride carries a per-vertex normal; world-space position/normal go through a new `ApplyAffineColumnMajor()` helper (`World` directly, not `WorldInverseTranspose` — a documented simplification); the reflection vector is sampled against the cube map via a new `SampleCubeMap()` (standard face-select + UV projection, nearest-neighbor only); Fresnel weighting and the specular tint match FNA's `PSEnvMapSpecular` formula. `SkinnedEffect`: a new 52-byte `VertexPositionNormalTextureSkinned` stride (this project's own existing canonical skinned-vertex layout) is read; up to `WeightsPerVertex` bone matrices are blended and applied to the vertex position before `World*View*Projection`. New `Software_DualEnvmapSkinned` CTest (4/4, all passed on the first run). |
| `d670cbc4` | SOFTWARE-83 | Software backend **real near-plane polygon clipping**, replacing the old "cull the whole triangle if any vertex has `clip.W<=~0`" behavior with real Sutherland-Hodgman clipping (a new `ClipVertex`/`ClipTriangleNearPlane()`/`ClipVertexToRasterVertex()`), preserving winding so backface culling (`SOFTWARE-81`) stays correct on the result. New `Software_Clipping` CTest (4/4) checks that each surviving, non-clipped vertex's own screen projection stays correct and total red-pixel count is bounded, rather than asserting exact colors at the (correctly) enormous clip-vertex coordinates a near-eye-plane clip produces. |
| `7b844091` | SOFTWARE-81 | Software backend **backface culling** — `ApplyRasterizerState()` now stores the real `CullMode`; a new `ShouldCullTriangle()` check (screen-space signed-area sign) is applied in the rasterizer, including for `SpriteBatch` quads (matching real FNA, whose `SpriteBatch` defaults to `CullCounterClockwise`, not `CullNone`). The area-sign-to-winding mapping was derived by hand then verified empirically with a new `Software_Culling` CTest (5/5) rather than trusted on paper alone. Existing `software_rasterizer_test.cpp`/`software_effects_test.cpp` triangles turned out to be counter-clockwise-as-displayed (would be newly culled under the real default) — both now explicitly set `RasterizerState::CullNone`. |
| `a818bca3` | SOFTWARE-80 | Software backend **bilinear texture sampling** (was nearest-neighbor only). Caught and fixed a real clamp-order bug during implementation: the first version clamped `x0`/`y0` and then computed `x1=clamp(x0+1,...)` *from the already-clamped* `x0`, shifting the second sample to the wrong texel right at a texture edge — caught by `Software_Effects`' own corner-sampling check. |
