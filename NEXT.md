...
> ✅ **WebGPU 2D baseline + readback are complete; 3D strides 16/20/24/32 + AlphaTestEffect all
> work (2026-07-12).** `WEBGPU-124`–`131` (2D) and `WEBGPU-88`–`92` (readback + pixel tests,
> `WEBGPU-92` 🟨 only sampler address-mode left) are done — see §3 for the real bugs found+fixed
> along the way (`WEBGPU-132` SpriteBatch alpha blend, `ApplyDepthStencilState` unimplemented, a
> zero-direction-`normalize()` NaN-poisoning risk in `lit_textured3d.wgsl`'s lighting math, and a
> WGSL grammar gap around unparenthesized `<`/`>` in `select()`). **3D**: stride 16
> (`colored3d.wgsl`, unlit/untextured, real depth test), stride 20 (`textured3d.wgsl`, real texture
> sampling), stride 24 (`colored_textured3d.wgsl`, both at once), stride 32 (`lit_textured3d.wgsl`,
> real Blinn-Phong lighting — 3 directional lights, ambient, specular, emissive, ported from FNA's
> `Lighting.fxh` via Vulkan's own shader), and `AlphaTestEffect`'s per-pixel discard on strides
> 20/24/32 (`alpha_test3d.wgsl`, priority over the other stride dispatches) all work end-to-end
> through `DrawPrimitivesEx`/`DrawIndexedPrimitivesEx` with real `GpuDrawParams` (`DiffuseColor`
> genuinely multiplies, not just passed through). Six new CTests (`WebGPU_Colored3D`,
> `WebGPU_DrawPrimitivesEx`, `WebGPU_Textured3D`, `WebGPU_ColoredTextured3D`, `WebGPU_LitTextured3D`,
> `WebGPU_AlphaTest3D`), all git-stash-verified discriminating — see §3. Dual texture/env map/skinned
> still fall back to the colour-only path. **Recommended next step**: `dual_texture3d.wgsl` (a
> genuinely new bind-group shape — a SECOND texture, unlike everything done so far which reuses a
> single texture binding), then `env_map3d.wgsl` (cubemap sampling) and `skinned3d.wgsl`
> (bone-matrix array UBO) — each is architecturally distinct from the "one UBO/one texture" shape
> all 5 shaders so far have shared.
