...
| Commit | Task | Summary |
|---|---|---|
| `pending` | 709 | **Real bug found and fixed — `SetRenderTargets` silently bound only the first target and ignored the rest, plus a process-methodology finding correcting the SDL_Renderer baseline for Tasks 704-708.** `SdlGraphicsBackend` never overrode `SetRenderTargets`, so the shared default (`SetRenderTarget2D(rts[0])`) ran unmodified for `count > 1` — silently dropping all but the first target with no error. Fixed by overriding it to throw a clear `std::runtime_error` for `count > 1`, matching this backend's single-active-render-target reality. **Process finding**: `SetRenderTargets_FourTargets_DoesNotThrow`/`_OneTarget_DoesNotThrow` — 2 of the "13 known baseline failures" — were already passing again by the time this task started (Tasks 704/708 had already fixed their real cause), but this went unnoticed since only touched example targets + the `CNA` library were rebuilt before each regression, never the separate `CnaTests` binary (`ctest` doesn't build anything). **True baseline from Task 704 onward is 11, not 13** — see §5. Making `SetRenderTargets` throw for `count>1` now makes the 4-target test throw again on SDL_Renderer specifically (correctly — that test's own history, Task 881, is about the backend-agnostic `MAX_RENDERTARGET_BINDINGS=4` cap, not MRT capability); updated with a `#if defined(CNA_BACKEND_SDL_RENDERER)` conditional matching the established `TextureCubeTests.cpp` pattern. New `sdlrenderer_rendertargets_mrt_throws_test.cpp`: 2-binding call throws clearly; device remains fully usable afterward. Both pass. **Discriminating power via `git stash`**: reverting reproduced the exact predicted silent-no-throw on check 1 only; restored and reconfirmed. **Cross-backend verification**: EasyGL's `GraphicsDeviceValidationTest` suite (4 tests) 4/4 passed. **Closes the entire RenderTarget2D section (704-709).** Full regression (full rebuild first, corrected process): `ctest` — see commit for exact counts (true baseline now 11 — zero new failures). |
