...
- **Current development phase:** Phases 1–33 are complete. **Phase 34 (SurfaceFormat
  implementation matrix, `GRAPHICS_TASKS.md` Tasks 281–290) is in progress** — Tasks 281–288 done,
  Task 289 is next (see §8).
...
- **EasyGL (`cmake-build-debug`), full `ctest`:** 2020/2022 (serial `-j1`) pass. 2 pre-existing,
...
1. **`GRAPHICS_TASKS.md` Task 289 — implement/verify HDR formats: `HdrBlendable`, `Rgba1010102`,
   `Rgba64` (document fallback)**
   - Goal: continue Phase 34's SurfaceFormat matrix. Pre-checked this session: same shape as Tasks
     285–288 — `GetBlockSizeSquaredEXT`(1)/`GetFormatSizeEXT` (`Rgba1010102`=4/`Rgba64`=8/
     `HdrBlendable`=8) already correct (Task 282) with existing tests; no backend special-cases any
     of the 3; `docs/surface-format-support.md` (from Task 281) already documents the fallback
     status/required GPU format for all 3 (`HdrBlendable` noted as "alias for RGBA16F in FNA, same
     as HalfVector4"). No construction-throws test exists yet for these 3 specifically — likely
     another "add the missing test" task, not greenfield — but verify, don't just assume.
   - Files: `src/Microsoft/Xna/Framework/Graphics/Texture.cpp`,
     `tests/Microsoft/Xna/Framework/Graphics/Texture2DTests.cpp`.
   - Verification: unit tests confirming `Texture2D` construction throws `std::runtime_error` for
     `SurfaceFormat::HdrBlendable`/`Rgba1010102`/`Rgba64`, mirroring the
     `UnsupportedFormatConstructionTest` pattern from Tasks 286–288.
...
GRAPHICS_TASKS.md Tasks 281-290) is in progress: Tasks 281-288 done. All three backend builds
...
all non-textured rendering (Task 284). Tasks 285-288 all turned out to be verification/
test-coverage tasks, not greenfield implementation: CPU-side size/packing math was already correct
(from Task 282's exhaustive port) for every format checked so far; the recurring real gap was
missing throws-clearly construction tests, now added one `UnsupportedFormatConstructionTest` case
at a time (`Texture2DTests.cpp`). Task 286 additionally found its own premise half wrong -
NormalizedShort2/4 are VertexElementFormat-only in FNA, not SurfaceFormat, and were already
correctly implemented as vertex formats. Two silent-failure gaps remain open and tracked but not
...
Next task: GRAPHICS_TASKS.md Task 289 - implement/verify HDR formats HdrBlendable/Rgba1010102/
Rgba64 (document fallback). Already pre-checked this session: GetBlockSizeSquaredEXT/
GetFormatSizeEXT are already correct for all 3 (Task 282), no backend special-cases them, and
docs/surface-format-support.md already documents their fallback status (from Task 281) - so this
is very likely another "just add the missing throws-clearly test" task like 286-288, not
greenfield or a new doc. Verify this assumption, then add throws-clearly unit tests mirroring the
UnsupportedFormatConstructionTest pattern in Texture2DTests.cpp. After Task 289, only Task 290
(exhaustive every-format throw-or-work test) remains to close out Phase 34 - it may subsume/
formalize the per-format throw tests added incrementally in 286-289 into one exhaustive sweep over
all 27 SurfaceFormat values, so check its actual wording before duplicating work again.
