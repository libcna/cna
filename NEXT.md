...
| Commit | Task | Summary |
|---|---|---|
| `pending` | 705 | **Real memory-safety bug found and fixed — undefined behavior in ALL 3 `SdlSpriteBatchBackend::Draw()` overloads, latent since inception.** `RenderTarget2D`'s backend (`SdlRenderTargetBackend`) is a SIBLING class of `SdlTextureBackend`, not a subclass — but `Draw()` unconditionally did `static_cast<const SdlTextureBackend&>(texture)`. Whenever a `RenderTarget2D` is drawn via `SpriteBatch` after unbinding, this is an unchecked downcast between unrelated classes (UB). Silently "worked" on this compiler/ABI (both classes' `texture` field coincidentally lands at the same offset), so a plain pixel test can't distinguish it — built a throwaway `-DCNA_SANITIZE=address,undefined` scratchpad build specifically to catch it via UBSan's `vptr` check. Fixed by switching to the already-existing safe virtual accessors `GetNativeTexture()`/`GetWidth()`/`GetHeight()`. New `sdlrenderer_rendertarget2d_sample_test.cpp`: renders into a bound RT, unbinds, draws it back via `SpriteBatch::Draw` as a plain `Texture2D`, reads back both regions — both pass (identically with or without the fix, on a normal build). **Discriminating power via UBSan**: with the bug reverted, the identical run produced 6 distinct `vptr` errors ("object is of type ... SdlRenderTargetBackend"); restored and reconfirmed zero sanitizer errors. Full regression: same 13-failure baseline (see commit for exact counts; SDL_Renderer-specific code, no cross-backend spot-check needed). |
