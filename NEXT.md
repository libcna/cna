...
permission-classifier stops in the prior session — see section 3). **Tasks 11.1-11.8
(skeleton, body, materials, facial morphs, hair/clothes, animations, glTF export,
validation) are all done — Phase 11a's "one male + one female avatar that draws"
milestone is functionally complete.** `male_avatar.glb`/`female_avatar.glb` export
cleanly, reopen cleanly in Blender, validate clean via `validate_gltf.py`, and export is
deterministic to within 1 float32 ULP. A real elbow/sleeve tearing artifact under
bending, plus a handful of zero-weight/over-4-influence vertices, were confirmed and are
still open (not fixed). Task 11.9 (`README.md` — usage/design-rationale writeup, largely
already done incrementally across Tasks 11.1-11.8) is next** — see section 8.
...
- **Task 11.8 done (this session):** `tools/avatar_builder/validate_gltf.py` (plain
...
**No technical blocker, but a few confirmed cosmetic defects. Phase 11a's "one male + one
female avatar that draws" milestone (Tasks 11.1-11.8) is functionally complete.** Phase 11
is planned in detail in `plan_net.md`. `tools/avatar_builder/` has all of
`generate_skeleton.py`, `generate_body.py`, `generate_materials.py`, `generate_morphs.py`,
`generate_hair.py`, `generate_clothes.py`, `generate_animations.py`, `generate_avatar.py`,
`export_gltf.py`, `validate_gltf.py`, and `README.md`. Both `--gender male`/`--gender
female` export cleanly, reopen cleanly in Blender, and validate clean via
`validate_gltf.py`; export is deterministic to within 1 float32 ULP.
...
this "functional, not polished" pipeline milestone. The next session's job is to begin
Task 11.9 (`tools/avatar_builder/README.md` — a lot of its required content already exists
incrementally from Tasks 11.1-11.8; this task is mostly about making sure it reads as one
coherent document end to end, not writing from scratch).
...
2. ~~**Task 11.2 — `tools/avatar_builder/generate_body.py`.**~~ **Done.**
   ...
   vertex group for all 19 bones; an early manual pose-mode bend test (body only, no clothes)
   showed no tearing. **Update from Task 11.6:** the real bend-artifact check (clothed avatar,
   real `Wave` animation) found a genuine tear at the elbow — see item 6 below.
...
5. ~~**Task 11.5 — `tools/avatar_builder/generate_hair.py` + `generate_clothes.py`.**~~
   **Done.** `generate_clothes.py` builds `CNAAvatarShirt`/`Pants`/`Shoes` as
...
6. ~~**Task 11.6 — `tools/avatar_builder/generate_animations.py`.**~~ **Done.**
   `Stand0` (idle bob/rock, loops) and `Wave` (arm raise + elbow-fold oscillation) built as
   Blender Actions via `build_animations()`. First `Wave` attempt rotated `LowerArm.R`
   around its own length axis — an invisible twist, caught by rendering it — switched to a
   visibly-bending axis instead. Verified headless (nonzero frame ranges) and by rendering
   the actual motion. **Also did the real bend-artifact visual check deferred from Task
   11.2:** posed the clothed avatar through `Wave`'s peak fold, rendered a close-up —
   confirms a real, visible tear at the elbow (forearm/hand separate from the shirt
   sleeve). Confirmed, not fixed; out of scope for this milestone.

7. ~~**Task 11.7 — `tools/avatar_builder/export_gltf.py` + `generate_avatar.py`.**~~
   **Done.** `export_avatar(output_path, objects)` selects exactly the given
   objects and calls Blender's glTF2 exporter (GLB, per-Action animations, morphs/skins/
   Y-up). `build_avatar(gender)` clears the scene, runs every Task 11.1-11.6 builder in
   order; `--gender female` applies a coarse overall `armature_obj.scale = (0.93, 0.93,
   0.93)` (not real proportion differentiation — deferred to Task 11.13). Verified beyond
   the plan's bar: both genders reopen cleanly in Blender with correct
   objects/parenting/actions/shape keys; running the male export twice gives
   byte-identical JSON and a binary buffer differing by exactly 1 float32 ULP (solver
   rounding noise) — satisfies "byte-identical or near-identical." Also confirmed by
   direct inspection: 24 `CNAAvatarShirt` vertices exceed glTF's 4-joint limit (trimmed by
   the exporter), 32 `CNAAvatarBody` vertices have zero bone weight (covered by a
   synthetic `neutral_bone` joint). Neither breaks anything; documented as known gaps for
   a future weight-painting pass.

8. ~~**Task 11.8 — `tools/avatar_builder/validate_gltf.py`.**~~ **Done this session.**
   Plain `python3` (no Blender) — 4 checks via `pygltflib`: non-empty mesh, skin joints
   cover all 19 canonical bones, `Stand0`/`Wave` present, `Smile`/`Blink` present in some
   mesh's `extras["targetNames"]`. Required fixing `generate_skeleton.py` to lazily
   import `bpy` (inside `build_skeleton()`) instead of at module level, so `BONES` can be
   imported from plain Python without pulling in bpy. Verified beyond the happy path: ran
   against a nonexistent file, a garbage file, a copy missing `Wave`, and a copy missing
   `Blink` — each failed with a distinct correct message and exit 1. Both real
   `male_avatar.glb`/`female_avatar.glb` validate clean. **This closes out Phase 11a — the
   "one male + one female avatar that draws" milestone (Tasks 11.1-11.8) is functionally
   complete.**

9. **Task 11.9 — `tools/avatar_builder/README.md`. (next task)**
   Goal: usage instructions, the canonical skeleton bone list, design rationale (why
   procedural, why no retargeting needed), how to run each script standalone vs. via the
   top-level driver, and a clear statement of what's placeholder-quality vs. intended to
   be improved later.
   Files: `tools/avatar_builder/README.md` (already has almost all of this content,
   written incrementally across Tasks 11.1-11.8 — this task is mostly a coherence pass,
   not a from-scratch write).
   Verify: re-read the whole file top to bottom as a newcomer would; fix anything that
   assumes context from a specific task rather than standing on its own.
...
```
Read NEXT.md first, in full, before doing anything else. Task 11.9 is a documentation
coherence pass on tools/avatar_builder/README.md, not new code — almost all the required
content (usage, bone list, design rationale, per-script status, placeholder-vs-final
statements) was already written incrementally across Tasks 11.1-11.8. Re-read the whole
file top to bottom as a newcomer would (don't just skim your own prior edits) and fix
anything that assumes context from a specific task rather than standing on its own, is
now stale (e.g. any leftover "not yet implemented" language for something that's since
been done), or is scattered when it should be consolidated. Do not refactor unrelated
code. Do not re-attempt MakeHuman or CharMorph automation. Make the improvement, verify
by re-reading it cold, and update NEXT.md after finishing.

After Task 11.9, Phase 11a (Tasks 11.1-11.9, "one male + one female avatar that draws")
is fully done and Phase 11b (Task 11.10+, CNA engine integration — feeding the real
exported .glb through the existing content pipeline/AvatarRenderer, not another synthetic
fixture) can start.
