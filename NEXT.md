...
permission-classifier stops in the prior session — see section 3). **Tasks 11.1-11.7
(skeleton, body, materials, facial morphs, hair/clothes, animations, glTF export) are
done — `male_avatar.glb`/`female_avatar.glb` export cleanly, reopen cleanly in Blender,
and export is deterministic to within 1 float32 ULP. A real elbow/sleeve tearing
artifact under bending, plus a handful of zero-weight/over-4-influence vertices, were
confirmed and are still open (not fixed). Task 11.8 (`validate_gltf.py`) is next** —
see section 8.
...
- **Task 11.7 done (this session):** `tools/avatar_builder/export_gltf.py`'s
...
**No technical blocker, but a few confirmed cosmetic defects.** Phase 11 is planned in detail
in `plan_net.md`. Tasks 11.1-11.7 (canonical skeleton, procedural body, placeholder
materials, facial morphs, hair/clothes, animations, glTF export) are done —
`tools/avatar_builder/` has all of `generate_skeleton.py`, `generate_body.py`,
`generate_materials.py`, `generate_morphs.py`, `generate_hair.py`, `generate_clothes.py`,
`generate_animations.py`, `generate_avatar.py`, `export_gltf.py`, and `README.md`. Both
`--gender male`/`--gender female` export cleanly and reopen cleanly in Blender; export is
deterministic to within 1 float32 ULP.

Confirmed, not-fixed defects so far (all documented in `tools/avatar_builder/README.md`,
none of them blocking): a real elbow/sleeve tear under bending (`Wave`'s peak fold, Task
...
7. ~~**Task 11.7 — `tools/avatar_builder/export_gltf.py` + `generate_avatar.py`.**~~
   **Done this session.** `export_avatar(output_path, objects)` selects exactly the given
...
8. **Task 11.8 — `tools/avatar_builder/validate_gltf.py`. (next task)**
...
Task 11.8 (tools/avatar_builder/validate_gltf.py does not exist yet - this is a
from-scratch first step; generate_avatar.py's build_avatar()/export_gltf.py's
export_avatar(), Task 11.7, are done and already produce a working .glb — call them, or
just point validate_gltf.py at an existing /tmp/male_avatar.glb, rather than re-deriving
the build/export). pygltflib is already a project dependency (see
tools/avatar_asset_pipeline/convert_avatar.py for prior usage). Note: several confirmed,
not-fixed defects exist already (elbow/sleeve tear under bending, Task 11.6; 32 zero-weight
body vertices + 24 over-4-influence shirt vertices, Task 11.7 — see section 4) — Task
11.8's validator should probably still pass despite these (they don't make the file
invalid), don't try to "fix" them as part of writing the validator. Do not refactor
unrelated code. Do not re-attempt MakeHuman or CharMorph automation. Make one small,
verified improvement (Task 11.8 from plan_net.md's Phase 11), run the verification command
for that task, and update NEXT.md after finishing.
