...
- **Current phase:** Phase 55 already declared a qualified **~90% XNA/FNA compatibility milestone**
  ...
  **Task 946 (manually porting BloomSample's 3 HLSL shaders to GLSL, as the data point for Task
  945) was started 2026-07-11** — the reference HLSL source was located
  (`/rv/tmp/XNAGameStudio/Samples/BloomSample_4_0/BloomPostprocess/Content/*.fx`) and, while
  prototyping the workflow, uncovered and fixed a real, previously-unknown `cna_graphics` bug
  (Task 1077 — see §3): `ShaderEffect::SetUniformXxx()` never actually reached a `SpriteBatch`-driven
  draw on EasyGL. **Task 946 itself is now PAUSED pending explicit project-owner go-ahead** — do
  not resume porting the 3 shaders (`BloomExtract`/`GaussianBlur`/`BloomCombine`) or touch
  `../cna-samples` without explicit instruction. **Tasks 954/955 closed 2026-07-11** (a real cross-repo investigation of
