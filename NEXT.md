...
> ✅ **WebGPU 2D baseline + readback are complete; 3D strides 16/20/24/32 all work (2026-07-12).**
> `WEBGPU-124`–`131` (2D) and `WEBGPU-88`–`92` (readback + pixel tests, `WEBGPU-92` 🟨 only sampler
> address-mode left) are done — see §3 for the real bugs found+fixed along the way
> (`WEBGPU-132` SpriteBatch alpha blend, `ApplyDepthStencilState` unimplemented, and a
> zero-direction-`normalize()` NaN-poisoning risk in `lit_textured3d.wgsl`'s lighting math). **3D**:
> stride 16 (`colored3d.wgsl`, unlit/untextured, real depth test), stride 20 (`textured3d.wgsl`,
> real texture sampling), stride 24 (`colored_textured3d.wgsl`, both at once), and now stride 32
> (`lit_textured3d.wgsl`, real Blinn-Phong lighting — 3 directional lights, ambient, specular,
> emissive, ported from FNA's `Lighting.fxh` via Vulkan's own shader) all work end-to-end through
> `DrawPrimitivesEx`/`DrawIndexedPrimitivesEx` with real `GpuDrawParams` (`DiffuseColor` genuinely
> multiplies, not just passed through). Five new CTests (`WebGPU_Colored3D`, `WebGPU_DrawPrimitivesEx`,
> `WebGPU_Textured3D`, `WebGPU_ColoredTextured3D`, `WebGPU_LitTextured3D`), all git-stash-verified
> discriminating — see §3. Other effects (alpha test/dual texture/env map/skinned) still fall back
> to the colour-only path. **Recommended next step**: the remaining `WEBGPU-66` scope (dispatch for
> those effects), or the next Phase 58 shader (`alpha_test3d.wgsl`, `dual_texture3d.wgsl`,
> `env_map3d.wgsl`, `skinned3d.wgsl`, `instanced3d.wgsl`) — all of Phase 58's "one UBO/one bind-group
> shape" variants (strides 16/20/24/32) are now done, so the next task is a genuinely new shape
> (multi-texture or bone-matrix data) rather than reusing an already-proven pattern.
