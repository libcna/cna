...
| Commit | Task | Summary |
|---|---|---|
| `ff4846ec` | 954 | XNA culling compatibility audit (cross-repo, `../cna-samples` SimpleAnimation finding). **Framework proven correct** on all 3 backends via 2 new NDC-area-prediction reproducer tests (36/36 PASS) covering real camera/projection/transform/draw-path combos the pre-existing identity-only CullMode tests never exercised — no CNA change made, none justified. Asset-level root cause narrowed to a localized `turret_geo` mesh-conversion winding defect (not yet fixed — see `docs/xna_culling_compatibility_audit.md`). Found+reverted a real, separate Bgfx `DrawIndexedPrimitivesEx` startIndex/baseVertex bug (needs its own follow-up task). |
...
| Confirmed bug, found+reverted, needs its own task | `BgfxGraphicsBackend::DrawIndexedPrimitivesEx`'s non-wireframe path silently discards `GpuDrawParams::startIndex`/`baseVertex` (always binds the whole buffer from index/vertex 0). Not visible in any current CNA sample/test (every real `Model`/`ModelMeshPart` owns its own dedicated buffer starting at 0), but affects any genuine sub-range indexed draw. A fix was attempted (offset-aware `bgfx::setIndexBuffer`/`setVertexBuffer` overloads) but caused a worse regression (the offset draw stopped rendering at all) — reverted, not committed. See `docs/xna_culling_compatibility_audit.md` §6. | 954 |
| Open, strong evidence gathered, not yet fixed | `../cna-samples` SimpleAnimation: `turret_geo`'s own converted mesh data has a localized winding-orientation defect (a sub-region, not the whole mesh) causing a dark underside disc to incorrectly render — confirmed via live authoritative XNA 4.0 screenshot comparison. Root-caused to the asset/conversion level, not CNA's (proven-correct) culling framework. See `docs/xna_culling_compatibility_audit.md` for the full trail and recommended next steps (needs either a real XNA-content-pipeline reference of the same FBX, or further raw-FBX winding analysis, before a targeted — not blanket — index fix can be safely applied). | 954 |
