...
namespace CNA::Internal::Backends::Magnum
{
    ...
    MagnumGraphicsBackend::MagnumGraphicsBackend(SDL_Window* window,
    ...
    {
        if (window_ == nullptr)
            throw std::runtime_error("MagnumGraphicsBackend initialized with null window.");

        // The SDL window is NOT owned here -- it belongs to GraphicsDevice / the platform layer.
        // The GL context created below IS owned by this backend.
        ...
        if (sampleCount_ > 1)
        {
            ...
            // off-screen buffer this backend would have to resolve by hand every Present.
        ...
        std::cout << "CNA: Magnum backend on " << std::string{version.data(), version.size()}
    ...
    MagnumGraphicsBackend::~MagnumGraphicsBackend()
    {
        UnregisterForWindow(window_);

        // Every GL object this backend owns has to die while the context that created it is still
    ...
    void MagnumGraphicsBackend::GetPhysicalSize(int& width, int& height) const
    {
        SDL_GetWindowSize(window_, &width, &height);
    }

    void MagnumGraphicsBackend::GetLogicalSize(int& width, int& height) const
    {
        if (virtualHeight_ <= 0)
        {
    ...
    void MagnumGraphicsBackend::GetViewportSize(int& width, int& height)
    {
        GetLogicalSize(width, height);
    }

    void MagnumGraphicsBackend::SetVirtualResolution(int width, int height)
    {
        virtualWidth_ = width;
        virtualHeight_ = height;
    }

    void MagnumGraphicsBackend::SetPresentationMode(int mode)
    {
        presentationMode_ = static_cast<CnaPresentationMode>(mode);
    }

    void MagnumGraphicsBackend::SetSwapInterval(int interval)
    {
        swapInterval_ = interval;
        SDL_GL_SetSwapInterval(interval);
    }

    bool MagnumGraphicsBackend::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool MagnumGraphicsBackend::TransformLogicalToWindow(float logX, float logY,
    ...
    void MagnumGraphicsBackend::Present()
    {
    ...
    Mg::GL::AbstractFramebuffer& MagnumGraphicsBackend::GetActiveFramebuffer() const
    {
        if (bound_->mrtCount > 0 && mrtFramebuffer_)
            return *mrtFramebuffer_;
        if (auto* target = dynamic_cast<MagnumRenderTargetBackend*>(bound_->renderTarget2D))
            return target->GetFramebuffer();
        if (auto* cube = dynamic_cast<MagnumRenderTargetCubeBackend*>(bound_->renderTargetCube))
            return cube->GetFramebuffer();
        return Mg::GL::defaultFramebuffer;
    }

    Mg::Vector2i MagnumGraphicsBackend::DestinationExtent() const
    {
    ...
    bool MagnumGraphicsBackend::GetCurrentRenderTarget2DSize(int& width, int& height) const
    {
    ...
    void MagnumGraphicsBackend::BindDefaultFramebuffer()
    {
    ...
    void MagnumGraphicsBackend::FinalizeCurrentTargets()
    {
        // Nothing else in this backend invokes UnbindAsRenderTarget(), so a destination switch is
    ...
    void MagnumGraphicsBackend::SetRenderTarget2D(IRenderTargetBackend* rt)
    {
    ...
    void MagnumGraphicsBackend::SetRenderTargetCubeFace(IRenderTargetCubeBackend* rt, int face)
    {
        FinalizeCurrentTargets();
        if (rt == nullptr)
        {
    ...
    void MagnumGraphicsBackend::SetRenderTargets(const RenderTargetBindingDescriptor* renderTargets,
                                                 int count)
    {
        FinalizeCurrentTargets();
        ...
        {
            auto* target = dynamic_cast<MagnumRenderTargetBackend*>(
    ...
    std::unique_ptr<ITextureBackend> MagnumGraphicsBackend::CreateTexture(const ImageData& data)
    {
        return std::make_unique<MagnumTextureBackend>(data);
    }

    std::unique_ptr<ISpriteBatchBackend> MagnumGraphicsBackend::CreateSpriteBatch()
    {
        return std::make_unique<MagnumSpriteBatchBackend>(this);
    }

    std::unique_ptr<IOcclusionQueryBackend> MagnumGraphicsBackend::CreateOcclusionQuery()
    {
        return std::make_unique<MagnumOcclusionQueryBackend>();
    }

    std::unique_ptr<IRenderTargetBackend> MagnumGraphicsBackend::CreateRenderTarget2D(
        int w, int h, int depthFormat, bool preserveContents, bool mipMap, int multiSampleCount)
    {
        ...
        return std::make_unique<MagnumRenderTargetBackend>(w, h, depthFormat, bound_, mipMap,
                                                           multiSampleCount);
    }

    std::unique_ptr<IRenderTargetCubeBackend> MagnumGraphicsBackend::CreateRenderTargetCube(
        int size, int depthFormat, bool preserveContents, bool mipMap, int multiSampleCount)
    {
        ...
        (void)preserveContents;
        return std::make_unique<MagnumRenderTargetCubeBackend>(size, depthFormat, bound_, mipMap,
                                                               multiSampleCount);
    }

    std::unique_ptr<ITexture3DBackend> MagnumGraphicsBackend::CreateTexture3D(
        int w, int h, int depth, bool mipMap, int surfaceFormat)
    {
        return std::make_unique<MagnumTexture3DBackend>(w, h, depth, mipMap, surfaceFormat);
    }

    std::unique_ptr<ITextureCubeBackend> MagnumGraphicsBackend::CreateTextureCube(
        int size, bool mipMap, int surfaceFormat)
    {
        return std::make_unique<MagnumTextureCubeBackend>(size, mipMap, surfaceFormat);
    }

    std::unique_ptr<IEffectBackend> MagnumGraphicsBackend::CreateEffectBackend(
        const std::string& vertSrc, const std::string& fragSrc)
    {
        auto effect = std::make_unique<MagnumEffectBackend>();
    ...
    std::unique_ptr<IVertexBufferBackend> MagnumGraphicsBackend::CreateVertexBuffer(
        int vertex_capacity)
    {
        return std::make_unique<MagnumVertexBufferBackend>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferBackend> MagnumGraphicsBackend::CreateIndexBuffer16(
        int index_capacity)
    {
        return std::make_unique<MagnumIndexBufferBackend>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferBackend> MagnumGraphicsBackend::CreateIndexBuffer32(
        int index_capacity)
    {
        return std::make_unique<MagnumIndexBufferBackend>(index_capacity, true);
    }

    void MagnumGraphicsBackend::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void MagnumGraphicsBackend::ApplyColorWriteMasks()
    {
    ...
    void MagnumGraphicsBackend::ForceAllColorWriteMasks()
    {
        const int activeSlots = std::max(1, bound_->mrtCount);
        for (int slot = 0; slot < activeSlots; ++slot)
    ...
    bool MagnumGraphicsBackend::HasRestrictedColorWriteMask() const
    {
        const int activeSlots = std::max(1, bound_->mrtCount);
        for (int slot = 0; slot < activeSlots; ++slot)
        {
    ...
    void MagnumGraphicsBackend::Clear(float r, float g, float b, float a)
    {
    ...
    void MagnumGraphicsBackend::ClearDepth(float depth)
    {
    ...
    void MagnumGraphicsBackend::ClearStencil(int stencil)
    {
    ...
    void MagnumGraphicsBackend::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }

    void MagnumGraphicsBackend::ClearDepthAndStencil(float depth, int stencil)
    {
        if (!depthWriteEnabled_)
            Renderer::setDepthMask(true);
        if (stencilWriteMask_ != 0xFF)
    ...
    void MagnumGraphicsBackend::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        ClearStencil(stencil);
    }

    void MagnumGraphicsBackend::ClearColorDepthAndStencil(float r, float g, float b, float a,
    ...
    void MagnumGraphicsBackend::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
                                                ...
                                                const BlendWriteState& writeState)
    {
        ...
        // BlendState.MultiSampleMask reaches this backend but only its all-ones default is applied:
    ...
    void MagnumGraphicsBackend::ApplyDepthStencilState(
    ...
    void MagnumGraphicsBackend::ApplyRasterizerState(int cullMode, int fillMode,
    ...
    void MagnumGraphicsBackend::ApplySamplerState(int slot, int filter, int addressU, int addressV,
                                                  int maxAnisotropy)
    {
    ...
    void MagnumGraphicsBackend::BindTextureToSlot(int slot, const ITextureBackend* texture)
    {
        ...
        if (auto* plain = dynamic_cast<const MagnumTextureBackend*>(texture))
        {
            ApplySamplerStateTo(plain->GetTexture(), state, plain->GetLevelCount());
            plain->GetTexture().bind(slot);
            return;
        }
        if (auto* target = dynamic_cast<const MagnumRenderTargetBackend*>(texture))
        {
    ...
    void MagnumGraphicsBackend::SetBlendFactor(float r, float g, float b, float a)
    {
        Renderer::setBlendColor(ToMagnumColor(r, g, b, a));
    }

    void MagnumGraphicsBackend::SetReferenceStencil(int value)
    {
    ...
    void MagnumGraphicsBackend::SetScissorRect(int x, int y, int w, int h)
    {
        if (w <= 0 || h <= 0)
            return;
    ...
    void MagnumGraphicsBackend::ApplyViewport(const Mg::Range2Di& viewport)
    {
        currentViewport_ = viewport;
        GetActiveFramebuffer().setViewport(viewport);
    }

    void MagnumGraphicsBackend::SetViewport(int x, int y, int w, int h,
    ...
    void MagnumGraphicsBackend::SetDepthTestEnabled(bool enabled)
    {
        Renderer::setFeature(Renderer::Feature::DepthTest, enabled);
    }

    void MagnumGraphicsBackend::SetBlendEnabled(bool enabled)
    {
        Renderer::setFeature(Renderer::Feature::Blending, enabled);
    }

    void MagnumGraphicsBackend::SetDepthWriteEnabled(bool enabled)
    {
    ...
    MagnumProgram* MagnumGraphicsBackend::SelectProgram(std::size_t stride,
                                                        const GpuDrawParams& params)
    {
        if (params.customEffectBackend != nullptr)
        {
            if (auto* effect = dynamic_cast<MagnumEffectBackend*>(params.customEffectBackend))
                                            ...
                                            int instanceCount, const GpuDrawParams& params)
    {
        ...
        const auto* vertexBuffer = dynamic_cast<const MagnumVertexBufferBackend*>(&vb);
        ...
        // stream 0.
        const auto bindAttribute = [&mesh](const MagnumVertexBufferBackend& buffer,
    ...
    void MagnumGraphicsBackend::DrawIndexedColoredPrimitives(const IVertexBufferBackend& vb,
                                                             const IIndexBufferBackend& ib,
                                                             const Matrix& world, const Matrix& view,
                                                             const Matrix& projection,
    ...
    void MagnumGraphicsBackend::DrawPrimitivesEx(const IVertexBufferBackend& vb,
                                                 const Matrix& world, const Matrix& view,
                                                 const Matrix& projection,
                                                 PrimitiveType primitive, int primitiveCount,
                                                 const GpuDrawParams& params)
    ...
    void MagnumGraphicsBackend::DrawIndexedPrimitivesEx(const IVertexBufferBackend& vb,
                                                        const IIndexBufferBackend& ib,
                                                        const Matrix& world, const Matrix& view,
                                                        const Matrix& projection,
                                                        PrimitiveType primitive, int primitiveCount,
    ...
    void MagnumGraphicsBackend::DrawInstancedPrimitivesEx(const IVertexBufferBackend& vb,
                                                          const IIndexBufferBackend& ib,
                                                          const Matrix& world, const Matrix& view,
                                                          const Matrix& projection,
                                                          PrimitiveType primitive,
...
namespace CNA::Internal::Backends
{
    ...
    {
        return std::make_unique<Magnum::MagnumGraphicsBackend>(
