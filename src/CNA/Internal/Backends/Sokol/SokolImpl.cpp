...
// else in this backend includes the same two headers for their declarations only.
//
// Kept separate from SokolGraphicsBackend.cpp deliberately: sokol_gfx.h's implementation pulls in
// the platform's GL (or D3D11/Metal/WebGPU) headers and is a ~20k-line C body, so isolating it
// keeps it out of the backend's own incremental rebuilds and stops its macros leaking into CNA
// code.
