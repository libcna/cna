...
std::unique_ptr<IRenderTargetBackend>OpenGL1GraphicsBackend::CreateRenderTarget2D(int w,int h,int depthFormat,bool,bool mipMap,int multiSampleCount){if(!caps_.framebufferObject)return nullptr;try{return std::make_unique<OpenGL1RenderTargetBackend>(w,h,depthFormat,mipMap,multiSampleCount,RegistryIfEnabled());}catch(const std::exception&){return nullptr;}}
