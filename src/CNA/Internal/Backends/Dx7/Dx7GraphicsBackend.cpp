...
// plan_dx7.md: real DirectX 7 graphics backend -- DirectDraw v7 (IDirectDraw7/
// IDirectDrawSurface7/DDSURFACEDESC2/DDSCAPS2, created via DirectDrawCreateEx) + Direct3D v7
// (IDirect3D7/IDirect3DDevice7). Unlike DX6 (no new interface at all vs DX5), DX7 is a real
...
// DDPF_ZBUFFER|DDPF_STENCILBUFFER) is unchanged from DX6, ported verbatim -- spike-confirmed it
...
// Dx7_ExecuteBufferDiscipline CTest (scripts/check-dx7-execute-buffer-discipline.sh, plan_dx7.md
// design decision 16).
#include <ddraw.h>
#include <d3d.h>
...
namespace CNA::Internal::Backends::Dx7
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx7TextureBackend and
        // Dx7RenderTargetBackend (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
        ...
        // no texture-handle/QueryInterface indirection at all in DX7) for GpuDrawParams::texture0
        ...
        [[noreturn]] void ThrowMipLevelUnsupported(int level)
        {
            throw std::runtime_error(
                "DX7 (DirectDraw v4) does not support mip-level texture uploads (level " +
        ...
        enum class Dx7BlendMode { Opaque, AlphaBlend, NonPremultiplied, Additive };
        ...
        // found-and-fixed bug).
        Dx7BlendMode DetectBlendMode(int colorSrc, int alphaSrc, int colorDst, int alphaDst,
                                     int colorFunc, int alphaFunc)
        {
            ...
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 5 && alphaDst == 5) return Dx7BlendMode::NonPremultiplied;
            // Additive: ColorSrc=SourceAlpha, AlphaSrc=SourceAlpha, ColorDst=One, AlphaDst=One
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 0 && alphaDst == 0) return Dx7BlendMode::Additive;
            // Opaque: ColorSrc=One, AlphaSrc=One, ColorDst=Zero, AlphaDst=Zero
            if (bothAdd && colorSrc == 0 && alphaSrc == 0 && colorDst == 1 && alphaDst == 1) return Dx7BlendMode::Opaque;
            ...
            return Dx7BlendMode::AlphaBlend;
        }
        ...
        DWORD Dx7CompareFunctionToD3D(int compareFunction)
        {
        ...
        DWORD Dx7StencilOperationToD3D(int stencilOperation)
        {
        ...
        DWORD Dx7CullModeToD3D(int cullMode)
        {
        ...
        DWORD Dx7FillModeToD3D(int fillMode)
        {
        ...
        DWORD Dx7TextureAddressModeToD3D(int addressMode)
        {
        ...
        struct Dx7FilterPair { DWORD mag; DWORD min; };
        ...
        Dx7FilterPair Dx7TextureFilterToD3D(int textureFilter)
        {
        ...
        void CompositeQuad(uint8_t* dstBase, int dstPitch, int dstW, int dstH,
                           ...
                           Dx7BlendMode blendMode, int filter, int addressU, int addressV)
        {
            ...
            for (int y = y0; y < y1; ++y)
            {
                for (int x = x0; x < x1; ++x)
                {
                    ...
                    for (const auto& tri : kTriangles)
                    {
                        ...
                        case Dx7BlendMode::NonPremultiplied:
                        {
    ...
    // Common accessor so Dx7SpriteBatchBackend can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx7TextureBackend/Dx7RenderTargetBackend themselves.
    class Dx7SurfaceOwner
    {
    public:
        virtual ~Dx7SurfaceOwner() = default;
        [[nodiscard]] virtual LPDIRECTDRAWSURFACE7 Surface() const = 0;
    };

    struct Dx7GraphicsBackend::Impl
    {
        ...
        // the constructor) -- every other DirectDraw call this backend makes, and every surface it
        ...
        // CreateBackBuffer() replaces it, ported unchanged from this backend family's original
        ...
        // every prior backend in this family, there is no `viewport` member here; SetViewport/Clear
        ...
        // AddAttachedSurface, sized to match it exactly (ported unchanged from DX6).
        LPDIRECTDRAWSURFACE7 zbuffer = nullptr;

        // Phase O3: the offscreen surface owned by the currently-bound Dx7RenderTargetBackend, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx7RenderTargetBackend::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE7 rather than
        // a Dx7RenderTargetBackend* so Dx7RenderTargetBackend never needs to name this private Impl
        ...
        Dx7BlendMode currentBlendMode = Dx7BlendMode::AlphaBlend;
        ...
        void Create3DDevice(int width, int height)
        {
            ...
            // IUnknown* outer parameter DX5/DX6's IDirect3D3::CreateDevice needed -- spike-confirmed
    ...
    Dx7GraphicsBackend::Dx7GraphicsBackend(const GraphicsBackendCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx7GraphicsBackend initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx7GraphicsBackend: could not obtain a real HWND from the SDL window.");

        // plan_dx7.md design decision 3, spike-confirmed (DX7-0a2): DirectDrawCreateEx is the new,
        // correct-for-this-era DX7 entry point, requesting IID_IDirectDraw7 directly -- unlike
        // every prior backend in this family, there is no v1 DirectDrawCreate()+QueryInterface
        // upgrade chain at all (the old chain also empirically works here, DX7-0a1, but this plan
        // uses the real DX7 API instead). From here on, every surface uses v7
    ...
    Dx7GraphicsBackend::~Dx7GraphicsBackend() = default;

    void Dx7GraphicsBackend::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx7GraphicsBackend::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx7GraphicsBackend::Present: ClientToScreen failed");
    ...
    void Dx7GraphicsBackend::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx7GraphicsBackend::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx7GraphicsBackend::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx7GraphicsBackend::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx7GraphicsBackend::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx7GraphicsBackend::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx7GraphicsBackend::TransformLogicalToWindow(float logX, float logY,
    ...
    // DX7 anyway, per docs/directx-legacy-backends-analysis.md section 3.1's table).

    class Dx7VertexBufferBackend final : public IVertexBufferBackend
    {
    public:
        explicit Dx7VertexBufferBackend(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx7VertexBufferBackend::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx7VertexBufferBackend::SetData: stride_in_bytes must be > 0");
    ...
    class Dx7IndexBufferBackend final : public IIndexBufferBackend
    {
    public:
        Dx7IndexBufferBackend(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx7IndexBufferBackend: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx7IndexBufferBackend: SetData bit-width does not match the buffer's declared width");
    ...
    Dx7ClipVertex Dx7LerpClipVertex(const Dx7ClipVertex& a, const Dx7ClipVertex& b, float t)
    {
        Dx7ClipVertex out;
        out.x = a.x + t * (b.x - a.x);
        out.y = a.y + t * (b.y - a.y);
    ...
    int Dx7ClipTriangleNearPlane(const Dx7ClipVertex verts[3], Dx7ClipVertex out[4])
    {
        constexpr float kNearEpsilon = 1e-5f;
        int count = 0;
        for (int i = 0; i < 3; ++i)
        {
            const Dx7ClipVertex& cur = verts[i];
            const Dx7ClipVertex& prev = verts[(i + 2) % 3];
            const bool curIn = cur.w > kNearEpsilon;
            const bool prevIn = prev.w > kNearEpsilon;
            if (curIn != prevIn)
            {
                const float t = (kNearEpsilon - prev.w) / (cur.w - prev.w);
                out[count++] = Dx7LerpClipVertex(prev, cur, t);
            }
            if (curIn)
                out[count++] = cur;
        }
    ...
    Dx7ClipVertex Dx7BuildPositionColorClipVertex(const uint8_t* raw, const Matrix& combined)
    {
        Vector3 position;
    ...
    /// Result of Dx7ComputeVertexLighting: the diffuse (base vertex color) and specular (additive,
    ...
    Dx7LitColor Dx7ComputeVertexLighting(const Vector3& localPosition, const Vector3& localNormal,
    ...
    /// normal (32/52), Dx7ComputeVertexLighting() replaces the raw/white vertex color with real
    ...
    Dx7ClipVertex Dx7BuildGenericClipVertex(const uint8_t* raw, std::size_t stride, const Matrix& combined,
                                            const Matrix& world, const GpuDrawParams& params)
    {
        Vector3 position;
        std::memcpy(&position, raw, sizeof(Vector3));
        ...
        else if (stride == 32 || stride == 52)
        {
            ...
            if (params.lightingEnabled)
            {
                ...
                const Dx7LitColor litColor = Dx7ComputeVertexLighting(position, normal, world, params);
    ...
    D3DTLVERTEX Dx7ClipVertexToD3DTLVERTEX(const Dx7ClipVertex& cv, int viewportWidth, int viewportHeight)
    {
    ...
    /// Resolves a GpuDrawParams::texture0-style ITextureBackend* into the real IDirectDrawSurface7*
    ...
    LPDIRECTDRAWSURFACE7 Dx7ResolveTextureSurface(const ITextureBackend* texture)
    {
        if (!texture) return nullptr;
        const auto* owner = dynamic_cast<const Dx7SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx7GraphicsBackend: texture0 is not a DX7 surface (created by a different graphics backend?)");
        return owner->Surface();
    }

    /// Shared core for all 4 draw entry points: clips each triangle (Dx7ClipTriangleNearPlane),
    /// packs the result into D3DTLVERTEX (Dx7ClipVertexToD3DTLVERTEX), and submits via a single
    ...
    /// additively after the texture-modulate stage, ported unchanged from DX6);
    ...
    void SubmitDx7Primitives(LPDIRECT3DDEVICE7 device7, int viewportWidth, int viewportHeight,
                             const std::function<Dx7ClipVertex(int)>& fetchVertex,
                             int primitiveCount, LPDIRECTDRAWSURFACE7 texSurface, bool specularEnabled)
    {
        ...
        for (int i = 0; i < primitiveCount; ++i)
        {
            ...
            Dx7ClipVertex clipped[4];
            const int clippedCount = Dx7ClipTriangleNearPlane(cv, clipped);
            if (clippedCount == 0) continue;

            const auto baseIdx = static_cast<WORD>(verts.size());
            for (int k = 0; k < clippedCount; ++k)
                verts.push_back(Dx7ClipVertexToD3DTLVERTEX(clipped[k], viewportWidth, viewportHeight));
        ...
        // Real, empirically-found DX7 API restriction (not a spike-anticipated gap): Wine's
        ...
        // same diffuse*texture modulation DX2..DX6's D3DTBLEND_MODULATE always requested.
    ...
    /// target's surface).
    void Dx7CheckDrawPreconditions(const char* who, bool customRenderTargetBound,
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx7GraphicsBackend::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx7GraphicsBackend::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx7GraphicsBackend::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class Dx7TextureBackend : public ITextureBackend, public Dx7SurfaceOwner
    {
    public:
        Dx7TextureBackend(LPDIRECTDRAW7 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx7TextureBackend(LPDIRECTDRAW7 dd, const ImageData& data)
            : Dx7TextureBackend(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx7TextureBackend() override { if (surface_) surface_->Release(); }

        Dx7TextureBackend(const Dx7TextureBackend&) = delete;
        Dx7TextureBackend& operator=(const Dx7TextureBackend&) = delete;
    ...
    class Dx7RenderTargetBackend final : public IRenderTargetBackend, public Dx7SurfaceOwner
    {
    public:
        Dx7RenderTargetBackend(LPDIRECTDRAWSURFACE7* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx7RenderTargetBackend() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx7RenderTargetBackend(const Dx7RenderTargetBackend&) = delete;
        Dx7RenderTargetBackend& operator=(const Dx7RenderTargetBackend&) = delete;
    ...
    std::unique_ptr<ITextureBackend> Dx7GraphicsBackend::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx7TextureBackend>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetBackend> Dx7GraphicsBackend::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx7RenderTargetBackend>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx7GraphicsBackend::SetRenderTarget2D(IRenderTargetBackend* rt)
    {
        if (rt)
    ...
    void Dx7GraphicsBackend::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        // DirectDraw has no multi-render-target concept -- single active surface only.
        if (count > 1)
            throw std::runtime_error(
                "DX7 (DirectDraw v4) does not support multiple simultaneous render targets (MRT): "
        ...
        if (count > 0 && renderTargets[0].IsRenderTargetCubeFace())
            throw std::runtime_error(
                "DX7 (DirectDraw v7) does not support RenderTargetCube face bindings.");
    ...
    // Never named outside this .cpp, same reasoning as Dx7TextureBackend/Dx7RenderTargetBackend.

    class Dx7SpriteBatchBackend final : public ISpriteBatchBackend
    {
    public:
        Dx7SpriteBatchBackend(std::function<LPDIRECTDRAWSURFACE7()> getActiveSurface,
                              std::function<void(int&, int&)> getActiveSurfaceSize,
                              std::function<Dx7BlendMode()> getBlendMode)
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx7SpriteBatchBackend::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx7SpriteBatchBackend::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        void SetCustomEffect(Effect* effect) override
        {
            if (effect != nullptr)
                throw std::runtime_error(
                    "DX7 (DirectDraw v4) does not support custom SpriteBatch Effects: no "
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx7SpriteBatchBackend::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx7SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx7SpriteBatchBackend::Draw: texture backend is not a DX7 surface (created by "
            ...
            if (isIdentityGeometry && isWhiteTint && getBlendMode_() == Dx7BlendMode::Opaque)
            {
    ...
    private:
        ...
        std::function<Dx7BlendMode()> getBlendMode_;
        bool begun_ = false;
    ...
    std::unique_ptr<ISpriteBatchBackend> Dx7GraphicsBackend::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx7SpriteBatchBackend>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx7GraphicsBackend::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
                                             ...
                                             const BlendWriteState& /*writeState*/)
    {
        ...
        impl_->device7->SetRenderState(D3DRENDERSTATE_SRCBLEND, Dx7BlendToD3D(colorSrcBlend));
        impl_->device7->SetRenderState(D3DRENDERSTATE_DESTBLEND, Dx7BlendToD3D(colorDstBlend));
    }

    void Dx7GraphicsBackend::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
                                                    ...
                                                    int /*ccwStencilFail*/, int /*ccwStencilDepthFail*/)
    {
        ...
        impl_->device7->SetRenderState(D3DRENDERSTATE_ZFUNC, Dx7CompareFunctionToD3D(depthFunc));
        ...
        impl_->device7->SetRenderState(D3DRENDERSTATE_STENCILFUNC, Dx7CompareFunctionToD3D(stencilFunc));
        impl_->device7->SetRenderState(D3DRENDERSTATE_STENCILFAIL, Dx7StencilOperationToD3D(stencilFail));
        impl_->device7->SetRenderState(D3DRENDERSTATE_STENCILZFAIL, Dx7StencilOperationToD3D(stencilDepthFail));
        impl_->device7->SetRenderState(D3DRENDERSTATE_STENCILPASS, Dx7StencilOperationToD3D(stencilPass));
    ...
    void Dx7GraphicsBackend::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
                                                  float /*depthBias*/, float /*slopeScaleDepthBias*/)
    {
        ...
        impl_->device7->SetRenderState(D3DRENDERSTATE_CULLMODE, Dx7CullModeToD3D(cullMode));
        impl_->device7->SetRenderState(D3DRENDERSTATE_FILLMODE, Dx7FillModeToD3D(fillMode));
    }

    void Dx7GraphicsBackend::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
        ...
        const Dx7FilterPair filterPair = Dx7TextureFilterToD3D(filter);
        ...
        impl_->device7->SetRenderState(D3DRENDERSTATE_TEXTUREADDRESS, Dx7TextureAddressModeToD3D(addressU));
    ...
    void Dx7GraphicsBackend::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }
    ...
    // DX7 removed the whole viewport object DX2..DX6 needed a Clear2 call through. Spike-confirmed
    ...
    void Dx7GraphicsBackend::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    // Stencil (unchanged from DX6, ported verbatim): these clear real stencil values via the
    // device-direct Clear's own D3DCLEAR_STENCIL flag, spike-confirmed (DX7-0e/DX7-0f) to survive
    // the viewport-object removal.
    void Dx7GraphicsBackend::ClearStencil(int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
    ...
    void Dx7GraphicsBackend::ClearDepthAndStencil(float depth, int stencil)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
        D3DRECT fullRect{0, 0, static_cast<LONG>(width), static_cast<LONG>(height)};
    ...
    void Dx7GraphicsBackend::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
        Clear(r, g, b, a);
        ClearStencil(stencil);
    }
    void Dx7GraphicsBackend::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
        Clear(r, g, b, a);
        ClearDepthAndStencil(depth, stencil);
    }
    void Dx7GraphicsBackend::SetDepthTestEnabled(bool enabled)
    {
    ...
    void Dx7GraphicsBackend::SetBlendEnabled(bool) {}

    void Dx7GraphicsBackend::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device7->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferBackend> Dx7GraphicsBackend::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx7VertexBufferBackend>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferBackend> Dx7GraphicsBackend::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx7IndexBufferBackend>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferBackend> Dx7GraphicsBackend::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx7IndexBufferBackend>(index_capacity, true);
    }

    void Dx7GraphicsBackend::DrawColoredPrimitives(const IVertexBufferBackend& vb,
                                                   const Matrix& world, const Matrix& view, const Matrix& projection,
                                                   PrimitiveType primitive, int primitiveCount)
    {
        Dx7CheckDrawPreconditions("DrawColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx7GraphicsBackend::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx7VertexBufferBackend&>(vb);
        ...
        SubmitDx7Primitives(impl_->device7, vw, vh,
            [&](int i) { return Dx7BuildPositionColorClipVertex(base + static_cast<std::size_t>(i) * stride, combined); },
            primitiveCount, nullptr, false);
    }

    void Dx7GraphicsBackend::DrawIndexedColoredPrimitives(const IVertexBufferBackend& vb,
                                                          const IIndexBufferBackend& ib,
                                                          const Matrix& world, const Matrix& view, const Matrix& projection,
                                                          PrimitiveType primitive, int primitiveCount)
    {
        Dx7CheckDrawPreconditions("DrawIndexedColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx7GraphicsBackend::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx7VertexBufferBackend&>(vb);
        const auto& dxIb = static_cast<const Dx7IndexBufferBackend&>(ib);
        ...
        SubmitDx7Primitives(impl_->device7, vw, vh,
            [&](int i) {
                ...
                return Dx7BuildPositionColorClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, combined);
            },
            primitiveCount, nullptr, false);
    }
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferBackend& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::BackendRefusesIt, "DX7", route);
    }

    void Dx7GraphicsBackend::DrawPrimitivesEx(const IVertexBufferBackend& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-nonindexed");
        Dx7CheckDrawPreconditions("DrawPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx7GraphicsBackend::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx7GraphicsBackend::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx7VertexBufferBackend&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx7GraphicsBackend::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        LPDIRECTDRAWSURFACE7 texSurface = params.textureEnabled
            ? Dx7ResolveTextureSurface(params.texture0) : nullptr;
        const int vertexStart = params.vertexStart;

        SubmitDx7Primitives(impl_->device7, vw, vh,
            [&](int i) { return Dx7BuildGenericClipVertex(base + static_cast<std::size_t>(vertexStart + i) * stride,
                                                          stride, combined, world, params); },
            primitiveCount, texSurface, params.lightingEnabled);
    }

    void Dx7GraphicsBackend::DrawIndexedPrimitivesEx(const IVertexBufferBackend& vb, const IIndexBufferBackend& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-indexed");
        Dx7CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx7GraphicsBackend::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx7GraphicsBackend::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx7GraphicsBackend::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        SubmitDx7Primitives(impl_->device7, vw, vh,
            [&](int i) {
                ...
                return Dx7BuildGenericClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, stride,
...
namespace CNA::Internal::Backends
{
    ...
    {
        return std::make_unique<Dx7::Dx7GraphicsBackend>(args);
    }
#endif
}
