...
// Dx8GraphicsBackend.hpp's own comment.
#include <d3d8.h>
...
namespace CNA::Internal::Backends::Dx8
{
    ...
    namespace
    {
        ...
        // No native mip chain on IDirect3DTexture8 as created by this backend (levels=1, design
        ...
        [[noreturn]] void ThrowMipLevelUnsupported(int level)
        {
            throw std::runtime_error(
                ...
                "): this backend's IDirect3DTexture8 objects have no native mip chain. Use "
        ...
        // No mip-filter concept is mapped -- every texture this backend creates has exactly one
        ...
        // Every surface this backend creates uses an explicit D3DFMT_A8R8G8B8 format -- unlike
        // DirectDraw (which picks whatever the display negotiates), D3D8 lets this backend request
    ...
    // Common accessor so Dx8SpriteBatchBackend/the 3D draw path can reach the underlying
    // IDirect3DTexture8* of either concrete backend it might be asked to sample from (a plain
    ...
    struct Dx8GraphicsBackend::Impl
    {
        ...
        // backend renders everything into a logical-resolution offscreen render target (real
        // Direct3D8 texture+surface, same render-target-as-texture pattern as
        // Dx8RenderTargetBackend) and Present() draws THAT texture as a single full-screen quad
        ...
        // The offscreen surface owned by the currently-bound Dx8RenderTargetBackend, or nullptr
        // when no custom render target is bound (the logical render target is active). Set
        // directly by Dx8RenderTargetBackend::BindAsRenderTarget/UnbindAsRenderTarget via a
    ...
    Dx8GraphicsBackend::Dx8GraphicsBackend(const GraphicsBackendCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx8GraphicsBackend initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx8GraphicsBackend: could not obtain a real HWND from the SDL window.");
        ...
        // Design decision 3: D3DCREATE_SOFTWARE_VERTEXPROCESSING, not hardware -- this backend does
    ...
    Dx8GraphicsBackend::~Dx8GraphicsBackend() = default;

    void Dx8GraphicsBackend::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx8GraphicsBackend::GetViewportSize(int& width, int& height)
    {
        width = impl_->virtualWidth;
        height = impl_->virtualHeight;
    }

    void Dx8GraphicsBackend::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
        if (w <= 0 || h <= 0) return;
    ...
    void Dx8GraphicsBackend::SetVirtualResolution(int width, int height)
    {
        // Unlike D3D9GraphicsBackend's own vestigial handling of this field (its swap chain always
        // matches the real window, so nothing needs resizing), Dx8GraphicsBackend owns a REAL
    ...
    void Dx8GraphicsBackend::SetPresentationMode(int mode)
    {
        impl_->presentationMode = static_cast<CnaPresentationMode>(mode);
    }

    SDL_Window* Dx8GraphicsBackend::GetWindowInternal() const
    {
        return impl_->window;
    }

    // ---- Textures and render targets: real Direct3D8 resources (design decision 7) ----
    // Never named outside this .cpp (only returned polymorphically as ITextureBackend/
    // IRenderTargetBackend).

    class Dx8TextureBackend final : public ITextureBackend, public Dx8TextureOwner
    {
    public:
        Dx8TextureBackend(IDirect3DDevice8* device, const ImageData& data)
        ...
        ~Dx8TextureBackend() override { if (texture_) texture_->Release(); }
    ...
    class Dx8RenderTargetBackend final : public IRenderTargetBackend, public Dx8TextureOwner
    {
    public:
        Dx8RenderTargetBackend(IDirect3DSurface8** currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx8RenderTargetBackend() override
        {
        ...
        // matching every DirectDraw-based backend in this family's own identical boundary).
    ...
    std::unique_ptr<ITextureBackend> Dx8GraphicsBackend::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx8TextureBackend>(impl_->device8, data);
    }

    std::unique_ptr<IRenderTargetBackend> Dx8GraphicsBackend::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx8RenderTargetBackend>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->device8, w, h, multiSampleCount);
    }

    void Dx8GraphicsBackend::SetRenderTarget2D(IRenderTargetBackend* rt)
    {
        if (rt)
    ...
    // uses (each backend in this family keeps its own private copy rather than sharing one).
    ...
    void Dx8GraphicsBackend::Present()
    {
        ...
        // to Dx8SpriteBatchBackend::Draw's own quad corners).
    ...
    bool Dx8GraphicsBackend::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx8GraphicsBackend::TransformLogicalToWindow(float logX, float logY,
    ...
    class Dx8SpriteBatchBackend final : public ISpriteBatchBackend
    {
    public:
        Dx8SpriteBatchBackend(IDirect3DDevice8* device, std::function<void(int&, int&)> getActiveSurfaceSize)
        ...
        void Begin() override
        {
            if (begun_) throw std::runtime_error("Dx8SpriteBatchBackend::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        void End() override
        {
            if (!begun_) throw std::runtime_error("Dx8SpriteBatchBackend::End: End() called without a matching Begin()");
            FlushBatch();
            begun_ = false;
        }
        ...
        void SetCustomEffect(Effect* effect) override
        {
            if (effect != nullptr)
                throw std::runtime_error(
                    ...
                    "programmable shader stage is used by this backend (scope decision).");
        }
        ...
        {
            if (!begun_) throw std::runtime_error("Dx8SpriteBatchBackend::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx8TextureOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx8SpriteBatchBackend::Draw: texture backend is not a DX8 texture (created by "
            ...
            const auto placeCorner = [&](float px, float py) -> Vector2
            {
                ...
                // convention (D3D9SpriteBatchBackend bakes the same correction into its own
                // orthographic projection matrix; this backend's quad is already in screen space
    ...
    std::unique_ptr<ISpriteBatchBackend> Dx8GraphicsBackend::CreateSpriteBatch()
    {
        return std::make_unique<Dx8SpriteBatchBackend>(impl_->device8,
    ...
    void Dx8GraphicsBackend::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
    ...
    void Dx8GraphicsBackend::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
    ...
    void Dx8GraphicsBackend::ApplySamplerState(int slot, int filter, int addressU, int addressV,
                                               int maxAnisotropy)
    {
    ...
    void Dx8GraphicsBackend::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
    ...
    void Dx8GraphicsBackend::ClearDepth(float depth)
    {
    ...
    void Dx8GraphicsBackend::ClearStencil(int stencil)
    {
    ...
    void Dx8GraphicsBackend::ClearDepthAndStencil(float depth, int stencil)
    {
    ...
    void Dx8GraphicsBackend::ClearColorAndStencil(float r, float g, float b, float a, int stencil)
    {
    ...
    void Dx8GraphicsBackend::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int stencil)
    {
    ...
    void Dx8GraphicsBackend::SetDepthTestEnabled(bool enabled)
    {
        impl_->device8->SetRenderState(D3DRS_ZENABLE, enabled ? D3DZB_TRUE : D3DZB_FALSE);
    }
    // Deliberate no-op, matching D3D9's/D3D11's/every prior backend in this family's own identical
    // choice: real blend configuration always arrives via ApplyBlendState().
    void Dx8GraphicsBackend::SetBlendEnabled(bool) {}
    void Dx8GraphicsBackend::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device8->SetRenderState(D3DRS_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    // ---- VertexBuffer/IndexBuffer backends: plain CPU-side storage (design decision 10) ----

    class Dx8VertexBufferBackend final : public IVertexBufferBackend
    {
    public:
        explicit Dx8VertexBufferBackend(int vertexCapacity) : capacity_(vertexCapacity) {}
        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx8VertexBufferBackend::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx8VertexBufferBackend::SetData: stride_in_bytes must be > 0");
    ...
    class Dx8IndexBufferBackend final : public IIndexBufferBackend
    {
    public:
        Dx8IndexBufferBackend(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx8IndexBufferBackend: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx8IndexBufferBackend: SetData bit-width does not match the buffer's declared width");
    ...
    std::unique_ptr<IVertexBufferBackend> Dx8GraphicsBackend::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx8VertexBufferBackend>(vertex_capacity);
    }
    std::unique_ptr<IIndexBufferBackend> Dx8GraphicsBackend::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx8IndexBufferBackend>(index_capacity, false);
    }
    std::unique_ptr<IIndexBufferBackend> Dx8GraphicsBackend::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx8IndexBufferBackend>(index_capacity, true);
    }
    ...
    /// Lambertian diffuse + Blinn-Phong specular -- ported conceptually from every prior backend
    /// in this family's own identical Phase O9 math (EasyGLGraphicsBackend's GLSL, not re-derived).
    ...
    /// Resolves a GpuDrawParams::texture0-style ITextureBackend* into the real IDirect3DTexture8*
    /// to bind, or nullptr (no texture) when `texture` is null.
    IDirect3DTexture8* Dx8ResolveTexture(const ITextureBackend* texture)
    {
        if (!texture) return nullptr;
        const auto* owner = dynamic_cast<const Dx8TextureOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx8GraphicsBackend: texture0 is not a DX8 texture (created by a different graphics backend?)");
    ...
    /// device-side opaque-white texture color, matching every prior backend's own established
                                   ...
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx8GraphicsBackend::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx8GraphicsBackend::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx8GraphicsBackend::") + who + ": only TriangleList is supported");
    }

    void Dx8GraphicsBackend::DrawColoredPrimitives(const IVertexBufferBackend& vb,
                                                   ...
                                                   PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx8GraphicsBackend::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");
    ...
    void Dx8GraphicsBackend::DrawIndexedColoredPrimitives(const IVertexBufferBackend& vb,
                                                          ...
                                                          PrimitiveType primitive, int primitiveCount)
    {
        ...
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx8GraphicsBackend::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx8VertexBufferBackend&>(vb);
        const auto& dxIb = static_cast<const Dx8IndexBufferBackend&>(ib);
                                             ...
                                             const GpuDrawParams& params)
    {
        ...
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx8GraphicsBackend::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx8GraphicsBackend::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx8VertexBufferBackend&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx8GraphicsBackend::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported)");
    ...
    void Dx8GraphicsBackend::DrawIndexedPrimitivesEx(const IVertexBufferBackend& vb, const IIndexBufferBackend& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        ...
        Dx8CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx8GraphicsBackend::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx8GraphicsBackend::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx8GraphicsBackend::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported)");
...
namespace CNA::Internal::Backends
{
    ...
    {
        return std::make_unique<Dx8::Dx8GraphicsBackend>(args);
    }
}
