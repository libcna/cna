...
// plan_dx5.md: real DirectX 5 graphics backend -- DirectDraw v4 (IDirectDraw4/
...
// IDirect3D/IDirect3DDevice) is permanently forbidden, asserted by the Dx5_ExecuteBufferDiscipline
// CTest (scripts/check-dx5-execute-buffer-discipline.sh, plan_dx5.md design decision 12).
#include <ddraw.h>
#include <d3d.h>
...
namespace CNA::Internal::Backends::Dx5
{
    ...
    namespace
    {
        ...
        // ---- Phase O3: shared offscreen-surface helpers, used by both Dx5TextureBackend and
        // Dx5RenderTargetBackend (design decision 7: 32bpp DDSCAPS_OFFSCREENPLAIN only, no
        ...
        // Dx5_Smoke/Dx5_Blend/etc.'s Clear()+GetBackBufferData() checks all still passed), but a
        ...
        [[noreturn]] void ThrowMipLevelUnsupported(int level)
        {
            throw std::runtime_error(
                "DX5 (DirectDraw v4) does not support mip-level texture uploads (level " +
        ...
        enum class Dx5BlendMode { Opaque, AlphaBlend, NonPremultiplied, Additive };
        ...
        // found-and-fixed bug).
        Dx5BlendMode DetectBlendMode(int colorSrc, int alphaSrc, int colorDst, int alphaDst,
                                     int colorFunc, int alphaFunc)
        {
            ...
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 5 && alphaDst == 5) return Dx5BlendMode::NonPremultiplied;
            // Additive: ColorSrc=SourceAlpha, AlphaSrc=SourceAlpha, ColorDst=One, AlphaDst=One
            if (bothAdd && colorSrc == 4 && alphaSrc == 4 && colorDst == 0 && alphaDst == 0) return Dx5BlendMode::Additive;
            // Opaque: ColorSrc=One, AlphaSrc=One, ColorDst=Zero, AlphaDst=Zero
            if (bothAdd && colorSrc == 0 && alphaSrc == 0 && colorDst == 1 && alphaDst == 1) return Dx5BlendMode::Opaque;
            ...
            return Dx5BlendMode::AlphaBlend;
        }
        ...
        DWORD Dx5CompareFunctionToD3D(int compareFunction)
        {
        ...
        DWORD Dx5CullModeToD3D(int cullMode)
        {
        ...
        DWORD Dx5FillModeToD3D(int fillMode)
        {
        ...
        DWORD Dx5TextureAddressModeToD3D(int addressMode)
        {
        ...
        struct Dx5FilterPair { DWORD mag; DWORD min; };
        ...
        Dx5FilterPair Dx5TextureFilterToD3D(int textureFilter)
        {
        ...
        void CompositeQuad(uint8_t* dstBase, int dstPitch, int dstW, int dstH,
                           ...
                           Dx5BlendMode blendMode, int filter, int addressU, int addressV)
        {
            ...
            for (int y = y0; y < y1; ++y)
            {
                for (int x = x0; x < x1; ++x)
                {
                    ...
                    for (const auto& tri : kTriangles)
                    {
                        ...
                        case Dx5BlendMode::NonPremultiplied:
                        {
    ...
    // Common accessor so Dx5SpriteBatchBackend can reach the underlying IDirectDrawSurface* of
    ...
    // this .cpp, same reasoning as Dx5TextureBackend/Dx5RenderTargetBackend themselves.
    class Dx5SurfaceOwner
    {
    public:
        virtual ~Dx5SurfaceOwner() = default;
        [[nodiscard]] virtual LPDIRECTDRAWSURFACE4 Surface() const = 0;
    };

    struct Dx5GraphicsBackend::Impl
    {
        ...
        // Phase O3: the offscreen surface owned by the currently-bound Dx5RenderTargetBackend, or
        // nullptr when no custom render target is bound (i.e. the shadow backbuffer is active). Set
        // directly by Dx5RenderTargetBackend::BindAsRenderTarget/UnbindAsRenderTarget via a pointer
        // to this field (passed at construction) -- kept as a plain LPDIRECTDRAWSURFACE4 rather than
        // a Dx5RenderTargetBackend* so Dx5RenderTargetBackend never needs to name this private Impl
        ...
        Dx5BlendMode currentBlendMode = Dx5BlendMode::AlphaBlend;
    ...
    Dx5GraphicsBackend::Dx5GraphicsBackend(const GraphicsBackendCreateArgs& args)
        : impl_(std::make_unique<Impl>())
    {
        if (!args.window) throw std::runtime_error("Dx5GraphicsBackend initialized with null window.");
        ...
        if (!impl_->hwnd)
            throw std::runtime_error("Dx5GraphicsBackend: could not obtain a real HWND from the SDL window.");
    ...
    Dx5GraphicsBackend::~Dx5GraphicsBackend() = default;

    void Dx5GraphicsBackend::Clear(float r, float g, float b, float a)
    {
    ...
    void Dx5GraphicsBackend::Present()
    {
        ...
        if (!ClientToScreen(impl_->hwnd, &topLeft))
            throw std::runtime_error("Dx5GraphicsBackend::Present: ClientToScreen failed");
    ...
    void Dx5GraphicsBackend::GetViewportSize(int& width, int& height)
    {
        width = impl_->logicalWidth;
        height = impl_->logicalHeight;
    }

    void Dx5GraphicsBackend::ReadBackbuffer(int x, int y, int w, int h, uint8_t* pixels)
    {
    ...
    void Dx5GraphicsBackend::SetVirtualResolution(int width, int height)
    {
    ...
    void Dx5GraphicsBackend::SetPresentationMode(int mode)
    {
    ...
    SDL_Window* Dx5GraphicsBackend::GetWindowInternal() const
    {
        return impl_->window;
    }

    bool Dx5GraphicsBackend::TransformWindowToLogical(float windowX, float windowY,
    ...
    bool Dx5GraphicsBackend::TransformLogicalToWindow(float logX, float logY,
    ...
    class Dx5VertexBufferBackend final : public IVertexBufferBackend
    {
    public:
        explicit Dx5VertexBufferBackend(int vertexCapacity) : capacity_(vertexCapacity) {}

        void SetData(const void* data, int vertex_count, std::size_t stride_in_bytes) override
        {
            if (vertex_count < 0 || vertex_count > capacity_)
                throw std::runtime_error("Dx5VertexBufferBackend::SetData: vertex_count exceeds capacity");
            if (stride_in_bytes == 0)
                throw std::runtime_error("Dx5VertexBufferBackend::SetData: stride_in_bytes must be > 0");
    ...
    class Dx5IndexBufferBackend final : public IIndexBufferBackend
    {
    public:
        Dx5IndexBufferBackend(int indexCapacity, bool thirtyTwoBit)
    ...
    private:
        void Upload(const void* data, int index_count, bool dataIsThirtyTwoBit)
        {
            if (index_count < 0 || index_count > capacity_)
                throw std::runtime_error("Dx5IndexBufferBackend: index_count exceeds capacity");
            if (dataIsThirtyTwoBit != thirtyTwoBit_)
                throw std::runtime_error("Dx5IndexBufferBackend: SetData bit-width does not match the buffer's declared width");
    ...
    Dx5ClipVertex Dx5LerpClipVertex(const Dx5ClipVertex& a, const Dx5ClipVertex& b, float t)
    {
        Dx5ClipVertex out;
        out.x = a.x + t * (b.x - a.x);
        out.y = a.y + t * (b.y - a.y);
    ...
    int Dx5ClipTriangleNearPlane(const Dx5ClipVertex verts[3], Dx5ClipVertex out[4])
    {
        constexpr float kNearEpsilon = 1e-5f;
        int count = 0;
        for (int i = 0; i < 3; ++i)
        {
            const Dx5ClipVertex& cur = verts[i];
            const Dx5ClipVertex& prev = verts[(i + 2) % 3];
            const bool curIn = cur.w > kNearEpsilon;
            const bool prevIn = prev.w > kNearEpsilon;
            if (curIn != prevIn)
            {
                const float t = (kNearEpsilon - prev.w) / (cur.w - prev.w);
                out[count++] = Dx5LerpClipVertex(prev, cur, t);
            }
            if (curIn)
                out[count++] = cur;
        }
    ...
    Dx5ClipVertex Dx5BuildPositionColorClipVertex(const uint8_t* raw, const Matrix& combined)
    {
        Vector3 position;
    ...
    /// Result of Dx5ComputeVertexLighting: the diffuse (base vertex color) and specular (additive,
    ...
    Dx5LitColor Dx5ComputeVertexLighting(const Vector3& localPosition, const Vector3& localNormal,
    ...
    /// normal (32/52), Dx5ComputeVertexLighting() replaces the raw/white vertex color with real
    ...
    Dx5ClipVertex Dx5BuildGenericClipVertex(const uint8_t* raw, std::size_t stride, const Matrix& combined,
                                            const Matrix& world, const GpuDrawParams& params)
    {
        Vector3 position;
        std::memcpy(&position, raw, sizeof(Vector3));
        ...
        else if (stride == 32 || stride == 52)
        {
            ...
            if (params.lightingEnabled)
            {
                ...
                const Dx5LitColor litColor = Dx5ComputeVertexLighting(position, normal, world, params);
    ...
    D3DTLVERTEX Dx5ClipVertexToD3DTLVERTEX(const Dx5ClipVertex& cv, int viewportWidth, int viewportHeight)
    {
    ...
    /// (not cached) rather than stored on Dx5TextureBackend: a handle is only valid for the device
    ...
    D3DTEXTUREHANDLE Dx5ResolveTextureHandle(const ITextureBackend* texture, LPDIRECT3DDEVICE3 device3)
    {
        if (!texture) return 0;
        const auto* owner = dynamic_cast<const Dx5SurfaceOwner*>(texture);
        if (!owner)
            throw std::runtime_error(
                "Dx5GraphicsBackend: texture0 is not a DX5 surface (created by a different graphics backend?)");
    ...
    /// Shared core for all 4 draw entry points: clips each triangle (Dx5ClipTriangleNearPlane),
    /// packs the result into D3DTLVERTEX (Dx5ClipVertexToD3DTLVERTEX), and submits via a single
    ...
    void SubmitDx5Primitives(LPDIRECT3DDEVICE3 device3, int viewportWidth, int viewportHeight,
                             const std::function<Dx5ClipVertex(int)>& fetchVertex,
                             int primitiveCount, D3DTEXTUREHANDLE texHandle, bool specularEnabled)
    {
        ...
        for (int i = 0; i < primitiveCount; ++i)
        {
            ...
            Dx5ClipVertex clipped[4];
            const int clippedCount = Dx5ClipTriangleNearPlane(cv, clipped);
            if (clippedCount == 0) continue;

            const auto baseIdx = static_cast<WORD>(verts.size());
            for (int k = 0; k < clippedCount; ++k)
                verts.push_back(Dx5ClipVertexToD3DTLVERTEX(clipped[k], viewportWidth, viewportHeight));
    ...
    /// target's surface).
    void Dx5CheckDrawPreconditions(const char* who, bool customRenderTargetBound,
                                   PrimitiveType primitive, int primitiveCount)
    {
        if (customRenderTargetBound)
            throw std::runtime_error(std::string("Dx5GraphicsBackend::") + who +
        ...
        if (primitiveCount <= 0)
            throw std::runtime_error(std::string("Dx5GraphicsBackend::") + who + ": primitiveCount must be > 0");
        if (primitive != PrimitiveType::TriangleList)
            throw std::runtime_error(std::string("Dx5GraphicsBackend::") + who + ": only TriangleList is supported in v1");
    }
    ...
    class Dx5TextureBackend : public ITextureBackend, public Dx5SurfaceOwner
    {
    public:
        Dx5TextureBackend(LPDIRECTDRAW4 dd, int width, int height)
            : width_(width), height_(height), surface_(CreateOffscreenSurface(dd, width, height))
        {
        }

        Dx5TextureBackend(LPDIRECTDRAW4 dd, const ImageData& data)
            : Dx5TextureBackend(dd, data.width, data.height)
        {
            WriteSurfacePixels(surface_, width_, height_, data.pixels.data(), width_ * 4);
        }

        ~Dx5TextureBackend() override { if (surface_) surface_->Release(); }

        Dx5TextureBackend(const Dx5TextureBackend&) = delete;
        Dx5TextureBackend& operator=(const Dx5TextureBackend&) = delete;
    ...
    class Dx5RenderTargetBackend final : public IRenderTargetBackend, public Dx5SurfaceOwner
    {
    public:
        Dx5RenderTargetBackend(LPDIRECTDRAWSURFACE4* currentTargetSlot, int* currentTargetWidthSlot,
        ...
        ~Dx5RenderTargetBackend() override
        {
            UnbindAsRenderTarget();
            if (surface_) surface_->Release();
        }

        Dx5RenderTargetBackend(const Dx5RenderTargetBackend&) = delete;
        Dx5RenderTargetBackend& operator=(const Dx5RenderTargetBackend&) = delete;
    ...
    std::unique_ptr<ITextureBackend> Dx5GraphicsBackend::CreateTexture(const ImageData& data)
    {
        return std::make_unique<Dx5TextureBackend>(impl_->dd, data);
    }

    std::unique_ptr<IRenderTargetBackend> Dx5GraphicsBackend::CreateRenderTarget2D(
        int w, int h, int /*depthFormat*/, bool /*preserveContents*/, bool /*mipMap*/, int multiSampleCount)
    {
        return std::make_unique<Dx5RenderTargetBackend>(&impl_->currentTargetSurface,
                                                         &impl_->currentTargetWidth, &impl_->currentTargetHeight,
                                                         impl_->dd, w, h, multiSampleCount);
    }

    void Dx5GraphicsBackend::SetRenderTarget2D(IRenderTargetBackend* rt)
    {
        if (rt)
    ...
    void Dx5GraphicsBackend::SetRenderTargets(
        const RenderTargetBindingDescriptor* renderTargets, int count)
    {
        // DirectDraw has no multi-render-target concept -- single active surface only.
        if (count > 1)
            throw std::runtime_error(
                "DX5 (DirectDraw v4) does not support multiple simultaneous render targets (MRT): "
        ...
        if (count > 0 && renderTargets[0].IsRenderTargetCubeFace())
            throw std::runtime_error(
                "DX5 (DirectDraw v4) does not support RenderTargetCube face bindings.");
    ...
    // Never named outside this .cpp, same reasoning as Dx5TextureBackend/Dx5RenderTargetBackend.

    class Dx5SpriteBatchBackend final : public ISpriteBatchBackend
    {
    public:
        Dx5SpriteBatchBackend(std::function<LPDIRECTDRAWSURFACE4()> getActiveSurface,
                              std::function<void(int&, int&)> getActiveSurfaceSize,
                              std::function<Dx5BlendMode()> getBlendMode)
        ...
        {
            if (begun_)
                throw std::runtime_error("Dx5SpriteBatchBackend::Begin: Begin() called without a matching End()");
            begun_ = true;
        }
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx5SpriteBatchBackend::End: End() called without a matching Begin()");
            begun_ = false;
        ...
        void SetCustomEffect(Effect* effect) override
        {
            if (effect != nullptr)
                throw std::runtime_error(
                    "DX5 (DirectDraw v4) does not support custom SpriteBatch Effects: no "
        ...
        {
            if (!begun_)
                throw std::runtime_error("Dx5SpriteBatchBackend::Draw: Draw() called before Begin()");

            const auto* owner = dynamic_cast<const Dx5SurfaceOwner*>(&texture);
            if (!owner)
                throw std::runtime_error(
                    "Dx5SpriteBatchBackend::Draw: texture backend is not a DX5 surface (created by "
            ...
            if (isIdentityGeometry && isWhiteTint && getBlendMode_() == Dx5BlendMode::Opaque)
            {
    ...
    private:
        ...
        std::function<Dx5BlendMode()> getBlendMode_;
        bool begun_ = false;
    ...
    std::unique_ptr<ISpriteBatchBackend> Dx5GraphicsBackend::CreateSpriteBatch()
    {
        Impl* impl = impl_.get();
        return std::make_unique<Dx5SpriteBatchBackend>(
            [impl]() { return impl->ActiveSurface(); },
            [impl](int& w, int& h) { impl->ActiveSurfaceSize(w, h); },
            [impl]() { return impl->currentBlendMode; });
    }

    void Dx5GraphicsBackend::ApplyBlendState(int colorSrcBlend, int alphaSrcBlend,
                                             ...
                                             const BlendWriteState& /*writeState*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_SRCBLEND, Dx5BlendToD3D(colorSrcBlend));
        impl_->device3->SetRenderState(D3DRENDERSTATE_DESTBLEND, Dx5BlendToD3D(colorDstBlend));
    }

    void Dx5GraphicsBackend::ApplyDepthStencilState(bool depthEnable, bool depthWriteEnable, int depthFunc,
                                                    ...
                                                    int /*ccwStencilFail*/, int /*ccwStencilDepthFail*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZFUNC, Dx5CompareFunctionToD3D(depthFunc));
    }

    void Dx5GraphicsBackend::ApplyRasterizerState(int cullMode, int fillMode, bool /*scissorTestEnable*/,
                                                  float /*depthBias*/, float /*slopeScaleDepthBias*/)
    {
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_CULLMODE, Dx5CullModeToD3D(cullMode));
        impl_->device3->SetRenderState(D3DRENDERSTATE_FILLMODE, Dx5FillModeToD3D(fillMode));
    }

    void Dx5GraphicsBackend::ApplySamplerState(int slot, int filter, int addressU, int /*addressV*/,
                                               int maxAnisotropy)
    {
        ...
        const Dx5FilterPair filterPair = Dx5TextureFilterToD3D(filter);
        ...
        impl_->device3->SetRenderState(D3DRENDERSTATE_TEXTUREADDRESS, Dx5TextureAddressModeToD3D(addressU));
    ...
    void Dx5GraphicsBackend::ClearColorAndDepth(float r, float g, float b, float a, float depth)
    {
        Clear(r, g, b, a);
        ClearDepth(depth);
    }
    ...
    void Dx5GraphicsBackend::ClearDepth(float depth)
    {
        int width = 0, height = 0;
        impl_->ActiveSurfaceSize(width, height);
    ...
    void Dx5GraphicsBackend::ClearStencil(int) {}
    void Dx5GraphicsBackend::ClearDepthAndStencil(float depth, int) { ClearDepth(depth); }
    void Dx5GraphicsBackend::ClearColorAndStencil(float r, float g, float b, float a, int) { Clear(r, g, b, a); }
    void Dx5GraphicsBackend::ClearColorDepthAndStencil(float r, float g, float b, float a, float depth, int)
    {
        ClearColorAndDepth(r, g, b, a, depth);
    }
    void Dx5GraphicsBackend::SetDepthTestEnabled(bool enabled)
    {
    ...
    void Dx5GraphicsBackend::SetBlendEnabled(bool) {}

    void Dx5GraphicsBackend::SetDepthWriteEnabled(bool enabled)
    {
        impl_->device3->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, enabled ? TRUE : FALSE);
    }

    std::unique_ptr<IVertexBufferBackend> Dx5GraphicsBackend::CreateVertexBuffer(int vertex_capacity)
    {
        return std::make_unique<Dx5VertexBufferBackend>(vertex_capacity);
    }

    std::unique_ptr<IIndexBufferBackend> Dx5GraphicsBackend::CreateIndexBuffer16(int index_capacity)
    {
        return std::make_unique<Dx5IndexBufferBackend>(index_capacity, false);
    }

    std::unique_ptr<IIndexBufferBackend> Dx5GraphicsBackend::CreateIndexBuffer32(int index_capacity)
    {
        return std::make_unique<Dx5IndexBufferBackend>(index_capacity, true);
    }

    void Dx5GraphicsBackend::DrawColoredPrimitives(const IVertexBufferBackend& vb,
                                                   const Matrix& world, const Matrix& view, const Matrix& projection,
                                                   PrimitiveType primitive, int primitiveCount)
    {
        Dx5CheckDrawPreconditions("DrawColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx5GraphicsBackend::DrawColoredPrimitives: primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx5VertexBufferBackend&>(vb);
        ...
        SubmitDx5Primitives(impl_->device3, vw, vh,
            [&](int i) { return Dx5BuildPositionColorClipVertex(base + static_cast<std::size_t>(i) * stride, combined); },
            primitiveCount, 0, false);
    }

    void Dx5GraphicsBackend::DrawIndexedColoredPrimitives(const IVertexBufferBackend& vb,
                                                          const IIndexBufferBackend& ib,
                                                          const Matrix& world, const Matrix& view, const Matrix& projection,
                                                          PrimitiveType primitive, int primitiveCount)
    {
        Dx5CheckDrawPreconditions("DrawIndexedColoredPrimitives", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx5GraphicsBackend::DrawIndexedColoredPrimitives: primitiveCount needs more indices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx5VertexBufferBackend&>(vb);
        const auto& dxIb = static_cast<const Dx5IndexBufferBackend&>(ib);
        ...
        SubmitDx5Primitives(impl_->device3, vw, vh,
            [&](int i) {
                ...
                return Dx5BuildPositionColorClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, combined);
            },
            primitiveCount, 0, false);
    }
    ...
    static void RequireFaithfulDeclarationEXT(const IVertexBufferBackend& vb, const char* route)
    {
        ...
        CNA::Internal::Graphics::RequireFaithfulVertexDeclaration(
            ...
            CNA::Internal::Graphics::UnlistedStrideLayout::BackendRefusesIt, "DX5", route);
    }

    void Dx5GraphicsBackend::DrawPrimitivesEx(const IVertexBufferBackend& vb,
                                              const Matrix& world, const Matrix& view, const Matrix& projection,
                                              PrimitiveType primitive, int primitiveCount,
                                              const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-nonindexed");
        Dx5CheckDrawPreconditions("DrawPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx5GraphicsBackend::DrawPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.vertexStart + primitiveCount * 3 > vb.GetVertexCount())
            throw std::runtime_error(
                "Dx5GraphicsBackend::DrawPrimitivesEx: vertexStart + primitiveCount needs more vertices than the bound buffer has");

        const auto& dxVb = static_cast<const Dx5VertexBufferBackend&>(vb);
        const std::size_t stride = dxVb.Stride();
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx5GraphicsBackend::DrawPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        const D3DTEXTUREHANDLE texHandle = params.textureEnabled
            ? Dx5ResolveTextureHandle(params.texture0, impl_->device3) : 0;
        const int vertexStart = params.vertexStart;

        SubmitDx5Primitives(impl_->device3, vw, vh,
            [&](int i) { return Dx5BuildGenericClipVertex(base + static_cast<std::size_t>(vertexStart + i) * stride,
                                                          stride, combined, world, params); },
            primitiveCount, texHandle, params.lightingEnabled);
    }

    void Dx5GraphicsBackend::DrawIndexedPrimitivesEx(const IVertexBufferBackend& vb, const IIndexBufferBackend& ib,
                                                     const Matrix& world, const Matrix& view, const Matrix& projection,
                                                     PrimitiveType primitive, int primitiveCount,
                                                     const GpuDrawParams& params)
    {
        RequireFaithfulDeclarationEXT(vb, "ordinary-indexed");
        Dx5CheckDrawPreconditions("DrawIndexedPrimitivesEx", impl_->currentTargetSurface != nullptr,
                                  primitive, primitiveCount);
        if (params.textureEnabled && params.texture0 == nullptr)
            throw std::runtime_error("Dx5GraphicsBackend::DrawIndexedPrimitivesEx: textureEnabled=true but texture0 is null");
        if (params.startIndex + primitiveCount * 3 > ib.GetIndexCount())
            throw std::runtime_error(
                "Dx5GraphicsBackend::DrawIndexedPrimitivesEx: startIndex + primitiveCount needs more indices than the bound buffer has");
        ...
        if (stride != 16 && stride != 20 && stride != 24 && stride != 32 && stride != 52)
            throw std::runtime_error(
                "Dx5GraphicsBackend::DrawIndexedPrimitivesEx: unsupported vertex stride (only 16/20/24/32/52 supported in v1)");
        ...
        SubmitDx5Primitives(impl_->device3, vw, vh,
            [&](int i) {
                ...
                return Dx5BuildGenericClipVertex(vbBase + static_cast<std::size_t>(idx) * stride, stride,
...
namespace CNA::Internal::Backends
{
    ...
    {
        return std::make_unique<Dx5::Dx5GraphicsBackend>(args);
    }
#endif
}
