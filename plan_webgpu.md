...
4. 3D backlog (Phases 57–66) is now underway, all landed 2026-07-12:
   ...
   - Next: `WEBGPU-22` (`lit_textured3d.wgsl`, stride 32, first real lighting — a materially
     different task than the three done so far, since none of them touch the lighting math at
     all) and its `WEBGPU-33` dispatch, or the remaining `WEBGPU-66` scope (alpha test/dual
     texture/env map/skinned dispatch).
...
| WEBGPU-33 | `GetOrCreatePipelineExt3D()` — stride 20/24/32 dispatch matching Vulkan | 🟨 | Strides 20 (`GetOrCreatePipelineTextured3D()`) and 24 (`GetOrCreatePipelineColoredTextured3D()`) done, both cached by `(topology, depthFunc, depthTest, depthWrite)` matching `WEBGPU-32`'s shape, verified 2026-07-12 by `WebGPU_Textured3D`/`WebGPU_ColoredTextured3D`. Stride 32 (`lit_textured3d.wgsl`, `WEBGPU-22`, real lighting) remains open — no shader exists yet. |
...
| WEBGPU-66 | `DrawPrimitivesEx()`: dispatch by `GpuDrawParams` (stride, textureEnabled, lightingEnabled, dualTexture, skinned, instanced) | 🟨 | Verified 2026-07-12 for stride 16 (`colored3d.wgsl`), stride 20 (`textured3d.wgsl`, requires `params.texture0 != nullptr`) and stride 24 (`colored_textured3d.wgsl`, same texture requirement): all three forward the caller's real `GpuDrawParams` (`DiffuseColor`, `VertexColorEnabled`, texture binding) via `FillExtUniforms()` (mirrors Vulkan's `FillExtPushConst()` field-for-field), instead of the `DrawColoredPrimitives` fallback's hardcoded white/true/no-texture. `WebGPU_DrawPrimitivesEx`/`WebGPU_Textured3D`/`WebGPU_ColoredTextured3D` CTests prove this concretely. Stride 32 (`lit_textured3d.wgsl`, real lighting) remains open, as do other effects (alpha test/dual texture/env map/skinned) — all still fall back to `DrawColoredPrimitives()` exactly as the interface default did. |
