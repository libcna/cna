...
| WEBGPU-58 | MSAA: `WGPUTexture` with `sampleCount=4`; resolve in render pass via `resolveTarget` | ⬜ | Deliberately deferred, not attempted, 2026-07-18 alongside `WEBGPU-53`/`54` landing: honouring a real sample count needs a new dimension threaded through every one of the ~10 `GetOrCreatePipeline*3D()` families' `pipeline.multisample.count` (currently hardcoded to 1 everywhere) plus a genuine multisampled colour texture + explicit `resolveTarget` view for both the backbuffer and any `RenderTarget2D`/depth texture (wgpu-native does not auto-resolve), and `ApplyMultiSampleCount()` reconfiguring the backbuffer's MSAA in place — a materially larger, separately-testable (MSAA-vs-non-MSAA edge-pixel comparison) unit than the RT round-trip this row's siblings just closed. `CreateRenderTarget2D()`'s `multiSampleCount` parameter is accepted but ignored; `WebGPURenderTargetBackend`/backbuffer `GetMultiSampleCount()` both honestly report 0 (verified by `WebGPU_RenderTarget2D` Check F) rather than silently claiming a sample count that was never applied. |
