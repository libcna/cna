...
4. 3D backlog (Phases 57–66) is now underway, all landed 2026-07-12:
   ...
   - 2026-07-18: `WEBGPU-53`/`54` — real single-target `RenderTarget2D` support (`WebGPURenderTargetBackend`,
     ...
     (`mipMap=true` throws) and MSAA (`multiSampleCount` accepted but ignored, `WEBGPU-58` remains
     open — see its own row for the scope discussion). MRT (`WEBGPU-85`/`86`/`87`) is untouched, a
     separate follow-up.
   - Next: `env_map3d.wgsl` (cubemap sampling, a third distinct bind-group shape) and
     `GetOrCreatePipelineEnvMap3D()` (`WEBGPU-25`/`36`/`74`), or instancing (`WEBGPU-38`/`68`).
...
| WEBGPU-58 | MSAA: `WGPUTexture` with `sampleCount=4`; resolve in render pass via `resolveTarget` | 🟨 | Attempted 2026-07-18. **Infrastructure implemented and verified (`WebGPU_Msaa`, 3/6 PASS — Checks A/C/D-1-of-2):** MSAA is a single BACKEND-GLOBAL `sampleCount_` (not a pipeline-cache-key dimension), mirroring `VulkanGraphicsBackend`'s own precedent — every one of the ~10 `GetOrCreatePipeline*3D()` families plus both SpriteBatch pipelines read the same `sampleCount_` for `WGPUMultisampleState.count`. `ApplyMultiSampleCount()` invalidates and lazily rebuilds every pipeline cache (`ClearAllPipelineCaches()`, see below) and recreates the backbuffer's depth texture and a new multisampled colour texture (`RecreateMsaaColorTexture()`) at the new sample count. `PickSampleCount()`/`Supports4xMsaa()` empirically probe real device capability via `wgpuDevicePushErrorScope`/a scratch 4-sample texture create/`wgpuDevicePopErrorScope` (never assumed) — on this machine/adapter the probe genuinely found exactly 4x supported, nothing higher (wgpu-native itself only defines sample counts 1 and 4, unlike Vulkan's 1/2/4/8/16/32/64 range), and `ApplyMultiSampleCount(8)`'s clamped-return-value contract is verified to return 4, never the raw unsupported request (Check C). `WebGPURenderTargetBackend` unconditionally mirrors the backend's global `sampleCount_` at construction time with its own multisampled colour+depth textures and a real single-sample resolve target, verified to report the SAME real applied value as the backbuffer regardless of its own constructor's requested count (Check D, 1 of 2). Backbuffer's `WGPURenderPassColorAttachment.resolveTarget` is the swapchain's own current texture (wgpu-native auto-resolves at render-pass end, no explicit resolve command) when `sampleCount_ > 1`. **KNOWN OPEN BUG — genuine multisample rendering does not work end-to-end (Checks B, D-2-of-2, E FAIL):** despite the infrastructure above, a diagonal-edged triangle drawn through the real `GraphicsDevice`/`BasicEffect`/`DrawUserPrimitives` → `EnsureFrameRendered()`/`RenderColoredDraws()`/`GetOrCreatePipelineColored3D()` call chain after `ApplyMultiSampleCount()` engages 4x produces a blank/flat readback (no triangle content reaches the resolve target at all — not merely "no blending", genuinely nothing), for both the backbuffer and a `RenderTarget2D`. Extensively investigated and ruled out as causes: surface/texture format, depth-clear timing (`clearDepthPending_`/`clearColorPending_`/`clearStencilPending_` are now explicitly forced `true` after any sample-count change so a freshly-recreated, undefined-content depth/colour texture is never read via `WGPULoadOp_Load` — a real, independently-defensible fix, kept, but did not resolve this bug), storeOp, viewport/scissor, cull mode, blend state/constant, sequential-texture-reuse, and `wgpuDevicePushErrorScope`/`wgpuDevicePopErrorScope` (used by `Supports4xMsaa()`) leaving stray error-scope state — none of these reproduced or fixed the symptom. Multiple standalone reproductions using the exact same device/queue and pipeline/attachment shapes (bypassing `GraphicsDevice`/`SpriteBatch` entirely, including calling the real `GetOrCreatePipelineColored3D()` directly) showed correct multisample resolve in *some* configurations — specifically, a completely ad-hoc, one-off `WGPUShaderModule`/`WGPUBindGroupLayout`/`WGPUPipelineLayout` created inline just for the probe, combined with scratch (non-persistent) MSAA/depth/resolve textures, rendered correctly — which led to a real, applied fix: `ClearAllPipelineCaches()` was rewritten to fully tear down and recreate every 3D/sprite shader module, bind group layout, pipeline layout AND pipeline cache on every sample-count change (previously it only released/rebuilt the `WGPURenderPipeline` objects, reusing the existing long-lived shader/layout objects unchanged), on the finding that reusing an already-used shader/layout combination for a new `WGPUMultisampleState.count` can silently misrender with zero validation errors. This fix is kept (architecturally sound, verified via pointer comparison to genuinely produce fresh objects, zero regression on the full pre-existing `WebGPU_*` suite, 15/15) but did **not** resolve Checks B/D-2/E: a further isolating test using scratch textures combined with the real, now-genuinely-fresh `coloredShader_`/`coloredBindGroupLayout_`/`coloredPipelineLayout_` (accessed via the real class members and `GetOrCreatePipelineColored3D()`, not a hand-rolled one-off) still failed, contradicting the "shader/layout reuse" theory as the complete explanation — the true root cause remains unidentified. `CreateRenderTarget2D()`'s per-instance `multiSampleCount` parameter is still intentionally ignored (RT always mirrors the backend's global state). Left as an honest, open, tracked gap for a follow-up task rather than declared complete — see `examples/webgpu_msaa_test.cpp`'s own top-of-file comment for the same summary next to the test that documents it. |
...
| WEBGPU-97 | Environment-map and skinned-effect tests. | 🟨 | Skinned-effect half done: `WebGPU_Skinned3D` (9/9) and `WebGPU_SkinnedPbr3D` (5/5), see `WEBGPU-26`. Environment-map tests remain open, scheduled with `WEBGPU-25`/`36`/`74`. |
