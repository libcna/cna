...
| ccache | the then-shared cache on the build partition, 1.2 GiB of 5 GiB at session start; that cache is gone and the project's single shared cache is now `/rv/cnaccache` |
...
All on the owner-designated build partition, which no longer exists; the shared build
directory that replaced it is `build-probe/`. Nothing was built in the
