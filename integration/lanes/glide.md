...
Builds used persistent Ninja/ccache trees on `/media/robertvokac/claude` (external disk, no longer mounted; build trees now live in-repo under `build-probe/` and the shared ccache at `/rv/cnaccache`). Every controlled build
...
```text
CCACHE_DIR=/rv/cnaccache cmake -S /rv/data/development/github.com/openeggbert/cnaglide -B build-probe/cmake-build-glide-pre -G Ninja -DCMAKE_TOOLCHAIN_FILE=/rv/data/development/github.com/openeggbert/cnaglide/cmake/toolchains/mingw-w64-i686.cmake -DCNA_GRAPHICS_BACKEND=GLIDE -DCNA_BUILD_TESTS=ON -DCNA_USE_CCACHE=ON -DCMAKE_C_COMPILER_LAUNCHER=ccache -DCMAKE_CXX_COMPILER_LAUNCHER=ccache
```
