...
byte-exact vertex and index buffers CNA must produce, packed by `l5.py` from the same authored
values the asset itself is built from. They are regenerated by the ordinary command — there is no
separate golden step:
...
* **The generator never reads CNA.** `l5.py` packs from the manifest's own L3 streams, so a golden
  can disagree with the importer — which is the entire point of having one. A golden regenerated
  because "the test failed" is a golden that proves nothing.
