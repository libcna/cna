...
ABI_HEADER_PATH = C_API_DIR / "include" / "CNA" / "C" / "abi.h"
...
# CBIND-115, 2026-08-28: the release name used to carry the ABI version as a literal, and it went
# stale the way a literal does -- `release_gate.json` still read "CNA C ABI 0.9.0" while `abi.h` had
# moved through 0.11 to 0.17, so the published gate announced a release eight minor versions behind
# the header it governs. A gate whose own first line is remembered rather than measured is the thing
# this tool's docstring argues against, so the version is now read from `abi.h` on every run and the
# declaration keeps only the part a human actually decides: the wording around it.
def abi_version() -> str:
    """Read the ABI version from `abi.h`, which is the only place it is declared."""
    if not ABI_HEADER_PATH.is_file():
        raise SystemExit(f"cannot read the ABI version: {ABI_HEADER_PATH} is missing")
    text = ABI_HEADER_PATH.read_text(encoding="utf-8", errors="replace")
    parts = []
    for component in ("MAJOR", "MINOR", "PATCH"):
        ...
        if not match:
            raise SystemExit(
                f"cannot read the ABI version: CNA_ABI_VERSION_{component} is not declared "
                f"in {ABI_HEADER_PATH.relative_to(REPO_ROOT)}"
            )
        parts.append(match.group(1))
    return ".".join(parts)


def release_name(declaration: dict) -> str:
    """Fill the declared release wording with the measured ABI version."""
    template = declaration["release"]["name_template"]
    if "{abi_version}" not in template:
        raise SystemExit(
            "release.name_template must contain {abi_version}; a literal version goes stale"
        )
    return template.format(abi_version=abi_version())
...
def render(evaluation: dict) -> str:
    ...
    add("")
    add(f"**Release:** {release_name(declaration)}")
    add("")
