...
"""Validate the public C++ to C API coverage and generate its reports.
...
reports access levels and emits stable source locations. ``docs/c-api/COVERAGE.md`` is a compact,
reviewable summary; the multi-megabyte per-symbol matrix is an ignored build artifact generated on
demand. ``--check`` is a mandatory gate (CBIND-043): the CTest test ``CApiCoverageMatrix`` and the
``c-api-coverage-gate`` workflow both run it.
"""
...
SYMBOL_OWNER_OVERRIDES: dict[str, str] = {
    ...
    # CBIND-122, 2026-08-30. The untyped external/root XNB dispatch added after the completed
    # content-reader slices is an internal loading seam exposed from public headers. Keep its five
    # newly discovered declarations on one open design/disposition task rather than assigning
    # them to completed CBIND-036/CBIND-105.
    "Microsoft::Xna::Framework::Content::ContentManager::ContentReader": "CBIND-122",
    "Microsoft::Xna::Framework::Content::ContentManager::LoadUntypedXnbReference": "CBIND-122",
...
def render_summary_markdown(
    ...
    """Render the small tracked record; the complete row matrix stays a build artifact."""
    status_counts = Counter(mappings[symbol.identity].status for symbol in symbols)
    module_headers: Counter[str] = Counter()
    module_symbols: Counter[str] = Counter()
    module_statuses: dict[str, Counter[str]] = defaultdict(Counter)
...
def build_inventory(
    ...
    *,
    ignore_approval: bool = False,
) -> tuple[list[Path], list[Path], list[Symbol], dict[str, Mapping], dict[str, list[str]]]:
    """Build and validate the canonical in-memory inventory for this repository."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    mode.add_argument(
        "--write",
        action="store_true",
        help="regenerate the tracked summary and ignored full inventory artifact",
    )
    mode.add_argument("--check", action="store_true", help="fail if the tracked summary is stale")
    mode.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="override the compact summary path (primarily for isolated generator tests)",
    )
    parser.add_argument(
        "--full-output",
        type=Path,
        help="override the complete per-symbol Markdown artifact path",
    )
    parser.add_argument(
        "--rule",
        action="append",
...
def display_path(path: Path, root: Path) -> str:
    """Return a concise repository-relative path without rejecting external test outputs."""
    try:
...
def main() -> int:
    ...
    full_output = arguments.full_output or root / "build" / "c-api" / "COVERAGE_FULL.md"
    if not output.is_absolute():
        output = root / output
    if not full_output.is_absolute():
        full_output = root / full_output
    rules_path = root / "tools" / "c-api" / "coverage_mappings.json"

    headers, excluded, symbols, mappings, usage = build_inventory(
    ...
    rendered = render_summary_markdown(
        headers, excluded, symbols, mappings, full_sha256
    )
    ...
    if arguments.write:
        ...
        full_output.write_text(full_rendered, encoding="utf-8")
        print(
            f"{summary_text}; wrote {display_path(output, root)} and "
            f"{display_path(full_output, root)} ({full_sha256})"
        )
        return 0
    ...
    if existing != rendered:
        ...
        return 1
    print(f"{summary_text}; tracked summary is current; full SHA-256={full_sha256}")
    return 0
