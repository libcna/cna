...
The full coverage inventory is complete and mechanical, but too detailed for somebody deciding
whether this ABI can carry their program. This tool builds the same canonical in-memory inventory
and answers the other question — *what is missing, and what do I do about it* — by collapsing the
partially mapped and unmapped symbols into the reasons behind them.
...
import generate_coverage_inventory as coverage_inventory
...
def read_rows() -> tuple[list[dict], dict[str, int]]:
    """Return the inventory rows that record a limitation."""
    _, _, symbols, mappings, _ = coverage_inventory.build_inventory(REPO_ROOT)
    rows = []
    counts = collections.Counter(mapping.status for mapping in mappings.values())
    for symbol in symbols:
        mapping = mappings[symbol.identity]
        if mapping.status == "partial":
            kind = "partial"
        elif mapping.status == "not-applicable":
            kind = "not-applicable"
        else:
            continue
        rows.append({
...
def analyze() -> dict:
    ...
    rows, inventory_counts = read_rows()
    ...
    return {
        ...
        "inventory_symbols": sum(inventory_counts.values()),
        "implemented_symbols": inventory_counts.get("implemented", 0),
        "partial": partial,
        "unmapped": unmapped,
        "problems": problems,
...
def render(analysis: dict) -> str:
    ...
    add("")
    add("This file is generated by `tools/c-api/generate_limitations.py` from the same in-memory")
    add("inventory that produces the compact [`COVERAGE.md`](COVERAGE.md) summary, plus the declared")
    add("non-symbol limitations in `tools/c-api/limitations.json`. It is not another inventory:")
    add(f"the full generated matrix records all {analysis['inventory_symbols']:,} public C++")
    add("declarations and what became of each. This records only the ones that did **not** become a")
    add("callable C route, grouped by reason, so a consumer can decide whether one is relevant.")
    add("")
