...
//
// Phase 16: CNA X11 window -> the platform's VK_KHR_xlib_surface service -> a real VkSurfaceKHR on
// a real physical device -> swapchain, acquire, clear, present, resize, recreate, present again.
//
...
#if defined(CNA_X11_VALIDATION_HAVE_VULKAN)
...
namespace CnaX11Validation {
    ...
    namespace VulkanLifetime {
        int Iteration(Session&, bool, const std::string&, std::string& error)
        {
...
#undef CNA_VK_GLOBAL
            ...
            application.pApplicationName = "cna_x11_desktop_validation";
...
#undef CNA_VK_DEVICE
    ...
    namespace VulkanLifetime {
        int Iteration(Session& session, const bool validation, const std::string& wantedDevice,
                      std::string& error)
        {
            auto window = session.Make("CNA Vulkan lifetime", 320, 240, true,
                                       WindowRenderIntent::Vulkan, 1100, 400);
            session.WaitFor(window->GetId(), WindowEventKind::Exposed);
            Context vk;
            std::string device;
            if (!Build(vk, session.Platform(), *window, validation, wantedDevice, error, device))
            {
                return -1;
            }
    ...
    int RunVulkan(const std::vector<std::string>& arguments)
    {
        ...
        auto window = session.Make("CNA Vulkan validation", 800, 600, true,
                                   WindowRenderIntent::Vulkan);
        ...
        // --- resize: the swapchain goes out of date and is rebuilt at the new size -------------------
        {
            ...
            window->Sync();
            session.WaitFor(id, WindowEventKind::PixelSizeChanged);
            std::string detail;
            ...
            Check(verified == VK_SUCCESS && vk.extent.width == 1100 && vk.extent.height == 700,
