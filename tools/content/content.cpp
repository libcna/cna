    ...
    void PrintUsage()
    {
        std::cerr
            ...
            << "--xnb-compress lzx is the compression Microsoft XNA 4.0 itself produced and is\n"
               "the only compressed form an XNA 4.0 runtime loads. NOTE: a genuine XNA 4.0\n"
               "runtime REFUSES what CNA currently writes -- every fixture in the LZX corpus\n"
               "fails with \"Error decompressing content data\" (tests/interop/xna40/README.md).\n"
               "CNA's own decoder and an independent parser both accept those files, so the\n"
               "defect is inside the bitstream; until it is fixed, use --xnb-compress none for\n"
               "content an XNA 4.0 runtime has to load. CNA emits real LZX: one\n"
