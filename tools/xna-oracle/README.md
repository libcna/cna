...
| `texturenull` | `true`/`false` (default `false`) — with `texture=true`, create **no** texture object: every effect's `Texture` stays null while `BasicEffect.TextureEnabled` stays `true`. Measures what an unbound stock-effect sampler reads (`plans/plan_graphics_shared_cleanup.md` GSC-0004) |
