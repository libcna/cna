...
int main(int argc, char** argv)
{
    if (argc != 2)
    {
        std::cerr << "usage: cna_xnb_interop_fixtures <output-directory>\n"
    ...
    index.Key("compression"); index.String("none");
    index.Key("readerNameStyle"); index.String("xna40");
    index.Key("license");
    index.String("MS-PL (same license as CNA itself); every byte here was generated by CNA, and "
                 "no third-party asset is embedded.");
    index.Key("note");
    index.String(
        "These files were written by CNA, not by Microsoft XNA. They exist so an XNA-capable "
        "Windows installation can attempt ContentManager.Load<T>() on them and compare against "
        "each fixture's expectation manifest. Nothing here has been validated against a real XNA "
        "4.0 runtime; see tests/interop/xna40/README.md.");
    index.Key("fixtures");
    index.BeginArray();
