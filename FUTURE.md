...
| **FUTURE** | Phase 2 — renderer expansion (OPENGLES2 + 13 new renderers) | **started**: the `FNA3D` lane is implemented and awaiting integration (see the Phase 2 table below and NEXT.md); the other 13 additions are **not started**, and no longer blocked by anything: all pre-expansion preparation (modularization, naming normalization, module-owned examples) is complete and public, so its base is the current public `develop` head produced by the 2026-08-10 pre-expansion promotion. Like every phase here it still requires a fresh explicit owner instruction before any work begins |
