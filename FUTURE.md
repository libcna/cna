...
- glTF is **not** corrected — the campaign is under way, not finished, and this line is retired only when `GLTF-458`
  ...
  `cna-gltf-viewer` **has** completed the pinned 14-row Gate C retake and the application-owned
  alpha/double-sided boundary is pixel-proven. The remaining CORE blocker is §27.1 row 7's real
  second-UV/per-map selection gap (`GLTF-182`/`183`), not missing viewer evidence.
