...
- **Working document:** `plan_gltf.md`, 460 numbered rows. **412 closed (`✔` 266, `✅` 146),
  37 `⬜` remaining.** The other 11 carry a deliberate partial marker: 2 `🔬` (investigation, no
...
| `*Gltf*` on `SOFTWARE` | **526 passed, 0 skipped**; its first run found and fixed the optional-base-map and 48/56/68-byte layout boundary; `GLTF-206`/`GLTF-179` added focused policy/parity witnesses |
