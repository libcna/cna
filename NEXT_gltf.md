...
- **Working document:** `plan_gltf.md`, 460 numbered rows. **413 closed (`✔` 267, `✅` 146),
  37 `⬜` remaining.** The other 10 carry a deliberate partial marker: 1 `🔬` (investigation, no
...
| `*Gltf*` on `SOFTWARE` | **528 passed, 0 skipped**; its first run found and fixed the optional-base-map and 48/56/68-byte layout boundary; `GLTF-206`/`GLTF-179` added focused policy/parity witnesses, and `GLTF-163` added the two all-renderer index-width policy gates |
