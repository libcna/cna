...
- The `RLGL` renderer identity and standalone raylib 6.0 `rlgl.h` dependency baseline, pinned to
  ...
  runnable GraphicsDevice slice remains under active development in `plans/plan_rlgl.md`. The
  append-only C renderer identity advances the experimental C ABI to 0.27.0.
