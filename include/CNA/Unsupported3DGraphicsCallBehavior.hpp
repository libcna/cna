...
{
    /**
     * @brief Controls how a permanently 2D-only graphics backend handles unsupported 3D calls.
     *
     * This policy complements GraphicsDevice::SupportsCapability(): capability queries describe
     * what the active backend can do, while this policy controls what happens if application code
     ...
     * or unfinished operations on an otherwise 3D-capable backend.
     */
    enum class Unsupported3DGraphicsCallBehavior
    {
        /**
         * @brief Preserve the backend's established failure behavior.
         *
        ...
        /**
         * @brief Log one warning per backend operation and use a safe no-op/null-object stub.
         *
