...
// plan_dx1.md: real DirectX 1 (DirectDraw v1) graphics backend. 2D-only, like SDL_RENDERER/DX3 --
// every 3D entry point throws (Phase O7). Unlike DX3 (which fronts the ../free-direct sibling
// reimplementation), this backend talks to a REAL Windows ddraw.h, using only v1 COM interfaces
...
// a Dx1GraphicsBackend pointer. All real <ddraw.h> usage lives in Dx1GraphicsBackend.cpp behind the
// Impl pimpl below.
...
{
    /**
     * DX1 graphics backend (plan_dx1.md): a CPU 2D compositor that uses a REAL
     ...
     * real Win32 HWND obtained from CNA's own already-existing SDL_Window via
     ...
     * DirectDraw model: the primary IS the display, not "this window") -- so this backend never
     ...
     * DDSCAPS_OFFSCREENPLAIN surfaces (Dx1TextureBackend/Dx1RenderTargetBackend, defined entirely in
     * Dx1GraphicsBackend.cpp -- neither is ever named outside it), and SetRenderTarget2D() redirects
     * Clear()/ReadBackbuffer() to whichever surface is currently bound. The SpriteBatch CPU
     * compositor (Phase O4, Dx1SpriteBatchBackend) is ported verbatim from DX3's own
     ...
     */
    class Dx1GraphicsBackend final : public IGraphicsBackend
    {
    public:
        explicit Dx1GraphicsBackend(const GraphicsBackendCreateArgs& args);
        ~Dx1GraphicsBackend() override;

        Dx1GraphicsBackend(const Dx1GraphicsBackend&) = delete;
        Dx1GraphicsBackend& operator=(const Dx1GraphicsBackend&) = delete;
        ...
        // uses), DX1 throws because there genuinely is no Direct3D COM interface reachable from a
        ...
        // CreateEffectBackend/CreateTexture3D/CreateTextureCube/CreateRenderTargetCube deliberately
