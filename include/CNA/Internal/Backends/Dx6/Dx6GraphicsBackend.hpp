...
// a Dx6GraphicsBackend pointer. All real <ddraw.h> usage lives in Dx6GraphicsBackend.cpp behind the
// Impl pimpl below.
...
namespace CNA::Internal::Backends::Dx6
{
     ...
     * and empirically at DX1-0/DX2-0/DX30-0/DX5-0/DX6-0, all of which this backend's history
     * inherits). The primary surface is desktop-sized (the real historical DirectDraw model: the
     * primary IS the display, not "this window") -- so this backend never composites directly
     ...
     * (Dx6TextureBackend/Dx6RenderTargetBackend, defined entirely in Dx6GraphicsBackend.cpp --
     ...
     * (Dx6SpriteBatchBackend): IDirectDrawSurface4::Blt/BltFast has never supported rotation in any
     ...
     */
    class Dx6GraphicsBackend final : public IGraphicsBackend
    {
    public:
        explicit Dx6GraphicsBackend(const GraphicsBackendCreateArgs& args);
        ~Dx6GraphicsBackend() override;

        Dx6GraphicsBackend(const Dx6GraphicsBackend&) = delete;
        Dx6GraphicsBackend& operator=(const Dx6GraphicsBackend&) = delete;
        ...
        // one texture stage in this backend's own scope, decision 6) and honors filter/addressU/
        ...
        // Phase O5 (design decision 8): plain CPU-side storage (Dx6VertexBufferBackend/
        // Dx6IndexBufferBackend, defined entirely in Dx6GraphicsBackend.cpp), matching the
