...
namespace CNA::Internal::Backends::HtmlDom
{
     ...
     *
     * The practical consequence is stated plainly in `docs/html-dom-backend.md`: uploading a
     * texture costs a PNG encode, which is fine once at load time and expensive every frame.
     */
    class HtmlDomTextureBackend : public ITextureBackend
    {
    public:
        /**
         ...
         */
        explicit HtmlDomTextureBackend(const ImageData& data);

        /**
         * @brief Creates a blank (fully transparent) texture of the given size.
         *
         * Used by HtmlDomRenderTargetBackend, which has no initial pixels to upload.
         *
         * @param width  Width in pixels.
         * @param height Height in pixels.
         */
        HtmlDomTextureBackend(int width, int height);

        /** @brief Destroys the JS-side canvas and every cached variant URL derived from it. */
        ~HtmlDomTextureBackend() override;
         ...
         *         backend's texture canvases have a mip chain or per-level LOD selection, the same
         ...
         *
         * Used by HtmlDomSpriteBatchBackend to name the source texture in a draw command, and by
         * HtmlDomRenderTargetBackend for its own bind/readback.
         *
