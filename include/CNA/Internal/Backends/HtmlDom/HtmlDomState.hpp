...
namespace CNA::Internal::Backends::HtmlDom
{
     ...
     * this backend needs (`plus-lighter`) and no blend-factor model at all, so three of the four
     ...
     * `IGraphicsBackend::ApplyBlendState` documents (Blend: One=0, Zero=1, SourceAlpha=4,
     ...
     *
     * Shared between HtmlDomGraphicsBackend (which sets it) and HtmlDomSpriteBatchBackend (which
     ...
     * Canvas2D context instead. The sprite-batch backend reads this to pick its path.
     *
