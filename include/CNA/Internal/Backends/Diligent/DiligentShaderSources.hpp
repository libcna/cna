...
// DiligentGraphicsBackend.cpp so a shader/lighting change is reviewable on its own instead of
...
// embed step -- included by DiligentGraphicsBackend.cpp only, no behavior change from moving them.
#pragma once
