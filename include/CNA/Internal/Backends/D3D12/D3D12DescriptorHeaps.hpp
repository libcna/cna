...
//
// DX-103 gave this backend four descriptor heaps and one monotonic bump cursor each, with no free
...
// value. This backend currently submits synchronously (ExecuteCommandListAndWaitEXT), so in
...
namespace CNA::Internal::Backends::D3D12
{
    ...
    {
        /** @brief The backend's shared fence. Held as a ComPtr so the clock stays usable even if
         *  the backend is destroyed before the last resource that frees a descriptor. */
    ...
    public:
         ...
         * @param type RTV or DSV -- the only two non-shader-visible types this backend uses.
    ...
    public:
         ...
         * is what every draw path in this backend does.
         *
         * @param index An index returned by Allocate().
     ...
     *
     * Held by the backend through a std::shared_ptr and captured by every resource that owns a
     * descriptor, so a resource destroyed AFTER its backend still frees its slot into a live
    ...
    {
        /** @brief Shared fence reading; updated by the backend after every submission. */
