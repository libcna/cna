...
{
    class MagnumRenderTargetBackend;
    class MagnumRenderTargetCubeBackend;

    /**
     * @brief The one record of which render target this backend currently draws into.
     *
    ...
    class MagnumRenderTargetBackend : public IRenderTargetBackend
    {
    public:
        /**
         ...
         * @param binding          The creating backend's shared binding record, held weakly so a
         *                         target outliving its backend detaches from nothing.
         ...
         */
        MagnumRenderTargetBackend(int width, int height, int depthFormat,
                                  std::weak_ptr<MagnumBoundTarget> binding,
                                  bool mipMap, int multiSampleCount);
        ~MagnumRenderTargetBackend() override;
    ...
    class MagnumRenderTargetCubeBackend : public IRenderTargetCubeBackend
    {
    public:
        /**
         ...
         * @param depthFormat      Raw `DepthFormat` ordinal; `None` allocates no depth storage.
         * @param binding          The creating backend's shared binding record, held weakly.
         * @param mipMap           Whether a full mip chain is allocated and regenerated on unbind.
         * @param multiSampleCount Requested sample count, clamped to what the driver supports.
         */
        MagnumRenderTargetCubeBackend(int size, int depthFormat,
                                      std::weak_ptr<MagnumBoundTarget> binding,
                                      bool mipMap, int multiSampleCount);
        ~MagnumRenderTargetCubeBackend() override;
