...
{
    /** @brief Magnum implementation of `IEffectBackend`: a `ShaderEffect`'s own linked program. */
    class MagnumEffectBackend : public IEffectBackend
    {
    public:
        /** @brief Creates an unlinked effect; call `CompileProgram` before binding it. */
        MagnumEffectBackend() = default;
        ~MagnumEffectBackend() override = default;
