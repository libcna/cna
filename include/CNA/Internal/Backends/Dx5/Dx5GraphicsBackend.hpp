...
// a Dx5GraphicsBackend pointer. All real <ddraw.h> usage lives in Dx5GraphicsBackend.cpp behind the
// Impl pimpl below.
...
namespace CNA::Internal::Backends::Dx5
{
     ...
     * and empirically at DX1-0/DX2-0/DX30-0/DX5-0, all of which this backend's history inherits).
     * The primary surface is desktop-sized (the real historical DirectDraw model: the primary IS
     * the display, not "this window") -- so this backend never composites directly onto it.
     ...
     * (Dx5TextureBackend/Dx5RenderTargetBackend, defined entirely in Dx5GraphicsBackend.cpp --
     ...
     * (Dx5SpriteBatchBackend): IDirectDrawSurface4::Blt/BltFast has never supported rotation in any
    ...
    {
    public:
        explicit Dx5GraphicsBackend(const GraphicsBackendCreateArgs& args);
        ~Dx5GraphicsBackend() override;

        Dx5GraphicsBackend(const Dx5GraphicsBackend&) = delete;
        Dx5GraphicsBackend& operator=(const Dx5GraphicsBackend&) = delete;
        ...
        // Phase O5 (design decision 8): plain CPU-side storage (Dx5VertexBufferBackend/
        // Dx5IndexBufferBackend, defined entirely in Dx5GraphicsBackend.cpp), matching the
