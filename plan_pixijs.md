...
| PIXIJS-84 | A real Emscripten toolchain build of `-DCNA_GRAPHICS_RENDERER=PIXIJS`, in whatever future session has `emsdk` available — the honest first real verification step for everything above | ⬜ | A native (`SDL_RENDERER`) configure+`CnaTests` build was run in this session and confirmed the shared identity-registry edits (`GraphicsRendererType.hpp` and friends) don't break any other renderer -- real, but it exercises none of `PixiJsRenderer.cpp`'s own code (Emscripten-gated, never compiled under a native selection). Blocking prerequisite for upgrading *any* 🟨 in this plan to ✅. |
