#!/usr/bin/env bash
# plan_dx5.md design decision 11: run a Windows cross-compiled .exe (DX5 renderer, CNA's real
...
#
# Usage: scripts/run-wine-dx5.sh <path-to.exe> [args...]
#
...
if [ "${CNA_DX5_SKIP_DDRAW_GATE:-0}" != "1" ]; then
    if ! grep -Eq ':ddraw:' "$logFile"; then
        echo "error: DX5's ddraw-engagement gate failed -- no 'trace:ddraw:' WINEDEBUG line was" >&2
        echo "seen in this run's output, so real Wine ddraw.dll may never have actually handled a" >&2
        echo "DirectDraw call. This invalidates any DX5 pixel-test result from this run -- fix the" >&2
