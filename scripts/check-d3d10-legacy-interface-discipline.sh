#!/usr/bin/env bash
# plan_d3d10.md: a real, automated proof that the D3D10 backend never quietly reaches for any of
# the fixed-function-era symbols this whole DX1..DX8 family used but D3D10 genuinely removed --
...
# these would mean accidental copy-paste from an earlier backend, not real D3D10 code. Registered
# as the D3D10_LegacyInterfaceDiscipline CTest (cmake/Tests/D3D10Tests.cmake) -- pure text check, no
...
# Strip BOTH // line comments and /* ... */ block comments (this backend's own Doxygen /** */
...
if [ "$violations" -ne 0 ]; then
    echo "error: D3D10 backend source references a forbidden fixed-function-era symbol above --" >&2
...
echo "OK: D3D10 backend source has no fixed-function-era symbols and uses only real HLSL shaders + state objects."
exit 0
