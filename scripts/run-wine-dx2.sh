#!/usr/bin/env bash
# plan_dx2.md design decision 11: run a Windows cross-compiled .exe (DX2 renderer) under Wine
# against real DirectDraw v1, using this project's own dedicated Wine prefix.
#
# Usage: scripts/run-wine-dx2.sh <path-to.exe> [args...]
#
...
#
# Set CNA_DX2_WINEPREFIX to point at a different prefix; defaults to ~/.wine-cna-dx1 -- DX2
# deliberately REUSES DX1's own already-initialized prefix rather than creating a fresh
# ~/.wine-cna-dx2 (plan_dx2.md design decision 11: the DX2-0 spike itself ran entirely against
# ~/.wine-cna-dx1 and confirmed it works for DX2's own DirectDraw v1 + Direct3D v2 needs, so there
...
# engaged (not a silently-missing/no-op DirectDraw implementation), the DX2 equivalent of
...
# CNA_DX2_SKIP_DDRAW_GATE=1 for a binary that legitimately never creates a DirectDraw object at all
...
export WINEPREFIX="${CNA_DX2_WINEPREFIX:-$HOME/.wine-cna-dx1}"
export WINEDEBUG="${CNA_DX2_WINEDEBUG:-+ddraw}"
...
logFile="$(mktemp "${TMPDIR:-/tmp}/cna-dx2-ddraw-log.XXXXXX")"
trap 'rm -f "$logFile"' EXIT

wine "$@" 2>&1 | tee "$logFile"
wineExit="${PIPESTATUS[0]}"

if [ "${CNA_DX2_SKIP_DDRAW_GATE:-0}" != "1" ]; then
    if ! grep -Eq ':ddraw:' "$logFile"; then
        echo "error: DX2's ddraw-engagement gate failed -- no 'trace:ddraw:' WINEDEBUG line was" >&2
        echo "seen in this run's output, so real Wine ddraw.dll may never have actually handled a" >&2
        echo "DirectDraw call. This invalidates any DX2 pixel-test result from this run -- fix the" >&2
        echo "Wine prefix rather than ignoring this." >&2
        echo "(Set CNA_DX2_SKIP_DDRAW_GATE=1 for a binary that legitimately never opens a DirectDraw object.)" >&2
        exit 3
    fi
fi
