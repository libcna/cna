...
# as the Dx6_ExecuteBufferDiscipline CTest (cmake/Tests/Dx6Tests.cmake) -- pure text check, no
# compiled binary, no Wine needed, runs identically whether cross-compiling or not.
#
# Adapted verbatim from scripts/check-dx5-execute-buffer-discipline.sh -- DX6 introduces no new
...
# discipline is identical to DX5's: (1) IDirectDraw4/IDirectDrawSurface4/DDSURFACEDESC2 are
...
# enum's value names, e.g. D3DVT_TLVERTEX) are forbidden -- DX6 keeps DX5's D3DFVF_TLVERTEX
# submission unchanged; the new-for-DX6 stencil render states (D3DRENDERSTATE_STENCILENABLE etc.,
# D3DSTENCILOP_* values) are legitimate on the same IDirect3DDevice3 and are not restricted by this
# check.
set -uo pipefail

repo_root="$1"
dx6_src="${repo_root}/src/CNA/Internal/Backends/Dx6"
