...
# Dx2_ExecuteBufferDiscipline CTest (cmake/Tests/Dx2Tests.cmake) -- pure text check, no compiled
# binary, no Wine needed, runs identically whether cross-compiling or not.
#
# Unlike scripts/check-dx1-v1-only.sh (which forbids ALL of IDirect3D*/d3d.h, since DirectX 1 has no
# Direct3D at all), DX2 legitimately uses IDirect3D2/IDirect3DDevice2/IDirect3DViewport2/
...
# DX2's DirectDraw (2D) layer stays v1-only, same permanent boundary DX1-1's own grep CTest already
# enforces: IDirectDraw2+/IDirectDrawSurface2+/DDSURFACEDESC2 are forbidden here too.
set -uo pipefail

repo_root="$1"
dx2_src="${repo_root}/src/CNA/Internal/Backends/Dx2"
