#!/usr/bin/env bash
# plan_dx7.md design decision 16: a real, automated proof that the DX7 backend never quietly
...
# here. Registered as the Dx7_ExecuteBufferDiscipline CTest (cmake/Tests/Dx7Tests.cmake) -- pure
# text check, no compiled binary, no Wine needed, runs identically whether cross-compiling or not.
#
# Unlike DX6 (no new interface at all vs DX5, so its discipline check is identical to DX5's), DX7
# is a real architectural change -- this check is stricter in four new ways: (1) DX7 is v7-only, not
...
# backend's own comments make when explaining what earlier eras used -- comments are stripped
# before matching, so this is safe). (2) IDirect3D3/IDirect3DDevice3/IDirect3DViewport3 (and any
# bare \bIDirect3D7\b look-alike typo of an older version) are forbidden -- DX7 uses ONLY
# IDirect3D7/IDirect3DDevice7. (3) CreateViewport/AddViewport/SetCurrentViewport/DeleteViewport/
# Clear2 are forbidden -- the whole viewport object no longer exists in DX7 at all
# (IDirect3DDevice7::SetViewport/Clear are direct device methods instead). (4)
# D3DRENDERSTATE_TEXTUREHANDLE and IDirect3DTexture2 are forbidden -- DX7 binds textures via a
# direct SetTexture(stage, surface) call, with no handle indirection at all. (5) Real, empirically
# found while running the first full DX7 CTest pass (not anticipated by the DX7-0 spike, which never
...
repo_root="$1"
dx7_src="${repo_root}/modules/renderers/dx7/src"
dx7_include="${repo_root}/modules/renderers/dx7/include/CNA/Internal/Backends/Dx7"

if [ ! -d "$dx7_src" ] || [ ! -d "$dx7_include" ]; then
    echo "error: DX7 backend directories not found under ${repo_root}" >&2
    exit 1
fi
...
# "IDirect3DViewport3" or "D3DRENDERSTATE_TEXTUREHANDLE" -- while explaining what DX7 removed) before
...
# hit reports the actual file/line. Unlike DX6's own script (plain `sed 's|//.*||'` sufficed there,
# since none of its forbidden symbols appear in that backend's own doc comments), DX7's forbidden
...
if [ "$violations" -ne 0 ]; then
    echo "error: DX7 backend source references a forbidden execute-buffer/legacy-interface/" >&2
    echo "old-vertex-type/removed-viewport-object/texture-handle symbol above -- DX7 must use ONLY" >&2
...
echo "OK: DX7 backend source uses only v7-only DirectDraw/Direct3D symbols, no viewport object, and direct SetTexture binding."
exit 0
