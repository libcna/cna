#!/usr/bin/env bash
# plan_dx7.md design decision 16: a real, automated proof that the DX7 renderer never quietly
...
dx7_include="${repo_root}/modules/renderers/dx7/include/CNA/Internal/Renderers/Dx7"

if [ ! -d "$dx7_src" ] || [ ! -d "$dx7_include" ]; then
    echo "error: DX7 renderer directories not found under ${repo_root}" >&2
    exit 1
fi
...
# since none of its forbidden symbols appear in that renderer's own doc comments), DX7's forbidden
...
if [ "$violations" -ne 0 ]; then
    echo "error: DX7 renderer source references a forbidden execute-buffer/legacy-interface/" >&2
...
echo "OK: DX7 renderer source uses only v7-only DirectDraw/Direct3D symbols, no viewport object, and direct SetTexture binding."
exit 0
