#!/usr/bin/env bash
# plan_dx8.md design decision 16: a real, automated proof that the DX8 backend never quietly
...
# backend, not real DX8 code), the removed D3DTLVERTEX macro/D3DVT_* old vertex-type enum (neither
# exists in the real d3d8types.h headers at all, but a copy-paste from an earlier backend could
# still introduce a bare reference to the name), or the legacy D3DRENDERSTATE_* render-state naming
# DX8 renamed to D3DRS_* (the underlying enum values are identical, but this backend should use
...
dx8_include="${repo_root}/modules/renderers/dx8/include/CNA/Internal/Backends/Dx8"

if [ ! -d "$dx8_src" ] || [ ! -d "$dx8_include" ]; then
    echo "error: DX8 backend directories not found under ${repo_root}" >&2
    exit 1
fi
...
if [ "$violations" -ne 0 ]; then
    echo "error: DX8 backend source references a forbidden DirectDraw/legacy-vertex-type/legacy-" >&2
...
echo "OK: DX8 backend source has no DirectDraw references, no legacy vertex-type symbols, and uses only modern D3DRS_* render-state naming."
exit 0
