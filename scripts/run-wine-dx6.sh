#!/usr/bin/env bash
# plan_dx6.md design decision 11: run a Windows cross-compiled .exe (DX6 renderer, CNA's real
...
#
# Usage: scripts/run-wine-dx6.sh <path-to.exe> [args...]
#
...
#
# Set CNA_DX6_WINEPREFIX to point at a different prefix; defaults to ~/.wine-cna-dx1 -- DX6
...
# for DX6's IDirectDraw4 + Direct3D v3 needs, so there is nothing DX6-specific a separate prefix
# would buy).
#
...
if [ "${CNA_DX6_SKIP_DDRAW_GATE:-0}" != "1" ]; then
    if ! grep -Eq ':ddraw:' "$logFile"; then
        echo "error: DX6's ddraw-engagement gate failed -- no 'trace:ddraw:' WINEDEBUG line was" >&2
        echo "seen in this run's output, so real Wine ddraw.dll may never have actually handled a" >&2
        echo "DirectDraw call. This invalidates any DX6 pixel-test result from this run -- fix the" >&2
