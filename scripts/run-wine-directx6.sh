...
#
# Usage: scripts/run-wine-directx6.sh <path-to.exe> [args...]
#
...
# least one DirectDraw call. Set CNA_DX6_SKIP_DDRAW_GATE=1 for a binary that legitimately never
...
export WINEPREFIX="${CNA_DX6_WINEPREFIX:-$HOME/.wine-cna-dx1}"
export WINEDEBUG="${CNA_DX6_WINEDEBUG:-+ddraw}"
...
logFile="$(mktemp "${TMPDIR:-/tmp}/cna-dx6-ddraw-log.XXXXXX")"
trap 'rm -f "$logFile"' EXIT

wine "$@" 2>&1 | tee "$logFile"
wineExit="${PIPESTATUS[0]}"

if [ "${CNA_DX6_SKIP_DDRAW_GATE:-0}" != "1" ]; then
    if ! grep -Eq ':ddraw:' "$logFile"; then
        echo "error: DIRECTX6's ddraw-engagement gate failed -- no 'trace:ddraw:' WINEDEBUG line was" >&2
        echo "seen in this run's output, so real Wine ddraw.dll may never have actually handled a" >&2
        echo "DirectDraw call. This invalidates any DIRECTX6 pixel-test result from this run -- fix the" >&2
        echo "Wine prefix rather than ignoring this." >&2
        echo "(Set CNA_DX6_SKIP_DDRAW_GATE=1 for a binary that legitimately never opens a DirectDraw object.)" >&2
        exit 3
    fi
fi
