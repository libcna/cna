...
// (modules/c-api/wasm/browser_probe.html), registered as the ctest CApi_WasmBrowserProbe, runs
// through this script. It was first written for the PixiJS renderer's pixel suite
// (plans/plan_pixijs.md PIXIJS-85, a renderer retired on 2026-09-17 -- see
// docs/removed-renderers.md) and kept, renamed, because it is not renderer-specific: it decides how
// a page is served, what counts as a pass, and what "Playwright is available" means.
//
// The run is reproducible rather than a one-off hand-driven session. A page reports by
...
//
// Usage:
//   node scripts/run_browser_tests.mjs <build-dir> <page.html> [page.html ...]
//   node scripts/run_browser_tests.mjs --check-playwright
//
...
async function main() {
    ...
    if (!buildDir) {
        console.error('usage: run_browser_tests.mjs <build-dir> <page.html> [page.html ...]');
        console.error('       run_browser_tests.mjs --check-playwright');
        return 2;
    }
    if (requested.length === 0) {
        console.error('run_browser_tests.mjs: name at least one page to run');
        return 2;
    }
