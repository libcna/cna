#!/usr/bin/env bash
# plan_dx3.md design decision 9: a real, automated proof that the DX3 backend never quietly
...
# legitimately uses IDirectDraw2 (plan_dx3.md design decision 2 -- this backend's whole reason for
...
repo_root="$1"
dx3_src="${repo_root}/src/CNA/Internal/Backends/Dx3"
...
if [ "$violations" -ne 0 ]; then
    echo "error: DX3 backend source references a forbidden execute-buffer/legacy-interface symbol" >&2
...
echo "OK: DX3 backend source uses only DrawPrimitive-based Direct3D v2 and v2-only DirectDraw symbols."
exit 0
