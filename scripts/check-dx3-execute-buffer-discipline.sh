#!/usr/bin/env bash
# plan_dx3.md design decision 9: a real, automated proof that the DX3 renderer never quietly
...
# IDirect3DDevice2::DrawPrimitive/DrawIndexedPrimitive one -- see plan_dx3.md's status note (the
# DX30-0 spike, itself building on DX2-0) for why the execute-buffer surface is permanently
# off-limits here. Registered as the Dx3_ExecuteBufferDiscipline CTest
# (cmake/Tests/Dx3Tests.cmake) -- pure text check, no compiled binary, no Wine needed, runs
# identically whether cross-compiling or not.
#
# Adapted from scripts/check-dx2-execute-buffer-discipline.sh with exactly one change: DX3
# legitimately uses IDirectDraw2 (plan_dx3.md design decision 2 -- this renderer's whole reason for
...
# IDirectDraw3+/IDirectDrawSurface2+/DDSURFACEDESC2 remain forbidden -- DX3 is v2-only, not v3+.
set -uo pipefail

repo_root="$1"
dx3_src="${repo_root}/modules/renderers/dx3/src"
dx3_include="${repo_root}/modules/renderers/dx3/include/CNA/Internal/Renderers/Dx3"

if [ ! -d "$dx3_src" ] || [ ! -d "$dx3_include" ]; then
    echo "error: DX3 renderer directories not found under ${repo_root}" >&2
    exit 1
fi
...
if [ "$violations" -ne 0 ]; then
    echo "error: DX3 renderer source references a forbidden execute-buffer/legacy-interface symbol" >&2
    echo "above -- DX3 must use ONLY IDirect3DDevice2::DrawPrimitive/DrawIndexedPrimitive for its" >&2
    ...
    echo "DDSURFACEDESC2), per plan_dx3.md design decision 9." >&2
    exit 1
fi

echo "OK: DX3 renderer source uses only DrawPrimitive-based Direct3D v2 and v2-only DirectDraw symbols."
exit 0
