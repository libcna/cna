...
# 2. Unlike DX8 (which could avoid a real DXGI swap chain entirely), D3D10 genuinely needs one --
...
# mirroring run-wine-dx8.sh/run-wine-dxvk.sh's own identical gate -- a "DXVK: <version>" log line
