#!/usr/bin/env bash
# plan_dx7.md design decision 15: run a Windows cross-compiled .exe (DX7 renderer, CNA's real
...
# Wine prefix.
#
# Usage: scripts/run-wine-dx7.sh <path-to.exe> [args...]
#
...
#
# Set CNA_DX7_WINEPREFIX to point at a different prefix; defaults to ~/.wine-cna-dx1 -- DX7
...
# for DX7's IDirectDraw7 + Direct3D v7 needs, so there is nothing DX7-specific a separate prefix
# would buy).
#
...
# least one DirectDraw call. Set CNA_DX7_SKIP_DDRAW_GATE=1 for a binary that legitimately never
...
export WINEPREFIX="${CNA_DX7_WINEPREFIX:-$HOME/.wine-cna-dx1}"
export WINEDEBUG="${CNA_DX7_WINEDEBUG:-+ddraw}"
...
logFile="$(mktemp "${TMPDIR:-/tmp}/cna-dx7-ddraw-log.XXXXXX")"
trap 'rm -f "$logFile"' EXIT

wine "$@" 2>&1 | tee "$logFile"
wineExit="${PIPESTATUS[0]}"

if [ "${CNA_DX7_SKIP_DDRAW_GATE:-0}" != "1" ]; then
    if ! grep -Eq ':ddraw:' "$logFile"; then
        echo "error: DX7's ddraw-engagement gate failed -- no 'trace:ddraw:' WINEDEBUG line was" >&2
        echo "seen in this run's output, so real Wine ddraw.dll may never have actually handled a" >&2
        echo "DirectDraw call. This invalidates any DX7 pixel-test result from this run -- fix the" >&2
        echo "Wine prefix rather than ignoring this." >&2
        echo "(Set CNA_DX7_SKIP_DDRAW_GATE=1 for a binary that legitimately never opens a DirectDraw object.)" >&2
        exit 3
    fi
fi
