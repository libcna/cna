...
| SOFTWARE-61 | (Stretch goal) Cross-backend diagnostic test: render an identical simple scene on `SOFTWARE` and a real GPU backend, compare within a tolerance — directly serves the owner's stated "diagnostika rozdílů mezi backendy" use case, but depends on a real GPU backend being available in the test environment; may not be runnable in every CI | ⬜ | Not implemented — explicitly flagged as a stretch goal from the start, and genuinely depends on infrastructure (a real GPU backend + display available in the same CI run as `SOFTWARE`) this session didn't set up. A reasonable follow-up, not a gap in the backend itself. |
