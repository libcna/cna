...
> Phases N1–N4 and N6's core are done; `HeadlessTrace` mode (Phase N5) has its data-collection
> plumbing in place but is not yet exercised by a dedicated test, and a few validation/scoping
> corners were intentionally narrowed from their original wording once real interface constraints
> were discovered during implementation — see each task's own Notes column and the "Implementation
...
| HEADLESS-5 | `CNA_HEADLESS_MODE` environment-variable parsing (`Fast`/`Validation`/`Trace`, case-insensitive, default `Validation` on unrecognized/unset) plus a `NOXNA` programmatic override API (e.g. `HeadlessGraphicsBackend::SetMode(HeadlessMode)`) callable before `Game::Run()` | ✅ | Verified 2026-07-13: `ParseHeadlessModeFromEnvironment()` implemented and used at construction; `SetMode()` exercised directly by `Headless_Smoke` (switches `Validation`↔`Fast` mid-run and confirms the same out-of-range draw call throws under one and not the other). The env-var parsing path itself is implemented but not separately unit-tested (only the programmatic `SetMode()` path is exercised) — a small remaining gap, not blocking. |
...
| HEADLESS-11 | `HeadlessIndexBufferBackend` — same, both 16-bit and 32-bit | ✅ | Implemented for both; `Headless_Smoke` exercises the 16-bit path (`IndexElementSize::SixteenBits`). The 32-bit path compiles and shares the same `Upload()` logic but has no dedicated test yet. |
...
| HEADLESS-13 | `HeadlessTextureCubeBackend` | ✅ | Implemented (6-face pixel storage, per-face `SetData`/`GetData` with argument validation); compiles and is reachable via `CreateTextureCube()`, not yet exercised by a dedicated test. |
| HEADLESS-14 | `HeadlessTexture3DBackend` | ✅ | Implemented (voxel storage, `SetData`/`GetData` with argument validation); compiles and is reachable via `CreateTexture3D()`, not yet exercised by a dedicated test. |
| HEADLESS-15 | `HeadlessRenderTargetBackend` / `HeadlessRenderTargetCubeBackend` — track size/format only; `SetRenderTarget`/`SetRenderTargets` update backend-internal "current target" state for validation (`HEADLESS-23`) without needing an actual attachment | ✅ | Implemented; `HeadlessGraphicsBackend::GetViewportSize()` genuinely consults the currently-bound render target's tracked size when one is bound (real behaviour, not a stub). Not yet exercised by a dedicated test. |
| HEADLESS-16 | `HeadlessEffectBackend` — accepts any GLSL/HLSL/WGSL source string without compiling it; tracks which uniforms were set (name → last value) so a test can assert "did the game set this uniform" without a real shader compiler | ✅ | Implemented, including `UniformValues()` for test introspection; reachable via `CreateEffectBackend()` (used by `ShaderEffect`), not yet exercised by a dedicated test — `Headless_Smoke` only uses the built-in `BasicEffect`, which does not go through `IEffectBackend` at all (it is a stock effect implemented directly against `GpuDrawParams`, same as every other backend). |
...
| HEADLESS-23 | Validate viewport/scissor rectangles and `SetRenderTarget(s)` calls against the currently-bound target's tracked size (from `HEADLESS-15`) — catch off-target rectangles instead of silently accepting them | 🟨 | **Narrower than planned**: `SetViewport`/`SetScissorRect` validate non-negative width/height (a real, useful check) but do not cross-reference the currently-bound render target's actual dimensions (the originally-planned "off-target rectangle" check) — that needs the viewport rect threaded through to `HeadlessRenderTargetBackend`, which the current `SetViewport`/`SetScissorRect` signatures don't carry a target reference to do cheaply. Recorded as a real, scoped-down gap, not silently dropped. |
...
| HEADLESS-32 | Per-frame snapshot in addition to the cumulative counters (reset at each `Present()`) — needed so a test can assert "this single `Draw()` call issued exactly N draw calls" without subtracting cumulative totals by hand | ✅ | `GetLastFrameStatistics()` implemented (diffs cumulative stats against a snapshot taken at the last `Present()`); the diffing logic itself is not yet exercised by a dedicated assertion in `Headless_Smoke` (which reads cumulative `GetStatistics()` directly instead) — implemented and compiles, verification of the diff math specifically is a remaining gap. |
| HEADLESS-33 | Resource creation/destruction counters, broken down by resource type (vertex buffer, index buffer, `Texture2D`, `TextureCube`, `Texture3D`, `RenderTarget2D`, `RenderTargetCube`, effect, `SpriteBatch`) plus a live "currently alive" count per type, backed by the `HEADLESS-18` registry | ✅ | Creation counters verified (`vertexBuffersCreated == 3` in `Headless_Smoke`); "live count per type" is available via `AliveResources()` (returns type name per record) but no test asserts on the per-type breakdown specifically, only the aggregate alive count. |
...
| # | Task | Status | Notes |
|---|---|---|---|
| HEADLESS-40 | Structured call log: every `IGraphicsBackend`-surface method call recorded (method name, a short argument summary, frame index, monotonic call index) into an in-memory buffer | 🟨 | `RecordTrace()`/`TraceLog()` infrastructure implemented and wired into most call sites (`SetData`, `Clear`, `Present`, `DrawColoredPrimitives`/`DrawIndexedColoredPrimitives`, resource creation), correctly gated to only run in `HeadlessTrace` mode (verified by code inspection: `RecordTrace()` early-returns unless `TraceEnabled()`). Not yet exercised by a dedicated test asserting on actual log contents — compiles and is reachable, verification is the remaining gap. Not every method call site has a `RecordTrace()` call yet (e.g. `ApplyBlendState`/state-change methods don't log), so "every method call" from the task's own wording is not fully met. |
| HEADLESS-41 | Creation-site tracking: every `Headless*Backend` resource created in `HeadlessTrace` mode also records `std::source_location` (or an explicit debug-label parameter, whichever is more useful in this codebase's call sites) so a leak report can point at the exact `new Texture2D(...)`/`VertexBuffer(...)` call responsible | ⬜ | The `creationSite` field exists on `HeadlessResourceRecord` and `NullResourceRegistry::Register()` accepts an optional site string, but no call site actually populates it yet (every `registry.Register(typeName)` call omits the second argument) — the plumbing exists, the actual capture (via `std::source_location` or a debug label threaded from the XNA layer) is not implemented. |
| HEADLESS-42 | Trace log export: dump the accumulated log to stdout or a file at end of run, in a format that's easy to diff between two CI runs | ⬜ | Not implemented — `TraceLog()` returns the in-memory vector, but nothing dumps it anywhere automatically. |
...
| HEADLESS-51 | Verify `SpriteBatch`, every stock `Effect` (`BasicEffect`, `AlphaTestEffect`, `DualTextureEffect`, `EnvironmentMapEffect`, `SkinnedEffect`), and `Model.Draw()` all route through the `HEADLESS` backend without requiring a display, a GPU, or throwing | 🟨 | `SpriteBatch` and `BasicEffect` verified via `Headless_Smoke`. `AlphaTestEffect`/`DualTextureEffect`/`EnvironmentMapEffect`/`SkinnedEffect`/`Model.Draw()` are not individually exercised by a test yet — they all route through the same `DrawPrimitivesEx`/`DrawIndexedPrimitivesEx` path already proven to work for `BasicEffect`, and nothing effect-specific in their `FillGpuDrawParams()` implementations should behave differently against this backend, but that's an inference from code reading, not a verified fact for each one individually. |
| HEADLESS-52 | `Mouse`/`Keyboard`/`GamePad` input backends: confirm (or, if needed, adjust) that input polling degrades gracefully with no real window to receive OS input events, rather than crashing on a null window handle | ⬜ | **Not audited.** `Headless_Smoke` never calls into `Mouse`/`Keyboard`/`GamePad` at all, so this remains an open, unverified risk for any game that polls input during a headless test run — flagged explicitly rather than silently assumed fine, per this plan's own Boundaries section. |
...
| HEADLESS-54 | `docs/`: document how/why to use the `HEADLESS` backend for CI game-logic tests, explicitly distinguishing it from the pixel-asserted tests the other backends use (`HEADLESS` proves "the game ran without crashing and did the right number of draws/state changes", not "the pixels are correct") | 🟨 | The `docs/graphics-backend-feature-matrix.md` note added for `HEADLESS-6` covers the "why" briefly; no dedicated how-to doc (env var usage, `CNA_HEADLESS_MODE`, `AssertNoLeaks()`/`GetStatistics()` API walkthrough) exists yet. |
...
| HEADLESS-61 | Draw-call/state-change counting tests: a known, fixed sequence of `Draw*`/`SetBlendState`/etc. calls produces the exact expected `HeadlessStatistics` values, both cumulative and per-frame | 🟨 | Cumulative counters verified exactly (`Headless_Smoke` Check C: drawCallCount/clearCount/presentCount/vertexBuffersCreated all exact expected values). Per-frame (`GetLastFrameStatistics()`) diffing is implemented but not asserted on by a test — see `HEADLESS-32`. |
...
| HEADLESS-63 | Mode-switching tests: the exact same invalid-argument call throws under `HeadlessValidation`/`HeadlessTrace` and does not throw under `HeadlessFast` | 🟨 | `HeadlessValidation` vs `HeadlessFast` verified (`Headless_Smoke` Check F). `HeadlessTrace` mode's validation behaviour (should match `HeadlessValidation`, per design decision 3 — `TraceEnabled()` implies `ValidationEnabled()`) is implemented but not separately tested. |
...
whole exercise — a genuine `Game::Run()` loop passes 10/10 checks with **no display server present
at all** (`DISPLAY`/`WAYLAND_DISPLAY` unset, `SDL_INIT_VIDEO` never called), in ~0.11s.
...
- **Disposed-object guards (`HEADLESS-19`/`24`)** don't map onto this codebase's actual ownership
  ...
  detection — the literal "disposed guard" framing was the part that didn't fit reality.
- **Viewport/scissor-vs-render-target validation (`HEADLESS-23`)** checks non-negative dimensions
  but not cross-referencing the bound target's actual size (needs plumbing not present in the
  current method signatures).
- **`HeadlessTrace` mode (Phase N5)** has its data-collection plumbing built and correctly
  mode-gated, but isn't exercised by any test yet, and creation-site tracking (`HEADLESS-41`) has
  an unused field rather than a real capture.
- Several resource backends (`TextureCube`, `Texture3D`, `RenderTarget2D`/`Cube`, custom `Effect`)
  are implemented and reachable but not yet individually exercised by a dedicated test — only
  `VertexBuffer`/`IndexBuffer`/`Texture2D`/`SpriteBatch`/`BasicEffect` are proven end-to-end so far.
