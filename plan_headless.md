...
at all** (`DISPLAY`/`WAYLAND_DISPLAY` unset, `SDL_INIT_VIDEO` never called), in ~0.11s. Since the
first commit: `Mouse`/`Keyboard`/`GamePad` are fully audited and safe with no window
...
verified (`HEADLESS-23`/`40`/`41`/`42`); and `../mobile-eggbert`, a real third-party game, builds
...
- **`HeadlessTrace`'s call log (`HEADLESS-40`)** doesn't yet cover every `IGraphicsBackend` method
  (state-change methods like `ApplyBlendState` aren't logged), so "every method call" from the
  task's own wording is met for the high-value call sites (draws, clears, resource creation,
  `SetData`), not literally everything.
