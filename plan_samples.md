...
| SAMPLE-32 | DistortionSample (032) | ⬜ | Blocked on custom shader (#11, Phase 78 Task 947) — **partial progress 2026-07-16**: 4 of 5 shader techniques the sample actually uses are ported and pixel-verified (`Distort.fx`'s `Distort` technique; `Distorters.fx`'s `DisplacementMapped`/`HeatHaze`/`PullIn`, its 3 actually-used techniques — `ZeroDisplacement` is unused, "provided for reference" per the shader's own comment). Still blocked on `Distort.fx`'s `DistortBlur` technique (same shader, 15-tap weighted-sum blur path — a separate, larger port, not attempted yet; needed since 2 of the sample's 3 distorters set `DistortionBlur=true`). |
...
| SAMPLE-39 | BillboardSample (039) | ⬜ | Blocked on custom shader (#11, Phase 78 Task 947). |
...
| SAMPLE-66 | ShipGame (066) | ⬜ | Blocked on custom shader (#11, Phase 78 Task 947). |
