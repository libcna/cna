...
| SAMPLE-33 | NonPhotoRealistic (033) | ⬜ | Blocked on custom shader (#11, Phase 78 Task 947) — **partial progress 2026-07-16**: all 3 techniques of `CartoonEffect.Fx` are ported and pixel-verified (`Lambert`/`Toon`/`NormalDepth`). Still blocked on `PostprocessEffect.Fx`'s 5 techniques (`EdgeDetect`/`EdgeDetectMonoSketch`/`EdgeDetectColorSketch`/`MonoSketch`/`ColorSketch`, all one parameterized pixel shader compiled 5 ways — edge detection off `NormalDepth`'s output + a pencil-sketch overlay effect — a separate, larger port, not attempted yet). |
