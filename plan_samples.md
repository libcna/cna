...
| SAMPLE-66 | ShipGame (066) | ⬜ | Blocked on custom shader (#11, Phase 78 Task 947) — **partial progress 2026-07-16**: has 4 distinct custom shaders (`AnimSprite.fx`, `NormalMapping.fx`, `Blur.fx`, `Particle.fx` — confirmed via `diff` that `NormalMapping.fx`/`Particle.fx` are NOT the same files already ported for `NormalMappingSample`/`Particles3DSample`). `AnimSprite.fx`, `Blur.fx`, and now `NormalMapping.fx` (all 3 techniques: `PlainMapping`/`NormalMapping`/`ViewMapping`, including its `TextureCube` reflection map via Task 1081) are ported and pixel-verified — 3 of 4 shaders done. Still blocked on `Particle.fx` (uses real GPU point sprites via `PSIZE` — needs point-sprite rendering support, not yet confirmed supported) — not attempted yet. |
