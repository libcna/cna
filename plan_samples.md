...
| SAMPLE-66 | ShipGame (066) | ⬜ | Blocked on custom shader (#11, Phase 78 Task 947) — **partial progress 2026-07-16**: has 4 distinct custom shaders (`AnimSprite.fx`, `NormalMapping.fx`, `Blur.fx`, `Particle.fx` — confirmed via `diff` that `NormalMapping.fx`/`Particle.fx` are NOT the same files already ported for `NormalMappingSample`/`Particles3DSample`). `AnimSprite.fx` (sprite-sheet frame cross-fade) and `Blur.fx` (5-technique box blur/colour/colour-texture) both ported and pixel-verified. Still blocked on `NormalMapping.fx` (3 techniques, cube-map reflection + specular/glow maps — needs `TextureCube` sampling in `ShaderEffect`, not yet confirmed supported) and `Particle.fx` (uses real GPU point sprites via `PSIZE` — needs point-sprite rendering support, not yet confirmed supported) — not attempted yet. |
