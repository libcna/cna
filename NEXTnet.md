...
a Blender-scripted pipeline) is the active initiative, not yet started** — the user decided CNA
should have real avatars, generated by Claude Code itself via original Blender Python scripts
(not by driving external GUI tools or cloning external character-creator repos, both of which hit
...
`plan_net.md` but **no implementation has started** — `tools/avatar_builder/` does not exist yet.
The next session's job is to begin Task 11.1.
...
Full detail for all of these is in `plan_net.md`'s Phase 11 section (Tasks 11.1–11.16). Smallest
first slice:

1. **Task 11.1 — `tools/avatar_builder/generate_skeleton.py`.**
   Goal: build and export (or at least construct in-memory and print/verify) a new, CNA-original
   canonical biped armature via `bpy` edit-mode bone creation (~19 bones: Hips, Spine, Spine1,
   Neck, Head, Shoulder.L/R, UpperArm.L/R, LowerArm.L/R, Hand.L/R, UpperLeg.L/R, LowerLeg.L/R,
   Foot.L/R). Document the exact bone list/hierarchy in `tools/avatar_builder/README.md`.
   Files: new `tools/avatar_builder/generate_skeleton.py`, `tools/avatar_builder/README.md`.
   Verify: `blender --background --python tools/avatar_builder/generate_skeleton.py` runs without
   error and (once Task 11.7's export step exists) the resulting armature has exactly the
   documented bone names/parents.

2. **Task 11.2 — `tools/avatar_builder/generate_body.py`.**
   Goal: procedural low-poly humanoid mesh, parented to the Task 11.1 armature with automatic
   weights.
   Files: new `tools/avatar_builder/generate_body.py`.
   Verify: run headless, confirm the mesh has vertex groups matching every bone name from Task
   11.1.

3. **Task 11.6 — `tools/avatar_builder/generate_animations.py`.**
...
4. **Task 11.7 — `tools/avatar_builder/export_gltf.py` + `generate_avatar.py`.**
   Goal: orchestrate Tasks 11.1/11.2/11.6 and export a single `male_avatar.glb`.
...
5. **Task 11.8 — `tools/avatar_builder/validate_gltf.py`.**
...
Task 11.1 (tools/avatar_builder/generate_skeleton.py does not exist yet - this is a from-scratch
first step). Do not refactor unrelated code. Do not re-attempt MakeHuman or CharMorph automation.
Make one small, verified improvement (Task 11.1 from plan_net.md's Phase 11), run the verification
...
Blender: blender --background --python tools/avatar_builder/generate_skeleton.py
```
