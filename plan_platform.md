...
| PLAT-19 | `IPlatform` root interface | ⬜ | Subsystem `Acquire`/`Release`/`IsInitialized` (**not** `Initialize`/`Shutdown` — PLAT-4 found global SDL lifetime belongs to the host application, and CNA production code never calls `SDL_Init`/`SDL_Quit`), `CreateWindow`, `PollEvents(std::vector<PlatformEvent>&)`, timing (`GetPerformanceCounter`/`GetPerformanceFrequency`/`GetTicksNs`/`Delay`), `GetCapabilities`, and accessors for each service interface below. Batch event API per design decision 4. Must satisfy all seven requirements in the PLAT-4 audit. |
...
| PLAT-29 | Subsystem lifecycle and refcounting | ⬜ | Implements PLAT-4's audited semantics exactly: lazy per-subsystem acquisition, tolerance of unpaired releases (`GraphicsDevice` releases video unconditionally; audio is pinned and never released — both deliberate), the preserved headless branch that skips video acquisition entirely, the process-wide mutex, and the pre-`SDL_Quit()` shutdown hook. **Must not** route subsystem calls through a main-thread dispatch — it deadlocks with no event pump running, which `Microsoft::Devices` supports. `DevicesShutdownOrderingTests`, `DevicesShutdownCoordinatorTests`, `SensorSubsystemOwnershipTests` and `VibrateControllerTests` must pass unchanged. |
