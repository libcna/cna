...
| Files referencing SDL (all) | **574** |
| Production files (`src/` + `include/`) referencing SDL | **252** |
...
| `modules/devices-ext` | 24 | clipboard, message box, file dialog, tray, camera, locale, power, display, URL |
| `modules/platform` | 18 | - |
...
| PLAT-78c | Carry IME candidate lists in the contract | ⬜ | **Total loss found by the PLAT-78 gap audit.** `ProcessEvent` handles `SDL_EVENT_TEXT_EDITING_CANDIDATES` (candidate list, count, selected index, horizontal flag) and feeds `TextInputEXT::INTERNAL_OnTextEditingCandidates`. There is **no `PlatformEvent` alternative for it and no case in `MapSdlEvent`**, so migrating as-is would silently make that entry point unreachable — IME candidate UI would simply stop appearing for CJK input. Needs a new event alternative *and* a mapper case. |
| PLAT-78d | Carry touch motion deltas | ⬜ | **Field gap found by the audit.** `TouchEvent` has `x`, `y`, `pressure` but no `dx`/`dy`; `ProcessEvent` reads `tfinger.dx/dy` on motion and passes them to `TouchPanel::INTERNAL_onTouchEvent`. Post-migration every motion would report a zero delta, changing gesture behaviour rather than breaking a build. |
| PLAT-78e | Reconcile the gamepad axis numerics | ⬜ | **Four silent divergences found by the audit**, all between the bridge and the mapper. **(1) The negative-half divisor**: the bridge divides by 32767 with a comment recording that 32768 "diverged from FNA at every non-endpoint negative sample" — and the mapper does exactly the rejected thing. **(2) Y inversion**: the bridge negates thumbstick Y for XNA's Y-up convention, the mapper does not, and `ControllerAxisEvent` has no field recording whether it was applied. **(3) Trigger range**: the bridge clamps triggers to [0,1], the mapper produces [-1,1] uniformly. **(4) Axis and button indices**: the bridge translates SDL's into CNA's `GamePadAxis`/`GamePadButton`, the mapper passes SDL's raw index through. None of these fail to compile; all of them move a player's stick. |
| PLAT-78f | Preserve the mouse-wheel flip convention | ⬜ | **Behaviour change found by the audit, in the opposite direction from a gap.** `ProcessEvent` never reads `wheel.direction`; the mapper *does*, and negates on `SDL_MOUSEWHEEL_FLIPPED`. So migrating would reverse scrolling on a flipped-wheel system. Decide which is correct and make both agree deliberately rather than by accident. |
| PLAT-78 | Migrate `SdlInputBridge` to `PlatformEvent` | ⛔ **blocked on PLAT-78c–f** | The bridge consumes `PlatformEvent` instead of `SDL_Event` and is renamed accordingly. Its golden tests (`SdlInputBridgeGoldenTests`) are the equivalence oracle and must pass unchanged in substance. |
...
| PLAT-77c | Gamepad capabilities `IPlatformGamepad` cannot express | ⬜ | **Gap found by PLAT-77.** `SdlGamepadBackend` has 33 methods against the contract's 5, and most of the difference is real capability, not redundancy: identity beyond a name (path, serial, firmware, Steam handle, connection state, vendor/product), touchpads (count, fingers, per-finger state), the pad's *own* motion sensors (distinct from `IPlatformSensors`, which are the device's), trigger rumble and LED, and player index get/set — which is how slot assignment survives a hotplug. Each is a capability a second implementation must be able to refuse, so each is added deliberately rather than smuggled into the migration commit. |
