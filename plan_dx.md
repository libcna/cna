...
> **Status: Phase DX1 in progress, authorized 2026-07-13.** `DX-1`/`DX-6` closed with real findings
> (see their rows); `DX-2`/`DX-3`/`DX-4` blocked on installing the `dxvk-wine64` Debian package,
> which needs an interactive `sudo` password this environment doesn't have — see those rows'
> Notes. **Phase DX2 onward (the actual `D3D11GraphicsBackend` CMake/skeleton work) is still NOT
> authorized** — per the project owner's own explicit instruction, do not proceed past Phase DX1
> without first reporting Phase DX1's full findings and getting a separate go-ahead. No
> `D3D11`/`D3D12` backend directory exists yet and `CNA_GRAPHICS_BACKEND` does not yet accept either
> value. **Direct3D 11 is the actual near-term target; Direct3D 12 is written up in full but
> deliberately deferred** (Phase DX12) — see "Why D3D11 first, D3D12 later" below for the reasoning,
> which came from the project owner directly.
>
...
| DX-2 | Install and configure a dedicated Wine prefix + DXVK for D3D11 testing (`WINEPREFIX=~/.wine-cna-d3d11`, DXVK's `d3d11.dll`/`dxgi.dll` installed into it), following the project owner's own researched steps | 🟨 | **Blocked 2026-07-13, needs a human with sudo.** Wine itself is already installed on this machine (`wine`/`wine64`/`libwine` packages, Wine 10.0). DXVK does **not** need a manual GitHub-release download here — Debian packages it directly (`apt-cache search dxvk` → `dxvk`, `dxvk-wine64`, `dxvk-wine32`), which is simpler than this plan's own original assumption. Blocked purely on `sudo apt-get install -y dxvk-wine64` requiring an interactive password (`sudo: a password is required`, no passwordless sudo in this sandboxed session) — not a technical or environment-suitability problem. Needs the project owner to either run `! sudo apt-get install -y dxvk-wine64` themselves or grant the needed access. |
| DX-3 | `scripts/run-wine-dxvk.sh` wrapper (mirrors the existing `scripts/run-all-backend-smoke-tests.sh` convention): sets `WINEPREFIX`, optional `DXVK_HUD`/`DXVK_LOG_LEVEL`, execs `wine "$1"` (**not** `wine64` — see Notes) | 🟨 | Blocked behind `DX-2`. One real environment difference already found while spiking `DX-1`/`DX-6` on this machine: **there is no separate `wine64` command** — this Debian's Wine 10.0 packaging merged the 32/64-bit launchers into a single `wine` binary that auto-detects PE32 vs. PE32+ (confirmed: `wine64: command not found`, but `wine spike.exe` ran the PE32+ binary correctly). The wrapper script should invoke `wine`, not hardcode `wine64` — a small, real correction to this plan's own original wording, worth carrying into the actual script and into `docs/d3d11-backend.md` (`DX-95`). |
| DX-4 | Prove the loop end-to-end with a minimal non-CNA smoke program: create a bare `ID3D11Device`+swap chain, clear to a known color, run under `wine` with DXVK installed, confirm no crash/error in `DXVK_LOG_LEVEL=info` output **and confirm DXVK is actually the thing that ran** (see Notes) | 🟨 | Blocked behind `DX-2`. **A real preview already exists**: `DX-1`'s own spike ran successfully under vanilla Wine (no DXVK) on this machine — `D3D11CreateDevice`, the full `IDXGIDevice`→`IDXGIFactory5` chain, and `CheckFeatureSupport`'s tearing query all genuinely succeeded via `WineD3D` (not DXVK). This is exactly the "ran under Wine but might be the wrong D3D implementation" case this task's own acceptance criterion warns about — proves the *toolchain/wine* path works, but is explicitly **not** sufficient to close `DX-4` (or `DX-85`) since it wasn't DXVK. Once `DX-2` unblocks, confirming DXVK involvement (not WineD3D) becomes the one remaining real check here. |
...
| # | Task | Status | Notes |
|---|---|---|---|
| DX-10 | Add `"D3D11"` (only) to `CNA_GRAPHICS_BACKEND`'s CMake `STRINGS` property and a matching `CNA_BACKEND_D3D11` option flag, following the exact existing pattern (`CMakeLists.txt` lines ~94–139) | ⬜ | `D3D12` is deliberately **not** added here — see this phase's own intro and design decision 9. |
| DX-11 | `FATAL_ERROR` guard: reject `CNA_GRAPHICS_BACKEND` = `D3D11` when `CMAKE_SYSTEM_NAME` is not `Windows`, with a message pointing at `cmake/toolchains/mingw-w64.cmake` (design decision 2) | ⬜ | Place near the existing `BGFX`-platform-`WARNING` check (`CMakeLists.txt` line ~161) for discoverability, but as `FATAL_ERROR` not `WARNING` — this truly cannot build elsewhere. `DX-101` extends this same guard to also cover `D3D12` when that phase starts, rather than writing a `D3D12` branch here in advance. |
| DX-12 | `cna_backend_graphics_d3dcommon` static library target + `cna_backend_graphics_d3d11` target (`elseif(CNA_GRAPHICS_BACKEND STREQUAL "D3D11")` block, mirroring every existing backend's own block) | ⬜ | Link only `DX-1`'s confirmed minimum set (design decision 3) — expected `target_link_libraries(cna_backend_graphics_d3d11 PUBLIC cna_backend_graphics_d3dcommon d3d11 dxgi)`, adding `dxguid` only if `DX-1` actually found it necessary. `d3dcompiler` is deliberately **not** on this list — it's linked only where `D3DCompile()` is actually called (the shader-compile tool from `DX-14-compile`, and/or `DX-58`'s runtime custom-effect path), so a plain `cna_backend_graphics_d3d11` build carries no `d3dcompiler_47.dll` runtime dependency. No `find_package`/`FetchContent` needed either way. No `cna_backend_graphics_d3d12` target here — `D3DCommon` itself is written to be reusable by a future D3D12 target (design decision 4), but that target isn't created until `DX-101`. |
...
| DX-14 | `CreateGraphicsBackend()` factory dispatch for `D3D11` | ⬜ | No `D3D12` placeholder/throwing branch here — `CNA_GRAPHICS_BACKEND=D3D12` isn't even a recognized `STRINGS` value yet (`DX-10`'s own scoping), so there is nothing for this dispatch to guard against until `DX-101`. |
...
| DX-26 | Real `Present()`: sync interval and tearing are **backend state**, not a direct D3D11 "set swap interval" API — there is no such entry point. `SetSwapInterval`/`SetPresentationMode` just update stored fields (`vsyncEnabled_`; `allowTearingRequested_`, design decision 13; `exclusiveFullscreen_` from whatever presentation-mode plumbing already tracks fullscreen state); `Present()` itself computes `const bool mayTear = allowTearingSupported_ && allowTearingRequested_ && !vsyncEnabled_ && !exclusiveFullscreen_;`, `UINT syncInterval = vsyncEnabled_ ? 1 : 0`, `UINT flags = mayTear ? DXGI_PRESENT_ALLOW_TEARING : 0`, then calls `swapChain->Present(syncInterval, flags)` | ⬜ | `DXGI_PRESENT_ALLOW_TEARING` is only valid for windowed/borderless presentation, never classic exclusive fullscreen — `mayTear` must exclude that case explicitly, not just check the tearing-support capability bit, or `Present()` can return an error/undefined result while in exclusive fullscreen. `allowTearingRequested_` (policy, design decision 13) keeps a tearing-capable swap chain from becoming the default just because the hardware happens to support it. Corrects this plan's own earlier, inaccurate framing of `SetSwapInterval` as something mapped directly onto a present-time call — it's a state write now, applied every `Present()`. |
| DX-27 | Device-lost/removed **detection**, starting here in Phase DX4 — not deferred to `DX-90`: check the `HRESULT` returned by `Present()` (and any other call that can surface it) for `DXGI_ERROR_DEVICE_REMOVED`/`DXGI_ERROR_DEVICE_RESET`; on either, call `device->GetDeviceRemovedReason()` and log/report the reason | ⬜ | Detection/diagnostics only at this stage — full automatic recovery recreates all three lifetime groups (device, swap chain, window-size views — design decision 11), which is real, additional work, legitimately deferred, but every later phase's draw/present call sites should already be able to surface "the device is gone" instead of silently misbehaving or crashing. `DX-90` verifies full recovery against a *real* device-removal scenario on real hardware/driver, which Wine+DXVK cannot manufacture convincingly. |
| DX-28 | `ReadBackbuffer()`/`GetBackBufferData()`: real GPU→CPU readback via a staging `ID3D11Texture2D` (`D3D11_USAGE_STAGING` + `CopyResource` + `Map`) — this backend's first genuine pixel-correctness proof, same bar `SOFTWARE-13` set. **Must read via `D3D11_MAPPED_SUBRESOURCE::RowPitch`, one row at a time** — never assume the mapped rows are tightly packed (`RowPitch` can exceed `width * bytesPerPixel` due to driver-side row alignment) | ⬜ | First real pixel test candidate (Phase DX10): clear to a known color, read back, assert exact match. A test that only reads pixel (0,0) would not catch a `RowPitch` bug — make sure at least one readback test samples a pixel past the first row. |
| DX-29 | Window resize handling — touches only the window-size-view group plus a `ResizeBuffers` call on the existing swap chain (design decision 11), nothing else: (1) unbind the current RTV/DSV (`OMSetRenderTargets(0, nullptr, nullptr)`); (2) release **every** reference to the old back buffer — `DX-24`'s RTV, the back-buffer texture reference, depth texture, DSV, **and** any SRV or staging/readback-cache resource (`DX-28`) that was ever bound off the old back buffer; (3) `context->Flush()` to help ensure the immediate context isn't still holding an implicit reference before resizing; (4) call `swapChain->ResizeBuffers(...)` on `DX-23`'s **existing** swap-chain object; (5) recreate `DX-24`'s RTV/DSV/viewport sized to the new dimensions | ⬜ | `DX-20`/`DX-21`'s device/context/shader/state caches and `DX-22`'s factory chain/`allowTearingSupported_` are untouched — a plain resize must not re-run the `IDXGIFactory5::CheckFeatureSupport` tearing query (device-lifetime, one-time work) and must not destroy-and-recreate the `IDXGISwapChain1` object (`DX-23`'s own instance is reused via `ResizeBuffers`, never `CreateSwapChainForHwnd` again). `ResizeBuffers` fails (`DXGI_ERROR_INVALID_CALL`) if *any* outstanding reference to a back-buffer-derived resource survives step (2) — a stale readback staging texture or cached SRV is a real, easy way to trip this. |
