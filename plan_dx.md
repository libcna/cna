...
> **Status: Phase DX1 fully closed 2026-07-13** (`DX-1`–`DX-6a`, all ✅ with real, verified findings
> on this actual machine — see each row). Headline result: a real DXBC-independent D3D11 device was
> created **through DXVK 2.6.0 on a real GPU** (`AMD Radeon 780M`, RADV driver, feature level
> negotiated to `11_1`) via `wine` + `scripts/run-wine-dxvk.sh`, not a simulated/assumed result — the
> DXVK log genuinely identifies DXVK (not `WineD3D`) and a real hardware adapter (`llvmpipe`
> explicitly skipped). Confirmed minimum link set is `d3d11`+`dxgi` only (no `dxguid`), COM ownership
> is `Microsoft::WRL::ComPtr<T>` (works cleanly, no project-local type needed), and DXVK installs via
> the Debian package `dxvk-wine64` + its bundled `dxvk-setup` tool rather than a manual GitHub-release
> download — see `programs.md` §10 for the exact commands. **Phase DX2 onward (the actual
> `D3D11GraphicsBackend` CMake/skeleton work) is still NOT authorized** — per the project owner's own
> explicit instruction, do not proceed past Phase DX1 without a separate go-ahead. No `D3D11`/`D3D12`
> backend directory exists yet and `CNA_GRAPHICS_BACKEND` does not yet accept either value.
> **Direct3D 11 is the actual near-term target; Direct3D 12 is written up in full but deliberately
> deferred** (Phase DX12) — see "Why D3D11 first, D3D12 later" below for the reasoning, which came
> from the project owner directly.
>
