...
| DX-69 | Fog (`GpuDrawParams::fogEnabled`/`fogColor`/`fogStart`/`fogEnd`) added to every 3D variant above, matching the Vulkan `FogParams` precedent (`plan_graphics.md` Task 899) | 🟨 | **Partially closed 2026-07-13.** `colored3d`/`textured3d`/`colored_textured3d` (via `D3DFogConstants`, `DX-62`), `lit_textured3d` (fog fields folded into `D3DLightingConstants`, `DX-63`), and `alpha_test3d` (fog folded directly into `D3DAlphaTestConstants`, `DX-64`) all wire real fog data through — every HLSL shader already computed its own fog factor/lerp when `DX-13-hlsl` ported it, this task just confirms the C++ side actually populates those fields from `GpuDrawParams` instead of leaving them zeroed, which `DrawPrimitivesExImpl` now does for all 5 variants above. **Not yet exercised by a dedicated fog-on/fog-off pixel test** (Check Q/R/S all draw with `fogEnabled` left at its default `false`) — a real, honest gap, not a false completeness claim; a small follow-up check belongs in `Phase DX10`. **Still fully open** for `dual_texture3d`/`env_map3d`/`skinned3d`/`sprite2d`/`instanced3d` — those variants' own draw-call wiring (`DX-65`/`DX-66`/`DX-67`/`DX-68`) hasn't landed yet, so there's nothing to wire fog into there yet. |
