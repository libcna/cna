...
| DX-6a | If a project-local `CNA::ComPtr<T>` is built (`DX-6`), add dedicated unit tests using a fake `IUnknown`-like test double with `AddRef`/`Release` call counters — no real D3D device needed: destruction calls `Release` exactly once; move construction/assignment leaves the source null (and calls neither `AddRef` nor `Release`); `Attach()` does **not** call `AddRef`; `Detach()` does **not** call `Release`; `Reset()` releases the previously-held object; `ReleaseAndGetAddressOf()` releases the previously-held object before returning the address (the entire point of it existing, per `DX-6`'s own Notes); `As<U>()` propagates a failed `QueryInterface` `HRESULT` and leaves its output null | ⬜ | A cheap, fast, no-GPU-needed test suite — lives under `CnaTests`, buildable on the Windows cross-target without an actual `ID3D11Device`. Directly guards against the `GetAddressOf()`-vs-`ReleaseAndGetAddressOf()` leak class `DX-6`'s Notes call out as the real risk — skip entirely if `DX-6` ends up choosing `Microsoft::WRL::ComPtr` instead (already tested upstream). |
