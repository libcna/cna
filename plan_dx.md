...
> `src/CNA/Internal/Backends/D3DCommon/shaders/*.hlsl`, hand-reviewed against the GLSL, **not yet
> compiler-verified** (no `D3DCompile()`/`fxc`/`dxc` run against them — that's `DX-14-compile`'s
> job). Matrix convention: `row_major` cbuffer matrices + `mul(v, M)` everywhere GLSL reads `M * v`
> (standard XNA/HLSL idiom). Two genuine HLSL-vs-GLSL deviations found and handled: a hand-written
...
> see that row's own notes). `DX-14-compile`/`DX-15-embed` are the concrete next step. Authorized
> 2026-07-13 by the project owner to continue autonomously through Phase DX3 (remainder) → DX5 →
> DX6 → DX7 → DX8 → DX9 → DX10 → DX11, and Phase DX12 (D3D12) afterward if time/context allow —
> this is no longer gated per-phase the way the banner text below used to require; `DX-90`'s
> real-Windows checklist stays explicitly `needs_human` (no such machine available here).
