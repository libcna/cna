...
> **`DX-111` is now 9 of 10 stock variants real** — only `env_map3d` remains, genuinely blocked
> behind a new `D3D12TextureCubeBackend` (no D3D12 cube-texture support exists yet), a real, scoped
> follow-up task; this row stays 🟨, not ✅, until that lands. **The next open Phase DX12 step is
> either `env_map3d` (closing `DX-111` to a full ✅) or `DX-113` (test-suite formalization) —
> `DX-112` (SpriteBatch) is unaffected, already closed against `sprite2d`.**
>
