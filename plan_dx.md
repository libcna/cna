...
> built on. **Phase DX3 onward (shared `D3DCommon` core, then vertex/index buffers, textures,
> shaders/effects, SpriteBatch) is NOT yet authorized** — this session's go-ahead covered Phase
> DX1/DX2/DX4's core + `DX-80` specifically, not the whole plan; get an explicit go-ahead before
> continuing further. **Direct3D 11 is the actual near-term target; Direct3D 12 is written up in
> full but deliberately deferred** (Phase DX12) — see "Why D3D11 first, D3D12 later" below.
>
