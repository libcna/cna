...
> manual-regen convention (no CMake build-target wiring, matching the SPIR-V precedent). `DX-15-embed`
> (wiring these bytes into `CreateVertexShader`/`CreatePixelShader`) is the concrete next step.
> Authorized 2026-07-13 by the project owner to continue autonomously through Phase DX3 (remainder)
> → DX5 → DX6 → DX7 → DX8 → DX9 → DX10 → DX11, and Phase DX12 (D3D12) afterward if time/context
> allow — this is no longer gated per-phase the way the banner text below used to require; `DX-90`'s
